...
void main()
{
	...
	ivec3 gridCoord;
	vec3 fract;
	vec3 lightOrigin = globalPosition - lightGridOrigin;
	for( int i = 0; i < 3; i++ )
	{
		float v;
		v = max( 0, lightOrigin[i] * ( 1.0 / lightGridSize[i] ) );
		gridCoord[i] = int( floor( v ) );
		fract[ i ] = v - gridCoord[ i ];
	...
	for( int i = 0; i < 8; i++ )
	{
		...
		#if 1
		vec2 octCoordNormalizedToTextureDimensions = ( normalizedOctCoordZeroOne + atlasOffset ) * rpScreenCorrectionFactor.w;
		...
		vec2 atlasCoord = normalizedProbeTopLeftPosition + octCoordNormalizedToTextureDimensions;
		#else 
		vec2 atlasCoord = normalizedOctCoordZeroOne + atlasOffset;
		#endif 
		vec3 color = texture( samp7, atlasCoord, 0 ).rgb;
	...
	normalizedOctCoordZeroOne = ( normalizedOctCoord + vec2( 1.0 ) ) * 0.5;
