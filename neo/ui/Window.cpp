...
idCVar hud_titlesafe(
    "hud_titlesafe", "0.0", CVAR_GUI | CVAR_FLOAT,
...
================
*/
void idWindow::SetFont() { dc->SetFont(font); }
...
================
*/
void idWindow::AddUpdateVar(idWinVar* var) { updateVars.AddUnique(var); }
...
const char* idWindow::HandleEvent(const sysEvent_t* event,
  ...
  if (visible && !noEvents) {
    if (event->evType == SE_KEY) {
      ...
      } else if ((event->evValue == K_ESCAPE || event->evValue == K_JOY9) &&
                 event->evValue2) {
        if (GetFocusedChild()) {
          const char* childRet =
              GetFocusedChild()->HandleEvent(event, updateVisuals);
...
void idWindow::Redraw(float x, float y, bool hud) {
  ...
  if (hud) {
    ...
    dc->SetSize(forceAspectWidth * tileSafeScale,
                forceAspectHeight * tileSafeScale);
    dc->SetOffset(forceAspectWidth * tileSafeOffset,
                  forceAspectHeight * tileSafeOffset);
  } else {
...
================
*/
void idWindow::LoseCapture() { flags &= ~WIN_CAPTURE; }
...
================
*/
void idWindow::SetFlag(unsigned int f) { flags |= f; }
...
================
*/
void idWindow::ClearFlag(unsigned int f) { flags &= ~f; }
...
================
*/
void idWindow::SetParent(idWindow* w) { parent = w; }
...
================
*/
bool idWindow::ParseScript(idTokenParser* src, idGuiScriptList& list,
                           int* timeParm, bool elseBlock) {
...
void idWindow::SaveExpressionParseState() {
  saveTemps =
      (bool*)Mem_Alloc(MAX_EXPRESSION_REGISTERS * sizeof(bool), TAG_CRAP);
...
================
*/
idWinVar* idWindow::GetWinVarByName(const char* _name, bool fixup,
                                    drawWin_t** owner) {
...
================
*/
idStr* idWindow::GetStrPtrByName(const char* _name) { return NULL; }
...
================
*/
void idWindow::AddTransition(idWinVar* dest, idVec4 from, idVec4 to, int time,
...
================
*/
void idWindow::StartTransition() { flags |= WIN_INTRANSITION; }
...
intptr_t idWindow::EmitOp(intptr_t a, intptr_t b, wexpOpType_t opType,
                          wexpOp_t** opp) {
  // DG end
  wexpOp_t* op;
  /*
...
*/
// DG: a, b and the return value are really pointers, so use intptr_t
intptr_t idWindow::ParseEmitOp(idTokenParser* src, intptr_t a,
                               wexpOpType_t opType, int priority,
                               wexpOp_t** opp) {
...
intptr_t idWindow::ParseTerm(idTokenParser* src, idWinVar* var,
                             intptr_t component) {
  // DG end
  idToken token;
...
intptr_t idWindow::ParseExpressionPriority(idTokenParser* src, int priority,
                                           idWinVar* var, intptr_t component) {
  idToken token;
  intptr_t a;
...
*/
// DG: component and the return value are really pointers, so use intptr_t
intptr_t idWindow::ParseExpression(idTokenParser* src, idWinVar* var,
                                   intptr_t component) {
  // DG end
      ...
      if (dw->win != NULL) {
        if (transitions[i].offset == (ptrdiff_t)&rect - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->rect;
                   ...
                   (ptrdiff_t)&backColor - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->backColor;
        } else if (transitions[i].offset ==
                   (ptrdiff_t)&matColor - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->matColor;
        } else if (transitions[i].offset ==
                   (ptrdiff_t)&foreColor - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->foreColor;
        } else if (transitions[i].offset ==
                   (ptrdiff_t)&borderColor - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->borderColor;
        } else if (transitions[i].offset ==
                   (ptrdiff_t)&textScale - (ptrdiff_t)this) {
          transitions[i].data = &dw->win->textScale;
        } else if (transitions[i].offset ==
                   (ptrdiff_t)&rotate - (ptrdiff_t)this) {
...
===============
*/
void idWindow::AddChild(idWindow* win) { win->childID = children.Append(win); }
...
================
*/
void idWindow::SetChildWinVarVal(const char* name, const char* var,
                                 const char* val) {
...
================
*/
int idWindow::GetChildCount() { return drawWindows.Num(); }
...
================
*/
idWindow* idWindow::GetChild(int index) { return drawWindows[index].win; }
