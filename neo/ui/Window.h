...
class idWindow {
 public:
  ...
  void AddTransition(idWinVar* dest, idVec4 from, idVec4 to, int time,
  ...
  bool ParseScript(idTokenParser* src, idGuiScriptList& list,
                   int* timeParm = NULL, bool allowIf = false);
  ...
  intptr_t ParseExpression(idTokenParser* src, idWinVar* var = NULL,
                           intptr_t component = 0);
  // DG end
 ...
 protected:
  ...
  // DG: a, b, component and the return values are really pointers, so use
  // intptr_t
  intptr_t EmitOp(intptr_t a, intptr_t b, wexpOpType_t opType,
                  wexpOp_t** opp = NULL);
  intptr_t ParseEmitOp(idTokenParser* src, intptr_t a, wexpOpType_t opType,
                       int priority, wexpOp_t** opp = NULL);
  intptr_t ParseTerm(idTokenParser* src, idWinVar* var = NULL,
                     intptr_t component = 0);
  intptr_t ParseExpressionPriority(idTokenParser* src, int priority,
                                   idWinVar* var = NULL,
                                   intptr_t component = 0);
  // DG end
  ...
  void ConvertRegEntry(const char* name, idTokenParser* src, idStr& out,
                       int tabs);
  ...
  idList<wexpOp_t, TAG_OLD_UI>*
      saveOps;  // evaluate to make expressionRegisters
