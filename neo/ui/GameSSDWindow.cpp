...
SSDEntity::SSDEntity() { EntityInit(); }
...
void SSDEntity::SetGame(idGameSSDWindow* _game) { game = _game; }
...
void SSDEntity::SetPosition(const idVec3& _position) { position = _position; }

void SSDEntity::SetSize(const idVec2& _size) { size = _size; }
...
void SSDEntity::SetRotation(float _rotation) { rotation = _rotation; }
...
void SSDEntity::DestroyEntity() { inUse = false; }
...
void SSDAsteroid::Init(idGameSSDWindow* _game, const idVec3& startPosition,
                       const idVec2& _size, float _speed, float rotate,
                       int _health) {
  EntityInit();
...
void SSDAsteroid::EntityUpdate() { SSDMover::EntityUpdate(); }

SSDAsteroid* SSDAsteroid::GetNewAsteroid(idGameSSDWindow* _game,
                                         const idVec3& startPosition,
                                         const idVec2& _size, float _speed,
                                         float rotate, int _health) {
...
void SSDAstronaut::Init(idGameSSDWindow* _game, const idVec3& startPosition,
                        float _speed, float rotate, int _health) {
  EntityInit();
  MoverInit(idVec3(0, 0, -_speed), rotate);
...
SSDExplosion::SSDExplosion() { type = SSD_ENTITY_EXPLOSION; }
...
void SSDExplosion::Init(idGameSSDWindow* _game, const idVec3& _position,
                        const idVec2& _size, int _length, int _type,
                        SSDEntity* _buddy, bool _killBuddy, bool _followBuddy) {
  EntityInit();
...
SSDExplosion* SSDExplosion::GetNewExplosion(idGameSSDWindow* _game,
                                            ...
                                            const idVec2& _size, int _length,
                                            int _type, SSDEntity* _buddy,
                                            bool _killBuddy,
                                            bool _followBuddy) {
...
SSDPoints::SSDPoints() { type = SSD_ENTITY_POINTS; }
...
void SSDPoints::Init(idGameSSDWindow* _game, SSDEntity* _ent, int _points,
                     int _length, int _distance, const idVec4& color) {
  EntityInit();
...
SSDPoints* SSDPoints::GetNewPoints(idGameSSDWindow* _game, SSDEntity* _ent,
                                   int _points, int _length, int _distance,
                                   const idVec4& color) {
...
SSDPoints* SSDPoints::GetSpecificPoints(int id) { return &pointsPool[id]; }
...
SSDProjectile::SSDProjectile() { type = SSD_ENTITY_PROJECTILE; }
...
void SSDProjectile::Init(idGameSSDWindow* _game, const idVec3& _beginPosition,
                         const idVec3& _endPosition, float _speed,
                         float _size) {
  EntityInit();
...
SSDPowerup* SSDPowerup::GetNewPowerup(idGameSSDWindow* _game, float _speed,
                                      float _rotation) {
...
SSDPowerup* SSDPowerup::GetSpecificPowerup(int id) { return &powerupPool[id]; }
...
idGameSSDWindow::~idGameSSDWindow() { ResetGameStats(); }
...
idWinVar* idGameSSDWindow::GetWinVarByName(const char* _name, bool winLookup,
                                           drawWin_t** owner) {
...
void idGameSSDWindow::StartGame() { gameStats.gameRunning = true; }

void idGameSSDWindow::StopGame() { gameStats.gameRunning = false; }
...
void idGameSSDWindow::FireWeapon(int key) {
  ...
  if (key == K_MOUSE1) {
    ...
    if (gameStats.levelStats.targetEnt) {
      ...
      // SSDProjectile* newProj = new (TAG_OLD_UI) SSDProjectile(this,
      // idVec3(320,0,0), gameStats.levelStats.targetEnt->position,
...
void idGameSSDWindow::OnSuperBlaster() { StartSuperBlaster(); }
