...
// These used to be in every window, but they all pointed at the same one in
// idUserInterfaceManagerLocal. Made a global so it can be switched out
// dynamically to test optimized versions.
idDeviceContext* dc;

idCVar g_useNewGuiCode(
    "g_useNewGuiCode", "1", CVAR_GAME | CVAR_INTEGER,
    "use optimized device context code, 2 = toggle on/off every frame");
...
void idUserInterfaceManagerLocal::EndLevelLoad(const char* mapName) {
  ...
  if (cvarSystem->GetCVarBool("fs_buildresources") && mapName != NULL &&
      mapName[0] != '\0') {
...
const char* idUserInterfaceLocal::Name() const { return source; }
...
bool idUserInterfaceLocal::IsInteractive() const { return interactive; }
...
const char* idUserInterfaceLocal::GetStateString(
    const char* varName, const char* defaultString) const {
