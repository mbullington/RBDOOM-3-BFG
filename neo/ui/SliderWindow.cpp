...
idWinVar* idSliderWindow::GetWinVarByName(const char* _name, bool fixup,
                                          drawWin_t** owner) {
...
const char* idSliderWindow::HandleEvent(const sysEvent_t* event,
  ...
  if (key == K_RIGHTARROW || key == K_KP_6 ||
      (key == K_MOUSE2 && gui->CursorY() > thumbRect.y)) {
    value = value + stepSize;
  }

  if (key == K_LEFTARROW || key == K_KP_4 ||
      (key == K_MOUSE2 && gui->CursorY() < thumbRect.y)) {
    value = value - stepSize;
  }
...
void idSliderWindow::SetBuddy(idWindow* buddy) { buddyWin = buddy; }
...
void idSliderWindow::InitWithDefaults(
    const char* _name, const idRectangle& _rect, const idVec4& _foreColor,
    const idVec4& _matColor, const char* _background, const char* thumbShader,
    bool _vertical, bool _scrollbar) {
...
void idSliderWindow::SetValue(float _value) { value = _value; }
