...
idRenderWindow::~idRenderWindow() { renderSystem->FreeRenderWorld(world); }
...
void idRenderWindow::BuildAnimation(int time) {
  ...
  if (animName.Length() && animClass.Length()) {
    ...
    worldEntity.joints = (idJointMat*)Mem_Alloc16(
        SIMD_ROUND_JOINTS(worldEntity.numJoints) * sizeof(*worldEntity.joints),
        TAG_JOINTMAT);
...
void idRenderWindow::PostParse() { idWindow::PostParse(); }

//
//
idWinVar* idRenderWindow::GetWinVarByName(const char* _name, bool fixup,
                                          drawWin_t** owner) {
  //
