...
class idStreamingVoiceContext : public IXAudio2VoiceCallback {
 public:
  ...
  STDMETHOD_(void, OnBufferStart)(void* pContext) {
    idSoundSystemLocal::bufferContext_t* bufferContext =
        (idSoundSystemLocal::bufferContext_t*)pContext;
    bufferContext->voice->OnBufferStart(bufferContext->sample,
                                        bufferContext->bufferNumber);
  }
...
========================
*/
void idSoundVoice_XAudio2::Create(const idSoundSample* leadinSample_,
  ...
  if (pSourceVoice != NULL && CompatibleFormat(leadinSample)) {
    ...
    soundSystemLocal.hardware.pXAudio2->CreateSourceVoice(
        &pSourceVoice, (const WAVEFORMATEX*)&leadinSample->format,
...
========================
*/
int idSoundVoice_XAudio2::SubmitBuffer(idSoundSample_XAudio2* sample,
  ...
  idSoundSystemLocal::bufferContext_t* bufferContext =
      soundSystemLocal.ObtainStreamBufferContext();
  ...
  if (offset > 0) {
    ...
    buffer.PlayLength =
        sample->buffers[bufferNumber].numSamples - previousNumSamples - offset;
  }
...
bool idSoundVoice_XAudio2::Update() {
  ...
  pSourceVoice->SetOutputMatrix(soundSystemLocal.hardware.pMasterVoice,
                                srcChannels, dstChannels, pLevelMatrix,
                                OPERATION_SET);
  ...
  // the soundworld is paused or unpaused UnPause();
  return true;
}
...
========================
*/
void idSoundVoice_XAudio2::SetSampleRate(uint32 newSampleRate,
                                         uint32 operationSet) {
  if (pSourceVoice == NULL || leadinSample == NULL) {
    return;
  }
  ...
  if (cutoffFrequency * 6.0f >= (float)sampleRate) {
    ...
    filter.Frequency =
        2.0f * idMath::Sin(idMath::PI * cutoffFrequency / (float)sampleRate);
  }
  assert(filter.Frequency >= 0.0f &&
         filter.Frequency <= XAUDIO2_MAX_FILTER_FREQUENCY);
  filter.Frequency =
      idMath::ClampFloat(0.0f, XAUDIO2_MAX_FILTER_FREQUENCY, filter.Frequency);

  pSourceVoice->SetFilterParameters(&filter, operationSet);

  float freqRatio = pitch * (float)sampleRate / (float)sourceVoiceRate;
  assert(freqRatio >= XAUDIO2_MIN_FREQ_RATIO &&
         freqRatio <= XAUDIO2_MAX_FREQ_RATIO);
  freqRatio = idMath::ClampFloat(XAUDIO2_MIN_FREQ_RATIO, XAUDIO2_MAX_FREQ_RATIO,
                                 freqRatio);

  // if the value specified for maxFreqRatio is too high for the specified
  // format, the call to CreateSourceVoice will fail
  if (numChannels == 1) {
...
========================
*/
void idSoundVoice_XAudio2::OnBufferStart(idSoundSample_XAudio2* sample,
                                         int bufferNumber) {
