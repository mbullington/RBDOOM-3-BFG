...
idCVar s_showLevelMeter("s_showLevelMeter", "0", CVAR_BOOL | CVAR_ARCHIVE,
                        "Show VU meter");
idCVar s_meterTopTime(
    "s_meterTopTime", "1000", CVAR_INTEGER | CVAR_ARCHIVE,
    "How long (in milliseconds) peaks are displayed on the VU meter");
idCVar s_meterPosition("s_meterPosition", "100 100 20 200", CVAR_ARCHIVE,
                       "VU meter location (x y w h)");
idCVar s_device(
    "s_device", "-1", CVAR_INTEGER | CVAR_ARCHIVE,
    "Which audio device to use (listDevices to list, -1 for default)");
idCVar s_showPerfData("s_showPerfData", "0", CVAR_BOOL,
                      "Show XAudio2 Performance data");
extern idCVar s_volume_dB;

#if defined(USE_WINRT)
HRESULT GetAudioDeviceDetails(_In_ IMMDevice* immDevice,
                              _Out_ AudioDevice* pInfo) {
...
std::vector<AudioDevice> EnumerateAudioDevices(
    _Out_opt_ AudioDevice* defaultDevice = nullptr) {
  ...
  HRESULT hResult =
      CoCreateInstance(__uuidof(MMDeviceEnumerator), NULL, CLSCTX_ALL,
                       __uuidof(IMMDeviceEnumerator), (void**)&immDevEnum);
  ...
  if (defaultDevice != nullptr) {
    ...
    // @pjb: get the default audio endpoint and make it the first one in the
    // list
    if (SUCCEEDED(immDevEnum->GetDefaultAudioEndpoint(eRender, eConsole,
                                                      &defaultImmDev))) {
  ...
  hResult = immDevEnum->EnumAudioEndpoints(eRender, DEVICE_STATE_ACTIVE,
                                           &immDevCollection);
  if (FAILED(hResult)) {
...
// RB: not available on Windows 8 SDK
  ...
  for (size_t i = 0; i < vAudioDevices.size(); ++i) {
    idLib::Printf("%s %3d: %S %S\n",
                  vAudioDevices[i].id == defaultDevice.id ? "*" : " ", i,
                  vAudioDevices[i].name.c_str(), vAudioDevices[i].id.c_str());
  }
#else
  UINT32 deviceCount = 0;
  ...
  for (unsigned int i = 0; i < deviceCount; i++) {
    ...
    if (deviceDetails.OutputFormat.dwChannelMask &
    ...
    wcstombs(mbcsDisplayName, deviceDetails.DisplayName,
             sizeof(mbcsDisplayName));
    idLib::Printf("%3d: %s\n", i, mbcsDisplayName);
    ...
    if (channelNames.Num() != deviceDetails.OutputFormat.Format.nChannels) {
      idLib::Printf(S_COLOR_YELLOW
                    "WARNING: " S_COLOR_RED
                    "Mismatch between # of channels and channel mask\n");
    }
...
void idSoundHardware_XAudio2::Init() {
  cmdSystem->AddCommand("listDevices", listDevices_f, 0,
                        "Lists the connected sound devices", NULL);

  DWORD xAudioCreateFlags = 0;

// RB: not available on Windows 8 SDK
#if !defined(USE_WINRT) && \
    defined(_DEBUG)  // (_WIN32_WINNT < 0x0602 /*_WIN32_WINNT_WIN8*/) &&
                     // defined(_DEBUG)
  ...
  if (FAILED(XAudio2Create(&pXAudio2, xAudioCreateFlags, xAudioProcessor))) {
    ...
    if (xAudioCreateFlags & XAUDIO2_DEBUG_ENGINE) {
      ...
      xAudioCreateFlags &= ~XAUDIO2_DEBUG_ENGINE;
      if (FAILED(
              XAudio2Create(&pXAudio2, xAudioCreateFlags, xAudioProcessor))) {
        idLib::FatalError(
            "Failed to create XAudio2 engine.  Try installing the latest "
            "DirectX.");
        return;
      }
    } else
  ...
  DWORD outputSampleRate =
      44100;  // Max( (DWORD)XAUDIO2FX_REVERB_MIN_FRAMERATE, Min(
              // (DWORD)XAUDIO2FX_REVERB_MAX_FRAMERATE,
              // deviceDetails.OutputFormat.Format.nSamplesPerSec ) );
  ...
  if (!vAudioDevices.empty()) {
    ...
    bool validPreference =
        (preferredDevice >= 0 && preferredDevice < (int)vAudioDevices.size());
    ...
    if (SUCCEEDED(pXAudio2->CreateMasteringVoice(
            &pMasterVoice, XAUDIO2_DEFAULT_CHANNELS, outputSampleRate, 0,
            selectedDevice.id.c_str(), NULL, AudioCategory_GameEffects))) {
  ...
  if (preferredDevice < 0 || preferredDevice >= (int)deviceCount) {
    ...
    for (unsigned int i = 0; i < deviceCount; i++) {
      ...
      if (deviceDetails.Role & DefaultGameDevice) {
        // if we find a device the user marked as their preferred 'game' device,
  ...
  if (pXAudio2->GetDeviceDetails(preferredDevice, &deviceDetails) != S_OK) {
    // One way this could happen is if a device is removed between the loop and
    // this line of code Highly unlikely but possible
  ...
  const char* channelNames[] = {"L",  "R",  "C",  "S",  "Lb", "Rb",
                                "Lf", "Rf", "Cb", "Ls", "Rs"};
  for (int i = 0, ci = 0; ci < sizeof(channelNames) / sizeof(channelNames[0]);
       ci++) {
  ...
  // ---------------------
  if (FAILED(pXAudio2->CreateSubmixVoice(&pSubmixVoice, 1, outputSampleRate, 0,
                                         0, NULL, NULL))) {
...
void idSoundHardware_XAudio2::Shutdown() {
  ...
  if (pXAudio2 != NULL) {
    ...
    idLib::Printf(
        "Final pXAudio2 performanceData: Voices: %d/%d CPU: %.2f%% Mem: %dkb\n",
        perfData.ActiveSourceVoiceCount, perfData.TotalSourceVoiceCount,
        perfData.AudioCyclesSinceLastQuery /
            (float)perfData.TotalCyclesSinceLastQuery,
        perfData.MemoryUsageInBytes / 1024);
    pXAudio2->Release();
    pXAudio2 = NULL;
  }
  if (vuMeterRMS != NULL) {
...
========================
*/
idSoundVoice* idSoundHardware_XAudio2::AllocateVoice(
    const idSoundSample* leadinSample, const idSoundSample* loopingSample) {
  if (leadinSample == NULL) {
    return NULL;
  }
  if (loopingSample != NULL) {
    if ((leadinSample->format.basic.formatTag !=
         loopingSample->format.basic.formatTag) ||
        (leadinSample->format.basic.numChannels !=
         loopingSample->format.basic.numChannels)) {
      idLib::Warning("Leadin/looping format mismatch: %s & %s",
...
void idSoundHardware_XAudio2::Update() {
  ...
  sscanf(s_meterPosition.GetString(), "%f %f %f %f", &left, &top, &width,
         &height);
