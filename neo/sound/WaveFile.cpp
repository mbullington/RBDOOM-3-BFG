...
Returns true if the Open was successful and the file matches the expected
format. If this returns false, there is no need to call Close.
========================
*/
bool idWaveFile::Open(const char* filename) {
  ...
  while (offset < riffSize) {
    ...
    if (file->Read(&chunkHeader, sizeof(chunkHeader)) != sizeof(chunkHeader)) {
      // It seems like some tools have extra data after the last chunk for no
      // apparent reason so don't treat this as an error
      return true;
    }
...
Seeks to the specified chunk and returns the size of the chunk or 0 if the chunk
wasn't found.
========================
*/
...
const char* idWaveFile::ReadWaveFormat(waveFmt_t& format) {
  ...
  if (format.basic.formatTag == FORMAT_PCM) {
    ...
    if (memcmp(&pcmGuid, &format.extra.extensible.subFormat, sizeof(pcmGuid)) !=
        0) {
...
bool idWaveFile::WriteSampleDataDirect(idList<sampleData_t>& sampleData,
                                       idFile* file) {
...
Reads a loop point from a 'smpl' chunk in a wave file, returns 0 if none are
found.
========================
*/
bool idWaveFile::ReadLoopData(int& start, int& end) {
  ...
  if (smpl.numSampleLoops < 1) {
    return false;  // this is possible returning false lets us know there are
                   // more then 1 sample look in the file and is not appropriate
