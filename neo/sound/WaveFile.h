  ...
  struct waveFmt_t {
    ...
    union extra_t {
      ...
      struct xma2_t {
        ...
        uint32
            samplesEncoded;  // Total number of PCM samples the file decodes to
        uint32
            bytesPerBlock;  // XMA block size (but the last one may be shorter)
        uint32 playBegin;   // First valid sample in the decoded audio
        uint32 playLength;  // Length of the valid part of the decoded audio
        uint32
            loopBegin;  // Beginning of the loop region in decoded sample terms
        uint32 loopLength;  // Length of the loop region in decoded sample terms
        byte loopCount;     // Number of loop repetitions; 255 = infinite
        byte encoderVersion;  // Version of XMA encoder that generated the file
        uint16
            blockCount;  // XMA blocks in file (and entries in its seek table)
      } xma2;
    } extra;
  };
...
#pragma pack(pop)
  ...
  static bool WriteSampleDataDirect(idList<sampleData_t>& sampleData,
                                    idFile* file);
