...
idCVar s_singleEmitter("s_singleEmitter", "0", CVAR_INTEGER,
                       "mute all sounds but this emitter");
idCVar s_showStartSound("s_showStartSound", "0", CVAR_BOOL,
                        "print a message every time a sound starts/stops");
idCVar s_useOcclusion("s_useOcclusion", "1", CVAR_BOOL,
                      "Attenuate sounds based on walls");
idCVar s_centerFractionVO("s_centerFractionVO", "0.75", CVAR_FLOAT,
                          "Portion of VO sounds routed to the center channel");
...
float idSoundFade::GetVolume(const int soundTime) const {
  ...
  if (fadeDuration <= 0.0f) {
    ...
    return fadeStartVolume +
           (fadeEndVolume - fadeStartVolume) * playTime / fadeDuration;
  } else {
    return fadeStartVolume;
  }
}
...
Never actually mute VO because we can't restart them precisely enough for lip
syncing to not fuck up
========================
*/
bool idSoundChannel::CanMute() const { return true; }
...
void idSoundChannel::UpdateVolume(int currentTime) {
  ...
  // if it is a private sound, set the volume to zero unless we match the
  // listener.id
  ...
  if (parms.soundClass >= 0 && parms.soundClass < SOUND_MAX_CLASSES) {
    newVolumeDB +=
        soundWorld->soundClassFade[parms.soundClass].GetVolume(currentTime);
  }
  ...
  if (!global && !emitterIsListener) {
    float distance = (parms.soundShaderFlags & SSF_NO_OCCLUSION) == 0
                         ? emitter->spatializedDistance
                         : emitter->directDistance;
  ...
  if (soundSystemLocal.musicMuted &&
      (parms.soundShaderFlags & SSF_MUSIC) != 0) {
    newVolumeDB = DB_SILENCE;
  }
...
void idSoundChannel::UpdateHardware(float volumeAdd, int currentTime) {
  ...
  if (omni || global || emitterIsListener) {
    ...
    hardwareVoice->SetPosition(
        (emitter->spatializedOrigin - soundWorld->listener.pos) *
        soundWorld->listener.axis.Transpose());
  }
  ...
  hardwareVoice->SetPitch(soundWorld->slowmoSpeed *
                          idMath::ClampFloat(0.2f, 5.0f, timescale.GetFloat()));
  ...
  if (issueStart) {
    hardwareVoice->Start(
        startOffset,
        parms.soundShaderFlags | (parms.shakes == 0.0f ? SSF_NO_FLICKER : 0));
  } else {
...
========================
*/
idSoundEmitterLocal::idSoundEmitterLocal() { Init(0, NULL); }
...
========================
*/
idSoundEmitterLocal::~idSoundEmitterLocal() { assert(channels.Num() == 0); }
...
void idSoundEmitterLocal::Init(int i, idSoundWorldLocal* sw) {
  ...
  // Init should only be called on a freshly constructed sound emitter or in a
  // Reset()
...
void idSoundEmitterLocal::Update(int currentTime) {
  ...
  directDistance =
      (soundWorld->listener.pos - origin).LengthFast() * DOOM_TO_METERS;
  ...
  if (s_singleEmitter.GetInteger() > 0 &&
      s_singleEmitter.GetInteger() != index) {
    return;
  }
  ...
  if (emitterId != soundWorld->listener.id) {
    for (int i = 0; i < channels.Num(); i++) {
      ...
      useOcclusion = useOcclusion ||
                     ((chan->parms.soundShaderFlags & SSF_NO_OCCLUSION) == 0);
  ...
  if (useOcclusion && s_useOcclusion.GetBool()) {
    // work out virtual origin and distance, which may be from a portal instead
...
========================
*/
void idSoundEmitterLocal::UpdateEmitter(const idVec3& origin, int listenerId,
...
int idSoundEmitterLocal::StartSound(const idSoundShader* shader,
                                    ...
                                    float diversity, int shaderFlags,
                                    bool allowSlow) {
  assert(soundWorld != NULL);
  assert(soundWorld->emitters[this->index] == this);
  ...
  if (showStartSound) {
    idLib::Printf("%dms: StartSound(%d:%d): %s: ", currentTime, index, channel,
                  shader->GetName());
  }
  ...
  if (chanParms.soundShaderFlags & SSF_PLAY_ONCE) {
    for (int i = 0; i < channels.Num(); i++) {
      idSoundChannel* chan = channels[i];
      if (chan->soundShader == shader &&
          !chan->CheckForCompletion(currentTime)) {
        if (showStartSound) {
  ...
  for (int i = 0; i < channels.Num(); i++) {
    idSoundChannel* chan = channels[i];
    if (chan->soundShader == shader && chan->startTime == currentTime &&
        chan->endTime != 1) {
      if (showStartSound) {
  ...
  if (channel != SCHANNEL_ANY) {
    for (int i = 0; i < channels.Num(); i++) {
      ...
      if (chan->soundShader && chan->logicalChannel == channel) {
        if (showStartSound) {
          idLib::Printf(S_COLOR_YELLOW "OVERRIDE %s: ",
  ...
  // a hardware voice will be allocated next update if the volume is high enough
  // to be audible
  if (channels.Num() == channels.Max()) {
    CheckForCompletion(
        currentTime);  // as a last chance try to release finished sounds here
    if (channels.Num() == channels.Max()) {
      if (showStartSound) {
...
This is a shortened version of StartSound, called whenever a sound shader is
reloaded. If the emitter is currently playing the given sound shader, restart it
so a change in the sound sample used for a given sound shader will be picked up.
========================
*/
...
void idSoundEmitterLocal::StopSound(const s_channelType channel) {
  ...
  for (int i = 0; i < channels.Num(); i++) {
    ...
    if (s_showStartSound.GetBool()) {
      idLib::Printf("%dms: StopSound(%d:%d): %s\n", soundWorld->GetSoundTime(),
                    index, channel, chan->soundShader->GetName());
    }
  ...
  for (int i = channels.Num() - 1; i >= 0; i--) {
    ...
    if (s_showStartSound.GetBool()) {
      idLib::Printf("%dms: ModifySound(%d:%d): %s\n",
                    soundWorld->GetSoundTime(), index, channel,
                    chan->soundShader->GetName());
    }
...
========================
*/
void idSoundEmitterLocal::FadeSound(const s_channelType channel, float to,
                                    float over) {
  assert(soundWorld != NULL);
  assert(soundWorld->emitters[this->index] == this);
  ...
  for (int i = 0; i < channels.Num(); i++) {
    ...
    if (s_showStartSound.GetBool()) {
      idLib::Printf("%dms: FadeSound(%d:%d): %s to %.2fdb over %.2f seconds\n",
                    soundWorld->GetSoundTime(), index, channel,
                    chan->soundShader->GetName(), to, over);
    }

    // fade it
    chan->volumeFade.Fade(to - chan->parms.volume, overMSec,
                          soundWorld->GetSoundTime());
  }
}
...
float idSoundEmitterLocal::CurrentAmplitude() {
  ...
  for (int i = 0; i < channels.Num(); i++) {
    ...
    if (chan == NULL || currentTime < chan->startTime ||
        (chan->endTime > 0 && currentTime >= chan->endTime)) {
      continue;
    }
    ...
    if (relativeTime < leadinLength) {
      amplitude =
          Max(amplitude, chan->leadinSample->GetAmplitude(relativeTime));
    } else if (chan->loopingSample != NULL) {
      amplitude = Max(amplitude, chan->loopingSample->GetAmplitude(
                                     (relativeTime - leadinLength) %
                                     chan->loopingSample->LengthInMsec()));
    }
  }
  return amplitude;
}
