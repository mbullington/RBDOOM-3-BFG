...
===============
*/
idSoundShader::idSoundShader() { Init(); }
...
=================
*/
size_t idSoundShader::Size() const { return sizeof(idSoundShader); }
...
bool idSoundShader::ParseShader(idLexer& src) {
  ...
  while (1) {
    ...
    // the wave files
      ...
      if (token.IcmpPrefixPath("sound/vo/") == 0 ||
          token.IcmpPrefixPath("sound/guis/") == 0) {
      ...
      // add to the wav list
      if (s_maxSamples.GetInteger() == 0 ||
          (s_maxSamples.GetInteger() > 0 &&
           entries.Num() < s_maxSamples.GetInteger())) {
...
void idSoundShader::List() const {
  ...
  for (int k = 0; k < entries.Num(); k++) {
    ...
    if (objectp) {
      common->Printf("      %5dms %4dKb %s\n", objectp->LengthInMsec(),
                     (objectp->BufferSize() / 1024), objectp->GetName());
    }
  }
}
...
===============
*/
int idSoundShader::GetNumSounds() const { return entries.Num(); }
