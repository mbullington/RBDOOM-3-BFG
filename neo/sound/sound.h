...
static const int SSF_VO =
    BIT(10);  // VO - direct a portion of the sound through the center channel
              // (set automatically on shaders that contain files that start
              // with "sound/vo/")
static const int SSF_MUSIC =
    BIT(11);  // Music - Muted when the player is playing his own music
...
class idSoundEmitter {
 public:
  ...
  virtual int StartSound(const idSoundShader* shader,
                         ...
                         int shaderFlags = 0, bool allowSlow = true) = 0;
  ...
  virtual bool CurrentlyPlaying(
      const s_channelType channel = SCHANNEL_ANY) const = 0;
