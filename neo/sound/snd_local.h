...
// A single voice can play multiple channels (up to 5.1, but most commonly
// stereo) This is the maximum number of channels which can play simultaneously
// This is limited primarily by seeking on the optical drive, secondarily by
// memory consumption, and tertiarily by CPU time spent mixing
#define MAX_HARDWARE_CHANNELS 64

// We may need up to 3 buffers for each hardware voice if they are all long
// sounds
...
DEFINE_PROPERTYKEY(PKEY_AudioEndpoint_Path, 0x9c119480, 0xddc2, 0x4954, 0xa1,
                   0x50, 0x5b, 0xd2, 0x40, 0xd4, 0x54, 0xad, 1);
...
class idSoundChannel {
 public:
  ...
  float volumeDB;  // last volume at which this channel will play (calculated in
                   // UpdateVolume)
...
class idSoundWorldLocal : public idSoundWorld {
 public:
  ...
  virtual void FadeSoundClasses(const int soundClass, const float to,
                                const float over);
 ...
 public:
  // even though all these variables are public, nobody outside the sound system
  // includes SoundWorld_local.h so this is equivalent to making it private and
  // friending all the other classes in the sound system

  idSoundFade volumeFade;  // master volume knob for the entire world
  idSoundFade soundClassFade[SOUND_MAX_CLASSES];

  idRenderWorld*
      renderWorld;  // for debug visualization and light amplitude sampling
 ...
 public:
  ...
  void ResolveOrigin(const int stackDepth, const soundPortalTrace_t* prevStack,
                     const int soundArea, const float dist,
                     const idVec3& soundOrigin, idSoundEmitterLocal* def);
};
...
class idSoundEmitterLocal : public idSoundEmitter {
 public:
  ...
  virtual void UpdateEmitter(const idVec3& origin, int listenerId,
                             const soundShaderParms_t* parms);

  virtual int StartSound(const idSoundShader* shader,
                         const s_channelType channel, float diversity = 0,
                         int shaderFlags = 0, bool allowSlow = true);
  ...
  virtual bool CurrentlyPlaying(
      const s_channelType channel = SCHANNEL_ANY) const;
  ...
  bool canFree;  // if true, this emitter can be canFree (once channels.Num() ==
                 // 0)
...
class idSoundSystemLocal : public idSoundSystem {
 public:
  ...
  // Since otherwise there will be problems with different subsystems muting and
  // unmuting at different times
  ...
  virtual void* GetIXAudio2() const;  // FIXME: stupid name; get rid of this?
  ...
  idSoundVoice* AllocateVoice(const idSoundSample* leadinSample,
                              const idSoundSample* loopingSample);
  ...
  struct bufferContext_t {
    bufferContext_t() : voice(NULL), sample(NULL), bufferNumber(0) {}
    ...
    // from idSoundVoice_XAudio2), casting the latter to the former isn't
    // possible so we need this ugly #ifdef ..
