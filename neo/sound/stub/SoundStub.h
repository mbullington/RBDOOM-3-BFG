...
/*
 * DG: a stub to get d3 bfg to compile without XAudio2, because that doesn't
 * work with MinGW or on non-Windows platforms.
 *
 * Please note that many methods are *not* virtual, so just inheriting from the
 * stubs for the actual implementations may *not* work! (Making them virtual
 * should be evaluated for performance-loss though, it would make the code
...
class idSoundVoice : public idSoundVoice_Base {
 public:
  void Create(const idSoundSample* leadinSample,
  ...
  // returns the RMS levels of the most recently processed block of audio,
  // SSF_FLICKER must have been passed to Start
...
class idSoundHardware {
 public:
  ...
  // FIXME: this is a bad name when having multiple sound backends... and maybe
  // it's not even needed
  void* GetIXAudio2() const  // NOTE: originally this returned IXAudio2*, but
                             // that was casted to void later anyway
  {
    return NULL;
  }
...
class idSoundSample {
 public:
  ...
  ~idSoundSample();  // destructor should be public so lists of  soundsamples
