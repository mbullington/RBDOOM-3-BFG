...
idCVar s_lockListener("s_lockListener", "0", CVAR_BOOL,
                      "lock listener updates");
idCVar s_constantAmplitude("s_constantAmplitude", "-1", CVAR_FLOAT, "");
idCVar s_maxEmitterChannels(
    "s_maxEmitterChannels", "48", CVAR_INTEGER,
    "Can be set lower than the absolute max of MAX_HARDWARE_VOICES");
idCVar s_cushionFadeChannels(
    "s_cushionFadeChannels", "2", CVAR_INTEGER,
    "Ramp currentCushionDB so this many emitter channels should be silent");
idCVar s_cushionFadeRate("s_cushionFadeRate", "60", CVAR_FLOAT,
                         "DB / second change to currentCushionDB");
idCVar s_cushionFadeLimit("s_cushionFadeLimit", "-30", CVAR_FLOAT,
                          "Never cushion fade beyond this level");
idCVar s_cushionFadeOver(
    "s_cushionFadeOver", "10", CVAR_FLOAT,
    "DB above s_cushionFadeLimit to start ramp to silence");
idCVar s_unpauseFadeInTime(
    "s_unpauseFadeInTime", "250", CVAR_INTEGER,
    "When unpausing a sound world, milliseconds to fade sounds in over");
idCVar s_doorDistanceAdd(
    "s_doorDistanceAdd", "150", CVAR_FLOAT,
    "reduce sound volume with this distance when going through a door");
idCVar s_drawSounds("s_drawSounds", "0", CVAR_INTEGER, "", 0, 2,
                    idCmdSystem::ArgCompletion_Integer<0, 2>);
idCVar s_showVoices("s_showVoices", "0", CVAR_BOOL, "show active voices");
idCVar s_volume_dB("s_volume_dB", "0", CVAR_ARCHIVE | CVAR_FLOAT,
                   "volume in dB");
extern idCVar s_noSound;

extern void WriteDeclCache(idDemoFile* f, int demoCategory, int demoCode,
                           declType_t declType);
...
class idActiveChannel {
 public:
  idActiveChannel() : channel(NULL), sortKey(0) {}
  idActiveChannel(idSoundChannel* channel_, int sortKey_)
      : channel(channel_), sortKey(sortKey_) {}
...
To avoid this, make sure that the last few hardware voices are mixed with a
volume of zero, so they won't make a difference as they come and go. It isn't
obvious what the exact best volume ramping method should be, just that it
smoothly change frame to frame.
========================
*/
static float AdjustForCushionChannels(
    const idStaticList<idActiveChannel, MAX_HARDWARE_VOICES>&
        activeEmitterChannels,
    const int uncushionedChannels, const float currentCushionDB,
    const float driftRate) {
  ...
  if (activeEmitterChannels.Num() <= uncushionedChannels) {
    ...
    // we should be able to hear all of them
    targetCushionDB =
        activeEmitterChannels[uncushionedChannels].channel->volumeDB;
...
void idSoundWorldLocal::Update() {
  ...
  // An idSoundChannel is a channel on an emitter, which may have an explicit
  // channel assignment or SND_CHANNEL_ANY A hardware channel is a channel from
  // the sound file itself (IE: left, right, LFE) We only allow
  // MAX_HARDWARE_CHANNELS channels, which may wind up being a smaller number of
  // idSoundChannels
  idStaticList<idActiveChannel, MAX_HARDWARE_VOICES> activeEmitterChannels;
  int maxEmitterChannels = s_maxEmitterChannels.GetInteger() +
                           1;  // +1 to leave room for insert-before-sort
  ...
  for (int e = emitters.Num() - 1; e >= 0; e--) {
    ...
    for (int i = 0; i < emitters[e]->channels.Num(); i++) {
      ...
      // channels by adding a large value to the sort key.
      const int sortKey = idMath::Ftoi(channel->volumeDB * 100.0f +
                                       (canMute ? 0.0f : 100000.0f));

      // Keep track of the total number of hardware channels.
      // This is done after calculating the sort key to avoid a load-hit-store
      // that would occur when using the sort key in the loop below after the
      // Ftoi above.
      ...
      int insertIndex = 0;
      for (insertIndex = 0; insertIndex < activeEmitterChannels.Num();
           insertIndex++) {
      ...
      if (insertIndex == activeEmitterChannels.Num()) {
        // Always leave one spot free in the 'activeEmitterChannels' so there is
        // room to insert sort a potentially louder sound later.
        if (activeEmitterChannels.Num() + 1 >= activeEmitterChannels.Max() ||
            activeHardwareChannels + sampleChannels > MAX_HARDWARE_CHANNELS) {
          // We don't have enough voices to play this, so mute it if it was
          // playing.
          channel->Mute();
          continue;
        }
      ...
      activeEmitterChannels.Insert(idActiveChannel(channel, sortKey),
                                   insertIndex);
      activeHardwareChannels += sampleChannels;

      // If we are over our voice limit or at our channel limit, mute sounds
      // until it fits. If activeEmitterChannels is full, always remove the last
      // one so there is room to insert sort a potentially louder sound later.
      while (activeEmitterChannels.Num() == maxEmitterChannels ||
             activeHardwareChannels > MAX_HARDWARE_CHANNELS) {
        const int indexToRemove = activeEmitterChannels.Num() - 1;
        idSoundChannel* const channelToMute =
            activeEmitterChannels[indexToRemove].channel;
        channelToMute->Mute();
  ...
  // ------------------
  // In the very common case of having more sounds that would contribute to the
  // mix than there are available hardware voices, it can be an audible
  // discontinuity when a channel initially gets a voice or loses a voice. To
  // avoid this, make sure that the last few hardware voices are mixed with a
  // volume of zero, so they won't make a difference as they come and go. It
  // isn't obvious what the exact best volume ramping method should be, just
  // that it smoothly change frame to frame.
  // ------------------
  const int uncushionedChannels =
      maxEmitterChannels - s_cushionFadeChannels.GetInteger();
  currentCushionDB = AdjustForCushionChannels(
      activeEmitterChannels, uncushionedChannels, currentCushionDB,
      s_cushionFadeRate.GetFloat() * secondsPerFrame);
  ...
  if (showVoices) {
    showVoiceTable.Format(
        "currentCushionDB: %5.1f  freeVoices: %i zombieVoices: %i "
        "buffers:%i/%i\n",
        currentCushionDB, soundSystemLocal.hardware.GetNumFreeVoices(),
        soundSystemLocal.hardware.GetNumZombieVoices(),
        soundSystemLocal.activeStreamBufferContexts.Num(),
        soundSystemLocal.freeStreamBufferContexts.Num());
  }
  for (int i = 0; i < activeEmitterChannels.Num(); i++) {
    idSoundChannel* chan = activeEmitterChannels[i].channel;
    ...
    if (showVoices) {
      idStr voiceLine;
      voiceLine.Format("%5.1f db [%3i:%2i] %s", chan->volumeDB,
                       chan->emitter->index, chan->logicalChannel,
                       chan->CanMute() ? "" : " <CANT MUTE>\n");
      ...
      if (loopingSample == NULL) {
        voiceLine.Append(va("%ikhz*%i %s\n", leadinSample->SampleRate() / 1000,
                            leadinSample->NumChannels(),
                            leadinSample->GetName()));
      } else if (loopingSample == leadinSample) {
        voiceLine.Append(
            va("%ikhz*%i <LOOPING> %s\n", leadinSample->SampleRate() / 1000,
               leadinSample->NumChannels(), leadinSample->GetName()));
      } else {
        voiceLine.Append(
            va("%ikhz*%i %s | %ikhz*%i %s\n", leadinSample->SampleRate() / 1000,
               leadinSample->NumChannels(), leadinSample->GetName(),
               loopingSample->SampleRate() / 1000, loopingSample->NumChannels(),
               loopingSample->GetName()));
      }
    ...
    shakeAmp += chan->parms.shakes * chan->hardwareVoice->GetGain() *
                chan->currentAmplitude;
  }
  if (showVoices) {
    ...
    for (int e = 0; e < emitters.Num(); e++) {
      ...
      for (int c = 0; c < emitter->channels.Num(); c++) {
        if (emitter->channels[c]->hardwareVoice != NULL) {
          audible = true;
          maxGain =
              Max(maxGain, emitter->channels[c]->hardwareVoice->GetGain());
        }
      }
      if (!audible) {
        continue;
      ...
      // draw a box
      renderWorld->DebugBounds(idVec4(maxGain, maxGain, 1.0f, 1.0f), ref,
                               emitter->origin, lifetime);
      if (emitter->origin != emitter->spatializedOrigin) {
        renderWorld->DebugLine(idVec4(1.0f, 0.0f, 0.0f, 1.0f), emitter->origin,
                               emitter->spatializedOrigin, lifetime);
      }

      // draw the index
      idVec3 textPos = emitter->origin;
      textPos.z -= 8;
      renderWorld->DrawText(va("%i", e), textPos, 0.1f, idVec4(1, 0, 0, 1),
                            listener.axis, 1, lifetime);
      textPos.z += 8;
      ...
      for (int k = 0; k < emitter->channels.Num(); k++) {
        ...
        const char* defaulted =
            chan->leadinSample->IsDefault() ? " *DEFAULTED*" : "";
        idStr text;
        text.Format("%s (%i %i/%i)%s", chan->soundShader->GetName(),
                    idMath::Ftoi(emitter->spatializedDistance),
                    idMath::Ftoi(min), idMath::Ftoi(max), defaulted);
        renderWorld->DrawText(text, textPos, 0.1f, idVec4(1, 0, 0, 1),
                              listener.axis, 1, lifetime);
        textPos.z += 8;
      }
    }
...
idSoundEmitter* idSoundWorldLocal::EmitterForIndex(int index) {
  ...
  if (index >= emitters.Num()) {
    idLib::Error("idSoundWorldLocal::EmitterForIndex: %i >= %i", index,
                 emitters.Num());
  }
...
int idSoundWorldLocal::PlayShaderDirectly(const char* name, int channel) {
  ...
  if (shader == NULL) {
    ...
    return localSound->StartSound(shader, channel,
                                  soundSystemLocal.random.RandomFloat(),
                                  SSF_GLOBAL, true);
  }
}
...
def->spatializedDistance will remain set at maxDistance
===================
*/
...
void idSoundWorldLocal::ResolveOrigin(const int stackDepth,
                                      ...
                                      const int soundArea, const float dist,
...
========================
*/
void idSoundWorldLocal::StopWritingDemo() { writeDemo = NULL; }
...
void idSoundWorldLocal::ReadFromSaveGame(idFile* savefile) {
  struct helper {
    static void ReadSoundFade(idFile* savefile, idSoundFade& sf,
                              int timeDelta) {
  ...
  for (int e = 1; e < numEmitters; e++) {
    ...
    for (int c = 0; c < numChannels; c++) {
      ...
      // If the leadin entry is not valid (possible if the shader changed after
      // saving) then the looping entry can't be valid either
...
=================
*/
void idSoundWorldLocal::FadeSoundClasses(const int soundClass, const float to,
                                         const float over) {
...
=================
*/
void idSoundWorldLocal::SetSlowmoSpeed(float speed) { slowmoSpeed = speed; }
