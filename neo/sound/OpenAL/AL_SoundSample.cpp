...
========================
*/
static void FreeBuffer(void* p) { return Mem_Free(p); }
...
========================
*/
idSoundSample_OpenAL::~idSoundSample_OpenAL() { FreeData(); }
...
bool idSoundSample_OpenAL::LoadGeneratedSample(const idStr& filename) {
  ...
  if (fileIn != NULL) {
    ...
    for (int i = 0; i < num; i++) {
      ...
      buffers[i].buffer =
          GPU_CONVERT_CPU_TO_CPU_CACHED_READONLY_ADDRESS(buffers[i].buffer);
    }
    return true;
  }
#endif
...
void idSoundSample_OpenAL::LoadResource() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if ((i == 0) &&
        !sampleName.Replace("/vo/", va("/vo/%s/", sys_lang.GetString()))) {
      i++;
    }
...
void idSoundSample_OpenAL::CreateOpenALBuffer() {
  ...
  if (CheckALErrors() != AL_NO_ERROR) {
    common->Error(
        "idSoundSample_OpenAL::CreateOpenALBuffer: error generating OpenAL "
        "hardware buffer");
  }
  ...
  if (alIsBuffer(openalBuffer)) {
    ...
    if (format.basic.formatTag == idWaveFile::FORMAT_ADPCM) {
      ...
      if (MS_ADPCM_decode((uint8**)&buffer, &bufferSize) < 0) {
        common->Error(
            "idSoundSample_OpenAL::CreateOpenALBuffer: could not decode ADPCM "
            "'%s' to 16 bit format",
            GetName());
      }
      ...
      common->Error(
          "idSoundSample_OpenAL::CreateOpenALBuffer: could not decode XMA2 "
          "'%s' to 16 bit format",
          GetName());
    } else if (format.basic.formatTag == idWaveFile::FORMAT_EXTENSIBLE) {
      // RB: not used in the PC version of the BFG edition
      common->Error(
          "idSoundSample_OpenAL::CreateOpenALBuffer: could not decode "
          "extensible WAV format '%s' to 16 bit format",
          GetName());
    } else {
...
#endif
    {
      alBufferData(openalBuffer, GetOpenALBufferFormat(), buffer, bufferSize,
                   format.basic.samplesPerSec);
    }

    if (CheckALErrors() != AL_NO_ERROR) {
      common->Error(
          "idSoundSample_OpenAL::CreateOpenALBuffer: error loading data into "
          "OpenAL hardware buffer");
    }
  }
}
...
bool idSoundSample_OpenAL::LoadWav(const idStr& filename) {
  ...
  if (format.basic.formatTag == idWaveFile::FORMAT_PCM ||
    ...
    if (format.basic.bitsPerSample != 16) {
      idLib::Warning("LoadWav( %s ) : %s", filename.c_str(),
                     "Not a 16 bit PCM wav file");
      MakeDefault();
      return false;
    }
    ...
    if (format.basic.bitsPerSample == 16) {
      idSwap::LittleArray((short*)buffers[0].buffer,
                          totalBufferSize / sizeof(short));
    }

    buffers[0].buffer =
        GPU_CONVERT_CPU_TO_CPU_CACHED_READONLY_ADDRESS(buffers[0].buffer);
    ...
    if (format.extra.xma2.blockCount == 0) {
      idLib::Warning("LoadWav( %s ) : %s", filename.c_str(),
                     "No data blocks in file");
      MakeDefault();
      return false;
    }

    int bytesPerBlock = format.extra.xma2.bytesPerBlock;
    assert(format.extra.xma2.blockCount ==
           ALIGN(totalBufferSize, bytesPerBlock) / bytesPerBlock);
    assert(format.extra.xma2.blockCount * bytesPerBlock >= totalBufferSize);
    assert(format.extra.xma2.blockCount * bytesPerBlock <
           totalBufferSize + bytesPerBlock);
    ...
    if (seekTableSize != 4 * buffers.Num()) {
      idLib::Warning("LoadWav( %s ) : %s", filename.c_str(),
                     "Wrong number of entries in seek table");
      MakeDefault();
      return false;
    }
    ...
    if (buffers[buffers.Num() - 1].numSamples < playBegin + playLength) {
      ...
      for (int i = 0; i < buffers.Num(); i++) {
        if (buffers[i].numSamples > playBegin + playLength) {
          ...
          // Ideally, the following loop should always have 0 iterations because
          // playBegin + playLength ends in the last block already But there is
          // no guarantee for that, so to be safe, discard all buffers beyond
          // this one
    ...
    idLib::Warning("LoadWav( %s ) : Unsupported wave format %d",
                   filename.c_str(), format.basic.formatTag);
    MakeDefault();
    return false;
  }
  ...
  if (format.basic.formatTag == idWaveFile::FORMAT_EXTENSIBLE) {
    // HACK: XAudio2 doesn't really support FORMAT_EXTENSIBLE so we convert it
    // to a basic format after extracting the channel mask
...
void idSoundSample_OpenAL::MakeDefault() {
  ...
  format.basic.blockSize =
      format.basic.numChannels * format.basic.bitsPerSample / 8;
  format.basic.avgBytesPerSec =
      format.basic.samplesPerSec * format.basic.blockSize;
  ...
  if (CheckALErrors() != AL_NO_ERROR) {
    common->Error(
        "idSoundSample_OpenAL::MakeDefault: error generating OpenAL hardware "
        "buffer");
  }

  if (alIsBuffer(openalBuffer)) {
    CheckALErrors();
    alBufferData(openalBuffer, GetOpenALBufferFormat(), defaultBuffer,
                 totalBufferSize, format.basic.samplesPerSec);
    if (CheckALErrors() != AL_NO_ERROR) {
      common->Error(
          "idSoundSample_OpenAL::MakeDefault: error loading data into OpenAL "
          "hardware buffer");
    }
  }
}
...
void idSoundSample_OpenAL::FreeData() {
  ...
  if (alIsBuffer(openalBuffer)) {
    ...
    if (CheckALErrors() != AL_NO_ERROR) {
      common->Error(
          "idSoundSample_OpenAL::FreeData: error unloading data from OpenAL "
          "hardware buffer");
    } else {
      openalBuffer = 0;
    }
...
ALenum idSoundSample_OpenAL::GetOpenALBufferFormat() const {
  ...
  if (format.basic.formatTag == idWaveFile::FORMAT_PCM) {
    ...
    alFormat = NumChannels() == 1 ? AL_FORMAT_MONO16 : AL_FORMAT_STEREO16;
    // alFormat = NumChannels() == 1 ? AL_FORMAT_MONO_IMA4 :
...
int32 idSoundSample_OpenAL::MS_ADPCM_nibble(MS_ADPCM_decodeState_t* state,
                                            int8 nybble) {
  const int32 max_audioval = ((1 << (16 - 1)) - 1);
  const int32 min_audioval = -(1 << (16 - 1));
  const int32 adaptive[] = {230, 230, 230, 230, 307, 409, 512, 614,
                            768, 614, 512, 409, 307, 230, 230, 230};

  int32 new_sample, delta;

  new_sample =
      ((state->iSamp1 * state->coef1) + (state->iSamp2 * state->coef2)) / 256;
...
int idSoundSample_OpenAL::MS_ADPCM_decode(uint8** audio_buf,
                                          uint32* audio_len) {
  ...
  *audio_len = (encoded_len / format.basic.blockSize) *
  ...
  *audio_buf = (uint8*)Mem_Alloc(*audio_len, TAG_AUDIO);
  ...
  while (encoded_len >= format.basic.blockSize) {
    ...
    samplesleft =
        (format.extra.adpcm.samplesPerBlock - 2) * format.basic.numChannels;
