...
Copyright (c) 2010 by Chris Robinson <chris.kcat@gmail.com> (OpenAL Info
Utility)
...
void listDevices_f(const idCmdArgs& args) {
  ...
  idSoundHardware_OpenAL::PrintALCInfo(
      (ALCdevice*)soundSystem->GetOpenALDevice());
}
...
void idSoundHardware_OpenAL::Init() {
  ...
  if (openalDevice == NULL) {
    common->FatalError(
        "idSoundHardware_OpenAL::Init: alcOpenDevice() failed\n");
    return;
  }
  ...
  if (alcMakeContextCurrent(openalContext) == 0) {
    common->FatalError(
        "idSoundHardware_OpenAL::Init: alcMakeContextCurrent( %p) failed\n",
        openalContext);
    return;
  }
  ...
  */
  ...
  const char* channelNames[] = { "L", "R", "C", "S", "Lb", "Rb", "Lf", "Rf",
  "Cb", "Ls", "Rs" }; for( int i = 0, ci = 0; ci < sizeof( channelNames ) /
  sizeof( channelNames[0] ); ci++ )
  {
...
========================
*/
idSoundVoice* idSoundHardware_OpenAL::AllocateVoice(
...
void idSoundHardware_OpenAL::Update() {
  ...
  perfData.AudioCyclesSinceLastQuery / ( float
  ...
  */
  ...
  for( int i = 0; i < outputChannels; i++ )
  {
          ...
          {
                  vuMeterPeak->SetValue( i, vuMeterPeak->GetValue( i ) * 0.9f,
  colorRed );
          }
  }
  ...
  {
          vuMeterRMS->SetValue( i, rmsLevels[ i ], idVec4( 0.5f, 1.0f,
  0.0f, 1.00f ) ); if( peakLevels[ i ] >= vuMeterPeak->GetValue( i ) )
          {
                  vuMeterPeak->SetValue( i, peakLevels[ i ], colorRed );
                  vuMeterPeakTimes[i] = currentTime +
  s_meterTopTime.GetInteger();
          }
  }
  */
}
