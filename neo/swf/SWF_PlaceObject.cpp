...
void idSWFSpriteInstance::PlaceObject2(idSWFBitStream& bitstream) {
  ...
  if ((flags & PlaceFlagMove) != 0) {
    ...
    if (display == NULL) {
      idLib::Warning(
          "PlaceObject2: trying to modify entry %d, which doesn't exist",
          depth);
      return;
    }
    if (characterID >= 0) {
      ...
      if (display->spriteInstance || display->textInstance) {
        idLib::Warning(
            "PlaceObject2: Trying to change the character of a sprite after "
            "it's been created");
        return;
      }
      ...
      if (dictEntry != NULL) {
        if (dictEntry->type == SWF_DICT_SPRITE ||
            dictEntry->type == SWF_DICT_EDITTEXT) {
          idLib::Warning(
              "PlaceObject2: Trying to change the character of a shape to a "
              "sprite");
          return;
        }
      }
    ...
    if (characterID < 0) {
      idLib::Warning(
          "PlaceObject2: Trying to create a new object without a character");
      return;
    }
    // create a new entry
    ...
    if (display == NULL) {
      idLib::Warning(
          "PlaceObject2: trying to create a new entry at %d, but an item "
...
void idSWFSpriteInstance::PlaceObject3(idSWFBitStream& bitstream) {
  ...
  if ((flags2 & PlaceFlagHasClassName) != 0 ||
      (((flags2 & PlaceFlagHasImage) != 0) &&
       ((flags1 & PlaceFlagHasCharacter) != 0))) {
  ...
  if ((flags1 & PlaceFlagMove) != 0) {
    ...
    if (display == NULL) {
      idLib::Warning(
          "PlaceObject3: trying to modify entry %d, which doesn't exist",
          depth);
      return;
    }
    if (characterID >= 0) {
      ...
      if (display->spriteInstance || display->textInstance) {
        idLib::Warning(
            "PlaceObject3: Trying to change the character of a sprite after "
            "it's been created");
        return;
      }
      idSWFDictionaryEntry* dictEntry =
      ...
      if (dictEntry != NULL) {
        if (dictEntry->type == SWF_DICT_SPRITE ||
            dictEntry->type == SWF_DICT_EDITTEXT) {
          idLib::Warning(
              "PlaceObject3: Trying to change the character of a shape to a "
              "sprite");
          return;
        }
      }
    ...
    if (characterID < 0) {
      idLib::Warning(
          "PlaceObject3: Trying to create a new object without a character");
      return;
    }
    // create a new entry
    display = AddDisplayEntry(depth, characterID);
    if (display == NULL) {
      idLib::Warning(
          "PlaceObject3: trying to create a new entry at %d, but an item "
          "already exists there",
          depth);
      return;
    }
