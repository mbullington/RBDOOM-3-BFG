...
class idSWFShapeParser {
 ...
 private:
  void ParseShapes(idSWFBitStream& bitstream1, idSWFBitStream* bitstream2,
                   bool swap);
  void ReadFillStyle(idSWFBitStream& bitstream);
  void ParseEdge(idSWFBitStream& bitstream, int32& penX, int32& penY,
                 swfSPEdge_t& edge);
  void MakeLoops();
  void TriangulateSoup(idSWFShape& shape);
  void TriangulateSoup(idSWFFontGlyph& shape);
  int FindEarVert(const swfSPLineLoop_t& loop);
  void AddUniqueVert(idSWFShapeDrawFill& drawFill, const idVec2& start,
                     const idVec2& end);
};
