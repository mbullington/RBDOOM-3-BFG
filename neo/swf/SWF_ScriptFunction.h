...
NOTE: The ref count starts at 0 here because it assumes you do an AddRef on the
allocated object.  The proper way is to start it at 1 and force the caller to
Release, but that's really kind of a pain in the ass.  It was made to be used
like this: object->Set( "myFunction", new idSWFScriptFunction_MyFunction() );
========================
*/
...
/*
========================
The idSWFStack class is just a helper routine for treating an idList like a
stack
========================
*/
...
/*
========================
idSWFScriptFunction_Script is a script function that's implemented in action
script
========================
*/
class idSWFScriptFunction_Script : public idSWFScriptFunction {
 public:
  ...
  // This could all be passed to Alloc (and was at one time) but in some places
  // it's far more convenient to specify each separately
 ...
 private:
  idSWFScriptVar Run(idSWFScriptObject* thisObject, idSWFStack& stack,
  ...
  idStr ExportToScript(idSWFScriptObject* thisObject, idSWFStack& stack,
                       idSWFBitStream& bitstream, const char* filename,
 ...
 private:
  ...
  idSWFSpriteInstance*
      defaultSprite;  // some actions have an implicit sprite they work off of
                      // (e.g. Action_GotoFrame outside of object scope)
