...
idCVar swf_debugShowAddress(
    "swf_debugShowAddress", "0", CVAR_BOOL,
    "shows addresses along with object types when they are serialized");
...
========================
*/
idSWFScriptObject::swfNamedVar_t& idSWFScriptObject::swfNamedVar_t::operator=(
    const swfNamedVar_t& other) {
  if (&other != this) {
    index = other.index;
    name = other.name;
...
========================
*/
void idSWFScriptObject::AddRef() { refCount++; }
...
void idSWFScriptObject::Set(const char* name, const idSWFScriptVar& value) {
  if (objectType == SWF_OBJECT_ARRAY) {
    if (idStr::Cmp(name, "length") == 0) {
      ...
      for (int i = 0; i < variables.Num(); i++) {
        int hash = idStr::Hash(variables[i].name.c_str()) &
                   (VARIABLE_HASH_BUCKETS - 1);
...
========================
*/
void idSWFScriptObject::SetNative(const char* name,
...
========================
*/
idSWFScriptObject::swfNamedVar_t* idSWFScriptObject::GetVariable(int index,
                                                                 bool create) {
  for (int i = 0; i < variables.Num(); i++) {
...
========================
*/
idSWFScriptObject::swfNamedVar_t* idSWFScriptObject::GetVariable(
  ...
  if (prototype != NULL) {
    ...
    if ((variable != NULL) && (variable->native || !create)) {
      // If the variable is native, we want to pull it from the prototype even
      // if we're going to set it
      return variable;
    }
  }
...
========================
*/
idSWFScriptObject* idSWFScriptObject::GetNestedObj(
    const char* arg1, const char* arg2, const char* arg3, const char* arg4,
    const char* arg5, const char* arg6) {
...
========================
*/
idSWFTextInstance* idSWFScriptObject::GetNestedText(
    const char* arg1, const char* arg2, const char* arg3, const char* arg4,
    const char* arg5, const char* arg6) {
  idSWFScriptVar var = GetNestedVar(arg1, arg2, arg3, arg4, arg5, arg6);
  return var.ToText();
