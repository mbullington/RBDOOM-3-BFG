...
idCVar swf_textScrollSpeed("swf_textScrollSpeed", "80", CVAR_INTEGER,
                           "scroll speed for text");
idCVar swf_textRndLetterSpeed("swf_textRndLetterSpeed", "8", CVAR_INTEGER,
                              "scroll speed for text");
idCVar swf_textRndLetterDelay("swf_textRndLetterDelay", "100", CVAR_INTEGER,
                              "scroll speed for text");
idCVar swf_textParagraphSpeed("swf_textParagraphSpeed", "15", CVAR_INTEGER,
                              "scroll speed for text");
idCVar swf_textParagraphInc("swf_textParagraphInc", "1.3", CVAR_FLOAT,
                            "scroll speed for text");
idCVar swf_subtitleExtraTime("swf_subtitleExtraTime", "3500", CVAR_INTEGER,
                             "time after subtitles vo is complete");
idCVar swf_subtitleEarlyTrans("swf_subtitleEarlyTrans", "3500", CVAR_INTEGER,
                              "early time out to switch the line");
idCVar swf_subtitleLengthGuess("swf_subtitleLengthGuess", "10000", CVAR_INTEGER,
                               "early time out to switch the line");
idCVar swf_textMaxInputLength(
    "swf_textMaxInputLength", "104", CVAR_INTEGER,
    "max number of characters that can go into the input line");
idCVar swf_textStrokeSize("swf_textStrokeSize", "1.65f", CVAR_FLOAT,
                          "size of font glyph stroke", 0.0f, 2.0f);
idCVar swf_textStrokeSizeGlyphSpacer(
    "swf_textStrokeSizeGlyphSpacer", "1.5f", CVAR_FLOAT,
    "additional space for spacing glyphs using stroke");
...
========================
*/
idSWFTextInstance::idSWFTextInstance() { swf = NULL; }
...
float idSWFTextInstance::GetTextLength() {
  ...
  if (verify(swf != NULL)) {
    ...
    idSWFDictionaryEntry* fontEntry =
        swf->FindDictionaryEntry(shape->fontID, SWF_DICT_FONT);
    ...
    while (index < tlen) {
      ...
      fontInfo->GetScaledGlyph(glyphScale, txtLengthCheck.UTF8Char(index),
                               glyph);

      len += glyph.xSkip;
      if (useStroke) {
        len += (swf_textStrokeSizeGlyphSpacer.GetFloat() * strokeWeight *
                glyphScale);
      }
...
==============================================
*/
void idSWFTextInstance::SwitchSubtitleText(int time) { subNeedsSwitch = false; }
...
bool idSWFTextInstance::UpdateSubtitle(int time) {
  ...
  if (subUpdating && !subForceKill) {
    if ((time >= subSwitchTime && !subNeedsSwitch) ||
        (!subNeedsSwitch && subInitialLine)) {
...
void idSWFTextInstance::LastWordChanged(int wordCount, int time) {
  ...
  subSwitchTime =
      subStartTime + subtitleTimingInfo[subLastWordIndex]
                         .startTime;  // - swf_subtitleEarlyTrans.GetInteger();
  ...
  if (time >= subSwitchTime) {
    ...
    for (int i = wordIndex; i < subtitleTimingInfo.Num(); ++i) {
      if (subtitleTimingInfo[i].forceBreak) {
        ...
        int timeSpan = subtitleTimingInfo[i].startTime -
                       subtitleTimingInfo[wordIndex].startTime;
    ...
    if (!setSwitchTime && subtitleTimingInfo.Num() > 0) {
      subSwitchTime = subStartTime +
                      subtitleTimingInfo[subtitleTimingInfo.Num() - 1]
                          .startTime;  // - swf_subtitleEarlyTrans.GetInteger();
...
int idSWFTextInstance::CalcNumLines() {
  ...
  while (charIndex < textCheck.Length()) {
    if (textCheck[charIndex] == '\n') {
      ...
      float glyphSkip = glyph.xSkip;
      if (useStroke) {
        glyphSkip += (swf_textStrokeSizeGlyphSpacer.GetFloat() * strokeWeight *
                      glyphScale);
      }
...
#define SWF_TEXT_FUNCTION_DEFINE(x)                                           \
  idSWFScriptVar                                                              \
      idSWFScriptObject_TextInstancePrototype::idSWFScriptFunction_##x::Call( \
          idSWFScriptObject* thisObject, const idSWFParmList& parms)
#define SWF_TEXT_NATIVE_VAR_DEFINE_GET(x)                                     \
  idSWFScriptVar                                                              \
      idSWFScriptObject_TextInstancePrototype::idSWFScriptNativeVar_##x::Get( \
          class idSWFScriptObject* object)
#define SWF_TEXT_NATIVE_VAR_DEFINE_SET(x)                                      \
  void idSWFScriptObject_TextInstancePrototype::idSWFScriptNativeVar_##x::Set( \
      class idSWFScriptObject* object, const idSWFScriptVar& value)
...
idSWFScriptObject_TextInstancePrototype::
  ...
  SWF_TEXT_NATIVE_VAR_SET(
      _textLength);  // only works on single lines of text not multiline
