...
========================
*/
void idSWFShapeParser::Parse(idSWFBitStream& bitstream, idSWFShape& shape,
                             int recordType) {
  ...
  for (int i = 0; i < lineDraws.Num(); i++) {
    ...
    for (int e = 0; e < spld.edges.Num(); e++) {
      ...
      // Rather then tesselating curves at run time, we do them once here by
      // inserting a vert every 10 units It may not wind up being 10 actual
      // pixels when rendered because the shape may have scaling applied to it
      if (spld.edges[e].start.cp != 0xFFFF) {
        assert(spld.edges[e].end.cp != 0xFFFF);
        float length1 =
            (verts[spld.edges[e].start.v0] - verts[spld.edges[e].start.v1])
                .Length();
        float length2 =
            (verts[spld.edges[e].end.v0] - verts[spld.edges[e].end.v1])
                .Length();
...
========================
*/
void idSWFShapeParser::ParseMorph(idSWFBitStream& bitstream,
                                  idSWFShape& shape) {
  extendedCount = true;
  lineStyle2 = false;
  rgba = true;
  ...
  // offset is the byte offset from the current read position to the 'endShape'
  // record we read the entire block into 'bitstream1' which moves the read
  // pointer of 'bitstream' to the start of the 'endShape' record
...
========================
*/
void idSWFShapeParser::ParseFont(idSWFBitStream& bitstream,
                                 idSWFFontGlyph& shape) {
  extendedCount = false;
  lineStyle2 = false;
  rgba = false;
  morph = false;
...
========================
*/
void idSWFShapeParser::ParseShapes(idSWFBitStream& bitstream1,
                                   idSWFBitStream* bitstream2, bool swap) {
  int32 pen1X = 0;
  int32 pen1Y = 0;
  int32 pen2X = 0;
  int32 pen2Y = 0;
  ...
  while (true) {
    if (!bitstream1.ReadBool()) {
      ...
      if ((stateNewStyles || stateLineStyle || stateFillStyle1 ||
           stateFillStyle0 || stateMoveTo) == false) {
        // end record
        if (bitstream2) {
      ...
      if (bitstream2) {
        ...
        if (isEdge) {
          idLib::Warning(
              "idSWFShapeParser: morph stream 1 defines style change, but "
              "stream 2 does not");
          break;
        }
        ...
        if (stateFillStyle0) {
          if (bitstream2->ReadU(numFillBits2) != fillStyle0) {
            idLib::Warning(
                "idSWFShapeParser: morph stream 2 defined a different "
                "fillStyle0 from stream 1");
            break;
          }
        }
        if (stateFillStyle1) {
      ...
      if (bitstream2) {
        ...
        if (!isEdge) {
          idLib::Warning(
              "idSWFShapeParser: morph stream 1 defines an edge, but stream 2 "
              "does not");
          break;
        }
...
========================
*/
void idSWFShapeParser::ParseEdge(idSWFBitStream& bitstream, int32& penX,
                                 int32& penY, swfSPEdge_t& edge) {
...
void idSWFShapeParser::MakeLoops() {
  ...
  for (int i = 0; i < fillDraws.Num(); i++) {
    ...
    for (int e = 0; e < fill.edges.Num(); e++) {
      if ((fill.edges[e].start.v0 == fill.edges[e].start.v1) ||
          (fill.edges[e].end.v0 == fill.edges[e].end.v1)) {
    ...
    while (unusedEdges.Num() > 0) {
      ...
      while (true) {
        ...
        if (fill.edges[e1].start.cp != 0xFFFF) {
          ...
          float length1 =
              (verts[fill.edges[e1].start.v0] - verts[fill.edges[e1].start.v1])
                  .Length();
          float length2 =
              (verts[fill.edges[e1].end.v0] - verts[fill.edges[e1].end.v1])
                  .Length();
          int numPoints = 1 + idMath::Ftoi(Max(length1, length2) / 10.0f);
          for (int ti = 0; ti < numPoints; ti++) {
            float t0 = (ti + 1) / ((float)numPoints + 1.0f);
      ...
      const idVec2& v1 =
          verts[loop.vindex1[(loop.vindex1.Num() + leftMostIndex - 1) %
                             loop.vindex1.Num()]];
      const idVec2& v2 = verts[loop.vindex1[leftMostIndex]];
      const idVec2& v3 =
          verts[loop.vindex1[(leftMostIndex + 1) % loop.vindex1.Num()]];
      idVec2 a = v1 - v2;
      idVec2 b = v3 - v2;
      float normal = (a.x * b.y - a.y * b.x);
    ...
    while (true) {
      ...
      // now that we have a hole and the shape it's inside, merge the two
      // together
...
void idSWFShapeParser::TriangulateSoup(idSWFShape& shape) {
  ...
  // Now turn the (potentially) concave line loops into triangles by using ear
  // clipping
  ...
  for (int i = 0; i < fillDraws.Num(); i++) {
    ...
    for (int j = 0; j < spDrawFill.loops.Num(); j++) {
      ...
      for (int k = 0; k < numVerts - 2; k++) {
        ...
        AddUniqueVert(drawFill, verts[loop.vindex1[v1]],
                      verts[loop.vindex2[v1]]);
        AddUniqueVert(drawFill, verts[loop.vindex1[v2]],
                      verts[loop.vindex2[v2]]);
        AddUniqueVert(drawFill, verts[loop.vindex1[v3]],
                      verts[loop.vindex2[v3]]);
...
========================
*/
void idSWFShapeParser::AddUniqueVert(idSWFShapeDrawFill& drawFill,
  ...
  for (int i = 0; i < fillStyleCount; i++) {
    ...
    if (fillStyle.type == 0) {
      ...
      if (fillStyle.subType == 3) {
        assert(morph ==
               false);  // focal gradients aren't allowed in morph shapes
