...
void idFile_SWF::WriteByte(byte bits) { file->WriteUnsignedChar(bits); }
...
void idFile_SWF::WriteColorXFormRGBA(const swfColorXform_t& xcf) {
  bool hasAddTerms = (xcf.add.x != 0.0f || xcf.add.y != 0.0f ||
                      xcf.add.z != 0.0f || xcf.add.w != 0.0f);
  WriteUBits(hasAddTerms ? 1 : 0, 1);

  bool hasMulTerms = (xcf.mul.x != 0.0f || xcf.mul.y != 0.0f ||
                      xcf.mul.z != 0.0f || xcf.mul.w != 0.0f);
