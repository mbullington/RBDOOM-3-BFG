...
class idSWF {
 public:
  ...
  int PlaySound(const char* sound, int channel = SCHANNEL_ANY,
                bool blocking = false);
  ...
  idSWFScriptObject* HitTest(idSWFSpriteInstance* spriteInstance,
                             const swfRenderState_t& renderState, int x, int y,
 ...
 private:
  ...
  // certain screens need to be rendered when the pause menu is up so if this
  // flag is set on the gui we will allow it to render at a paused state;
  bool pausedRender;
...
#define SWF_NATIVE_FUNCTION_SWF_DECLARE(x)                                   \
  ...
  SWF_NATIVE_VAR_DECLARE_NESTED_READONLY(platform,
                                         idSWFScriptFunction_getPlatform,
                                         Call(object, idSWFParmList()));
  ...
  struct keyButtonImages_t {
    ...
    keyButtonImages_t(const char* _key, const char* _xbImage,
                      const char* _psImage, int w, int h, int _baseline) {
      key = _key;
      xbImage = _xbImage;
      psImage = _psImage;
      width = w;
 ...
 private:
  ...
  void WriteSWF(const char* filename, const byte* atlasImageRGBA,
  ...
  void DrawStretchPic(float x, float y, float w, float h, float s1, float t1,
                      float s2, float t2, const idMaterial* material);
  ...
  void RenderMask(idRenderSystem* gui, const swfDisplayEntry_t* mask,
                  const swfRenderState_t& renderState, const int stencilMode);
  void RenderShape(idRenderSystem* gui, const idSWFShape* shape,
                   const swfRenderState_t& renderState);
  void RenderMorphShape(idRenderSystem* gui, const idSWFShape* shape,
                        const swfRenderState_t& renderState);
  void DrawEditCursor(idRenderSystem* gui, float x, float y, float w, float h,
                      const swfMatrix_t& matrix);
  void DrawLine(idRenderSystem* gui, const idVec2& p1, const idVec2& p2,
                float width, const swfMatrix_t& matrix);
  void RenderEditText(idRenderSystem* gui, idSWFTextInstance* textInstance,
                      const swfRenderState_t& renderState, int time,
                      bool isSplitscreen = false);
  ...
  // RB: debugging tools
  swfRect_t CalcRect(const idSWFSpriteInstance* sprite,
                     const swfRenderState_t& renderState);
  void DrawRect(idRenderSystem* gui, const swfRect_t& rect,
                const idVec4& color);
  int DrawText(idRenderSystem* gui, float x, float y, float scale, idVec4 color,
               const char* text, float adjust, int limit, int style);
  int DrawText(idRenderSystem* gui, const char* text, float textScale,
               int textAlign, idVec4 color, const swfRect_t& rectDraw,
               bool wrap, int cursor = -1, bool calcOnly = false,
               idList<int>* breaks = NULL, int limit = 0);
  // RB end
  ...
  struct imageToPack_t {
    ...
    idVec2i allocSize;  // in DXT tiles, includes a border texel and rounding up
                        // to DXT blocks
  };
