...
idCVar swf_loadBinary("swf_loadBinary", "1", CVAR_INTEGER,
                      "used to set whether to load binary swf from generated");
// RB begin
...
idSWF::idSWF(const char* filename_, idSoundWorld* soundWorld_) {
  ...
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY1>", "guis/assets/hud/controller/xb360/a",
                        "guis/assets/hud/controller/ps3/cross", 37, 37, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY2>", "guis/assets/hud/controller/xb360/b",
                        "guis/assets/hud/controller/ps3/circle", 37, 37, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY3>", "guis/assets/hud/controller/xb360/x",
                        "guis/assets/hud/controller/ps3/square", 37, 37, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY4>", "guis/assets/hud/controller/xb360/y",
                        "guis/assets/hud/controller/ps3/triangle", 37, 37, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY_TRIGGER2>", "guis/assets/hud/controller/xb360/rt",
                        "guis/assets/hud/controller/ps3/r2", 64, 52, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY_TRIGGER1>", "guis/assets/hud/controller/xb360/lt",
                        "guis/assets/hud/controller/ps3/l2", 64, 52, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY5>", "guis/assets/hud/controller/xb360/lb",
                        "guis/assets/hud/controller/ps3/l1", 52, 32, 0));
  tooltipButtonImage.Append(
      keyButtonImages_t("<JOY6>", "guis/assets/hud/controller/xb360/rb",
                        "guis/assets/hud/controller/ps3/r1", 52, 32, 0));
  tooltipButtonImage.Append(keyButtonImages_t(
      "<MOUSE1>", "guis/assets/hud/controller/mouse1", "", 64, 52, 0));
  tooltipButtonImage.Append(keyButtonImages_t(
      "<MOUSE2>", "guis/assets/hud/controller/mouse2", "", 64, 52, 0));
  tooltipButtonImage.Append(keyButtonImages_t(
      "<MOUSE3>", "guis/assets/hud/controller/mouse3", "", 64, 52, 0));

  for (int index = 0; index < tooltipButtonImage.Num(); index++) {
    if ((tooltipButtonImage[index].xbImage != NULL) &&
        (tooltipButtonImage[index].xbImage[0] != '\0')) {
      declManager->FindMaterial(tooltipButtonImage[index].xbImage);
    }
    if ((tooltipButtonImage[index].psImage != NULL) &&
        (tooltipButtonImage[index].psImage[0] != '\0')) {
  ...
  if (/*!loadedFromJSON &&*/ (postLoadExportFlashToJSON.GetBool() ||
                              postLoadExportFlashAtlas.GetBool() ||
                              postLoadExportFlashToSWF.GetBool())) {
    // try loading the TGA first
    ID_TIME_T timestamp;
    // LoadTGA( atlasFileName.c_str(), &atlasExportImageRGBA,
    // &atlasExportImageWidth, &atlasExportImageHeight, &timestamp );
    LoadSTBImage(atlasFileName.c_str(), &atlasExportImageRGBA,
                 &atlasExportImageWidth, &atlasExportImageHeight, &timestamp);
    ...
    if ((atlasExportImageRGBA == NULL) ||
      ...
      ID_TIME_T binaryFileTime =
          im.LoadFromGeneratedFile(FILE_NOT_FOUND_TIMESTAMP);
      ...
      if (binaryFileTime != FILE_NOT_FOUND_TIMESTAMP) {
        ...
        if (imgHeader.format == FMT_DXT1) {
          ...
          if (imgHeader.colorFormat == CFM_NORMAL_DXT5) {
            dxt.DecompressNormalMapDXT5(data, rgba.Ptr(), img.width,
                                        img.height);
        ...
        } else if (imgHeader.format == FMT_LUM8 ||
                   imgHeader.format == FMT_INT8) {
          ...
          for( int i = 0; i < img.dataSize / 2; i++ )
          {
                  unsigned short color = ( ( pic[ i * 4 + 0 ] >> 3 ) << 11 ) | (
          ( pic[ i * 4 + 1 ] >> 2 ) << 5 ) | ( pic[ i * 4 + 2 ] >> 3 );
        ...
        R_WritePNG(atlasFileNameExport, rgba.Ptr(), 4, img.width, img.height,
                   true, "fs_basepath");
  ...
  if (postLoadExportFlashToSWF.GetBool()) {
    ...
    WriteSWF(swfFileName, atlasExportImageRGBA, atlasExportImageWidth,
             atlasExportImageHeight);
  }
  ...
  globals->SetNative("platform",
                     swfScriptVar_platform.Bind(&scriptFunction_getPlatform));
  ...
  // But disable script warnings because many globals won't have been created
  // yet
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_inhibitControl::Call(
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_useInhibit::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_deactivate::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
idSWFScriptVar idSWF::idSWFScriptFunction_precacheSound::Call(
  ...
  const idSoundShader* soundShader =
      declManager->FindSound(parms[0].ToString(), true);
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_playSound::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_stopSounds::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  int channel = SCHANNEL_ANY;
  if (parms.Num() == 1) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_getPlatform::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_getTruePlatform::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  return 2;
}
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_strReplace::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 3) {
    return "";
  }
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_getLocalString::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() == 0) {
    return idSWFScriptVar();
  }
...
========================
*/
bool idSWF::UseCircleForAccept() { return false; }
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_swapPS3Buttons::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_getCVarInteger::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_setCVarInteger::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_acos::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_cos::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_sin::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_round::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_pow::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 2) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_sqrt::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_abs::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1) {
    return idSWFScriptVar();
  }
...
===================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_rand::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  float min = 0.0f;
  float max = 1.0f;
  switch (parms.Num()) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_floor::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_ceil::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
  if (parms.Num() != 1 || !parms[0].IsNumeric()) {
...
========================
*/
idSWFScriptVar idSWF::idSWFScriptFunction_toUpper::Call(
    idSWFScriptObject* thisObject, const idSWFParmList& parms) {
...
idSWFScriptVar idSWF::idSWFScriptNativeVar_blackbars::Get(
    idSWFScriptObject* object) {
  return pThis->blackbars;
}

void idSWF::idSWFScriptNativeVar_blackbars::Set(idSWFScriptObject* object,
                                                const idSWFScriptVar& value) {
  pThis->blackbars = value.ToBool();
}

idSWFScriptVar idSWF::idSWFScriptNativeVar_crop::Get(
    idSWFScriptObject* object) {
  return pThis->crop;
}

void idSWF::idSWFScriptNativeVar_crop::Set(idSWFScriptObject* object,
                                           const idSWFScriptVar& value) {
