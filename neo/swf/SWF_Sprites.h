...
class idSWFSprite {
 public:
  ...
  void WriteJSON_PlaceObject3(idFile* f, idSWFBitStream& bitstream,
                              int characterID, int commandID,
                              const char* indentPrefix = "");
  void WriteJSON_RemoveObject2(idFile* f, idSWFBitStream& bitstream,
                               int characterID, int commandID,
                               const char* indentPrefix = "");
  void WriteJSON_DoAction(idFile* f, idSWFBitStream& bitstream, int characterID,
                          int commandID, const char* indentPrefix = "");
 ...
 private:
  ...
  class idSWF*
      swf;  // this is required so things can access the dictionary, it would be
            // kind of nice if we just had an idSWFDictionary pointer instead

  uint16 frameCount;

  // frameOffsets contains offsets into the commands list for each frame
  // the first command for frame 3 is frameOffsets[2] and the last command is
  // frameOffsets[3]
