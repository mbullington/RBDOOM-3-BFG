...
========================
*/
void idSWFSpriteInstance::Init(idSWFSprite* _sprite,
                               idSWFSpriteInstance* _parent, int _depth) {
  sprite = _sprite;
  parent = _parent;
  depth = _depth;
  ...
  for (int i = 0; i < sprite->doInitActions.Num(); i++) {
    actionScript->SetData(sprite->doInitActions[i].Ptr(),
                          sprite->doInitActions[i].Length());
...
void idSWFSpriteInstance::FreeDisplayList() {
  ...
  displayList.SetNum(
      0);  // not calling Clear() so we don't continuously re-allocate memory
  currentFrame = 0;
}
...
swfDisplayEntry_t* idSWFSpriteInstance::AddDisplayEntry(int depth,
  ...
  swfDisplayEntry_t& display =
      displayList[displayList.Insert(swfDisplayEntry_t(), i)];
  display.depth = depth;
  display.characterID = characterID;

  idSWFDictionaryEntry* dictEntry =
      sprite->swf->FindDictionaryEntry(characterID);
  if (dictEntry != NULL) {
...
class idSort_SpriteDepth
    : public idSort_Quick<swfDisplayEntry_t, idSort_SpriteDepth> {
 public:
...
===================
*/
void idSWFSpriteInstance::Stop() { isPlaying = false; }
...
void idSWFSpriteInstance::RunTo(int targetFrame) {
  ...
  for (uint32 c = sprite->frameOffsets[currentFrame];
       c < sprite->frameOffsets[targetFrame]; c++) {
...
========================
*/
idSWFSpriteInstance* idSWFSpriteInstance::FindChildSprite(
...
========================
*/
idSWFSpriteInstance* idSWFSpriteInstance::ResolveTarget(
    const char* targetName) {
  if (targetName[0] == 0) {
    return this;
  }
  ...
  for (int index = 0, ofs = spriteNames[index].Find('/'); ofs != -1;
       index++, ofs = spriteNames[index].Find('/')) {
  ...
  for (int i = 0; i < spriteNames.Num(); i++) {
    ...
    if (target == NULL) {
      ...
      idLib::Warning("SWF: Could not resolve %s, %s not found", targetName,
                     spriteNames[i].c_str());
      return this;
    }
  }
...
uint32 idSWFSpriteInstance::FindFrame(const char* labelName) const {
  ...
  idLib::Warning("Could not find frame '%s' in sprite '%s'", labelName,
                 GetName());
  return currentFrame;
}
...
========================
*/
bool idSWFSpriteInstance::IsBetweenFrames(const char* frameLabel1,
                                          const char* frameLabel2) const {
  return currentFrame >= FindFrame(frameLabel1) &&
         currentFrame <= FindFrame(frameLabel2);
}
...
========================
*/
void idSWFSpriteInstance::SetMaterial(const idMaterial* material, int width,
                                      int height) {
  materialOverride = material;

  if (materialOverride != NULL) {
    // Converting this to a short should be safe since we don't support images
    // larger than 8k anyway
    if (materialOverride->GetStage(0) != NULL &&
        materialOverride->GetStage(0)->texture.cinematic != NULL) {
      materialWidth = 256;
      materialHeight = 256;
    } else {
      assert(materialOverride->GetImageWidth() > 0 &&
             materialOverride->GetImageHeight() > 0);
      assert(materialOverride->GetImageWidth() <= 8192 &&
             materialOverride->GetImageHeight() <= 8192);
...
float idSWFSpriteInstance::GetXPos() const {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "GetXPos: Couldn't find our display entry in our parent's display list "
        "for depth %d",
        depth);
    return 0.0f;
  }
...
float idSWFSpriteInstance::GetYPos(bool overallPos) const {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "GetYPos: Couldn't find our display entry in our parents display list "
        "for depth %d",
        depth);
    return 0.0f;
  }
...
void idSWFSpriteInstance::SetXPos(float xPos) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "_y: Couldn't find our display entry in our parents display list");
    return;
  }
...
void idSWFSpriteInstance::SetRotation(float rot) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "_rotation: Couldn't find our display entry in our parents display "
        "list");
    return;
  }
...
void idSWFSpriteInstance::SetScale(float x, float y) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "scale: Couldn't find our display entry in our parents display list");
    return;
  }
...
bool idSWFSpriteInstance::UpdateMoveToScale(float speed) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "SetMoveToScale: Couldn't find our display entry in our parents "
        "display list");
    return false;
  }
...
void idSWFSpriteInstance::SetAlpha(float val) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != this) {
    idLib::Warning(
        "_alpha: Couldn't find our display entry in our parents display list");
    return;
  }
...
========================
*/
#define SWF_SPRITE_FUNCTION_DEFINE(x)                              \
  idSWFScriptVar idSWFScriptObject_SpriteInstancePrototype::       \
      idSWFScriptFunction_##x::Call(idSWFScriptObject* thisObject, \
                                    const idSWFParmList& parms)
#define SWF_SPRITE_NATIVE_VAR_DEFINE_GET(x)                  \
  idSWFScriptVar idSWFScriptObject_SpriteInstancePrototype:: \
      idSWFScriptNativeVar_##x::Get(class idSWFScriptObject* object)
#define SWF_SPRITE_NATIVE_VAR_DEFINE_SET(x)                                  \
  void idSWFScriptObject_SpriteInstancePrototype::idSWFScriptNativeVar_##x:: \
      Set(class idSWFScriptObject* object, const idSWFScriptVar& value)
...
SWF_SPRITE_FUNCTION_DEFINE(duplicateMovieClip) {
  ...
  swfDisplayEntry_t* thisDisplayEntry =
      pThis->parent->FindDisplayEntry(pThis->depth);
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "duplicateMovieClip: Couldn't find our display entry in our parents "
        "display list");
    return idSWFScriptVar();
  }
  ...
  swfDisplayEntry_t* display = pThis->parent->AddDisplayEntry(
      16384 + parms[1].ToInteger(), thisDisplayEntry->characterID);
  if (display == NULL) {
    return idSWFScriptVar();
  }
...
SWF_SPRITE_NATIVE_VAR_DEFINE_GET(_xscale) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "_xscale: Couldn't find our display entry in our parents display list");
    return 1.0f;
  }
...
SWF_SPRITE_NATIVE_VAR_DEFINE_GET(_yscale) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "_yscale: Couldn't find our display entry in our parents display list");
    return 1.0f;
  }
...
SWF_SPRITE_NATIVE_VAR_DEFINE_GET(_brightness) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "_brightness: Couldn't find our display entry in our parents display "
        "list");
    return 1.0f;
  }
  // This works as long as the user only used the "brightess" control in the
  // editor If they used anything else (tint/advanced) then this will return
  // fairly random values
...
SWF_SPRITE_NATIVE_VAR_DEFINE_GET(_mousex) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "_mousex: Couldn't find our display entry in our parents display list");
    return pThis->sprite->GetSWF()->GetMouseX();
  }
...
SWF_SPRITE_NATIVE_VAR_DEFINE_GET(_mousey) {
  ...
  if (thisDisplayEntry == NULL || thisDisplayEntry->spriteInstance != pThis) {
    idLib::Warning(
        "_mousey: Couldn't find our display entry in our parents display list");
    return pThis->sprite->GetSWF()->GetMouseY();
  }
