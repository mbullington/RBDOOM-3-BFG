...
void idSWF::DefineFont2(idSWFBitStream& bitstream) {
  ...
  if (flags & BIT(3)) {
    ...
    idSWFBitStream offsetStream(bitstream.ReadData(offsetTableSize),
                                offsetTableSize, false);
    if (offsetStream.ReadU32() != offsetTableSize) {
      idLib::Warning(
          "idSWF::DefineFont2: first glyph offset != offsetTableSize");
      return;
    }
    ...
    for (int i = 0; i < numGlyphs; i++) {
      ...
      idSWFBitStream shapeStream(bitstream.ReadData(shapeSize), shapeSize,
                                 false);
...
========================
*/
void idSWF::DefineFont3(idSWFBitStream& bitstream) { DefineFont2(bitstream); }
...
void idSWF::DefineEditText(idSWFBitStream& bitstream) {
  ...
  idSWFDictionaryEntry* entry =
      AddDictionaryEntry(characterID, SWF_DICT_EDITTEXT);
  if (entry == NULL) {
    return;
  }
