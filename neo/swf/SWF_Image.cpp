...
idCVar swf_useChannelScale("swf_useChannelScale", "0", CVAR_BOOL,
                           "compress texture atlas colors");
...
========================
*/
byte* idSWF::idDecompressJPEG::Load(const byte* input, int inputSize,
  ...
  try {
    ...
    if (cinfo->output_components != 4) {
      // This shouldn't really be possible, unless the source image is some kind
      // of strange grayscale format or something
    ...
    while (cinfo->output_scanline < cinfo->output_height) {
      JSAMPROW scanlines = output + cinfo->output_scanline *
                                        cinfo->output_width *
...
void idSWF::WriteSwfImageAtlas(const char* filename) {
  ...
  // without re-packing on the 360 and PS3.  The growth checks in
  // RectAllocator() will always align, but a single image won't necessarily be.
  ...
  for (int i = 0; i < packImages.Num(); i++) {
    ...
    // The GPU hardware addressing should completely ignore texels outside the
    // true block size, but the compressor works on complete blocks, regardless
    // of the true rect size.
    x <<= 2;
    y <<= 2;
  ...
  // R_WriteTGA( filename, swfAtlas.Ptr(), atlasWidth, atlasHeight, false,
  // "fs_basepath" );
  R_WritePNG(filename, swfAtlas.Ptr(), 4, atlasWidth, atlasHeight, true,
             "fs_basepath");
}
...
========================
*/
void idSWF::LoadImage(int characterID, const byte* imageData, int width,
                      int height) {
  ...
  // save the data off so we can do the image atlas allocation after we have
  // collected all the images that are used by the entire swf
  imageToPack_t pack;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    // Swf images are usually completely random in size, but perform all
    // allocations in DXT blocks of 4.  If we choose to DCT / HDP encode the
    // image block, we should probably increae the block size to 8 or 16 to
    // prevent neighbor effects.
    v = (v + 3) >> 2;

    // Allways allocate a single pixel border around the images so there won't
    // be any edge bleeds.  This can often be hidden in in the round-up to DXT
    // size.
...
come before any DefineBits tags We don't have to worry about clearing the jpeg
object because jpeglib will automagically overwrite any tables that are already
set (I think?)
========================
*/
void idSWF::JPEGTables(idSWFBitStream& bitstream) {
  ...
  int width, height;
  jpeg.Load(bitstream.ReadData(bitstream.Length()), bitstream.Length(), width,
            height);
}
...
void idSWF::DefineBits(idSWFBitStream& bitstream) {
  ...
  int width, height;
  byte* imageData =
      jpeg.Load(bitstream.ReadData(jpegSize), jpegSize, width, height);
  if (imageData == NULL) {
    return;
  }
...
Identical to DefineBits, except it uses a local JPEG table (not the one defined
by JPEGTables)
========================
*/
...
Mostly identical to DefineBitsJPEG2, except it has an additional zlib compressed
alpha map
========================
*/
void idSWF::DefineBitsJPEG3(idSWFBitStream& bitstream) {
...

  {
    ...
    int alphaSize =
        bitstream.Length() - jpegSize - sizeof(characterID) - sizeof(jpegSize);
    if (!Inflate(bitstream.ReadData(alphaSize), alphaSize, alphaMap.Ptr(),
                 (int)alphaMap.Size())) {
...
void idSWF::DefineBitsLossless(idSWFBitStream& bitstream) {
  ...
  if (format == 3) {
    ...
    if (!Inflate(bitstream.ReadData(colorDataSize), colorDataSize,
                 colorMapData.Ptr(), (int)colorMapData.Size())) {
    ...
    uint32 colorDataSize = bitstream.Length() - bitstream.Tell();
    if (!Inflate(bitstream.ReadData(colorDataSize), colorDataSize,
                 (byte*)bitmapData.Ptr(), (int)bitmapData.Size())) {
