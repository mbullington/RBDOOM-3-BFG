...
int maskForNumBits[33] = {
    NBM(0x00), NBM(0x01), NBM(0x02), NBM(0x03), NBM(0x04), NBM(0x05), NBM(0x06),
    NBM(0x07), NBM(0x08), NBM(0x09), NBM(0x0A), NBM(0x0B), NBM(0x0C), NBM(0x0D),
    NBM(0x0E), NBM(0x0F), NBM(0x10), NBM(0x11), NBM(0x12), NBM(0x13), NBM(0x14),
    NBM(0x15), NBM(0x16), NBM(0x17), NBM(0x18), NBM(0x19), NBM(0x1A), NBM(0x1B),
    NBM(0x1C), NBM(0x1D), NBM(0x1E), NBM(0x1F), -1};

#define NBS(x) \
  (int32)((unsigned long)(-1L) << (x - 1))  // SRS - Cast to unsigned long
int signForNumBits[33] = {
    NBS(0x01), NBS(0x01), NBS(0x02), NBS(0x03), NBS(0x04), NBS(0x05), NBS(0x06),
    NBS(0x07), NBS(0x08), NBS(0x09), NBS(0x0A), NBS(0x0B), NBS(0x0C), NBS(0x0D),
    NBS(0x0E), NBS(0x0F), NBS(0x10), NBS(0x11), NBS(0x12), NBS(0x13), NBS(0x14),
    NBS(0x15), NBS(0x16), NBS(0x17), NBS(0x18), NBS(0x19), NBS(0x1A), NBS(0x1B),
...
========================
*/
ID_FORCE_INLINE unsigned int idSWFBitStream::ReadInternalU(
    uint64& regCurrentBit, uint64& regCurrentByte, unsigned int numBits) {
  assert(numBits <= 32);

  // read bits with only one microcoded shift instruction (shift with variable)
  // on the consoles this routine never reads more than 7 bits beyond the
  // requested number of bits from the stream such that calling ResetBits()
  // never discards more than 7 bits and aligns with the next byte
  ...
  readp += numExtraBytes;
  return (unsigned int)((regCurrentByte >> regCurrentBit) &
                        maskForNumBits[numBits]);
}
...
========================
*/
ID_FORCE_INLINE int idSWFBitStream::ReadInternalS(uint64& regCurrentBit,
                                                  uint64& regCurrentByte,
                                                  unsigned int numBits) {
  int i = (int)ReadInternalU(regCurrentBit, regCurrentByte, numBits);

  // sign extend without microcoded shift instrunction (shift with variable) on
  // the consoles
...
void idSWFBitStream::ReadGradient(swfGradient_t& grad, bool rgba) {
  grad.numGradients = ReadU8() & 0xF;  // the top 4 bits control spread and
                                       // interpolation mode, but we ignore them
