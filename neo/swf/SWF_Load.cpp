...
#define BSWF_VERSION \
  16  // bumped to 16 for storing atlas image dimensions for unbuffered loads
...
bool idSWF::LoadSWF(const char* fullpath) {
  ...
  // slurp the raw file into a giant array, which is somewhat atrocious when
  // loading from the preload since it's already an idFile_Memory
...
void idSWF::WriteSWF(const char* swfFilename, const byte* atlasImageRGBA,
      ...
      case SWF_DICT_IMAGE: {
        ...
        for (int h = 0; h < height; h++) {
          for (int w = 0; w < width; w++) {
            int atlasPixelOfs =
                (entry.imageAtlasOffset[0] + w +
                 ((entry.imageAtlasOffset[1] + h) * atlasImageWidth)) *
                4;
        ...
        sprintf(imageExportFileName, "exported/%s/image_characterid_%i.png",
                filenameWithoutExt.c_str(), i);

        R_WritePNG(imageExportFileName.c_str(), pngData.Ptr(), 4, width, height,
                   true, "fs_basepath");
        ...
        if (!Deflate(colorData.Ptr(), colorDataSize,
                     (byte*)compressedData.Ptr(), compressedDataSize)) {
...
bool idSWF::LoadBinary(const char* bfilename, ID_TIME_T sourceTimeStamp) {
  ...
  // a release build
  if (magic != BSWF_MAGIC ||
      (!fileSystem->InProductionMode() &&
       (sourceTimeStamp != FILE_NOT_FOUND_TIMESTAMP) &&
       (sourceTimeStamp != 0) && (sourceTimeStamp != btimestamp))) {
    delete f;
    return false;
  }
  // RB end
    ...
    switch (dictionary[i].type) {
      ...
      case SWF_DICT_FONT: {
        ...
        for (int g = 0; g < font->glyphs.Num(); g++) {
          ...
          f->ReadBigArray(font->glyphs[g].verts.Ptr(),
                          font->glyphs[g].verts.Num());
          f->ReadBig(num);
          font->glyphs[g].indices.SetNum(num);
          f->ReadBigArray(font->glyphs[g].indices.Ptr(),
                          font->glyphs[g].indices.Num());
        }
        break;
      }
      case SWF_DICT_TEXT: {
    ...
    switch (dictionary[i].type) {
      ...
      case SWF_DICT_SHAPE: {
        ...
        for (int d = 0; d < shape->fillDraws.Num(); d++) {
          ...
          for (int g = 0; g < fillDraw.style.gradient.numGradients; g++) {
            file->WriteBig(
                fillDraw.style.gradient.gradientRecords[g].startRatio);
            file->WriteBig(fillDraw.style.gradient.gradientRecords[g].endRatio);
            file->Write(&fillDraw.style.gradient.gradientRecords[g].startColor,
                        4);
            file->Write(&fillDraw.style.gradient.gradientRecords[g].endColor,
                        4);
          }
          file->WriteBig(fillDraw.style.focalPoint);
          file->WriteBig(fillDraw.style.bitmapID);
          file->WriteBig(fillDraw.startVerts.Num());
          file->WriteBigArray(fillDraw.startVerts.Ptr(),
                              fillDraw.startVerts.Num());
          file->WriteBig(fillDraw.endVerts.Num());
          file->WriteBigArray(fillDraw.endVerts.Ptr(), fillDraw.endVerts.Num());
          file->WriteBig(fillDraw.indices.Num());
          file->WriteBigArray(fillDraw.indices.Ptr(), fillDraw.indices.Num());
        ...
        for (int d = 0; d < shape->lineDraws.Num(); d++) {
          ...
          file->Write(&lineDraw.style.endColor, 4);
          file->WriteBig(lineDraw.startVerts.Num());
          file->WriteBigArray(lineDraw.startVerts.Ptr(),
                              lineDraw.startVerts.Num());
          file->WriteBig(lineDraw.endVerts.Num());
          file->WriteBigArray(lineDraw.endVerts.Ptr(), lineDraw.endVerts.Num());
          file->WriteBig(lineDraw.indices.Num());
          file->WriteBigArray(lineDraw.indices.Ptr(), lineDraw.indices.Num());
      ...
      case SWF_DICT_FONT: {
        ...
        for (int g = 0; g < font->glyphs.Num(); g++) {
          ...
          file->WriteBigArray(font->glyphs[g].verts.Ptr(),
                              font->glyphs[g].verts.Num());
          file->WriteBig(font->glyphs[g].indices.Num());
          file->WriteBigArray(font->glyphs[g].indices.Ptr(),
                              font->glyphs[g].indices.Num());
        }
        break;
      }
...
bool idSWF::LoadJSON(const char* filename) {
  ...
  for (SizeType i = 0; i < a.Size(); i++) {
    ...
    if (type == "IMAGE") {
      ...
      int width, height;
      ID_TIME_T timestamp;
      R_LoadImage(imageName.c_str(), &imageData, &width, &height, &timestamp,
                  false, NULL);
      if (imageData != NULL) {
      ...
      if (entry.HasMember("fillDraws")) {
        ...
        for (int d = 0; d < shape->fillDraws.Num(); d++) {
          ...
          // 0 = linear, 2 = radial, 3 = focal; 0 = repeat, 1 = clamp, 2 = near
          // repeat, 3 = near clamp
          ...
          if (fillDraw.style.type == 0)  // style.HasMember["startColor"] )// )
          {
            ...
            fillDraw.style.startColor.g =
                (uint8)(startColor[1].GetDouble() * 255) & 0xFF;
            fillDraw.style.startColor.b =
                (uint8)(startColor[2].GetDouble() * 255) & 0xFF;
            fillDraw.style.startColor.a =
            ...
            if (style.HasMember("endColor")) {
              ...
              fillDraw.style.endColor.g =
                  (uint8)(startColor[1].GetDouble() * 255) & 0xFF;
              fillDraw.style.endColor.b =
                  (uint8)(startColor[2].GetDouble() * 255) & 0xFF;
              fillDraw.style.endColor.a =
                  (uint8)(startColor[3].GetDouble() * 255) & 0xFF;
            } else {
      ...
      if (entry.HasMember("lineDraws")) {
        ...
        for (int d = 0; d < shape->lineDraws.Num(); d++) {
          ...
          lineDraw.style.startColor.g =
              (uint8)(startColor[1].GetDouble() * 255) & 0xFF;
          lineDraw.style.startColor.b =
              (uint8)(startColor[2].GetDouble() * 255) & 0xFF;
          lineDraw.style.startColor.a =
              (uint8)(startColor[3].GetDouble() * 255) & 0xFF;
          ...
          if (style.HasMember("endColor")) {
            ...
            lineDraw.style.endColor.g =
                (uint8)(startColor[1].GetDouble() * 255) & 0xFF;
            lineDraw.style.endColor.b =
                (uint8)(startColor[2].GetDouble() * 255) & 0xFF;
            lineDraw.style.endColor.a =
                (uint8)(startColor[3].GetDouble() * 255) & 0xFF;
          } else {
  ...
  for (int i = 0; i < dictionary.Num(); i++) {
    ...
    // if( dictionary[i].type != SWF_DICT_NULL )
    {
      ...
      file->WriteFloatString("\t\t\t\"characterID\": %i%s\n", i,
                             dictionary[i].type != SWF_DICT_NULL ? "," : "");
    }
    ...
    switch (dictionary[i].type) {
      case SWF_DICT_IMAGE: {
        if (dictionary[i].material) {
          ...
          file->WriteFloatString(
              "\t\t\t\"imageFile\": \"%s/image_characterid_%i.png\",\n",
              filenameWithoutExt.c_str(), i);
        }

        file->WriteFloatString(
            "\t\t\t\"width\": %i, \"height\": %i, \"atlasOffsetX\": %i, "
            "\"atlasOffsetY\": %i,\n",
            entry.imageSize[0], entry.imageSize[1], entry.imageAtlasOffset[0],
            entry.imageAtlasOffset[1]);

        // file->WriteFloatString( "\t\t\t\"channelScale\": { \"x\": %f, \"y\":
        // %f, \"z\": %f, \"w\": %f }\n", entry.channelScale.x,
        // entry.channelScale.y, entry.channelScale.z, entry.channelScale.w );
        file->WriteFloatString(
            "\t\t\t\"channelScale\": { \"x\": 1, \"y\": 1, \"z\": 1, \"w\": 1 "
            "}\n");
        break;
      }
      ...
      case SWF_DICT_SHAPE: {
        ...
        // file->WriteFloatString( "\t\t\t\"imageSize\": { \"width\": %f,
        // \"height\": %f },\n", dictionary[i].imageSize[0],
        // dictionary[i].imageSize[1] ); file->WriteFloatString(
        // "\t\t\t\"startBounds\": { \"x\": %f, \"y\": %f, \"width\": %f,
        // \"height\": %f },\n", x, y, width, height );
        ...
        file->WriteFloatString("\t\t\t\"endBounds\": [ %f, %f, %f, %f ]", tl.x,
                               tl.y, br.x, br.y);
        ...
        if (shape->fillDraws.Num() > 0) {
          ...
          if (shape->fillDraws.Num() > 1) {
            idLib::Printf(S_COLOR_YELLOW "WARNING: " S_COLOR_RED
                                         "%s.Shape%i has %i fill draws\n",
          ...
          for (int d = 0; d < shape->fillDraws.Num(); d++) {
            ...
            // 0 = linear, 2 = radial, 3 = focal; 0 = repeat, 1 = clamp, 2 =
            // near repeat, 3 = near clamp
            file->WriteFloatString(",\n\t\t\t\t\t\t\"subType\": ");
            ...
            if (fillDraw.style.type == 1 && fillDraw.style.subType == 3) {
              file->WriteFloatString(",\n\t\t\t\t\t\t\"focalPoint\": %f",
                                     fillDraw.style.focalPoint);
            }

            if (fillDraw.style.type == 4) {
              file->WriteFloatString(",\n\t\t\t\t\t\t\"bitmapID\": %i",
                                     fillDraw.style.bitmapID);
            }

            if (fillDraw.style.type == 0) {
              idVec4 color = fillDraw.style.startColor.ToVec4();
              file->WriteFloatString(
                  ",\n\t\t\t\t\t\t\"startColor\": [ %f, %f, %f, %f ]", color.x,
            ...
            if (!fillDraw.startVerts.Num() && !fillDraw.endVerts.Num() &&
                !fillDraw.indices.Num()) {
              file->WriteFloatString("\n");
            }
            // else
            {
              // skip \t\t\t\t}
              // file->WriteFloatString( "%s\n", ( d == ( shape->fillDraws.Num()
              // - 1 ) ) ? "" : "," );
            }
      ...
      case SWF_DICT_FONT: {
        ...
        file->WriteFloatString(
            "\t\t\t\"name\": \"%s\", \"ascent\": %i, \"descent\": %i, "
            "\"leading\": %i\n",  //, \"glyphsNum\": %i\n",
            font->fontID->GetName(), font->ascent, font->descent,
            font->leading);  //, font->glyphs.Num() );
      ...
      case SWF_DICT_TEXT: {
        ...
        file->WriteFloatString(
            "\t\t\t<Bounds x=\"%f\" y=\"%f\" width=\"%f\" height=\"%f\" />\n",
            x, y, width, height);
        ...
        for (int t = 0; t < text->textRecords.Num(); t++) {
          ...
          file->WriteFloatString(
              "\t\t\t\t<Record fontID=\"%i\" xOffet=\"%i\" yOffset=\"%i\" "
              "textHeight=\"%f\" firstGlyph=\"%i\" numGlyphs=\"%i\">\n",
              textRecord.fontID, textRecord.xOffset, textRecord.yOffset,
              textRecord.textHeight, textRecord.firstGlyph,
              textRecord.numGlyphs);
      ...
      case SWF_DICT_EDITTEXT: {
        ...
        file->WriteFloatString(
            "\t\t\t\"flags\": %i, \"fontID\": %i, \"fontHeight\": %i, "
            "\"maxLength\": %i, \"align\": \"%s\", \"leftMargin\": %i, "
            "\"rightMargin\": %i, \"indent\": %i, \"leading\": %i, "
            "\"variable\": \"%s\", \"initialText\": %s,\n",
            et->flags, et->fontID, et->fontHeight, et->maxLength,
            idSWF::GetEditTextAlignName(et->align), et->leftMargin,
            et->rightMargin, et->indent, et->leading, et->variable.c_str(),
            initialText.c_str());
