...
idSWFSprite::idSWFSprite(idSWF* _swf)
    : swf(_swf), frameCount(0), commandBuffer(NULL) {}
...
========================
*/
idSWFSprite::~idSWFSprite() { Mem_Free(commandBuffer); }
...
void idSWF::DefineSprite(idSWFBitStream& bitstream) {
  ...
  idSWFDictionaryEntry* entry =
      AddDictionaryEntry(characterID, SWF_DICT_SPRITE);
  if (entry == NULL) {
    return;
  }
...
void idSWFSprite::Load(idSWFBitStream& bitstream, bool parseDictionary) {
  ...
  // run through the file once, building the dictionary and accumulating control
  // tags
  ...
  while (true) {
    ...
    idSWFBitStream tagStream(bitstream.ReadData(recordLength), recordLength,
                             false);
...
#undef HANDLE_SWF_TAG
    ...
    // ----------------------
    // Control tags
    // control tags are stored off in the commands list and processed at run
    // time except for a couple really special control tags like "End" and
    // "FrameLabel"
    // ----------------------
    switch (tag) {
      case Tag_End:
      ...
      case Tag_DoInitAction: {
        ...
        initaction.Load(tagStream.ReadData(recordLength - 2), recordLength - 2,
                        true);
      } break;
      ...
      case Tag_RemoveObject2: {
        ...
        command.tag = tag;
        command.stream.Load(tagStream.ReadData(recordLength), recordLength,
                            true);
      } break;
...
void idSWFSprite::ReadJSON(const id::JSON& entry) {
  ...
  for (int i = 0; i < commands.Num(); i++) {
    ...
    if (type == "Tag_PlaceObject2" || type == "Tag_PlaceObject3") {
      ...
      commands[i].stream.Load(
          (byte*)static_cast<idFile_Memory*>((idFile*)file)->GetDataPtr(),
          streamLength, true);
      ...
      uint32 streamLength =
          file->Length() - 1;  // skip trailing zero added by Decode()
      commands[i].stream.Load(
          (byte*)static_cast<idFile_Memory*>((idFile*)file)->GetDataPtr(),
          streamLength, true);
    }
...
void idSWFSprite::WriteJSON(idFile* f, int characterID) {
  ...
  if (frameOffsets.Num()) {
    ...
    for (int i = 0; i < frameOffsets.Num(); i++) {
      f->WriteFloatString("%i%s", frameOffsets[i],
                          (i == frameOffsets.Num() - 1) ? "" : ", ");
    }
    f->WriteFloatString(" ],\n");
  }
  ...
  if (frameLabels.Num()) {
    ...
    for (int i = 0; i < frameLabels.Num(); i++) {
      f->WriteFloatString(
          "\t\t\t\t{ \"frameNum\": %i, \"frameLabel\": \"%s\" }%s\n",
          frameLabels[i].frameNum, frameLabels[i].frameLabel.c_str(),
          (i == frameLabels.Num() - 1) ? "" : ", ");
    }
    f->WriteFloatString("\t\t\t],\n");
  }
  ...
  for (int i = 0; i < commands.Num(); i++) {
    ...
    // f->WriteFloatString( "%s\t<Command tag=\"%s\"
    // streamLength=\"%i\">%s</Command>\n", indentPrefix, idSWF::GetTagName(
    // commands[i].tag ), src.Length(), src.c_str() ); f->WriteFloatString(
    // "%s\t<Command tag=\"%s\" streamLength=\"%i\">%s</Command>\n",
    // indentPrefix, idSWF::GetTagName( commands[i].tag ),
    // commands[i].stream.Length(), base64.c_str() );

    // f->WriteFloatString( "%s\t<Command tag=\"%s\" streamLength=\"%i\">\n",
    // indentPrefix, idSWF::GetTagName( command.tag ), command.stream.Length(),
    // base64.c_str() ); f->WriteFloatString( "%s\t\t<Stream>%s</Stream>\n",
...
#define HANDLE_SWF_TAG(x)                             \
  case Tag_##x:                                       \
    WriteJSON_##x(f, command.stream, characterID, i); \
    break;
...
#undef HANDLE_SWF_TAG
  ...
  if (doInitActions.Num()) {
    ...
    for (int i = 0; i < doInitActions.Num(); i++) {
      ...
      f->WriteFloatString(
          "\t\t\t\"DoInitAction\": { \"streamLength\": %i, \"stream\": \"%s\" "
          "}%s\n",
          doInitActions[i].Length(), base64.c_str(),
          (i == doInitActions.Num() - 1) ? "" : ", ");
    }
    f->WriteFloatString("\t\t\t]");
  }
#endif
...
void idSWFSprite::WriteJSON_PlaceObject2(idFile* file,
                                         idSWFBitStream& bitstream,
                                         int sourceCharacterID, int commandID,
  ...
  file->WriteFloatString("\t\t\t\t\t\"flags\": %i, \"depth\": %i", flags1,
                         depth);
  ...
  if ((flags1 & PlaceFlagHasMatrix) != 0) {
    ...
    file->WriteFloatString(
        ",\n\t\t\t\t\t\"startMatrix\": [ %f, %f, %f, %f, %f, %f ]", m.xx, m.yy,
        m.xy, m.yx, m.tx, m.ty);
  }
...
void idSWFSprite::WriteJSON_PlaceObject3(idFile* file,
                                         idSWFBitStream& bitstream,
                                         int sourceCharacterID, int commandID,
                                         const char* indentPrefix) {
  uint8 flags1 = bitstream.ReadU8();
  ...
  file->WriteFloatString(
      "\t\t\t\t\t\"flags1\": %i, \"flags2\": %i, \"depth\": %i", flags1, flags2,
      depth);
  ...
  if ((flags2 & PlaceFlagHasFilterList) != 0) {
    // we don't support filters and because the filter list is variable length
    // we can't support anything after the filter list either (blend modes and
    // clip actions)
...
void idSWFSprite::WriteJSON_RemoveObject2(idFile* file,
                                          idSWFBitStream& bitstream,
                                          int sourceCharacterID, int commandID,
                                          const char* indentPrefix) {
  int depth = bitstream.ReadU16();
  ...
  file->WriteFloatString(
      "%s\t\t\t\t{\t\"type\": \"Tag_RemoveObject2\", \"depth\": %i }",
      (commandID != 0) ? ",\n" : "", depth);
}

void idSWFSprite::WriteJSON_DoAction(idFile* file, idSWFBitStream& bitstream,
                                     int characterID, int commandID,
                                     const char* indentPrefix) {
#if 1
  idBase64 base64;
...
#if 1
  // file->WriteFloatString( "%s\t\t\t\t{\t\"type\": \"Tag_DoAction\",
  // \"streamLength\": %i, \"stream\": \"%s\" }", ( commandID != 0 ) ? ",\n" :
  // "", bitstream.Length(), base64.c_str() );
  file->WriteFloatString(
      "%s\t\t\t\t{\t\"type\": \"Tag_DoAction\", \"streamLength\": %i, "
      "\"stream\": \"FIXME\" }",
      (commandID != 0) ? ",\n" : "", bitstream.Length());
#else
  ...
  idStr scriptText = actionScript->CallToScript(
      scriptObject, idSWFParmList(), file->GetName(), characterID, commandID);
  idStr quotedText = idStr::CStyleQuote(scriptText.c_str());

  file->WriteFloatString(
      "%s\t\t\t\t{\n\t\t\t\t\t\"type\": \"Tag_DoAction\", \"streamLength\": "
      "%i, \"stream\": \"%s\",\n\t\t\t\t\t\"luaCode\": %s\n\t\t\t\t}",
      (commandID != 0) ? ",\n" : "", bitstream.Length(), base64.c_str(),
      quotedText.c_str());

  // file->WriteFloatString( "%s\t<DoAction
  // streamLength=\"%i\">%s</DoAction>\n", indentPrefix, bitstream.Length(),
  // base64.c_str() );
...
void idSWFSprite::WriteSWF(idFile_SWF& f, int characterID) {
  ...
  for (int i = 0; i < doInitActions.Num(); i++) {
    tagLength += idFile_SWF::GetTagHeaderSize(Tag_DoInitAction,
                                              doInitActions[i].Length());
  ...
  for (int i = 0; i < commands.Num(); i++) {
    ...
    switch (command.tag) {
      ...
      case Tag_DoAction:
        tagLength +=
            idFile_SWF::GetTagHeaderSize(command.tag, command.stream.Length());
  ...
  for( int i = 0; i < frameLabels.Num(); i++ )
  {
          f->WriteFloatString( "%s\t<FrameLabel frameNum=\"%i\"
  frameLabel=\"%s\"/>\n", indentPrefix, frameLabels[i].frameNum,
  frameLabels[i].frameLabel.c_str() );
  }
  */
