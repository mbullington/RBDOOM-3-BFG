...
idCVar swf_debug(
    "swf_debug", "0", CVAR_INTEGER | CVAR_ARCHIVE,
    "debug swf scripts.  1 shows traces/errors.  2 also shows warnings.  3 "
    "also shows disassembly.  4 shows parameters in the disassembly.");
idCVar swf_debugInvoke("swf_debugInvoke", "0", CVAR_INTEGER,
                       "debug swf functions being called from game.");
...
========================
*/
void idSWFScriptFunction_Script::SetScope(
    idList<idSWFScriptObject*>& newScope) {
  assert(scope.Num() == 0);
  for (int i = 0; i < scope.Num(); i++) {
    if (verify(scope[i])) {
      scope[i]->Release();
...
========================
*/
idSWFScriptVar idSWFScriptFunction_Script::Call(idSWFScriptObject* thisObject,
  ...
  for (int i = 0; i < parms.Num(); i++) {
    ...
    // Unfortunately at this point we don't have the function name anymore, so
    // our warning messages aren't very detailed
    if (i < parameters.Num()) {
  ...
  if (flags & BIT(7)) {
    ...
    if (thisObject->GetSprite() != NULL &&
        thisObject->GetSprite()->parent != NULL) {
      registers[preloadReg].SetObject(
          thisObject->GetSprite()->parent->scriptObject);
    } else {
...
// RB begin
idStr idSWFScriptFunction_Script::CallToScript(idSWFScriptObject* thisObject,
                                               const idSWFParmList& parms,
                                               const char* filename,
                                               int characterID, int actionID) {
  idSWFBitStream bitstream(data, length, false);
  ...
  idStr retVal = ExportToScript(thisObject, stack, bitstream, filename,
...
========================
*/
idSWFScriptVar idSWFScriptFunction_Script::Run(idSWFScriptObject* thisObject,
                                               idSWFStack& stack,
  ...
  while (bitstream.Tell() < bitstream.Length()) {
    ...
    switch (code) {
      ...
      case Action_Push: {
        idSWFBitStream pushstream(bitstream.ReadData(recordLength),
                                  recordLength, false);
      ...
      case Action_StringAdd:
        ...
        break;
      case Action_StringExtract:
        stack.C().SetString(stack.C().ToString().Mid(stack.B().ToInteger(),
                                                     stack.A().ToInteger()));
        stack.Pop(2);
        break;
      case Action_StringLess:
      ...
      case Action_GetProperty: {
        if (verify(thisSprite != NULL)) {
          idSWFSpriteInstance* target =
              thisSprite->ResolveTarget(stack.B().ToString());
          stack.B() =
              target->scriptObject->Get(GetPropertyName(stack.A().ToInteger()));
        } else if (swf_debug.GetInteger() > 0) {
      ...
      case Action_SetProperty: {
        if (verify(thisSprite != NULL)) {
          idSWFSpriteInstance* target =
              thisSprite->ResolveTarget(stack.C().ToString());
          target->scriptObject->Set(GetPropertyName(stack.B().ToInteger()),
                                    stack.A());
        } else if (swf_debug.GetInteger() > 0) {
      ...
      case Action_GetTime:
        ...
        break;
      case Action_RandomNumber:
        assert(thisSprite && thisSprite->sprite &&
               thisSprite->sprite->GetSWF());
        stack.A().SetInteger(
            thisSprite->sprite->GetSWF()->GetRandom().RandomInt(
                stack.A().ToInteger()));
        break;
      case Action_CallFunction: {
        ...
        if (function.IsFunction() && verify(object)) {
          ...
          idLib::PrintfIf(swf_debug.GetInteger() > 0,
                          "SWF: unknown function %s\n", functionName.c_str());
          stack.Alloc().SetUndefined();
        }
      ...
      case Action_CallMethod: {
        idStr functionName = stack.A().ToString();
        // If the top stack is undefined but there is an object, it's calling
        // the constructor
        if (functionName.IsEmpty() || stack.A().IsUndefined() ||
            stack.A().IsNULL()) {
        ...
        if (stack.B().IsObject()) {
          ...
          if (!function.IsFunction()) {
            idLib::PrintfIf(swf_debug.GetInteger() > 1,
                            "SWF: unknown method %s on %s\n",
                            functionName.c_str(),
                            object->DefaultValue(true).ToString().c_str());
          }
        } else {
          idLib::PrintfIf(swf_debug.GetInteger() > 1,
                          "SWF: NULL object for method %s\n",
                          functionName.c_str());
        }
      ...
      case Action_DefineFunction: {
        ...
        idSWFScriptFunction_Script* newFunction =
            idSWFScriptFunction_Script::Alloc();
      ...
      case Action_DefineFunction2: {
        ...
        // The number of registers is from 0 to 255, although valid values are 1
        // to 256. There must always be at least one register for
        // DefineFunction2, to hold "this" or "super" when required.
        uint8 numRegs = bitstream.ReadU8() + 1;

        // Note that SWF byte-ordering causes the flag bits to be reversed
        // per-byte from how the swf_file_format_spec_v10.pdf document describes
        // the ordering in ActionDefineFunction2. PreloadThisFlag is byte 0, not
        // 7, PreloadGlobalFlag is 8, not 15.
        ...
        for (int i = 0; i < numParms; i++) {
          ...
          if (reg >= numRegs) {
            idLib::Warning(
                "SWF: Parameter %s in function %s bound to out of range "
                "register %d",
                name, functionName.c_str(), reg);
            reg = 0;
          }
      ...
      case Action_GetMember: {
        if ((stack.B().IsUndefined() || stack.B().IsNULL()) &&
            swf_debug.GetInteger() > 1) {
          idLib::Printf(
              "SWF: tried to get member %s on an invalid object in sprite "
              "'%s'\n",
              stack.A().ToString().c_str(),
              thisSprite != NULL ? thisSprite->GetName() : "");
        }
        if (stack.B().IsObject()) {
          ...
          if (propertyName.Cmp("prototype") == 0) {
            // if this is a function, it's a class definition function, and it
            // just wants the prototype object create it if it hasn't been
      ...
      case Action_NewObject: {
        ...
        if (functionName.Cmp("Array") == 0) {
          ...
          idSWFScriptFunction* baseObj = baseObjConstructor.GetFunction();
          object->Set("__proto__", baseObj->GetPrototype());
          // object prototype is not set here because it will be auto created
          // from Object later
        } else {
          idSWFParmList parms;
          parms.SetNum(stack.A().ToInteger());
          ...
          if (objdef.IsFunction()) {
            ...
            idLib::Warning("SWF: Unknown class definition %s",
                           functionName.c_str());
          }
        }
      ...
      case Action_Extends: {
        idSWFScriptFunction* superclassConstructorFunction =
            stack.A().GetFunction();
        idSWFScriptFunction* subclassConstructorFunction =
            stack.B().GetFunction();
        stack.Pop(2);

        idSWFScriptObject* scriptObject = idSWFScriptObject::Alloc();
        scriptObject->SetPrototype(
            superclassConstructorFunction->GetPrototype());
        scriptObject->Set(
            "__proto__",
            idSWFScriptVar(superclassConstructorFunction->GetPrototype()));
        scriptObject->Set("__constructor__",
                          idSWFScriptVar(superclassConstructorFunction));
      ...
      case Action_TargetPath: {
        if (!stack.A().IsObject()) {
          ...
          if (object->GetSprite() == NULL) {
            ...
            for (idSWFSpriteInstance* target = object->GetSprite()->parent;
                 target != NULL; target = target->parent) {
      ...
      case Action_With: {
        ...
        idSWFBitStream bitstream2(bitstream.ReadData(withSize), withSize,
                                  false);
        if (stack.A().IsObject()) {
      ...
      case Action_BitRShift:
        ...
        break;
      case Action_BitURShift:
        stack.B().SetInteger((uint32)stack.B().ToInteger() >>
                             stack.A().ToInteger());
        stack.Pop(1);
        break;
      case Action_BitXor:
      ...
      case Action_Delete: {
        ...
        // We no longer support deleting variables because the performance cost
        // of updating the hash tables is not worth it
        stack.Pop(2);
        break;
      }
      case Action_Delete2: {
      ...
      // These are functions we just don't support because we never really
      // needed to
      case Action_CloneSprite:
      case Action_RemoveSprite:
      case Action_Call:
      ...
      default:
        ...
        // We have to abort here because the rest of the script is basically
        // meaningless now
        assert(false);
        callstackLevel--;
        return idSWFScriptVar();
...
idStr idSWFScriptFunction_Script::BuildActionCode(
    const idList<ActionBlock>& blocks, int level) {
  idStr ret;
...
idStr idSWFScriptFunction_Script::ExportToScript(
    idSWFScriptObject* thisObject, idSWFStack& stack, idSWFBitStream& bitstream,
    const char* filename, int characterID, int actionID) {
  static int callstackLevel = -1;
  idSWFSpriteInstance* thisSprite = thisObject->GetSprite();
  idSWFSpriteInstance* currentTarget = thisSprite;
  ...
  currentBlock->line =
      va("function sprite%i_action%i()", characterID, actionID);
  ...
  while (bitstream.Tell() < bitstream.Length()) {
    ...
    switch (code) {
      ...
      case Action_Add:
        // stack.B().SetFloat( stack.B().ToFloat() + stack.A().ToFloat() );
        stack.B().SetResult(va("%s + %s", stack.B().ToString().c_str(),
                               stack.A().ToString().c_str()));
        stack.Pop(1);
        break;
      case Action_Subtract:
      ...
      case Action_StringAdd:
        ...
        break;
      case Action_StringExtract:
        stack.C().SetResult(stack.C().ToString().Mid(stack.B().ToInteger(),
                                                     stack.A().ToInteger()));
        stack.Pop(2);
        break;
      case Action_StringLess:
      ...
      case Action_CallFunction: {
        ...
        for (int i = 0; i < parms.Num(); i++) {
          ...
          if (stack.A().IsString()) {
            line += va("\"%s\"%s", stack.A().ToString().c_str(),
                       (i < (parms.Num() - 1)) ? ", " : " ");
          } else {
            line += va("%s%s", stack.A().ToString().c_str(),
                       (i < (parms.Num() - 1)) ? ", " : " ");
      ...
      case Action_CallMethod: {
        ...
        idStr line =
            va("%s.%s( ", stack.B().ToString().c_str(), functionName.c_str());
      ...
      case Action_GetMember: {
        ...
        else if( stack.B().IsFunction() )
        {
                ...
                if( propertyName.Cmp( "prototype" ) == 0 )
                {
                        // if this is a function, it's a class definition
        function, and it just wants the prototype object
                        ...
                        {
                                object = idSWFScriptObject::Alloc();
                                // Set the __proto__ to the main Object
        prototype idSWFScriptVar baseObjConstructor = scope[0]->Get( "Object" );
                                idSWFScriptFunction* baseObj =
        baseObjConstructor.GetFunction(); object->Set( "__proto__",
        baseObj->GetPrototype() ); sfs->SetPrototype( object );
                        }
        ...
        */
        ...
        // line = va( "%s.%s", stack.B().ToString().c_str(),
        // stack.A().ToString().c_str() );
      ...
      case Action_ToNumber:
        if (stack.A().IsString()) {
          stack.A().SetResult(
              va("tonumber( \"%s\" )", stack.A().ToString().c_str()));
        } else {
          stack.A().SetResult(
              va("tonumber( %s )", stack.A().ToString().c_str()));
        }
      ...
      case Action_TypeOf:
        if (stack.A().IsString()) {
          stack.A().SetResult(
              va("type( \"%s\" )", stack.A().ToString().c_str()));
        } else {
