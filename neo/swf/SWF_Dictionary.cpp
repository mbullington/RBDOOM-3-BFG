...
========================
*/
idSWFDictionaryEntry& idSWFDictionaryEntry::operator=(
...
========================
*/
idSWFDictionaryEntry* idSWF::AddDictionaryEntry(int characterID,
                                                swfDictType_t type) {
...
idSWFDictionaryEntry* idSWF::FindDictionaryEntry(int characterID,
                                                 swfDictType_t type) {
  if (dictionary.Num() < characterID + 1) {
    idLib::Warning("%s: Could not find character %d", filename.c_str(),
                   characterID);
    return NULL;
  }

  if (dictionary[characterID].type != type) {
    idLib::Warning("%s: Character %d is the wrong type", filename.c_str(),
                   characterID);
    return NULL;
  }
