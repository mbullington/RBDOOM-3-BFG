...
class idMultiplayerGame {
 public:
  ...
  void PlayGlobalSound(int toPlayerNum, snd_evt_t evt,
                       const char* shader = NULL);
  void PlayTeamSound(int toTeam, snd_evt_t evt,
  ...
  void ProcessChatMessage(int clientNum, bool team, const char* name,
                          const char* text, const char* sound);
  ...
  void GetSpectateText(idPlayer* player, idStr spectatetext[2],
                       bool scoreboard);
 ...
 private:
  ...
  void UpdateRankColor(idUserInterface* gui, const char* mask, int i,
                       const idVec3& vec);
