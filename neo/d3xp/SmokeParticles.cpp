...
bool idSmokeParticles::EmitSmoke(const idDeclParticle* smoke,
                                 ...
                                 const float diversity, const idVec3& origin,
                                 const idMat3& axis, int timeGroup /*_D3XP*/) {
  ...
  for (int stageNum = 0; stageNum < smoke->stages.Num(); stageNum++) {
    ...
    if (finalParticleTime == 0) {
      ...
      prevCount = floor(
          ((float)(deltaMsec - (gameLocal.time - gameLocal.previousTime)) /
           finalParticleTime) *
...
bool idSmokeParticles::UpdateRenderEntity(renderEntity_s* renderEntity,
  ...
  if (renderView->time[renderEntity->timeGroup] == currentParticleTime &&
