...
================
*/
void idMover::Event_PostRestore(int start, int total, int accel, int decel,
                                int useSplineAng) {
...
void idMover::Spawn() {
  ...
  physicsObj.SetClipModel(new (TAG_PHYSICS_CLIP_MOVER)
...
============
*/
void idMover::Killed(idEntity* inflictor, idEntity* attacker, int damage,
...
==============================
*/
void idMover::ClientThink(const int curTime, const float fraction,
                          const bool predict) {
  // HACK. because I'm not sure all the other stuff this will screw up.
  // There was a reason we weren't fully interpolating movers ( Which would
  // evaluate bound objects ). I just cant remember what it was.

  // Evaluating the Team will update the parts that bound to the entity.
  // but because we interpolate the master, we don't want to run evaluate on the
  // mover itself. sending in true to the interpolatePhysicsOnly will run the
  // TeamChain Evaluate, but only on Objects bound to the entity.
  if (this->name == "blueshotty_door" || this->name == "redshotty_door" ||
      this->name == "Red_blastshield_mover" ||
      this->name == "Blue_blastshield_mover") {
...
================
*/
void idMover::Event_FindGuiTargets() { FindGuiTargets(); }
...
================
*/
void idMover::Event_MoveToPos(idVec3& pos) { MoveToPos(pos); }
...
================
*/
void idMover::Event_EnableSplineAngles() { useSplineAngles = true; }
...
================
*/
void idMover::Event_DisableSplineAngles() { useSplineAngles = false; }
...
================
*/
void idMover_Binary::Enable(bool b) { enabled = b; }
...
================
*/
void idMover_Binary::Event_FindGuiTargets() { FindGuiTargets(); }
...
================
*/
void idMover_Binary::InitSpeed(idVec3& mpos1, idVec3& mpos2, float mspeed,
                               float maccelTime, float mdecelTime) {
  idVec3 move;
  float distance;
  float speed;
...
================
*/
void idMover_Binary::InitTime(idVec3& mpos1, idVec3& mpos2, float mtime,
                              float maccelTime, float mdecelTime) {
  pos1 = mpos1;
  pos2 = mpos2;
...
================
*/
bool idMover_Binary::IsBlocked() { return blocked; }
...
void idDoor::Spawn() {
  ...
  spawnArgs.GetString("requires", "", doesReq);
...
================
*/
void idDoor::ClientThink(const int curTime, const float fraction,
                         const bool predict) {
  idVec3 masterOrigin;
  idMat3 masterAxis;
...
================
*/
void idDoor::PreBind() { idMover_Binary::PreBind(); }
...
void idDoor::Use(idEntity* other, idEntity* activator) {
  if (gameLocal.RequirementMet(activator, doesReq, removeItem)) {
    if (syncLock.Length()) {
...
================
*/
void idDoor::Open() { GotoPosition2(); }
...
================
*/
void idDoor::Close() { GotoPosition1(); }
...
================
*/
int idDoor::IsLocked() { return spawnArgs.GetInt("locked"); }
...
================
*/
bool idDoor::IsOpen() { return (moverState != MOVER_POS1); }
...
================
*/
bool idDoor::IsNoTouch() { return noTouch; }
...
===============
*/
void idDoor::SetCompanion(idDoor* door) { companionDoor = door; }
...
================
*/
void idDoor::Event_Lock(int f) { Lock(f); }
...
void idPlat::RunPhysics_NoBlocking() {
  ...
  will handle everything if ( teamMaster && teamMaster != this ) { return false;
  }
  */
...
================
*/
void idPlat::ClientThink(const int curTime, const float fraction,
                         const bool predict) {
  InterpolatePhysicsOnly(fraction);
  ...
  // Dont bother with blocking entities on clients.. host tells us our move
  state. RunPhysics_NoBlocking();
  ...
  if ( thinkFlags & TH_PHYSICS ) {
          ...
          if ( GetMasterPosition( masterOrigin, masterAxis ) ) {
                  if ( trigger ) {
                          trigger->Link( gameLocal.clip, this, 0, masterOrigin +
  localTriggerOrigin * masterAxis, localTriggerAxis * masterAxis );
                  }
          }
  }
  */
...
================
*/
void idPlat::PreBind() { idMover_Binary::PreBind(); }
...
void idPlat::SpawnPlatTrigger(idVec3& pos) {
  ...
  trigger = new (TAG_PHYSICS_CLIP_MOVER)
      idClipModel(idTraceModel(idBounds(tmin, tmax)));
  trigger->Link(gameLocal.clip, this, 255, GetPhysics()->GetOrigin(),
                mat3_identity);
  trigger->SetContents(CONTENTS_TRIGGER);
}
...
===============
*/
idRotater::idRotater() { activatedBy = this; }
...
===============
*/
void idRotater::Save(idSaveGame* savefile) const { activatedBy.Save(savefile); }
