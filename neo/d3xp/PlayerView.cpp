...
==============
*/
idPlayerView::~idPlayerView() { delete fxManager; }
/*
==============
idPlayerView::Save
==============
...
===================
*/
idMat3 idPlayerView::ShakeAxis() const { return shakeAng.ToMat3(); }
...
void idPlayerView::SingleView(const renderView_t* view,
  ...
  gameSoundWorld->PlaceListener(view->vieworg, view->viewaxis,
  ...
  if (!pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool()) {
    if (!player->spectating) {
      for (int i = 0; i < MAX_SCREEN_BLOBS; i++) {
        ...
        float fade = (float)(blob->finishTime - gameLocal.fast.time) /
    ...
    if (armorPulse > 0.0f && armorPulse < 1.0f) {
      ...
      renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                   renderSystem->GetVirtualHeight(), 0.0f, 0.0f,
                                   1.0f, 1.0f, armorMaterial);
    }

    // tunnel vision
    /*
    RB: disabled tunnel vision because the renderer converts colors set by
    SetColor4 to bytes which are clamped to [0,255] so materials that want to
    access float values greater than 1 with parm0 - parm3 are always broken
    ...
    if( alpha < 1.0f )
    {
            renderSystem->SetColor4( ( player->health <= 0.0f ) ? MS2SEC(
    gameLocal.slow.time ) : lastDamageTime, 1.0f, 1.0f, ( player->health <= 0.0f
    ) ? 0.0f : alpha ); renderSystem->DrawStretchPic( 0.0f, 0.0f,
    renderSystem->GetVirtualWidth(), renderSystem->GetVirtualHeight(), 0.0f,
    0.0f, 1.0f, 1.0f, tunnelMaterial );
    }
    RB end
    */
...
idCVar stereoRender_interOccularCentimeters(
    "stereoRender_interOccularCentimeters", "3.0", CVAR_ARCHIVE | CVAR_RENDERER,
    "Distance between eyes");
idCVar stereoRender_convergence(
    "stereoRender_convergence", "6", CVAR_RENDERER,
    "0 = head mounted display, otherwise world units to convergence plane");
...
float CentimetersToInches(const float cm) { return cm / 2.54f; }
...
float CalculateWorldSeparation(const float screenSeparation,
...
stereoDistances_t CaclulateStereoDistances(
    const float interOcularCentimeters,  // distance between two eyes,
                                         // typically 6.0 - 7.0
    ...
    const float fov_x_degrees)  // edge to edge horizontal field of view,
                                // typically 60 - 90
{
  ...
  // 3DTV mode
  dists.screenSeparation =
      0.5f * interOcularCentimeters / screenWidthCentimeters;
  dists.worldSeparation = CalculateWorldSeparation(
      dists.screenSeparation, convergenceWorldUnits, fov_x_degrees);
...
float GetScreenSeparationForGuis() {
  const stereoDistances_t dists = CaclulateStereoDistances(
      ...
      stereoRender_convergence.GetFloat(), 80.0f /* fov */);
...
===================
*/
void idPlayerView::EmitStereoEyeView(const int eye,
                                     idMenuHandler_HUD* hudManager) {
  ...
  const stereoDistances_t dists = CaclulateStereoDistances(
      stereoRender_interOccularCentimeters.GetFloat(),
      renderSystem->GetPhysicalScreenWidthInCentimeters(),
      stereoRender_convergence.GetFloat(), view->fov_x);
...
===================
*/
int idPlayerView::AddWarp(idVec3 worldOrigin, float centerx, float centery,
                          float initialRadius, float durationMsec) {
...
==================
*/
void FullscreenFX::Save(idSaveGame* savefile) { fader.Save(savefile); }
...
==================
*/
void FullscreenFX::Restore(idRestoreGame* savefile) { fader.Restore(savefile); }
...
void FullscreenFX_Helltime::Initialize() {
  ...
  captureMaterials[0] =
      declManager->FindMaterial("textures/d3bfg/bloodorb1/capture");
  captureMaterials[1] =
      declManager->FindMaterial("textures/d3bfg/bloodorb2/capture");
  captureMaterials[2] =
      declManager->FindMaterial("textures/d3bfg/bloodorb3/capture");
...
void FullscreenFX_Helltime::AccumPass(const renderView_t* view) {
  ...
  if (clearAccumBuffer) {
    ...
    renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), 0.0f, t0,
                                 1.0f, t1, initMaterial);
  } else {
    renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), 0.0f, t0,
                                 1.0f, t1, captureMaterials[level]);
  }
}
...
void FullscreenFX_Helltime::HighQuality() {
  ...
  renderSystem->SetColor4(1.0f, 1.0f, 1.0f, 1.0f);
  renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                               renderSystem->GetVirtualHeight(), 0.0f, t0, 1.0f,
                               t1, drawMaterial);
}
...
void FullscreenFX_Multiplayer::Initialize() {
  ...
  captureMaterial =
      declManager->FindMaterial("textures/d3bfg/multiplayer/capture");
  drawMaterial = declManager->FindMaterial("textures/d3bfg/bloodorb/draw");
  clearAccumBuffer = true;
}
...
void FullscreenFX_EnviroSuit::HighQuality() {
  ...
  float t1 = 0.0f;
  renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                               renderSystem->GetVirtualHeight(), s0, t0, s1, t1,
                               material);
}
...
bool FullscreenFX_InfluenceVision::Active() {
  ...
  if (player != NULL &&
      (player->GetInfluenceMaterial() || player->GetInfluenceEntity())) {
    return true;
  }
...
void FullscreenFX_InfluenceVision::HighQuality() {
  ...
  if (player->GetInfluenceMaterial()) {
    ...
    renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), 0.0f, 0.0f,
                                 1.0f, 1.0f, player->GetInfluenceMaterial());
...
void FullscreenFX_Bloom::HighQuality() {
  ...
  for (int i = 0; i < num; i++) {
    ...
    renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), s1,
                                 t2 * yScale, s2, t1 * yScale, drawMaterial);
...
==================
*/
FullscreenFXManager::~FullscreenFXManager() { fx.DeleteContents(); }
...
void FullscreenFXManager::Blendback(float alpha) {
  ...
  if (alpha < 1.f) {
    ...
    float t1 = 0.0f;
    renderSystem->DrawStretchPic(0.0f, 0.0f, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), s0, t0, s1,
                                 t1, blendBackMaterial);
  }
}
...
idCVar player_allowScreenFXInStereo("player_allowScreenFXInStereo", "1",
                                    CVAR_BOOL,
                                    "allow full screen fx in stereo mode");
