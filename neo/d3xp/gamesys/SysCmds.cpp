...
CONSOLE_COMMAND(reloadScript2,
                "Doesn't thow an error...  Use this when switching game modes",
                0) {
  // shutdown the map because entities may point to script objects
  gameLocal.MapShutdown();
...
void Cmd_Give_f(const idCmdArgs& args) {
  ...
  if (give_all || idStr::Icmp(name, "ammo") == 0) {
    for (i = 0; i < AMMO_NUMTYPES; i++) {
      player->inventory.SetInventoryAmmoForType(
          i, player->inventory.MaxAmmoForAmmoClass(
  ...
  if (idStr::Icmp(name, "pda") == 0) {
    if (args.Argc() == 2) {
      ...
      for (int i = 0; i < declManager->GetNumDecls(DECL_PDA); i++) {
        player->GivePDA(static_cast<const idDeclPDA*>(
                            declManager->DeclByIndex(DECL_PDA, i)),
                        NULL);
      }
    } else {
      const idDeclPDA* pda = static_cast<const idDeclPDA*>(
          declManager->FindType(DECL_PDA, args.Argv(2), false));
      if (pda == NULL) {
  ...
  if (idStr::Icmp(name, "video") == 0) {
    const idDeclVideo* video = static_cast<const idDeclVideo*>(
        declManager->FindType(DECL_VIDEO, args.Argv(2), false));
    if (video == NULL) {
...
static void Cmd_Say(bool team, const idCmdArgs& args) {
  ...
  if (common->IsServer() && gameLocal.GetLocalClientNum() == -1) {
    ...
    name = session->GetActingGameStateLobbyBase().GetLobbyUserName(
        gameLocal.lobbyUserIDs[gameLocal.GetLocalClientNum()]);
  ...
  if (common->IsClient()) {
    ...
    session->GetActingGameStateLobbyBase().SendReliableToHost(
        team ? GAME_RELIABLE_MESSAGE_TCHAT : GAME_RELIABLE_MESSAGE_CHAT,
        outMsg);
  } else {
    gameLocal.mpGame.ProcessChatMessage(gameLocal.GetLocalClientNum(), team,
                                        name, text, NULL);
  }
}
...
==================
*/
static void Cmd_Say_f(const idCmdArgs& args) { Cmd_Say(false, args); }
...
==================
*/
static void Cmd_SayTeam_f(const idCmdArgs& args) { Cmd_Say(true, args); }
...
static void Cmd_RecordViewNotes_f(const idCmdArgs& args) {
  ...
  // TODO_SPARTY: removed old hud need to find a way of doing this with the new
  // hud
...
static void Cmd_ShowViewNotes_f(const idCmdArgs& args) {
  ...
  if (parser.ExpectTokenString("view") &&
    ...
    // TODO_SPARTY: removed old hud need to find a way of doing this with the
    // new hud
...
=================
*/
bool FindEntityGUIs(idEntity* ent, const modelSurface_t** surfaces,
                    int maxSurfs, int& guiSurfaces) {
...
void Cmd_TestId_f(const idCmdArgs& args) {
  ...
  gameLocal.mpGame.AddChatLine(idLocalization::GetString(id), "<nothing>",
                               "<nothing>", "<nothing>");
}
...
void idGameLocal::InitConsoleCommands() {
  ...
  // RB begin
  cmdSystem->AddCommand("exportScriptEvents", idClass::ExportScriptEvents_f,
                        CMD_FL_GAME | CMD_FL_TOOL,
                        "update script/doom_events.script");
  cmdSystem->AddCommand("editLights", idClass::EditLights_f,
                        CMD_FL_GAME | CMD_FL_TOOL,
                        "launches the in-game Light Editor");
  cmdSystem->AddCommand(
      "saveEnvprobes", Cmd_SaveEnvprobes_f, CMD_FL_GAME | CMD_FL_CHEAT,
      "saves all autogenerated env_probes to a .extras_env_probes.map file");
  // RB end

  // multiplayer client commands ( replaces old impulses stuff )
  // cmdSystem->AddCommand( "clientDropWeapon",
  // idMultiplayerGame::DropWeapon_f, CMD_FL_GAME,			"drop current weapon"
  // );
  ...
  //idMultiplayerGame::Vote_f,	CMD_FL_GAME,				"cast your vote:
  //clientVote yes | no" ); 	cmdSystem->AddCommand( "clientCallVote",
  //idMultiplayerGame::CallVote_f,	CMD_FL_GAME,			"call a vote:
  //clientCallVote si_.. proposed_value" );
