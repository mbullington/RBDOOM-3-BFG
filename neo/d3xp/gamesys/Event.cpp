...
================
*/
idEventDef::idEventDef(const char* command, const char* formatspec,
                       char returnType) {
  idEventDef* ev;
  int i;
  unsigned int bits;
...
================
*/
int idEventDef::NumEventCommands() { return numEventDefs; }
...
void idEvent::CopyArgs(const idEventDef* evdef, int numargs, va_list args,
...
void idEvent::Restore(idRestoreGame* savefile) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (argsize != (int)event->eventdef->GetArgSize()) {
      ...
      savefile->Error(
          "idEvent::Restore: arg size (%zd) doesn't match saved arg size(%zd) "
          "on event '%s'",
    ...
    if (argsize) {
      ...
      for (j = 0, size = 0; j < event->eventdef->GetNumArgs(); ++j) {
        ...
        switch (format[j]) {
          ...
          case D_EVENT_ENTITY_NULL:
            // RB: 64 bit fix, changed alignment to sizeof( intptr_t )
            reinterpret_cast<idEntityPtr<idEntity>*>(dataPtr)->Restore(
                savefile);
            size += sizeof(intptr_t);
            // RB end
            break;
