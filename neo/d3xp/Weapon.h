...
class idWeapon : public idAnimatedEntity {
 public:
  ...
  bool GetGlobalJointTransform(bool viewModel, const jointHandle_t jointHandle,
  ...
  idEntity* DropItem(const idVec3& velocity, int activateDelay, int removeDelay,
                     bool died);
  bool CanDrop() const;
  void WeaponStolen();
 ...
 private:
  ...
  // nozzle effects
  bool
      nozzleFx;  // does this use nozzle effects ( parm5 at rest, parm6 firing )
  ...
  void Event_LaunchProjectiles(int num_projectiles, float spread,
  ...
  void Event_LaunchProjectilesEllipse(int num_projectiles, float spreada,
                                      float spreadb, float fuseOffset,
                                      float power);
