...
const int idMultiplayerGame::numSkins = sizeof(idMultiplayerGame::skinNames) /
                                        sizeof(idMultiplayerGame::skinNames[0]);
...
const char* gameTypeNames_WithCTF[] = {"Deathmatch", "Tourney", "Team DM",
                                       "Last Man",   "CTF",     ""};
const char* gameTypeDisplayNames_WithCTF[] = {"#str_04260",        "#str_04261",
                                              "#str_04262",        "#str_04263",
                                              "#str_swf_mode_ctf", ""};

const char* gameTypeNames_WithoutCTF[] = {"Deathmatch", "Tourney", "Team DM",
                                          "Last Man", ""};
const char* gameTypeDisplayNames_WithoutCTF[] = {
    "#str_04260", "#str_04261", "#str_04262", "#str_04263", ""};
...
idCVar g_spectatorChat("g_spectatorChat", "0",
...
void idMultiplayerGame::SpawnPlayer(int clientNum) {
  ...
  AddChatLine(idLocalization::GetString("#str_07177"),
              lobby.GetLobbyUserName(lobbyUserID));

  memset(&playerState[clientNum], 0, sizeof(playerState[clientNum]));
  if (!common->IsClient()) {
    if (gameLocal.gameType == GAME_LASTMAN) {
      // Players spawn with no lives to prevent them from getting the clean
      // slate achievement if they didn't earn it.
...
================
*/
void idMultiplayerGame::UpdateRankColor(idUserInterface* gui, const char* mask,
                                        int i, const idVec3& vec) {
  for (int j = 1; j < 4; j++) {
...
================
*/
void idMultiplayerGame::UpdateScoreboard(idMenuHandler_Scoreboard* scoreboard,
                                         idPlayer* owner) {
  if (owner == NULL) {
    return;
  }
  ...
  for (int i = 0; i < MAX_CLIENTS; ++i) {
    if (i < gameLocal.numClients) {
      ...
      score = idMath::ClampInt(MP_PLAYER_MINFRAGS, MP_PLAYER_MAXFRAGS,
                               playerState[playerNum].fragCount);
      ...
      wins =
          idMath::ClampInt(0, MP_PLAYER_MAXWINS, playerState[playerNum].wins);
      ...
      if (gameState == WARMUP) {
        ...
        if (gameLocal.gameType == GAME_LASTMAN &&
            playerState[playerNum].fragCount == LASTMAN_NOLIVES) {
      ...
      info.voiceState = session->GetDisplayStateFromVoiceState(
          session->GetLobbyUserVoiceState(lobbyUserID));
  ...
  if (gameState == GAMEREVIEW) {
    ...
    if (gameLocal.gameType == GAME_LASTMAN) {
      if (gameState == GAMEON || gameState == SUDDENDEATH) {
        gameInfo = va("%s: %i", idLocalization::GetString("#str_04264"),
                      startFragLimit);
      } else {
        gameInfo = va("%s: %i", idLocalization::GetString("#str_04264"),
      ...
      gameInfo = va("%s: %i", idLocalization::GetString("#str_01982"),
                    gameLocal.serverInfo.GetInt("si_fragLimit"));
    }

    if (gameLocal.serverInfo.GetInt("si_timeLimit") > 0) {
      gameInfo.Append(va("        %s: %i",
                         idLocalization::GetString("#str_01983"),
...
const char* idMultiplayerGame::GameTime() {
  ...
  if (gameState == COUNTDOWN) {
    ...
    if (ms <= 0) {
      ...
      warmupEndTime = gameLocal.serverTime +
                      1000 * cvarSystem->GetCVarInteger("g_countDown");
      ms = warmupEndTime - gameLocal.serverTime;
    }
...
================
*/
void idMultiplayerGame::PlayerDeath(idPlayer* dead, idPlayer* killer,
                                    bool telefrag) {
  ...
  if (dead == NULL) {
    idLib::Warning(
        "idMultiplayerGame::PlayerDeath dead ptr == NULL, kill will not count");
    return;
  }
  ...
  if (killer && killer == dead) {
    ...
    if (telefrag) {
      killer->GetAchievementManager().EventCompletesAchievement(
          ACHIEVEMENT_MP_KILL_PLAYER_VIA_TELEPORT);
      PrintMessageEvent(MSG_TELEFRAGGED, dead->entityNumber,
                        killer->entityNumber);
    } else if (IsGametypeTeamBased() && dead->team == killer->team) /* CTF */
    {
      PrintMessageEvent(MSG_KILLEDTEAM, dead->entityNumber,
                        killer->entityNumber);
    } else {
      if (killer->PowerUpActive(INVISIBILITY)) {
        killer->GetAchievementManager().EventCompletesAchievement(
            ACHIEVEMENT_MP_KILL_5_PLAYERS_USING_INVIS);
      }
      if (killer->PowerUpActive(BERSERK)) {
        killer->GetAchievementManager().EventCompletesAchievement(
            ACHIEVEMENT_MP_USE_BERSERK_TO_KILL_PLAYER);
      }
  ...
  switch (news) {
    case GAMEON: {
      ...
      session->GetActingGameStateLobbyBase().SendReliable(
          GAME_RELIABLE_MESSAGE_RESTART, outMsg, false);
      ...
      matchStartedTimeMsg.InitWrite(matchStartedTimeMsgBuf,
                                    sizeof(matchStartedTimeMsgBuf));
      matchStartedTimeMsg.WriteLong(matchStartedTime);
      session->GetActingGameStateLobbyBase().SendReliable(
          GAME_RELIABLE_MESSAGE_MATCH_STARTED_TIME, matchStartedTimeMsg, false);
    ...
    case COUNTDOWN: {
      ...
      session->GetActingGameStateLobbyBase().SendReliable(
          GAME_RELIABLE_MESSAGE_WARMUPTIME, outMsg, false);
...
Called on both servers and clients once when the game state changes to
COUNTDOWN.
================
*/
...
void idMultiplayerGame::FillTourneySlots() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < gameLocal.numClients; j++) {
      ...
      if (p->wantSpectate) {
        idLib::Printf(
            "FillTourneySlots: Skipping Player %d ( Wants Spectate )\n",
            p->entityNumber);
        continue;
      }
...
void idMultiplayerGame::UpdateTourneyLine() {
  ...
  for (int j = 1; j <= gameLocal.numClients; j++) {
    ...
    session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
        gameLocal.lobbyUserIDs[imax], GAME_RELIABLE_MESSAGE_TOURNEYLINE,
        outMsg);
...
================
*/
bool idMultiplayerGame::Warmup() { return (gameState == WARMUP); }
...
void idMultiplayerGame::GameHasBeenWon() {
  ...
  if (matchParameters.matchFlags & MATCH_RANKED) {
    ...
    for (int playerIdx = 0; playerIdx < gameLocal.numClients; playerIdx++) {
      leaderboardStats_t stats = {
          playerState[playerIdx].fragCount, playerState[playerIdx].wins,
          playerState[playerIdx].teamFragCount, playerState[playerIdx].deaths};

      LeaderboardLocal_Upload(gameLocal.lobbyUserIDs[playerIdx],
                              gameLocal.gameType, stats);
    }

    // Flush all the collectively queued leaderboards all at once. ( Otherwise
    // you get a busy signal on the second "flush" )
  ...
  for (int i = 0; i < gameLocal.numClients; i++) {
    ...
    if (playerState[i].deaths == 0) {
      player->GetAchievementManager().EventCompletesAchievement(
          ACHIEVEMENT_MP_COMPLETE_MATCH_WITHOUT_DYING);
    }
  }
}
  ...
  switch (gameState) {
    ...
    case WARMUP: {
      if (EnoughClientsToPlay()) {
        NewState(COUNTDOWN);
        nextState = GAMEON;
        nextStateSwitch = gameLocal.serverTime +
                          1000 * cvarSystem->GetCVarInteger("g_countDown");
      }
      one = two = three = false;
      break;
    ...
    case GAMEON: {
      ...
      if (player) {
        ...
        if (gameLocal.serverTime > fragLimitTimeout) {
          ...
          PrintMessageEvent(MSG_FRAGLIMIT, IsGametypeTeamBased()
                                               ? player->team
                                               : player->entityNumber);
          GameHasBeenWon();
        }
...
bool idMultiplayerGame::Draw(int clientNum) {
  ...
  if (scoreboardManager != NULL) {
    scoreboardManager->UpdateSpectating(spectatetext[0].c_str(),
                                        spectatetext[1].c_str());
  }
...
================
*/
void idMultiplayerGame::GetSpectateText(idPlayer* player, idStr spectatetext[2],
                                        bool scoreboard) {
  if (!player->spectating) {
    return;
  }
  if (player->wantSpectate && !scoreboard) {
    if (gameLocal.gameType == GAME_LASTMAN && gameState == GAMEON) {
      // If we're in GAMEON in lastman, you can't actully spawn, or you'll have
      // an unfair advantage with more lives than everyone else.
      ...
      if (gameLocal.gameType == GAME_TOURNEY &&
          ...
          (currentTourneyPlayer[0] != player->entityNumber &&
           currentTourneyPlayer[1] != player->entityNumber)) {
        spectatetext[0] = idLocalization::GetString("#str_04246");
        switch (player->tourneyLine) {
          ...
          case 2:
            ...
            break;
          default:
            spectatetext[0] += va(idLocalization::GetString("#str_07006"),
                                  player->tourneyLine);
            break;
        }
    ...
    if (gameLocal.gameType == GAME_TOURNEY) {
      ...
      switch (player->tourneyLine) {
        ...
        default:
          spectatetext[0] +=
              va(idLocalization::GetString("#str_07006"), player->tourneyLine);
          break;
      }
    } else if (gameLocal.gameType == GAME_LASTMAN) {
  ...
  if (player->spectator != player->entityNumber) {
    idLobbyBase& lobby = session->GetActingGameStateLobbyBase();
    spectatetext[1] =
        va(idLocalization::GetString("#str_07008"),
           lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[player->spectator]));
  }
}
    ...
    if (gameState == GAMEON || gameState == COUNTDOWN || gameState == WARMUP) {
      if (IsGametypeTeamBased() && !IsGametypeFlagBased()) {
        for (int i = 0; i < gameLocal.numClients; ++i) {
          ...
          idPlayer* player = static_cast<idPlayer*>(ent);
          hud->SetTeamScore(player->team,
                            playerState[player->entityNumber].teamFragCount);
        }
      }
    }
...
void idMultiplayerGame::SetScoreboardActive(bool active) {
  if (scoreboardManager != NULL) {
    if (active) {
      if (IsGametypeTeamBased() || IsGametypeFlagBased()) {
        scoreboardManager->SetActivationScreen(SCOREBOARD_AREA_TEAM,
                                               MENU_TRANSITION_SIMPLE);
      } else {
        scoreboardManager->SetActivationScreen(SCOREBOARD_AREA_DEFAULT,
                                               MENU_TRANSITION_SIMPLE);
      }
    ...
    if (chatDataUpdated) {
      ...
      while (i < chatHistoryIndex) {
        player->AddChatMessage(
            j, Min(4, (int)chatHistory[i % NUM_CHAT_NOTIFY].fade),
...
void idMultiplayerGame::WriteToSnapshot(idBitMsg& msg) const {
  ...
  // This is a hack - I need a place to read the lobby ids before the player
  // entities are read (SpawnPlayer requires a valid lobby id for the player).
...
================
*/
void idMultiplayerGame::PlayGlobalSound(int toPlayerNum, snd_evt_t evt,
                                        const char* shader) {
  ...
  if (!common->IsClient() && toPlayerNum != gameLocal.GetLocalClientNum()) {
    ...
    if (shader) {
      ...
      outMsg.WriteLong(
          gameLocal.ServerRemapDecl(-1, DECL_SOUND, shaderDecl->Index()));
    ...
    if (toPlayerNum >= 0) {
      session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
          gameLocal.lobbyUserIDs[toPlayerNum], type, outMsg);
    } else {
...
================
*/
void idMultiplayerGame::PlayTeamSound(int toTeam, snd_evt_t evt,
                                      const char* shader) {
  for (int i = 0; i < gameLocal.numClients; i++) {
    idEntity* ent = gameLocal.entities[i];
    if (!ent || !ent->IsType(idPlayer::Type)) {
...
void idMultiplayerGame::PrintMessageEvent(msg_evt_t evt, int parm1, int parm2) {
  ...
  switch (evt) {
    case MSG_LEFTGAME:
      assert(parm1 >= 0);
      AddChatLine(idLocalization::GetString("#str_11604"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      break;
    case MSG_SUICIDE:
      assert(parm1 >= 0);
      AddChatLine(idLocalization::GetString("#str_04293"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      break;
    case MSG_KILLED:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(idLocalization::GetString("#str_04292"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));
      break;
    case MSG_KILLEDTEAM:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(idLocalization::GetString("#str_04291"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));
      break;
    case MSG_TELEFRAGGED:
      assert(parm1 >= 0 && parm2 >= 0);
      AddChatLine(idLocalization::GetString("#str_04290"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));
      break;
    case MSG_DIED:
      assert(parm1 >= 0);
      AddChatLine(idLocalization::GetString("#str_04289"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      break;
    case MSG_SUDDENDEATH:
      ...
      break;
    case MSG_JOINEDSPEC:
      AddChatLine(idLocalization::GetString("#str_04285"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      break;
    case MSG_TIMELIMIT:
    ...
    case MSG_FRAGLIMIT:
      if (gameLocal.gameType == GAME_LASTMAN) {
        AddChatLine(idLocalization::GetString("#str_04283"),
                    lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      } else if (IsGametypeTeamBased()) /* CTF */
      {
        AddChatLine(idLocalization::GetString("#str_04282"),
                    idLocalization::GetString(teamNames[parm1]));
      } else {
        AddChatLine(idLocalization::GetString("#str_04281"),
                    lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]));
      }
      break;
    case MSG_JOINTEAM:
      AddChatLine(idLocalization::GetString("#str_04280"),
                  lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm1]),
                  parm2 ? idLocalization::GetString("#str_02500")
                        : idLocalization::GetString("#str_02499"));
      break;
    case MSG_HOLYSHIT:
      AddChatLine(idLocalization::GetString("#str_06732"));
      break;
    case MSG_POINTLIMIT:
      AddChatLine(idLocalization::GetString("#str_11100"),
                  parm1 ? idLocalization::GetString("#str_11110")
                        : idLocalization::GetString("#str_11111"));
      break;
    ...
    case MSG_FLAGTAKEN:
      ...
      if (gameLocal.GetLocalPlayer()->team != parm1) {
        AddChatLine(idLocalization::GetString("#str_11101"),
                    lobby.GetLobbyUserName(
                        gameLocal.lobbyUserIDs[parm2]));  // your team
      } else {
        AddChatLine(
            idLocalization::GetString("#str_11102"),
            lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));  // enemy
      }
      break;
    ...
    case MSG_FLAGRETURN:
      ...
      if (parm2 >= 0 && parm2 < MAX_CLIENTS) {
        if (gameLocal.GetLocalPlayer()->team != parm1) {
          AddChatLine(idLocalization::GetString("#str_11120"),
                      lobby.GetLobbyUserName(
                          gameLocal.lobbyUserIDs[parm2]));  // your team
        } else {
          AddChatLine(
              idLocalization::GetString("#str_11121"),
              lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));  // enemy
        }
      } else {
        AddChatLine(idLocalization::GetString("#str_11105"),
                    parm1 ? idLocalization::GetString("#str_11110")
                          : idLocalization::GetString("#str_11111"));
      }
      break;
    ...
    case MSG_FLAGCAPTURE:
      ...
      if (gameLocal.GetLocalPlayer()->team != parm1) {
        AddChatLine(idLocalization::GetString("#str_11122"),
                    lobby.GetLobbyUserName(
                        gameLocal.lobbyUserIDs[parm2]));  // your team
      } else {
        AddChatLine(
            idLocalization::GetString("#str_11123"),
            lobby.GetLobbyUserName(gameLocal.lobbyUserIDs[parm2]));  // enemy
      }

      //			AddChatLine( idLocalization::GetString(
      //"#str_11106" ), parm1 ? idLocalization::GetString( "#str_11110" ) :
      //idLocalization::GetString( "#str_11111" ) );
      break;
  ...
  if (!common->IsClient()) {
    ...
    session->GetActingGameStateLobbyBase().SendReliable(
        GAME_RELIABLE_MESSAGE_DB, outMsg, false);
  }
}
        ...
        if (gameLocal.gameType == GAME_DM ||  // CTF : 3wave sboily, was DM
          ...
          if (i == currentTourneyPlayer[0] || i == currentTourneyPlayer[1]) {
            ...
            if (i == currentTourneyPlayer[0] || i == currentTourneyPlayer[1]) {
              p->ServerSpectate(false);
              idLib::Printf(
                  "TOURNEY CheckRespawns WARMUP :> Player %d On Deck \n",
                  p->entityNumber);
            }
          }
        } else if (gameLocal.gameType == GAME_LASTMAN) {
...
void idMultiplayerGame::DropWeapon_f(const idCmdArgs& args) {
  ...
  idBitMsg outMsg;
  session->GetActingGameStateLobbyBase().SendReliableToHost(
      GAME_RELIABLE_MESSAGE_DROPWEAPON, outMsg);
}
...
void idMultiplayerGame::BalanceTeams() {
  ...
  // First move players from the large team that match a party token on the
  // small team
    ...
    if (playerA->team == largeTeam && CanPlay(playerA)) {
      for (int b = 0; b < gameLocal.numClients; b++) {
        ...
        if (playerB->team == smallTeam && CanPlay(playerB)) {
          if (lobby.GetLobbyUserPartyToken(gameLocal.lobbyUserIDs[a]) ==
              lobby.GetLobbyUserPartyToken(gameLocal.lobbyUserIDs[b])) {
  ...
  // Then move players from the large team that DON'T match party tokens on the
  // large team
  for (int a = 0; a < gameLocal.numClients; a++) {
    idPlayer* playerA = gameLocal.GetClientByNum(a);
    if (playerA->team == largeTeam && CanPlay(playerA)) {
...
void idMultiplayerGame::SwitchToTeam(int clientNum, int oldteam, int newteam) {
  ...
  session->GetActingGameStateLobbyBase().SetLobbyUserTeam(
      gameLocal.lobbyUserIDs[clientNum], newteam);
...
================
*/
void idMultiplayerGame::ProcessChatMessage(int clientNum, bool team,
  ...
  if (!send_to) {
    ...
    session->GetActingGameStateLobbyBase().SendReliable(
        GAME_RELIABLE_MESSAGE_CHAT, outMsg, false);
    if (sound) {
    ...
    for (i = 0; i < gameLocal.numClients; i++) {
      ...
      if (send_to == 1 && pent->spectating) {
        ...
        if (i == gameLocal.GetLocalClientNum()) {
          ...
          session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
              gameLocal.lobbyUserIDs[i], GAME_RELIABLE_MESSAGE_CHAT, outMsg);
        }
...
void idMultiplayerGame::ToggleSpectate() {
  ...
  idPlayer* player =
      (idPlayer*)gameLocal.entities[gameLocal.GetLocalClientNum()];
  ...
  session->GetActingGameStateLobbyBase().SendReliableToHost(
      GAME_RELIABLE_MESSAGE_SPECTATE, outMsg);
}
...
================
*/
bool idMultiplayerGame::CanPlay(idPlayer* p) { return !p->wantSpectate; }
...
void idMultiplayerGame::VoiceChat(const idCmdArgs& args, bool team) {
  ...
  session->GetActingGameStateLobbyBase().SendReliableToHost(
      GAME_RELIABLE_MESSAGE_VCHAT, outMsg);
}
...
void idMultiplayerGame::ProcessVoiceChat(int clientNum, bool team, int index) {
  ...
  name = session->GetActingGameStateLobbyBase().GetLobbyUserName(
      gameLocal.lobbyUserIDs[clientNum]);
...
void idMultiplayerGame::ServerWriteInitialReliableMessages(
    int clientNum, lobbyUserID_t lobbyUserID) {
  idBitMsg outMsg;
  byte msgBuf[MAX_GAME_MESSAGE_SIZE];
  ...
  session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
      lobbyUserID, GAME_RELIABLE_MESSAGE_STARTSTATE, outMsg);
  ...
  if (gameState == COUNTDOWN) {
    ...
    outMsg.WriteLong(warmupEndTime);
    session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
        lobbyUserID, GAME_RELIABLE_MESSAGE_WARMUPTIME, outMsg);
  }
}
...
void idMultiplayerGame::ClientReadStartState(const idBitMsg& msg) {
  ...
  while ((client = msg.ReadByte()) != MAX_CLIENTS) {
    ...
    for (int i = 0; i < MAX_POWERUPS; i++) {
      if (powerup & (1 << i)) {
        static_cast<idPlayer*>(gameLocal.entities[client])
            ->GivePowerUp(i, 0, ITEM_GIVE_FEEDBACK | ITEM_GIVE_UPDATE_STATE);
      }
    }
...
void idMultiplayerGame::FindTeamFlags() {
  const char* flagDefs[2] = {"team_CTF_redflag", "team_CTF_blueflag"};
...
================
*/
void idMultiplayerGame::SetFlagMsg(bool b) { flagMsgOn = b; }
...
================
*/
int idMultiplayerGame::GetGameModes(const char*** gameModes,
