...
// This should match up to the ordering of the gameType_t. ( in
// MultiplayerGame.h )
const columnGameMode_t gameMode_columnDefs[] = {
    {public_Deathmatch, ARRAY_COUNT(public_Deathmatch), RANK_GREATEST_FIRST,
     false, false, "DM"},  // DEATHMATCH
    {public_Tourney, ARRAY_COUNT(public_Tourney), RANK_GREATEST_FIRST, false,
     false, "TOURNEY"},  // TOURNEY
    {public_TeamDeathmatch, ARRAY_COUNT(public_TeamDeathmatch),
     RANK_GREATEST_FIRST, false, false, "TDM"},  // TEAM DEATHMATCH
    {public_LastmanStanding, ARRAY_COUNT(public_LastmanStanding),
     RANK_GREATEST_FIRST, false, false, "LMS"},  // LASTMAN STANDING
    {public_CaptureTheFlag, ARRAY_COUNT(public_CaptureTheFlag),
     RANK_GREATEST_FIRST, false, false, "CTF"},  // CAPTURE THE FLAG
};
...
void LeaderboardLocal_Init() {
  ...
  // Iterate through all the available maps, and generate leaderboard Defs, and
  // IDs for each.
  for (int mapIdx = 0; mapIdx < maps.Num(); mapIdx++) {
    for (int modeIdx = 0; modeIdx < numModes; modeIdx++) {
      ...
      if (maps[mapIdx].supportedModes & BIT(modeIdx)) {
        ...
        // Create and Register the leaderboard with the sys_stats registered
        // Leaderboards
        leaderboardDefinition_t* newLeaderboardDef = Sys_CreateLeaderboardDef(
            boardID, gamemode.numColumns, gamemode.columnDef,
            gamemode.rankOrder, gamemode.supportsAttachments,
            gamemode.checkAgainstCurrent);

        // Set the leaderboard name.
        const char* mapname = idLocalization::GetString(maps[mapIdx].mapName);
        newLeaderboardDef->boardName.Format("%s %s", mapname,
                                            gamemode.abrevName);
...
=====================================
*/
void LeaderboardLocal_Shutdown() { Sys_DestroyLeaderboardDefs(); }
...
void LeaderboardLocal_Upload(lobbyUserID_t lobbyUserID, int gameType,
                             leaderboardStats_t& stats) {
  assert(gameType > GAME_RANDOM);

  int mapIdx = 0;

  // Need to figure out  What stat columns we want to base rank on. ( for now
  // we'll use wins )
  const column_t* gameTypeColumn = NULL;
  const column_t defaultStats[] = {stats.wins};

  // calculate DM score.
  int dmScore = stats.frags * FRAG_MULTIPLIER +
                stats.deaths * DEATH_MULTIPLIER + stats.wins * WINS_MULTIPLIER;
  // TODO: Once leaderboard menu has correct columns- enable this -> const
  // column_t dmStats[] = { stats.frags, stats.deaths, stats.wins, dmScore };
  const column_t dmStats[] = {dmScore};

  // Calculate TDM score.
  int tdmScore = stats.frags * FRAG_MULTIPLIER +
                 stats.teamfrags * FRAG_MULTIPLIER +
                 stats.deaths * DEATH_MULTIPLIER + stats.wins * WINS_MULTIPLIER;
  ...
  int lmsScore =
      lmsFrags * FRAG_MULTIPLIER + stats.wins * (WINS_MULTIPLIER * 10);
  const column_t lmsStats[] = {lmsScore};

  // Calculate CTF score.
  int ctfScore = stats.frags * FRAG_MULTIPLIER +
                 stats.deaths * DEATH_MULTIPLIER +
                 stats.wins * (WINS_MULTIPLIER * 10);
  ...
  if (board) {
    ...
    idLib::Warning("LeaderboardLocal_Upload invalid leaderboard with id of %d",
                   boardID);
  }
}
...
CONSOLE_COMMAND(testLeaderboardDownload, "<id 0 - n > <start = 1> <end = 100>",
                0) {
  ...
  const leaderboardDefinition_t* leaderboardDef =
      Sys_FindLeaderboardDef(leaderboardID);

  if (leaderboardDef) {
    session->LeaderboardDownload(0, leaderboardDef, start, end,
                                 leaderboardCallbackTest);
  } else {
...
CONSOLE_COMMAND(testLeaderboardUpload,
                "<gameType 0 - 4 > <frags = 0> <wins = 1>", 0) {
...
CONSOLE_COMMAND(testLeaderboardUpload_SendToClients,
                "<gameType 0 - 4 > <frags = 0> <wins = 1>", 0) {
