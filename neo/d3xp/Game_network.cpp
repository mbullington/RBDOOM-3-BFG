...
idCVar net_clientSmoothing("net_clientSmoothing", "0.8", CVAR_GAME | CVAR_FLOAT,
                           "smooth other clients angles and position.", 0.0f,
                           0.95f);
idCVar net_clientSelfSmoothing(
    "net_clientSelfSmoothing", "0.6", CVAR_GAME | CVAR_FLOAT,
    "smooth self position if network causes prediction error.", 0.0f, 0.95f);
extern idCVar net_clientMaxPrediction;

idCVar cg_predictedSpawn_debug("cg_predictedSpawn_debug", "0", CVAR_BOOL,
                               "Debug predictive spawning of presentables");
idCVar g_clientFire_checkLineOfSightDebug("g_clientFire_checkLineOfSightDebug",
                                          "0", CVAR_BOOL, "");
...
================
*/
int idGameLocal::ClientRemapDecl(declType_t type, int index) { return index; }
...
================
*/
void idGameLocal::ServerWriteInitialReliableMessages(
  ...
  cvarSystem->MoveCVarsToDict(CVAR_NETWORKSYNC, syncedCvars, true);
  outMsg.WriteDeltaDict(syncedCvars, NULL);
  lobby.SendReliableToLobbyUser(lobbyUserID, GAME_RELIABLE_MESSAGE_SYNCEDCVARS,
                                outMsg);
  ...
  for (entityNetEvent_t* event = savedEventQueue.Start(); event;
       event = event->next) {
    outMsg.InitWrite(msgBuf, sizeof(msgBuf));
    outMsg.BeginWriting();
    ...
    lobby.SendReliableToLobbyUser(lobbyUserID, GAME_RELIABLE_MESSAGE_EVENT,
                                  outMsg);
  }
...
================
*/
void idGameLocal::SaveEntityNetworkEvent(const idEntity* ent, int eventId,
                                         const idBitMsg* msg) {
...
void idGameLocal::ServerWriteSnapshot(idSnapShot& ss) {
  ...
  for (int i = 0; i < numPortals; i++) {
    msg.WriteBits(gameRenderWorld->GetPortalState((qhandle_t)(i + 1)),
  ...
  // Build PVS data for each player and write their player state to the snapshot
  // as well
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (player->spectating && player->spectator != i &&
    ...
    int numSourceAreas = gameRenderWorld->BoundsInAreas(
        spectated->GetPlayerPhysics()->GetAbsBounds(), sourceAreas,
        idEntity::MAX_PVS_AREAS);
    pvsHandles[i] =
        pvs.SetupCurrentPVS(sourceAreas, numSourceAreas, PVS_NORMAL);
    if (portalSkyPVS.i >= 0) {
  ...
  for (idEntity* ent = spawnedEntities.Next(); ent != NULL;
    ...
    msg.WriteBits(ServerRemapDecl(-1, DECL_ENTITYDEF, ent->entityDefNumber),
                  entityDefBits);
...
================
*/
void idGameLocal::ProcessReliableMessage(int clientNum, int type,
                                         const idBitMsg& msg) {
...
================
*/
void idGameLocal::ServerProcessReliableMessage(int clientNum, int type,
                                               const idBitMsg& msg) {
  if (clientNum < 0) {
    return;
  }
  switch (type) {
    ...
    case GAME_RELIABLE_MESSAGE_TCHAT: {
      ...
      mpGame.ProcessChatMessage(clientNum, type == GAME_RELIABLE_MESSAGE_TCHAT,
                                name, text, NULL);
      break;
    }
    ...
    case GAME_RELIABLE_MESSAGE_SPECTATE: {
      ...
      if (serverInfo.GetBool("si_spectators")) {
        ...
        if (mpGame.GetGameState() == idMultiplayerGame::SUDDENDEATH && !spec &&
            player->wantSpectate) {
        ...
        // If the server turned off si_spectators while a player is spectating,
        // then any spectate message forces the player out of spectate mode
    ...
    case GAME_RELIABLE_MESSAGE_CLIENT_HITSCAN_HIT: {
      ...
      idPlayer& attacker =
          static_cast<idPlayer&>(*gameLocal.entities[attackerNum]);
      ...
      attacker.weapon.GetEntity()->GetProjectileLaunchOriginAndAxis(
          muzzleOrigin, muzzleAxis);

      idVec3 targetLocation =
          victim.GetRenderEntity()->origin +
          victim.GetRenderEntity()->joints[location].ToVec3() *
              victim.GetRenderEntity()->axis;

      trace_t tr;
      gameLocal.clip.Translation(tr, muzzleOrigin, targetLocation, NULL,
      ...
      const idDeclEntityDef* damageDef = static_cast<const idDeclEntityDef*>(
          declManager->DeclByIndex(DECL_ENTITYDEF, damageDefIndex, false));

      if (damageDef != NULL) {
        victim.Damage(NULL, gameLocal.entities[attackerNum], dir,
                      damageDef->GetName(), damageScale, location);
      }
      break;
    }
    default: {
...
void idGameLocal::ClientReadSnapshot(const idSnapShot& ss) {
  ...
  for (int o = 0; o < ss.NumObjects(); o++) {
    ...
    if (snapObjectNum >= SNAP_PLAYERSTATE &&
        snapObjectNum < SNAP_PLAYERSTATE_END) {
      ...
      const int lobbyIndex =
          session->GetActingGameStateLobbyBase()
              .GetLobbyUserIndexFromLobbyUserID(lobbyUserIDs[playerNumber]);
      if (lobbyIndex < 0 ||
          session->GetActingGameStateLobbyBase().IsLobbyUserConnected(
              lobbyIndex) == false) {
        continue;
      }
      ...
      if (otherPlayer != NULL) {
        ...
        if (otherPlayer !=
            entities[GetLocalClientNum()])  // This happens when we spectate
                                            // another player
        {
          idWeapon* weap = otherPlayer->weapon.GetEntity();
          if (weap && (weap->GetRenderEntity()->bounds[0] ==
                       weap->GetRenderEntity()->bounds[1])) {
            // update the weapon's viewmodel bounds so that the model doesn't
            // flicker in the spectator's view
    ...
    if (snapObjectNum >= SNAP_LAST_CLIENT_FRAME &&
        snapObjectNum < SNAP_LAST_CLIENT_FRAME_END) {
    ...
    int entityDefNumber =
        ClientRemapDecl(DECL_ENTITYDEF, msg.ReadBits(entityDefBits));
    ...
    if (!typeInfo) {
      idLib::Error("Unknown type number %d for entity %d with class number %d",
                   typeNum, entityNumber, entityDefNumber);
    }

    // If there is no entity on this client, but the server's entity matches a
    // predictionKey, move the client's predicted entity to the normal,
    // replicated area in the entity list.
    if (entities[entityNumber] == NULL) {
      if (predictedKey != idEntity::INVALID_PREDICTION_KEY) {
        ...
        if (predictedEntity != NULL) {
          // This presentable better be in the proper place in the list or bad
          // things will happen if we move this presentable around
          assert(predictedEntity->GetEntityNumber() >=
        ...
        /* else {
                idLib::Warning( "Could not find predicted entity - key: %d.
        EntityIndex: %d", predictedKey, entityNum );
        } */
      }
    }
    ...
    if (ss.ObjectIsStaleByIndex(o)) {
          ...
          !ent->spawnArgs.GetBool("net_dynamic", "0"))  //_D3XP
      {
        ...
        common->DWarning("map entity 0x%x (%s) is stale", ent->entityNumber,
                         ent->name.c_str());
      } else {
...
void idGameLocal::ClientRunFrame(idUserCmdMgr& cmdMgr, bool lastPredictFrame,
  ...
  idLobbyBase& lobby = session->GetActingGameStateLobbyBase();
  if (lobby.GetPeerTimeSinceLastPacket(lobby.PeerIndexForHost()) >=
  ...
  for (ent = activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next()) {
    ...
    if (ent->entityNumber != GetLocalClientNum()) {
      ent->ClientThink(netInterpolationInfo.serverGameMs,
...
========================
*/
idEntity* idGameLocal::FindPredictedEntity(uint32 predictedKey,
                                           idTypeInfo* type) {
  for (idEntity* predictedEntity = activeEntities.Next();
       predictedEntity != NULL;
       predictedEntity = predictedEntity->activeNode.Next()) {
    if (!verify(predictedEntity != NULL)) {
      continue;
    }
    if (!predictedEntity->IsReplicated() &&
        predictedEntity->GetPredictedKey() == predictedKey) {
      if (predictedEntity->GetType() != type) {
        idLib::Warning("Mismatched presentable type. Predicted: %s Actual: %s",
                       predictedEntity->GetType()->classname, type->classname);
      }
...
========================
*/
uint32 idGameLocal::GeneratePredictionKey(idWeapon* weapon,
                                          idPlayer* playerAttacker,
                                          int overrideKey) {
  if (overrideKey != -1) {
    ...
    if (common->IsServer()) {
      peerIndex = session->GetActingGameStateLobbyBase().PeerIndexFromLobbyUser(
          lobbyUserIDs[playerAttacker->entityNumber]);
    } else {
