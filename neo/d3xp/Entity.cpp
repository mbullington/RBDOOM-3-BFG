...
idCVar net_errorSmoothingMaxDecay("net_errorSmoothingMaxDecay", "25.0",
                                  CVAR_FLOAT,
                                  "Max rate at which origin error smoothing "
                                  "decays (in units per game frame)");
idCVar net_errorSmoothingDecay(
    "net_errorSmoothingDecay", "0.06", CVAR_FLOAT,
...
void idGameEdit::ParseSpawnArgsToRenderEntity(const idDict* args,
  ...
  // RB: TrenchBroom interop
    ...
    // if the name starts with genmodel_ it means that it was generated by
    // exportModelsToTrenchBroom and we have a special entityDef for it in
    // base/def/_tb_models.def
...
void idEntity::Spawn() {
  ...
  // go dormant within 100ms so that when the map starts most monsters are
  // dormant
...
================
*/
const char* idEntity::GetName() const { return name.c_str(); }
...
================
*/
bool idEntity::IsActive() const { return activeNode.InList(); }
...
================
*/
const idDeclSkin* idEntity::GetSkin() const { return renderEntity.customSkin; }
...
================
*/
bool idEntity::IsHidden() const { return fl.hidden; }
...
================
*/
void idEntity::ClearPVSAreas() { numPVSAreas = -1; }
...
void idEntity::BecomeReplicated() {
  ...
  if (GetPhysics()) {
    GetPhysics()->ResetInterpolationState(GetPhysics()->GetOrigin(),
                                          GetPhysics()->GetAxis());
  }
}
...
==============
*/
void idEntity::ProjectOverlay(const idVec3& origin, const idVec3& dir,
...
================
*/
renderEntity_t* idEntity::GetRenderEntity() { return &renderEntity; }
...
================
*/
int idEntity::GetModelDefHandle() { return modelDefHandle; }
...
================
*/
idAnimator* idEntity::GetAnimator() { return NULL; }
...
================
*/
bool idEntity::CanPlayChatterSounds() const { return true; }
...
================
*/
bool idEntity::StartSound(const char* soundName, const s_channelType channel,
                          int soundShaderFlags, bool broadcast, int* length) {
...
bool idEntity::StartSoundShader(const idSoundShader* shader,
                                ...
                                int soundShaderFlags, bool broadcast,
                                int* length) {
  float diversity;
  int len;
...
================
*/
void idEntity::SetSoundVolume(float volume) { refSound.parms.volume = volume; }
...
================
*/
int idEntity::GetListenerId() const { return refSound.listenerId; }
...
================
*/
void idEntity::BindToJoint(idEntity* master, const char* jointname,
                           bool orientated) {
...
================
*/
void idEntity::BindToJoint(idEntity* master, jointHandle_t jointnum,
                           bool orientated) {
  if (!InitBind(master)) {
    return;
  }
...
================
*/
jointHandle_t idEntity::GetBindJoint() const { return bindJoint; }
...
================
*/
int idEntity::GetBindBody() const { return bindBody; }
...
bool idEntity::RunPhysics() {
  ...
  // This is to hack around an issue when reloading a game saved on certain
  // movers would eject the player randomly through the world due to the first
  // couple of frames imparting large pushVelocities.
    ...
    if (ent->physics->IsType(idPhysics_Player::Type)) {
      ...
      if (useAbnormalVelocityHack) {
        idPhysics_Player* physics =
            static_cast<idPhysics_Player*>(ent->physics);
...
void idEntity::InterpolatePhysics(const float fraction) {
  ...
  // if this entity is a team slave, we still need to interpolate it's current
  // position from the snapshot. The team master probably depends on the current
  // physics state, and may be unaware of prev/next or interpolation.
  if (teamMaster && teamMaster != this) {
...
================
*/
void idEntity::SetAngles(const idAngles& ang) { SetAxis(ang.ToMat3()); }
...
================
*/
void idEntity::GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                             impactInfo_t* info) {
...
================
*/
void idEntity::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
...
================
*/
void idEntity::AddForce(idEntity* ent, int id, const idVec3& point,
                        const idVec3& force) {
...
================
*/
bool idEntity::IsAtRest() const { return GetPhysics()->IsAtRest(); }
...
================
*/
void idEntity::DamageFeedback(idEntity* victim, idEntity* inflictor,
                              int& damage) {
...
================
*/
void idEntity::AddDamageEffect(const trace_t& collision, const idVec3& velocity,
...
============
*/
bool idEntity::Pain(idEntity* inflictor, idEntity* attacker, int damage,
...
================
*/
bool idEntity::ShouldConstructScriptObjectAtSpawn() const { return true; }
...
================
*/
void idEntity::SetSignal(signalNum_t signalnum, idThread* thread,
  ...
  if (entityGui && cmds && *cmds) {
    ...
    while (1) {
      ...
      if (!token.Icmp("martianbuddycomplete")) {
        gameLocal.GetLocalPlayer()->GiveEmail(
            static_cast<const idDeclEmail*>(declManager->FindType(
                DECL_EMAIL, "MartianBuddyGameComplete", false)));
        continue;
      }
...
================
*/
void idEntity::Teleport(const idVec3& origin, const idAngles& angles,
...
================
*/
void idEntity::Event_GetName() { idThread::ReturnString(name.c_str()); }
...
===============
*/
void idEntity::Event_FindTargets() { FindTargets(); }
...
================
*/
void idEntity::Event_NumTargets() { idThread::ReturnFloat(targets.Num()); }
...
================
*/
void idEntity::Event_SetModel(const char* modelname) { SetModel(modelname); }
...
================
*/
void idEntity::Event_SetShaderParms(float parm0, float parm1, float parm2,
                                    float parm3) {
...
================
*/
void idEntity::Event_IsHidden() { idThread::ReturnInt(fl.hidden); }
...
================
*/
void idEntity::Event_SetOrigin(idVec3 const& org) { SetOrigin(org); }
...
================
*/
void idEntity::Event_SetAngles(idAngles const& ang) { SetAngles(ang); }
...
================
*/
void idEntity::ClientThink(const int curTime, const float fraction,
                           const bool predict) {
...
Updates the interpolationBehavior so that subclasses will know if it's safe to
interpolate. Only call this when a new snapshot has been received for this
entity!
================
*/
...
================
*/
void idEntity::ServerSendEvent(int eventId, const idBitMsg* msg, bool saveEvent,
                               lobbyUserID_t excluding) const {
  idBitMsg outMsg;
...
void idEntity::ClientSendEvent(int eventId, const idBitMsg* msg) const {
  ...
  session->GetActingGameStateLobbyBase().SendReliableToHost(
      GAME_RELIABLE_MESSAGE_EVENT, outMsg);
}
...
================
*/
void idEntity::SetGrabbedState(bool grabbed) { fl.grabbed = grabbed; }
...
================
*/
bool idEntity::IsGrabbed() { return fl.grabbed; }
...
void idEntity::DecayOriginAndAxisDelta() {
  ...
  idQuat q;
  q.Slerp(axisDelta.ToQuat(), mat3_identity.ToQuat(),
          net_errorSmoothingDecay.GetFloat());
  axisDelta = q.ToMat3();
}
...
========================
*/
void idEntity::CreateDeltasFromOldOriginAndAxis(const idVec3& oldOrigin,
                                                const idMat3& oldAxis) {
  // Set smooth values so we transition from the old position/axis to what we
  // are now (visual only)
  if (GetPhysics()) {
...
================
*/
void idAnimatedEntity::ClientThink(const int curTime, const float fraction,
                                   const bool predict) {
  InterpolatePhysics(fraction);
  UpdateAnimation();
  Present();
...
==============
*/
int idAnimatedEntity::GetDefaultSurfaceType() const { return SURFTYPE_METAL; }
...
================
*/
bool idAnimatedEntity::ClientReceiveEvent(int event, int time,
                                          const idBitMsg& msg) {
  int damageDefIndex;
  int materialIndex;
  jointHandle_t jointNum;
...
================
*/
void idAnimatedEntity::Event_ClearAllJoints() { animator.ClearAllJoints(); }
