...
idCVar binaryLoadAnim("binaryLoadAnim", "1", 0,
                      "enable binary load/write of idMD5Anim");

static const byte B_ANIM_MD5_VERSION = 101;
static const unsigned int B_ANIM_MD5_MAGIC =
    ('B' << 24) | ('M' << 16) | ('D' << 8) | B_ANIM_MD5_VERSION;

static const int JOINT_FRAME_PAD =
    1;  // one extra to be able to read one more float than is necessary
...
====================
*/
int idMD5Anim::NumFrames() const { return numFrames; }
...
====================
*/
int idMD5Anim::NumJoints() const { return numJoints; }
...
====================
*/
int idMD5Anim::Length() const { return animLength; }
...
=====================
*/
const idVec3& idMD5Anim::TotalMovementDelta() const { return totaldelta; }
...
=====================
*/
const char* idMD5Anim::Name() const { return name; }
...
bool idMD5Anim::LoadAnim(const char* filename) {
  ...
  // Get the timestamp on the original file, if it's newer than what is stored
  // in binary model, regenerate it
  ...
  componentFrames[numAnimatedComponents * numFrames + JOINT_FRAME_PAD - 1] =
      0.0f;
...
bool idMD5Anim::LoadBinary(idFile* file, ID_TIME_T sourceTimeStamp) {
  ...
  if (!fileSystem->InProductionMode() &&
      ...
      (sourceTimeStamp != loadedTimeStamp)) {
    return false;
  }
  // RB end
...
====================
*/
void idMD5Anim::IncreaseRefs() const { ref_count++; }
...
====================
*/
void idMD5Anim::DecreaseRefs() const { ref_count--; }
...
====================
*/
int idMD5Anim::NumRefs() const { return ref_count; }
...
====================
*/
void idMD5Anim::ConvertTimeToFrame(int time, int cyclecount,
...
====================
*/
void idMD5Anim::GetOriginRotation(idQuat& rotation, int time,
...
====================
*/
int DecodeInterpolatedFrames(idJointQuat* joints, idJointQuat* blendJoints,
                             int* lerpIndex, const float* frame1,
                             const float* frame2,
                             const jointAnimInfo_t* jointInfo, const int* index,
...
====================
*/
void idMD5Anim::GetInterpolatedFrame(frameBlend_t& frame, idJointQuat* joints,
                                     const int* index, int numIndexes) const {
  // copy the baseframe
  ...
  idJointQuat* blendJoints =
      (idJointQuat*)_alloca16(baseFrame.Num() * sizeof(blendJoints[0]));
  ...
  int numLerpJoints =
      DecodeInterpolatedFrames(joints, blendJoints, lerpIndex, frame1, frame2,
                               jointInfo.Ptr(), index, numIndexes);
...
====================
*/
void idMD5Anim::GetSingleFrame(int framenum, idJointQuat* joints,
                               const int* index, int numIndexes) const {
  // copy the baseframe
  SIMDProcessor->Memcpy(joints, baseFrame.Ptr(),
                        baseFrame.Num() * sizeof(baseFrame[0]));
...
void idMD5Anim::CheckModelHierarchy(const idRenderModel* model) const {
  ...
  if (jointInfo.Num() != model->NumJoints()) {
    if (!fileSystem->InProductionMode()) {
      ...
      // gameLocal.Warning( "Model '%s' has different # of joints than anim
      // '%s'", model->Name(), name.c_str() );
      return;
    }
  }
...
====================
*/
idAnimManager::~idAnimManager() { Shutdown(); }
...
void idAnimManager::Preload(const idPreloadManifest& manifest) {
  if (manifest.NumResources() >= 0) {
    ...
    common->Printf(
        "%05d anims preloaded ( or were already loaded ) in %5.1f seconds\n",
