...
enum achievement_t {
  ...
  ACHIEVEMENT_KILL_20_ENEMY_FISTS_HANDS,  // 9 // DONE --- kill 20 enemies with
                                          // fists & hands
  ACHIEVEMENT_KILL_SCI_NEXT_TO_RCR,  // 10 // DONE -----> ADD TARGET TO MAP kill
                                     // scientist trapped next to reactor
                                     // control room
  ...
  ACHIEVEMENT_GET_BFG_FROM_SECURITY_OFFICE,  // 14 // DONE -----> ADD TARGET TO
                                             // MAP
  ...
  ACHIEVEMENT_BLOODY_HANDWORK_OF_BETRUGER,  // 25 // DONE -----> ADD TARGET TO
                                            // MAP
  ...
  ACHIEVEMENT_MP_CATCH_ENEMY_IN_ROFC,  // 46 // DONE -- needs to be tested --
                                       // Reactor of Frag Chamber
...
compile_time_assert(ACHIEVEMENTS_NUM <=
                    idPlayerProfile::MAX_PLAYER_PROFILE_STATS);
...
class idAchievementManager {
 public:
  ...
  // Adds a count to the tracked number of events, these events can be applied
  // to multiple achievements
 ...
 private:
  ...
  idArray<int, ACHIEVEMENTS_NUM>
      counts;  // How many times has each achievement been given
