...
================
*/
bool idInterpreter::GetRegisterValue(const char* name, idStr& out,
                                     int scopeDepth) {
  varEval_t reg;
  idVarDef* d;
...
================
*/
int idInterpreter::GetCallstackDepth() const { return callStackDepth; }
...
================
*/
const prstack_t* idInterpreter::GetCallstack() const { return &callStack[0]; }
...
================
*/
idThread* idInterpreter::GetThread() const { return thread; }
...
================
*/
void idInterpreter::SetThread(idThread* pThread) { thread = pThread; }
...
====================
*/
void idInterpreter::ThreadCall(idInterpreter* source, const function_t* func,
                               int args) {
  Reset();
...
================
*/
void idInterpreter::EnterObjectFunction(idEntity* self, const function_t* func,
                                        bool clearStack) {
  if (clearStack) {
    Reset();
  }
  ...
  while (!doneProcessing && !threadDying) {
    ...
    switch (st->op) {
      ...
      case OP_UDIV_V:
        ...
        if (*var_a.floatPtr == 0.0f) {
          Warning("Divide by zero");
          var_b.vectorPtr->Set(idMath::INFINITUM, idMath::INFINITUM,
                               idMath::INFINITUM);
        } else {
...
// RB: moved from Script_Interpreter.h to avoid include problems with the script
// debugger
/*
================
