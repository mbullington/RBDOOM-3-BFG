...
idTypeDef type_scriptevent(ev_scriptevent, &def_scriptevent, "scriptevent",
                           sizeof(intptr_t), NULL);
idTypeDef type_namespace(ev_namespace, &def_namespace, "namespace",
                         sizeof(intptr_t), NULL);
...
idTypeDef type_vector(ev_vector, &def_vector, "vector", E_EVENT_SIZEOF_VEC,
                      NULL);
...
idTypeDef type_function(ev_function, &def_function, "function",
                        sizeof(intptr_t), &type_void);
idTypeDef type_virtualfunction(ev_virtualfunction, &def_virtualfunction,
                               "virtual function", sizeof(intptr_t), NULL);
idTypeDef type_pointer(ev_pointer, &def_pointer, "pointer", sizeof(intptr_t),
                       NULL);
idTypeDef type_object(ev_object, &def_object, "object", sizeof(intptr_t),
                      NULL);  // stored as entity number pointer
idTypeDef type_jumpoffset(ev_jumpoffset, &def_jumpoffset, "<jump>",
                          sizeof(intptr_t),
...
idTypeDef type_boolean(ev_boolean, &def_boolean, "boolean", sizeof(intptr_t),
                       NULL);
// RB end
...
================
*/
void function_t::SetName(const char* name) { this->name = name; }
...
================
*/
const char* function_t::Name() const { return name; }
...
================
*/
idTypeDef::idTypeDef(etype_t etype, idVarDef* edef, const char* ename,
                     int esize, idTypeDef* aux) {
  name = ename;
  type = etype;
  def = edef;
  size = esize;
...
================
*/
idTypeDef::idTypeDef(const idTypeDef& other) { *this = other; }
...
================
*/
void idTypeDef::SetName(const char* newname) { name = newname; }
...
================
*/
const char* idTypeDef::Name() const { return name; }
...
================
*/
etype_t idTypeDef::Type() const { return type; }
...
================
*/
int idTypeDef::Size() const { return size; }
...
================
*/
int idTypeDef::NumParameters() const { return parmTypes.Num(); }
...
================
*/
int idTypeDef::NumFunctions() const { return functions.Num(); }
...
============
*/
const char* idVarDef::Name() const { return name->Name(); }
...
============
*/
idScriptObject::~idScriptObject() { Free(); }
...
============
*/
bool idScriptObject::HasObject() const { return (type != &type_object); }
...
============
*/
idTypeDef* idScriptObject::GetTypeDef() const { return type; }
...
============
*/
const char* idScriptObject::GetTypeName() const { return type->Name(); }
...
============
*/
idTypeDef* idProgram::AllocType(etype_t etype, idVarDef* edef,
                                const char* ename, int esize, idTypeDef* aux) {
  idTypeDef* newtype =
      new (TAG_SCRIPT) idTypeDef(etype, edef, ename, esize, aux);
...
idTypeDef* idProgram::GetType(idTypeDef& type, bool allocate) {
  for (int i = typesHash.First(idStr::Hash(type.Name())); i != -1;
       i = typesHash.Next(i)) {
...
idTypeDef* idProgram::FindType(const char* name) {
  for (int i = typesHash.First(idStr::Hash(name)); i != -1;
       i = typesHash.Next(i)) {
...
#if defined(USE_EXCEPTIONS)
    throw idCompileError(
        va("Exceeded global memory size (%zd bytes)", sizeof(variables)));
#else
    gameLocal.Error("Exceeded global memory size (%zd bytes)",
                    sizeof(variables));
#endif
  }
...
// RB: moved from AllocDef
idVarDef* idProgram::AllocVarDef(idTypeDef* type, const char* name,
                                 idVarDef* scope) {
  // allocate a new def
...
============
*/
idVarDef* idProgram::AllocDef(idTypeDef* type, const char* name,
                              idVarDef* scope, bool constant) {
  idVarDef* def;
  idStr element;
  idVarDef* def_x;
  idVarDef* def_y;
...
============
*/
idVarDef* idProgram::GetDef(const idTypeDef* type, const char* name,
...
============
*/
idVarDef* idProgram::FindFreeResultDef(idTypeDef* type, const char* name,
                                       idVarDef* scope, const idVarDef* a,
                                       const idVarDef* b) {
  idVarDef* def;
...
function_t& idProgram::AllocFunction(idVarDef* def) {
  if (functions.Num() >= functions.Max()) {
    ...
    throw idCompileError(va("Exceeded maximum allowed number of functions (%d)",
                            functions.Max()));
#else
    gameLocal.Error("Exceeded maximum allowed number of functions (%d)",
                    functions.Max());
#endif
  }
  ...
  if (statements.Num() >= statements.Max()) {
    ...
    throw idCompileError(
        va("Exceeded maximum allowed number of statements (%d)",
           statements.Max()));
#else
    gameLocal.Error("Exceeded maximum allowed number of statements (%d)",
                    statements.Max());
#endif
  }
...
int idProgram::CalculateChecksum() const {
  ...
  statementBlock_t* statementList =
      new (TAG_SCRIPT) statementBlock_t[statements.Num()];
