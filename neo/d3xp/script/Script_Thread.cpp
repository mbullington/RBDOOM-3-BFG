...
const idEventDef EV_Thread_VecToOrthoBasisAngles("VecToOrthoBasisAngles", "v",
                                                 'v');
...
================
*/
idThread* idThread::CurrentThread() { return currentThread; }
...
================
*/
idThread::idThread(idInterpreter* source, idEntity* self,
                   const function_t* func, int args) {
  assert(self);
...
================
*/
void idThread::ManualDelete() { interpreter.terminateOnExit = false; }
...
void idThread::DisplayInfo() {
  ...
  if (interpreter.threadDying) {
    ...
    } else if ((waitingFor != ENTITYNUM_NONE) && (waitingFor < MAX_GENTITIES) &&
...
================
*/
void idThread::SetThreadName(const char* name) { threadName = name; }
...
================
*/
void idThread::CallFunction(idEntity* self, const function_t* func,
                            bool clearStack) {
  assert(self);
  ClearWaitFor();
...
================
*/
void idThread::Event_SetThreadName(const char* name) { SetThreadName(name); }
...
================
*/
void idThread::Event_Execute() { Execute(); }
...
================
*/
void idThread::WaitSec(float time) { WaitMS(SEC2MS(time)); }
...
================
*/
void idThread::Event_Pause() { Pause(); }
...
================
*/
void idThread::Event_Wait(float time) { WaitSec(time); }
...
================
*/
void idThread::Event_WaitFrame() { WaitFrame(); }
...
================
*/
void idThread::Event_Print(const char* text) { gameLocal.Printf("%s", text); }
...
================
*/
void idThread::Event_Assert(float value) { assert(value); }
...
void idThread::Event_GetTime() {
  ...
  /*  Script always uses realClient time to determine scripty stuff. ( This
  Fixes Weapon Animation timing bugs ) if ( common->IsMultiplayer() ) {
...
================
*/
void idThread::Event_KillThread(const char* name) { KillThread(name); }
...
================
*/
void idThread::Event_VecLength(idVec3& vec) { ReturnFloat(vec.Length()); }
...
================
*/
void idThread::Event_FirstPerson() { gameLocal.SetCamera(NULL); }
...
================
*/
void idThread::Event_Trace(const idVec3& start, const idVec3& end,
                           const idVec3& mins, const idVec3& maxs,
                           int contents_mask, idEntity* passEntity) {
...
================
*/
void idThread::Event_TracePoint(const idVec3& start, const idVec3& end,
                                int contents_mask, idEntity* passEntity) {
  gameLocal.clip.TracePoint(trace, start, end, contents_mask, passEntity);
  ReturnFloat(trace.fraction);
}
...
================
*/
void idThread::Event_GetTraceFraction() { ReturnFloat(trace.fraction); }
...
================
*/
void idThread::Event_GetTraceEndPos() { ReturnVector(trace.endpos); }
...
================
*/
void idThread::Event_Warning(const char* text) { Warning("%s", text); }
...
================
*/
void idThread::Event_Error(const char* text) { Error("%s", text); }
...
================
*/
void idThread::Event_RadiusDamage(const idVec3& origin, idEntity* inflictor,
                                  idEntity* attacker, idEntity* ignore,
                                  const char* damageDefName, float dmgPower) {
...
================
*/
void idThread::Event_IsClient() { idThread::ReturnFloat(common->IsClient()); }
...
================
*/
void idThread::Event_DebugLine(const idVec3& color, const idVec3& start,
                               const idVec3& end, const float lifetime) {
...
================
*/
void idThread::Event_DebugArrow(const idVec3& color, const idVec3& start,
                                const idVec3& end, const int size,
...
================
*/
void idThread::Event_DebugCircle(const idVec3& color, const idVec3& origin,
                                 const idVec3& dir, const float radius,
                                 const int numSteps, const float lifetime) {
...
================
*/
void idThread::Event_DebugBounds(const idVec3& color, const idVec3& mins,
                                 const idVec3& maxs, const float lifetime) {
...
================
*/
void idThread::Event_DrawText(const char* text, const idVec3& origin,
                              float scale, const idVec3& color, const int align,
                              const float lifetime) {
