...
============
*/
idVarDef* idCompiler::FindImmediate(const idTypeDef* type, const eval_t* eval,
...
============
*/
idVarDef* idCompiler::GetImmediate(idTypeDef* type, const eval_t* eval,
                                   const char* string) {
  idVarDef* def;
...
============
*/
idVarDef* idCompiler::OptimizeOpcode(const opcode_t* op, idVarDef* var_a,
                                     idVarDef* var_b) {
  eval_t c;
  idTypeDef* type;
...
============
*/
idVarDef* idCompiler::EmitOpcode(const opcode_t* op, idVarDef* var_a,
                                 idVarDef* var_b) {
...
============
*/
idVarDef* idCompiler::EmitFunctionParms(int op, idVarDef* func, int startarg,
                                        int startsize, idVarDef* object) {
  idVarDef* e;
...
idVarDef* idCompiler::ParseSysObjectCall(idVarDef* funcDef) {
  ...
  assert(funcDef->value.functionPtr->eventdef !=
         NULL);  // to remove stupid analyze warning
...
void idCompiler::CompileFile(const char* text, const char* filename,
...
#endif
  {
    ...
    if (console) {
      ...
      sprintf(error, "Error: file %s, line %d: %s\n",
              ...
              currentLineNumber, err.GetError());
    }
  ...
  if (error) {
    common->Printf(
        "Error: idCompiler::CompileFile: file %s, line %d: unknown error\n",
