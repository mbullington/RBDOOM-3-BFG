...
Useful because:
-Hides client from having to manually declare "last" state and manually checking
against it -using int counter prevents problems w/ dropped snapshots

(ie if we just serialized a bool to true for a single ss, if that ss is
dropped,skipped,whatever the client would never handle it. By incrementing a
wrapped counter, we are guaranteed to detect the state change no matter what
happens at the net layer).
================================================
*/
template <int max>
struct idNetEvent {
...
inline void WriteToBitMsg(const netBoolEvent_t& netEvent, idBitMsg& msg) {
  msg.WriteBits(netEvent.count,
                idMath::BitsForInteger(netBoolEvent_t::Maximum));
...
inline void ReadFromBitMsg(netBoolEvent_t& netEvent, const idBitMsg& msg) {
  netEvent.count =
      msg.ReadBits(idMath::BitsForInteger(netBoolEvent_t::Maximum));
...
class idEntity : public idClass {
 public:
  ...
  idLinkList<idEntity>
      aimAssistNode;  // linked into gameLocal.aimAssistEntities
  ...
  bool snapshotStale;  // Set to true if this entity is considered stale in the
                       // snapshot
  ...
  idList<idEntityPtr<idEntity>, TAG_ENTITY>
 ...
 public:
  ...
  virtual void ProjectOverlay(const idVec3& origin, const idVec3& dir,
  ...
  static bool ModelCallback(renderEntity_t* renderEntity,
                            const renderView_t* renderView);
  virtual idAnimator*
  GetAnimator();  // returns animator object used by this entity

  // sound
  virtual bool CanPlayChatterSounds() const;
  bool StartSound(const char* soundName, const s_channelType channel,
                  int soundShaderFlags, bool broadcast, int* length);
  bool StartSoundShader(const idSoundShader* shader,
                        const s_channelType channel, int soundShaderFlags,
  ...
  virtual void GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                             impactInfo_t* info);
  // apply an impulse to the physics object, 'ent' is the entity applying the
  // impulse
  virtual void ApplyImpulse(idEntity* ent, int id, const idVec3& point,
                            const idVec3& impulse);
  // add a force to the physics object, 'ent' is the entity adding the force
  virtual void AddForce(idEntity* ent, int id, const idVec3& point,
                        const idVec3& force);
  ...
  virtual void AddDamageEffect(const trace_t& collision, const idVec3& velocity,
  ...
  void SetSignal(signalNum_t signalnum, idThread* thread,
                 const function_t* function);
  ...
  void CreateDeltasFromOldOriginAndAxis(const idVec3& oldOrigin,
 ...
 private:
  ...
  bool useClientInterpolation;  // disables interpolation for some objects
                                // (handy for weapon world models)
  ...
  uint32 predictionKey;  // Unique key used to sync predicted ents (projectiles)
                         // in MP.
...
class idAnimatedEntity : public idEntity {
 public:
  ...
  bool GetJointWorldTransform(jointHandle_t jointHandle, int currentTime,
                              idVec3& offset, idMat3& axis);
  bool GetJointTransformForAnim(jointHandle_t jointHandle, int animNum,
  ...
  virtual void AddDamageEffect(const trace_t& collision, const idVec3& velocity,
                               const char* damageDefName);
  void AddLocalDamageEffect(jointHandle_t jointNum, const idVec3& localPoint,
 ...
 private:
  ...
  void Event_SetJointPos(jointHandle_t jointnum,
                         jointModTransform_t transform_type, const idVec3& pos);
