...
enum itemGiveFlags_t {
  ...
  ITEM_GIVE_FROM_WEAPON = BIT(2),  // indicates this was given via a weapon's
                                   // launchPowerup (for bloodstone powerups)
};
...
class idMoveableItem : public idItem {
 public:
  ...
  static void DropItems(idAnimatedEntity* ent, const char* type,
                        idList<idEntity*>* list);
  static idEntity* DropItem(const char* classname, const idVec3& origin,
