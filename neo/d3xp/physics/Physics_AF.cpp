...
================
*/
const idVecX& idAFConstraint::GetMultiplier() { return lm; }
...
================
*/
void idAFConstraint::Evaluate(float invTimeStep) { assert(0); }
...
================
*/
void idAFConstraint::Translate(const idVec3& translation) { assert(0); }
...
================
*/
void idAFConstraint::Rotate(const idRotation& rotation) { assert(0); }
...
================
*/
void idAFConstraint::GetCenter(idVec3& center) { center.Zero(); }
...
================
*/
idAFConstraint_Fixed::idAFConstraint_Fixed(const idStr& name, idAFBody* body1,
                                           idAFBody* body2) {
  assert(body1);
...
*/
idAFConstraint_BallAndSocketJoint::idAFConstraint_BallAndSocketJoint(
    const idStr& name, idAFBody* body1, idAFBody* body2) {
  assert(body1);
...
void idAFConstraint_BallAndSocketJoint::SetPyramidLimit(
    const idVec3& pyramidAxis, const idVec3& baseAxis, const float angle1,
    const float angle2, const idVec3& body1Axis) {
  if (coneLimit) {
    delete coneLimit;
    coneLimit = NULL;
  }
...
================
*/
idAFConstraint_Hinge::idAFConstraint_Hinge(const idStr& name, idAFBody* body1,
                                           idAFBody* body2) {
  assert(body1);
...
================
*/
void idAFConstraint_Hinge::SetLimit(const idVec3& axis, const float angle,
                                    const idVec3& body1Axis) {
  if (!coneLimit) {
    coneLimit = new (TAG_PHYSICS_AF) idAFConstraint_ConeLimit;
    coneLimit->SetPhysics(physics);
...
================
*/
idAFConstraint_Slider::idAFConstraint_Slider(const idStr& name, idAFBody* body1,
                                             idAFBody* body2) {
  assert(body1);
...
================
*/
idAFConstraint_Line::idAFConstraint_Line(const idStr& name, idAFBody* body1,
                                         idAFBody* body2) {
  assert(0);  // FIXME: implement
}
...
================
*/
idAFConstraint_Plane::idAFConstraint_Plane(const idStr& name, idAFBody* body1,
                                           idAFBody* body2) {
  assert(body1);
...
================
*/
idAFConstraint_Spring::idAFConstraint_Spring(const idStr& name, idAFBody* body1,
                                             idAFBody* body2) {
  assert(body1);
...
================
*/
void idAFConstraint_Spring::SetSpring(const float stretch, const float compress,
                                      const float damping,
...
================
*/
void idAFConstraint_Contact::Setup(idAFBody* b1, idAFBody* b2,
                                   contactInfo_t& c) {
  idVec3 p;
  idVec6 v;
  float vel;
...
================
*/
void idAFConstraint_ConeLimit::Setup(idAFBody* b1, idAFBody* b2,
...
================
*/
void idAFConstraint_PyramidLimit::Setup(idAFBody* b1, idAFBody* b2,
...
================
*/
void idAFConstraint_Suspension::Setup(const char* name, idAFBody* body,
...
================
*/
void idAFConstraint_Suspension::SetSuspension(const float up, const float down,
                                              const float k, const float d,
                                              const float f) {
  suspensionUp = up;
  suspensionDown = down;
...
================
*/
idAFBody::~idAFBody() { delete clipModel; }
...
================
*/
inline void idAFBody::InverseWorldSpatialInertiaMultiply(idVecX& dst,
                                                         const float* v) const {
...
================
*/
void idAFTree::Response(const idAFConstraint* constraint, int row,
...
void idPhysics_AF::RemoveFrameConstraints() {
  ...
  auxiliaryConstraints.SetNum(auxiliaryConstraints.Num() -
                              frameConstraints.Num());
...
================
*/
void idPhysics_AF::SetJointFrictionDent(const float dent, const float start,
                                        const float end) {
...
================
*/
void idPhysics_AF::SetContactFrictionDent(const float dent, const float start,
                                          const float end) {
...
================
*/
void idPhysics_AF::PutToRest() { Rest(); }
...
================
*/
void idPhysics_AF::EnableImpact() { noImpact = false; }
...
================
*/
void idPhysics_AF::DisableImpact() { noImpact = true; }
...
================
*/
void idPhysics_AF::SetClipModel(idClipModel* model, float density, int id,
                                bool freeOld) {}
...
================
*/
int idPhysics_AF::GetNumClipModels() const { return bodies.Num(); }
...
================
*/
int idPhysics_AF::GetTime() const { return gameLocal.time; }
...
void DrawTraceModelSilhouette(const idVec3& projectionOrigin,
  ...
  for (i = 0; i < numSilEdges; i++) {
    v1 = trm->verts[trm->edges[abs(silEdges[i])]
                        .v[INT32_SIGNBITSET(silEdges[i])]];
    v2 = trm->verts[trm->edges[abs(silEdges[i])]
                        .v[INT32_SIGNBITNOTSET(silEdges[i])]];
...
void idPhysics_AF::BuildTrees() {
  ...
  if (af_useLinearTime.GetBool()) {
    ...
    if (trees.Num() > 1) {
      gameLocal.Warning(
          "Articulated figure has multiple separate tree structures for entity "
          "'%s' type '%s'.",
          self->name.c_str(), self->GetType()->classname);
    }
...
================
*/
int idPhysics_AF::GetNumBodies() const { return bodies.Num(); }
...
================
*/
int idPhysics_AF::GetNumConstraints() const { return constraints.Num(); }
...
================
*/
void idPhysics_AF::SetDefaultFriction(float linear, float angular,
                                      float contact) {
  if (linear < 0.0f || linear > 1.0f || angular < 0.0f || angular > 1.0f ||
...
================
*/
void idPhysics_AF::GetImpactInfo(const int id, const idVec3& point,
...
================
*/
void idPhysics_AF::ApplyImpulse(const int id, const idVec3& point,
  ...
  idMat3 invWorldInertiaTensor = bodies[id]->current->worldAxis.Transpose() *
                                 ...
                                 bodies[id]->current->worldAxis;
  bodies[id]->current->spatialVelocity.SubVec3(0) +=
      bodies[id]->invMass * impulse.Truncate(maxImpulse);
  bodies[id]->current->spatialVelocity.SubVec3(1) +=
      invWorldInertiaTensor * (point - bodies[id]->current->worldOrigin)
                                  .Cross(impulse)
                                  .Truncate(maxRotation);
  Activate();
}
...
================
*/
void idPhysics_AF::AddForce(const int id, const idVec3& point,
                            const idVec3& force) {
  if (noImpact) {
    return;
  }
...
================
*/
bool idPhysics_AF::IsAtRest() const { return current.atRest >= 0; }
...
================
*/
int idPhysics_AF::GetRestStartTime() const { return current.atRest; }
...
================
*/
void idPhysics_AF::ClipTranslation(trace_t& results, const idVec3& translation,
...
================
*/
void idPhysics_AF::ClipRotation(trace_t& results, const idRotation& rotation,
                                const idClipModel* model) const {
  int i;
  idAFBody* body;
  trace_t bodyResults;
