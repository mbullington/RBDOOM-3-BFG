...
float idPhysics_Player::CmdScale(const usercmd_t& cmd) const {
  ...
  // specified
  if (!walking) {
    upmove = ((cmd.buttons & BUTTON_JUMP) ? 127 : 0) -
             ((cmd.buttons & BUTTON_CROUCH) ? 127 : 0);
  }
...
==============
*/
void idPhysics_Player::Accelerate(const idVec3& wishdir, const float wishspeed,
                                  const float accel) {
#if 1
  // q2 style
...
bool idPhysics_Player::SlideMove(bool gravity, bool stepUp, bool stepDown,
                                 bool push) {
  int i, j, k, pushFlags;
...
void idPhysics_Player::WaterMove() {
  ...
  if (!scale) {
    ...
    wishvel -= scale * gravityNormal *
               (((command.buttons & BUTTON_JUMP) ? 127 : 0) -
                ((command.buttons & BUTTON_CROUCH) ? 127 : 0));
  }
...
void idPhysics_Player::LadderMove() {
  ...
  if (command.buttons & (BUTTON_JUMP | BUTTON_CROUCH)) {
    wishvel += -0.5f * gravityNormal * scale *
               (float)(((command.buttons & BUTTON_JUMP) ? 127 : 0) -
                       ((command.buttons & BUTTON_CROUCH) ? 127 : 0));
  }
...
================
*/
waterLevel_t idPhysics_Player::GetWaterLevel() const { return waterLevel; }
...
================
*/
int idPhysics_Player::GetWaterType() const { return waterType; }
...
================
*/
bool idPhysics_Player::OnLadder() const { return ladder; }
...
void idPhysics_Player::SetPlayerInput(const usercmd_t& cmd,
                                      const idVec3& forwardVector) {
  command = cmd;
  commandForward =
      forwardVector;  // can't use cmd.angles cause of the delta_angles
}
...
================
*/
float idPhysics_Player::GetMaxStepHeight() const { return maxStepHeight; }
...
================
*/
void idPhysics_Player::SetDebugLevel(bool set) { debugLevel = set; }
...
bool idPhysics_Player::Interpolate(const float fraction) {
  ...
  */
  ...
  if (deltaLengthSq > pm_clientInterpolation_Divergence.GetFloat()) {
    idLib::Printf(
        "Client Interpolation Divergence exceeded, snapping client to next "
        "position\n");
  ...
  // current.localOrigin = Lerp( previous.localOrigin, next.localOrigin,
  // fraction );
  if (self != NULL && (self->entityNumber != gameLocal.GetLocalClientNum())) {
    current.velocity = Lerp(previous.velocity, next.velocity, fraction);
  }
  // current.pushVelocity = Lerp( previous.pushVelocity, next.pushVelocity,
  // fraction );

  // current.movementTime = Lerp( previous.movementTime, next.movementTime,
  // fraction ); current.stepUp = Lerp( previous.stepUp, next.stepUp, fraction
  // );
  ...
  // const playerPState_t & flagStateToUse = ( fraction < 0.5f ) ? previous :
  // next;
...
================
*/
int idPhysics_Player::GetTime() const { return gameLocal.time; }
...
================
*/
void idPhysics_Player::GetImpactInfo(const int id, const idVec3& point,
...
================
*/
void idPhysics_Player::ApplyImpulse(const int id, const idVec3& point,
...
================
*/
bool idPhysics_Player::IsAtRest() const { return false; }
...
================
*/
int idPhysics_Player::GetRestStartTime() const { return -1; }
...
void idPhysics_Player::SetPushed(int deltaTime) {
  ...
  if (self->entityNumber != gameLocal.GetLocalClientNum() &&
      common->IsClient()) {
    return;
  }
...
NOTE: Aside from the velocity hack, this MUST be identical to
idPhysics_Player::SetPushed
================
*/
void idPhysics_Player::SetPushedWithAbnormalVelocityHack(int deltaTime) {
  ...
  // There is a bug where on the first 1 to 2 frames after a load, the player on
  // the boat in le_hell_post will be pushed an abnormal amount by the boat
  // mover, causing them to be thrown off of the boat.
  //
  // We're resolving this by just watching for the abnormal velocities and
  // ignoring the push in those cases.  Since it is literally only 1 or 2
  // frames, the remaining updates should continue to push the player by sane
  // values.
  //
  const float ABNORMAL_VELOCITY =
      600.0f;  // anything with a magnitude of this or higher will be ignored
...
================
*/
void idPhysics_Player::ClearPushedVelocity() { current.pushVelocity.Zero(); }
...
bool idPhysics_Player::ClientPusherLocked(bool& justBecameUnlocked) {
  ...
  for (int i = 0; i < contacts.Num(); i++) {
    ...
    if (ent) {
      ...
      if (p != NULL) {
        // Testing IsAtRest seems cleaner but there are edge cases of clients
        // jumping right before a mover starts to move
        if (p->IsType(idPhysics_Static::Type) == false &&
            p->IsType(idPhysics_StaticMulti::Type) == false) {
          hasPhysicsContact = true;

          clientPusherLocked = true;  // locked until you have a ground contact
                                      // that isn't a non static phys obj

          // HACK - Tomiko Reactor rotating disks screw up if server locks the
          // pushed clients, but elevators need clients to be locked ( otherwise
          // clients will clip through elevators )
          if (strcmp(ent->GetName(), "cylinder_disk1") == 0 ||
              strcmp(ent->GetName(), "cylinder_disk2") == 0 ||
              strcmp(ent->GetName(), "cylinder_disk3") == 0) {
...
void idPhysics_Player::ReadFromSnapshot(const idBitMsg& msg) {
  ...
  next.velocity[0] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                   PLAYER_VELOCITY_MANTISSA_BITS);
  next.velocity[1] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                   PLAYER_VELOCITY_MANTISSA_BITS);
  next.velocity[2] = msg.ReadFloat(PLAYER_VELOCITY_EXPONENT_BITS,
                                   PLAYER_VELOCITY_MANTISSA_BITS);
  // idLib::Printf("Reading Velocity: x %2f, y %2f, z %2f \n", next.velocity[0],
  // next.velocity[1], next.velocity[2] );
