...
#ifdef TEST_COLLISION_DETECTION
  if (gameLocal.clip.Contents(next.i.position, clipModel, next_.i.orientation,
                              clipMask, self)) {
    if (!startsolid) {
      int bah = 1;
    }
...
idPhysics_RigidBody::idPhysics_RigidBody() {
  ...
  integrator = new (TAG_PHYSICS) idODE_Euler(
      sizeof(rigidBodyIState_t) / sizeof(float), RigidBodyDerivatives, this);
...
void idPhysics_RigidBody::SetClipModel(idClipModel* model, const float density,
                                       int id, bool freeOld) {
  int minIndex;
  idMat3 inertiaScale;
...
================
*/
int idPhysics_RigidBody::GetNumClipModels() const { return 1; }
...
================
*/
float idPhysics_RigidBody::GetMass(int id) const { return mass; }
...
================
*/
void idPhysics_RigidBody::SetFriction(const float linear, const float angular,
                                      const float contact) {
...
================
*/
void idPhysics_RigidBody::NoContact() { noContact = true; }
...
================
*/
void idPhysics_RigidBody::PutToRest() { Rest(); }
...
================
*/
void idPhysics_RigidBody::EnableImpact() { noImpact = false; }
...
================
*/
void idPhysics_RigidBody::DisableImpact() { noImpact = true; }
...
bool idPhysics_RigidBody::Interpolate(const float fraction) {
  ...
  if (self->GetInterpolationBehavior() == idEntity::USE_LATEST_SNAP_ONLY) {
    ...
    current.i.orientation = idQuat()
                                .Slerp(previous.i.orientation.ToQuat(),
                                       next.i.orientation.ToQuat(), fraction)
                                .ToMat3();
    current.i.linearMomentum =
        Lerp(previous.i.linearMomentum, next.i.linearMomentum, fraction);
    return true;
  }
...
================
*/
void idPhysics_RigidBody::ResetInterpolationState(const idVec3& origin,
                                                  const idMat3& axis) {
  previous = current;
  next = current;
}
...
================
*/
int idPhysics_RigidBody::GetTime() const { return gameLocal.time; }
...
================
*/
void idPhysics_RigidBody::GetImpactInfo(const int id, const idVec3& point,
...
================
*/
void idPhysics_RigidBody::ApplyImpulse(const int id, const idVec3& point,
...
================
*/
void idPhysics_RigidBody::AddForce(const int id, const idVec3& point,
                                   const idVec3& force) {
  if (noImpact) {
    return;
  }
...
================
*/
bool idPhysics_RigidBody::IsAtRest() const { return current.atRest >= 0; }
...
================
*/
int idPhysics_RigidBody::GetRestStartTime() const { return current.atRest; }
...
================
*/
void idPhysics_RigidBody::DisableClip() { clipModel->Disable(); }
...
================
*/
void idPhysics_RigidBody::EnableClip() { clipModel->Enable(); }
...
================
*/
void idPhysics_RigidBody::UnlinkClip() { clipModel->Unlink(); }
...
void idPhysics_RigidBody::ReadFromSnapshot(const idBitMsg& msg) {
  ...
  next.i.linearMomentum[0] =
      msg.ReadFloat(RB_MOMENTUM_EXPONENT_BITS, RB_MOMENTUM_MANTISSA_BITS);
  next.i.linearMomentum[1] =
      msg.ReadFloat(RB_MOMENTUM_EXPONENT_BITS, RB_MOMENTUM_MANTISSA_BITS);
  next.i.linearMomentum[2] =
      msg.ReadFloat(RB_MOMENTUM_EXPONENT_BITS, RB_MOMENTUM_MANTISSA_BITS);
  ...
  if (clipModel) {
    clipModel->Link(gameLocal.clip, self, clipModel->GetId(), next.i.position,
                    next.i.orientation);
  }
}
