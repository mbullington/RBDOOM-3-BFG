...
class idClipModel {
 ...
 public:
  ...
  void Link(idClip& clp, idEntity* ent, int newId, const idVec3& newOrigin,
            const idMat3& newAxis, int renderModelHandle = -1);
  ...
  void GetMassProperties(const float density, float& mass, idVec3& centerOfMass,
...
inline void idClipModel::Enable() { enabled = true; }

inline void idClipModel::Disable() { enabled = false; }

inline void idClipModel::SetMaterial(const idMaterial* m) { material = m; }

inline const idMaterial* idClipModel::GetMaterial() const { return material; }
...
inline void idClipModel::SetEntity(idEntity* newEntity) { entity = newEntity; }

inline idEntity* idClipModel::GetEntity() const { return entity; }

inline void idClipModel::SetId(int newId) { id = newId; }

inline int idClipModel::GetId() const { return id; }

inline void idClipModel::SetOwner(idEntity* newOwner) { owner = newOwner; }

inline idEntity* idClipModel::GetOwner() const { return owner; }

inline const idBounds& idClipModel::GetBounds() const { return bounds; }

inline const idBounds& idClipModel::GetAbsBounds() const { return absBounds; }

inline const idVec3& idClipModel::GetOrigin() const { return origin; }

inline const idMat3& idClipModel::GetAxis() const { return axis; }

inline bool idClipModel::IsRenderModel() const {
...
inline bool idClipModel::IsLinked() const { return (clipLinks != NULL); }

inline bool idClipModel::IsEnabled() const { return enabled; }
...
inline const idTraceModel* idClipModel::GetTraceModel() const {
  if (!IsTraceModel()) {
    return NULL;
  }
...
class idClip {
 ...
 public:
  ...
  bool Translation(trace_t& results, const idVec3& start, const idVec3& end,
                   const idClipModel* mdl, const idMat3& trmAxis,
                   int contentMask, const idEntity* passEntity);
  bool Rotation(trace_t& results, const idVec3& start,
                const idRotation& rotation, const idClipModel* mdl,
                const idMat3& trmAxis, int contentMask,
                const idEntity* passEntity);
  bool Motion(trace_t& results, const idVec3& start, const idVec3& end,
              const idRotation& rotation, const idClipModel* mdl,
              const idMat3& trmAxis, int contentMask,
              const idEntity* passEntity);
  int Contacts(contactInfo_t* contacts, const int maxContacts,
               ...
               const idEntity* passEntity);
  int Contents(const idVec3& start, const idClipModel* mdl,
               const idMat3& trmAxis, int contentMask,
               const idEntity* passEntity);
  ...
  bool TracePoint(trace_t& results, const idVec3& start, const idVec3& end,
                  int contentMask, const idEntity* passEntity);
  bool TraceBounds(trace_t& results, const idVec3& start, const idVec3& end,
                   const idBounds& bounds, int contentMask,
                   const idEntity* passEntity);

  // clip versus a specific model
  void TranslationModel(trace_t& results, const idVec3& start,
                        const idVec3& end, const idClipModel* mdl,
                        const idMat3& trmAxis, int contentMask,
                        cmHandle_t model, const idVec3& modelOrigin,
                        const idMat3& modelAxis);
  void RotationModel(trace_t& results, const idVec3& start,
                     const idRotation& rotation, const idClipModel* mdl,
                     const idMat3& trmAxis, int contentMask, cmHandle_t model,
                     const idVec3& modelOrigin, const idMat3& modelAxis);
  int ContactsModel(contactInfo_t* contacts, const int maxContacts,
                    const idVec3& start, const idVec6& dir, const float depth,
                    const idClipModel* mdl, const idMat3& trmAxis,
                    int contentMask, cmHandle_t model,
                    const idVec3& modelOrigin, const idMat3& modelAxis);
  int ContentsModel(const idVec3& start, const idClipModel* mdl,
                    const idMat3& trmAxis, int contentMask, cmHandle_t model,
                    const idVec3& modelOrigin, const idMat3& modelAxis);

  // clip versus all entities but not the world
  void TranslationEntities(trace_t& results, const idVec3& start,
                           const idVec3& end, const idClipModel* mdl,
                           const idMat3& trmAxis, int contentMask,
                           const idEntity* passEntity);
  ...
  int EntitiesTouchingBounds(const idBounds& bounds, int contentMask,
                             idEntity** entityList, int maxCount) const;
  ...
  void DrawClipModels(const idVec3& eye, const float radius,
                      const idEntity* passEntity);
 ...
 private:
  ...
  void TraceRenderModel(trace_t& trace, const idVec3& start, const idVec3& end,
                        const float radius, const idMat3& axis,
                        idClipModel* touch) const;
};

inline bool idClip::TracePoint(trace_t& results, const idVec3& start,
                               const idVec3& end, int contentMask,
                               const idEntity* passEntity) {
...
inline bool idClip::TraceBounds(trace_t& results, const idVec3& start,
                                const idVec3& end, const idBounds& bounds,
                                int contentMask, const idEntity* passEntity) {
...
inline const idBounds& idClip::GetWorldBounds() const { return worldBounds; }

inline idClipModel* idClip::DefaultClipModel() { return &defaultClipModel; }
