...
idCVar com_forceGenericSIMD("com_forceGenericSIMD", "0",
...
idCVar net_usercmd_timing_debug("net_usercmd_timing_debug", "0", CVAR_BOOL,
                                "Print messages about usercmd timing.");

// List of all defs used by the player that will stay on the fast timeline
static const char* fastEntityList[] = {"player_doommarine",
                                       "weapon_chainsaw",
                                       "weapon_fists",
                                       "weapon_flashlight",
...
void idGameLocal::GetSaveGameDetails(idSaveGameDetails& gameDetails) {
  idLocationEntity* locationEnt =
      LocationForPoint(gameLocal.GetLocalPlayer()->GetEyePosition());
  const char* locationStr = locationEnt
                                ? locationEnt->GetLocation()
                                : idLocalization::GetString("#str_02911");
  ...
  const idDeclEntityDef* mapDef = static_cast<const idDeclEntityDef*>(
      declManager->FindType(DECL_MAPDEF, shortMapName, false));
  const char* mapPrettyName =
      mapDef ? idLocalization::GetString(
                   mapDef->dict.GetString("name", shortMapName))
             : shortMapName.c_str();
  idPlayer* player = GetClientByNum(0);
  int playTime = player ? player->GetPlayedTime() : 0;
  gameExpansionType_t expansionType =
      player ? player->GetExpansionType() : GAME_BASE;

  gameDetails.descriptors.Clear();
  gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_EXPANSION,
                                 expansionType);
  ...
  // even though we don't use this when we enumerate, when we save, we use this
  // descriptors file later so we need the date populated now
...
void idGameLocal::Printf(const char* fmt,
                         ...) const  // DG: FIXME: printf-annotation
{
  va_list argptr;
...
============
*/
const idDict& idGameLocal::GetServerInfo() { return serverInfo; }
...
void idGameLocal::MapRestart() {
  if (common->IsClient()) {
    ...
    idBitMsg msg;
    session->GetActingGameStateLobbyBase().SendReliable(
        GAME_RELIABLE_MESSAGE_RESTART, msg, false);
...
void idGameLocal::MapPopulate() {
  ...
  // Must set GAME_FPS for script after populating, because some maps run their
  // own scripts when spawning the world, and GAME_FPS will not be found before
  // then.
...
void idGameLocal::PopulateEnvironmentProbes() {
  ...
  for (int i = 0; i < numAreas; i++) {
    ...
    if (areaNum < 0) {
      Warning(
          "PopulateEnvironmentProbes: location '%i' is not in a valid area\n",
          i);
      continue;
    }
...
void idGameLocal::InitFromNewMap(const char* mapName,
                                 ...
                                 idSoundWorld* soundWorld, int gameMode,
                                 int randseed) {
  this->gameType =
      (gameType_t)idMath::ClampInt(GAME_SP, GAME_COUNT - 1, gameMode);
...
bool idGameLocal::InitFromSaveGame(
    const char* mapName, idRenderWorld* renderWorld, idSoundWorld* soundWorld,
    idFile* saveGameFile, idFile* stringTableFile, int saveGameVersion) {
  int i;
  int num;
  idEntity* ent;
  idDict si;
  ...
  idFile_SaveGamePipelined* pipelineFile =
      new (TAG_SAVEGAMES) idFile_SaveGamePipelined();
  ...
  // There should only be 1 player and 1 lobby user, but I'm using a loop just
  // to be safe
...
int idGameLocal::GetLocalClientNum() const {
  localUserHandle_t localUserHandle =
      session->GetSignInManager().GetMasterLocalUserHandle();
...
void idGameLocal::SpawnPlayer(int clientNum) {
  ...
  // It's important that we increment numClients before calling SpawnEntityDef,
  // because some entities want to check gameLocal.numClients to see who to
  // operate on (such as target_removeweapons)
...
idPlayer* idGameLocal::GetLocalPlayer() const {
  ...
  if (!entities[GetLocalClientNum()] ||
      !entities[GetLocalClientNum()]->IsType(idPlayer::Type)) {
...
================
*/
const idVec3& idGameLocal::GetGravity() const { return gravity; }
...
void idGameLocal::RunFrame(idUserCmdMgr& cmdMgr, gameReturn_t& ret) {
  ...
  if (!common->IsMultiplayer() && g_stopTime.GetBool()) {
    ...
    do {
      ...
      // This is done after the main think so that all projectiles will be
      // up-to-date when snapshots are created.
      // if ( common->IsMultiplayer() ) {
      // while ( SimulateProjectiles() ) {
      // clientGame.gameLibEffects.Update( clientGame.GetGameMs(),
      // clientGame.GetGameMsPerFrame(), clientGame.GetServerGameTime() );
      //}
      //}
    ...
    // i think this loop always play all that content until we skip the
    // cinematic, when then we keep until the end of the function
  }

  // ret.syncNextGameFrame = skipCinematic; // this is form dhewm3 but it seems
  ...
  if (sessionCommand.Length()) {
    ...
    sessionCommand
        .Clear();  // Fixes a double loading bug for the e3 demo.  Since we run
                   ...
                   // re-set the ret.sessionCommand to a stale sessionCommand
                   // (since that doesn't get cleared until LoadMap is called!)
  }
}
...
void idGameLocal::RunSingleUserCmd(usercmd_t& cmd, idPlayer& player) {
  ...
  // milliseconds in the case of the server running at 60Hz and the client
  // running at 120Hz, we need to set the script's GAME_FPS value to the
  // client's effective rate. (I'd like to just fix the script to use
  // milliseconds, but we don't want to change assets at this point.)
  if (!player.IsLocallyControlled()) {
    const float usercmdMillisecondDelta =
        player.usercmd.clientGameMilliseconds -
        player.oldCmd.clientGameMilliseconds;
    ...
    const float forcedClientEngineHz =
        (clientEngineHz < 90.0f) ? 60.0f : 120.0f;
  ...
  if (!common->IsMultiplayer() || common->IsServer()) {
    ...
    player.ClientThink(netInterpolationInfo.serverGameMs,
                       netInterpolationInfo.pct, true);
  }

  // Since the client is authoritative on its position, we have to update the
  // usercmd that will be sent over the network after the player thinks.
...
void idGameLocal::RunAllUserCmdsForPlayer(idUserCmdMgr& cmdMgr,
  ...
  // Make sure to run a command for remote players. May duplicate the previous
  // command if the server is running faster, or run an "empty" command if the
  // buffer underflows.
  if (cmdMgr.HasUserCmdForPlayer(player.GetEntityNumber())) {
    const int clientTimeOfNextCommand =
        cmdMgr.GetNextUserCmdClientTime(playerNumber);
    const int timeDeltaBetweenClientCommands =
        clientTimeOfNextCommand - lastCmdRunTimeOnClient[playerNumber];
    const int timeSinceServerRanLastCommand =
        gameLocal.time - lastCmdRunTimeOnServer[playerNumber];
    int clientTimeRunSoFar = 0;

    // Handle clients who may be running faster than the server. Potentiallly
    // runs multiple usercmds so that the server can catch up.
    if (timeDeltaBetweenClientCommands - timeSinceServerRanLastCommand <= 1) {
      while (clientTimeRunSoFar < (timeSinceServerRanLastCommand - 1) &&
             cmdMgr.HasUserCmdForPlayer(player.GetEntityNumber())) {
        usercmd_t& currentCommand =
            cmdMgr.GetWritableUserCmdForPlayer(playerNumber);
        RunSingleUserCmd(currentCommand, player);
        lastCmdRunTimeOnClient[playerNumber] =
            currentCommand.clientGameMilliseconds;
        ...
        if (net_usercmd_timing_debug.GetBool()) {
          idLib::Printf(
              "[%d]Running initial cmd for player %d, %d buffered, %d so far, "
              "%d serverDelta.\n",
              common->GetGameFrame(), playerNumber,
              cmdMgr.GetNumUnreadFrames(playerNumber), clientTimeRunSoFar,
              timeSinceServerRanLastCommand);
        }
      }
    } else {
      // If we get here, it is likely that the client is running at 60Hz but the
      // server is running at 120Hz. Duplicate the previous usercmd and run it
      // so that the server doesn't starve.
      usercmd_t lastPlayerCmd = player.usercmd;
      RunSingleUserCmd(lastPlayerCmd, player);
      if (net_usercmd_timing_debug.GetBool()) {
        idLib::Printf(
            "[%d]Running duplicated command for player %d to prevent server "
            "from starving. clientCmdTimeDelta = %d, runCmdTimeDeltaOnServer = "
            "%d.\n",
            common->GetGameFrame(), playerNumber,
            timeDeltaBetweenClientCommands, timeSinceServerRanLastCommand);
      }
    }
  } else {
  ...
  for (; numPasses < MaxExtraCommandsPerFrame; numPasses++) {
    ...
    const int extraFrameMilliseconds =
        FRAME_TO_MSEC(common->GetGameFrame() + 2) -
        FRAME_TO_MSEC(common->GetGameFrame());
    const int millisecondBuffer =
        MSEC_ALIGN_TO_FRAME(net_ucmdRate.GetInteger() + extraFrameMilliseconds);

    const bool hasNextCmd =
        cmdMgr.HasUserCmdForClientTimeBuffer(playerNumber, millisecondBuffer);
    ...
    if (hasNextCmd) {
      ...
      if (net_usercmd_timing_debug.GetBool()) {
        idLib::Printf(
            "[%d]Pass %d, running extra cmd for player %d, %d buffered\n",
...
================
*/
const char* idGameLocal::GetMapName() const { return mapFileName.c_str(); }
...
==================
*/
int idGameLocal::NumAAS() const { return aasList.Num(); }
...
void idGameLocal::SetAASAreaState(const idBounds& bounds,
                                  const int areaContents, bool closed) {
  int i;
...
===================
*/
void idGameLocal::RegisterEntity(idEntity* ent, int forceSpawnId,
                                 const idDict& spawnArgsToCopy) {
  int spawn_entnum;

  ent->fl.skipReplication =
      spawnArgsToCopy.GetBool("net_skip_replication", false);
  ...
  if (!spawnArgsToCopy.GetInt("spawn_entnum", "0", spawn_entnum)) {
    const int freeListType =
        (common->IsMultiplayer() && ent->GetSkipReplication()) ? 1 : 0;
    const int maxEntityNum =
        (common->IsMultiplayer() && !ent->GetSkipReplication())
            ? ENTITYNUM_FIRST_NON_REPLICATED
...
void idGameLocal::UnregisterEntity(idEntity* ent) {
  ...
  if ((ent->entityNumber != ENTITYNUM_NONE) &&
    ...
    int freeListType = (common->IsMultiplayer() &&
                        ent->entityNumber >= ENTITYNUM_FIRST_NON_REPLICATED)
                           ? 1
                           : 0;
...
===================
*/
bool idGameLocal::SpawnEntityDef(const idDict& args, idEntity** ent,
                                 bool setDefaults) {
...
bool idGameLocal::InhibitEntitySpawn(idDict& spawnArgs) {
  ...
  // RB: TrenchBroom interop skip func_group entities
  {
    ...
    if (idStr::Icmp(name, "func_group") == 0 &&
        (idStr::Icmp(groupType, "_tb_group") == 0 ||
         idStr::Icmp(groupType, "_tb_layer") == 0)) {
      result = true;
    }
  }
...
==============
*/
gameState_t idGameLocal::GameState() const { return gamestate; }
...
=============
*/
idEntity* idGameLocal::FindTraceEntity(idVec3 start, idVec3 end,
                                       const idTypeInfo& c,
...
================
*/
int idGameLocal::EntitiesWithinRadius(const idVec3 org, float radius,
                                      idEntity** entityList,
                                      int maxCount) const {
  idEntity* ent;
  idBounds bo(org);
...
================
*/
bool idGameLocal::RequirementMet(idEntity* activator, const idStr& doesReq,
                                 int removeItem) {
  if (doesReq.Length()) {
    if (activator->IsType(idPlayer::Type)) {
      idPlayer* player = static_cast<idPlayer*>(activator);
      idDict* item = player->FindInventoryItem(doesReq);
      if (item) {
        if (removeItem) {
...
============
*/
void idGameLocal::RadiusDamage(const idVec3& origin, idEntity* inflictor,
                               idEntity* attacker, idEntity* ignoreDamage,
                               idEntity* ignorePush, const char* damageDefName,
                               float dmgPower) {
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    if (common->IsMultiplayer() && ent->entityNumber < MAX_CLIENTS &&
    ...
    if (ent->CanDamage(origin, damagePoint)) {
      ...
      if (!common->IsMultiplayer() &&
          ent->entityNumber == GetLocalClientNum()) {
        if (ent->IsType(idPlayer::Type)) {
      ...
      // if our inflictor is a projectile, we want to count up how many kills
      // the splash damage has chalked up.
...
==============
*/
void idGameLocal::RadiusPush(const idVec3& origin, const float radius,
                             const float push, const idEntity* inflictor,
                             const idEntity* ignore, float inflictorScale,
                             const bool quake) {
...
==============
*/
void idGameLocal::RadiusPushClipModel(const idVec3& origin, const float push,
  ...
  for (i = 0; i < trm->numPolys; i++) {
    ...
    for (j = 0; j < poly->numEdges; j++) {
      v = trm->verts[trm->edges[abs(poly->edges[j])]
                         .v[INT32_SIGNBITSET(poly->edges[j])]];
      center += v;
      v -= localOrigin;
...
===============
*/
void idGameLocal::ProjectDecal(const idVec3& origin, const idVec3& dir,
                               float depth, bool parallel, float size,
                               const char* material, float angle) {
  float s, c;
...
==============
*/
void idGameLocal::BloodSplat(const idVec3& origin, const idVec3& dir,
...
=============
*/
idCamera* idGameLocal::GetCamera() const { return camera; }
...
=============
*/
bool idGameLocal::SkipCinematicScene() { return SkipCinematic(); }
/*
=============
idGameLocal::CheckInCinematic
=============
*/
bool idGameLocal::CheckInCinematic() { return inCinematic; }
/*
======================
...
================
*/
const idMaterial* idGameLocal::GetGlobalMaterial() { return globalMaterial; }
...
=================
*/
bool idGameLocal::IsPortalSkyAcive() { return portalSkyActive; }
...
============
*/
void idGameLocal::QuickSlowmoReset() { quickSlowmoReset = true; }
...
================
*/
int idGameLocal::GetMPGameModes(const char*** gameModes,
                                const char*** gameModesDisplay) {
...
bool idGameLocal::IsPlayerChatting() const {
  return GetLocalPlayer() && (GetLocalPlayer()->isChatting > 0) &&
         !GetLocalPlayer()->spectating;
}
...
bool idGameLocal::InhibitControls() {
  return (Shell_IsActive() || IsPDAOpen() || IsPlayerChatting() ||
          (common->IsMultiplayer() && mpGame.IsScoreboardActive()));
}
...
===============================
*/
void idGameLocal::Leaderboards_Init() { LeaderboardLocal_Init(); }
...
===============================
*/
void idGameLocal::Leaderboards_Shutdown() { LeaderboardLocal_Shutdown(); }
...
void idGameLocal::Shell_ClosePause() {
  if (shellHandler != NULL) {
    if (!common->IsMultiplayer() && GetLocalPlayer() &&
        GetLocalPlayer()->health <= 0) {
      return;
    }
...
========================
*/
void idGameLocal::Shell_UpdateLeaderboard(
...
bool idGameLocal::SimulateProjectiles() {
  ...
  for (int i = 0; i < idProjectile::MAX_SIMULATED_PROJECTILES; i++) {
    if (idProjectile::projectilesToSimulate[i].projectile != NULL &&
      ...
      idProjectile::projectilesToSimulate[i]
          .projectile->SimulateProjectileFrame(endTime - startTime, endTime);

      if (idProjectile::projectilesToSimulate[i].projectile != NULL) {
        if (endTime >= previousServerTime) {
          idProjectile::projectilesToSimulate[i].projectile->PostSimulate(
              endTime);
          idProjectile::projectilesToSimulate[i].startTime = 0;
