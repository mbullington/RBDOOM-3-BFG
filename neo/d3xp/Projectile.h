...
class idProjectile : public idEntity {
 public:
  ...
  virtual void Launch(const idVec3& start, const idVec3& dir,
  ...
  void Event_CreateProjectile(idEntity* owner, const idVec3& start,
                              const idVec3& dir);
  void Event_LaunchProjectile(const idVec3& start, const idVec3& dir,
                              const idVec3& pushVelocity);
  ...
  // This list is used to "catch up" client projectiles to the current time on
  // the server
  static idArray<simulatedProjectile_t, MAX_SIMULATED_PROJECTILES>
      projectilesToSimulate;
...
class idSoulCubeMissile : public idGuidedProjectile {
 public:
  ...
  virtual void Launch(const idVec3& start, const idVec3& dir,
                      const idVec3& pushVelocity,
                      const float timeSinceFire = 0.0f,
                      const float power = 1.0f, const float dmgPower = 1.0f);
