...
class idAimAssist {
 ...
 private:
  ...
  float ComputeEntityAimAssistScore(const idVec3& targetPos,
                                    const idVec3& cameraPos,
                                    const idMat3& cameraAxis);
  bool ComputeTargetPos(idEntity* pTarget, idVec3& primaryTargetPos,
  ...
  idEntityPtr<idEntity>
      targetEntity;      // the last target we had (updated every frame)
