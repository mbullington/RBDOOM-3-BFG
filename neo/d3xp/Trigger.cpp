    ...
    if (ent->GetPhysics()->GetContents() &
      ...
      if (!show) {
        for (i = 0; i < ent->targets.Num(); i++) {
          ...
          if (target != NULL && viewBounds.IntersectsBounds(
...
================
*/
void idTrigger::Event_Enable() { Enable(); }
...
================
*/
void idTrigger::Event_Disable() { Disable(); }
...
================
*/
idTrigger::idTrigger() { scriptFunction = NULL; }
...
void idTrigger_Multi::Event_Trigger(idEntity* activator) {
  ...
  // see if this trigger doesReq an item
...
void idTrigger_Multi::Event_Touch(idEntity* other, trace_t* trace) {
  ...
  // see if this trigger doesReq an item
  if (!gameLocal.RequirementMet(other, doesReq, removeItem)) {
    return;
  }
...
================
*/
void idTrigger_Hurt::Event_Toggle(idEntity* activator) { on = !on; }
...
================
*/
idTrigger_Touch::idTrigger_Touch() { clipModel = NULL; }
...
================
*/
void idTrigger_Touch::Enable() { BecomeActive(TH_THINK); }
...
================
*/
void idTrigger_Touch::Disable() { BecomeInactive(TH_THINK); }
