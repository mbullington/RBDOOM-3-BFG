...
idCVar achievements_Verbose("achievements_Verbose", "1", CVAR_BOOL,
                            "debug spam");
...
const struct achievementInfo_t {
  int required;
  bool lifetime;  // true means the current count is stored on the player
                  // profile.  Doesn't matter for single count achievements.
...
*/
idAchievementManager::idAchievementManager()
    : lastPlayerKilledTime(0), lastImpKilledTime(0), playerTookDamage(false) {
  counts.Zero();
  ResetHellTimeKills();
}
...
idLocalUser* idAchievementManager::GetLocalUser() {
  ...
  return session->GetGameLobbyBase().GetLocalUserFromLobbyUser(
      gameLocal.lobbyUserIDs[owner->GetEntityNumber()]);
}
...
========================
*/
void idAchievementManager::EventCompletesAchievement(
    const achievement_t eventId) {
  ...
  if (localUser == NULL || localUser->GetProfile() == NULL) {
    ...
    if (owner != NULL) {
      ...
      session->GetActingGameStateLobbyBase().SendReliableToLobbyUser(
          ...
          GAME_RELIABLE_MESSAGE_ACHIEVEMENT_UNLOCK, msg);
    }
  ...
  // If so, don't do again because we don't want to autosave every time a
  // trigger is hit
  ...
  if (common->GetConsoleUsed()) {
    if (!cheatingDialogShown) {
      common->Dialog().AddDialog(GDM_ACHIEVEMENTS_DISABLED_DUE_TO_CHEATING,
                                 DIALOG_ACCEPT, NULL, NULL, true);
...
CONSOLE_COMMAND(AchievementsList, "Lists achievements and status", NULL) {
  ...
  idLocalUser* user =
      (player == NULL) ? session->GetSignInManager().GetMasterLocalUser()
  ...
  bool achievementStateValid =
      session->GetAchievementSystem().GetAchievementState(user,
                                                          achievementState);
  ...
  for (int i = 0; i < ACHIEVEMENTS_NUM; i++) {
    ...
    bool descriptionValid =
        session->GetAchievementSystem().GetAchievementDescription(user, i,
                                                                  data);

    idLib::Printf(
        "%02d: %2d/%2d | %12.12s | %12.12s | %s%s\n", i, count,
        achievementInfo[i].required, pInfo, sInfo,
