...
idCVar cg_projectile_clientAuthoritative_maxCatchup(
    "cg_projectile_clientAuthoritative_maxCatchup", "500", CVAR_INTEGER, "");

idCVar g_useWeaponDepthHack("g_useWeaponDepthHack", "1", CVAR_BOOL,
                            "Crunch z depth on weapons");

idCVar g_weaponShadows("g_weaponShadows", "0", CVAR_BOOL | CVAR_ARCHIVE,
                       "Cast shadows from weapons");
...
void idWeapon::SetFlashlightOwner(idPlayer* _owner) {
  ...
  if (worldModel.GetEntity()) {
    worldModel.GetEntity()->SetName(
        va("%s_weapon_flashlight_worldmodel", owner->name.c_str()));
  }
}
...
================
*/
bool idWeapon::ShouldConstructScriptObjectAtSpawn() const { return false; }
...
================
*/
const char* idWeapon::Icon() const { return icon; }
...
================
*/
const char* idWeapon::PdaIcon() const { return pdaIcon; }
...
void idWeapon::UpdateFlashPosition() {
  ...
  if (isPlayerFlashlight) {
    ...
    //		muzzleFlash.origin += adjustPos.x * muzzleFlash.axis[1] + adjustPos.y
    //* muzzleFlash.axis[0] + adjustPos.z * muzzleFlash.axis[2];
    muzzleFlash.origin += owner->GetViewBob();

    //		static idAngles baseAdjustAng = ang_zero;	//idAngles( 0.0f, 10.0f,
    //0.0f );
    idAngles adjustAng =
        /*baseAdjustAng +*/ idAngles(fraccos * yscale, 0.0f, fraccos2 * pscale);
    idAngles bobAngles = owner->GetViewBobAngles();
    SwapValues(bobAngles.pitch, bobAngles.roll);
    adjustAng += bobAngles * 3.0f;
    muzzleFlash.axis =
        adjustAng.ToMat3() * muzzleFlash.axis /** adjustAng.ToMat3()*/;
  }
...
void idWeapon::FlashlightOn() {
  ...
  if (!func) {
    common->Warning("Can't find function 'TurnOn' in object '%s'",
                    scriptObject.GetTypeName());
    return;
  }
...
void idWeapon::FlashlightOff() {
  ...
  if (!func) {
    common->Warning("Can't find function 'TurnOff' in object '%s'",
                    scriptObject.GetTypeName());
    return;
  }
...
================
*/
bool idWeapon::IsReloading() const { return (status == WP_RELOAD); }
...
================
*/
bool idWeapon::IsHolstered() const { return (status == WP_HOLSTERED); }
...
bool idWeapon::ShowCrosshair() const {
  // JDC: this code would never function as written, I'm assuming they wanted
  // the following behavior
  //	return !( state == idStr( WP_RISING ) || state == idStr( WP_LOWERING )
  //|| state == idStr( WP_HOLSTERED ) );
  return !(status == WP_RISING || status == WP_LOWERING ||
           status == WP_HOLSTERED || status == WP_RELOAD);
}
...
=====================
*/
idEntity* idWeapon::DropItem(const idVec3& velocity, int activateDelay,
                             int removeDelay, bool died) {
  if (!weaponDef || !worldModel.GetEntity()) {
    return NULL;
  }
  if (!allowDrop) {
...
bool idWeapon::GetMuzzlePositionWithHacks(idVec3& origin, idMat3& axis) {
  ...
  // we probably should fix the above hacks above that are based on texture
  // names above at some point
  if (weaponDef != NULL) {
    if ((idStr::Icmp("weapon_shotgun_double", weaponDef->GetName()) == 0) ||
        (idStr::Icmp("weapon_shotgun_double_mp", weaponDef->GetName()) == 0)) {
      // joint doesn't point straight, so rotate it
      std::swap(axis[0], axis[2]);
...
void idWeapon::PresentWeapon(bool showViewModel) {
  ...
  if (isPlayerFlashlight) {
    ...
    unsigned int val =
        (gameLocal.framenum + gameLocal.framenum / divisor) & 255;
    ...
    idVec3 adjustPos =
        baseAdjustPos;  // + ( idVec3( fraccos, 0.0f, fraccos2 ) * scale );
    viewWeaponOrigin += adjustPos.x * viewWeaponAxis[1] +
                        adjustPos.y * viewWeaponAxis[0] +
                        adjustPos.z * viewWeaponAxis[2];
    //		viewWeaponOrigin += owner->viewBob;

    static idAngles baseAdjustAng = idAngles(88.0f, 10.0f, 0.0f);  //
    idAngles adjustAng =
        baseAdjustAng + idAngles(fraccos * pscale, fraccos2 * yscale, 0.0f);
        ...
        if (part->smoke) {
          ...
          if (part->emitter != NULL) {
            ...
            if (part->emitter->GetModelDefHandle() != -1) {
              gameRenderWorld->UpdateEntityDef(
  ...
  // constant rumble...
  float highMagnitude =
      weaponDef->dict.GetFloat("controllerConstantShakeHighMag");
  ...
  if (owner->IsLocallyControlled()) {
    owner->SetControllerShake(highMagnitude, highDuration, lowMagnitude,
                              lowDuration);
  }
}
...
================
*/
int idWeapon::GetZoomFov() { return zoomFov; }
...
int idWeapon::AmmoAvailable() const {
  if (owner) {
    ...
    if (g_infiniteAmmo.GetBool()) {
      return 10;  // arbitrary number, just so whatever's calling thinks there's
                  // sufficient ammo...
    } else {
      return 0;
    }
...
================
*/
int idWeapon::AmmoInClip() const { return ammoClip.Get(); }
...
================
*/
void idWeapon::ResetAmmoClip() { ammoClip = -1; }
...
================
*/
ammo_t idWeapon::GetAmmoType() const { return ammoType; }
...
================
*/
int idWeapon::ClipSize() const { return clipSize; }
...
================
*/
int idWeapon::LowAmmo() const { return lowAmmo; }
...
================
*/
int idWeapon::AmmoRequired() const { return ammoRequired; }
...
void idWeapon::ReadFromSnapshot(const idBitMsg& msg) {
  ...
  // Local clients predict the ammo in the clip. Only use the ammo cpunt from
  // the snapshot for local clients if the server has processed the same usercmd
  // in which we predicted the ammo count.
  if (owner != NULL) {
  ...
  // Only update the flashlight state if it has changed, and if this isn't the
  // local player. The local player sets their flashlight immediately for
  // responsiveness.
  if (owner != NULL && !owner->IsLocallyControlled() &&
      lightOn != snapshotLightOn) {
    if (snapshotLightOn) {
      FlashlightOn();
    } else {
      FlashlightOff();
...
bool idWeapon::ClientReceiveEvent(int event, int time, const idBitMsg& msg) {
  switch (event) {
    case EVENT_RELOAD: {
      // Local clients predict reloads, only process this event for remote
      // clients.
      if (owner != NULL && !owner->IsLocallyControlled() &&
          (gameLocal.time - time < 1000)) {
...
===============
*/
void idWeapon::Event_Clear() { Clear(); }
...
===============
*/
void idWeapon::Event_GetOwner() { idThread::ReturnEntity(owner); }
...
void idWeapon::Event_WeaponState(const char* statename, int blendFrames) {
  ...
  // HACK, Fixes reload animation on player not playing on second reload ( on
  // non local client players, and only with host viewing. )
  if (common->IsMultiplayer() &&
      strcmp(weaponDef->GetName(), "weapon_shotgun_double_mp") == 0) {
...
===============
*/
void idWeapon::Event_ClipSize() { idThread::ReturnFloat(clipSize); }
...
void idWeapon::Event_SetSkin(const char* skinname) {
  ...
  if (renderEntity.customSkin == skinDecl && worldModel.GetEntity() != NULL &&
      worldModel.GetEntity()->GetSkin() == skinDecl) {
    return;
  }
  ...
  // Hack, don't send message if flashlight, because clients process the
...
================
*/
void idWeapon::Event_SetLightParms(float parm0, float parm1, float parm2,
                                   float parm3) {
...
================
*/
void idWeapon::Event_GrabberHasTarget() { idThread::ReturnInt(grabberState); }
...
void idWeapon::GetProjectileLaunchOriginAndAxis(idVec3& origin, idMat3& axis) {
  ...
  axis = playerViewAxis;  // Fix for plasma rifle not firing correctly on
                          // initial shot of a burst fire
}
...
================
*/
void idWeapon::Event_LaunchProjectiles(int num_projectiles, float spread,
  ...
  if (common->IsServer() || owner->IsLocallyControlled()) {
    ...
    if (clipSize == 0) {
      // Weapons with a clip size of 0 launch straight from inventory without
      // moving to a clip
  ...
  // "Predict" damage effects on clients by just spawning a local projectile
  // that deals no damage. Used only for sound & visual effects. Damage will be
  // handled through reliable messages to the host.
  const bool isHitscan = projectileDict.GetBool("net_instanthit");
  const bool attackerIsLocal = owner->IsLocallyControlled();
  const bool actuallySpawnProjectile =
  ...
  if (actuallySpawnProjectile) {
    ...
    for (i = 0; i < num_projectiles; i++) {
      ...
      dir = muzzleAxis[0] + muzzleAxis[2] * (ang * idMath::Sin(spin)) -
            muzzleAxis[1] * (ang * idMath::Cos(spin));
      dir.Normalize();
      ...
      if (projectileEnt) {
        ...
        if (common->IsClient()) {
          ...
          // Must be set before spawn, so that the entity can be spawned into
          // the correct area of the entities array.
      ...
      if (i == 0) {
        ...
        if ((ownerBounds - projBounds)
                .RayIntersection(muzzle_pos, muzzleAxis[0], distance)) {
      ...
      // If this is the server simulating a remote client, the client has
      // spawned the projectile in the past. The server will catch-up the
      // projectile so that its position will be as if the projectile had
      // spawned when the client fired it.
      if (common->IsServer() && owner != NULL &&
          !owner->IsLocallyControlled() &&
          !projectileDict.GetBool("net_instanthit")) {
        int serverTimeOnClient = owner->usercmd.serverGameMilliseconds;

        int delta = idMath::ClampInt(
            0, cg_projectile_clientAuthoritative_maxCatchup.GetInteger(),
            gameLocal.GetServerGameTimeMs() - serverTimeOnClient);
        ...
        // predictively spawned, but no need to simulate - was needed for
        // correct processing of client mines when spawned on the server
        // (because we're futzing with the clip models)
        proj->QueueToSimulate(startTime);

        if (cg_predictedSpawn_debug.GetBool()) {
          idLib::Printf(
              "Spawning throw item projectile for player %d. PredictiveKey: %d "
              "\n",
...
void idWeapon::Event_LaunchProjectilesEllipse(int num_projectiles,
                                              float spreada, float spreadb,
                                              float fuseOffset, float power) {
  idProjectile* proj;
  idEntity* ent;
  int i;
  idVec3 dir;
 ...
 */
void idWeapon::Event_LaunchPowerup(const char* powerup, float duration,
                                   int useAmmo) {
  if (IsHidden()) {
    return;
  }
  ...
  owner->Give(
      powerup, va("%f", duration),
      ITEM_GIVE_FEEDBACK | ITEM_GIVE_UPDATE_STATE | ITEM_GIVE_FROM_WEAPON);
}
...
void idWeapon::Event_Melee() {
  ...
  if (!common->IsClient()) {
    ...
    if (ent != NULL) {
      ...
      // twice
      if (common->IsMultiplayer() && weaponDef->dict.GetBool("stealing") &&
...
===============
*/
void idWeapon::ClientThink(const int curTime, const float fraction,
                           const bool predict) {
  UpdateAnimation();
}
...
===============
*/
void idWeapon::ClientPredictionThink() { UpdateAnimation(); }

void idWeapon::ForceAmmoInClip() { ammoClip = clipSize; }
