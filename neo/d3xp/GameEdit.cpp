...
===============
*/
idCursor3D::idCursor3D() { draggedPosition.Zero(); }
...
void idDragEntity::Update(idPlayer* player) {
  ...
  if (drag) {
    ...
    if (joint != INVALID_JOINT && renderEntity != NULL &&
        dragAnimator != NULL) {
...
=============
*/
bool idEditEntities::SelectEntity(const idVec3& origin, const idVec3& dir,
                                  const idEntity* skip) {
  idVec3 end;
  idEntity* ent;
...
=============
*/
bool idEditEntities::EntityIsSelectable(idEntity* ent, idVec4* color,
                                        idStr* text) {
  for (int i = 0; i < selectableEntityClasses.Num(); i++) {
...
================
*/
void idGameEdit::EntityDelete(idEntity* ent) { delete ent; }
...
================
*/
void idGameEdit::MapSetEntityKeyVal(const char* name, const char* key,
...
================
*/
void idGameEdit::MapCopyDictToEntityAtOrigin(const idVec3& origin,
                                             const idDict* dict) const {
  idMapFile* mapFile = gameLocal.GetLevelMap();
  if (mapFile)  //&& name && *name )
  {
...
================
*/
int idGameEdit::MapGetUniqueMatchingKeyVals(const char* key, const char* list[],
                                            int max) const {
  idMapFile* mapFile = gameLocal.GetLevelMap();
  int count = 0;
  if (mapFile) {
