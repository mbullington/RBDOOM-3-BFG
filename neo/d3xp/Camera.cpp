...
void idCameraAnim::LoadAnim() {
  ...
  if (!filename.Length()) {
    // RB: TrenchBroom interop use anim.<name> instead so we can build this up
    // using the FGD files
    filename = spawnArgs.GetString(va("anim.%s", key));
    if (!filename.Length()) {
...
void idCameraAnim::Start() {
  ...
  if (gameLocal.GetLocalPlayer()->GetRenderView()->time[TIME_GROUP2] ==
      gameLocal.fast.time) {
...
================
*/
void idCameraAnim::Event_Start() { Start(); }
...
================
*/
void idCameraAnim::Event_Stop() { Stop(); }
