...
void idBrittleFracture::Restore(idRestoreGame* savefile) {
  ...
  for (int i = 0; i < numEvents; i++) {
    ...
    if (restoredEvent.eventType == EVENT_PROJECT_DECAL) {
      ProjectDecal(restoredEvent.point, restoredEvent.vector, gameLocal.time,
                   NULL);
    } else {
...
void idBrittleFracture::Spawn() {
  ...
  // make sure that max is greater than min  ( otherwise negative number square
  // root happens )
  if (maxShatterRadius < minShatterRadius) {
    idLib::Warning(
        "BrittleFracture, minShatterRadius(%2f) is greater than "
        "maxShatterRadius(%2f). Unknown results will ensue.",
        minShatterRadius, maxShatterRadius);
  }
...
bool idBrittleFracture::UpdateRenderEntity(
    renderEntity_s* renderEntity, const renderView_t* renderView) const {
...
================
*/
void idBrittleFracture::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
...
================
*/
void idBrittleFracture::AddForce(idEntity* ent, int id, const idVec3& point,
                                 const idVec3& force) {
  if (id < 0 || id >= shards.Num()) {
    return;
  }
...
================
*/
void idBrittleFracture::ProjectDecal(const idVec3& point, const idVec3& dir,
                                     const int time,
...
================
*/
void idBrittleFracture::DropShard(shard_t* shard, const idVec3& point,
                                  const idVec3& dir, const float impulse,
                                  const int time) {
  int i, j, clipModelId;
  float dist, f;
  idVec3 dir2, origin;
  ...
  f = dist > maxShatterRadius
          ? 1.0f
          : idMath::Sqrt(idMath::Fabs(dist - minShatterRadius)) *
                (1.0f / idMath::Sqrt(
                            idMath::Fabs(maxShatterRadius - minShatterRadius)));
...
================
*/
void idBrittleFracture::Shatter(const idVec3& point, const idVec3& impulse,
                                const int time) {
  int i;
  idVec3 dir;
  shard_t* shard;
...
================
*/
bool idBrittleFracture::IsBroken() const { return (fl.takedamage == false); }
...
================
*/
void idBrittleFracture::Killed(idEntity* inflictor, idEntity* attacker,
...
static float fractureSplitTable[] = {1365.123f, 5.324f,    1125.34f,
                                     50.34f,    555.252f,  100.12f,
                                     230.53f,   10000.87f, 10000.87f};
...
void idBrittleFracture::CreateFractures(const idRenderModel* renderModel) {
  ...
  if (isXraySurface) {
    ...
    for (int j = 0; j < surf->geometry->numIndexes; j += 3) {
      ...
      idPlane p1;
      w.GetPlane(p1);
      for (int k = j + 3; k < surf->geometry->numIndexes &&
                          (w.GetNumPoints() + 1 < MAX_POINTS_ON_WINDING);
           k += 3) {
        int i3 = indexes[k + 0];
        int i4 = indexes[k + 1];
        int i5 = indexes[k + 2];
        ...
        for (int w1i = 0; w1i < w.GetNumPoints(); w1i++) {
          for (int w2i = 0; w2i < w2.GetNumPoints(); w2i++) {
            if (CompareVec5(w[w1i], w2[w2i]) &&
                CompareVec5(w[(w1i + 1) % w.GetNumPoints()],
                            w2[(w2i + 2) % w2.GetNumPoints()])) {
              w.InsertPoint(w2[(w2i + 1) % w2.GetNumPoints()],
                            (w1i + 1) % w.GetNumPoints());
              j = k;
              found = true;
              break;
            }
...
================
*/
void idBrittleFracture::ClientThink(const int curTime, const float fraction,
                                    const bool predict) {
...
================
*/
bool idBrittleFracture::ClientReceiveEvent(int event, int time,
                                           const idBitMsg& msg) {
  idVec3 point, dir;
