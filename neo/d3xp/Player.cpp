...
idCVar flashlight_batteryDrainTimeMS(
    "flashlight_batteryDrainTimeMS", "30000", CVAR_INTEGER,
    "amount of time (in MS) it takes for full battery to drain (-1 == no "
    "battery drain)");
...
// RB: changed to internal event as it was not exposed to Doom Script by default
// and broke the automatic export
...
void idInventory::Restore(idRestoreGame* savefile) {
  ...
  for (i = 0; i < num; i++) {
    ...
    pdas.Append(
        static_cast<const idDeclPDA*>(declManager->FindType(DECL_PDA, strPda)));
  }
...
void idPlayer::DrawHUD(idMenuHandler_HUD* _hudManager) {
  ...
  if (!weapon.GetEntity() || influenceActive != INFLUENCE_NONE ||
      privateCameraView || gameLocal.GetCamera() || !g_showHud.GetBool()) {
    return;
  }
  ...
  idPlayer* localPlayer =
      static_cast<idPlayer*>(gameLocal.entities[gameLocal.GetLocalClientNum()]);
    ...
    switch (powerup) {
      case BERSERK: {
        if (giveFlags & ITEM_GIVE_FROM_WEAPON) {
          // Berserk is granted by the bloodstone in ROE, but we don't want any
          // of the standard behavior (sound fx, switch to fists) when you get
          // it this way.
        } else {
          if (giveFlags & ITEM_GIVE_FEEDBACK) {
      ...
      case INVISIBILITY: {
        ...
        /*				if ( spawnArgs.GetString( "snd_invisibility",
           "", &sound ) ) { StartSoundShader( declManager->FindSound( sound ),
...
void idPlayer::BobCycle(const idVec3& pushVelocity) {
  ...
  if (!physicsObj.HasGroundContacts() || influenceActive == INFLUENCE_LEVEL2 ||
    ...
    old = bobCycle;
    bobCycle =
        (int)(old + bobmove * (gameLocal.time - gameLocal.previousTime)) & 255;
...
bool idPlayer::AllowClientAuthPhysics() {
  ...
  return (usercmd.serverGameMilliseconds > serverOverridePositionTime &&
          commonLocal.GetUCmdMgr().HasUserCmdForPlayer(entityNumber));
}
  ...
  // Correction
  {
    ...
    if (delta > pm_clientAuthoritative_minDist.GetFloat()) {
      ...
      if (pm_clientAuthoritative_debug.GetBool() &&
        ...
        idLib::Warning(
            "Remote client player physics: delta movement for frame was %f "
            "units",
            delta);
