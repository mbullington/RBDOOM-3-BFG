...
void idMoveable::Spawn() {
  ...
  physicsObj.SetClipModel(new (TAG_PHYSICS_CLIP_MOVER) idClipModel(trm),
                          density);
...
============
*/
void idMoveable::Killed(idEntity* inflictor, idEntity* attacker, int damage,
...
================
*/
bool idMoveable::AllowStep() const { return allowStep; }
  ...
  if (initialSpline != NULL) {
    if (gameLocal.time <
      ...
      idVec3 linearVelocity =
          (splinePos - physicsObj.GetOrigin()) * com_engineHz_latched;
      ...
      angularVelocity *= idMath::ACos16(dir * splineDir / splineDir.Length()) *
...
================
*/
void idMoveable::ClientThink(const int curTime, const float fraction,
                             const bool predict) {
...
================
*/
void idMoveable::Event_BecomeNonSolid() { BecomeNonSolid(); }
...
================
*/
void idMoveable::SetAttacker(idEntity* ent) { attacker = ent; }
...
================
*/
void idBarrel::ClientThink(const int curTime, const float fraction,
                           const bool predict) {
...
================
*/
void idExplodingBarrel::ClientThink(const int curTime, const float fraction,
                                    const bool predict) {
  UpdateLight();
  InterpolatePhysics(fraction);
  Present();
...
================
*/
void idExplodingBarrel::SetStability(bool stability) { isStable = stability; }
...
================
*/
bool idExplodingBarrel::IsStable() { return isStable; }
...
================
*/
void idExplodingBarrel::Killed(idEntity* inflictor, idEntity* attacker,
  ...
  // Any time a barrel explodes, attribute it towards the 'Boomtastic'
  // achievement, since there's no way a barrel can explode without player
  // interference
  idPlayer* player = gameLocal.GetLocalPlayer();
  if (player != NULL && !common->IsMultiplayer()) {
    player->GetAchievementManager().EventCompletesAchievement(
        ACHIEVEMENT_DESTROY_BARRELS);
  }
}
...
================
*/
void idExplodingBarrel::Damage(idEntity* inflictor, idEntity* attacker,
...
================
*/
void idExplodingBarrel::Event_TriggerTargets() { ActivateTargets(this); }
...
================
*/
bool idExplodingBarrel::ClientReceiveEvent(int event, int time,
                                           const idBitMsg& msg) {
  switch (event) {
    case EVENT_EXPLODE: {
