...
================
*/
idLight::idLight() : previousBaseColor(vec3_zero), nextBaseColor(vec3_zero) {
  memset(&renderLight, 0, sizeof(renderLight));
...
void idLight::Spawn() {
  ...
  previousBaseColor.Set(renderLight.shaderParms[SHADERPARM_RED],
                        renderLight.shaderParms[SHADERPARM_GREEN],
                        renderLight.shaderParms[SHADERPARM_BLUE]);
  nextBaseColor.Set(renderLight.shaderParms[SHADERPARM_RED],
                    renderLight.shaderParms[SHADERPARM_GREEN],
                    renderLight.shaderParms[SHADERPARM_BLUE]);
  ...
  if (numStyles > 0) {
    ...
    for (int i = 0; i < 12; i++) {
      idStr style =
          spawnArgs.GetString(va("light_style%d", i), predef_lightstyles[i]);
      light_styles.Append(style);
    }
  }
  // jmarshall end
...
================
*/
void idLight::SetLightParms(float parm0, float parm1, float parm2,
                            float parm3) {
...
================
*/
void idLight::FadeOut(float time) { Fade(colorBlack, time); }
...
================
*/
void idLight::Killed(idEntity* inflictor, idEntity* attacker, int damage,
...
void idLight::BecomeBroken(idEntity* activator) {
  ...
  if (brokenModel.Length()) {
    ...
    if (!spawnArgs.GetBool("nonsolid")) {
      GetPhysics()->SetClipModel(
          new (TAG_PHYSICS_CLIP_ENTITY) idClipModel(brokenModel.c_str()), 1.0f);
...
void idLight::SharedThink() {
  ...
  {
    lightStyleState.dl_oldframe =
        lightStyleState.dl_oldframe + (int)lightStyleState.dl_backlerp;
    lightStyleState.dl_frame = lightStyleState.dl_oldframe + 1;
    if (lightStyleState.dl_oldframe >= stringlength) {
      lightStyleState.dl_oldframe =
          (lightStyleState.dl_oldframe) % stringlength;
      // if (cent->dl_oldframe < 3 && cent->dl_sound) { // < 3 so if an alarm
      // comes back into the pvs it will only start a sound if it's going to be
      // closely synced with the light, otherwise wait till the next cycle
      //	engine->S_StartSound(NULL, cent->currentState.number, CHAN_AUTO,
    ...
    lightStyleState.dl_backlerp =
        lightStyleState.dl_backlerp - (int)lightStyleState.dl_backlerp;
  }

  lastch = dl_stylestring[lightStyleState.dl_oldframe] - 'a';
  nextch = dl_stylestring[lightStyleState.dl_frame] - 'a';

  lightval = (lastch * (1.0 - lightStyleState.dl_backlerp)) +
             (nextch * lightStyleState.dl_backlerp);
...
================
*/
void idLight::ClientThink(const int curTime, const float fraction,
                          const bool predict) {
...
================
*/
void idLight::Event_SetLightParms(float parm0, float parm1, float parm2,
                                  float parm3) {
...
================
*/
void idLight::Event_FadeOut(float time) { FadeOut(time); }
...
================
*/
void idLight::Event_FadeIn(float time) { FadeIn(time); }
...
================
*/
void idLight::ClientPredictionThink() { Think(); }
