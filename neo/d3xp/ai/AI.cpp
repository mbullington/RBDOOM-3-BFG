...
============
*/
idAASFindCover::~idAASFindCover() { gameLocal.pvs.FreeCurrentPVS(hidePVS); }
...
void idAI::Spawn() {
  ...
  while (kv) {
    ...
    // RB: TrenchBroom interop use look_joint.<name> instead so we can build
...
void idAI::Think() {
  ...
  /*	this still draws in retail builds.. not sure why.. don't care at this
     ...
     gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1, 1 );
          }
  */
...
=====================
*/
bool idAI::PathToGoal(aasPath_t& path, int areaNum, const idVec3& origin,
                      int goalAreaNum, const idVec3& goalOrigin) const {
  idVec3 org;
  idVec3 goal;
...
=====================
*/
bool idAI::MoveDone() const { return (move.moveCommand == MOVE_NONE); }
...
=====================
*/
bool idAI::EntityCanSeePos(idActor* actor, const idVec3& actorOrigin,
                           const idVec3& pos) {
  idVec3 eye, point;
  trace_t results;
  pvsHandle_t handle;
...
void idAI::Turn() {
  ...
  if (anim_turn_angles && animflags.anim_turn) {
    ...
    turnVel +=
        AI_TURN_SCALE * diff * MS2SEC(gameLocal.time - gameLocal.previousTime);
    if (turnVel > turnRate) {
      turnVel = turnRate;
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugLine(
        colorRed, org, org + idAngles(0, ideal_yaw, 0).ToForward() * 64, 1);
    gameRenderWorld->DebugLine(
        colorGreen, org, org + idAngles(0, current_yaw, 0).ToForward() * 48, 1);
    gameRenderWorld->DebugLine(
        colorYellow, org,
        org + idAngles(0, current_yaw + turnVel, 0).ToForward() * 32, 1);
  }
}
...
================
*/
void idAI::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
...
=====================
*/
void idAI::GetMoveDelta(const idMat3& oldaxis, const idMat3& axis,
                        idVec3& delta) {
...
void idAI::CheckObstacleAvoidance(const idVec3& goalPos, idVec3& newPos) {
  ...
  if (ai_showObstacleAvoidance.GetBool()) {
    gameRenderWorld->DebugLine(colorBlue, goalPos + idVec3(1.0f, 1.0f, 0.0f),
                               goalPos + idVec3(1.0f, 1.0f, 64.0f), 1);
    gameRenderWorld->DebugLine(foundPath ? colorYellow : colorRed, path.seekPos,
                               path.seekPos + idVec3(0.0f, 0.0f, 64.0f), 1);
  }
...
void idAI::AnimMove() {
  ...
  if (!af_push_moveables && attack.Length() && TestMelee()) {
    ...
    if (blockEnt != NULL && blockEnt->IsType(idMoveable::Type) &&
...
=====================
*/
idVec3 Seek(idVec3& vel, const idVec3& org, const idVec3& goal,
            float prediction) {
  idVec3 predictedPos;
  idVec3 goalDelta;
  idVec3 seekVel;
...
void idAI::SlideMove() {
  ...
  vel -=
      vel * AI_FLY_DAMPENING * MS2SEC(gameLocal.time - gameLocal.previousTime);
  ...
  if (origin.z > goalPos.z) {
    ...
    if (ai_debugMove.GetBool()) {
      gameRenderWorld->DebugBounds(goLower ? colorRed : colorGreen,
                                   physicsObj.GetBounds(), path.endPos, 1);
    }
  }
...
void idAI::AdjustFlySpeed(idVec3& vel) {
  ...
  speed = vel.Normalize();
  speed +=
      (move.speed - speed) * MS2SEC(gameLocal.time - gameLocal.previousTime);
  if (speed < 0.0f) {
    speed = 0.0f;
...
void idAI::FlyMove() {
  ...
  if (ai_debugMove.GetBool()) {
    ...
    gameRenderWorld->DebugLine(colorRed, org,
                               org + physicsObj.GetLinearVelocity(), 1, true);
...
=====================
*/
bool idAI::Pain(idEntity* inflictor, idEntity* attacker, int damage,
  ...
  if (idStr::Icmp(name, "guardian_spawn") == 0) {
    ...
    if (player != NULL && player->GetExpansionType() == GAME_BASE) {
      player->GetAchievementManager().EventCompletesAchievement(
          ACHIEVEMENT_DEFEAT_GUARDIAN_BOSS);
    }
  }
...
=====================
*/
idActor* idAI::GetEnemy() const { return enemy.GetEntity(); }
...
void idAI::UpdateEnemyPosition() {
  ...
  if (ai_debugMove.GetBool()) {
    gameRenderWorld->DebugBounds(colorLtGrey,
                                 ...
                                 lastReachableEnemyPos, 1);
    gameRenderWorld->DebugBounds(colorWhite,
                                 enemyEnt->GetPhysics()->GetBounds(),
                                 lastVisibleReachableEnemyPos, 1);
  }
}
...
void idAI::SetEnemy(idActor* newEnemy) {
  ...
  if (!newEnemy) {
    ...
    if (enemyEnt != NULL && enemyEnt->IsType(idPlayer::Type) &&
        newEnemy->IsType(idAI::Type) && newEnemy->team == this->team &&
        (idStr::Icmp(newEnemy->GetName(), "hazmat_dummy") != 0)) {
      idPlayer* player = gameLocal.GetLocalPlayer();
      if (player != NULL) {
        player->GetAchievementManager().EventCompletesAchievement(
            ACHIEVEMENT_TWO_DEMONS_FIGHT_EACH_OTHER);
      }
    }
...
============
*/
idVec3 idAI::FirstVisiblePointOnPath(const idVec3 origin, const idVec3& target,
...
void idAI::CreateProjectileClipModel() const {
  if (projectileClipModel == NULL) {
    ...
    projectileClipModel =
        new (TAG_MODEL) idClipModel(idTraceModel(projectileBounds));
  }
}
...
=====================
*/
bool idAI::GetAimDir(const idVec3& firePos, idEntity* aimAtEnt,
                     const idEntity* ignore, idVec3& aimDir) const {
  idVec3 targetPos1;
  idVec3 targetPos2;
  idVec3 delta;
  float max_height;
...
=====================
*/
idProjectile* idAI::LaunchProjectile(const char* jointname, idEntity* target,
...
idEntity* idAI::StartEmitter(const char* name, const char* joint,
                             const char* particle) {
...
bool idAI::UpdateAnimationControllers() {
  ...
  if (focusJoint != INVALID_JOINT) {
    ...
    if (ai_debugMove.GetBool()) {
      gameRenderWorld->DebugLine(colorRed, eyepos,
                                 eyepos + orientationJointAxis[0] * 32.0f, 1);
    }
  } else {
  ...
  // a separate head, we end up transforming the joints twice per frame.
...
=====================
*/
bool idCombatNode::IsDisabled() const { return disabled; }
...
=====================
*/
void idCombatNode::Event_Activate(idEntity* activator) { disabled = !disabled; }
