...
============
*/
unsigned short idAASLocal::AreaTravelTime(int areaNum, const idVec3& start,
...
void idAASLocal::CalculateAreaTravelTimes() {
  ...
  areaTravelTimes = (unsigned short*)Mem_Alloc(
      numAreaTravelTimes * sizeof(unsigned short), TAG_AAS);
  ...
  assert(((ptrdiff_t)bytePtr - (ptrdiff_t)areaTravelTimes) <=
...
void idAASLocal::SetupRoutingCache() {
  ...
  portalCacheIndex = (idRoutingCache**)Mem_ClearedAlloc(
      portalCacheIndexSize * sizeof(idRoutingCache*), TAG_AAS);

  areaUpdate = (idRoutingUpdate*)Mem_ClearedAlloc(
      file->GetNumAreas() * sizeof(idRoutingUpdate), TAG_AAS);
  portalUpdate = (idRoutingUpdate*)Mem_ClearedAlloc(
      (file->GetNumPortals() + 1) * sizeof(idRoutingUpdate), TAG_AAS);

  goalAreaTravelTimes = (unsigned short*)Mem_ClearedAlloc(
      file->GetNumAreas() * sizeof(unsigned short), TAG_AAS);
...
============
*/
bool idAASLocal::SetAreaState_r(int nodeNum, const idBounds& bounds,
...
============
*/
bool idAASLocal::SetAreaState(const idBounds& bounds, const int areaContents,
                              bool disabled) {
  idBounds expBounds;
...
============
*/
void idAASLocal::GetBoundsAreas_r(int nodeNum, const idBounds& bounds,
...
============
*/
idRoutingCache* idAASLocal::GetAreaRoutingCache(int clusterNum, int areaNum,
  ...
  if (!cache) {
    cache = new (TAG_AAS)
        idRoutingCache(file->GetCluster(clusterNum).numReachableAreas);
...
============
*/
idRoutingCache* idAASLocal::GetPortalRoutingCache(int clusterNum, int areaNum,
                                                  int travelFlags) const {
  idRoutingCache* cache;
...
============
*/
bool idAASLocal::RouteToGoalArea(int areaNum, const idVec3 origin,
...
============
*/
int idAASLocal::TravelTimeToGoalArea(int areaNum, const idVec3& origin,
...
============
*/
bool idAASLocal::FindNearestGoal(aasGoal_t& goal, int areaNum,
