...
enum {
  ...
  GAME_RELIABLE_MESSAGE_RESPAWN_AVAILABLE,  // Used just to show clients the
                                            // respawn text on the hud.
...
class idGameLocal : public idGame {
 public:
  ...
  idArray<lobbyUserID_t, MAX_CLIENTS>
  ...
  idEntityPtr<idPlayer>
      playerActivateFragChamber;  // The player that activated the frag chamber
  ...
  virtual void InitFromNewMap(const char* mapName, idRenderWorld* renderWorld,
                              idSoundWorld* soundWorld, int gameType,
                              int randSeed);
  virtual bool InitFromSaveGame(const char* mapName, idRenderWorld* renderWorld,
                                idSoundWorld* soundWorld, idFile* saveGameFile,
                                idFile* stringTableFile, int saveGameVersion);
  ...
  virtual void ProcessReliableMessage(int clientNum, int type,
                                      const idBitMsg& msg);
  ...
  virtual int GetMPGameModes(const char*** gameModes,
  ...
  void SetAASAreaState(const idBounds& bounds, const int areaContents,
                       bool closed);
  ...
  bool SpawnEntityDef(const idDict& args, idEntity** ent = NULL,
  ...
  void RegisterEntity(idEntity* ent, int forceSpawnId,
                      const idDict& spawnArgsToCopy);
  void UnregisterEntity(idEntity* ent);
  const idDict& GetSpawnArgs() const { return spawnArgs; }

  bool RequirementMet(idEntity* activator, const idStr& doesReq,
                      int removeItem);
  ...
  int GetTargets(const idDict& args, idList<idEntityPtr<idEntity> >& list,
  ...
  idEntity* FindTraceEntity(idVec3 start, idVec3 end, const idTypeInfo& c,
                            const idEntity* skip) const;
  idEntity* FindEntity(const char* name) const;
  idEntity* FindEntityUsingDef(idEntity* from, const char* match) const;
  int EntitiesWithinRadius(const idVec3 org, float radius,
                           idEntity** entityList, int maxCount) const;

  void KillBox(idEntity* ent, bool catch_teleport = false);
  void RadiusDamage(const idVec3& origin, idEntity* inflictor,
                    idEntity* attacker, idEntity* ignoreDamage,
                    idEntity* ignorePush, const char* damageDefName,
                    float dmgPower = 1.0f);
  void RadiusPush(const idVec3& origin, const float radius, const float push,
                  const idEntity* inflictor, const idEntity* ignore,
                  float inflictorScale, const bool quake);
  void RadiusPushClipModel(const idVec3& origin, const float push,
                           const idClipModel* clipModel);

  void ProjectDecal(const idVec3& origin, const idVec3& dir, float depth,
                    bool parallel, float size, const char* material,
                    float angle = 0);
  void BloodSplat(const idVec3& origin, const idVec3& dir, float size,
                  const char* material);
  ...
  void SaveEntityNetworkEvent(const idEntity* ent, int event,
                              const idBitMsg* msg);
  ...
  uint32 GeneratePredictionKey(idWeapon* weapon, idPlayer* playerAttacker,
                               int overrideKey);
 ...
 private:
  ...
  idArray<int, MAX_PLAYERS>
      usercmdLastClientMilliseconds;  // The latest client time the server has
                                      // run.
  ...
  void NetworkEventWarning(const entityNetEvent_t* event,
