...
================
*/
idItem::idItem() : clientPredictPickupMilliseconds(0) {
  spin = false;
  inView = false;
  inViewTime = 0;
  lastCycle = 0;
...
void idItem::Spawn() {
  ...
  if (spawnArgs.GetBool("spin") ||
      (common->IsMultiplayer() && !this->IsType(idItemTeam::Type))) {
    spin = true;
...
bool idItem::Pickup(idPlayer* player) {
  ...
  // Store the time so clients know when to stop predicting and let snapshots
  // overwrite.
...
================
*/
void idItem::ClientThink(const int curTime, const float fraction,
                         const bool predict) {
...
void idItem::ReadFromSnapshot(const idBitMsg& msg) {
  if (msg.ReadBits(1)) {
    ...
    // Fix mispredictions
    if (gameLocal.GetLastClientUsercmdMilliseconds(
            gameLocal.GetLocalClientNum()) >= clientPredictPickupMilliseconds) {
...
===============
*/
idItemTeam::~idItemTeam() { FreeLightDef(); }
/*
===============
idItemTeam::Spawn
===============
...
void idItemTeam::Think() {
  ...
  if (common->IsServer() && nuggetName && carried &&
...
void idItemTeam::Event_TakeFlag(idPlayer* player) {
  ...
  if (common->IsServer()) {
    ...
    gameLocal.mpGame.PrintMessageEvent(idMultiplayerGame::MSG_FLAGTAKEN, team,
                                       player->entityNumber);
...
void idItemTeam::Event_FlagReturn(idPlayer* player) {
  ...
  if (common->IsServer()) {
    ...
    if (gameLocal.mpGame.IsFlagMsgOn()) {
      ...
      gameLocal.mpGame.PrintMessageEvent(idMultiplayerGame::MSG_FLAGRETURN,
                                         team, entitynum);
    }
  }
...
void idItemTeam::Event_FlagCapture() {
  ...
  if (common->IsServer()) {
    ...
    if (playerIdx != -1) {
      ...
      gameLocal.mpGame.PrintMessageEvent(idMultiplayerGame::MSG_FLAGCAPTURE,
                                         team, playerIdx);
    } else {
      playerIdx = 255;
    }
...
================
*/
idObjective::idObjective() { playerPos.Zero(); }
    ...
    if (spawnArgs.GetString("inv_objective", NULL)) {
      if (player) {
        player->GiveObjective(spawnArgs.GetString("objectivetitle"),
                              spawnArgs.GetString("objectivetext"), screenshot);
...
bool idVideoCDItem::GiveToPlayer(idPlayer* player, unsigned int giveFlags) {
  ...
  const idDeclVideo* video = static_cast<const idDeclVideo*>(
      declManager->FindType(DECL_VIDEO, spawnArgs.GetString("video"), false));
  if (video == NULL) {
    return false;
  }
...
bool idPDAItem::GiveToPlayer(idPlayer* player, unsigned int giveFlags) {
  ...
  if (pdaName != NULL && pdaName[0] != 0) {
    ...
    pda = static_cast<const idDeclPDA*>(
        declManager->FindType(DECL_PDA, pdaName, false));
    if (pda == NULL) {
      idLib::Warning("PDA Item '%s' references unknown PDA %s", GetName(),
                     pdaName);
      return false;
    }
  }
...
void idMoveableItem::Spawn() {
  ...
  trigger = new (TAG_PHYSICS_CLIP_ENTITY)
      idClipModel(idTraceModel(idBounds(vec3_origin).Expand(tsize)));
  trigger->Link(gameLocal.clip, this, 0, GetPhysics()->GetOrigin(),
                GetPhysics()->GetAxis());
...
================
*/
void idMoveableItem::ClientThink(const int curTime, const float fraction,
                                 const bool predict) {
...
================
*/
idEntity* idMoveableItem::DropItem(const char* classname, const idVec3& origin,
                                   const idMat3& axis, const idVec3& velocity,
                                   int activateDelay, int removeDelay) {
  idDict args;
  idEntity* item;
...
================
*/
void idMoveableItem::DropItems(idAnimatedEntity* ent, const char* type,
...
void idMoveableItem::ReadFromSnapshot(const idBitMsg& msg) {
  ...
  if (snapshotHidden) {
    ...
    if (gameLocal.GetLastClientUsercmdMilliseconds(
            gameLocal.GetLocalClientNum()) >= GetPredictPickupMilliseconds()) {
...
================
*/
idObjectiveComplete::idObjectiveComplete() { playerPos.Zero(); }
