...
struct gameReturn_t {
  gameReturn_t()
      : sessionCommand(
            ""),  // SRS - Explicitly init sessionCommand otherwise can be
                  // optimized out and skipped with gcc or Apple clang
...
class idGame {
 public:
  ...
  virtual bool InitFromSaveGame(const char* mapName, idRenderWorld* renderWorld,
                                ...
                                idFile* stringTableFile,
                                int saveGameVersion) = 0;

  // Saves the current game state, common may have written some data to the file
  // already.
  ...
  virtual void Shell_UpdateLeaderboard(
...
class idGameEdit {
 public:
  ...
  virtual void ParseSpawnArgsToRenderEnvprobe(
      const idDict* args, renderEnvironmentProbe_t* renderEnvprobe);  // RB
  ...
  virtual idRenderModel* AF_CreateMesh(const idDict& args, idVec3& meshOrigin,
  ...
  virtual void MapSetEntityKeyVal(const char* name, const char* key,
                                  const char* val) const;
  virtual void MapCopyDictToEntity(const char* name, const idDict* dict) const;
  virtual void MapCopyDictToEntityAtOrigin(const idVec3& org,
                                           const idDict* dict) const;
  virtual int MapGetUniqueMatchingKeyVals(const char* key, const char* list[],
                                          const int max) const;
