...
idPredictedValue<type_>::idPredictedValue()
    : value(), clientPredictedMilliseconds(0) {}
...
idPredictedValue<type_>::idPredictedValue(const type_& value_)
    : value(value_), clientPredictedMilliseconds(0) {}
...
void idPredictedValue<type_>::UpdatePredictionTime() {
  if (gameLocal.GetLocalPlayer() != NULL) {
    clientPredictedMilliseconds =
        gameLocal.GetLocalPlayer()->usercmd.clientGameMilliseconds;
  }
}
...
*/
template <class type_>
idPredictedValue<type_>& idPredictedValue<type_>::operator=(
    const type_& newValue) {
  Set(newValue);
  return *this;
}
...
Only updates the actual value if the snapshot usercmd frame is newer than the
one in which the client predicted this value.

Returns true if the value was set, false if not.
===============
*/
template <class type_>
bool idPredictedValue<type_>::UpdateFromSnapshot(const type_& valueFromSnapshot,
                                                 int clientNumber) {
  ...
  if (gameLocal.GetLastClientUsercmdMilliseconds(clientNumber) >=
      clientPredictedMilliseconds) {
    value = valueFromSnapshot;
    return true;
  }
...
bool operator==(const idPredictedValue<firstType_>& lhs,
                const idPredictedValue<secondType_>& rhs) {
...
*/
template <class firstType_, class secondType_>
bool operator==(const firstType_ lhs,
                const idPredictedValue<secondType_>& rhs) {
  return lhs == rhs.Get();
}
