...
class idAF {
 public:
  ...
  void GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                     impactInfo_t* info);
  void ApplyImpulse(idEntity* ent, int id, const idVec3& point,
                    const idVec3& impulse);
  void AddForce(idEntity* ent, int id, const idVec3& point,
                const idVec3& force);
 ...
 protected:
  ...
  idList<jointConversion_t, TAG_AF>
      jointMods;  // list with transforms from skeletal model joints to
                  // articulated figure bodies
  idList<int, TAG_AF>
      jointBody;  // table to find the nearest articulated figure body for a
                  // joint of the skeletal model
 ...
 protected:
  ...
  void AddBody(idAFBody* body, const idJointMat* joints, const char* jointName,
