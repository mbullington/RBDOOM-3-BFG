...
// EVENT( EV_Envprobe_SetRadiusXYZ,
// EnvironmentProbe::Event_SetRadiusXYZ ) EVENT( EV_Envprobe_SetRadius,
// EnvironmentProbe::Event_SetRadius )
...
================
*/
void idGameEdit::ParseSpawnArgsToRenderEnvprobe(
    const idDict* args, renderEnvironmentProbe_t* renderEnvprobe) {
  idVec3 color;
  ...
  args->GetFloat("shaderParm3", "1",
                 renderEnvprobe->shaderParms[SHADERPARM_TIMESCALE]);
  if (!args->GetFloat("shaderParm4", "0",
                      renderEnvprobe->shaderParms[SHADERPARM_TIMEOFFSET])) {
  ...
  args->GetFloat("shaderParm7", "0",
                 renderEnvprobe->shaderParms[SHADERPARM_MODE]);
}
...
void EnvironmentProbe::UpdateChangeableSpawnArgs(const idDict* source) {
  ...
  gameEdit->ParseSpawnArgsToRenderEnvprobe(source ? source : &spawnArgs,
                                           &renderEnvprobe);
...
*/
EnvironmentProbe::EnvironmentProbe()
    : previousBaseColor(vec3_zero), nextBaseColor(vec3_zero) {
  memset(&renderEnvprobe, 0, sizeof(renderEnvprobe));
...
void EnvironmentProbe::Spawn() {
  ...
  localEnvprobeOrigin = (renderEnvprobe.origin - GetPhysics()->GetOrigin()) *
                        GetPhysics()->GetAxis().Transpose();
  localEnvprobeAxis =
      /*renderEnvprobe.axis * */ GetPhysics()->GetAxis().Transpose();

  // set the base color from the shader parms
  baseColor.Set(renderEnvprobe.shaderParms[SHADERPARM_RED],
                renderEnvprobe.shaderParms[SHADERPARM_GREEN],
                renderEnvprobe.shaderParms[SHADERPARM_BLUE]);
  previousBaseColor.Set(renderEnvprobe.shaderParms[SHADERPARM_RED],
                        renderEnvprobe.shaderParms[SHADERPARM_GREEN],
                        renderEnvprobe.shaderParms[SHADERPARM_BLUE]);
  nextBaseColor.Set(renderEnvprobe.shaderParms[SHADERPARM_RED],
                    renderEnvprobe.shaderParms[SHADERPARM_GREEN],
                    renderEnvprobe.shaderParms[SHADERPARM_BLUE]);
...
================
*/
void EnvironmentProbe::SetEnvprobeParms(float parm0, float parm1, float parm2,
                                        float parm3) {
...
================
*/
void EnvironmentProbe::FadeOut(float time) { Fade(colorBlack, time); }
...
void EnvironmentProbe::Present() {
  ...
  renderEnvprobe.origin =
      GetPhysics()->GetOrigin() + GetPhysics()->GetAxis() * localEnvprobeOrigin;
...
================
*/
void EnvironmentProbe::ClientThink(const int curTime, const float fraction,
                                   const bool predict) {
...
================
*/
bool EnvironmentProbe::GetPhysicsToSoundTransform(idVec3& origin,
                                                  idMat3& axis) {
...
================
*/
void EnvironmentProbe::Event_SetEnvprobeParms(float parm0, float parm1,
...
================
*/
void EnvironmentProbe::Event_On() { On(); }
...
================
*/
void EnvironmentProbe::Event_Off() { Off(); }
...
void EnvironmentProbe::Event_SetSoundHandles()
{
        ...
        for( i = 0; i < targets.Num(); i++ )
        {
                ...
                if( targetEnt != NULL && targetEnt->IsType(
EnvironmentProbe::Type ) )
                {
                        ...
                        light->renderEntity.referenceSound =
...
================
*/
void EnvironmentProbe::Event_FadeOut(float time) { FadeOut(time); }
...
================
*/
void EnvironmentProbe::Event_FadeIn(float time) { FadeIn(time); }
...
================
*/
void EnvironmentProbe::ClientPredictionThink() { Think(); }
...
void EnvironmentProbe::WriteToSnapshot(idBitMsg& msg) const {
  ...
  */
  ...
  msg.WriteLong(
      PackColor(idVec4(renderEnvprobe.shaderParms[SHADERPARM_RED],
                       renderEnvprobe.shaderParms[SHADERPARM_GREEN],
                       renderEnvprobe.shaderParms[SHADERPARM_BLUE],
...
*/
/*
bool EnvironmentProbe::ClientReceiveEvent( int event, int time, const idBitMsg&
msg )
{
