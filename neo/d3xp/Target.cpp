...
void idTarget_EndLevel::Event_Activate(idEntity* activator) {
  ...
  if (!isTutorialMap && player != NULL) {
    if (!player->GetAchievementManager().GetPlayerTookDamage()) {
      player->GetAchievementManager().EventCompletesAchievement(
          ACHIEVEMENT_COMPLETE_LEVEL_WITHOUT_TAKING_DMG);
    }
  ...
  if (!isTutorialMap && spawnArgs.GetBool("endOfGame")) {
    if (player != NULL) {
      ...
      switch (expansion) {
        case GAME_D3XP:
          // The fall-through is done here on purpose so compleating the game on
          // one difficulty will unlock all the easier difficulties
...
void idTarget_WaitForButton::Think() {
  ...
  if (thinkFlags & TH_THINK) {
    ...
    if (player != NULL && (!(player->oldButtons & BUTTON_ATTACK)) &&
...
void idTarget_GiveEmail::Event_Activate(idEntity* activator) {
  ...
  if (pda) {
    player->GiveEmail(static_cast<const idDeclEmail*>(declManager->FindType(
        DECL_EMAIL, spawnArgs.GetString("email"), false)));
  } else {
...
================
*/
idTarget_Tip::idTarget_Tip() { playerPos.Zero(); }
...
void idTarget_RemoveWeapons::Event_Activate(idEntity* activator) {
  for (int i = 0; i < gameLocal.numClients; i++) {
    if (gameLocal.entities[i]) {
      ...
      // Everywhere that we use target_removeweapons the intent is to remove ALL
      // of the weapons that hte player has (save a few: flashlights, fists,
      // soul cube).
...
idCVar g_checkpoints("g_checkpoints", "1", CVAR_BOOL | CVAR_ARCHIVE,
...
void idTarget_RumbleJoystick::Event_Activate(idEntity* activator) {
  ...
  if (player != NULL) {
    ...
    player->SetControllerShake(highMagnitude, highDuration, lowMagnitude,
                               lowDuration);
  }
}
