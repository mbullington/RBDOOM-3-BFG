...
=====================
*/
idAnimState::~idAnimState() { delete thread; }
...
=====================
*/
void idAnimState::BecomeIdle() { idleAnim = true; }
...
=====================
*/
bool idAnimState::Disabled() const { return disabled; }
...
=====================
*/
bool idAnimState::IsIdle() const { return disabled || idleAnim; }
...
void idActor::Spawn() {
  ...
  if (headEnt) {
    ...
    for (kv = spawnArgs.MatchPrefix("copy_joint", NULL); kv != NULL;
      ...
      // RB: TrenchBroom interop use copy_joint_world.<name> instead so we can
      // build this up using the FGD files
      if (jointName.StripLeadingOnce("copy_joint_world ") ||
          jointName.StripLeadingOnce("copy_joint_world.")) {
...
==============
*/
int idActor::GetDefaultSurfaceType() const { return SURFTYPE_FLESH; }
...
================
*/
void idActor::ProjectOverlay(const idVec3& origin, const idVec3& dir,
...
================
*/
bool idActor::ShouldConstructScriptObjectAtSpawn() const { return false; }
...
=====================
*/
void idActor::SetFOV(float fov) { fovDot = (float)cos(DEG2RAD(fov * 0.5f)); }
...
=====================
*/
void idActor::SetEyeHeight(float height) { eyeOffset.z = height; }
...
=====================
*/
float idActor::EyeHeight() const { return eyeOffset.z; }
...
=====================
*/
void idActor::GetAIAimTargets(const idVec3& lastSightPos, idVec3& headPos,
                              idVec3& chestPos) {
...
================
*/
idClipModel* idActor::GetCombatModel() const { return combatModel; }
...
================
*/
void idActor::Teleport(const idVec3& origin, const idAngles& angles,
...
================
*/
const idAngles& idActor::GetDeltaViewAngles() const { return deltaViewAngles; }
...
================
*/
bool idActor::OnLadder() const { return false; }
...
=====================
*/
void idActor::SetAnimState(int channel, const char* statename,
                           int blendFrames) {
  const function_t* func;
...
=====================
*/
void idActor::SetWaitState(const char* _waitstate) { waitState = _waitstate; }
...
===============
*/
void idActor::SyncAnimChannels(int channel, int syncToChannel,
                               int blendFrames) {
  idAnimator* headAnimator;
  idAFAttachment* headEnt;
  int anim;
...
============
*/
idCVar actor_noDamage("actor_noDamage", "0", CVAR_BOOL,
                      "actors don't take damage -- for testing");
void idActor::Damage(idEntity* inflictor, idEntity* attacker, const idVec3& dir,
  ...
  if (finalBoss &&
      idStr::FindText(GetEntityDefName(), "monster_boss_cyberdemon") == 0 &&
      !inflictor->IsType(idSoulCubeMissile::Type)) {
    return;
  }

  // for killed by fists achievement
  if (attacker->IsType(idPlayer::Type) &&
      idStr::Cmp("damage_fists", damageDefName)) {
  ...
  if (finalBoss &&
      idStr::Icmp(GetEntityDefName(), "monster_hunter_helltime") == 0 &&
      idStr::Icmp(inflictor->GetEntityDefName(),
        ...
        if (player != NULL) {
          if (!damageNotByFists && player->GetExpansionType() == GAME_BASE) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_KILL_20_ENEMY_FISTS_HANDS);
          }
          if (player->PowerUpActive(HELLTIME)) {
            player->GetAchievementManager().IncrementHellTimeKills();
          }
          if (player->PowerUpActive(BERSERK) &&
              player->GetExpansionType() == GAME_D3XP && !damageNotByFists) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_ARTIFACT_WITH_BERSERK_PUNCH_20);
          }
          if (player->GetCurrentWeaponSlot() == player->weapon_chainsaw) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_KILL_20_ENEMY_WITH_CHAINSAW);
          }
          if ((!name.Find("monster_boss_vagary") ||
               !name.Find("vagaryaftercin")) &&
              player->GetExpansionType() == GAME_BASE) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_VAGARY_BOSS);
          }
          if (!name.Find("monster_boss_sabaoth")) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_SABAOTH_BOSS);
          }
          if (!name.Find("monster_boss_cyberdemon")) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_CYBERDEMON_BOSS);
          }
          if (name.Icmp("hunter_berzerk") == 0) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_BERSERK_HUNTER);
          }
          if (!name.Find("monster_hunter_helltime")) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_HELLTIME_HUNTER);
          }
          if (name.Icmp("hunter") == 0) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_DEFEAT_INVULNERABILITY_HUNTER);
          }
          if (inflictor && inflictor->IsType(idSoulCubeMissile::Type)) {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_USE_SOUL_CUBE_TO_DEFEAT_20_ENEMY);
          }
          if (inflictor && inflictor->IsType(idMoveable::Type)) {
            ...
            // then the player has killed an enemy with a launched moveable from
            // the Grabber
            if (moveable != NULL && moveable->GetAttacker() != NULL &&
                moveable->GetAttacker()->IsType(idPlayer::Type) &&
                moveable->GetAttacker() == attacker &&
                player->GetExpansionType() == GAME_D3XP &&
                team != player->team) {
              player->GetAchievementManager().EventCompletesAchievement(
                  ACHIEVEMENT_GRABBER_KILL_20_ENEMY);
            }
          }
          ...
          if (inflictor != NULL && inflictor->IsType(idProjectile::Type)) {
            ...
            if (projectile != NULL) {
              if (projectile->GetLaunchedFromGrabber() &&
                  player->GetExpansionType() == GAME_D3XP &&
                  team != player->team) {
                player->GetAchievementManager().EventCompletesAchievement(
                    ACHIEVEMENT_GRABBER_KILL_20_ENEMY);
              ...
              if (renderEntity.hModel &&
                  idStr::Icmp(renderEntity.hModel->Name(),
                              "models/md5/monsters/imp/imp.md5mesh") == 0) {
                if (idStr::FindText(inflictor->GetName(), "shotgun") > -1) {
                  idStr impName;
                  int lastKilledImpTime =
                      player->GetAchievementManager().GetLastImpKilledTime();
                  if ((gameLocal.GetTime() - lastKilledImpTime) <= 100 &&
                      (impName.Icmp(name) != 0)) {
                    player->GetAchievementManager().EventCompletesAchievement(
                        ACHIEVEMENT_KILL_TWO_IMPS_ONE_SHOTGUN);
                  } else {
                    player->GetAchievementManager().SetLastImpKilledTime(
                        gameLocal.GetTime());
                  }
                }
              }
          ...
          if (player->health == 1 &&
              player->team !=
                  this->team)  // make sure it doesn't unlock if you kill a
                               // friendly dude when you have 1 heath....
          {
            player->GetAchievementManager().EventCompletesAchievement(
                ACHIEVEMENT_KILL_MONSTER_WITH_1_HEALTH_LEFT);
          }
        }
      }
...
=====================
*/
void idActor::ClearPain() { pain_debounce_time = 0; }
...
=====================
*/
bool idActor::Pain(idEntity* inflictor, idEntity* attacker, int damage,
...
void idActor::SetupDamageGroups() {
  ...
  while (arg) {
    ...
    // RB: TrenchBroom interop use damage_zone.<name> instead so we can build
    // this up using the FGD files
  ...
  while (arg) {
    ...
    // RB: TrenchBroom interop use damage_scale.<name> instead so we can build
    // this up using the FGD files
...
===============
*/
void idActor::Event_Footstep() { PlayFootStepSound(); }
...
=====================
*/
void idActor::Event_EnableWalkIK() { walkIK.EnableAll(); }
...
=====================
*/
void idActor::Event_DisableWalkIK() { walkIK.DisableAll(); }
...
=====================
*/
void idActor::Event_EnableLegIK(int num) { walkIK.EnableLeg(num); }
...
=====================
*/
void idActor::Event_DisableLegIK(int num) { walkIK.DisableLeg(num); }
...
===============
*/
void idActor::Event_DisablePain() { allowPain = false; }
...
===============
*/
void idActor::Event_EnablePain() { allowPain = true; }
...
=====================
*/
void idActor::Event_SetAnimPrefix(const char* prefix) { animPrefix = prefix; }
...
===============
*/
void idActor::Event_AnimState(int channel, const char* statename,
                              int blendFrames) {
...
=====================
*/
void idActor::Event_GetHead() { idThread::ReturnEntity(head.GetEntity()); }
...
void idActor::Event_SetDamageCap(float _damageCap) { damageCap = _damageCap; }
