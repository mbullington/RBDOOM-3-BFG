...
================
*/
void idMultiModelAF::Spawn() { physicsObj.SetSelf(this); }
...
================
*/
void idChain::BuildChain(const idStr& name, const idVec3& origin,
                         float linkLength, float linkWidth, float density,
                         int numLinks, bool bindToWorld) {
  int i;
  ...
  for (i = 0; i < numLinks; i++) {
    ...
    if (bindToWorld) {
      if (!lastBody) {
        uj = new (TAG_PHYSICS_AF)
            idAFConstraint_UniversalJoint(name + idStr(i), body, lastBody);
        ...
        uj = new (TAG_PHYSICS_AF)
            idAFConstraint_UniversalJoint(name + idStr(i), lastBody, body);
        uj->SetShafts(idVec3(0, 0, 1), idVec3(0, 0, -1));
        // uj->SetConeLimit( idVec3( 0, 0, 1 ), 30.0f );
      }
      ...
      if (lastBody) {
        bsj = new (TAG_PHYSICS_AF) idAFConstraint_BallAndSocketJoint(
            "joint" + idStr(i), lastBody, body);
...
=====================
*/
void idAFAttachment::Spawn() { idleAnim = animator.GetAnim("idle"); }
...
=====================
*/
void idAFAttachment::SetBody(idEntity* bodyEnt, const char* model,
...
=====================
*/
idEntity* idAFAttachment::GetBody() const { return body; }
...
============
*/
void idAFAttachment::Damage(idEntity* inflictor, idEntity* attacker,
...
================
*/
void idAFAttachment::GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                                   impactInfo_t* info) {
  if (body) {
...
================
*/
void idAFAttachment::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
...
================
*/
void idAFAttachment::AddForce(idEntity* ent, int id, const idVec3& point,
                              const idVec3& force) {
  if (body) {
...
================
*/
idClipModel* idAFAttachment::GetCombatModel() const { return combatModel; }
...
================
*/
void idAFEntity_Base::LoadState(const idDict& args) { af.LoadState(args); }
...
================
*/
void idAFEntity_Base::AddBindConstraints() { af.AddBindConstraints(); }
...
================
*/
void idAFEntity_Base::RemoveBindConstraints() { af.RemoveBindConstraints(); }
...
================
*/
void idAFEntity_Base::AddDamageEffect(const trace_t& collision,
                                      const idVec3& velocity,
                                      const char* damageDefName) {
...
================
*/
void idAFEntity_Base::GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                                    impactInfo_t* info) {
  if (af.IsActive()) {
...
================
*/
void idAFEntity_Base::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
                                   const idVec3& impulse) {
  if (af.IsLoaded()) {
...
================
*/
void idAFEntity_Base::AddForce(idEntity* ent, int id, const idVec3& point,
                               const idVec3& force) {
  if (af.IsLoaded()) {
...
================
*/
idClipModel* idAFEntity_Base::GetCombatModel() const { return combatModel; }
...
================
*/
void idAFEntity_Base::DropAFs(idEntity* ent, const char* type,
...
================
*/
void idAFEntity_Gibbable::Damage(idEntity* inflictor, idEntity* attacker,
                                 const idVec3& dir, const char* damageDefName,
                                 const float damageScale, const int location) {
  if (!fl.takedamage) {
    return;
...
================
*/
idAFEntity_Generic::idAFEntity_Generic() { keepRunningPhysics = false; }
...
================
*/
idAFEntity_WithAttachedHead::idAFEntity_WithAttachedHead() { head = NULL; }
...
================
*/
void idAFEntity_WithAttachedHead::Think() { idAFEntity_Base::Think(); }
...
void idAFEntity_VehicleSimple::Think() {
  ...
  if (thinkFlags & TH_THINK) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      wheelAngles[i] += velocity *
                        MS2SEC(gameLocal.time - gameLocal.previousTime) /
                        wheelRadius;
...
================
*/
void idAFEntity_SteamPipe::Restore(idRestoreGame* savefile) { Spawn(); }
...
static bool GetJointTransform(void* model, const idJointMat* frame,
                              const char* jointName, idVec3& origin,
                              idMat3& axis) {
  int i;
...
================
*/
static const char* GetArgString(const idDict& args, const idDict* defArgs,
                                const char* key) {
  const char* s;
...
================
*/
idRenderModel* idGameEdit::AF_CreateMesh(const idDict& args, idVec3& meshOrigin,
                                         idMat3& meshAxis, bool& poseIsSet) {
  int i, jointNum;
  const idDeclAF* af = NULL;
  const idDeclAF_Body* fb = NULL;
  renderEntity_t ent;
  idVec3 origin, *bodyOrigin = NULL, *newBodyOrigin = NULL,
                 *modifiedOrigin = NULL;
...
void idHarvestable::SetParent(idEntity* parent) { parentEnt = parent; }
...
================
*/
idAFEntity_Harvest::idAFEntity_Harvest() { harvestEnt = NULL; }
...
================
*/
void idAFEntity_Harvest::Think() { idAFEntity_WithAttachedHead::Think(); }
