...
================
*/
void idAF::GetImpactInfo(idEntity* ent, int id, const idVec3& point,
                         impactInfo_t* info) {
...
================
*/
void idAF::ApplyImpulse(idEntity* ent, int id, const idVec3& point,
...
================
*/
void idAF::AddForce(idEntity* ent, int id, const idVec3& point,
                    const idVec3& force) {
  SetupPose(self, gameLocal.time);
...
================
*/
void idAF::AddBody(idAFBody* body, const idJointMat* joints,
                   const char* jointName, const AFJointModType_t mod) {
  int index;
  ...
  switch (fc->type) {
    ...
    case DECLAF_CONSTRAINT_BALLANDSOCKETJOINT: {
      ...
      if (c) {
        ...
        c = new (TAG_PHYSICS_AF)
            idAFConstraint_BallAndSocketJoint(fc->name, body1, body2);
        physicsObj.AddConstraint(c);
      }
    ...
    case DECLAF_CONSTRAINT_UNIVERSALJOINT: {
      ...
      if (c) {
        ...
        c = new (TAG_PHYSICS_AF)
            idAFConstraint_UniversalJoint(fc->name, body1, body2);
        physicsObj.AddConstraint(c);
      }
      c->SetAnchor(fc->anchor.ToVec3());
...
bool idAF::Load(idEntity* ent, const char* fileName) {
  ...
  for (i = 0; i < physicsObj.GetNumConstraints(); i++) {
    ...
    for (j = 0; j < file->constraints.Num(); j++) {
      // idADConstraint enum is a superset of declADConstraint, so the cast is
      // valid
      if (file->constraints[j]->name.Icmp(constraint->GetName()) == 0 &&
          (constraintType_t)(file->constraints[j]->type) ==
              constraint->GetType()) {
        break;
      }
    }
...
================
*/
void idAF::Rest() { physicsObj.PutToRest(); }
...
void idAF::AddBindConstraints() {
  ...
  for (kv = args.MatchPrefix("bindConstraint ", NULL); kv;
    ...
    if (type.Icmp("fixed") == 0) {
      ...
      c = new (TAG_PHYSICS_AF)
          idAFConstraint_BallAndSocketJoint(name, body, NULL);
      physicsObj.AddConstraint(c);
