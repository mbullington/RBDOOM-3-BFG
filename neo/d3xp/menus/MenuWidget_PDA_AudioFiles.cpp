...
void idMenuWidget_PDA_AudioFiles::Initialize(idMenuHandler* data) {
  idMenuWidget_DynamicList* pdaAudioList =
      new (TAG_SWF) idMenuWidget_DynamicList();
  ...
  while (pdaAudioList->GetChildren().Num() < MAX_AUDIO_ITEMS) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_SELECT_PDA_AUDIO, pdaAudioList->GetChildren().Num());
...
void idMenuWidget_PDA_AudioFiles::Update() {
  ...
  if (dataObj != NULL && pda != NULL) {
    ...
    idMenuWidget_DynamicList* const audioList =
        dynamic_cast<idMenuWidget_DynamicList*>(&GetChildByIndex(0));
    if (audioList != NULL) {
...
========================
*/
void idMenuWidget_PDA_AudioFiles::ObserveEvent(const idMenuWidget& widget,
  ...
  switch (event.type) {
    case WIDGET_EVENT_FOCUS_ON: {
      ...
      if (GetSprite() != NULL) {
        ...
        } else if ((pdaIndex == 0 || GetSprite()->GetCurrentFrame() <= 2 ||
                    GetSprite()->GetCurrentFrame() >
                        GetSprite()->FindFrame("idle")) &&
                   list->GetViewIndex() != 0) {
          GetSprite()->PlayFrame("rollOn");
        }
      }
      pdaIndex = list->GetViewIndex();
      if (GetParent() != NULL && menuData != NULL &&
