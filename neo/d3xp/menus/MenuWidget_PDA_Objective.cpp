...
void idMenuWidget_PDA_Objective::Update() {
  ...
  idSWFScriptObject* dataObj =
      GetSprite()->GetScriptObject()->GetNestedObj("info");
  ...
  if (dataObj != NULL && dataSprite != NULL) {
    ...
    if (player->GetInventory().objectiveNames.Num() == 0) {
      ...
      int displayCount = 0;
      for (int index = numObjectives - 1; displayCount < 2 && index >= 0;
           --index) {
        if (img != NULL) {
          if (player->GetInventory().objectiveNames[index].screenshot == NULL) {
            ...
            img->SetVisible(true);
            img->SetMaterial(
                player->GetInventory().objectiveNames[index].screenshot);
          }
        }

        idSWFSpriteInstance* objSel = dataObj->GetNestedSprite(
            va("obj%d", objStartIndex - displayCount), "sel");
        idSWFTextInstance* txtNote = dataObj->GetNestedText(
            va("obj%d", objStartIndex - displayCount), "txtVal");
        ...
        if (txtNote != NULL) {
          txtNote->SetText(
              player->GetInventory().objectiveNames[index].title.c_str());
        }

        if (displayCount == 0) {
          txtDesc->SetText(
              player->GetInventory().objectiveNames[index].text.c_str());
        }
    ...
    if (txtMainObj != NULL) {
      if (mainObj != NULL) {
        txtMainObj->SetText(mainObj->spawnArgs.GetString(
            "text", idLocalization::GetString("#str_04253")));
      } else {
...
========================
*/
void idMenuWidget_PDA_Objective::ObserveEvent(const idMenuWidget& widget,
