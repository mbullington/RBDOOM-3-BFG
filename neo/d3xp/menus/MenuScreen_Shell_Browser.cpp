...
any type, and provides accessors to these objects as well as Pair equality
operators. The main uses of Pairs in the engine are for the Tools and for
callbacks.
================================================
*/
...
class idSort_PlayerGamesList
    : public idSort_Quick<idPair<serverInfo_t, int>, idSort_PlayerGamesList> {
 public:
  int Compare(const idPair<serverInfo_t, int>& a,
              const idPair<serverInfo_t, int>& b) const {
...
void idMenuScreen_Shell_GameBrowser::Initialize(idMenuHandler* data) {
  ...
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_DOWN)
      .Set(WIDGET_ACTION_START_REPEATER, WIDGET_ACTION_SCROLL_VERTICAL, 1);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_UP)
      .Set(WIDGET_ACTION_START_REPEATER, WIDGET_ACTION_SCROLL_VERTICAL, -1);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RELEASE)
      .Set(WIDGET_ACTION_STOP_REPEATER);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_UP_RELEASE)
      .Set(WIDGET_ACTION_STOP_REPEATER);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK)
      .Set(WIDGET_ACTION_START_REPEATER, WIDGET_ACTION_SCROLL_VERTICAL, 1);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK)
      .Set(WIDGET_ACTION_START_REPEATER, WIDGET_ACTION_SCROLL_VERTICAL, -1);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE)
      .Set(WIDGET_ACTION_STOP_REPEATER);
  listWidget->AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE)
      .Set(WIDGET_ACTION_STOP_REPEATER);
  AddChild(listWidget);
  ...
  while (listWidget->GetChildren().Num() < NUM_SERVER_LIST_ITEMS) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PRESS_FOCUSED, listWidget->GetChildren().Num());
...
========================
*/
void idMenuScreen_Shell_GameBrowser::ShowScreen(
  ...
  if (mgr->GetCmdBar() != NULL) {
    ...
    buttonInfo =
        mgr->GetCmdBar()->GetButton(idMenuWidget_CommandBar::BUTTON_JOY2);
    ...
    buttonInfo =
        mgr->GetCmdBar()->GetButton(idMenuWidget_CommandBar::BUTTON_JOY3);
    buttonInfo->label = "#str_02276";
    buttonInfo->action.Set(WIDGET_ACTION_COMMAND,
                           BROWSER_COMMAND_REFRESH_SERVERS);
  }
...
========================
*/
void idMenuScreen_Shell_GameBrowser::HideScreen(
    const mainMenuTransition_t transitionType) {
  idMenuHandler_Shell* const mgr = dynamic_cast<idMenuHandler_Shell*>(menuData);
  if (mgr == NULL) {
    return;
...
void idMenuScreen_Shell_GameBrowser::UpdateServerList() {
  ...
  // need to show the pacifier before actually making the ListServers call,
  // because it can fail immediately and send back an error result to SWF.
  // Things get confused if the showLoadingPacifier then gets called after that.
  mgr->ShowPacifier("#str_online_mpstatus_searching");

  session->ListServers(
      MakeCallback(this, &idMenuScreen_Shell_GameBrowser::OnServerListReady));
}
...
void idMenuScreen_Shell_GameBrowser::OnServerListReady() {
  ...
  if (servers.Num() > 0) {
    ...
    listWidget->AddGame("#str_swf_no_servers_found", idStrId(), idStr(), -1, 0,
                        0, false, false);
    listWidget->Update();
    listWidget->SetViewOffset(0);
    listWidget->SetViewIndex(0);
  ...
  if (mgr->GetCmdBar() != NULL) {
    ...
    if (servers.Num() > 0) {
      buttonInfo =
          mgr->GetCmdBar()->GetButton(idMenuWidget_CommandBar::BUTTON_JOY1);
      if (menuData->GetPlatform() != 2) {
    ...
    if (servers.Num() > 0) {
      buttonInfo =
          mgr->GetCmdBar()->GetButton(idMenuWidget_CommandBar::BUTTON_JOY4);
...
========================
*/
void idMenuScreen_Shell_GameBrowser::DescribeServer(const serverInfo_t& server,
                                                    const int index) {
  idStr serverName;
  ...
  const bool isMapValid =
      (server.gameMap >= 0) && (server.gameMap < maps.Num());
  if (!isMapValid) {
    validMap = false;
    ...
    idStr mode = idLocalization::GetString(
        modes[idMath::ClampInt(0, modes.Num() - 1, server.gameMode)]);
    validMap = true;
    serverName = server.serverName;
    modeName = mode;
    players = server.numPlayers;
  ...
  listWidget->AddGame(serverName, mapName, modeName, serverIndex, players,
                      maxPlayers, joinable, validMap);
}
...
========================
*/
bool idMenuScreen_Shell_GameBrowser::HandleAction(idWidgetAction& action,
