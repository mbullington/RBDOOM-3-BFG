...
Provides a paged view of this widgets children.  Each child is expected to take
on the following naming scheme.  Children outside of the given window size
(NumVisibleOptions) are not rendered, and will affect which type of arrow
  ...
  for (int optionIndex = 0; optionIndex < GetNumVisibleOptions();
    ...
    if (optionIndex < GetChildren().Num()) {
      ...
      const int controlIndex =
          ...
          Min(GetNumVisibleOptions(), GetTotalNumberOfOptions()) + optionIndex;
    ...
    if (!shown) {
      // hide the item
      idSWFSpriteInstance* const sprite =
...
========================
*/
void idMenuWidget_List::ObserveEvent(const idMenuWidget& widget,
                                     const idWidgetEvent& event) {
  ExecuteEvent(event);
}
...
========================
*/
bool idMenuWidget_GameBrowserList::PrepareListElement(idMenuWidget& widget,
                                                      const int childIndex) {
  if (childIndex >= games.Num()) {
    return false;
  }

  idMenuWidget_ServerButton* const button =
      dynamic_cast<idMenuWidget_ServerButton*>(&widget);
  if (button == NULL) {
    return false;
  }
  ...
  button->SetButtonInfo(entry.serverName, entry.mapName, entry.modeName,
                        entry.index, entry.players, entry.maxPlayers,
                        entry.joinable, entry.validMap);
...
========================
*/
void idMenuWidget_GameBrowserList::ClearGames() { games.Clear(); }
...
========================
*/
void idMenuWidget_GameBrowserList::AddGame(idStr name_, idStrId mapName_,
