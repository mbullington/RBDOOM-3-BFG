...
void idMenuWidget_ScrollBar::Initialize(idMenuHandler* data) {
  ...
  AddEventAction(WIDGET_EVENT_DRAG_START)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_DRAG_START, WIDGET_EVENT_DRAG_START));
  AddEventAction(WIDGET_EVENT_DRAG_STOP)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_DRAG_STOP, WIDGET_EVENT_DRAG_STOP));
}
...
void idMenuWidget_ScrollBar::Update() {
  ...
  idSWFScriptObject* node =
      GetSprite()->GetScriptObject()->GetNestedObj("node");
  idSWFSpriteInstance* nodeSprite =
      GetSprite()->GetScriptObject()->GetNestedSprite("node");
  if (node != NULL && nodeSprite != NULL) {
    node->Set("onDrag", new (TAG_SWF) WrapWidgetSWFEvent(
                            this, WIDGET_EVENT_DRAG_START, 0));
    node->Set("onRelease", new (TAG_SWF) WrapWidgetSWFEvent(
                               this, WIDGET_EVENT_DRAG_STOP, 0));

    const idMenuWidget_DynamicList* const list =
        dynamic_cast<const idMenuWidget_DynamicList* const>(GetParent());
    if (list != NULL) {
      float percent = 0.0f;
      if ((list->GetTotalNumberOfOptions() - list->GetNumVisibleOptions()) >
          0) {
        percent = (float)list->GetViewOffset() /
                  ((float)list->GetTotalNumberOfOptions() -
                   list->GetNumVisibleOptions());
    ...
    idMenuWidget_InfoBox* const infoBox =
        dynamic_cast<idMenuWidget_InfoBox* const>(GetParent());
    if (infoBox != NULL) {
      float percent = 0.0f;
...
void idMenuWidget_ScrollBar::CalcTopAndBottom() {
  ...
  int tempPos = 0.0f;
  idSWFSpriteInstance* curMC =
      GetSprite()->GetScriptObject()->GetNestedSprite("top");
  if (curMC != NULL) {
...
void idMenuWidget_ScrollBar::CalculatePosition(float x, float y) {
  ...
  if (y >= yTop && y <= yBot) {
    ...
    idMenuWidget_DynamicList* const list =
        dynamic_cast<idMenuWidget_DynamicList* const>(GetParent());
    if (list != NULL) {
      float maxScroll =
          list->GetTotalNumberOfOptions() - list->GetNumVisibleOptions();
...
========================
*/
bool idMenuWidget_ScrollBar::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    ...
    case (widgetAction_t)
        WIDGET_ACTION_EVENT_DRAG_STOP:  // SRS - Cast actionHandler_t to
                                        // widgetAction_t
    {
      dragging = false;
      return true;
...
========================
*/
void idMenuWidget_ScrollBar::ObserveEvent(const idMenuWidget& widget,
                                          const idWidgetEvent& event) {
  switch (event.type) {
