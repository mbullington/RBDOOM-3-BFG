...
void idMenuScreen_PDA_Inventory::Initialize(idMenuHandler* data) {
  ...
  while (itemList.GetChildren().Num() < NUM_INVENTORY_ITEMS_VISIBLE) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_SELECT_PDA_ITEM, itemList.GetChildren().Num());
...
========================
*/
void idMenuScreen_PDA_Inventory::ShowScreen(
  ...
  if (player != NULL) {
    ...
    for (int j = 0; j < MAX_WEAPONS; j++) {
      ...
      if (weaponDef != NULL) {
        weaponIcons.Append(declManager->FindMaterial(
...
========================
*/
void idMenuScreen_PDA_Inventory::HideScreen(
    const mainMenuTransition_t transitionType) {
...
void idMenuScreen_PDA_Inventory::Update() {
  ...
  if (GetSprite() != NULL) {
    idSWFSpriteInstance* dpad =
        GetSprite()->GetScriptObject()->GetNestedSprite("info", "dpad");
    if (dpad != NULL) {
...
========================
*/
bool idMenuScreen_PDA_Inventory::HandleAction(idWidgetAction& action,
