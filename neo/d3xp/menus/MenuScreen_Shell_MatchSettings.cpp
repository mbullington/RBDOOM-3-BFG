...
void idMenuScreen_Shell_MatchSettings::Initialize(idMenuHandler* data) {
  ...
  control->SetDataSource(&matchData,
  ...
  control->SetDataSource(&matchData,
                         idMenuDataSource_MatchSettings::MATCH_FIELD_MAP);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_PRESS_FOCUSED, options->GetChildren().Num());
  options->AddChild(control);
  ...
  control->SetDataSource(&matchData,
                         idMenuDataSource_MatchSettings::MATCH_FIELD_TIME);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_PRESS_FOCUSED, options->GetChildren().Num());
  options->AddChild(control);
...
========================
*/
void idMenuScreen_Shell_MatchSettings::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_MatchSettings::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_MatchSettings::HandleAction(idWidgetAction& action,
    ...
    case WIDGET_ACTION_ADJUST_FIELD: {
      if (widget != NULL && widget->GetDataSource() != NULL &&
        ...
        if (matchData.MapChanged()) {
          idMenuWidget_ControlButton* button =
              dynamic_cast<idMenuWidget_ControlButton*>(
                  &options->GetChildByIndex(1));
          if (button != NULL) {
            button->Update();
          }
...
========================
*/
idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
...
========================
*/
void idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
    LoadData() {
  updateMap = false;
...
*/
void idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
    GetMapName(int index, idStr& name) {
...
*/
void idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
    GetModeName(int index, idStr& name) {
  idLobbyBase& lobby = session->GetActivePlatformLobbyBase();
  const idMatchParameters& matchParameters = lobby.GetMatchParms();
  name = "#str_swf_filter_random";
  if (matchParameters.gameMode >= 0) {
    ...
    name = idLocalization::GetString(
        modes[idMath::ClampInt(0, modes.Num() - 1, matchParameters.gameMode)]);
  }
}
...
void idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
  ...
  if (fieldIndex == MATCH_FIELD_MAP) {
    for (int i = 0; i < maps.Num(); i++) {
      ...
      if ((maps[matchParameters.gameMap].supportedModes &
           BIT(matchParameters.gameMode)) != 0) {
    ...
    matchParameters.serverInfo.SetInt("si_timeLimit",
                                      si_timeLimit.GetInteger());
    session->UpdateMatchParms(matchParameters);
    ...
    matchParameters.serverInfo.SetInt("si_fragLimit",
                                      si_fragLimit.GetInteger());
    session->UpdateMatchParms(matchParameters);
  }
...
========================
*/
bool idMenuScreen_Shell_MatchSettings::idMenuDataSource_MatchSettings::
    IsDataChanged() const {
  if (fields[MATCH_FIELD_TIME].ToString() !=
      originalFields[MATCH_FIELD_TIME].ToString()) {
    return true;
  }

  if (fields[MATCH_FIELD_MAP].ToString() !=
      originalFields[MATCH_FIELD_MAP].ToString()) {
    return true;
  }

  if (fields[MATCH_FIELD_MODE].ToString() !=
      originalFields[MATCH_FIELD_MODE].ToString()) {
    return true;
  }

  if (fields[MATCH_FIELD_SCORE].ToInteger() !=
      originalFields[MATCH_FIELD_SCORE].ToInteger()) {
    return true;
  }
