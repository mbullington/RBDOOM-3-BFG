...
void idMenuWidget_PDA_EmailInbox::Initialize(idMenuHandler* data) {
  ...
  while (emailList->GetChildren().Num() < MAX_EMAIL_ITEMS) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PDA_SELECT_EMAIL, emailList->GetChildren().Num());
    buttonWidget->AddEventAction(WIDGET_EVENT_FOCUS_ON)
        .Set(WIDGET_ACTION_REFRESH);
...
========================
*/
void idMenuWidget_PDA_EmailInbox::ObserveEvent(const idMenuWidget& widget,
