...
void idMenuHandler_Shell::Update() {
  ...
  //		// This clears the delete save dialog to catch the case of a delete
  //confirmation for an old device after we've changed the device.
  ...
  if ((IsPacifierVisible() || common->Dialog().IsDialogActive()) &&
      actionRepeater.isActive) {
  ...
  if (nextState != state) {
    ...
    if (nextState == SHELL_STATE_PRESS_START) {
      ...
      if (state != SHELL_STATE_IN_GAME) {
        ...
        ActiveScreen() == SHELL_AREA_PARTY_LOBBY ) { timeRemaining = 0;
                session->StartMatch();
    ...
    if (nextScreen == SHELL_AREA_INVALID) {
      if (activeScreen > SHELL_AREA_INVALID && activeScreen < SHELL_NUM_AREAS &&
        ...
        menuScreens[activeScreen]->HideScreen(
            static_cast<mainMenuTransition_t>(transition));
      }
      ...
      idSWFSpriteInstance* bg =
          gui->GetRootObject().GetNestedSprite("pause_bg");
      idSWFSpriteInstance* edging =
          gui->GetRootObject().GetNestedSprite("_fullscreen");
      ...
      if (nextScreen > SHELL_AREA_INVALID && nextScreen < SHELL_NUM_AREAS &&
  ...
  if (activeScreen == nextScreen && activeScreen == SHELL_AREA_LEADERBOARDS) {
    idMenuScreen_Shell_Leaderboards* screen =
        dynamic_cast<idMenuScreen_Shell_Leaderboards*>(
            menuScreens[SHELL_AREA_LEADERBOARDS]);
    if (screen != NULL) {
      screen->PumpLBCache();
    ...
    idMenuScreen_Shell_PartyLobby* screen =
        dynamic_cast<idMenuScreen_Shell_PartyLobby*>(
            menuScreens[SHELL_AREA_PARTY_LOBBY]);
    if (screen != NULL) {
      screen->UpdateLobby();
    }
  } else if (activeScreen == nextScreen &&
      ...
      if (!MatchTypeIsPrivate(matchParameters.matchFlags)) {
        if (Sys_Milliseconds() >= nextPeerUpdateMs) {
          ...
          session->GetActingGameStateLobbyBase().SendReliable(
              GAME_RELIABLE_MESSAGE_LOBBY_COUNTDOWN, msg, false);
        }
      }
    }

    idMenuScreen_Shell_GameLobby* screen =
        dynamic_cast<idMenuScreen_Shell_GameLobby*>(
            menuScreens[SHELL_AREA_GAME_LOBBY]);
    if (screen != NULL) {
      screen->UpdateLobby();
    }
  }
  ...
  if (continueWaitForEnumerate) {
    if (!session->GetSaveGameManager().IsWorking()) {
      ...
      idMenuScreen_Shell_Singleplayer* screen =
          dynamic_cast<idMenuScreen_Shell_Singleplayer*>(
              menuScreens[SHELL_AREA_CAMPAIGN]);
      if (screen != NULL) {
        screen->ContinueGame();
      }
    }
...
bool idMenuHandler_Shell::HandleGuiEvent(const sysEvent_t* sev) {
  ...
  if (waitForBinding) {
    if (sev->evType == SE_KEY && sev->evValue2 == 1) {
      if (sev->evValue >= K_JOY_STICK1_UP &&
          sev->evValue <= K_JOY_STICK2_RIGHT) {
        return true;
      }
      ...
      if (sev->evValue == K_ESCAPE) {
        ...
        idMenuScreen_Shell_Bindings* bindScreen =
            dynamic_cast<idMenuScreen_Shell_Bindings*>(
                menuScreens[SHELL_AREA_KEYBOARD]);
        if (bindScreen != NULL) {
        ...
        if (idStr::Icmp(idKeyInput::GetBinding(sev->evValue), "") ==
            0)  // no existing binding found
        {
          ...
          if (idStr::Icmp(waitBind, curBind) == 0) {
            ...
            idMenuScreen_Shell_Bindings* bindScreen =
                dynamic_cast<idMenuScreen_Shell_Bindings*>(
                    menuScreens[SHELL_AREA_KEYBOARD]);
            if (bindScreen != NULL) {
              class idSWFScriptFunction_RebindKey
                  : public idSWFScriptFunction_RefCounted {
               public:
                idSWFScriptFunction_RebindKey(
                    idMenuScreen_Shell_Bindings* _menu,
              ...
              common->Dialog().AddDialog(
                  GDM_BINDING_ALREDY_SET, DIALOG_ACCEPT_CANCEL,
                  new idSWFScriptFunction_RebindKey(
                      bindScreen, GDM_BINDING_ALREDY_SET, true, this,
                      sev->evValue, waitBind),
                  new idSWFScriptFunction_RebindKey(
                      bindScreen, GDM_BINDING_ALREDY_SET, false, this,
                      sev->evValue, waitBind),
                  false);
            }
          }
...
#define BIND_SHELL_SCREEN(screenId, className, menuHandler) \
  ...
  if (inGame) {
    ...
    BIND_SHELL_SCREEN(SHELL_AREA_SYSTEM_OPTIONS,
                      idMenuScreen_Shell_SystemOptions, this);
    BIND_SHELL_SCREEN(SHELL_AREA_GAME_OPTIONS, idMenuScreen_Shell_GameOptions,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_SAVE, idMenuScreen_Shell_Save, this);
    BIND_SHELL_SCREEN(SHELL_AREA_STEREOSCOPICS,
                      idMenuScreen_Shell_Stereoscopics, this);
    ...
    BIND_SHELL_SCREEN(SHELL_AREA_RESOLUTION, idMenuScreen_Shell_Resolution,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_CONTROLLER_LAYOUT,
                      idMenuScreen_Shell_ControllerLayout, this);
    ...
    BIND_SHELL_SCREEN(SHELL_AREA_CAMPAIGN, idMenuScreen_Shell_Singleplayer,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_SETTINGS, idMenuScreen_Shell_Settings, this);
    BIND_SHELL_SCREEN(SHELL_AREA_LOAD, idMenuScreen_Shell_Load, this);
    ...
    BIND_SHELL_SCREEN(SHELL_AREA_GAME_OPTIONS, idMenuScreen_Shell_GameOptions,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_PARTY_LOBBY, idMenuScreen_Shell_PartyLobby,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_GAME_LOBBY, idMenuScreen_Shell_GameLobby,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_STEREOSCOPICS,
                      idMenuScreen_Shell_Stereoscopics, this);
    BIND_SHELL_SCREEN(SHELL_AREA_DIFFICULTY, idMenuScreen_Shell_Difficulty,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_CONTROLS, idMenuScreen_Shell_Controls, this);
    BIND_SHELL_SCREEN(SHELL_AREA_KEYBOARD, idMenuScreen_Shell_Bindings, this);
    BIND_SHELL_SCREEN(SHELL_AREA_RESOLUTION, idMenuScreen_Shell_Resolution,
    ...
    BIND_SHELL_SCREEN(SHELL_AREA_LEADERBOARDS, idMenuScreen_Shell_Leaderboards,
                      this);
    BIND_SHELL_SCREEN(SHELL_AREA_GAMEPAD, idMenuScreen_Shell_Gamepad, this);
    BIND_SHELL_SCREEN(SHELL_AREA_MATCH_SETTINGS,
                      idMenuScreen_Shell_MatchSettings, this);
    BIND_SHELL_SCREEN(SHELL_AREA_MODE_SELECT, idMenuScreen_Shell_ModeSelect,
                      this);
  ...
  while (menuBar->GetChildren().Num() < MAX_MENU_OPTIONS) {
    idMenuWidget_MenuButton* const navButton =
        new (TAG_SWF) idMenuWidget_MenuButton();
    idMenuScreen_Shell_Root* rootScreen =
        dynamic_cast<idMenuScreen_Shell_Root*>(menuScreens[SHELL_AREA_ROOT]);
    if (rootScreen != NULL) {
  ...
  if (!inGame) {
    ...
    if ((shortMapName.Icmp("le_hell_post") == 0) ||
        (shortMapName.Icmp("hellhole") == 0) ||
...
void idMenuHandler_Shell::ActivateMenu(bool show) {
  ...
  if (show) {
    if (!inGame) {
      ...
      if (gui != NULL) {
        idSWFSpriteInstance* mars =
            gui->GetRootObject().GetNestedSprite("mars");
        if (mars) {
          mars->stereoDepth = STEREO_DEPTH_TYPE_FAR;

          idSWFSpriteInstance* planet =
              mars->GetScriptObject()->GetNestedSprite("planet");
    ...
    if (g_demoMode.GetBool()) {
      ...
      if (buttonWidget != NULL) {
        ...
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_DEMO0, index);
      ...
      if (g_demoMode.GetInteger() == 2) {
        ...
        if (buttonWidget != NULL) {
          buttonWidget->ClearEventActions();
          buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
              .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_DEMO1, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_SETTINGS, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_QUIT, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_DEV, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_CAMPAIGN, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_MULTIPLAYER, index);
      ...
      if (buttonWidget != NULL) {
        buttonWidget->ClearEventActions();
        buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, SHELL_CMD_CREDITS, index);
...
void idMenuHandler_Shell::HandleExitGameBtn() {
  ...
  callbacks.Append(new (TAG_SWF)
                       idSWFScriptFunction_QuitDialog(GDM_QUIT_GAME, 1));
  callbacks.Append(new (TAG_SWF)
                       idSWFScriptFunction_QuitDialog(GDM_QUIT_GAME, 0));
  callbacks.Append(new (TAG_SWF)
                       idSWFScriptFunction_QuitDialog(GDM_QUIT_GAME, -1));
  ...
  switch (actionType) {
    case WIDGET_ACTION_COMMAND: {
      ...
      if ((activeScreen == SHELL_AREA_GAME_LOBBY ||
           activeScreen == SHELL_AREA_MATCH_SETTINGS) &&
          cmd != SHELL_CMD_QUIT && cmd != SHELL_CMD_MULTIPLAYER) {
        session->Cancel();
        session->Cancel();
      } else if ((activeScreen == SHELL_AREA_PARTY_LOBBY ||
                  activeScreen == SHELL_AREA_LEADERBOARDS ||
                  activeScreen == SHELL_AREA_BROWSER ||
                  activeScreen == SHELL_AREA_MODE_SELECT) &&
                 cmd != SHELL_CMD_QUIT && cmd != SHELL_CMD_MULTIPLAYER) {
        session->Cancel();
      }

      if (cmd != SHELL_CMD_QUIT && (nextScreen == SHELL_AREA_STEREOSCOPICS ||
...
void idMenuHandler_Shell::ShowSmallFrame(bool show) {
  ...
  idSWFSpriteInstance* smallFrame =
      gui->GetRootObject().GetNestedSprite("smallFrame");
  if (smallFrame == NULL) {
    return;
  }
...
void idMenuHandler_Shell::ShowMPFrame(bool show) {
  ...
  idSWFSpriteInstance* smallFrame =
      gui->GetRootObject().GetNestedSprite("smallFrameMP");
  if (smallFrame == NULL) {
    return;
  }
...
void idMenuHandler_Shell::UpdateSavedGames() {
  if (activeScreen == SHELL_AREA_LOAD) {
    idMenuScreen_Shell_Load* screen =
        dynamic_cast<idMenuScreen_Shell_Load*>(menuScreens[SHELL_AREA_LOAD]);
    if (screen != NULL) {
      screen->UpdateSaveEnumerations();
    }
  } else if (activeScreen == SHELL_AREA_SAVE) {
    idMenuScreen_Shell_Save* screen =
        dynamic_cast<idMenuScreen_Shell_Save*>(menuScreens[SHELL_AREA_SAVE]);
    if (screen != NULL) {
      screen->UpdateSaveEnumerations();
    }
  }
...
void idMenuHandler_Shell::UpdateBGState() {
  if (smallFrameShowing) {
    if (nextScreen != SHELL_AREA_PLAYSTATION &&
        nextScreen != SHELL_AREA_SETTINGS &&
    ...
    if (nextScreen == SHELL_AREA_RESOLUTION ||
        nextScreen == SHELL_AREA_GAMEPAD ||
        nextScreen == SHELL_AREA_PLAYSTATION ||
        nextScreen == SHELL_AREA_SETTINGS ||
        nextScreen == SHELL_AREA_CAMPAIGN ||
        nextScreen == SHELL_AREA_CONTROLS || nextScreen == SHELL_AREA_DEV ||
        nextScreen == SHELL_AREA_DIFFICULTY) {
      ShowSmallFrame(true);
    }
  }
  ...
  if (largeFrameShowing) {
    if (nextScreen != SHELL_AREA_PARTY_LOBBY &&
        nextScreen != SHELL_AREA_GAME_LOBBY &&
        nextScreen != SHELL_AREA_CONTROLLER_LAYOUT &&
        nextScreen != SHELL_AREA_KEYBOARD &&
        nextScreen != SHELL_AREA_LEADERBOARDS &&
        nextScreen != SHELL_AREA_MATCH_SETTINGS &&
        nextScreen != SHELL_AREA_MODE_SELECT &&
        nextScreen != SHELL_AREA_BROWSER && nextScreen != SHELL_AREA_LOAD &&
...
========================
*/
void idMenuHandler_Shell::UpdateLeaderboard(
...
void idMenuHandler_Shell::UpdateLobby(idMenuWidget_LobbyList* lobbyList) {
  ...
  for (int i = 0; i < numLobbyPlayers; ++i) {
    ...
    // Voice
    lobbyPlayer->voiceState = session->GetDisplayStateFromVoiceState(
...
void idMenuHandler_Shell::ShowDoomIntro() {
  ...
  if (introGui != NULL) {
    ...
    idStr textEntries[NUM_DOOM_INTRO_LINES] = {
        va("%s %s", idLocalization::GetString("#str_04052"),
           idLocalization::GetString("#str_04053")),
        va("%s %s", idLocalization::GetString("#str_04054"),
           idLocalization::GetString("#str_04055")),
        idLocalization::GetString("#str_03012"),
        idLocalization::GetString("#str_04056"),
        idLocalization::GetString("#str_04057"),
        va("%s %s", idLocalization::GetString("#str_04058"),
           idLocalization::GetString("#str_04059")),
        va("%s %s", idLocalization::GetString("#str_04060"),
           idLocalization::GetString("#str_04061"))};

    for (int i = 0; i < numTextFields; ++i) {
      idSWFTextInstance* txtVal = introGui->GetRootObject().GetNestedText(
          va("info%d", i), "txtInfo", "txtVal");
      if (txtVal != NULL) {
      ...
      idSWFSpriteInstance* infoSprite =
          introGui->GetRootObject().GetNestedSprite(va("info%d", i));
      if (infoSprite != NULL && txtVal != NULL) {
        class idIntroTextUpdate : public idSWFScriptFunction_RefCounted {
         public:
          idIntroTextUpdate(idSWFTextInstance* _txtVal, int _numLines,
                            int _nextIndex, idMenuHandler_Shell* _shell,
                            idSWF* _gui) {
            txtVal = _txtVal;
            generating = false;
            numLines = _numLines;
            ...
            if (!generating) {
              ...
              if (!txtVal->generatingText) {
                ...
                if (newYPos <= 350.0f - (numLines * 36.0f)) {
                  ...
                  if (nextIndex < NUM_DOOM_INTRO_LINES) {
                    idSWFSpriteInstance* nextInfo =
                        gui->GetRootObject().GetNestedSprite(
                            va("info%d", nextIndex));
        ...
        infoSprite->GetScriptObject()->Set(
            "onEnterFrame",
    ...
    class idIntroVOStart : public idSWFScriptFunction_RefCounted {
     public:
      idIntroVOStart(idSWF* gui) { introGui = gui; }
      idSWFScriptVar Call(idSWFScriptObject* thisObject,
                          const idSWFParmList& parms) {
        if (introGui != NULL) {
    ...
    idSWFSpriteInstance* img =
        introGui->GetRootObject().GetNestedSprite("intro", "img");
    if (img != NULL) {
      if (mat != NULL) {
        img->SetMaterial(mat);
      }
         ...
         public:
          ...
          idSWFScriptVar Call(idSWFScriptObject* thisObject,
            ...
            if (!generating) {
              ...
              if (!txtVal->generatingText &&
                  thisObject->GetSprite()->IsVisible()) {
                if (nextIndex % 2 != 0) {
                  if (nextIndex < NUM_ROE_INTRO_LINES) {
                    ...
                    if (nextInfo != NULL && nextInfo->GetCurrentFrame() !=
                      ...
                      idSWFTextInstance* txtData =
                          nextInfo->GetScriptObject()->GetNestedText("txtInfo",
                                                                     "txtVal");
                      if (txtData != NULL && !txtData->generatingText) {
                        if (startFade == 0) {
                          ...
                          if (Sys_Milliseconds() - startFade >= 3000) {
                            ...
                            float alpha =
                                1.0f -
                                ((float)(Sys_Milliseconds() - startFade) /
                                 3000.0f);
  ...
  if (introGui != NULL) {
    ...
    idStr textEntry =
        va("%s\n%s\n%s", idLocalization::GetString("#str_00200071"),
           idLocalization::GetString("#str_00200072"),
           idLocalization::GetString("#str_00200073"));
    idSWFTextInstance* txtVal =
        introGui->GetRootObject().GetNestedText("info0", "txtInfo", "txtVal");
    if (txtVal != NULL) {
    ...
    idSWFSpriteInstance* infoSprite =
        introGui->GetRootObject().GetNestedSprite("info0");
    if (infoSprite != NULL) {
      class idIntroTextUpdate : public idSWFScriptFunction_RefCounted {
       public:
        idIntroTextUpdate(idSWFTextInstance* _txtVal,
                          idMenuHandler_Shell* _shell) {
          txtVal = _txtVal;
          generating = false;
          shell = _shell;
          startFade = 0;
          ...
          if (!generating) {
            ...
            if (!txtVal->generatingText) {
              if (startFade == 0) {
                ...
                float alpha =
                    1.0f - ((float)(Sys_Milliseconds() - startFade) / 3000.0f);
                if (alpha <= 0.0f) {
                  thisObject->GetSprite()->SetVisible(false);
                  shell->StartGame(2);
                  return idSWFScriptVar();
      ...
      infoSprite->GetScriptObject()->Set("onEnterFrame",
                                         new idIntroTextUpdate(txtVal, this));
    }
  }
}