...
This transparently supports the "UseCircleForAccept" behavior that we need for
Japanese PS3 SKU.
...
static const char* const BUTTON_NAMES[] = {"joy1", "joy2",  "joy3",
                                           "joy4", "joy10", "tab"};
compile_time_assert(sizeof(BUTTON_NAMES) / sizeof(BUTTON_NAMES[0]) ==
                    idMenuWidget_CommandBar::MAX_BUTTONS);
...
void idMenuWidget_CommandBar::Update() {
  ...
  // NOTE: Special consideration is done for JPN PS3 where the standard accept
  // button is swapped with the standard back button.  i.e. In US: X = Accept, O
  // = Back, but in JPN X = Back, O = Accept.
  ...
  for (int i = 0; i < buttonOrder.Num(); ++i) {
    ...
    idSWFSpriteInstance* const buttonSprite =
        GetSprite()->GetScriptObject()->GetSprite(buttonName);
    if (buttonSprite == NULL) {
      continue;
    }
    idSWFTextInstance* const buttonText =
        buttonSprite->GetScriptObject()->GetText("txt_info");
    if (buttonText == NULL) {
      continue;
    }
    idSWFSpriteInstance* const imageSprite =
        buttonSprite->GetScriptObject()->GetSprite("img");
    if (imageSprite == NULL) {
      continue;
    }
    ...
    if (buttons[i].action.GetType() != WIDGET_ACTION_NONE) {
      idSWFScriptObject* const shortcutKeys =
          GetSWFObject()->GetGlobal("shortcutKeys").GetObject();
      if (verify(shortcutKeys != NULL)) {
        buttonSprite->GetScriptObject()->Set(
            "onPress", new WrapWidgetSWFEvent(this, WIDGET_EVENT_COMMAND, i));
      ...
      if (buttons[i].label.IsEmpty()) {
        ...
        xPos += ALIGNMENT_SCALE *
                (buttonText->GetTextLength() + PER_BUTTON_PADDING);
      }
    } else {
      buttonSprite->SetVisible(false);
      idSWFScriptObject* const shortcutKeys =
