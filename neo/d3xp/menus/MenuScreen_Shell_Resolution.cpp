...
========================
*/
void idMenuScreen_Shell_Resolution::ShowScreen(
  ...
  for (int displayNum = 0; displayNum < displays.Num(); displayNum++) {
    ...
    for (int i = 0; i < modeList.Num(); i++) {
      ...
      idStr str;
      if (displays.Num() > 1) {
        str.Append(va("%s %i: ", idLocalization::GetString("#str_swf_monitor"),
                      displayNum + 1));
      }
...
========================
*/
bool idMenuScreen_Shell_Resolution::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    case WIDGET_ACTION_GO_BACK: {
      menuData->SetNextScreen(SHELL_AREA_SYSTEM_OPTIONS,
      ...
      if (options != NULL) {
        ...
        if (currentOption == originalOption) {
          ...
          class idSWFFuncAcceptVideoChanges
           ...
           public:
            idSWFFuncAcceptVideoChanges(idMenuHandler* _menu,
                                        gameDialogMessages_t _msg,
                                        const optionData_t& _optionData,
                                        bool _accept) {
              menuHandler = _menu;
              msg = _msg;
            ...
            idSWFScriptVar Call(idSWFScriptObject* thisObject,
              ...
              if (accept) {
                ...
                if (menuHandler != NULL) {
                  menuHandler->SetNextScreen(SHELL_AREA_SYSTEM_OPTIONS,
                                             MENU_TRANSITION_SIMPLE);
                }
              } else {
          ...
          common->Dialog().AddDialog(
              GDM_CONFIRM_VIDEO_CHANGES, DIALOG_TIMER_ACCEPT_REVERT,
              new (TAG_SWF) idSWFFuncAcceptVideoChanges(
                  menuData, GDM_CONFIRM_VIDEO_CHANGES, currentOption, true),
              new (TAG_SWF) idSWFFuncAcceptVideoChanges(
                  menuData, GDM_CONFIRM_VIDEO_CHANGES, originalOption, false),
              false);
        }
        return true;
      }
