...
========================
*/
void idMenuScreen_Shell_Load::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Load::HideScreen(
    const mainMenuTransition_t transitionType) {
...
void idMenuScreen_Shell_Load::LoadDamagedGame(int index) {
  ...
  class idSWFScriptFunction_LoadDamaged
      : public idSWFScriptFunction_RefCounted {
   public:
    idSWFScriptFunction_LoadDamaged(gameDialogMessages_t _msg, bool _accept,
                                    int _index,
                                    idMenuScreen_Shell_Load* _screen) {
      msg = _msg;
      accept = _accept;
      index = _index;
      screen = _screen;
  ...
  callbacks.Append(new (TAG_SWF) idSWFScriptFunction_LoadDamaged(
      GDM_LOAD_DAMAGED_FILE, true, index, this));
  callbacks.Append(new (TAG_SWF) idSWFScriptFunction_LoadDamaged(
      GDM_LOAD_DAMAGED_FILE, false, index, this));
  idStaticList<idStrId, 4> optionText;
  optionText.Append(idStrId("#str_02315"));  // DELETE
  optionText.Append(idStrId("#STR_SWF_CANCEL"));

  common->Dialog().AddDynamicDialog(GDM_LOAD_DAMAGED_FILE, callbacks,
                                    optionText, false, "");
}
...
void idMenuScreen_Shell_Load::LoadGame(int index) {
  ...
  if (mgr != NULL && mgr->GetInGame() && !isDead) {
    class idSWFScriptFunction_LoadDialog
        : public idSWFScriptFunction_RefCounted {
     public:
      idSWFScriptFunction_LoadDialog(gameDialogMessages_t _msg, bool _accept,
                                     const char* _name) {
        msg = _msg;
        accept = _accept;
        name = _name;
    ...
    if (index < sortedSaves.Num()) {
      ...
      common->Dialog().AddDialog(GDM_SP_LOAD_SAVE, DIALOG_ACCEPT_CANCEL,
                                 new idSWFScriptFunction_LoadDialog(
                                     GDM_SP_LOAD_SAVE, true, name.c_str()),
                                 new idSWFScriptFunction_LoadDialog(
                                     GDM_SP_LOAD_SAVE, false, name.c_str()),
                                 false);
    }
...
========================
*/
bool idMenuScreen_Shell_Load::HandleAction(idWidgetAction& action,
