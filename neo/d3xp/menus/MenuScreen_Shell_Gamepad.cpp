...
void idMenuScreen_Shell_Gamepad::Initialize(idMenuHandler* data) {
  ...
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_CONFIG);
  ...
  control->SetDataSource(&gamepadData,
                         idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_LEFTY);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_LEFTY);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData, idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_INVERT);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_INVERT);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData, idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_VIBRATE);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_VIBRATE);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData, idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_HOR_SENS);
  control->SetupEvents(2, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_HOR_SENS);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData, idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_VERT_SENS);
  control->SetupEvents(2, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_VERT_SENS);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData,
      idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_ACCELERATION);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_ACCELERATION);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &gamepadData, idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_THRESHOLD);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, GAMEPAD_CMD_THRESHOLD);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
...
========================
*/
void idMenuScreen_Shell_Gamepad::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Gamepad::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_Gamepad::HandleAction(idWidgetAction& action,
    ...
    case WIDGET_ACTION_COMMAND: {
      ...
      switch (parms[0].ToInteger()) {
        ...
        case GAMEPAD_CMD_CONFIG: {
          menuData->SetNextScreen(SHELL_AREA_CONTROLLER_LAYOUT,
        ...
        case GAMEPAD_CMD_INVERT: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_INVERT, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_LEFTY: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_LEFTY, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_VIBRATE: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_VIBRATE, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_HOR_SENS: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_HOR_SENS, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_VERT_SENS: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_VERT_SENS, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_ACCELERATION: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_ACCELERATION, 1);
          options->Update();
          break;
        }
        case GAMEPAD_CMD_THRESHOLD: {
          gamepadData.AdjustField(
              idMenuDataSource_GamepadSettings::GAMEPAD_FIELD_THRESHOLD, 1);
          options->Update();
          break;
        }
      }
...
void idMenuScreen_Shell_Gamepad::idMenuDataSource_GamepadSettings::LoadData() {
  ...
  fields[GAMEPAD_FIELD_LEFTY].SetBool(profile ? profile->GetLeftyFlip()
                                              : false);
  fields[GAMEPAD_FIELD_VIBRATE].SetBool(in_joystickRumble.GetBool());
  fields[GAMEPAD_FIELD_HOR_SENS].SetFloat(
      100.0f * ((joy_yawSpeed.GetFloat() - 100.0f) / 300.0f));
  fields[GAMEPAD_FIELD_VERT_SENS].SetFloat(
      100.0f * ((joy_pitchSpeed.GetFloat() - 60.0f) / 200.0f));
...
void idMenuScreen_Shell_Gamepad::idMenuDataSource_GamepadSettings::
  ...
  joy_yawSpeed.SetFloat(
      ((fields[GAMEPAD_FIELD_HOR_SENS].ToFloat() / 100.0f) * 300.0f) + 100.0f);
  joy_pitchSpeed.SetFloat(
      ((fields[GAMEPAD_FIELD_VERT_SENS].ToFloat() / 100.0f) * 200.0f) + 60.0f);
...
========================
*/
void idMenuScreen_Shell_Gamepad::idMenuDataSource_GamepadSettings::AdjustField(
    const int fieldIndex, const int adjustAmount) {
  if (fieldIndex == GAMEPAD_FIELD_INVERT || fieldIndex == GAMEPAD_FIELD_LEFTY ||
      fieldIndex == GAMEPAD_FIELD_VIBRATE ||
      fieldIndex == GAMEPAD_FIELD_ACCELERATION ||
      fieldIndex == GAMEPAD_FIELD_THRESHOLD) {
    fields[fieldIndex].SetBool(!fields[fieldIndex].ToBool());
  } else {
    float newValue = idMath::ClampFloat(
        0.0f, 100.0f, fields[fieldIndex].ToFloat() + adjustAmount);
...
========================
*/
bool idMenuScreen_Shell_Gamepad::idMenuDataSource_GamepadSettings::
    IsDataChanged() const {
  if (fields[GAMEPAD_FIELD_INVERT].ToBool() !=
      originalFields[GAMEPAD_FIELD_INVERT].ToBool()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_LEFTY].ToBool() !=
      originalFields[GAMEPAD_FIELD_LEFTY].ToBool()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_VIBRATE].ToBool() !=
      originalFields[GAMEPAD_FIELD_VIBRATE].ToBool()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_HOR_SENS].ToFloat() !=
      originalFields[GAMEPAD_FIELD_HOR_SENS].ToFloat()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_VERT_SENS].ToFloat() !=
      originalFields[GAMEPAD_FIELD_VERT_SENS].ToFloat()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_ACCELERATION].ToBool() !=
      originalFields[GAMEPAD_FIELD_ACCELERATION].ToBool()) {
    return true;
  }

  if (fields[GAMEPAD_FIELD_THRESHOLD].ToBool() !=
      originalFields[GAMEPAD_FIELD_THRESHOLD].ToBool()) {
    return true;
  }
