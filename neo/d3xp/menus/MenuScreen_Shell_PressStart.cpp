...
void idMenuScreen_Shell_PressStart::Initialize(idMenuHandler* data) {
  ...
  while (itemList->GetChildren().Num() < NUM_GAME_SELECTIONS_VISIBLE) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PRESS_FOCUSED, itemList->GetChildren().Num());
  ...
  doom2Cover =
      declManager->FindMaterial("guis/assets/mainmenu/doom2_cover.tga");
  doom3Cover =
      declManager->FindMaterial("guis/assets/mainmenu/doom3_cover.tga");
    ...
    if (g_demoMode.GetBool()) {
      ...
      if (startButton != NULL) {
        ...
        startButton->SetLabel(
            idLocalization::GetString("#str_swf_press_start"));
      }

      idSWFSpriteInstance* backing =
          GetSprite()->GetScriptObject()->GetNestedSprite("backing");
      if (backing != NULL) {
...
bool idMenuScreen_Shell_PressStart::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    ...
    case WIDGET_ACTION_SCROLL_HORIZONTAL: {
      ...
      if (direction == 1) {
        if (itemList->GetViewIndex() ==
            itemList->GetTotalNumberOfOptions() - 1) {
          return true;
        } else {
