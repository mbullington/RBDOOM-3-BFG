  ...
  for (int optionIndex = 0; optionIndex < GetNumVisibleOptions();
    ...
    if (child.BindSprite(root)) {
      if (optionIndex >= GetTotalNumberOfOptions()) {
        ...
        // const int controlIndex = GetNumVisibleOptions() - Min(
        // GetNumVisibleOptions(), GetTotalNumberOfOptions() ) + optionIndex;
...
========================
*/
bool idMenuWidget_DynamicList::PrepareListElement(idMenuWidget& widget,
                                                  const int childIndex) {
  idMenuWidget_ScoreboardButton* const sbButton =
      dynamic_cast<idMenuWidget_ScoreboardButton*>(&widget);
  if (sbButton != NULL) {
    return true;
  }
  ...
  idMenuWidget_Button* const button =
      dynamic_cast<idMenuWidget_Button*>(&widget);
  if (button != NULL) {
...
void idMenuWidget_ScoreboardList::Update() {
  ...
  for (int optionIndex = 0; optionIndex < GetNumVisibleOptions();
    ...
    if (child.BindSprite(root)) {
      ...
      if (optionIndex == focusIndex &&
          child.GetState() != WIDGET_STATE_SELECTED &&
          child.GetState() != WIDGET_STATE_SELECTING) {
        child.SetState(WIDGET_STATE_SELECTING);
      } else if (optionIndex != focusIndex &&
                 child.GetState() != WIDGET_STATE_NORMAL) {
        child.SetState(WIDGET_STATE_NORMAL);
      }
