...
void idMenuScreen_PDA_UserData::Initialize(idMenuHandler* data) {
  ...
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaAudioFiles.GetChildByIndex(0),
  ...
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RSTICK_RELEASE)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaAudioFiles.GetChildByIndex(0), WIDGET_ACTION_EVENT_STOP_REPEATER,
...
========================
*/
void idMenuScreen_PDA_UserData::ShowScreen(
  ...
  if (menuGUI != NULL && menuData != NULL) {
    ...
    if (pdaSprite != NULL && menuData != NULL &&
        menuData->ActiveScreen() != PDA_AREA_USER_EMAIL) {
      pdaSprite->SetVisible(true);
      pdaSprite->PlayFrame("rollOn");
    }

    idSWFSpriteInstance* navBar = root.GetNestedSprite("navBar");
    if (navBar != NULL && menuData != NULL &&
        menuData->ActiveScreen() == PDA_AREA_INVALID) {
...
========================
*/
void idMenuScreen_PDA_UserData::HideScreen(
    const mainMenuTransition_t transitionType) {
  if (menuGUI != NULL) {
    idSWFScriptObject& root = menuGUI->GetRootObject();
    idSWFSpriteInstance* pdaSprite = root.GetNestedSprite("pda_persons");
    if (pdaSprite != NULL && menuData != NULL &&
        menuData->NextScreen() != PDA_AREA_USER_EMAIL) {
      pdaSprite->SetVisible(true);
...
========================
*/
bool idMenuScreen_PDA_UserData::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    case WIDGET_ACTION_PRESS_FOCUSED: {
      ...
      idPlayer* player = gameLocal.GetLocalPlayer();
      if (player != NULL &&
          player->IsSoundChannelPlaying(SND_CHANNEL_PDA_AUDIO)) {
        player->EndAudioLog();
      } else {
        if (menuData != NULL && pdaAudioFiles.GetChildren().Num() > 0) {
          int index = pdaAudioFiles.GetChildByIndex(0).GetFocusIndex();
          idMenuHandler_PDA* pdaHandler =
              dynamic_cast<idMenuHandler_PDA* const>(menuData);
          if (pdaHandler != NULL) {
