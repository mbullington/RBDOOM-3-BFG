...
Shows a help tooltip message based on observed events.  It's expected that the
widgets being observed are all buttons, and therefore have a GetDescription()
...
void idMenuWidget_Help::Update() {
  ...
  const idStr& msg = (lastHoveredMessage.Length() > 0) ? lastHoveredMessage
                                                       : lastFocusedMessage;
  if (msg.Length() > 0 && !hideMessage) {
    // try to show it if...
    //		- we are on the first frame
    //		- we aren't still animating while being between the "show" and "shown"
    //frames
    //
    if (GetSprite()->GetCurrentFrame() != GetSprite()->FindFrame("shown") &&
        (GetSprite()->GetCurrentFrame() == 1 ||
         !(GetSprite()->IsPlaying() &&
           GetSprite()->IsBetweenFrames("show", "shown")))) {
      GetSprite()->PlayFrame("show");
    }

    idSWFScriptObject* const textObject =
        GetSprite()->GetScriptObject()->GetNestedObj("txtOption", "txtValue");
    if (textObject != NULL) {
...
========================
*/
void idMenuWidget_Help::ObserveEvent(const idMenuWidget& widget,
  ...
  switch (event.type) {
    ...
    case WIDGET_EVENT_FOCUS_OFF: {
      // Don't do anything when losing focus. Focus updates come in pairs, so we
      // can skip doing anything on the "lost focus" event, and instead do
      // updates on the "got focus" event.
      break;
    }
