...
void idMenuScreen_Shell_Controls::Initialize(idMenuHandler* data) {
  ...
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, CONTROLS_CMD_BINDINGS);
  ...
  control->RegisterEventObserver(helpWidget);
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, CONTROLS_CMD_GAMEPAD);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &controlData,
      idMenuDataSource_ControlSettings::CONTROLS_FIELD_GAMEPAD_ENABLED);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, CONTROLS_CMD_GAMEPAD_ENABLED);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &controlData,
      idMenuDataSource_ControlSettings::CONTROLS_FIELD_INVERT_MOUSE);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, CONTROLS_CMD_INVERT);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
  ...
  control->SetDataSource(
      &controlData,
      idMenuDataSource_ControlSettings::CONTROLS_FIELD_MOUSE_SENS);
  control->SetupEvents(2, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_COMMAND, CONTROLS_CMD_MOUSE_SENS);
  control->RegisterEventObserver(helpWidget);
  options->AddChild(control);
...
========================
*/
void idMenuScreen_Shell_Controls::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Controls::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_Controls::HandleAction(idWidgetAction& action,
    ...
    case WIDGET_ACTION_COMMAND: {
      ...
      switch (parms[0].ToInteger()) {
        ...
        case CONTROLS_CMD_INVERT: {
          controlData.AdjustField(
              idMenuDataSource_ControlSettings::CONTROLS_FIELD_INVERT_MOUSE, 1);
          if (options != NULL) {
            options->Update();
          }
          break;
        ...
        case CONTROLS_CMD_MOUSE_SENS: {
          controlData.AdjustField(
              idMenuDataSource_ControlSettings::CONTROLS_FIELD_MOUSE_SENS, 1);
          if (options != NULL) {
            options->Update();
          }
          break;
...
========================
*/
idMenuScreen_Shell_Controls::idMenuDataSource_ControlSettings::
...
void idMenuScreen_Shell_Controls::idMenuDataSource_ControlSettings::LoadData() {
  ...
  float mouseSpeed =
      ((in_mouseSpeed.GetFloat() - 0.25f) / (4.0f - 0.25)) * 100.0f;
...
========================
*/
void idMenuScreen_Shell_Controls::idMenuDataSource_ControlSettings::
    CommitData() {
  in_mouseInvertLook.SetBool(fields[CONTROLS_FIELD_INVERT_MOUSE].ToBool());
  float mouseSpeed =
      0.25f +
      ((4.0f - 0.25) * (fields[CONTROLS_FIELD_MOUSE_SENS].ToFloat() / 100.0f));
...
========================
*/
void idMenuScreen_Shell_Controls::idMenuDataSource_ControlSettings::AdjustField(
...
========================
*/
bool idMenuScreen_Shell_Controls::idMenuDataSource_ControlSettings::
    IsDataChanged() const {
  if (fields[CONTROLS_FIELD_INVERT_MOUSE].ToBool() !=
      originalFields[CONTROLS_FIELD_INVERT_MOUSE].ToBool()) {
    return true;
  }

  if (fields[CONTROLS_FIELD_MOUSE_SENS].ToFloat() !=
      originalFields[CONTROLS_FIELD_MOUSE_SENS].ToFloat()) {
    return true;
  }

  if (fields[CONTROLS_FIELD_GAMEPAD_ENABLED].ToFloat() !=
      originalFields[CONTROLS_FIELD_GAMEPAD_ENABLED].ToFloat()) {
    return true;
  }
