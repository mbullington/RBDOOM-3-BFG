...
void idMenuScreen_PDA_UserEmails::Initialize(idMenuHandler* data) {
  ...
  emailScrollbar.SetSpritePath(GetSpritePath(), "info", "email", "info",
                               "scrollbar");
  ...
  emailInfo.AddEventAction(WIDGET_EVENT_ROLL_OVER)
      .Set(WIDGET_ACTION_EMAIL_HOVER, 1);
  emailInfo.AddEventAction(WIDGET_EVENT_ROLL_OUT)
      .Set(WIDGET_ACTION_EMAIL_HOVER, 0);
  ...
  if (pdaInbox.GetEmailList() != NULL) {
    ...
    for (int i = 0; i < pdaInbox.GetEmailList()->GetChildren().Num(); ++i) {
      ...
      idMenuWidget_Button* const button =
          dynamic_cast<idMenuWidget_Button*>(&child);
      if (button != NULL) {
  ...
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN_RSTICK));
  AddEventAction(WIDGET_EVENT_SCROLL_UP_RSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
  ...
  class idInfoBoxRefresh : public idSWFScriptFunction_RefCounted {
   public:
    idInfoBoxRefresh(idMenuWidget_InfoBox* _widget) : widget(_widget) {}
  ...
  if (GetSWFObject() != NULL) {
    GetSWFObject()->SetGlobal("refreshInfoBox",
                              new (TAG_SWF) idInfoBoxRefresh(&emailInfo));
  }
}
...
========================
*/
void idMenuScreen_PDA_UserEmails::ShowScreen(
  ...
  if (menuGUI != NULL) {
    ...
    if (pdaSprite != NULL && menuData != NULL &&
        menuData->ActiveScreen() != PDA_AREA_USER_DATA) {
...
void idMenuScreen_PDA_UserEmails::Update() {
  if (menuData != NULL) {
    ...
    idMenuWidget_CommandBar* cmdBar =
        dynamic_cast<idMenuWidget_CommandBar* const>(
            menuData->GetChildFromIndex(PDA_WIDGET_CMD_BAR));
    if (cmdBar != NULL) {
      ...
      if (readingEmails) {
        ...
        idMenuWidget_DynamicList* pdaList =
            dynamic_cast<idMenuWidget_DynamicList*>(
                menuData->GetChildFromIndex(PDA_WIDGET_PDA_LIST));
        if (pdaList != NULL) {
            ...
            if (pdaIndex < player->GetInventory().pdas.Num()) {
              ...
              if (pda != NULL && pdaInbox.GetEmailList() != NULL) {
                ...
                if (emailIndex < pda->GetNumEmails()) {
                  buttonInfo =
                      cmdBar->GetButton(idMenuWidget_CommandBar::BUTTON_JOY1);
                  if (menuData->GetPlatform() != 2) {
...
========================
*/
void idMenuScreen_PDA_UserEmails::HideScreen(
    const mainMenuTransition_t transitionType) {
  if (menuGUI != NULL) {
    idSWFScriptObject& root = menuGUI->GetRootObject();
    idSWFSpriteInstance* pdaSprite = root.GetNestedSprite("pda_persons");
...
========================
*/
bool idMenuScreen_PDA_UserEmails::ScrollCorrectList(idWidgetAction& action,
                                                    const idWidgetEvent& event,
                                                    idMenuWidget* widget) {
  bool handled = false;
  bool leftScroll = false;
  if (event.type == WIDGET_EVENT_SCROLL_UP_LSTICK ||
      event.type == WIDGET_EVENT_SCROLL_DOWN_LSTICK ||
      event.type == WIDGET_EVENT_SCROLL_UP ||
      event.type == WIDGET_EVENT_SCROLL_DOWN) {
    leftScroll = true;
  }
  ...
  if (readingEmails) {
    ...
    idMenuWidget_DynamicList* pdaList =
        dynamic_cast<idMenuWidget_DynamicList* const>(
            menuData->GetChildFromIndex(PDA_WIDGET_PDA_LIST));
    if (pdaList != NULL) {
...
========================
*/
bool idMenuScreen_PDA_UserEmails::HandleAction(idWidgetAction& action,
                                               const idWidgetEvent& event,
                                               idMenuWidget* widget,
                                               bool forceHandled) {
  if (menuData == NULL) {
...
========================
*/
void idMenuScreen_PDA_UserEmails::ObserveEvent(const idMenuWidget& widget,
                                               const idWidgetEvent& event) {
  if (menuData != NULL && menuData->ActiveScreen() != PDA_AREA_USER_EMAIL) {
    return;
  }

  const idMenuWidget_Button* const button =
      dynamic_cast<const idMenuWidget_Button*>(&widget);
  if (button == NULL) {
    return;
  }
