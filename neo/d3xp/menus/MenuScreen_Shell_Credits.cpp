...
void idMenuScreen_Shell_Credits::SetupCreditList() {
  class idRefreshCredits : public idSWFScriptFunction_RefCounted {
   public:
    idRefreshCredits(idMenuScreen_Shell_Credits* _screen) : screen(_screen) {}
  ...
  if (GetSWFObject()) {
    GetSWFObject()->SetGlobal("updateCredits",
                              new (TAG_SWF) idRefreshCredits(this));
  }
  ...
  creditList.Append(
      creditInfo_t(0, "Robert Beckebans - Renderer + Engine upgrades, Linux"));
  creditList.Append(creditInfo_t());
  creditList.Append(creditInfo_t(1, "Additional Programming"));
  creditList.Append(
      creditInfo_t(0, "Daniel Gibson - Tons of code cleanups, netcode++"));
  creditList.Append(
      creditInfo_t(0, "Jonathan Young - Bugfixes, misc improvements"));
  creditList.Append(
      creditInfo_t(0, "Felix Rueegg - Doomclassic Linux support"));
  creditList.Append(creditInfo_t(0, "Carl Kenner - Bink video support"));
  creditList.Append(
      creditInfo_t(0, "Pat Raynor - Compiler tools, QOL patches"));
  creditList.Append(
      creditInfo_t(0, "Biel B. de Luna - Intro skipping, QOL patches"));
  ...
  creditList.Append(creditInfo_t(
      0, "Justin Marshall - QOL patches from IcedHellFire branch"));
  creditList.Append(creditInfo_t());
  creditList.Append(creditInfo_t());
  creditList.Append(creditInfo_t());
  creditList.Append(
      creditInfo_t(0, "Thanks to everyone who provided valuable feedback on"));
  ...
  creditList.Append(creditInfo_t());
  creditList.Append(
      creditInfo_t(1, "DOOM 3: Ressurection of Evil Developed by"));
...
========================
*/
void idMenuScreen_Shell_Credits::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Credits::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_Credits::HandleAction(idWidgetAction& action,
...
void idMenuScreen_Shell_Credits::UpdateCredits() {
  ...
  if (menuData->ActiveScreen() != SHELL_AREA_CREDITS &&
      menuData->NextScreen() != SHELL_AREA_CREDITS) {
    return;
  }
  ...
  idSWFScriptObject* options = GetSWFObject()->GetRootObject().GetNestedObj(
      "menuCredits", "info", "options");
  if (options != NULL) {
    for (int i = 15; i >= 0; --i) {
      int curIndex = creditIndex - i;
      idSWFTextInstance* heading =
          options->GetNestedText(va("item%d", 15 - i), "heading");
      idSWFTextInstance* subHeading =
          options->GetNestedText(va("item%d", 15 - i), "subHeading");
      idSWFTextInstance* title =
          options->GetNestedText(va("item%d", 15 - i), "title");
      idSWFTextInstance* txtEntry =
          options->GetNestedText(va("item%d", 15 - i), "entry");
