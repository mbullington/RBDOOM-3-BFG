...
static bindInfo_t keyboardBinds[] = {
    ...
    //{ "#str_04071",	"clientDropWeapon"						}	// DROP
    //WEAPON
};
...
void idMenuScreen_Shell_Bindings::Initialize(idMenuHandler* data) {
  ...
  restoreDefault->AddEventAction(WIDGET_EVENT_PRESS)
...
========================
*/
void idMenuScreen_Shell_Bindings::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Bindings::HideScreen(
    const mainMenuTransition_t transitionType) {
  if (bindingsChanged) {
...
void idMenuScreen_Shell_Bindings::UpdateBindingDisplay() {
  ...
  for (int i = 0; i < numBinds; ++i) {
    ...
    if ((idStr::Icmp(keyboardBinds[i].bind, "") != 0)) {
      keyBindings_t bind = idKeyInput::KeyBindingsFromBinding(
          keyboardBinds[i].bind, false, true);
      ...
      if (!bind.gamepad.IsEmpty() && in_useJoystick.GetBool()) {
        ...
        const char* buttonsWithImages[] = {
            ...
            "JOY6", "JOY_TRIGGER1", "JOY_TRIGGER2", 0};
...
void idMenuScreen_Shell_Bindings::HandleRestoreDefaults() {
  class idSWFScriptFunction_Restore : public idSWFScriptFunction_RefCounted {
   public:
    idSWFScriptFunction_Restore(gameDialogMessages_t _msg, bool _accept,
  ...
  common->Dialog().AddDialog(
      GDM_BINDINGS_RESTORE, DIALOG_ACCEPT_CANCEL,
      new idSWFScriptFunction_Restore(GDM_BINDINGS_RESTORE, true, this),
      new idSWFScriptFunction_Restore(GDM_BINDINGS_RESTORE, false, this),
      false);
}
...
========================
*/
bool idMenuScreen_Shell_Bindings::HandleAction(idWidgetAction& action,
