...
void idMenuScreen_Shell_Singleplayer::ContinueGame() {
  ...
  if (sortedSaves.Num() > 0) {
    if (sortedSaves[0].damaged) {
      class idSWFScriptFunction_ContinueDamaged
      ...
      common->Dialog().AddDynamicDialog(GDM_CORRUPT_CONTINUE, callbacks,
                                        optionText, false, "");
    } else {
    ...
    case WIDGET_ACTION_PRESS_FOCUSED: {
      ...
      if (canContinue) {
        if (selectionIndex == 0) {
          ...
          class idSWFScriptFunction_NewGame
              : public idSWFScriptFunction_RefCounted {
           public:
            idSWFScriptFunction_NewGame(idMenuHandler* _menuData,
                                        bool _accept) {
              menuData = _menuData;
              accept = _accept;
            }
          ...
          common->Dialog().AddDialog(
              GDM_DELETE_AUTOSAVE, DIALOG_ACCEPT_CANCEL,
              new idSWFScriptFunction_NewGame(menuData, true),
              new idSWFScriptFunction_NewGame(menuData, false), true);
