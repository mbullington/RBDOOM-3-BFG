...
void idMenuScreen_Scoreboard::Initialize(idMenuHandler* data) {
  ...
  while (playerList->GetChildren().Num() < MAX_SCOREBOARD_SLOTS) {
    idMenuWidget_ScoreboardButton* const buttonWidget =
        new (TAG_SWF) idMenuWidget_ScoreboardButton();
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PRESS_FOCUSED, playerList->GetChildren().Num());
    buttonWidget->AddEventAction(WIDGET_EVENT_COMMAND)
        .Set(WIDGET_ACTION_MUTE_PLAYER, playerList->GetChildren().Num());
  ...
  if (menuData != NULL) {
    idMenuWidget_CommandBar* cmdBar =
        dynamic_cast<idMenuWidget_CommandBar* const>(
            menuData->GetChildFromIndex(SCOREBOARD_WIDGET_CMD_BAR));
    if (cmdBar != NULL) {
...
========================
*/
void idMenuScreen_Scoreboard::ShowScreen(
  ...
  if (GetSWFObject()) {
    ...
    if (BindSprite(root)) {
      idSWFTextInstance* txtVal = GetSprite()->GetScriptObject()->GetNestedText(
          "info", "txtScoreboard");
      if (txtVal != NULL) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtGameType");
      if (txtVal != NULL) {
        idStr mode = idLocalization::GetString("#str_02376");
        mode.Append(": ");
        const idStrList& modes = common->GetModeDisplayList();
        idStr modeName = idLocalization::GetString(
            modes[idMath::ClampInt(0, modes.Num() - 1, gameLocal.gameType)]);
        mode.Append(
            idLocalization::GetString(idLocalization::GetString(modeName)));
        txtVal->SetText(mode);
        txtVal->SetStrokeInfo(true, 0.9f, 1.8f);
      }

      txtVal = GetSprite()->GetScriptObject()->GetNestedText("info",
                                                             "txtNameHeading");
      if (txtVal != NULL) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtScore");
      if (txtVal != NULL) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtScore2");
      if (txtVal != NULL) {
        if (gameLocal.gameType == GAME_LASTMAN) {
          txtVal->SetText(idLocalization::GetString("#str_04242"));
        } else if (gameLocal.gameType == GAME_CTF) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtWins2");
      if (txtVal != NULL) {
        txtVal->SetText("#str_02619");
        txtVal->SetStrokeInfo(true, 0.75f, 1.75f);
      }
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtPing2");
      if (txtVal != NULL) {
        txtVal->SetText("#str_02048");
        txtVal->SetStrokeInfo(true, 0.75f, 1.75f);
      }
...
========================
*/
void idMenuScreen_Scoreboard::SetPlayerData(
    idList<scoreboardInfo_t, TAG_IDLIB_LIST_MENU> data) {
  if (playerList != NULL) {
    for (int i = 0; i < data.Num(); ++i) {
      if (i < playerList->GetChildren().Num()) {
        idMenuWidget_ScoreboardButton* button =
            dynamic_cast<idMenuWidget_ScoreboardButton*>(
                &playerList->GetChildByIndex(i));
        if (button != NULL) {
          button->SetButtonInfo(data[i].index, data[i].values,
                                data[i].voiceState);
        }
      }
      playerList->Update();
  ...
  if (GetSWFObject()) {
    ...
    if (BindSprite(root)) {
      idSWFTextInstance* txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtGameInfo");
      if (txtVal != NULL) {
...
void idMenuScreen_Scoreboard::UpdateSpectating(idStr spectating, idStr follow) {
  if (GetSWFObject()) {
    ...
    if (BindSprite(root)) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtFollow");
      if (txtVal != NULL) {
...
void idMenuScreen_Scoreboard::UpdateTeamScores(int r, int b) {
  if (GetSWFObject()) {
    ...
    if (BindSprite(root)) {
      idSWFTextInstance* txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtRedScore");
      if (txtVal != NULL) {
      ...
      txtVal =
          GetSprite()->GetScriptObject()->GetNestedText("info", "txtBlueScore");
      if (txtVal != NULL) {
...
void idMenuScreen_Scoreboard::UpdateHighlight() {
  ...
  idMenuHandler_Scoreboard* data =
      dynamic_cast<idMenuHandler_Scoreboard*>(menuData);
...
========================
*/
bool idMenuScreen_Scoreboard::HandleAction(idWidgetAction& action,
