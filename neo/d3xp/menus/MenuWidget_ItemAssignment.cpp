...
void idMenuWidget_ItemAssignment::Update() {
  ...
  idSWFSpriteInstance* dpad =
      GetSprite()->GetScriptObject()->GetNestedSprite("dpad");
  ...
  for (int i = 0; i < NUM_QUICK_SLOTS; ++i) {
    idSWFSpriteInstance* item =
        GetSprite()->GetScriptObject()->GetNestedSprite(va("item%d", i));
    if (item != NULL) {
      if (i == slotIndex) {
        item->StopFrame(2);
      } else {
    ...
    idSWFSpriteInstance* itemIcon =
        GetSprite()->GetScriptObject()->GetNestedSprite(va("item%d", i), "img");
    if (itemIcon != NULL) {
      if (images[i] != NULL) {
    ...
    itemIcon = GetSprite()->GetScriptObject()->GetNestedSprite(va("item%d", i),
                                                               "imgTop");
    if (itemIcon != NULL) {
      if (images[i] != NULL) {
        itemIcon->SetVisible(true);
