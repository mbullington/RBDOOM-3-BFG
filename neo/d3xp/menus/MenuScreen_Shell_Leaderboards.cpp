...
void idMenuScreen_Shell_Leaderboards::Initialize(idMenuHandler* data) {
  ...
  while (options->GetChildren().Num() < NUM_LEADERBOARD_ITEMS) {
    idMenuWidget_Button* const buttonWidget =
        new (TAG_SWF) idMenuWidget_Button();
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PRESS_FOCUSED, options->GetChildren().Num());
  ...
  btnNext->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_TAB_NEXT, WIDGET_EVENT_TAB_NEXT));
  AddChild(btnNext);
  ...
  btnPrev->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_TAB_PREV, WIDGET_EVENT_TAB_PREV));
  AddChild(btnPrev);
  ...
  btnPageDwn->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_PAGE_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_PAGEDWN));
  btnPageDwn->AddEventAction(WIDGET_EVENT_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
  ...
  parms.Clear();
  parms.Append(MAX_STAT_LISTINGS - 1);
  btnPageUp->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_PAGE_UP_START_REPEATER,
          WIDGET_EVENT_SCROLL_PAGEUP));
  btnPageUp->AddEventAction(WIDGET_EVENT_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_PAGEUP_RELEASE));
  AddChild(btnPageUp);

  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER_VARIABLE,
          WIDGET_EVENT_SCROLL_DOWN_LSTICK));
  AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER_VARIABLE,
          WIDGET_EVENT_SCROLL_UP_LSTICK));
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE));
  AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
  ...
  AddEventAction(WIDGET_EVENT_SCROLL_UP_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_UP_RELEASE));
  AddEventAction(WIDGET_EVENT_SCROLL_PAGEDWN)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_PAGE_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_PAGEDWN));
  AddEventAction(WIDGET_EVENT_SCROLL_PAGEUP)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_SCROLL_PAGE_UP_START_REPEATER,
          WIDGET_EVENT_SCROLL_PAGEUP));
  AddEventAction(WIDGET_EVENT_SCROLL_PAGEDWN_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_PAGEDWN_RELEASE));
  AddEventAction(WIDGET_EVENT_SCROLL_PAGEUP_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(this, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_PAGEUP_RELEASE));
  AddEventAction(WIDGET_EVENT_TAB_NEXT)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_TAB_NEXT, WIDGET_EVENT_TAB_NEXT));
  AddEventAction(WIDGET_EVENT_TAB_PREV)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          this, WIDGET_ACTION_EVENT_TAB_PREV, WIDGET_EVENT_TAB_PREV));
...
void idMenuScreen_Shell_Leaderboards::Update() {
  if (menuData != NULL) {
    ...
    if (cmdBar != NULL) {
      ...
      if (!lbCache->IsLoadingNewLeaderboard() && !lbCache->IsRequestingRows() &&
          options != NULL && options->GetTotalNumberOfOptions() > 0) {
  ...
  if (BindSprite(root)) {
    ...
    idSWFSpriteInstance* gradient =
        GetSprite()->GetScriptObject()->GetNestedSprite("info", "gradient");
...
========================
*/
void idMenuScreen_Shell_Leaderboards::ShowScreen(
    const mainMenuTransition_t transitionType) {
  idMenuScreen::ShowScreen(transitionType);

  if (GetSprite() != NULL) {
    lbHeading =
        GetSprite()->GetScriptObject()->GetNestedText("info", "txtLbType");
    if (menuData != NULL && menuData->GetGUI() != NULL) {
      idSWFScriptObject* const shortcutKeys =
          menuData->GetGUI()->GetGlobal("shortcutKeys").GetObject();
      if (verify(shortcutKeys != NULL)) {
        // TAB NEXT
        idSWFScriptObject* const btnTabNext =
            GetSprite()->GetScriptObject()->GetNestedObj("info", "btnNext");
        if (btnTabNext != NULL) {
          btnTabNext->Set("onPress", new (TAG_SWF) WrapWidgetSWFEvent(
                                         this, WIDGET_EVENT_TAB_NEXT, 0));
        ...
        // TAB PREV
        idSWFScriptObject* const btnTabPrev =
            GetSprite()->GetScriptObject()->GetNestedObj("info", "btnPrevious");
        if (btnTabPrev != NULL) {
          btnTabPrev->Set("onPress", new (TAG_SWF) WrapWidgetSWFEvent(
                                         this, WIDGET_EVENT_TAB_PREV, 0));
        ...
        // TAB NEXT
        idSWFScriptObject* const btnDwn =
            GetSprite()->GetScriptObject()->GetNestedObj("info", "btnPageDwn");
        if (btnDwn != NULL) {
          btnDwn->Set("onPress", new (TAG_SWF) WrapWidgetSWFEvent(
                                     this, WIDGET_EVENT_SCROLL_PAGEDWN, 0));
        ...
        // TAB PREV
        idSWFScriptObject* const btnUp =
            GetSprite()->GetScriptObject()->GetNestedObj("info", "btnPageUp");
        if (btnUp != NULL) {
          btnUp->Set("onPress", new (TAG_SWF) WrapWidgetSWFEvent(
                                    this, WIDGET_EVENT_SCROLL_PAGEUP, 0));
  ...
  if (btnNext != NULL && btnNext->GetSprite() != NULL) {
    idSWFSpriteInstance* btnImg =
        btnNext->GetSprite()->GetScriptObject()->GetNestedSprite("btnImg");
  ...
  if (btnPrev != NULL && btnPrev->GetSprite() != NULL) {
    idSWFSpriteInstance* btnImg =
        btnPrev->GetSprite()->GetScriptObject()->GetNestedSprite("btnImg");
  ...
  if (btnPageDwn != NULL && btnPageDwn->GetSprite() != NULL) {
    idSWFSpriteInstance* btnImg =
        btnPageDwn->GetSprite()->GetScriptObject()->GetNestedSprite("btnImg");
  ...
  if (btnPageUp != NULL && btnPageUp->GetSprite() != NULL) {
    idSWFSpriteInstance* btnImg =
        btnPageUp->GetSprite()->GetScriptObject()->GetNestedSprite("btnImg");
...
========================
*/
void idMenuScreen_Shell_Leaderboards::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_Leaderboards::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    ...
    case WIDGET_ACTION_PRESS_FOCUSED: {
      ...
      const idLeaderboardCallback::row_t* row = lbCache->GetLeaderboardRow(
          lbCache->GetRowOffset() + lbCache->GetEntryIndex());
      if (row != NULL) {
...
========================
*/
void idMenuScreen_Shell_Leaderboards::UpdateLeaderboard(
...
void idMenuScreen_Shell_Leaderboards::SetLeaderboardIndex() {
  ...
  for (int i = 0; i < leaderboardDef->numColumns; i++) {
    /*if ( leaderboardDef->columnDefs[i].displayType !=
    STATS_COLUMN_NEVER_DISPLAY ) {
            gameLocal->GetMainMenu()->mainMenu->SetGlobal( va("columnname%d",i),
    leaderboardDef->columnDefs[i].locDisplayName );
    }*/
  }
...
void idMenuScreen_Shell_Leaderboards::RefreshLeaderboard() {
  ...
  if (!lbCache->IsLoadingNewLeaderboard() &&
      lbCache->GetErrorCode() == LEADERBOARD_DISPLAY_ERROR_NONE) {
    for (int addIndex = 0; addIndex < MAX_STAT_LISTINGS; ++addIndex) {
      ...
      const idLeaderboardCallback::row_t* row = lbCache->GetLeaderboardRow(
          index);  // If this row is not in the cache, this will kick off a
                   // request
      if (row != NULL) {
        values.Append(va("%i", (int)row->rank));
        values.Append(row->name);
        values.Append(FormatColumn(&lbCache->GetLeaderboard()->columnDefs[0],
                                   row->columns[0])
                          .ToString());
      }
    ...
    if ((lbCache->GetRowOffset() + MAX_STAT_LISTINGS) <
        lbCache->GetNumRowsInLeaderboard()) {
      downArrow = true;
    }
  }
  ...
  compile_time_assert(sizeof(leaderboardErrorStrings) /
                          sizeof(leaderboardErrorStrings[0]) ==
...
========================
*/
void idMenuScreen_Shell_Leaderboards::ShowMessage(bool show, idStr message,
                                                  bool spinner) {
  if (!menuData || !menuData->GetGUI()) {
    return;
  }

  idSWFSpriteInstance* pacifier =
      menuData->GetGUI()->GetRootObject().GetNestedSprite("menuLeaderboards",
                                                          "info", "pacifier");
  ...
  if (show) {
    ...
    idSWFTextInstance* txtMsg =
        pacifier->GetScriptObject()->GetNestedText("message");
    if (txtMsg != NULL) {
      txtMsg->SetText(message);
      txtMsg->SetStrokeInfo(true, 0.75f, 1.75f);
    }

    idSWFSpriteInstance* spriteSpinner =
        pacifier->GetScriptObject()->GetNestedSprite("graphic");
...
class LBCallback : public idLeaderboardCallback {
 public:
  ...
  void Call() { gameLocal.Shell_UpdateLeaderboard(this); }

  LBCallback* Clone() const { return new LBCallback(*this); }
};
...
========================
*/
void idLBCache::SetLeaderboard(const leaderboardDefinition_t* def_,
                               leaderboardFilterMode_t filter_) {
  // If we are busy waiting on results from a previous request, queue up this
  // request
  if (loadingNewLeaderboard || requestingRows) {
    pendingDef = def_;
  ...
  loadingNewLeaderboard = true;  // This means we are waiting on the first set
                                 // of results for this new leaderboard

  localIndex = -1;  // don't know where the user is in the rows yet

  // Kick off initial stats request (which is initially based on the filter
  // type)
  if (filter == LEADERBOARD_FILTER_MYSCORE) {
    ...
    LBCallback cb;
    session->LeaderboardDownload(0, def, -1, 100,
                                 cb);  // Request up to 100 friends
  } else {
    LBCallback cb;
    session->LeaderboardDownload(0, def, rowOffset + 1, MAX_ROWS_PER_BLOCK, cb);
    // session->LeaderboardDownload( 0, def, rowOffset + 1, 10, cb );		// For
    // testing
  }
}
...
idStr idLBCache::GetFilterStrType() {
  if (filter == LEADERBOARD_FILTER_FRIENDS) {
    ...
    return idLocalization::GetString(
        "#str_swf_leaderboards_global_self_heading");
  }
...
bool idLBCache::Scroll(int amount) {
  ...
  // Clamp row offset
  rowOffset = idMath::ClampInt(
      0, Max(numRowsInLeaderboard - MAX_STAT_LISTINGS, 0), rowOffset);
...
bool idLBCache::ScrollOffset(int amount) {
  ...
  if (rowOffset != oldRowOffset) {
    entryIndex -=
        amount;  // adjust in opposite direction so same item stays selected
    entryIndex =
        idMath::ClampInt(0, rowOffset + (MAX_STAT_LISTINGS - 1), entryIndex);
  } else {
    entryIndex += amount;
...
========================
*/
leaderboardDisplayError_t idLBCache::CallbackErrorToDisplayError(
    leaderboardError_t errorCode) {
  switch (errorCode) {
...
void idLBCache::Update(const idLeaderboardCallback* callback) {
  ...
  if (loadingNewLeaderboard && filter == LEADERBOARD_FILTER_MYSCORE &&
      callback->GetLocalIndex() == -1 &&
  ...
  if (callback->GetDef() != def) {
    // Not the leaderboard we are looking for (This should no longer be
    // possible)
  ...
  // Store the index that the master user is in, if we haven't already found the
  // index
  ...
  if (loadingNewLeaderboard == true) {
    ...
    if (callback->GetLocalIndex() != -1 &&
      ...
      entryIndex =
          (callback->GetLocalIndex() + callback->GetStartIndex()) - rowOffset;
    }
    loadingNewLeaderboard = false;
  }
...
const idLeaderboardCallback::row_t* idLBCache::GetLeaderboardRow(int row) {
  if (loadingNewLeaderboard) {
    return NULL;  // If we are refreshing (seeing this leaderboard for the first
                  // time), force NULL till we get first set of results
  }
