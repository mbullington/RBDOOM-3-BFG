...
static gamepadBindInfo_t gamepadBinds[] = {{"txtJoy1", K_JOY1},
                                           {"txtJoy2", K_JOY2},
                                           {"txtJoy3", K_JOY3},
                                           {"txtJoy4", K_JOY4},
...
static const int numGamepadBinds =
    sizeof(gamepadBinds) / sizeof(gamepadBinds[0]);
...
void idMenuScreen_Shell_ControllerLayout::Initialize(idMenuHandler* data) {
  ...
  control->SetDataSource(&layoutData,
                         idMenuDataSource_LayoutSettings::LAYOUT_FIELD_LAYOUT);
...
void idMenuScreen_Shell_ControllerLayout::Update() {
  ...
  if (BindSprite(root)) {
    ...
    if (menuData != NULL) {
      ...
      layout = GetSprite()->GetScriptObject()->GetNestedSprite(
          "info", "controlInfo", "layout360");
...
========================
*/
void idMenuScreen_Shell_ControllerLayout::ShowScreen(
  ...
  if (GetSprite() != NULL) {
    ...
    layout360 = GetSprite()->GetScriptObject()->GetNestedSprite(
        "info", "controlInfo", "layout360");
    layoutPS3 = GetSprite()->GetScriptObject()->GetNestedSprite(
        "info", "controlInfo", "layoutPS3");
...
========================
*/
void idMenuScreen_Shell_ControllerLayout::HideScreen(
    const mainMenuTransition_t transitionType) {
...
void idMenuScreen_Shell_ControllerLayout::UpdateBindingInfo() {
  ...
  for (int i = 0; i < numGamepadBinds; ++i) {
    ...
    txtVal = GetSprite()->GetScriptObject()->GetNestedText(
        "info", "controlInfo", "layout360", txtField);
    ...
    if (txtVal != NULL) {
      ...
      if (binding == NULL || binding[0] == 0) {
        ...
        idStr action =
            idLocalization::GetString(va("#str_swf_action%s", binding));
...
========================
*/
bool idMenuScreen_Shell_ControllerLayout::HandleAction(
    idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget,
    bool forceHandled) {
  if (menuData == NULL) {
    return true;
  }
...
========================
*/
idMenuScreen_Shell_ControllerLayout::idMenuDataSource_LayoutSettings::
...
========================
*/
void idMenuScreen_Shell_ControllerLayout::idMenuDataSource_LayoutSettings::
    LoadData() {
  ...
  fields[LAYOUT_FIELD_LAYOUT].SetString(
      idLocalization::GetString(va("#str_swf_config_360_%i", configSet)));
...
========================
*/
bool idMenuScreen_Shell_ControllerLayout::idMenuDataSource_LayoutSettings::
    IsDataChanged() const {
  bool hasLocalChanges = false;

  if (fields[LAYOUT_FIELD_LAYOUT].ToString() !=
      originalFields[LAYOUT_FIELD_LAYOUT].ToString()) {
    return true;
  }
