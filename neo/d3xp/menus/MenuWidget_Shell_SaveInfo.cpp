...
void idMenuWidget_Shell_SaveInfo::Update() {
  ...
  idStr info;
  if (loadIndex >= 0 && sortedSaves.Num() != 0 &&
      loadIndex < sortedSaves.Num()) {
    ...
    idStr summary;
    summary.Format(idLocalization::GetString("#str_swf_save_info_format"),
                   difficultyStr, Sys_SecToStr(details.GetPlaytime()),
                   expansionStr);
  ...
  idSWFTextInstance* infoSprite =
      GetSprite()->GetScriptObject()->GetNestedText("txtDesc");
  if (infoSprite != NULL) {
    infoSprite->SetText(info);
  }

  idSWFSpriteInstance* img =
      GetSprite()->GetScriptObject()->GetNestedSprite("img");
  if (img != NULL) {
...
========================
*/
void idMenuWidget_Shell_SaveInfo::ObserveEvent(const idMenuWidget& widget,
  ...
  switch (event.type) {
    case WIDGET_EVENT_FOCUS_ON: {
      const idMenuWidget_DynamicList* const list =
          dynamic_cast<const idMenuWidget_DynamicList* const>(listWidget);
      loadIndex = list->GetViewIndex();

      const saveGameDetailsList_t& detailList =
          session->GetSaveGameManager().GetEnumeratedSavegames();
      ...
      if (forSaveScreen &&
          ((detailList.Num() < MAX_SAVEGAMES - 1) ||
           ((detailList.Num() == MAX_SAVEGAMES - 1) && hasAutoSave))) {
        loadIndex -= 1;
      }
      ...
      idMenuScreen_Shell_Load* loadScreen =
          dynamic_cast<idMenuScreen_Shell_Load*>(GetParent());
      if (loadScreen) {
        loadScreen->UpdateSaveEnumerations();
      }

      idMenuScreen_Shell_Save* saveScreen =
          dynamic_cast<idMenuScreen_Shell_Save*>(GetParent());
      if (saveScreen) {
