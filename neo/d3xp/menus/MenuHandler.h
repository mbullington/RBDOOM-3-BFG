...
class mpScoreboardInfo {
 public:
  ...
  bool operator!=(const mpScoreboardInfo& otherInfo) const {
    if (otherInfo.score != score || otherInfo.wins != wins ||
        otherInfo.ping != ping || otherInfo.spectateData != spectateData ||
        otherInfo.name != name || otherInfo.team != team ||
        otherInfo.playerNum != playerNum ||
        otherInfo.voiceState != voiceState) {
      return true;
    }
...
struct lobbyPlayerInfo_t {
  lobbyPlayerInfo_t() : partyToken(0), voiceState(VOICECHAT_DISPLAY_NONE) {}
...
class idMenuHandler_Shell : public idMenuHandler {
 public:
  ...
  bool IsPacifierVisible() const {
    return (pacifier != NULL && pacifier->GetSprite() != NULL)
               ? pacifier->GetSprite()->IsVisible()
               : false;
  }
...
class idMenuHandler_PDA : public idMenuHandler {
 public:
  idMenuHandler_PDA()
      : audioLogPlaying(false), videoPlaying(false), audioFile(NULL) {}
...
class idMenuHandler_Scoreboard : public idMenuHandler {
 public:
  idMenuHandler_Scoreboard()
      : redScore(0), blueScore(0), activationScreen(SCOREBOARD_AREA_INVALID) {}
  ...
  void AddPlayerInfo(int index, voiceStateDisplay_t voiceState, int team,
                     idStr name, int score, int wins, int ping,
                     idStr spectateData);
