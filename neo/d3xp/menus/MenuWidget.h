...
struct widgetTransition_t {
  widgetTransition_t() : animationName(NULL) {}

  const char* animationName;  // name of the animation to run
  idStaticList<const char*, 4>
      prefixes;  // prefixes to try to use for animation
};
...
struct scoreboardInfo_t {
  scoreboardInfo_t() : index(-1), voiceState(VOICECHAT_DISPLAY_NONE) {}
...
class idSort_SavesByDate
    : public idSort_Quick<idSaveGameDetails, idSort_SavesByDate> {
 public:
...
class idWidgetEvent {
 public:
  idWidgetEvent() : type(WIDGET_EVENT_PRESS), arg(0), thisObject(NULL) {}
...
class idWidgetAction {
 public:
  idWidgetAction() : action(WIDGET_ACTION_NONE), scriptFunction(NULL) {}
  ...
  void Set(widgetAction_t action_, const idSWFScriptVar& var1,
           const idSWFScriptVar& var2, const idSWFScriptVar& var3) {
    action = action_;
    parms.Clear();
    parms.Append(var1);
    parms.Append(var2);
...
class idMenuWidget {
  ...
  */
  class WrapWidgetSWFEvent : public idSWFScriptFunction_RefCounted {
   public:
    WrapWidgetSWFEvent(idMenuWidget* widget, const widgetEvent_t event,
                       const int eventArg)
  ...
  // takes the information described in this widget and applies it to a given
  // script object. the script object should point to the root that you want to
  // run from. Running this will also create the sprite binding, if any.
  virtual void Update() {}
  virtual void Show();
  virtual void Hide();
  ...
  void SetSpritePath(const char* arg1, const char* arg2 = NULL,
                     const char* arg3 = NULL, const char* arg4 = NULL,
                     const char* arg5 = NULL);
  void SetSpritePath(const idList<idStr>& spritePath_, const char* arg1 = NULL,
  ...
  virtual void ObserveEvent(const idMenuWidget& widget,
  ...
  idList<idWidgetAction, TAG_IDLIB_LIST_MENU>* GetEventActions(
      const widgetEvent_t eventType);
  ...
  idMenuWidget* GetFocus() {
    return (focusIndex >= 0 && focusIndex < children.Num())
               ? children[focusIndex]
               : NULL;
  }
 ...
 protected:
  ...
  idList<idList<idWidgetAction, TAG_IDLIB_LIST_MENU>, TAG_IDLIB_LIST_MENU>
      eventActions;
...
class idMenuWidget_Button : public idMenuWidget {
 public:
  ...
  idMenuWidget_Button()
      : animState(ANIM_STATE_UP), img(NULL), ignoreColor(false) {}
 ...
 protected:
  void SetupTransitionInfo(widgetTransition_t& trans,
                           const widgetState_t buttonState,
                           const animState_t sourceAnimState,
                           const animState_t destAnimState) const;
...
class idMenuWidget_LobbyButton : public idMenuWidget_Button {
 public:
  idMenuWidget_LobbyButton() : voiceState(VOICECHAT_DISPLAY_NONE) {}
...
class idMenuWidget_ScoreboardButton : public idMenuWidget_Button {
 public:
  idMenuWidget_ScoreboardButton()
      : voiceState(VOICECHAT_DISPLAY_NONE), index(-1) {}

  virtual void Update();
  void SetButtonInfo(int index_, idList<idStr>& list,
...
class idMenuWidget_ControlButton : public idMenuWidget_Button {
 public:
  idMenuWidget_ControlButton()
      : optionType(OPTION_BUTTON_TEXT), disabled(false) {}
...
class idMenuWidget_ServerButton : public idMenuWidget_Button {
 public:
  idMenuWidget_ServerButton()
      : index(0), players(0), maxPlayers(0), joinable(false), validMap(false) {}

  virtual void Update();
  void SetButtonInfo(idStr name_, idStrId mapName_, idStr modeName_,
                     int index_ = 0, int players_ = 0, int maxPlayers_ = 0,
                     bool joinable_ = false, bool validMap_ = false);
...
class idMenuWidget_NavButton : public idMenuWidget_Button {
 public:
  ...
  idMenuWidget_NavButton() : navIndex(0), xPos(0) {}
...
class idMenuWidget_GameBrowserList : public idMenuWidget_List {
 public:
  ...
  void ClearGames();
  void AddGame(idStr name_, idStrId mapName_, idStr modeName_, int index_ = 0,
               int players_ = 0, int maxPlayers_ = 0, bool joinable_ = false,
               bool validMap_ = false);
  int GetServerIndex();
...
class idMenuWidget_CommandBar : public idMenuWidget {
 public:
  ...
  idMenuWidget_CommandBar() : alignment(LEFT) { buttons.SetNum(MAX_BUTTONS); }
...
class idMenuWidget_DynamicList : public idMenuWidget_List {
 public:
  ...
  virtual void SetListData(
      idList<idList<idStr, TAG_IDLIB_LIST_MENU>, TAG_IDLIB_LIST_MENU>& list);
...
class idMenuWidget_SystemOptionsList : public idMenuWidget_DynamicList {
 public:
  ...
  virtual void Scroll(const int scrollAmount,
                      const bool wrapAround = false) override;
};
// RB end
...
class idMenuWidget_PDA_UserData : public idMenuWidget {
 public:
  idMenuWidget_PDA_UserData() : pdaIndex(0) {}
...
class idMenuWidget_ScrollBar : public idMenuWidget {
 public:
  idMenuWidget_ScrollBar() : yTop(0.0f), yBot(0.0f), dragging(false) {}
...
class idMenuWidget_InfoBox : public idMenuWidget {
 public:
  idMenuWidget_InfoBox() : scrollbar(NULL) {}
...
class idMenuWidget_PDA_Objective : public idMenuWidget {
 public:
  idMenuWidget_PDA_Objective() : pdaIndex(0) {}
  virtual void Update();
  virtual void ObserveEvent(const idMenuWidget& widget,
                            const idWidgetEvent& event);
...
class idMenuWidget_Shell_SaveInfo : public idMenuWidget {
 public:
  idMenuWidget_Shell_SaveInfo() : loadIndex(0), forSaveScreen(false) {}
  virtual void Update();
  virtual void ObserveEvent(const idMenuWidget& widget,
                            const idWidgetEvent& event);
...
class idMenuWidget_PDA_AudioFiles : public idMenuWidget {
 public:
  idMenuWidget_PDA_AudioFiles() : pdaIndex(0) {}
 ...
 private:
  int pdaIndex;
  idList<idList<idStr, TAG_IDLIB_LIST_MENU>, TAG_IDLIB_LIST_MENU>
      audioFileNames;
};
...
class idMenuWidget_PDA_EmailInbox : public idMenuWidget {
 public:
  idMenuWidget_PDA_EmailInbox()
      : emailList(NULL), scrollbar(NULL), pdaIndex(0) {}
  virtual void Update();
  virtual void Initialize(idMenuHandler* data);
  virtual void ObserveEvent(const idMenuWidget& widget,
                            const idWidgetEvent& event);
...
class idMenuWidget_ItemAssignment : public idMenuWidget {
 public:
  idMenuWidget_ItemAssignment() : slotIndex(0) {}
...
class idWidgetActionHandler : public idSWFScriptFunction_RefCounted {
 public:
  idWidgetActionHandler(idMenuWidget* widget, actionHandler_t actionEventType,
                        widgetEvent_t _event)
  ...
  idSWFScriptVar Call(idSWFScriptObject* thisObject,
    ...
    switch (type) {
      case WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER: {
        action.Set((widgetAction_t)WIDGET_ACTION_START_REPEATER,
                   WIDGET_ACTION_SCROLL_VERTICAL, 1);
        handled = true;
        break;
      }
      ...
      case WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER_VARIABLE: {
        action.Set((widgetAction_t)WIDGET_ACTION_START_REPEATER,
                   WIDGET_ACTION_SCROLL_VERTICAL_VARIABLE, 1);
        handled = true;
        break;
      }
      ...
      case WIDGET_ACTION_EVENT_SCROLL_LEFT_START_REPEATER: {
        action.Set((widgetAction_t)WIDGET_ACTION_START_REPEATER,
                   WIDGET_ACTION_SCROLL_HORIZONTAL, -1);
        handled = true;
        break;
      }
    ...
    if (handled) {
      targetWidget->HandleAction(
          action, idWidgetEvent(targetEvent, 0, thisObject, parms),
          targetWidget);
    }
