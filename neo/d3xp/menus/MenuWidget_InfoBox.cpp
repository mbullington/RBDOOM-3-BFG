...
void idMenuWidget_InfoBox::Update() {
  ...
  idSWFTextInstance* txtHeading = GetSprite()->GetScriptObject()->GetNestedText(
      "info", "heading", "txtVal");
  idSWFTextInstance* txtBody =
      GetSprite()->GetScriptObject()->GetNestedText("info", "txtBody");
  ...
  idSWFScriptObject* info =
      GetSprite()->GetScriptObject()->GetNestedObj("info");
  if (info != NULL) {
    info->Set("onRollOver", new (TAG_SWF) WrapWidgetSWFEvent(
...
void idMenuWidget_InfoBox::Scroll(int d) {
  ...
  if (txtBody != NULL && txtBody->scroll + d >= 0 &&
      txtBody->scroll + d <= txtBody->maxscroll) {
    txtBody->scroll += d;
  }
...
========================
*/
bool idMenuWidget_InfoBox::HandleAction(idWidgetAction& action,
...
========================
*/
void idMenuWidget_InfoBox::ObserveEvent(const idMenuWidget& widget,
                                        const idWidgetEvent& event) {
  switch (event.type) {
