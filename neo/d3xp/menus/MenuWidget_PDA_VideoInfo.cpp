...
void idMenuWidget_PDA_VideoInfo::Update() {
  ...
  idSWFTextInstance* txtHeading =
      GetSprite()->GetScriptObject()->GetNestedText("txtName");
  idSWFTextInstance* txtInfo =
      GetSprite()->GetScriptObject()->GetNestedText("txtInfo");
...
void idMenuWidget_PDA_VideoInfo::ObserveEvent(const idMenuWidget& widget,
    ...
    case WIDGET_EVENT_FOCUS_ON: {
      ...
      if (player != NULL) {
        ...
        if (video != NULL) {
          idSWFSpriteInstance* videoSprite =
              GetSprite()->GetScriptObject()->GetNestedSprite("video", "img");
          if (videoSprite != NULL) {
      ...
      if (GetParent() != NULL) {
        idMenuScreen_PDA_VideoDisks* screen =
            dynamic_cast<idMenuScreen_PDA_VideoDisks* const>(GetParent());
        if (screen != NULL) {
          screen->Update();
        }
      }
