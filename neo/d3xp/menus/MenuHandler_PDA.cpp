...
void idMenuHandler_PDA::Update() {
  ...
  if (activeScreen != nextScreen) {
    if (nextScreen == PDA_AREA_INVALID) {
      ...
      idMenuWidget_CommandBar* cmdBar = dynamic_cast<idMenuWidget_CommandBar*>(
          GetChildFromIndex(PDA_WIDGET_CMD_BAR));
      if (cmdBar != NULL) {
        cmdBar->ClearAllButtons();
        cmdBar->Update();
      }

      idSWFSpriteInstance* menu =
          gui->GetRootObject().GetNestedSprite("navBar");
      idSWFSpriteInstance* bg =
          gui->GetRootObject().GetNestedSprite("background");
      ...
      if (activeScreen > PDA_AREA_INVALID && activeScreen < PDA_NUM_AREAS &&
          menuScreens[activeScreen] != NULL) {
        menuScreens[activeScreen]->HideScreen(
            static_cast<mainMenuTransition_t>(transition));
      }

      if (nextScreen > PDA_AREA_INVALID && nextScreen < PDA_NUM_AREAS &&
...
void idMenuHandler_PDA::ActivateMenu(bool show) {
  ...
  if (show) {
    ...
    idMenuWidget_DynamicList* pdaList = dynamic_cast<idMenuWidget_DynamicList*>(
        GetChildFromIndex(PDA_WIDGET_PDA_LIST));
    if (pdaList != NULL) {
    ...
    idMenuWidget_NavBar* navBar = dynamic_cast<idMenuWidget_NavBar*>(
        GetChildFromIndex(PDA_WIDGET_NAV_BAR));
    if (navBar != NULL) {
...
#define BIND_PDA_SCREEN(screenId, className, menuHandler) \
  ...
  while (pdaList.GetChildren().Num() < MAX_PDA_ITEMS) {
    ...
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_PDA_SELECT_USER, pdaList.GetChildren().Num());
    buttonWidget->Initialize(this);
    if (menuScreens[PDA_AREA_USER_DATA] != NULL) {
      idMenuScreen_PDA_UserData* userDataScreen =
          dynamic_cast<idMenuScreen_PDA_UserData*>(
              menuScreens[PDA_AREA_USER_DATA]);
    ...
    if (menuScreens[PDA_AREA_USER_EMAIL] != NULL) {
      idMenuScreen_PDA_UserEmails* userEmailScreen =
          dynamic_cast<idMenuScreen_PDA_UserEmails*>(
              menuScreens[PDA_AREA_USER_EMAIL]);
  ...
  for (int count = 0; count < (MAX_NAV_OPTIONS * 2 - 1); ++count) {
    idMenuWidget_NavButton* const navButton =
        new (TAG_SWF) idMenuWidget_NavButton();

    if (count < MAX_NAV_OPTIONS - 1) {
      navButton->AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_PDA_SELECT_NAV, count);
    } else if (count < ((MAX_NAV_OPTIONS - 1) * 2)) {
      int index = (count - (MAX_NAV_OPTIONS - 1)) + 1;
      navButton->AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_PDA_SELECT_NAV, index);
    } else {
      navButton->AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_PDA_SELECT_NAV, -1);
    }
  ...
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN_LSTICK));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
          WIDGET_EVENT_SCROLL_UP_LSTICK));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_STOP_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN_LSTICK_RELEASE));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_STOP_REPEATER,
          WIDGET_EVENT_SCROLL_UP_LSTICK_RELEASE));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_DOWN)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_UP)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
          WIDGET_EVENT_SCROLL_UP));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RELEASE)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_STOP_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN_RELEASE));
  pdaList.AddEventAction(WIDGET_EVENT_SCROLL_UP_RELEASE)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          &pdaList, WIDGET_ACTION_EVENT_STOP_REPEATER,
...
========================
*/
bool idMenuHandler_PDA::HandleAction(idWidgetAction& action,
  ...
  switch (actionType) {
    case WIDGET_ACTION_PDA_SELECT_USER: {
      ...
      idMenuWidget_DynamicList* pdaList =
          dynamic_cast<idMenuWidget_DynamicList*>(
              GetChildFromIndex(PDA_WIDGET_PDA_LIST));
      if (pdaList != NULL) {
    ...
    case WIDGET_ACTION_PDA_SELECT_NAV: {
      ...
      if (index == -1 && activeScreen == PDA_AREA_USER_EMAIL) {
        idMenuScreen_PDA_UserEmails* screen =
            dynamic_cast<idMenuScreen_PDA_UserEmails* const>(
                menuScreens[PDA_AREA_USER_EMAIL]);
        if (screen) {
      ...
      if (activeScreen == PDA_AREA_USER_DATA) {
        ...
        if (change) {
          if (widget->GetParent() != NULL) {
            idMenuWidget_DynamicList* audioList =
                dynamic_cast<idMenuWidget_DynamicList*>(widget->GetParent());
            int index = parms[0].ToInteger();
            if (audioList != NULL) {
    ...
    case WIDGET_ACTION_SELECT_PDA_VIDEO: {
      if (activeScreen == PDA_AREA_VIDEO_DISKS) {
        ...
        if (menuScreens[PDA_AREA_VIDEO_DISKS] != NULL) {
          idMenuScreen_PDA_VideoDisks* screen =
              dynamic_cast<idMenuScreen_PDA_VideoDisks* const>(
                  menuScreens[PDA_AREA_VIDEO_DISKS]);
          if (screen != NULL) {
...
void idMenuHandler_PDA::UpdateAudioLogPlaying(bool playing) {
  if (playing != audioLogPlaying && activeScreen == PDA_AREA_USER_DATA &&
      menuScreens[activeScreen] != NULL) {
