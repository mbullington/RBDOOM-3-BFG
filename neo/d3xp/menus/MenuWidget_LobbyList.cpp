...
void idMenuWidget_LobbyList::Update() {
  ...
  for (int i = 0; i < headings.Num(); ++i) {
    idSWFTextInstance* txtHeading =
        GetSprite()->GetScriptObject()->GetNestedText(va("heading%d", i));
    if (txtHeading != NULL) {
...
bool idMenuWidget_LobbyList::PrepareListElement(idMenuWidget& widget,
  ...
  idMenuWidget_LobbyButton* const button =
...
========================
*/
void idMenuWidget_LobbyList::SetEntryData(int index, idStr name,
  ...
  idMenuWidget_LobbyButton* const button =
      dynamic_cast<idMenuWidget_LobbyButton*>(&child);
