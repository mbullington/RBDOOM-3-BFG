...
void idMenuScreen_Shell_Save::Initialize(idMenuHandler* data) {
  ...
  btnDelete->SetLabel("");
  btnDelete->AddEventAction(WIDGET_EVENT_PRESS)
...
void idMenuScreen_Shell_Save::UpdateSaveEnumerations() {
  ...
  if (session->GetSaveGameManager().IsWorking()) {
    ...
    if (saveGameInfo.Num() == MAX_SAVEGAMES ||
        (!hasAutosave && saveGameInfo.Num() == MAX_SAVEGAMES - 1)) {
      newSaveOffset = 0;
    }
  ...
  if (menuData != NULL) {
    ...
    if (cmdBar != NULL) {
      ...
      if (!session->GetSaveGameManager().IsWorking()) {
        ...
        if (options != NULL) {
          if (options->GetViewIndex() != 0 ||
              (options->GetViewIndex() == 0 && newSaveOffset == 0)) {
            buttonInfo =
                cmdBar->GetButton(idMenuWidget_CommandBar::BUTTON_JOY3);
            if (menuData->GetPlatform() != 2) {
  ...
  if (options != NULL && options->GetTotalNumberOfOptions() > 0 &&
      options->GetViewIndex() >= options->GetTotalNumberOfOptions()) {
...
========================
*/
void idMenuScreen_Shell_Save::ShowScreen(
...
========================
*/
void idMenuScreen_Shell_Save::HideScreen(
    const mainMenuTransition_t transitionType) {
...
void idMenuScreen_Shell_Save::SaveGame(int index) {
  ...
  if (saveGameInfo.Num() == MAX_SAVEGAMES ||
      ((saveGameInfo.Num() == MAX_SAVEGAMES - 1) && !hasAutosave)) {
    newSaveOffset = 0;
  }
  ...
  if (index == 0 && newSaveOffset == 1) {
    ...
    for (int slot = 0; slot < saveGameInfo.Num(); ++slot) {
      ...
      name.ToLower();  // PS3 saves are uppercase ... we need to lower case-ify
                       // them for comparison here
    ...
    class idSWFScriptFunction_OverwriteSave
        : public idSWFScriptFunction_RefCounted {
     public:
      idSWFScriptFunction_OverwriteSave(gameDialogMessages_t _msg, bool _accept,
                                        int _index,
                                        idMenuScreen_Shell_Save* _screen) {
        msg = _msg;
        accept = _accept;
        index = _index;
        screen = _screen;
    ...
    common->Dialog().AddDialog(GDM_OVERWRITE_SAVE, DIALOG_ACCEPT_CANCEL,
                               new idSWFScriptFunction_OverwriteSave(
                                   GDM_OVERWRITE_SAVE, true, index, this),
                               new idSWFScriptFunction_OverwriteSave(
                                   GDM_OVERWRITE_SAVE, false, index, this),
                               false);
  }
}
...
void idMenuScreen_Shell_Save::DeleteGame(int index) {
  class idSWFScriptFunction_DeleteGame : public idSWFScriptFunction_RefCounted {
   public:
    idSWFScriptFunction_DeleteGame(gameDialogMessages_t _msg, bool _accept,
                                   int _index,
                                   idMenuScreen_Shell_Save* _screen) {
      msg = _msg;
      accept = _accept;
  ...
  bool hasAutosave = false;
  const saveGameDetailsList_t& saveInfo =
      session->GetSaveGameManager().GetEnumeratedSavegames();
  ...
  if ((saveInfo.Num() < MAX_SAVEGAMES - 1) ||
      ((saveInfo.Num() == MAX_SAVEGAMES - 1) && hasAutosave)) {
    index--;  // Subtract 1 from the index for 'New Game'
  }

  common->Dialog().AddDialog(
      GDM_DELETE_SAVE, DIALOG_ACCEPT_CANCEL,
...
========================
*/
bool idMenuScreen_Shell_Save::HandleAction(idWidgetAction& action,
