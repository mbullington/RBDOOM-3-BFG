...
========================
*/
idMenuWidget::~idMenuWidget() { Cleanup(); }
...
bool idMenuWidget::ExecuteEvent(const idWidgetEvent& event) {
  idList<idWidgetAction, TAG_IDLIB_LIST_MENU>* const actions =
...
========================
*/
void idMenuWidget::SetSpritePath(const char* arg1, const char* arg2,
...
========================
*/
void idMenuWidget::SetSpritePath(const idList<idStr>& spritePath_,
...
void idMenuWidget::Show() {
  ...
  if (currentFrame == findFrame ||
      (currentFrame > 1 && currentFrame <= idleFrame)) {
    return;
  }
...
========================
*/
void idMenuWidget::SetDataSource(idMenuDataSource* dataSource_,
...
void idMenuWidget::SetFocusIndex(const int index, bool skipSound) {
  ...
  assert(index >= 0 &&
         index < GetChildren().Num());  //&& oldIndex >= 0 && oldIndex <
                                        //GetChildren().Num() );
  ...
  if (oldIndex != index && oldIndex >= 0 && oldIndex < GetChildren().Num() &&
      GetChildByIndex(oldIndex).GetState() != WIDGET_STATE_HIDDEN) {
    GetChildByIndex(oldIndex).ReceiveEvent(
        idWidgetEvent(WIDGET_EVENT_FOCUS_OFF, 0, NULL, parms));
  }

  // assert( GetChildByIndex( index ).GetState() != WIDGET_STATE_HIDDEN );
  GetChildByIndex(index).ReceiveEvent(
      idWidgetEvent(WIDGET_EVENT_FOCUS_ON, 0, NULL, parms));
}
    ...
    if (state == WIDGET_STATE_HIDDEN) {
      ...
      if (state == WIDGET_STATE_DISABLED) {
        ...
        if (widgetState == WIDGET_STATE_NORMAL) {
          GetSprite()->PlayFrame(
              "selecting");  // transition from unselected to selected
        } else {
        ...
        if (widgetState == WIDGET_STATE_SELECTING) {
          GetSprite()->PlayFrame(
              "unselecting");  // transition from selected to unselected
        } else if (widgetState != WIDGET_STATE_HIDDEN &&
                   widgetState != WIDGET_STATE_NORMAL) {
...
========================
*/
idList<idWidgetAction, TAG_IDLIB_LIST_MENU>* idMenuWidget::GetEventActions(
