...
void idMenuScreen_HUD::ShowScreen(const mainMenuTransition_t transitionType) {
  ...
  newPDAHeading =
      root.GetNestedText("_center", "pdaDownload", "info", "txtHeading");
  newPDA = root.GetNestedSprite("_bottomLeft", "newPDA");

  // Video Download
  newVideoDownload = root.GetNestedSprite("_center", "videoDownload");
  newVideoHeading =
      root.GetNestedText("_center", "videoDownload", "info", "txtHeading");
  ...
  // If the player loaded a save with enough souls to use the cube, the icon
  // wouldn't show.  We're setting this flag in idPlayer::Restore so we can show
  // the cube after loading a game
  ...
  class idTriggerNewPDAOrVideo : public idSWFScriptFunction_RefCounted {
   public:
    idTriggerNewPDAOrVideo(idMenuScreen_HUD* _screen) : screen(_screen) {}
    ...
    if (common->IsMultiplayer()) {
      ...
      if (!max_stamina) {
        ...
        float staminaPercent =
            idMath::Ftoi(100.0f * player->stamina / max_stamina);
...
void idMenuScreen_HUD::UpdateWeaponInfo(idPlayer* player) {
  ...
  if (player->weapon_bloodstone >= 0) {
    ammo_t ammo_i = player->inventory.AmmoIndexForWeaponClass(
  ...
  if (ammoAmount == -1 ||
      player->GetCurrentWeaponSlot() == player->weapon_bloodstone ||
      player->GetCurrentWeaponSlot() == player->weapon_soulcube) {
    ...
    if (!weapon.GetEntity()->IsReady()) {
      ...
      playerAmmo = weapon.GetEntity()->ClipSize()
                       ? va("%i", inClip)
                       : "--";  // how much in the current clip
      playerClip = weapon.GetEntity()->ClipSize()
    ...
    bool clipLow = (weapon.GetEntity()->ClipSize()
    ...
    if (showClip) {
      ...
      idSWFSpriteInstance* txtClipSprite =
          ammoInfo->GetScriptObject()->GetNestedSprite("info", "clip");
      idSWFSpriteInstance* clipLowSprite =
          ammoInfo->GetScriptObject()->GetNestedSprite("info", "lowAmmo");
      idSWFSpriteInstance* clipEmptySprite =
          ammoInfo->GetScriptObject()->GetNestedSprite("info", "clipEmpty");
      idSWFSpriteInstance* ammoEmptySprite =
          ammoInfo->GetScriptObject()->GetNestedSprite("info", "noAmmo");
      idSWFSpriteInstance* txtAmmoSprite =
          ammoInfo->GetScriptObject()->GetNestedSprite("info", "ammoCount");

      idSWFTextInstance* txtClip = ammoInfo->GetScriptObject()->GetNestedText(
          "info", "clip", "clipCount", "txtVal");
      idSWFTextInstance* txtAmmo = ammoInfo->GetScriptObject()->GetNestedText(
          "info", "ammoCount", "txtVal");
      ...
      idSWFTextInstance* txtAmmo =
          ammoInfo->GetScriptObject()->GetNestedText("info", "txtVal");
  ...
  if (weap != NULL && *weap != '\0') {
    ...
    if (weaponDef != NULL) {
      ...
      if (newWeapon != NULL) {
        ...
        idSWFSpriteInstance* topImg =
            newWeapon->GetScriptObject()->GetNestedSprite("topImg");
        idSWFSpriteInstance* botImg =
            newWeapon->GetScriptObject()->GetNestedSprite("botImg");
...
void idMenuScreen_HUD::UpdatePickupInfo(int index, const idStr& name) {
  ...
  idSWFTextInstance* txtItem = pickupInfo->GetScriptObject()->GetNestedText(
      va("item%d", index), "txtVal");
  if (txtItem != NULL) {
...
========================
*/
void idMenuScreen_HUD::SetCursorState(idPlayer* player, cursorState_t state,
                                      int set) {
  switch (state) {
    case CURSOR_TALK: {
      cursorNone = 0;
    ...
    if (cursorTalking) {
      ...
      if (cursorState != CURSOR_TALK) {
        ...
        if (talkCursor) {
          ...
          idSWFSpriteInstance* topBacking =
              talkCursor->GetScriptObject()->GetNestedSprite("backing",
                                                             "topBar");
          idSWFSpriteInstance* bottomBacking =
              talkCursor->GetScriptObject()->GetNestedSprite("backing",
                                                             "botBar");

          idSWFTextInstance* txtAction =
              talkCursor->GetScriptObject()->GetNestedText("info", "txtAction");
          idSWFTextInstance* txtFocus =
              talkCursor->GetScriptObject()->GetNestedText("info", "txtFocus");

          idSWFTextInstance* txtPrompt =
              talkCursor->GetScriptObject()->GetNestedText("talkPrompt",
                                                           "txtPrompt");
          ...
          if (txtAction) {
            if (!in_useJoystick.GetBool()) {
              ...
              keyBindings_t bind =
                  idKeyInput::KeyBindingsFromBinding("_use", true);
...
void idMenuScreen_HUD::ShowRespawnMessage(bool show) {
  ...
  if (show) {
    ...
    idSWFTextInstance* message =
        respawnMessage->GetScriptObject()->GetNestedText("info", "txtMessage");
    if (message != NULL) {
...
========================
*/
void idMenuScreen_HUD::UpdateWeaponStates(idPlayer* player,
                                          bool weaponChanged) {
  if (!weaponPills) {
    return;
  }
  ...
  if (common->IsMultiplayer()) {
    ...
    if (weaponChanged) {
      ...
      for (int i = -2; i < 3; ++i) {
        ...
        idSWFSpriteInstance* topValid = mpWeapons->GetNestedSprite(
            "list", va("weapon%i", index), "topValid");
        idSWFSpriteInstance* botValid = mpWeapons->GetNestedSprite(
            "list", va("weapon%i", index), "botValid");
        idSWFSpriteInstance* topInvalid = mpWeapons->GetNestedSprite(
            "list", va("weapon%i", index), "topInvalid");
        idSWFSpriteInstance* botInvalid = mpWeapons->GetNestedSprite(
            "list", va("weapon%i", index), "botInvalid");
        ...
        if (weap != NULL && *weap != '\0') {
          const idDeclEntityDef* weaponDef =
              gameLocal.FindEntityDef(weap, false);
          if (weaponDef != NULL) {
            hudIcon = declManager->FindMaterial(
                weaponDef->dict.GetString("hudIcon"), false);
            if (i == 0) {
    ...
    if (weaponImg) {
      if (weaponChanged && hudIcon != NULL) {
        ...
        idSWFSpriteInstance* topImg =
            weaponImg->GetScriptObject()->GetNestedSprite("topImg");
        idSWFSpriteInstance* botImg =
            weaponImg->GetScriptObject()->GetNestedSprite("botImg");
        ...
        /*if ( weaponName && weaponName->GetSprite() ) {
                ...
                idSWFTextInstance * txtVal = weaponName->GetNestedText( "info",
        "txtVal" ); if ( txtVal != NULL ) { txtVal->SetText( displayName );
...
void idMenuScreen_HUD::UpdateLocation(idPlayer* player) {
  ...
  idLocationEntity* locationEntity =
      gameLocal.LocationForPoint(playertoLoc->GetEyePosition());
  if (locationEntity) {
...
void idMenuScreen_HUD::DownloadPDA(const idDeclPDA* pda, bool newSecurity) {
  if (newPDADownload) {
    ...
    newPDAName =
        newPDADownload->GetScriptObject()->GetNestedText("info", "txtName");
    newPDAHeading =
        newPDADownload->GetScriptObject()->GetNestedText("info", "txtHeading");
...
void idMenuScreen_HUD::DownloadVideo() {
  if (newVideoDownload) {
    ...
    newVideoHeading = newVideoDownload->GetScriptObject()->GetNestedText(
        "info", "txtHeading");
...
void idMenuScreen_HUD::UpdateAudioLog(bool show) {
  ...
  if (show && !audioLog->IsVisible()) {
    ...
    for (int index = 0; index < 13; ++index) {
      idSWFSpriteInstance* node = audioLog->GetScriptObject()->GetNestedSprite(
          "bar", va("node%d", index));
      if (node != NULL) {
...
void idMenuScreen_HUD::UpdateCommunication(bool show, idPlayer* player) {
  ...
  if (show && !communication->IsVisible()) {
    ...
    for (int index = 0; index < 16; ++index) {
      idSWFSpriteInstance* node =
          communication->GetScriptObject()->GetNestedSprite(
              "info", "bar", va("node%d", index));
      if (node != NULL) {
        int frame = gameLocal.random.RandomInt(100);
        node->SetScale(100.0f, frame);
        float toFrame = gameLocal.random.RandomFloat();
...
void idMenuScreen_HUD::UpdateOxygen(bool show, int val) {
  ...
  if (show) {
    ...
    idSWFSpriteInstance* info =
        oxygen->GetScriptObject()->GetNestedSprite("info");
    if (info != NULL) {
      info->StopFrame(val + 1);
    }

    idSWFSpriteInstance* goodFrame =
        oxygen->GetScriptObject()->GetNestedSprite("goodFrame");
    idSWFSpriteInstance* badFrame =
        oxygen->GetScriptObject()->GetNestedSprite("badFrame");
    ...
    idSWFTextInstance* txtVal =
        oxygen->GetScriptObject()->GetNestedText("info", "txtHeading");
    if (txtVal != NULL) {
...
========================
*/
void idMenuScreen_HUD::SetupObjective(const idStr& title, const idStr& desc,
...
void idMenuScreen_HUD::ShowObjective(bool complete) {
  if (complete) {
    ...
    idSWFTextInstance* txtComplete =
        objectiveComplete->GetScriptObject()->GetNestedText("info",
                                                            "txtComplete");
    idSWFTextInstance* txtTitle =
        objectiveComplete->GetScriptObject()->GetNestedText("info", "txtTitle");
    idSWFSpriteInstance* rightArrow =
        objectiveComplete->GetScriptObject()->GetNestedSprite("info",
                                                              "right_arrows");
    ...
    idSWFTextInstance* txtNew =
        objective->GetScriptObject()->GetNestedText("info", "txtComplete");
    idSWFTextInstance* txtTitle =
        objective->GetScriptObject()->GetNestedText("info", "txtTitle");
    idSWFTextInstance* txtDesc =
        objective->GetScriptObject()->GetNestedText("info", "txtDesc");
    idSWFSpriteInstance* img =
        objective->GetScriptObject()->GetNestedSprite("info", "img");
    idSWFSpriteInstance* rightArrow =
        objective->GetScriptObject()->GetNestedSprite("info", "right_arrows");
...
void idMenuScreen_HUD::ToggleMPInfo(bool show, bool showTeams, bool isCTF) {
  ...
  if (show) {
    ...
    idSWFSpriteInstance* redTeam =
        mpInfo->GetScriptObject()->GetNestedSprite("redTeam");
    idSWFSpriteInstance* blueTeam =
        mpInfo->GetScriptObject()->GetNestedSprite("blueTeam");
    idSWFSpriteInstance* redFlag =
        mpInfo->GetScriptObject()->GetNestedSprite("redFlag");
    idSWFSpriteInstance* blueFlag =
        mpInfo->GetScriptObject()->GetNestedSprite("blueFlag");
...
void idMenuScreen_HUD::SetTeamScore(int team, int score) {
  ...
  if (team == 0) {
    txtScore =
        mpInfo->GetScriptObject()->GetNestedText("redTeam", "txtRedScore");
  } else if (team == 1) {
    txtScore =
        mpInfo->GetScriptObject()->GetNestedText("blueTeam", "txtBlueScore");
  }
...
void idMenuScreen_HUD::SetTeam(int team) {
  ...
  idSWFSpriteInstance* teamBacking =
      mpInfo->GetScriptObject()->GetNestedSprite("teamBack");
...
========================
*/
void idMenuScreen_HUD::TriggerHitTarget(bool show, const idStr& target,
                                        int color) {
  if (!mpHitInfo) {
    return;
  }
  ...
  if (show) {
    ...
    idSWFSpriteInstance* backing =
        mpHitInfo->GetScriptObject()->GetNestedSprite("bgColor");
    if (backing) {
...
void idMenuScreen_HUD::ShowNewItem(const char* name, const char* icon) {
  ...
  idSWFSpriteInstance* topImg =
      newItem->GetScriptObject()->GetNestedSprite("info", "topImg");
  idSWFSpriteInstance* botImg =
      newItem->GetScriptObject()->GetNestedSprite("info", "botImg");
  idSWFTextInstance* heading =
      newItem->GetScriptObject()->GetNestedText("info", "txtTitle");
  idSWFTextInstance* itemName =
      newItem->GetScriptObject()->GetNestedText("info", "txtItem");
...
void idMenuScreen_HUD::UpdateFlashlight(idPlayer* player) {
  ...
  if (player->flashlightBattery != flashlight_batteryDrainTimeMS.GetInteger()) {
    ...
    idSWFSpriteInstance* batteryLife =
        flashlight->GetScriptObject()->GetNestedSprite("info");
    if (batteryLife) {
      float power = ((float)player->flashlightBattery /
                     (float)flashlight_batteryDrainTimeMS.GetInteger()) *
                    100.0f;
...
void idMenuScreen_HUD::UpdateChattingHud(idPlayer* player) {
  ...
  if (player->isChatting == 0) {
    ...
    if (!mpChatObject->IsVisible()) {
      ...
      idSWFTextInstance* txtType =
          mpChatObject->GetScriptObject()->GetNestedText("info", "saybox");
      int length = 0;
      if (txtType) {
      ...
      idSWFSpriteInstance* sayBox =
          mpChatObject->GetScriptObject()->GetNestedSprite("info", "textEntry");
      if (sayBox) {
        sayBox->SetXPos(length + 10);
      }

      idSWFTextInstance* say = mpChatObject->GetScriptObject()->GetNestedText(
          "info", "textEntry", "txtVal");
      if (say != NULL) {
      ...
      idSWFScriptObject* const sayObj =
          mpChatObject->GetScriptObject()->GetNestedObj("info", "textEntry",
                                                        "txtVal");
      if (sayObj != NULL) {
        ...
        class idCancelTextChat : public idSWFScriptFunction_RefCounted {
         public:
          idCancelTextChat(idPlayer* _player) { player = _player; }
          idSWFScriptVar Call(idSWFScriptObject* thisObject,
                              const idSWFParmList& parms) {
            if (!player) {
              return idSWFScriptVar();
        ...
        idSWFScriptObject* const shortcutKeys =
