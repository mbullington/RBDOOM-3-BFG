...
// TRC requires a maximum interoccular distance of 6.5cm even though human
// adults can easily have an interoccular distance of over 7.5cm
...
static const char* stereoRender_enable_text[] = {
    ...
    "#str_swf_stereo_quad"};
static const int NUM_STEREO_ENABLE =
    sizeof(stereoRender_enable_text) / sizeof(stereoRender_enable_text[0]);
...
void idMenuScreen_Shell_Stereoscopics::Initialize(idMenuHandler* data) {
  ...
  control->SetDataSource(&stereoData,
                         idMenuDataSource_StereoSettings::STEREO_FIELD_ENABLE);
  ...
  control->SetDataSource(
      &stereoData, idMenuDataSource_StereoSettings::STEREO_FIELD_SEPERATION);
  ...
  control->SetDataSource(
      &stereoData, idMenuDataSource_StereoSettings::STEREO_FIELD_SWAP_EYES);
  control->SetupEvents(DEFAULT_REPEAT_TIME, options->GetChildren().Num());
  control->AddEventAction(WIDGET_EVENT_PRESS)
      .Set(WIDGET_ACTION_PRESS_FOCUSED, options->GetChildren().Num());
  options->AddChild(control);
...
void idMenuScreen_Shell_Stereoscopics::Update() {
  ...
  if (BindSprite(root)) {
    ...
    if (heading != NULL) {
      heading->SetText(
          "#str_swf_stereoscopics_heading");  // STEREOSCOPIC RENDERING
...
========================
*/
void idMenuScreen_Shell_Stereoscopics::ShowScreen(
    const mainMenuTransition_t transitionType) {
  stereoData.LoadData();
  idMenuScreen::ShowScreen(transitionType);

  if (GetSprite() != NULL) {
    idSWFSpriteInstance* leftEye =
        GetSprite()->GetScriptObject()->GetNestedSprite("info", "leftEye");
    idSWFSpriteInstance* rightEye =
        GetSprite()->GetScriptObject()->GetNestedSprite("info", "rightEye");
...
========================
*/
void idMenuScreen_Shell_Stereoscopics::HideScreen(
    const mainMenuTransition_t transitionType) {
...
========================
*/
bool idMenuScreen_Shell_Stereoscopics::HandleAction(idWidgetAction& action,
...
void idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::
    LoadData() {
  fields[STEREO_FIELD_ENABLE].SetInteger(
      renderSystem->GetStereoScopicRenderingMode());

  fields[STEREO_FIELD_SEPERATION].SetFloat(
      100.0f * (stereoRender_interOccularCentimeters.GetFloat() /
                MAX_INTEROCCULAR_DISTANCE));
...
void idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::
  ...
  if (fieldIndex == STEREO_FIELD_ENABLE) {
    ...
    stereoRender_interOccularCentimeters.SetFloat(
        (fields[STEREO_FIELD_SEPERATION].ToFloat() / 100.0f) *
        MAX_INTEROCCULAR_DISTANCE);
  }

  // do this so we don't save every time we modify a setting.  Only save once
...
*/
idSWFScriptVar
idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::GetField(
    const int fieldIndex) const {
  if (fieldIndex == STEREO_FIELD_ENABLE) {
    return idSWFScriptVar(
        stereoRender_enable_text[fields[fieldIndex].ToInteger()]);
  }
...
========================
*/
bool idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::
    IsDataChanged() const {
  if (fields[STEREO_FIELD_SWAP_EYES].ToBool() !=
      originalFields[STEREO_FIELD_SWAP_EYES].ToBool()) {
    return true;
  }

  if (fields[STEREO_FIELD_ENABLE].ToInteger() !=
      originalFields[STEREO_FIELD_ENABLE].ToInteger()) {
    return true;
  }

  if (fields[STEREO_FIELD_SEPERATION].ToFloat() !=
      originalFields[STEREO_FIELD_SEPERATION].ToFloat()) {
    return true;
  }
...
bool idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::
  ...
  if (fields[STEREO_FIELD_ENABLE].ToInteger() !=
      originalFields[STEREO_FIELD_ENABLE].ToInteger()) {
    if (fields[STEREO_FIELD_ENABLE].ToInteger() == STEREO3D_QUAD_BUFFER ||
        originalFields[STEREO_FIELD_ENABLE].ToInteger() ==
