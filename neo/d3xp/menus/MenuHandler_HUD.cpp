...
void idMenuHandler_HUD::Update() {
  ...
  if (nextScreen != activeScreen) {
    if (activeScreen > HUD_AREA_INVALID && activeScreen < HUD_NUM_AREAS &&
        menuScreens[activeScreen] != NULL) {
      menuScreens[activeScreen]->HideScreen(
          static_cast<mainMenuTransition_t>(transition));
    }

    if (nextScreen > HUD_AREA_INVALID && nextScreen < HUD_NUM_AREAS &&
...
idMenuScreen_HUD* idMenuHandler_HUD::GetHud() {
  idMenuScreen_HUD* screen =
      dynamic_cast<idMenuScreen_HUD*>(menuScreens[HUD_AREA_PLAYING]);
  return screen;
}
...
========================
*/
void idMenuHandler_HUD::ShowTip(const char* title, const char* tip,
                                bool autoHide) {
  // SRS - Changed to assign autoHide to autoHideTip vs. assign autoHideTip to
  // itself
