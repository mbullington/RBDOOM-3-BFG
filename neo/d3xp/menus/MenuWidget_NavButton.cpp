...
void idMenuWidget_NavButton::Update() {
  ...
  if (textObj != NULL) {
    textObj->Set("onPress",
                 new (TAG_SWF) WrapWidgetSWFEvent(this, WIDGET_EVENT_PRESS, 0));
    textObj->Set("onRelease", new (TAG_SWF) WrapWidgetSWFEvent(
    ...
    hitBox->Set("onRollOut", new (TAG_SWF) WrapWidgetSWFEvent(
...
void idMenuWidget_MenuButton::Update() {
  ...
  if (text != NULL) {
    ...
    idSWFSpriteInstance* hoverBar =
        spriteObject->GetNestedSprite("hover", "bar");
