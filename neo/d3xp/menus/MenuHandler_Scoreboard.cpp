  ...
  if (nextScreen != activeScreen) {
    if (nextScreen == SCOREBOARD_AREA_INVALID) {
      if (activeScreen > SCOREBOARD_AREA_INVALID &&
          activeScreen < SCOREBOARD_NUM_AREAS &&
      ...
      idMenuWidget_CommandBar* cmdBar = dynamic_cast<idMenuWidget_CommandBar*>(
          GetChildFromIndex(SCOREBOARD_WIDGET_CMD_BAR));
      if (cmdBar != NULL) {
      ...
      if (nextScreen > SCOREBOARD_AREA_INVALID &&
          nextScreen < SCOREBOARD_NUM_AREAS &&
...
========================
*/
void idMenuHandler_Scoreboard::TriggerMenu() { nextScreen = activationScreen; }
...
void idMenuHandler_Scoreboard::ActivateMenu(bool show) {
  ...
  class idSWFScriptFunction_activateMenu
...
========================
*/
void idMenuHandler_Scoreboard::Initialize(const char* swfFile,
                                          idSoundWorld* sw) {
...
#define BIND_SCOREBOARD_SCREEN(screenId, className, menuHandler) \
  ...
  BIND_SCOREBOARD_SCREEN(SCOREBOARD_AREA_DEFAULT, idMenuScreen_Scoreboard,
                         this);
  BIND_SCOREBOARD_SCREEN(SCOREBOARD_AREA_TEAM, idMenuScreen_Scoreboard_Team,
                         this);

  //
  // command bar
  //
  idMenuWidget_CommandBar* const commandBarWidget =
      new (TAG_SWF) idMenuWidget_CommandBar();
...
========================
*/
bool idMenuHandler_Scoreboard::HandleAction(idWidgetAction& action,
...
========================
*/
void idMenuHandler_Scoreboard::AddPlayerInfo(int index,
                                             voiceStateDisplay_t voiceState,
                                             int team, idStr name, int score,
                                             int wins, int ping,
                                             idStr spectateData) {
  ...
  if (spectateData.IsEmpty() ||
      gameLocal.mpGame.GetGameState() == idMultiplayerGame::GAMEREVIEW) {
...
void idMenuHandler_Scoreboard::UpdateSpectating(idStr spectate, idStr follow) {
  if (nextScreen == SCOREBOARD_AREA_DEFAULT) {
    idMenuScreen_Scoreboard* screen = dynamic_cast<idMenuScreen_Scoreboard*>(
        menuScreens[SCOREBOARD_AREA_DEFAULT]);
    if (screen) {
      screen->UpdateSpectating(spectate, follow);
    }
  } else if (nextScreen == SCOREBOARD_AREA_TEAM) {
    idMenuScreen_Scoreboard* screen = dynamic_cast<idMenuScreen_Scoreboard*>(
        menuScreens[SCOREBOARD_AREA_TEAM]);
    if (screen) {
      screen->UpdateSpectating(spectate, follow);
    }
  }
...
========================
*/
void idMenuHandler_Scoreboard::UpdateScoreboard(idList<mpScoreboardInfo>& data,
                                                idStr gameInfo) {
  bool changed = false;
  ...
  blueInfo.Clear();
  for (int i = 0; i < data.Num(); ++i) {
    AddPlayerInfo(data[i].playerNum, data[i].voiceState, data[i].team,
                  data[i].name, data[i].score, data[i].wins, data[i].ping,
                  data[i].spectateData);
  }
  ...
  if (nextScreen == SCOREBOARD_AREA_DEFAULT ||
      activationScreen == SCOREBOARD_AREA_DEFAULT) {
    idMenuScreen_Scoreboard* screen = dynamic_cast<idMenuScreen_Scoreboard*>(
        menuScreens[SCOREBOARD_AREA_DEFAULT]);
    if (screen) {
      screen->SetPlayerData(listItemInfo);
             ...
             activationScreen == SCOREBOARD_AREA_TEAM) {
    idMenuScreen_Scoreboard* screen = dynamic_cast<idMenuScreen_Scoreboard*>(
        menuScreens[SCOREBOARD_AREA_TEAM]);
    if (screen) {
      screen->SetPlayerData(listItemInfo);
