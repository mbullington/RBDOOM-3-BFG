...
void idMenuScreen_Shell_GameLobby::Initialize(idMenuHandler* data) {
  ...
  while (lobby->GetChildren().Num() < 8) {
    idMenuWidget_LobbyButton* const buttonWidget =
        new (TAG_SWF) idMenuWidget_LobbyButton();
    buttonWidget->AddEventAction(WIDGET_EVENT_PRESS)
        .Set(WIDGET_ACTION_SELECT_GAMERTAG, lobby->GetChildren().Num());
    buttonWidget->AddEventAction(WIDGET_EVENT_COMMAND)
        .Set(WIDGET_ACTION_MUTE_PLAYER, lobby->GetChildren().Num());
    buttonWidget->Initialize(data);
  ...
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          lobby, WIDGET_ACTION_EVENT_SCROLL_DOWN_START_REPEATER,
          WIDGET_EVENT_SCROLL_DOWN_RSTICK));
  AddEventAction(WIDGET_EVENT_SCROLL_UP_RSTICK)
      .Set(new (TAG_SWF) idWidgetActionHandler(
          lobby, WIDGET_ACTION_EVENT_SCROLL_UP_START_REPEATER,
          WIDGET_EVENT_SCROLL_UP_RSTICK));
  AddEventAction(WIDGET_EVENT_SCROLL_DOWN_RSTICK_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(lobby, WIDGET_ACTION_EVENT_STOP_REPEATER,
                                     WIDGET_EVENT_SCROLL_DOWN_RSTICK_RELEASE));
  AddEventAction(WIDGET_EVENT_SCROLL_UP_RSTICK_RELEASE)
      .Set(new (TAG_SWF)
               idWidgetActionHandler(lobby, WIDGET_ACTION_EVENT_STOP_REPEATER,
  ...
  if (privateGameLobby && options != NULL) {
    if (session->GetActivePlatformLobbyBase().IsHost() && !isHost) {
      ...
      options->GetChildByIndex(index)
          ...
          .Set(WIDGET_ACTION_COMMAND, GAME_CMD_START, 0);
      buttonWidget =
          dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index));
      ...
      options->GetChildByIndex(index)
          .AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_COMMAND, GAME_CMD_SETTINGS, 1);
      buttonWidget =
          dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index));
      if (buttonWidget != NULL) {
      ...
      options->GetChildByIndex(index)
          .AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_COMMAND, GAME_CMD_TOGGLE_PRIVACY, 2);
      buttonWidget =
          dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index));
      if (buttonWidget != NULL) {
      ...
      options->GetChildByIndex(index)
          .AddEventAction(WIDGET_EVENT_PRESS)
          .Set(WIDGET_ACTION_COMMAND, GAME_CMD_INVITE, 3);
      buttonWidget =
          dynamic_cast<idMenuWidget_Button*>(&options->GetChildByIndex(index));
      if (buttonWidget != NULL) {
      ...
      if (!isPeer) {
        ...
        options->GetChildByIndex(index)
            .AddEventAction(WIDGET_EVENT_PRESS)
            .Set(WIDGET_ACTION_COMMAND, GAME_CMD_INVITE, 0);
        buttonWidget = dynamic_cast<idMenuWidget_Button*>(
            &options->GetChildByIndex(index));
        if (buttonWidget != NULL) {
          buttonWidget->SetDescription("#str_swf_invite_desc");
        }
...
void idMenuScreen_Shell_GameLobby::ShowScreen(
  ...
  if (BindSprite(root)) {
    idSWFSpriteInstance* waitTime =
        GetSprite()->GetScriptObject()->GetNestedSprite("waitTime");
    if (waitTime != NULL) {
...
bool idMenuScreen_Shell_GameLobby::CanKickSelectedPlayer(lobbyUserID_t& luid) {
  ...
  if (session->GetSignInManager().GetMasterLocalUser() ==
      activeLobby.GetLocalUserFromLobbyUser(luid)) {
    return false;
  }
  ...
  switch (actionType) {
    ...
    case WIDGET_ACTION_GO_BACK: {
      ...
      if (activeLobby.GetNumActiveLobbyUsers() > 1) {
        common->Dialog().AddDialog(
            GDM_LEAVE_LOBBY_RET_NEW_PARTY, DIALOG_ACCEPT_CANCEL,
    ...
    case WIDGET_ACTION_COMMAND: {
      ...
      switch (parms[0].ToInteger()) {
        case GAME_CMD_START: {
          idMenuHandler_Shell* handler =
              dynamic_cast<idMenuHandler_Shell* const>(menuData);
          if (handler != NULL) {
        ...
        case GAME_CMD_SETTINGS: {
          menuData->SetNextScreen(SHELL_AREA_MATCH_SETTINGS,
                                  MENU_TRANSITION_SIMPLE);
          break;
        }
        case GAME_CMD_TOGGLE_PRIVACY: {
          idMatchParameters matchParameters = idMatchParameters(
              session->GetActivePlatformLobbyBase().GetMatchParms());
          ...
          // Must update the party parameters too for Xbox JSIP to work in game
          // lobbies.
          idMatchParameters partyParms =
              session->GetPartyLobbyBase().GetMatchParms();
        ...
        case GAME_CMD_INVITE: {
          if (session->GetActivePlatformLobbyBase().IsLobbyFull()) {
            common->Dialog().AddDialog(GDM_CANNOT_INVITE_LOBBY_FULL,
                                       DIALOG_CONTINUE, NULL, NULL, true,
...
void idMenuScreen_Shell_GameLobby::UpdateLobby() {
  ...
  if (!privateGameLobby) {
    ...
    if (session->GetActivePlatformLobbyBase().IsHost()) {
      ...
      if (handler != NULL) {
        ...
        } else if (session->GetActivePlatformLobbyBase().GetNumLobbyUsers() >
                   1) {
    ...
    if (BindSprite(root)) {
      idSWFTextInstance* waitTime =
          GetSprite()->GetScriptObject()->GetNestedText("waitTime", "txtVal");
      if (waitTime != NULL) {
        idStr status;
        if (ms == 1) {
          status =
  ...
  if (session->GetState() == idSession::GAME_LOBBY) {
    ...
    idSWFTextInstance* mapName = GetSprite()->GetScriptObject()->GetNestedText(
        "matchInfo", "txtMapName");
    idSWFTextInstance* modeName = GetSprite()->GetScriptObject()->GetNestedText(
        "matchInfo", "txtModeName");

    if (mapName != NULL) {
      const idList<mpMap_t> maps = common->GetMapList();
      idStr name = idLocalization::GetString(
          maps[idMath::ClampInt(0, maps.Num() - 1, matchParameters.gameMap)]
              .mapName);
    ...
    if (modeName != NULL) {
      ...
      idStr mode = idLocalization::GetString(modes[idMath::ClampInt(
          0, modes.Num() - 1, matchParameters.gameMode)]);
      modeName->SetText(mode);
      modeName->SetStrokeInfo(true);
    }

    idSWFTextInstance* privacy = GetSprite()->GetScriptObject()->GetNestedText(
        "matchInfo", "txtPrivacy");
    if (privacy != NULL) {
    ...
    if (user != NULL && options != NULL) {
      if (user->IsInParty() && user->GetPartyCount() > 1 &&
          !session->IsPlatformPartyInLobby() && menuOptions.Num() > 0) {
