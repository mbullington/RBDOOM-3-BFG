...
idCVar g_projectileDebug("g_projectileDebug", "0", CVAR_BOOL,
                         "Debug projectiles");

// This is used in MP to simulate frames to catchup a projectile's state.
// Similiar to how players work much lighter weight.

// This is used in MP to simulate frames to catchup a projectile's state.
// Similiar to how players work much lighter weight.
idArray<idProjectile::simulatedProjectile_t,
        idProjectile::MAX_SIMULATED_PROJECTILES>
    idProjectile::projectilesToSimulate;
...
================
*/
idProjectile::idProjectile() : launchOrigin(0.0f), launchAxis(mat3_identity) {
  owner = NULL;
  lightDefHandle = -1;
  thrust = 0.0f;
  thrust_end = 0;
...
void idProjectile::Spawn() {
  ...
  mNoExplodeDisappear =
      spawnArgs.GetBool("no_explode_disappear", mNoExplodeDisappear);
...
================
*/
idEntity* idProjectile::GetOwner() const { return owner.GetEntity(); }
...
================
*/
void idProjectile::Create(idEntity* owner, const idVec3& start,
                          const idVec3& dir) {
  idDict args;
  idStr shaderName;
  idVec3 light_color;
...
=================
*/
void idProjectile::Launch(const idVec3& start, const idVec3& dir,
                          const idVec3& pushVelocity, const float timeSinceFire,
                          const float launchPower, const float dmgPower) {
  float fuse;
  float startthrust;
  float endthrust;
  idVec3 velocity;
...
bool idProjectile::Collide(const trace_t& collision, const idVec3& velocity) {
  ...
  if (common->IsMultiplayer() && ent->IsType(idDoor::Type) &&
  ...
  CreateDeltasFromOldOriginAndAxis(GetPhysics()->GetOrigin(),
                                   GetPhysics()->GetAxis());
  ...
  if (ent->fl.takedamage) {
    ...
    if (damageDefName[0] != '\0') {
      ...
      // Only handle the server's own attacks here. Attacks by other players on
      // the server occur through reliable messages.
      if (!common->IsMultiplayer() || common->IsClient() ||
          (common->IsServer() &&
           owner.GetEntityNum() == gameLocal.GetLocalClientNum()) ||
  ...
  if (!common->IsClient() && owner.GetEntity() != NULL &&
      owner.GetEntity()->IsType(idPlayer::Type)) {
    idPlayer* player = static_cast<idPlayer*>(owner.GetEntity());
    int kills = player->GetProjectileKills();

    if (kills >= 2 && common->IsMultiplayer() &&
        strstr(GetName(), "projectile_rocket") != 0) {
      player->GetAchievementManager().EventCompletesAchievement(
          ACHIEVEMENT_MP_KILL_2_GUYS_IN_ROOM_WITH_BFG);
    }
...
void idProjectile::AddDefaultDamageEffect(const trace_t& collision,
  ...
  if (common->IsServer() && fl.networkSync) {
    ...
    if (spawnArgs.GetBool("net_instanthit") &&
        owner.GetEntityNum() < MAX_PLAYERS) {
...
================
*/
void idProjectile::Killed(idEntity* inflictor, idEntity* attacker, int damage,
...
================
*/
void idProjectile::Event_GetProjectileState() { idThread::ReturnInt(state); }
...
void idProjectile::Explode(const trace_t& collision, idEntity* ignore) {
  ...
  if (player != NULL && isHitscan == false) {
    ...
    float damageScale = idMath::ClampFloat(
        0.25f, 1.0f,
        (float)damage * (1.0f / 200.0f));  // 50...200 -> min...max rumble

    // distance
    float dist = (GetPhysics()->GetOrigin() - player->GetPhysics()->GetOrigin())
                     .LengthFast();
    float distScale =
        1.0f - idMath::ClampFloat(0.0f, 1.0f,
                                  (dist * (1.0f / 4000.0f)) +
                                      0.25f);  // 0...4000 -> max...min rumble

    distScale *= damageScale;  // apply damage scale here, weaker damage
                               // produces less rumble
...
================
*/
void idProjectile::Event_Fizzle() { Fizzle(); }
...
================
*/
int idProjectile::GetProjectileState() { return (int)state; }
...
================
*/
void idProjectile::Event_CreateProjectile(idEntity* owner, const idVec3& start,
                                          const idVec3& dir) {
...
================
*/
void idProjectile::ClientThink(const int curTime, const float fraction,
                               const bool predict) {
...
================
*/
bool idProjectile::ClientReceiveEvent(int event, int time,
                                      const idBitMsg& msg) {
  trace_t collision;
  idVec3 velocity;
...
void idProjectile::SimulateProjectileFrame(int msec, int endTime) {
  ...
  if (g_projectileDebug.GetBool()) {
    ...
    idLib::Printf("Simulated projectile %d. Delta: %.2f \n", GetEntityNumber(),
                  delta);
    // clientGame->renderWorld->DebugLine( idColor::colorYellow, oldOrigin,
    // GetPhysics()->GetOrigin(), 5000 );
  }
}
...
void idProjectile::PostSimulate(int endTime) {
  if (state == EXPLODED || state == FIZZLED) {
    // Already exploded. To see the explosion on the collision surface instead
    // of at the muzzle, don't set the deltas to the launch origin and axis.
    CreateDeltasFromOldOriginAndAxis(GetPhysics()->GetOrigin(),
                                     GetPhysics()->GetAxis());
  } else {
...
=================
*/
void idGuidedProjectile::Launch(const idVec3& start, const idVec3& dir,
                                const idVec3& pushVelocity,
                                const float timeSinceFire,
                                const float launchPower, float dmgPower) {
...
void idGuidedProjectile::SetEnemy(idEntity* ent) { enemy = ent; }
void idGuidedProjectile::Event_SetEnemy(idEntity* ent) { SetEnemy(ent); }
...
void idSoulCubeMissile::KillTarget(const idVec3& dir) {
  ...
  if (enemy.GetEntity() && enemy.GetEntity()->IsType(idActor::Type)) {
    ...
    if ((act->health > 0) && ownerEnt != NULL &&
        ownerEnt->IsType(idPlayer::Type) && (ownerEnt->health > 0) &&
        !act->spawnArgs.GetBool("boss")) {
...
=================
*/
void idSoulCubeMissile::Launch(const idVec3& start, const idVec3& dir,
                               const idVec3& pushVelocity,
                               const float timeSinceFire,
                               const float launchPower, float dmgPower) {
  idVec3 newStart;
...
=================
*/
void idBFGProjectile::Launch(const idVec3& start, const idVec3& dir,
                             const idVec3& pushVelocity,
                             const float timeSinceFire, const float power,
...
================
*/
void idDebris::Create(idEntity* owner, const idVec3& start,
                      const idMat3& axis) {
  Unbind();
...
void idDebris::Launch() {
  ...
  if (!collisionModelManager->TrmFromModel(clipModelName, trm)) {
    ...
    physicsObj.SetClipModel(new (TAG_PHYSICS_CLIP_ENTITY) idClipModel(trm),
                            1.0f);
  }
...
================
*/
void idDebris::Killed(idEntity* inflictor, idEntity* attacker, int damage,
                      const idVec3& dir, int location) {
  if (spawnArgs.GetBool("detonate_on_death")) {
    Explode();
  } else {
...
================
*/
void idDebris::Event_Explode() { Explode(); }
...
================
*/
void idDebris::Event_Fizzle() { Fizzle(); }
...
void idHomingProjectile::Think() {
  if (seekPos == vec3_zero) {
    // ai def uses a single def_projectile .. guardian has two projectile types
    // so when seekPos is zero, just run regular projectile
    idProjectile::Think();
    return;
  }
...
=================
*/
void idHomingProjectile::Launch(const idVec3& start, const idVec3& dir,
                                const idVec3& pushVelocity,
                                const float timeSinceFire,
                                const float launchPower, float dmgPower) {
  idProjectile::Launch(start, dir, pushVelocity, timeSinceFire, launchPower,
...
void idHomingProjectile::SetEnemy(idEntity* ent) { enemy = ent; }
void idHomingProjectile::SetSeekPos(idVec3 pos) { seekPos = pos; }
void idHomingProjectile::Event_SetEnemy(idEntity* ent) { SetEnemy(ent); }
