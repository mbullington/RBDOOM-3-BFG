...
ID_INLINE void idImage::DeriveOpts() {
  if (opts.format == FMT_NONE) {
    ...
    switch (usage) {
      ...
      case TD_DIFFUSE:
        // TD_DIFFUSE gets only set to when its a diffuse texture for an
        // interaction
        opts.gammaMips = true;
      ...
      case TD_FONT:
        opts.format = FMT_DXT1;
        opts.colorFormat = CFM_GREEN_ALPHA;
        opts.numLevels = 4;  // We only support 4 levels because we align to 16
                             // in the exporter
        opts.gammaMips = true;
        break;
      case TD_LIGHT:
      ...
      case TD_LOOKUP_TABLE_RGBA:
        ...
        break;
      // motorsep 05-17-2015; added this for uncompressed cubemap/skybox
      // textures
...
========================
*/
void idImage::AllocImage(const idImageOpts& imgOpts, textureFilter_t tf,
                         textureRepeat_t tr) {
  filter = tf;
  repeat = tr;
  opts = imgOpts;
...
===============
*/
void idImage::GetGeneratedName(idStr& _name, const textureUsage_t& _usage,
...
void idImage::ActuallyLoadImage(bool fromBackEnd) {
  ...
  // RB: the following does not load the source images from disk because pic is
  // NULL but it tries to get the timestamp to see if we have a newer file than
  // the one in the compressed .bimage

  // TODO also check for alternative names like .png suffices or _rmao.png or
  // even _rmaod.png files to support the PBR code path
  ...
  // Figure out opts.colorFormat and opts.format so we can make sure the binary
  // image is up to date
  DeriveOpts();
  ...
  if (binaryFileTime == FILE_NOT_FOUND_TIMESTAMP &&
    ...
    while (c-- > 0) {
      ...
      if (generatedName.Find("textures/decals/bulletglass1_d#__0100", false) >=
          0) {
      ...
      if (generatedName.Find("models/monsters/skeleton/skeleton01_d#__1000",
                             false) >= 0) {
  ...
  if ((fileSystem->InProductionMode() &&
      ...
      ((binaryFileTime != FILE_NOT_FOUND_TIMESTAMP) &&
       ...
       // SRS - Handle case when image read is cached and RGB565 format
    ...
    if (cvarSystem->GetCVarBool("fs_buildresources")) {
      // for resource gathering write this image to the preload file for this
      // map
    ...
    if (binaryFileTime == FILE_NOT_FOUND_TIMESTAMP) {
      binarizeReason =
          va("binarize: binary file not found '%s'", generatedName.c_str());
    } else if (header.colorFormat != opts.colorFormat) {
      binarizeReason =
          va("binarize: mismatch color format '%s'", generatedName.c_str());
    } else if (header.colorFormat != opts.colorFormat) {
      binarizeReason =
          va("binarize: mismatched color format '%s'", generatedName.c_str());
    } else if (header.textureType != opts.textureType) {
      binarizeReason =
          va("binarize: mismatched texture type '%s'", generatedName.c_str());
    }
    // else if( toolUsage )
    ...
    if (cubeFiles == CF_NATIVE || cubeFiles == CF_CAMERA ||
      ...
      if (!R_LoadCubeImages(GetName(), cubeFiles, pics, &size, &sourceFileTime,
                            cubeMapSize) ||
          size == 0) {
      ...
      commonLocal.LoadPacifierBinarizeFilename(generatedName.c_str(),
                                               binarizeReason.c_str());
      if (opts.numLevels > 1) {
        commonLocal.LoadPacifierBinarizeProgressTotal(opts.width * opts.width *
                                                      6 * 4 / 3);
      } else {
        commonLocal.LoadPacifierBinarizeProgressTotal(opts.width * opts.width *
                                                      6);
      ...
      im.LoadCubeFromMemory(size, (const byte**)pics, opts.numLevels,
                            opts.format, opts.gammaMips);
      ...
      R_LoadImageProgram(GetName(), &pic, &width, &height, &sourceFileTime,
                         &usage);

      if (pic == NULL) {
        idLib::Warning("Couldn't load image: %s : %s", GetName(),
                       generatedName.c_str());
        ...
        for (int level = 0; level < opts.numLevels; level++) {
          SubImageUpload(level, 0, 0, 0, opts.width >> level,
                         opts.height >> level, clear.Ptr());
        }
      ...
      if (cubeFiles == CF_2D_PACKED_MIPCHAIN) {
        im.Load2DAtlasMipchainFromMemory(width, opts.height, pic,
                                         opts.numLevels, opts.format,
                                         opts.colorFormat);
      } else {
        im.Load2DFromMemory(opts.width, opts.height, pic, opts.numLevels,
                            opts.format, opts.colorFormat, opts.gammaMips);
      }
      commonLocal.LoadPacifierBinarizeEnd();
...
void idImage::GenerateImage(const byte* pic, int width, int height,
                            ...
                            textureUsage_t usageParm, textureSamples_t samples,
                            cubeFiles_t _cubeFiles, bool isRenderTarget) {
  PurgeImage();
  ...
  if (pic == NULL || opts.textureType == TT_2D_MULTISAMPLE) {
    ...
    // I just make sure r_useSSAO is 0
       ...
       //  SRS - update layout of Ambient Occlusion image otherwise get Vulkan
       //  validation layer errors with SSAO enabled
    ...
    if (cubeFiles == CF_2D_PACKED_MIPCHAIN) {
      im.Load2DAtlasMipchainFromMemory(width, opts.height, pic, opts.numLevels,
                                       opts.format, opts.colorFormat);
    } else {
      im.Load2DFromMemory(width, height, pic, opts.numLevels, opts.format,
                          opts.colorFormat, opts.gammaMips);
    }
...
// RB begin
void idImage::GenerateShadowArray(int width, int height,
                                  textureFilter_t filterParm,
                                  textureRepeat_t repeatParm,
                                  textureUsage_t usageParm) {
  PurgeImage();
  ...
  for( int i = 0; i < im.NumImages(); i++ )
  {
          const bimageImage_t& img = im.GetImageHeader( i );
          const byte* data = im.GetImageData( i );
          SubImageUpload( img.level, 0, 0, img.destZ, img.width, img.height,
  data );
  }
  */
}
...
void idImage::UploadScratch(const byte* data, int cols, int rows) {
  ...
  if (rows == cols * 6) {
    ...
    if (opts.textureType != TT_2D || usage != TD_LOOKUP_TABLE_RGBA) {
      GenerateImage(data, cols, rows, TF_LINEAR, TR_REPEAT,
