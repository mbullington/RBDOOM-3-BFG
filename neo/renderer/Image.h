...
enum textureFormat_t {
  ...
  // straight alpha gives 0 for color, but we want 1.
  FMT_ALPHA,
...
class idImageOpts {
 public:
  ...
  int numLevels;   // if 0, will be 1 for NEAREST / LINEAR filters, otherwise
                   // based on size
  bool gammaMips;  // if true, mips will be generated with gamma correction
  bool readback;   // 360 specific - cpu reads back from this texture, so
...
typedef enum {
  ...
  // RB begin
  TD_SPECULAR_PBR_RMAO,   // may be compressed, and always zeros the alpha
                          // channel, linear RGB R = roughness, G = metal, B =
                          // ambient occlusion
  TD_SPECULAR_PBR_RMAOD,  // may be compressed, alpha channel contains
                          // displacement map
  TD_HIGHQUALITY_CUBE,    // motorsep - Uncompressed cubemap texture (RGB
                          // colorspace)
  TD_LOWQUALITY_CUBE,  // motorsep - Compressed cubemap texture (RGB colorspace
                       // DXT5)
...
typedef enum {
  ...
  CF_CAMERA,    // _forward, _back, etc, rotated and flipped as needed before
                // sending to GL
  CF_PANORAMA,  // TODO latlong encoded HDRI panorama typically used by
                // Substance or Blender
  CF_2D_ARRAY,  // not a cube map but not a single 2d texture either
  CF_2D_PACKED_MIPCHAIN,  // usually 2d but can be an octahedron, packed mipmaps
                          // into single 2d texture atlas and limited to dim^2
...
class idImage {
 ...
 public:
  ...
  // RB begin
  void GenerateShadowArray(int width, int height, textureFilter_t filter,
  ...
  void AllocImage(const idImageOpts& imgOpts, textureFilter_t filter,
  ...
  // z is 0 for 2D textures, 0 - 5 for cube maps, and 0 - uploadDepth for 3D
  // textures. Only one plane at a time of 3D textures can be uploaded. The data
  // is assumed to be correct for the format, either bytes, halfFloats, floats,
  // or DXT compressed. The data is assumed to be in OpenGL RGBA format, the
  // consoles may have to reorganize. pixelPitch is only needed when updating
  // from a source subrect. Width, height, and dest* are always in pixels, so
  // they must be a multiple of four for dxt data.
  void SubImageUpload(int mipLevel, int destX, int destY, int destZ, int width,
                      int height, const void* data, int pixelPitch = 0);

  // some scratch images are dynamically resized based on the display window
  // size.  This simply purges the image and recreates it if the sizes are
  // different, so it should not be done under any normal circumstances, and
  // probably not at all on consoles.
  ...
  static void GetGeneratedName(idStr& _name, const textureUsage_t& _usage,
                               const cubeFiles_t& _cube);
  ...
  void GenerateImage(const byte* pic, int width, int height,
                     ...
                     textureUsage_t usage, textureSamples_t samples = SAMPLE_1,
                     cubeFiles_t cubeFiles = CF_2D,
                     bool isRenderTarget = false);

  void GenerateCubeImage(const byte* pic[6], int size, textureFilter_t filter,
  ...
  void* GetImGuiTextureID() {
    if (!IsLoaded()) {
      // load the image on demand here, which isn't our normal game operating
      // mode
 ...
 private:
  ...
  bool defaulted;  // true if the default image was generated because a file
                   // couldn't be loaded
  ID_TIME_T sourceFileTime;  // the most recent of all images used in creation,
                             // for reloadImages command
  ...
  void SetImageLayout(VkImage image, VkImageSubresourceRange subresourceRange,
...
class idImageManager {
 public:
  ...
  idImage* ImageFromFile(const char* name, textureFilter_t filter,
                         textureRepeat_t repeat, textureUsage_t usage,
                         cubeFiles_t cubeMap = CF_2D, int cubeMapSize = 0);

  // look for a loaded image, whatever the parameters
  idImage* GetImage(const char* name) const;

  // look for a loaded image, whatever the parameters
  idImage* GetImageWithParameters(const char* name, textureFilter_t filter,
                                  textureRepeat_t repeat, textureUsage_t usage,
                                  cubeFiles_t cubeMap) const;
  ...
  idImage* currentNormalsImage;  // cheap G-Buffer replacement, holds normals
                                 // and surface roughness
  idImage* ambientOcclusionImage[2];  // contain AO and bilateral filtering keys
  idImage* hierarchicalZbufferImage;  // zbuffer with mip maps to accelerate
  ...
  idImage* originalCurrentRenderImage;  // currentRenderImage before any changes
...
// SP begin
// This method takes in a cubemap from a single image. Depending on the side
// (0-5), the image will be extracted from data and returned. The dimensions
// will be size x size.
...
bool R_LoadCubeImages(const char* cname, cubeFiles_t extensions, byte* pic[6],
                      int* size, ID_TIME_T* timestamp, int cubeMapSize = 0);
...
void R_LoadImageProgram(const char* name, byte** pic, int* width, int* height,
                        ID_TIME_T* timestamp, textureUsage_t* usage = NULL);
