...
idCVar r_motionBlur("r_motionBlur", "0",
                    CVAR_RENDERER | CVAR_INTEGER | CVAR_ARCHIVE,
                    "1 - 5, log2 of the number of motion blur samples");
idCVar r_forceZPassStencilShadows(
    "r_forceZPassStencilShadows", "0", CVAR_RENDERER | CVAR_BOOL,
    "force Z-pass rendering for performance testing");
idCVar r_useStencilShadowPreload(
    "r_useStencilShadowPreload", "0", CVAR_RENDERER | CVAR_BOOL,
    "use stencil shadow preload algorithm instead of Z-fail");
idCVar r_skipShaderPasses("r_skipShaderPasses", "0", CVAR_RENDERER | CVAR_BOOL,
                          "");
idCVar r_skipInteractionFastPath("r_skipInteractionFastPath", "1",
                                 CVAR_RENDERER | CVAR_BOOL, "");
idCVar r_useLightStencilSelect("r_useLightStencilSelect", "0",
                               CVAR_RENDERER | CVAR_BOOL,
                               "use stencil select pass");
...
================
*/
static void RB_SetMVPWithStereoOffset(const idRenderMatrix& mvp,
...
======================
*/
void idRenderBackend::BindVariableStageImage(const textureStage_t* texture,
  ...
  if (texture->cinematic) {
    ...
    if (cin.imageY != NULL) {
      ...
      // DG: imageY is only used for bink videos (with libbinkdec), so the bink
      // shader must be used
      if (viewDef->is2Dgui) {
      ...
      // SRS - Reenable shaders so ffmpeg and RoQ decoder cinematics are
      // rendered with correct colour
      if (viewDef->is2Dgui) {
      ...
      // because the shaders may have already been set - we need to make sure we
      // are not using a bink shader which would display incorrectly.  We may
      // want to get rid of RB_BindVariableStageImage and inline the code so
      // that the SWF GUI case is handled better, too
      renderProgManager.BindShader_TextureVertexColor();
    }
  } else {
...
================
*/
void idRenderBackend::PrepareStageTexturing(const shaderStage_t* pStage,
  ...
  if (pStage->texture.texgen == TG_REFLECT_CUBE) {
    ...
    float wobbleDegrees =
        surf->shaderRegisters[parms[0]] * (idMath::PI / 180.0f);
    float wobbleSpeed =
        surf->shaderRegisters[parms[1]] * (2.0f * idMath::PI / 60.0f);
    float rotateSpeed =
        surf->shaderRegisters[parms[2]] * (2.0f * idMath::PI / 60.0f);
    ...
    idVec3 axis[3];
    {
      ...
      axis[1][0] = -s * s * ws;
      axis[1][2] = -s * ws * ws;
      axis[1][1] = idMath::Sqrt(idMath::Fabs(
    ...
    float mat[16];
    R_MatrixMultiply(surf->space->modelViewMatrix, viewDef->projectionMatrix,
                     mat);

    RENDERLOG_PRINTF("TexGen : %s\n", (pStage->texture.texgen == TG_SCREEN)
                                          ? "TG_SCREEN"
                                          : "TG_SCREEN2");
    renderLog.Indent();
...
================
*/
void idRenderBackend::FinishStageTexturing(const shaderStage_t* pStage,
                                           const drawSurf_t* surf) {
...
// RB: moved this up because we need to call this several times for shadow
// mapping
void idRenderBackend::ResetViewportAndScissorToDefaultCamera(
    const viewDef_t* _viewDef) {
...
==================
*/
void idRenderBackend::FillDepthBufferGeneric(const drawSurf_t* const* drawSurfs,
                                             int numDrawSurfs) {
  for (int i = 0; i < numDrawSurfs; i++) {
    ...
    if (shader->TestMaterialFlag(MF_POLYGONOFFSET)) {
      ...
      GL_PolygonOffset(r_offsetFactor.GetFloat(),
    ...
    if (shader->Coverage() == MC_OPAQUE) {
      ...
      for (stage = 0; stage < shader->GetNumStages(); stage++) {
        ...
        assert((GL_GetCurrentState() & GLS_DEPTHFUNC_BITS) ==
               GLS_DEPTHFUNC_LESS);
...
The bulk of the surfaces should be simple opaque geometry that can be drawn very
rapidly.

If there are no subview surfaces, we could clear to black and use fast-Z
rendering on the 360.
=====================
*/
void idRenderBackend::FillDepthBufferFast(drawSurf_t** drawSurfs,
                                          int numDrawSurfs) {
  if (numDrawSurfs == 0) {
    return;
  }
  ...
  // draw all the subview surfaces, which will already be at the start of the
  // sorted list, with the general purpose path
  ...
  // that we will defer drawing until all opaque surfaces are done
  GL_State(GLS_DEFAULT);
...
==================
*/
void idRenderBackend::SetupInteractionStage(const shaderStage_t* surfaceStage,
    ...
    for (int i = 0; i < 4; i++) {
      ...
      color[i] =
          idMath::ClampFloat(0.0f, 1.0f,
                             surfaceRegs[surfaceStage->color.registers[i]]) *
          lightColor[i];
    }
  }
}
...
void idRenderBackend::DrawSingleInteraction(drawInteraction_t* din,
                                            bool useFastPath, bool useIBL,
                                            bool setInteractionShader) {
  if (!useFastPath) {
    ...
    if (din->specularImage == NULL || r_skipSpecular.GetBool() ||
        (din->vLight && din->vLight->lightShader->IsAmbientLight())) {
    ...
    const bool diffuseIsBlack =
        (din->diffuseImage == globalImages->blackImage) ||
        ((din->diffuseColor[0] <= 0) && (din->diffuseColor[1] <= 0) &&
         (din->diffuseColor[2] <= 0));
    const bool specularIsBlack =
        (din->specularImage == globalImages->blackImage) ||
    ...
    // diffuse matrix
    SetVertexParm(RENDERPARM_DIFFUSEMATRIX_S,
                  din->diffuseMatrix[0].ToFloatPtr());
    SetVertexParm(RENDERPARM_DIFFUSEMATRIX_T,
                  din->diffuseMatrix[1].ToFloatPtr());

    // specular matrix
    SetVertexParm(RENDERPARM_SPECULARMATRIX_S,
                  din->specularMatrix[0].ToFloatPtr());
    SetVertexParm(RENDERPARM_SPECULARMATRIX_T,
    ...
    SetFragmentParm(RENDERPARM_SPECULARMODIFIER,
  ...
  if (useIBL && currentSpace->useLightGrid && r_useLightGrid.GetBool()) {
    ...
    idVec4 lightGridOrigin(currentSpace->lightGridOrigin.x,
                           currentSpace->lightGridOrigin.y,
                           currentSpace->lightGridOrigin.z, 1.0f);
    idVec4 lightGridSize(currentSpace->lightGridSize.x,
                         currentSpace->lightGridSize.y,
                         currentSpace->lightGridSize.z, 1.0f);
    idVec4 lightGridBounds(currentSpace->lightGridBounds[0],
                           currentSpace->lightGridBounds[1],
                           currentSpace->lightGridBounds[2], 1.0f);

    renderProgManager.SetUniformValue(RENDERPARM_GLOBALLIGHTORIGIN,
                                      lightGridOrigin.ToFloatPtr());
    renderProgManager.SetUniformValue(RENDERPARM_JITTERTEXSCALE,
                                      lightGridSize.ToFloatPtr());
    renderProgManager.SetUniformValue(RENDERPARM_JITTERTEXOFFSET,
                                      lightGridBounds.ToFloatPtr());

    // individual probe sizes on the atlas image
    idVec4 probeSize;
    probeSize[0] = currentSpace->lightGridAtlasSingleProbeSize -
                   currentSpace->lightGridAtlasBorderSize;
    probeSize[1] = currentSpace->lightGridAtlasSingleProbeSize;
    probeSize[2] = currentSpace->lightGridAtlasBorderSize;
    probeSize[3] = float(currentSpace->lightGridAtlasSingleProbeSize -
                         currentSpace->lightGridAtlasBorderSize) /
                   currentSpace->lightGridAtlasSingleProbeSize;
    renderProgManager.SetUniformValue(
        RENDERPARM_SCREENCORRECTIONFACTOR,
        probeSize.ToFloatPtr());  // rpScreenCorrectionFactor

    // specular cubemap blend weights
    renderProgManager.SetUniformValue(
        RENDERPARM_LOCALLIGHTORIGIN, viewDef->radianceImageBlends.ToFloatPtr());

    if (specUsage == TD_SPECULAR_PBR_RMAO ||
        specUsage == TD_SPECULAR_PBR_RMAOD) {
    ...
    if (!r_useSSAO.GetBool() ||
        (viewDef->renderView.rdflags & (RDF_NOAMBIENT | RDF_IRRADIANCE))) {
      globalImages->whiteImage->Bind();
    } else {
    ...
    renderProgManager.SetUniformValue(RENDERPARM_CASCADEDISTANCES,
    ...
    if (specUsage == TD_SPECULAR_PBR_RMAO ||
      ...
      if (din->vLight->lightShader->IsAmbientLight()) {
        ...
        if (r_useShadowMapping.GetBool() && din->vLight->globalShadows) {
          // RB: we have shadow mapping enabled and shadow maps so do a shadow
          // compare

          if (din->vLight->parallel) {
            if (din->surf->jointCache) {
              renderProgManager
                  .BindShader_PBR_Interaction_ShadowMapping_Parallel_Skinned();
            } else {
              renderProgManager
                  .BindShader_PBR_Interaction_ShadowMapping_Parallel();
            }
          } else if (din->vLight->pointLight) {
            if (din->surf->jointCache) {
              renderProgManager
                  .BindShader_PBR_Interaction_ShadowMapping_Point_Skinned();
            } else {
              renderProgManager
                  .BindShader_PBR_Interaction_ShadowMapping_Point();
            }
          } else {
            if (din->surf->jointCache) {
              renderProgManager
                  .BindShader_PBR_Interaction_ShadowMapping_Spot_Skinned();
            } else {
      ...
      if (din->vLight->lightShader->IsAmbientLight()) {
        ...
        if (r_useShadowMapping.GetBool() && din->vLight->globalShadows) {
          ...
          if (din->vLight->parallel) {
            if (din->surf->jointCache) {
              renderProgManager
                  .BindShader_Interaction_ShadowMapping_Parallel_Skinned();
            } else {
            ...
            if (din->surf->jointCache) {
              renderProgManager
                  .BindShader_Interaction_ShadowMapping_Point_Skinned();
            } else {
...
=================
*/
static void RB_SetupForFastPathInteractions(const idVec4& diffuseColor,
...
=============
*/
void idRenderBackend::RenderInteractions(const drawSurf_t* surfList,
                                         const viewLight_t* vLight,
                                         int depthFunc, bool performStencilTest,
                                         bool useLightDepthBounds) {
  if (surfList == NULL) {
    return;
  }

  // change the scissor if needed, it will be constant across all the surfaces
  // lit by the light
  ...
  // some rare lights have multiple animating stages, loop over them outside the
  // surface list
  ...
  idStaticList<const drawSurf_t*, MAX_COMPLEX_INTERACTIONS_PER_LIGHT>
      complexSurfaces;
  for (const drawSurf_t* walk = surfList; walk != NULL;
       walk = walk->nextOnLight) {
    // make sure the triangle culling is done
    if (walk->shadowVolumeState != SHADOWVOLUME_DONE) {
      assert(walk->shadowVolumeState == SHADOWVOLUME_UNFINISHED ||
             walk->shadowVolumeState == SHADOWVOLUME_DONE);
  ...
  if (r_useShadowMapping.GetBool()) {
    ...
    screenCorrectionParm[3] = vLight->parallel
                                  ? r_shadowMapSunDepthBiasScale.GetFloat()
                                  : r_shadowMapRegularDepthBiasScale.GetFloat();
    SetFragmentParm(RENDERPARM_SCREENCORRECTIONFACTOR,
                    screenCorrectionParm);  // rpScreenCorrectionFactor

    float jitterTexScale[4];
    jitterTexScale[0] = r_shadowMapJitterScale.GetFloat() *
                        jitterSampleScale;  // TODO shadow buffer size fraction
    ...
    SetFragmentParm(RENDERPARM_JITTERTEXSCALE,
                    jitterTexScale);  // rpJitterTexScale

    float jitterTexOffset[4];
    jitterTexOffset[0] =
        1.0f / globalImages->blueNoiseImage256->GetUploadWidth();
    jitterTexOffset[1] =
        1.0f / globalImages->blueNoiseImage256->GetUploadWidth();
    ...
    SetFragmentParm(RENDERPARM_JITTERTEXOFFSET,
                    jitterTexOffset);  // rpJitterTexOffset
    ...
    if (vLight->parallel) {
      ...
      SetFragmentParm(RENDERPARM_CASCADEDISTANCES,
                      cascadeDistances);  // rpCascadeDistances
    }
  }
  // RB end
  ...
  for (int lightStageNum = 0; lightStageNum < lightShader->GetNumStages();
    ...
    // force the light textures to not use anisotropic filtering, which is
    // wasted on them all of the texture sampler parms should be constant for
    // all interactions, only the actual texture image bindings will change

    //----------------------------------
    // For all surfaces on this light list, generate an interaction for this
    // light stage
    //----------------------------------
    ...
    // even if the space does not change between light stages, each light stage
    // may need a different lightTextureMatrix baked in
    currentSpace = NULL;

    for (int sortedSurfNum = 0; sortedSurfNum < allSurfaces.Num();
         sortedSurfNum++) {
      ...
      // change the MVP matrix, view/light origin and light projection vectors
      // if needed
      if (surf->space != currentSpace) {
        currentSpace = surf->space;

        // turn off the light depth bounds test if this model is rendered with a
        // depth hack
        if (useLightDepthBounds) {
          if (!surf->space->weaponDepthHack &&
              surf->space->modelDepthHack == 0.0f) {
            if (lightDepthBoundsDisabled) {
              GL_DepthBoundsTest(vLight->scissorRect.zmin,
                                 vLight->scissorRect.zmax);
        ...
        idRenderMatrix::Transpose(*(idRenderMatrix*)surf->space->modelMatrix,
                                  modelMatrix);
        ...
        idRenderMatrix::Transpose(
            *(idRenderMatrix*)surf->space->modelViewMatrix, modelViewMatrix);
        SetVertexParms(RENDERPARM_MODELVIEWMATRIX_X, modelViewMatrix[0], 4);

        idVec4 globalLightOrigin(vLight->globalLightOrigin.x,
                                 vLight->globalLightOrigin.y,
                                 vLight->globalLightOrigin.z, 1.0f);
        SetVertexParm(RENDERPARM_GLOBALLIGHTORIGIN,
                      globalLightOrigin.ToFloatPtr());
        // RB end
        ...
        R_GlobalPointToLocal(surf->space->modelMatrix,
                             ...
                             localLightOrigin.ToVec3());
        R_GlobalPointToLocal(surf->space->modelMatrix,
                             viewDef->renderView.vieworg,
                             localViewOrigin.ToVec3());

        // set the local light/view origin
        SetVertexParm(RENDERPARM_LOCALLIGHTORIGIN,
                      localLightOrigin.ToFloatPtr());
        ...
        for (int i = 0; i < 4; i++) {
          R_GlobalPlaneToLocal(surf->space->modelMatrix,
                               vLight->lightProject[i], lightProjection[i]);
        }

        // optionally multiply the local light projection by the light texture
        // matrix
        if (lightStage->texture.hasMatrix) {
        ...
        SetVertexParm(RENDERPARM_LIGHTPROJECTION_S,
                      lightProjection[0].ToFloatPtr());
        SetVertexParm(RENDERPARM_LIGHTPROJECTION_T,
                      lightProjection[1].ToFloatPtr());
        SetVertexParm(RENDERPARM_LIGHTPROJECTION_Q,
                      lightProjection[2].ToFloatPtr());
        SetVertexParm(RENDERPARM_LIGHTFALLOFF_S,
                      lightProjection[3].ToFloatPtr());
        ...
        if (r_useShadowMapping.GetBool()) {
          if (vLight->parallel) {
            for (int i = 0; i < (r_shadowMapSplits.GetInteger() + 1); i++) {
              ...
              idRenderMatrix::Multiply(shadowV[i], modelMatrix,
                                       modelToShadowMatrix);

              idRenderMatrix shadowClipMVP;
              idRenderMatrix::Multiply(shadowP[i], modelToShadowMatrix,
                                       shadowClipMVP);

              idRenderMatrix shadowWindowMVP;
              idRenderMatrix::Multiply(renderMatrix_clipSpaceToWindowSpace,
                                       shadowClipMVP, shadowWindowMVP);

              SetVertexParms(
                  (renderParm_t)(RENDERPARM_SHADOW_MATRIX_0_X + i * 4),
                  shadowWindowMVP[0], 4);
            }
            ...
            idRenderMatrix modelToShadowMatrix;
            idRenderMatrix::Multiply(shadowV[0], modelMatrix,
                                     modelToShadowMatrix);

            idRenderMatrix shadowClipMVP;
            idRenderMatrix::Multiply(shadowP[0], modelToShadowMatrix,
                                     shadowClipMVP);

            SetVertexParms((renderParm_t)(RENDERPARM_SHADOW_MATRIX_0_X),
                           shadowClipMVP[0], 4);
          }
        }
        // RB end
      ...
      if (surfaceShader->GetFastPathBumpImage() &&
          !r_skipInteractionFastPath.GetBool()) {
      ...
      //
      // This is somewhat arcane because of the old support for video cards that
      // had to render interactions in multiple passes.
      //
      // We also have the very rare case of some materials that have conditional
      // interactions for the "hell writing" that can be shined on them.
      for (int surfaceStageNum = 0;
        ...
        switch (surfaceStage->lighting) {
          case SL_COVERAGE: {
            // ignore any coverage stages since they should only be used for the
            // depth fill pass for diffuse stages that use alpha test.
            break;
          }
          case SL_AMBIENT: {
          ...
          case SL_DIFFUSE: {
            ...
            SetupInteractionStage(
                surfaceStage, surfaceRegs, diffuseColor.ToFloatPtr(),
          ...
          case SL_SPECULAR: {
            // ignore stage that fails the condition
            if (!surfaceRegs[surfaceStage->conditionRegister] ||
                vLight->lightDef->parms.noSpecular)  // SRS - From RB forums
            {
              break;
            }
            // draw any previous interaction
            ...
            inter.vertexColor = surfaceStage->vertexColor;
            SetupInteractionStage(
                surfaceStage, surfaceRegs, specularColor.ToFloatPtr(),
...
#define BLEND_NORMALS 1
  ...
  if (viewDef->renderView.rdflags & RDF_IRRADIANCE) {
    ...
    // ambientColor.Set( energyConservation, energyConservation,
    // energyConservation, 1.0f );
    ...
    // apply the world-global overbright and tune down specular a bit so we have
    // less fresnel overglow
    diffuseColor = lightColor;
  ...
  for (int i = 0; i < numDrawSurfs; i++) {
    ...
    // bool isWorldModel = ( drawSurf->space->entityDef->parms.origin ==
    // vec3_origin );
    ...
    if (drawSurf->space != currentSpace) {
      ...
      // RB: if we want to store the normals in world space so we need the model
      // -> world matrix
      idRenderMatrix modelMatrix;
      idRenderMatrix::Transpose(*(idRenderMatrix*)drawSurf->space->modelMatrix,
                                modelMatrix);

      SetVertexParms(RENDERPARM_MODELMATRIX_X, modelMatrix[0], 4);

      // RB: if we want to store the normals in camera space so we need the
      // model -> camera matrix
      float modelViewMatrixTranspose[16];
      R_MatrixTranspose(drawSurf->space->modelViewMatrix,
    ...
    if( surfaceMaterial->TestMaterialFlag( MF_POLYGONOFFSET ) )
    {
            surfGLState |= GLS_POLYGON_OFFSET;
            GL_PolygonOffset( r_offsetFactor.GetFloat(),
    r_offsetUnits.GetFloat() * surfaceMaterial->GetPolygonOffset() );
    }
    ...
    {
            surfGLState |= GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO |
    GLS_DEPTHFUNC_LESS; color[0] = 1.0f; color[1] = 1.0f; color[2] = 1.0f;
            color[3] = 1.0f;
    }
    else
    {
    ...
    for (stage = 0; stage < surfaceMaterial->GetNumStages(); stage++) {
      ...
      switch (surfaceStage->lighting) {
        ...
        case SL_BUMP: {
          ...
          if (inter.bumpImage != NULL) {
            ...
            if (inter.vertexColor == SVC_IGNORE) {
              ...
              // RB: this is a bit hacky: use additive blending to blend the
              // normals
              GL_State(GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE | GLS_DEPTHMASK |
                       GLS_DEPTHFUNC_EQUAL);
            }
        ...
        case SL_DIFFUSE: {
          ...
          inter.diffuseImage = surfaceStage->texture.image;
          inter.vertexColor = surfaceStage->vertexColor;
          SetupInteractionStage(surfaceStage, surfaceRegs,
                                diffuseColor.ToFloatPtr(), inter.diffuseMatrix,
...
The stencil buffer should have been set to 128 on any surfaces that might
receive shadows.
=====================
*/
void idRenderBackend::StencilShadowPass(const drawSurf_t* drawSurfs,
  ...
  if (r_showShadows.GetInteger()) {
    ...
    if (r_showShadows.GetInteger() == 2) {
      // draw filled in
      glState = GLS_DEPTHMASK | GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE |
                GLS_DEPTHFUNC_LESS;
    } else {
      // draw as lines, filling the depth buffer
      glState = GLS_SRCBLEND_ONE | GLS_DSTBLEND_ZERO | GLS_POLYMODE_LINE |
                GLS_DEPTHFUNC_ALWAYS;
    }
  } else {
  ...
  GL_PolygonOffset(r_shadowPolygonFactor.GetFloat(),
                   -r_shadowPolygonOffset.GetFloat());

  // the actual stencil func will be set in the draw code, but we need to make
  // sure it isn't disabled here, and that the value will get reset for the
  // interactions without looking like a no-change-required

  // Two Sided Stencil reduces two draw calls to one for slightly faster shadows
  GL_State(glState | GLS_STENCIL_OP_FAIL_KEEP | GLS_STENCIL_OP_ZFAIL_KEEP |
           GLS_STENCIL_OP_PASS_INCR |
           GLS_STENCIL_MAKE_REF(STENCIL_SHADOW_TEST_VALUE) |
           GLS_STENCIL_MAKE_MASK(STENCIL_SHADOW_MASK_VALUE) |
           GLS_POLYGON_OFFSET | GLS_CULL_TWOSIDED);

  // process the chain of shadows with the current rendering state
  currentSpace = NULL;

  for (const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL;
       drawSurf = drawSurf->nextOnLight) {
    if (drawSurf->scissorRect.IsEmpty()) {
      continue;  // !@# FIXME: find out why this is sometimes being hit!
      // temporarily jump over the scissor and draw so the gl error callback
      // doesn't get hit
    }

    // make sure the shadow volume is done
    if (drawSurf->shadowVolumeState != SHADOWVOLUME_DONE) {
      assert(drawSurf->shadowVolumeState == SHADOWVOLUME_UNFINISHED ||
             drawSurf->shadowVolumeState == SHADOWVOLUME_DONE);
    ...
    if (!currentScissor.Equals(drawSurf->scissorRect) &&
        r_useScissor.GetBool()) {
      // change the scissor
    ...
    if (drawSurf->space != currentSpace) {
      ...
      // set the local light position to allow the vertex program to project the
      // shadow volume end cap to infinity
      idVec4 localLight(0.0f);
      R_GlobalPointToLocal(drawSurf->space->modelMatrix,
                           vLight->globalLightOrigin, localLight.ToVec3());
    ...
    if (r_useShadowDepthBounds.GetBool()) {
      GL_DepthBoundsTest(drawSurf->scissorRect.zmin,
                         drawSurf->scissorRect.zmax);
    }

    // Determine whether or not the shadow volume needs to be rendered with
    // Z-pass or Z-fail. It is worthwhile to spend significant resources to
    // reduce the number of cases where shadow volumes need to be rendered with
    // Z-fail because Z-fail rendering can be significantly slower even on
    // today's hardware. For instance, on NVIDIA hardware Z-fail rendering
    // causes the Z-Cull to be used in reverse: Z-near becomes Z-far (trivial
    // accept becomes trivial reject). Using the Z-Cull in reverse is far less
    // efficient because the Z-Cull only stores Z-near per 16x16 pixels while
    // the Z-far is stored per 4x2 pixels. (The Z-near coallesce buffer which
    // has 4x4 granularity is only used when updating the depth which is not the
    // case for shadow volumes.) Note that it is also important to NOT use a
    // Z-Cull reconstruct because that would clear the Z-near of the Z-Cull
    // which results in no trivial rejection for Z-fail stencil shadow
    // rendering.

    const bool renderZPass =
        (drawSurf->renderZFail == 0) || r_forceZPassStencilShadows.GetBool();
...
Deform the zeroOneCubeModel to exactly cover the light volume. Render the
deformed cube model to the stencil buffer in such a way that only fragments that
are directly visible and contained within the volume will be written creating a
...
void idRenderBackend::StencilSelectLight(const viewLight_t* vLight) {
  ...
  GL_State(glStateMinusStencil | GLS_STENCIL_FUNC_ALWAYS |
           GLS_STENCIL_MAKE_REF(STENCIL_SHADOW_TEST_VALUE) |
           GLS_STENCIL_MAKE_MASK(
  ...
  GL_State(GLS_COLORMASK | GLS_ALPHAMASK | GLS_CULL_TWOSIDED | GLS_DEPTHMASK |
           GLS_DEPTHFUNC_LESS | GLS_STENCIL_FUNC_ALWAYS |
           GLS_STENCIL_OP_FAIL_KEEP | GLS_STENCIL_OP_ZFAIL_REPLACE |
           GLS_STENCIL_OP_PASS_ZERO | GLS_BACK_STENCIL_OP_FAIL_KEEP |
           GLS_BACK_STENCIL_OP_ZFAIL_ZERO | GLS_BACK_STENCIL_OP_PASS_REPLACE |
           GLS_STENCIL_MAKE_REF(STENCIL_SHADOW_TEST_VALUE) |
           GLS_STENCIL_MAKE_MASK(STENCIL_SHADOW_MASK_VALUE));

  renderProgManager.BindShader_Depth();

  // set the matrix for deforming the 'zeroOneCubeModel' into the frustum to
  // exactly cover the light volume
  idRenderMatrix invProjectMVPMatrix;
  idRenderMatrix::Multiply(viewDef->worldSpace.mvp,
                           vLight->inverseBaseLightProject,
                           invProjectMVPMatrix);
...
*/
static void MatrixOrthogonalProjectionRH(float m[16], float left, float right,
...
void MatrixLookAtRH(float m[16], const idVec3& eye, const idVec3& dir,
                    const idVec3& up) {
  idVec3 dirN;
  idVec3 upN;
  idVec3 sideN;
...
=====================
*/
void idRenderBackend::ShadowMapPass(const drawSurf_t* drawSurfs,
                                    const viewLight_t* vLight, int side) {
  if (r_skipShadows.GetBool()) {
    return;
  }
  ...
  switch (r_shadowMapOccluderFacing.GetInteger()) {
    case 0:
      GL_State((glStateBits & ~(GLS_CULL_MASK)) | GLS_CULL_FRONTSIDED);
      GL_PolygonOffset(r_shadowMapPolygonFactor.GetFloat(),
  ...
  if (vLight->parallel && side >= 0) {
...
#if 1
    idRenderMatrix::CreateViewMatrix(viewDef->renderView.vieworg, rotation,
                                     lightViewRenderMatrix);
#else
    float lightViewMatrix[16];
    MatrixLookAtRH(lightViewMatrix, viewPos, lightDir, viewDir);
    idRenderMatrix::Transpose(*(idRenderMatrix*)lightViewMatrix,
                              lightViewRenderMatrix);
#endif
    ...
    idRenderMatrix::GetFrustumCorners(corners, vLight->inverseBaseLightProject,
                                      bounds_zeroOneCube);
    ...
    MatrixOrthogonalProjectionRH(lightProjectionMatrix, lightBounds[0][0],
                                 lightBounds[1][0], lightBounds[0][1],
                                 lightBounds[1][1], -lightBounds[1][2],
                                 -lightBounds[0][2]);
    idRenderMatrix::Transpose(*(idRenderMatrix*)lightProjectionMatrix,
                              lightProjectionRenderMatrix);

    // 	'frustumMVP' goes from global space -> camera local space -> camera
    // projective space invert the MVP projection so we can deform zero-to-one
    // cubes into the frustum pyramid shape and calculate global bounds

    idRenderMatrix splitFrustumInverse;
    if (!idRenderMatrix::Inverse(viewDef->frustumMVPs[FRUSTUM_CASCADE1 + side],
                                 splitFrustumInverse)) {
    ...
    idRenderMatrix::GetFrustumCorners(splitFrustumCorners, splitFrustumInverse,
                                      bounds_unitCube);
    ...
    idRenderMatrix::Multiply(lightProjectionRenderMatrix, lightViewRenderMatrix,
                             lightViewProjectionRenderMatrix);
    ...
    // idRenderMatrix::Transpose( *( idRenderMatrix* )cropMatrix,
    // cropRenderMatrix );

    // idRenderMatrix tmp = lightProjectionRenderMatrix;
    // idRenderMatrix::Multiply( cropRenderMatrix, tmp,
    // lightProjectionRenderMatrix );

    MatrixOrthogonalProjectionRH(lightProjectionMatrix, cropBounds[0][0],
                                 cropBounds[1][0], cropBounds[0][1],
                                 cropBounds[1][1], -cropBounds[1][2],
                                 -cropBounds[0][2]);
    idRenderMatrix::Transpose(*(idRenderMatrix*)lightProjectionMatrix,
                              lightProjectionRenderMatrix);
    ...
    memcpy(unflippedLightViewMatrix, viewMatrix,
           sizeof(unflippedLightViewMatrix));
    ...
    lightProjectionMatrix[2 * 4 + 2] =
        -0.999f;  // adjust value to prevent imprecision issues
  ...
  if (side < 0) {
    globalFramebuffers.shadowFBO[vLight->shadowLOD]->AttachImageDepthLayer(
        globalImages->shadowImage[vLight->shadowLOD], 0);
  } else {
    globalFramebuffers.shadowFBO[vLight->shadowLOD]->AttachImageDepthLayer(
        globalImages->shadowImage[vLight->shadowLOD], side);
  }

  globalFramebuffers.shadowFBO[vLight->shadowLOD]->Check();

  GL_ViewportAndScissor(0, 0, shadowMapResolutions[vLight->shadowLOD],
                        shadowMapResolutions[vLight->shadowLOD]);
  ...
  for (const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL;
    ...
    if (drawSurf->space != currentSpace) {
      ...
      idRenderMatrix::Multiply(lightViewRenderMatrix, modelRenderMatrix,
                               modelToLightRenderMatrix);

      idRenderMatrix clipMVP;
      idRenderMatrix::Multiply(lightProjectionRenderMatrix,
                               modelToLightRenderMatrix, clipMVP);

      if (vLight->parallel) {
        idRenderMatrix MVP;
        idRenderMatrix::Multiply(renderMatrix_clipSpaceToWindowSpace, clipMVP,
                                 MVP);
        ...
        // from OpenGL view space to OpenGL NDC ( -1 : 1 in XYZ )
        idRenderMatrix MVP;
        idRenderMatrix::Multiply(renderMatrix_windowSpaceToClipSpace, clipMVP,
                                 MVP);
      ...
      idVec4 localLight( 0.0f );
      R_GlobalPointToLocal( drawSurf->space->modelMatrix,
      vLight->globalLightOrigin, localLight.ToVec3() ); SetVertexParm(
      RENDERPARM_LOCALLIGHTORIGIN, localLight.ToFloatPtr() );
      */
  ...
  for (const viewLight_t* vLight = viewDef->viewLights; vLight != NULL;
    ...
    if (r_useShadowMapping.GetBool()) {
      ...
      if (vLight->localInteractions != NULL) {
        ...
        RenderInteractions(vLight->localInteractions, vLight,
                           GLS_DEPTHFUNC_EQUAL, false, useLightDepthBounds);
        renderLog.CloseBlock();
      }

      if (vLight->globalInteractions != NULL) {
        renderLog.OpenBlock("Global Light Interactions", colorPurple);
        RenderInteractions(vLight->globalInteractions, vLight,
                           GLS_DEPTHFUNC_EQUAL, false, useLightDepthBounds);
        renderLog.CloseBlock();
      }
    } else {
      // only need to clear the stencil buffer and perform stencil testing if
      // there are shadows
      const bool performStencilTest =
          (vLight->globalShadows != NULL || vLight->localShadows != NULL) &&
          !r_useShadowMapping.GetBool();

      // mirror flips the sense of the stencil select, and I don't want to risk
      // accidentally breaking it in the normal case, so simply disable the
      // stencil select in the mirror case
      const bool useLightStencilSelect =
          (r_useLightStencilSelect.GetBool() && viewDef->isMirror == false);
        ...
        if (useLightStencilSelect) {
          ...
          if (!currentScissor.Equals(rect) && r_useScissor.GetBool()) {
            GL_Scissor(viewDef->viewport.x1 + rect.x1,
                       viewDef->viewport.y1 + rect.y1, rect.x2 + 1 - rect.x1,
                       rect.y2 + 1 - rect.y1);
      ...
      if (vLight->localInteractions != NULL) {
        renderLog.OpenBlock("Local Light Interactions", colorPurple);
        RenderInteractions(vLight->localInteractions, vLight,
                           GLS_DEPTHFUNC_EQUAL, performStencilTest,
                           useLightDepthBounds);
        renderLog.CloseBlock();
      }
    ...
    if (vLight->translucentInteractions != NULL &&
      ...
      // Disable the depth bounds test because translucent surfaces don't work
      // with the depth bounds tests since they did not write depth during the
      // depth pass.
      if (useLightDepthBounds) {
        GL_DepthBoundsTest(0.0f, 0.0f);
      }
      ...
      // can never be constrained to perforated surfaces with the depthfunc
      // equal.
      ...
      // buffer and translucent surfaces do not contribute to the view depth
      // buffer.

      RenderInteractions(vLight->translucentInteractions, vLight,
                         GLS_DEPTHFUNC_LESS, false, false);
...
=====================
*/
int idRenderBackend::DrawShaderPasses(const drawSurf_t* const* const drawSurfs,
                                      const int numDrawSurfs,
                                      const float guiStereoScreenOffset,
                                      const int stereoEye) {
  ...
  currentSpace = (const viewEntity_t*)1;  // using NULL makes /analyze think
                                          // surf->space needs to be checked...
  ...
  for (; i < numDrawSurfs; i++) {
    ...
    // if the stereoEye value of a surface is 0 then we need to draw it for both
    // eyes.
    const int shaderStereoEye = shader->GetStereoEye();
    const bool isEyeValid = stereoRender_swapEyes.GetBool()
                                ? (shaderStereoEye == stereoEye)
                                : (shaderStereoEye != stereoEye);
    ...
    if (surf->space != currentSpace ||
        thisGuiStereoOffset != currentGuiStereoOffset) {
      currentSpace = surf->space;
      ...
      idVec4 localViewOrigin(1.0f);
      R_GlobalPointToLocal(space->modelMatrix, viewDef->renderView.vieworg,
                           localViewOrigin.ToVec3());
      SetVertexParm(RENDERPARM_LOCALVIEWORIGIN, localViewOrigin.ToFloatPtr());
    ...
    for (int stage = 0; stage < shader->GetNumStages(); stage++) {
      ...
      // masks
      if ((stageGLState & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) ==
      ...
      if (surf->space->isGuiSurface) {
        ...
        if (pStage->texture.cinematic) {
          if ((stageGLState & GLS_OVERRIDE) != 0) {
            ...
            // Old style guis do not, and we don't want them to use the new GUI
            // renederProg
            renderProgManager.BindShader_TextureVertexColor_sRGB();
          } else {
            renderProgManager.BindShader_TextureVertexColor();
...
=====================
*/
void idRenderBackend::T_BlendLight(const drawSurf_t* drawSurfs,
                                   const viewLight_t* vLight) {
  currentSpace = NULL;
    ...
    if (drawSurf->space != currentSpace) {
      ...
      for (int i = 0; i < 4; i++) {
        R_GlobalPlaneToLocal(drawSurf->space->modelMatrix,
                             vLight->lightProject[i],
                             lightProjectInCurrentSpace[i]);
      }

      SetVertexParm(RENDERPARM_TEXGEN_0_S,
                    lightProjectInCurrentSpace[0].ToFloatPtr());
      SetVertexParm(RENDERPARM_TEXGEN_0_T,
                    lightProjectInCurrentSpace[1].ToFloatPtr());
      SetVertexParm(RENDERPARM_TEXGEN_0_Q,
                    lightProjectInCurrentSpace[2].ToFloatPtr());
      SetVertexParm(RENDERPARM_TEXGEN_1_S,
                    lightProjectInCurrentSpace[3].ToFloatPtr());  // falloff
...
=====================
*/
void idRenderBackend::BlendLight(const drawSurf_t* drawSurfs,
...
=====================
*/
void idRenderBackend::T_BasicFog(
    const drawSurf_t* drawSurfs, const idPlane fogPlanes[4],
  ...
  for (const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL;
    ...
    if (drawSurf->space != currentSpace) {
      ...
      if (inverseBaseLightProject == NULL) {
        RB_SetMVP(drawSurf->space->mvp);
        for (int i = 0; i < 4; i++) {
          R_GlobalPlaneToLocal(drawSurf->space->modelMatrix, fogPlanes[i],
                               localFogPlanes[i]);
        }
      } else {
        idRenderMatrix invProjectMVPMatrix;
        idRenderMatrix::Multiply(viewDef->worldSpace.mvp,
                                 *inverseBaseLightProject, invProjectMVPMatrix);
        RB_SetMVP(invProjectMVPMatrix);
        for (int i = 0; i < 4; i++) {
          inverseBaseLightProject->InverseTransformPlane(
              fogPlanes[i], localFogPlanes[i], false);
        }
      }
...
==================
*/
void idRenderBackend::FogPass(const drawSurf_t* drawSurfs,
                              const drawSurf_t* drawSurfs2,
                              const viewLight_t* vLight) {
  ...
  // T-0
  fogPlanes[1][0] =
      0.0f;  // a * backEnd.viewDef->worldSpace.modelViewMatrix[0*4+0];
  fogPlanes[1][1] =
      0.0f;  // a * backEnd.viewDef->worldSpace.modelViewMatrix[1*4+0];
  fogPlanes[1][2] =
      0.0f;  // a * backEnd.viewDef->worldSpace.modelViewMatrix[2*4+0];
  fogPlanes[1][3] =
      0.5f;  // a * backEnd.viewDef->worldSpace.modelViewMatrix[3*4+0] + 0.5f;

  // T-1 will get a texgen for the fade plane, which is always the "top" plane
  // on unrotated lights
...
void idRenderBackend::FogAllLights() {
  ...
  for (viewLight_t* vLight = viewDef->viewLights; vLight != NULL;
       vLight = vLight->next) {
    if (vLight->lightShader->IsFogLight()) {
...
void idRenderBackend::DrawScreenSpaceAmbientOcclusion(const viewDef_t* _viewDef,
  ...
  if (downModulateScreen) {
    if (r_ssaoFiltering.GetBool()) {
      ...
      if (r_ssaoDebug.GetInteger() <= 0) {
        GL_State(GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO | GLS_ALPHAMASK |
                 GLS_DEPTHMASK | GLS_DEPTHFUNC_ALWAYS);
      }
...
#endif
  SetVertexParms(RENDERPARM_MODELMATRIX_X,
                 viewDef->unprojectionToCameraRenderMatrix[0], 4);
  ...
  jitterTexOffset[1] =
      1.0f / globalImages->blueNoiseImage256->GetUploadHeight();
  ...
  if (r_ssaoFiltering.GetBool()) {
    ...
    if (downModulateScreen) {
      ...
      if (r_ssaoDebug.GetInteger() <= 0) {
        GL_State(GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO | GLS_DEPTHMASK |
                 GLS_DEPTHFUNC_ALWAYS);
      }
    } else {
      globalFramebuffers.ambientOcclusionFBO[0]->Bind();
...
void idRenderBackend::DrawScreenSpaceGlobalIllumination(
    const viewDef_t* _viewDef) {
#if !defined(USE_VULKAN)
  if (!_viewDef->viewEntitys || _viewDef->is2Dgui) {
    // 3D views only
...
void idRenderBackend::DrawViewInternal(const viewDef_t* _viewDef,
  ...
  //-------------------------------------------------
  // FIXME, OPTIMIZE: merge this with FillDepthBufferFast like in a light
  // prepass deferred renderer
  //
  ...
  if (!r_skipShaderPasses.GetBool()) {
    ...
    processed =
        DrawShaderPasses(drawSurfs, numDrawSurfs, guiScreenOffset, stereoEye);
    renderLog.CloseMainBlock();
  }

  //-------------------------------------------------
  // use direct light and emissive light contributions to add indirect screen
  // space light
  //-------------------------------------------------
  // RB_SSGI( viewDef );
  ...
  if (processed < numDrawSurfs && !r_skipPostProcess.GetBool()) {
    ...
    DrawShaderPasses(drawSurfs + processed, numDrawSurfs - processed,
                     0.0f /* definitely not a gui */, stereoEye);
    renderLog.CloseMainBlock();
  }
...
// FIXME
  ...
  for (int surfNum = 0; surfNum < viewDef->numDrawSurfs; surfNum++) {
    ...
    if (!surf->space->weaponDepthHack && !surf->space->skipMotionBlur &&
        !surf->material->HasSubview()) {
  ...
  // we use the viewport dimensions for copying the buffers in case resolution
  // scaling is enabled.
  const idScreenRect& viewport = viewDef->viewport;
  globalImages->currentRenderImage->CopyFramebuffer(
      viewport.x1, viewport.y1, viewport.GetWidth(), viewport.GetHeight());
...
void idRenderBackend::PostProcess(const void* data) {
  // only do the post process step if resolution scaling is enabled. Prevents
  // the unnecessary copying of the framebuffer and corresponding full screen
  // quad pass.
  if (rs_enable.GetInteger() == 0 && !r_useFilmicPostProcessing.GetBool() &&
      r_antiAliasing.GetInteger() == 0) {
    return;
  }
...
// FIXME
  ...
  if (aaMode == ANTI_ALIASING_SMAA_1X) {
    ...
    globalImages->smaaInputImage->CopyFramebuffer(0, 0, screenWidth,
                                                  screenHeight);
...
#if 1
    // globalImages->smaaEdgesImage->CopyFramebuffer( viewport.x1, viewport.y1,
...
#if 1
    // GL_SelectTexture( 0 );
    // globalImages->smaaBlendImage->CopyFramebuffer( viewport.x1, viewport.y1,
    // viewport.GetWidth(), viewport.GetHeight() );
