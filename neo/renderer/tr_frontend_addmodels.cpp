...
idCVar r_skipStaticShadows("r_skipStaticShadows", "0",
                           CVAR_RENDERER | CVAR_BOOL, "skip static shadows");
idCVar r_skipDynamicShadows("r_skipDynamicShadows", "0",
                            CVAR_RENDERER | CVAR_BOOL, "skip dynamic shadows");
idCVar r_useParallelAddModels("r_useParallelAddModels", "1",
                              CVAR_RENDERER | CVAR_BOOL,
                              "add all models in parallel with jobs");
idCVar r_useParallelAddShadows("r_useParallelAddShadows", "1",
                               CVAR_RENDERER | CVAR_INTEGER,
                               "0 = off, 1 = threaded", 0, 1);
idCVar r_useShadowPreciseInsideTest(
    "r_useShadowPreciseInsideTest", "1", CVAR_RENDERER | CVAR_BOOL,
    "use a precise and more expensive test to determine whether the view is "
    "inside a shadow volume");
idCVar r_cullDynamicShadowTriangles(
    "r_cullDynamicShadowTriangles", "1", CVAR_RENDERER | CVAR_BOOL,
    "cull occluder triangles that are outside the light frustum so they do not "
    "contribute to the dynamic shadow volume");
idCVar r_cullDynamicLightTriangles(
    "r_cullDynamicLightTriangles", "1", CVAR_RENDERER | CVAR_BOOL,
    "cull surface triangles that are outside the light frustum so they do not "
    "get rendered for interactions");
idCVar r_forceShadowCaps("r_forceShadowCaps", "0", CVAR_RENDERER | CVAR_BOOL,
                         "0 = skip rendering shadow caps if view is outside "
                         "shadow volume, 1 = always render shadow caps");
// RB begin
idCVar r_forceShadowMapsOnAlphaTestedSurfaces(
    "r_forceShadowMapsOnAlphaTestedSurfaces", "1", CVAR_RENDERER | CVAR_BOOL,
    "0 = same shadowing as with stencil shadows, 1 = ignore noshadows for "
    "alpha tested materials");
// RB end
// foresthale 2014-11-24: cvar to control the material lod flags - this is the
// distance at which a mesh switches from lod1 to lod2, where lod3 will appear
// at this distance *2, lod4 at *4, and persistentLOD keyword will disable the
// max distance check (thus extending this LOD to all further distances, rather
// than disappearing)
idCVar r_lodMaterialDistance(
    "r_lodMaterialDistance", "500", CVAR_RENDERER | CVAR_FLOAT,
    "surfaces further than this distance will use lower quality versions (if "
    "their material uses the lod1-4 keywords, persistentLOD disables the max "
    "distance checks)");
...
bool R_IssueEntityDefCallback(idRenderEntityLocal* def) {
  ...
  if (r_checkBounds.GetBool()) {
    if (oldBounds[0][0] >
            def->localReferenceBounds[0][0] + CHECK_BOUNDS_EPSILON ||
        oldBounds[0][1] >
            def->localReferenceBounds[0][1] + CHECK_BOUNDS_EPSILON ||
        oldBounds[0][2] >
            def->localReferenceBounds[0][2] + CHECK_BOUNDS_EPSILON ||
        oldBounds[1][0] <
            def->localReferenceBounds[1][0] - CHECK_BOUNDS_EPSILON ||
        oldBounds[1][1] <
            def->localReferenceBounds[1][1] - CHECK_BOUNDS_EPSILON ||
        oldBounds[1][2] <
            def->localReferenceBounds[1][2] - CHECK_BOUNDS_EPSILON) {
...
This is also called by the game code for idRenderWorldLocal::ModelTrace(), and
idRenderWorldLocal::Trace() which is bad for performance...
...
idRenderModel* R_EntityDefDynamicModel(idRenderEntityLocal* def) {
  ...
  if (def->dynamicModel != NULL && model->DepthHack() != 0.0f &&
      tr.viewDef != NULL) {
    idPlane eye, clip;
    idVec3 ndc;
...
===================
*/
void R_SetupDrawSurfShader(drawSurf_t* drawSurf, const idMaterial* shader,
  ...
  if (likely(constRegs != NULL)) {
    ...
    if (unlikely(renderEntity->referenceShader != NULL)) {
      ...
      renderEntity->referenceShader->EvaluateRegisters(
          refRegs, renderEntity->shaderParms,
          tr.viewDef->renderView.shaderParms,
          tr.viewDef->renderView.time[renderEntity->timeGroup] * 0.001f,
    ...
    float* regs = (float*)R_FrameAlloc(
        shader->GetNumRegisters() * sizeof(float), FRAME_ALLOC_SHADER_REGISTER);
    drawSurf->shaderRegisters = regs;

    // process the shader expressions for conditionals / color / texcoords
    shader->EvaluateRegisters(
        regs, shaderParms, tr.viewDef->renderView.shaderParms,
        tr.viewDef->renderView.time[renderEntity->timeGroup] * 0.001f,
        renderEntity->referenceSound);
  }
}
...
===================
*/
void R_SetupDrawSurfJoints(drawSurf_t* drawSurf, const srfTriangles_t* tri,
                           const idMaterial* shader) {
  // RB: added check wether GPU skinning is available at all
  if (tri->staticModelWithJoints == NULL || !r_useGPUSkinning.GetBool() ||
      !glConfig.gpuSkinningAvailable) {
  ...
  if (!vertexCache.CacheIsCurrent(model->jointsInvertedBuffer)) {
    ...
    model->jointsInvertedBuffer =
        vertexCache.AllocJoint(model->jointsInverted, model->numInvertedJoints);
  }
...
void R_AddSingleModel(viewEntity_t* vEntity) {
  // we will add all interaction surfs here, to be chained to the lights in
  // later serial code
  ...
  SCOPED_PROFILE_EVENT(renderEntity->hModel == NULL
                           ? "Unknown Model"
                           : renderEntity->hModel->Name());

  // calculate the znear for testing whether or not the view is inside a shadow
  // projection
  const float znear = (viewDef->renderView.cramZNear)
                          ? (r_znear.GetFloat() * 0.25f)
                          : r_znear.GetFloat();
  ...
  const bool addInteractions =
      modelIsVisible &&
      (!viewDef->isXraySubview || entityDef->parms.xrayIndex == 2);
  ...
  if (renderEntity->hModel == NULL ||
    ...
    for (viewLight_t* vLight = viewDef->viewLights; vLight != NULL;
      ...
      if (vLight->entityInteractionState != NULL) {
        ...
        if (vLight->entityInteractionState[entityIndex] ==
            viewLight_t::INTERACTION_YES) {
          contactedLights[numContactedLights] = vLight;
          staticInteractions[numContactedLights] =
              world->interactionTable[vLight->lightDef->index *
                                          world->interactionTableWidth +
                                      entityIndex];
      ...
      if (!lightDef->globalLightBounds.IntersectsBounds(
              entityDef->globalReferenceBounds)) {
        continue;
      }

      if (R_CullModelBoundsToLight(lightDef, entityDef->localReferenceBounds,
                                   entityDef->modelRenderMatrix)) {
        continue;
      }
      ...
      if (!modelIsVisible) {
        ...
        if (lightDef->areaNum != -1) {
          ...
          // the light center (it is behind a solid, closed door), we can ignore
          // it
          bool areasConnected = false;
          for (areaReference_t* ref = entityDef->entityRefs; ref != NULL;
               ref = ref->ownerNext) {
            if (world->AreasAreConnected(lightDef->areaNum, ref->area->areaNum,
                                         PS_BLOCK_VIEW)) {
              areasConnected = true;
              break;
            }
        ...
        R_ShadowBounds(entityDef->globalReferenceBounds,
                       lightDef->globalLightBounds, lightDef->globalLightOrigin,
                       shadowBounds);

        // this doesn't say that the shadow can't effect anything, only that it
        // can't effect anything in the view
        if (idRenderMatrix::CullBoundsToMVP(viewDef->worldSpace.mvp,
                                            shadowBounds)) {
          continue;
        }
      }
  ...
  if (modelIsVisible) {
    ...
    if (entityDef->decals != NULL && !r_skipDecals.GetBool()) {
      ...
      for (unsigned int i = 0; i < numDrawSurfs; i++) {
        drawSurf_t* decalDrawSurf =
            entityDef->decals->CreateDecalDrawSurf(vEntity, i);
    ...
    if (entityDef->overlays != NULL && !r_skipOverlays.GetBool()) {
      ...
      for (unsigned int i = 0; i < numDrawSurfs; i++) {
        drawSurf_t* overlayDrawSurf =
            entityDef->overlays->CreateOverlayDrawSurf(vEntity, model, i);
  ...
  for (areaReference_t* ref = entityDef->entityRefs; ref != NULL;
    ...
    if (ref->area->lightGrid.lightGridPoints.Num() && lightGridImage &&
        !lightGridImage->IsDefaulted()) {
      vEntity->useLightGrid = true;
      vEntity->lightGridAtlasImage = lightGridImage;
      vEntity->lightGridAtlasSingleProbeSize =
          ref->area->lightGrid.imageSingleProbeSize;
  ...
  memcpy(vEntity->modelMatrix, entityDef->modelMatrix,
         sizeof(vEntity->modelMatrix));
  R_MatrixMultiply(entityDef->modelMatrix, viewDef->worldSpace.modelViewMatrix,
                   vEntity->modelViewMatrix);

  idRenderMatrix viewMat;
  idRenderMatrix::Transpose(*(idRenderMatrix*)vEntity->modelViewMatrix,
                            viewMat);
  idRenderMatrix::Multiply(viewDef->projectionRenderMatrix, viewMat,
                           vEntity->mvp);
  if (renderEntity->weaponDepthHack) {
    idRenderMatrix::ApplyDepthHack(vEntity->mvp);
  }
  if (renderEntity->modelDepthHack != 0.0f) {
    idRenderMatrix::ApplyModelDepthHack(vEntity->mvp,
                                        renderEntity->modelDepthHack);
  }

  // local light and view origins are used to determine if the view is
  // definitely outside an extruded shadow volume, which means we can skip
  // drawing the end caps
  idVec3 localViewOrigin;
  R_GlobalPointToLocal(vEntity->modelMatrix, viewDef->renderView.vieworg,
                       localViewOrigin);
  ...
  for (int surfaceNum = 0; surfaceNum < model->NumSurfaces(); surfaceNum++) {
    ...
    if (r_singleSurface.GetInteger() >= 0 &&
        surfaceNum != r_singleSurface.GetInteger()) {
      continue;
    }
    ...
    if (shader->IsLOD()) {
      // foresthale 2014-11-24: calculate the bounds and get the distance from
      // camera to bounds
      idBounds& localBounds = tri->bounds;
      if (tri->staticModelWithJoints) {
        // skeletal models have difficult to compute bounds for surfaces, so use
        // the whole entity
      ...
      if (!shader->IsLODVisibleForDistance(distance,
                                           r_lodMaterialDistance.GetFloat())) {
        continue;
      }
    }

    // foresthale 2014-09-01: don't skip surfaces that use the "forceShadows"
    // flag
    ...
    if (entityDef->parms.customShader != NULL) {
      // this is sort of a hack, but causes deformed surfaces to map to empty
      // surfaces, so the item highlight overlay doesn't highlight the
      // autosprite surface
      if (shader->Deform()) {
        continue;
      }
    ...
    if (r_checkBounds.GetBool()) {
      for (int j = 0; j < tri->numVerts; j++) {
        ...
        for (k = 0; k < 3; k++) {
          ...
          if (tri->verts[j].xyz[k] > entityDef->localReferenceBounds[1][k] +
                                         CHECK_BOUNDS_EPSILON ||
              tri->verts[j].xyz[k] < entityDef->localReferenceBounds[0][k] -
                                         CHECK_BOUNDS_EPSILON) {
    ...
    // individual surfaces.
    const bool surfaceDirectlyVisible =
        modelIsVisible &&
        !idRenderMatrix::CullBoundsToMVP(vEntity->mvp, tri->bounds);

    // RB: added check wether GPU skinning is available at all
    const bool gpuSkinned =
        (tri->staticModelWithJoints != NULL && r_useGPUSkinning.GetBool() &&
         glConfig.gpuSkinningAvailable);
    // RB end
    ...
    if (shader->IsDrawn()) {
      ...
      if (surfaceDirectlyVisible) {
        // make sure we have an ambient cache and all necessary normals /
        // tangents
        if (!vertexCache.CacheIsCurrent(tri->indexCache)) {
          tri->indexCache =
              vertexCache.AllocIndex(tri->indexes, tri->numIndexes);
        }

        if (!vertexCache.CacheIsCurrent(tri->ambientCache)) {
          // we are going to use it for drawing, so make sure we have the
          ...
          tri->ambientCache =
              vertexCache.AllocVertex(tri->verts, tri->numVerts);
        }

        // add the surface for drawing
        // we can re-use some of the values for light interaction surfaces
        baseDrawSurf = (drawSurf_t*)R_FrameAlloc(sizeof(*baseDrawSurf),
        ...
        if (shaderDeform != DFRM_NONE) {
          ...
          if (deformDrawSurf != NULL) {
            ...
            for (drawSurf_t *surf = deformDrawSurf, *next = NULL; surf != NULL;
                 surf = next) {
        ...
        // However, particles are rendered in conjunction with the source
        // surface.
        if (shaderDeform == DFRM_NONE || shaderDeform == DFRM_PARTICLE ||
            shaderDeform == DFRM_PARTICLE2) {
          // copy verts and indexes to this frame's hardware memory if they
    ...
    //----------------------------------------
    for (int contactedLight = 0; contactedLight < numContactedLights;
         contactedLight++) {
      ...
      if (interaction > INTERACTION_EMPTY && interaction->staticInteraction) {
        ...
        if (R_CullModelBoundsToLight(lightDef, tri->bounds,
                                     entityDef->modelRenderMatrix)) {
          continue;
        }
      }
      ...
      // Calculate the local light origin to determine if the view is inside the
      // shadow projection and to calculate the triangle facing for dynamic
      // shadow volumes.
      ...
      if (addInteractions && surfaceDirectlyVisible &&
          shader->ReceivesLighting()) {
        // static interactions can commonly find that no triangles from a
        // surface contact the light, even when the total model does
        if (surfInter == NULL || surfInter->lightTrisIndexCache > 0) {
          // make sure we have a valid shader register even if we didn't
          // generate a drawn mesh above
          ...
          if (shaderRegisters != NULL) {
            ...
            drawSurf_t* lightDrawSurf = (drawSurf_t*)R_FrameAlloc(
                sizeof(*lightDrawSurf), FRAME_ALLOC_DRAW_SURFACE);
            ...
            if (surfInter != NULL) {
              ...
              // throw the entire source surface at it without any per-triangle
              // culling
              ...
              // motorsep 11-09-2014; added && shader->SurfaceCastsShadow() per
              // Lordhavoc's recommendation; should skip shadows calculation for
              // surfaces with noShadows material flag when using shadow volumes
              if (r_cullDynamicLightTriangles.GetBool() &&
                  !r_skipDynamicShadows.GetBool() &&
                  !r_useShadowMapping.GetBool() &&
                  shader->SurfaceCastsShadow()) {
                vertCacheHandle_t lightIndexCache =
                    vertexCache.AllocIndex(NULL, lightDrawSurf->numIndexes);
                if (vertexCache.CacheIsCurrent(lightIndexCache)) {
                  lightDrawSurf->indexCache = lightIndexCache;

                  dynamicShadowParms =
                      (dynamicShadowVolumeParms_t*)R_FrameAlloc(
                          sizeof(dynamicShadowParms[0]),
                  ...
                  dynamicShadowParms->joints =
                      gpuSkinned ? tri->staticModelWithJoints->jointsInverted
                                 : NULL;
                  dynamicShadowParms->numJoints =
                      gpuSkinned ? tri->staticModelWithJoints->numInvertedJoints
                                 : 0;
                  ...
                  idRenderMatrix::Multiply(
                      ...
                      entityDef->modelRenderMatrix,
                      dynamicShadowParms->localLightProject);
                  ...
                  dynamicShadowParms->lightIndices =
                      (triIndex_t*)vertexCache.MappedIndexBuffer(
                          lightIndexCache);
                  dynamicShadowParms->maxLightIndices =
                      lightDrawSurf->numIndexes;
                  dynamicShadowParms->numLightIndices =
                      &lightDrawSurf->numIndexes;
                  dynamicShadowParms->renderZFail = NULL;
                  dynamicShadowParms->shadowZMin = NULL;
                  dynamicShadowParms->shadowZMax = NULL;
                  dynamicShadowParms->shadowVolumeState =
                      &lightDrawSurf->shadowVolumeState;
            ...
            lightDrawSurf->scissorRect =
                vLight->scissorRect;  // interactionScissor;
      ...
      if (!shader->SurfaceCastsShadow() &&
          !(r_useShadowMapping.GetBool() &&
            r_forceShadowMapsOnAlphaTestedSurfaces.GetBool() &&
            shader->Coverage() == MC_PERFORATED)) {
        continue;
      }
#else
      // Steel Storm 2 behaviour - this destroys many alpha tested shadows in
      // vanilla BFG

      // motorsep 11-08-2014; if r_forceShadowMapsOnAlphaTestedSurfaces is 0
      // when shadow mapping is on, don't render shadows from all alphaTest
      // surfaces. Useful as global performance booster for old GPUs to disable
      // shadows from grass/foliage/etc.
      ...
      if (!shader->SurfaceCastsShadow()) {
        // motorsep 11-08-2014; if r_forceShadowMapsOnAlphaTestedSurfaces is 1
        // when shadow mapping is on, check if a surface IS NOT alphaTested and
        // has "noShadows" global key; or if a surface IS alphaTested and has
        // "noShadows" global key; if either is true, don't make surfaces cast
        // shadow maps.
        if (r_useShadowMapping.GetBool()) {
          if (shader->Coverage() != MC_PERFORATED &&
              shader->TestMaterialFlag(MF_NOSHADOWS)) {
            continue;
      ...
      if (tri->silEdges == NULL) {
        continue;  // can happen for beam models (shouldn't use a shadow casting
                   // material, though...)
      }
      ...
      if (r_useShadowMapping.GetBool()) {
        ...
        // shader->ReceivesLighting() )
        {
          ...
          if (surfInter == NULL || surfInter->lightTrisIndexCache > 0) {
            // create a drawSurf for this interaction
            drawSurf_t* shadowDrawSurf = (drawSurf_t*)R_FrameAlloc(
                sizeof(*shadowDrawSurf), FRAME_ALLOC_DRAW_SURFACE);
            ...
            // shadowDrawSurf->linkChain = shader->TestMaterialFlag(
            // MF_NOSELFSHADOW ) ? &vLight->localShadows :
      ...
      if (lightDef->parms.prelightModel && lightDef->lightHasMoved == false &&
          entityDef->parms.hModel->IsStaticWorldModel() &&
          !r_skipPrelightShadows.GetBool()) {
        // static light / world model shadow interacitons
        // are always captured in the prelight shadow volume
        continue;
      }

      // If the shadow is drawn (or translucent), but the model isn't, we must
      // include the shadow caps because we may be able to see into the shadow
      // volume even though the view is outside it. This happens for the player
      // world weapon and possibly some animations in multiplayer.
      const bool forceShadowCaps =
          !addInteractions || r_forceShadowCaps.GetBool();
      ...
      if (surfInter != NULL) {
        ...
        shadowDrawSurf->scissorRect =
            vLight->scissorRect;  // default to the light scissor and light
                                  // depth bounds
        shadowDrawSurf->shadowVolumeState =
            SHADOWVOLUME_DONE;  // assume the shadow volume is done in case
                                // r_skipStaticShadows is set

        if (!r_skipStaticShadows.GetBool() && !r_useShadowMapping.GetBool()) {
          staticShadowVolumeParms_t* staticShadowParms =
              (staticShadowVolumeParms_t*)R_FrameAlloc(
                  sizeof(staticShadowParms[0]),
                  FRAME_ALLOC_SHADOW_VOLUME_PARMS);
          ...
          staticShadowParms->numShadowIndicesWithCaps =
              surfInter->numShadowIndexes;
          staticShadowParms->numShadowIndicesNoCaps =
              surfInter->numShadowIndexesNoCaps;
          ...
          staticShadowParms->useShadowPreciseInsideTest =
              r_useShadowPreciseInsideTest.GetBool();
          staticShadowParms->useShadowDepthBounds =
              r_useShadowDepthBounds.GetBool();
          ...
          staticShadowParms->shadowVolumeState =
              &shadowDrawSurf->shadowVolumeState;
        ...
        // When CPU skinning the dynamic shadow verts of a dynamic model may not
        // have been copied to buffer memory yet.
        if (!vertexCache.CacheIsCurrent(tri->shadowCache)) {
          assert(!gpuSkinned);  // the shadow cache should be static when using
                                // GPU skinning
          // Extracts just the xyz values from a set of full size drawverts, and
          // duplicates them with w set to 0 and 1 for the vertex program to
          // project. This is constant for any number of lights, the vertex
          // program takes care of projecting the verts to infinity for a
          // particular light.
          tri->shadowCache = vertexCache.AllocVertex(NULL, tri->numVerts * 2,
                                                     sizeof(idShadowVert));
          idShadowVert* shadowVerts =
              (idShadowVert*)vertexCache.MappedVertexBuffer(tri->shadowCache);
          idShadowVert::CreateShadowCache(shadowVerts, tri->verts,
                                          tri->numVerts);
        }

        const int maxShadowVolumeIndexes =
            tri->numSilEdges * 6 + tri->numIndexes * 2;

        shadowDrawSurf->numIndexes = 0;
        shadowDrawSurf->indexCache =
            vertexCache.AllocIndex(NULL, maxShadowVolumeIndexes);
        shadowDrawSurf->shadowCache = tri->shadowCache;
        shadowDrawSurf->scissorRect =
            vLight->scissorRect;  // default to the light scissor and light
                                  // depth bounds
        shadowDrawSurf->shadowVolumeState =
            SHADOWVOLUME_DONE;  // assume the shadow volume is done in case the
                                // index cache allocation failed

        // if the index cache was successfully allocated then setup the parms to
        // create a shadow volume in parallel
        if (vertexCache.CacheIsCurrent(shadowDrawSurf->indexCache) &&
            !r_skipDynamicShadows.GetBool() && !r_useShadowMapping.GetBool()) {
          // if the parms were not already allocated for culling interaction
          // triangles to the light frustum
          if (dynamicShadowParms == NULL) {
            dynamicShadowParms = (dynamicShadowVolumeParms_t*)R_FrameAlloc(
                sizeof(dynamicShadowParms[0]), FRAME_ALLOC_SHADOW_VOLUME_PARMS);
          } else {
            // the shadow volume will be rendered first so when the interaction
            // surface is drawn the triangles have been culled for sure
          ...
          dynamicShadowParms->numSilEdges = tri->numSilEdges;
          dynamicShadowParms->joints =
              gpuSkinned ? tri->staticModelWithJoints->jointsInverted : NULL;
          dynamicShadowParms->numJoints =
              gpuSkinned ? tri->staticModelWithJoints->numInvertedJoints : 0;
          dynamicShadowParms->triangleBounds = tri->bounds;
          dynamicShadowParms->triangleMVP = vEntity->mvp;
          dynamicShadowParms->localLightOrigin = localLightOrigin;
          dynamicShadowParms->localViewOrigin = localViewOrigin;
          idRenderMatrix::Multiply(vLight->lightDef->baseLightProject,
                                   entityDef->modelRenderMatrix,
                                   dynamicShadowParms->localLightProject);
          dynamicShadowParms->zNear = znear;
          dynamicShadowParms->lightZMin = vLight->scissorRect.zmin;
          dynamicShadowParms->lightZMax = vLight->scissorRect.zmax;
          dynamicShadowParms->cullShadowTrianglesToLight =
              r_cullDynamicShadowTriangles.GetBool();
          dynamicShadowParms->forceShadowCaps = forceShadowCaps;
          dynamicShadowParms->useShadowPreciseInsideTest =
              r_useShadowPreciseInsideTest.GetBool();
          dynamicShadowParms->useShadowDepthBounds =
              r_useShadowDepthBounds.GetBool();
          dynamicShadowParms->tempFacing = NULL;
          dynamicShadowParms->tempCulled = NULL;
          dynamicShadowParms->tempVerts = NULL;
          dynamicShadowParms->indexBuffer = NULL;
          dynamicShadowParms->shadowIndices =
              (triIndex_t*)vertexCache.MappedIndexBuffer(
                  shadowDrawSurf->indexCache);
          dynamicShadowParms->maxShadowIndices = maxShadowVolumeIndexes;
          dynamicShadowParms->numShadowIndices = &shadowDrawSurf->numIndexes;
          // dynamicShadowParms->lightIndices may have already been set for the
          // interaction surface dynamicShadowParms->maxLightIndices may have
          // already been set for the interaction surface
          // dynamicShadowParms->numLightIndices may have already been set for
          ...
          // if the parms we not already linked for culling interaction
          // triangles to the light frustum
      ...
      // determine which linked list to add the shadow surface to
      shadowDrawSurf->linkChain = shader->TestMaterialFlag(MF_NOSELFSHADOW)
                                      ? &vLight->localShadows
                                      : &vLight->globalShadows;
      shadowDrawSurf->nextOnLight = vEntity->drawSurfs;
      vEntity->drawSurfs = shadowDrawSurf;
...
void R_AddModels() {
  ...
  if (r_useParallelAddModels.GetBool()) {
    for (viewEntity_t* vEntity = tr.viewDef->viewEntitys; vEntity != NULL;
  ...
  //-------------------------------------------------
  if ((r_skipStaticShadows.GetBool() && r_skipDynamicShadows.GetBool()) ||
      r_useShadowMapping.GetBool()) {
    // no shadow volumes were chained to any entity, all are in DONE state, we
    // don't need to Submit() or Wait()
  } else {
    if (r_useParallelAddShadows.GetInteger() == 1) {
      for (viewEntity_t* vEntity = tr.viewDef->viewEntitys; vEntity != NULL;
           vEntity = vEntity->next) {
        for (staticShadowVolumeParms_t* shadowParms =
                 vEntity->staticShadowVolumes;
             shadowParms != NULL; shadowParms = shadowParms->next) {
          tr.frontEndJobList->AddJob((jobRun_t)StaticShadowVolumeJob,
                                     shadowParms);
        }
        for (dynamicShadowVolumeParms_t* shadowParms =
                 vEntity->dynamicShadowVolumes;
             shadowParms != NULL; shadowParms = shadowParms->next) {
          tr.frontEndJobList->AddJob((jobRun_t)DynamicShadowVolumeJob,
                                     shadowParms);
        }
        vEntity->staticShadowVolumes = NULL;
        vEntity->dynamicShadowVolumes = NULL;
      ...
      tr.frontEndJobList->Submit();
      // wait here otherwise the shadow volume index buffer may be unmapped
      // before all shadow volumes have been constructed
      tr.frontEndJobList->Wait();
    } else {
  ...
  tr.viewDef->maxDrawSurfs =
      0;  // will be set to INITIAL_DRAWSURFS on R_LinkDrawSurfToView
