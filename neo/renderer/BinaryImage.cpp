...
idCVar image_highQualityCompression("image_highQualityCompression", "0",
                                    CVAR_BOOL,
                                    "Use high quality (slow) compression");
idCVar r_useHighQualitySky("r_useHighQualitySky", "1", CVAR_BOOL | CVAR_ARCHIVE,
                           "Use high quality skyboxes");
...
void idBinaryImage::Load2DFromMemory(int width, int height,
                                     const byte* pic_const, int numLevels,
  ...
  for (int level = 0; level < images.Num(); level++) {
    ...
    // Images that are going to be DXT compressed and aren't multiples of 4 need
    // to be padded out before compressing.
    byte* dxtPic = pic;
    int dxtWidth = 0;
    int dxtHeight = 0;
    if (textureFormat == FMT_DXT5 || textureFormat == FMT_DXT1) {
      if ((scaledWidth & 3) || (scaledHeight & 3)) {
        ...
        for (int i = 0; i < scaledHeight; i++) {
          memcpy(dxtPic + i * dxtWidth * 4, pic + i * scaledWidth * 4,
                 scaledWidth * 4);
        }
      } else {
        dxtPic = pic;
    ...
    if (textureFormat == FMT_DXT1) {
      ...
      if (image_highQualityCompression.GetBool()) {
        commonLocal.LoadPacifierBinarizeInfo(
            va("(%d x %d) - DXT1HQ", width, height));

        dxt.CompressImageDXT1HQ(dxtPic, img.data, dxtWidth, dxtHeight);
      } else {
        commonLocal.LoadPacifierBinarizeInfo(
            va("(%d x %d) - DXT1Fast", width, height));
      ...
      if (colorFormat == CFM_NORMAL_DXT5) {
        if (image_highQualityCompression.GetBool()) {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - NormalMapDXT5HQ", width, height));

          dxt.CompressNormalMapDXT5HQ(dxtPic, img.data, dxtWidth, dxtHeight);
        } else {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - NormalMapDXT5Fast", width, height));
        ...
        if (image_highQualityCompression.GetBool()) {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - YCoCgDXT5HQ", width, height));

          dxt.CompressYCoCgDXT5HQ(dxtPic, img.data, dxtWidth, dxtHeight);
        } else {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - YCoCgDXT5Fast", width, height));
        ...
        if (image_highQualityCompression.GetBool()) {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT5HQ", width, height));

          dxt.CompressImageDXT5HQ(dxtPic, img.data, dxtWidth, dxtHeight);
        } else {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT5Fast", width, height));
...
========================
*/
void idBinaryImage::Load2DAtlasMipchainFromMemory(
    int width, int height, const byte* pic_const, int numLevels,
    textureFormat_t& textureFormat, textureColor_t& colorFormat) {
...
#if 1
        pic[(sy * scaledWidth + sx) * 4 + 0] =
            sourcePic[(y * width + x) * 4 + 0];
        pic[(sy * scaledWidth + sx) * 4 + 1] =
            sourcePic[(y * width + x) * 4 + 1];
        pic[(sy * scaledWidth + sx) * 4 + 2] =
            sourcePic[(y * width + x) * 4 + 2];
        pic[(sy * scaledWidth + sx) * 4 + 3] =
            sourcePic[(y * width + x) * 4 + 3];
#else
...
DXT Compression requres a complete 4x4 block, even if the GPU will only be
sampling a subset of it, so pad to 4x4 with replicated texels to maximize
compression.
========================
*/
static void PadImageTo4x4(const byte* src, int width, int height,
                          byte dest[64]) {
  // we probably will need to support this for non-square images, but I'll
  // address that when needed
...
========================
*/
void idBinaryImage::LoadCubeFromMemory(int width, const byte* pics[6],
                                       int numLevels,
                                       textureFormat_t& textureFormat,
                                       bool gammaMips) {
  ...
  for (int side = 0; side < 6; side++) {
    ...
    for (int level = 0; level < fileData.numLevels; level++) {
      ...
      const byte* padSrc;
      if (scaledWidth < 4 &&
          (textureFormat == FMT_DXT1 || textureFormat == FMT_DXT5)) {
      ...
      if (textureFormat == FMT_DXT1) {
        ...
        if (image_highQualityCompression.GetBool()) {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT1HQ", width, width));

          dxt.CompressImageDXT1HQ(padSrc, img.data, padSize, padSize);
        } else {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT1Fast", width, width));
        ...
        if (image_highQualityCompression.GetBool()) {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT5HQ", width, width));

          dxt.CompressImageDXT5HQ(padSrc, img.data, padSize, padSize);
        } else {
          commonLocal.LoadPacifierBinarizeInfo(
              va("(%d x %d) - DXT5Fast", width, width));
...
ID_TIME_T idBinaryImage::WriteGeneratedFile(ID_TIME_T sourceFileTime) {
  ...
  if (file == NULL) {
    idLib::Warning("idBinaryImage: Could not open file '%s'",
                   binaryFileName.c_str());
    return FILE_NOT_FOUND_TIMESTAMP;
  }
  idLib::Printf("Writing %s: %ix%i\n", binaryFileName.c_str(), fileData.width,
                fileData.height);
...
==========================
*/
bool idBinaryImage::LoadFromGeneratedFile(idFile* bFile,
  ...
  if (!fileSystem->InProductionMode() &&
      ...
      (sourceTimeStamp != fileData.sourceFileTime)) {
    return false;
  }
  // RB end
  ...
  for (int i = 0; i < numImages; i++) {
    ...
    assert(img.dataSize >= img.width * img.height *
                               BitsForFormat((textureFormat_t)fileData.format) /
                               8);
#if defined(__APPLE__) && defined(USE_VULKAN)
    int imgfile_dataSize = img.dataSize;
    // SRS - Allocate 2x memory to prepare for in-place conversion from
    // FMT_RGB565 to FMT_RGBA8
    ...
    // SRS - Convert FMT_RGB565 16-bits to FMT_RGBA8 32-bits in place using
    // pre-allocated space
    if ((textureFormat_t)fileData.format == FMT_RGB565) {
      // SRS - Make sure we have an integer number of RGBA8 storage slots
      assert(img.dataSize % 4 == 0);
      for (int pixelIndex = img.dataSize / 2 - 2; pixelIndex >= 0;
           pixelIndex -= 2) {
#if 1
        // SRS - Option 1: Scale and shift algorithm
        uint16 pixelValue_rgb565 =
            img.data[pixelIndex + 0] << 8 | img.data[pixelIndex + 1];
        img.data[pixelIndex * 2 + 0] =
            (((pixelValue_rgb565) >> 11) * 527 + 23) >> 6;
        img.data[pixelIndex * 2 + 1] =
            (((pixelValue_rgb565 & 0x07E0) >> 5) * 259 + 33) >> 6;
        img.data[pixelIndex * 2 + 2] =
            (((pixelValue_rgb565 & 0x001F)) * 527 + 23) >> 6;
#else
        ...
        img.data[pixelIndex * 2 + 0] =
            (pixelValue_rgb565_hi & 0xF8) | (pixelValue_rgb565_hi >> 5);
        img.data[pixelIndex * 2 + 1] = (pixelValue_rgb565_hi << 5) |
                                       ((pixelValue_rgb565_lo & 0xE0) >> 3) |
                                       ((pixelValue_rgb565_hi & 0x07) >> 1);
        img.data[pixelIndex * 2 + 2] =
            (pixelValue_rgb565_lo << 3) | ((pixelValue_rgb565_lo & 0x1F) >> 2);
#endif
