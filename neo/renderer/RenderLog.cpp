...
void PC_BeginNamedEvent(const char* szName, const idVec4& color) {
  ...
  if (glConfig.gremedyStringMarkerAvailable && glConfig.khronosDebugAvailable) {
    glPushDebugGroup(GL_DEBUG_SOURCE_APPLICATION_ARB, 0,
                     GLsizei(strlen(szName)), szName);
  }
#endif
...
void idRenderLog::OpenMainBlock(renderLogMainBlock_t block) {
  ...
  if (glConfig.timerQueryAvailable) {
    ...
    // SRS - For OSX use elapsed time query for Apple OpenGL 4.1 using
    // GL_TIME_ELAPSED vs GL_TIMESTAMP (which is not implemented on OSX) SRS -
    // OSX AMD drivers have a rendering bug (flashing colours) with an elasped
    // time query when Shadow Mapping is on - turn off query for that case
    // unless r_skipAMDWorkarounds is set
    if (!r_useShadowMapping.GetBool() || glConfig.vendor != VENDOR_AMD ||
        r_skipAMDWorkarounds.GetBool()) {
      if (glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity]
                                                  [mainBlock * 2 + 1] == 0) {
        glGenQueries(
            1, &glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity]
                                                        [mainBlock * 2 + 1]);
      }
...
========================
*/
void idRenderLog::CloseBlock() { PC_EndNamedEvent(); }
// RB end