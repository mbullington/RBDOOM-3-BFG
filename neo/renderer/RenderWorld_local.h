...
// however this 2021 version features Spherical Harmonics instead of ambient +
// directed color
...
class LightGrid {
 public:
  ...
  void SetupLightGrid(const idBounds& bounds, const char* baseName,
                      const idRenderWorld* world, const idVec3& gridSize,
                      int _area, int totalAreas, int maxProbes,
                      bool printToConsole);
...
typedef struct portalArea_s {
  int areaNum;
  int connectedAreaNum[NUM_PORTAL_ATTRIBUTES];  // if two areas have matching
                                                // connectedAreaNum, they are
  // not separated by a portal with the apropriate PS_BLOCK_* blockingBits

  idBounds
      globalBounds;  // RB: AABB of the BSP area used for light grid density
...
class idRenderWorldLocal : public idRenderWorld {
 public:
  ...
  virtual void
  ResetLocalRenderModels();  // Fixes a crash when switching between expansion
                             // packs in Doom3:BFG
  ...
  virtual void UpdateEnvprobeDef(qhandle_t envprobeHandle,
                                 const renderEnvironmentProbe_t* ep);
  virtual void FreeEnvprobeDef(qhandle_t envprobeHandle);
  virtual const renderEnvironmentProbe_t* GetRenderEnvprobe(
      qhandle_t envprobeHandle) const;
  // RB end
  ...
  virtual void ProjectDecalOntoWorld(const idFixedWinding& winding,
                                     ...
                                     const bool parallel, const float fadeDepth,
                                     const idMaterial* material,
                                     const int startTime);
  virtual void ProjectDecal(qhandle_t entityHandle,
                            const idFixedWinding& winding,
                            const idVec3& projectionOrigin, const bool parallel,
                            const float fadeDepth, const idMaterial* material,
                            const int startTime);
  ...
  virtual int BoundsInAreas(const idBounds& bounds, int* areas,
                            int maxAreas) const;
  ...
  virtual guiPoint_t GuiTrace(qhandle_t entityHandle, const idVec3 start,
                              const idVec3 end) const;
  virtual bool ModelTrace(modelTrace_t& trace, qhandle_t entityHandle,
                          const idVec3& start, const idVec3& end,
                          const float radius) const;
  virtual bool Trace(modelTrace_t& trace, const idVec3& start,
                     const idVec3& end, const float radius,
                     bool skipDynamic = true, bool skipPlayer = false) const;
  virtual bool FastWorldTrace(modelTrace_t& trace, const idVec3& start,
                              const idVec3& end) const;

  virtual void DebugClearLines(int time);
  virtual void DebugLine(const idVec4& color, const idVec3& start,
                         const idVec3& end, const int lifetime = 0,
                         const bool depthTest = false);
  virtual void DebugArrow(const idVec4& color, const idVec3& start,
                          const idVec3& end, int size, const int lifetime = 0);
  virtual void DebugWinding(const idVec4& color, const idWinding& w,
                            const idVec3& origin, const idMat3& axis,
                            const int lifetime = 0,
                            const bool depthTest = false);
  virtual void DebugCircle(const idVec4& color, const idVec3& origin,
                           const idVec3& dir, const float radius,
                           const int numSteps, const int lifetime = 0,
                           const bool depthTest = false);
  virtual void DebugSphere(const idVec4& color, const idSphere& sphere,
                           const int lifetime = 0, bool depthTest = false);
  virtual void DebugBounds(const idVec4& color, const idBounds& bounds,
                           const idVec3& org = vec3_origin,
                           const int lifetime = 0);
  virtual void DebugBox(const idVec4& color, const idBox& box,
                        const int lifetime = 0);
  virtual void DebugCone(const idVec4& color, const idVec3& apex,
                         const idVec3& dir, float radius1, float radius2,
                         const int lifetime = 0);
  virtual void DebugScreenRect(const idVec4& color, const idScreenRect& rect,
                               const viewDef_t* viewDef,
                               const int lifetime = 0);
  virtual void DebugAxis(const idVec3& origin, const idMat3& axis);

  virtual void DebugClearPolygons(int time);
  virtual void DebugPolygon(const idVec4& color, const idWinding& winding,
                            const int lifeTime = 0,
                            const bool depthTest = false);

  virtual void DrawText(const char* text, const idVec3& origin, float scale,
                        const idVec4& color, const idMat3& viewAxis,
                        const int align = 1, const int lifetime = 0,
  ...
  idRenderModel* ParseModel(idLexer* src, const char* mapName,
                            ID_TIME_T mapTimeStamp, idFile* fileOut);
  ...
  // RB begin
  bool CullEnvprobeByPortals(const RenderEnvprobeLocal* probe,
                             const portalStack_t* ps);
  void AddAreaViewEnvprobes(int areaNum, const portalStack_t* ps);
  // RB end

  void AddAreaToView(int areaNum, const portalStack_t* ps);
  idScreenRect ScreenRectFromWinding(const idWinding* w,
                                     const viewEntity_t* space);
  bool PortalIsFoggedOut(const portal_t* p);
  void FloodViewThroughArea_r(const idVec3& origin, int areaNum,
                              const portalStack_t* ps);
  void FlowViewThroughPortals(const idVec3& origin, int numPlanes,
                              const idPlane* planes);
  void BuildConnectedAreas_r(int areaNum);
  void BuildConnectedAreas();
  void FindViewLightsAndEntities();

  void FloodLightThroughArea_r(idRenderLightLocal* light, int areaNum,
                               const portalStack_t* ps);
  ...
  bool AreasAreConnected(int areaNum1, int areaNum2,
                         portalConnection_t connection) const;
  ...
  bool ProcessDemoCommand(idDemoFile* readDemo, renderView_t* demoRenderView,
                          int* demoTimeOffset);
  ...
  void WriteRenderLight(idDemoFile* f, qhandle_t handle,
                        const renderLight_t* light);
  void WriteRenderEntity(idDemoFile* f, idRenderEntityLocal* entity);
  void WriteRenderEnvprobe(qhandle_t handle,
                           const renderEnvironmentProbe_t* probe);  // RB
  ...
  void AddEnvprobeRefToArea(RenderEnvprobeLocal* probe,
                            portalArea_t* area);  // RB

  void RecurseProcBSP_r(modelTrace_t* results, int parentNodeNum, int nodeNum,
                        float p1f, float p2f, const idVec3& p1,
                        const idVec3& p2) const;
  void BoundsInAreas_r(int nodeNum, const idBounds& bounds, int* areas,
                       int* numAreas, int maxAreas) const;

  float DrawTextLength(const char* text, float scale, int len = 0);

  void FreeInteractions();

  void PushFrustumIntoTree_r(idRenderEntityLocal* def,
                             idRenderLightLocal* light,
                             const frustumCorners_t& corners, int nodeNum);
  void PushFrustumIntoTree(idRenderEntityLocal* def, idRenderLightLocal* light,
  ...
  //-------------------------------
  // tr_light.c
  void CreateLightDefInteractions(idRenderLightLocal* const ldef,
                                  const int renderViewID);
...
// if an entity / light combination has been evaluated and found to not genrate
// any surfaces or shadows, the constant INTERACTION_EMPTY will be stored in the
// interaction table, int contrasts to NULL, which means that the combination
// has not yet been tested for having surfaces.
