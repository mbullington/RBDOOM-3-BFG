...
idCVar preLoad_Images("preLoad_Images", "1", CVAR_SYSTEM | CVAR_BOOL,
                      "preload images during beginlevelload");
...
void R_ListImages_f(const idCmdArgs& args) {
  ...
  if (failed) {
    idLib::Printf(
        "usage: listImages [ sorted | namesort | unloaded | duplicated | "
        "showOverSized ]\n");
    return;
  }
  ...
  sortedImage_t* sortedArray =
      (sortedImage_t*)alloca(sizeof(sortedImage_t) * numImages);
...
idImage* idImageManager::ImageFromFunction(
    const char* _name, void (*generatorFunction)(idImage* image)) {
  ...
  for (int i = imageHash.First(hash); i != -1; i = imageHash.Next(i)) {
    ...
    if (name.Icmp(image->GetName()) == 0) {
      ...
      if ((!insideLevelLoad || preloadingMapImages) && !image->IsLoaded()) {
        image->referencedOutsideLevelLoad =
            (!insideLevelLoad && !preloadingMapImages);
        ...
        declManager->MediaPrint("%ix%i %s (reload for mixed referneces)\n",
                                ...
                                image->GetUploadHeight(), image->GetName());
      }
      return image;
    }
  }
...
===============
*/
idImage* idImageManager::ScratchImage(const char* name,
...
====================
*/
void idImageManager::Preload(const idPreloadManifest& manifest,
                             const bool& mapPreload) {
  if (preLoad_Images.GetBool() && manifest.NumResources() > 0) {
    ...
    for (int i = 0; i < manifest.NumResources(); i++) {
      ...
      if (p.resType == PRELOAD_IMAGE && !ExcludePreloadImage(p.resourceName)) {
        globalImages->ImageFromFile(
            p.resourceName, (textureFilter_t)p.imgData.filter,
            (textureRepeat_t)p.imgData.repeat, (textureUsage_t)p.imgData.usage,
            (cubeFiles_t)p.imgData.cubeMap);
        numLoaded++;
      }
    }
    ...
    idLib::Printf(
        "%05d images preloaded ( or were already loaded ) in %5.1f seconds\n",
