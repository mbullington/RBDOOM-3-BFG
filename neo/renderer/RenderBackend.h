...
// void RB_GetShaderTextureMatrix( const float* shaderRegisters, const
// textureStage_t* texture, float matrix[16] ); void RB_LoadShaderTextureMatrix(
// const float* shaderRegisters, const textureStage_t* texture ); void
// RB_BakeTextureMatrixIntoTexgen( idPlane lightProject[3], const float*
// textureMatrix );
...
// SRS - Generalized Vulkan SDL platform
  ...
  bool deviceProperties2Available;  // SRS - For getting device properties in
                                    // support of gfxInfo
  ...
  idArray<idArray<uint32, MRB_TOTAL_QUERIES>, NUM_FRAME_DATA>
      queryAssignedIndex;
...
class idRenderBackend {
 ...
 private:
  ...
  void DrawStencilShadowPass(const drawSurf_t* drawSurf,
  ...
  void BindVariableStageImage(const textureStage_t* texture,
                              const float* shaderRegisters);
  void PrepareStageTexturing(const shaderStage_t* pStage,
                             const drawSurf_t* surf);
  void FinishStageTexturing(const shaderStage_t* pStage,
                            const drawSurf_t* surf);

  void ResetViewportAndScissorToDefaultCamera(const viewDef_t* _viewDef);

  void FillDepthBufferGeneric(const drawSurf_t* const* drawSurfs,
                              int numDrawSurfs);
  ...
  void T_BasicFog(const drawSurf_t* drawSurfs, const idPlane fogPlanes[4],
                  const idRenderMatrix* inverseBaseLightProject);
  void FogPass(const drawSurf_t* drawSurfs, const drawSurf_t* drawSurfs2,
               const viewLight_t* vLight);
  void FogAllLights();

  void SetupInteractionStage(const shaderStage_t* surfaceStage,
                             const float* surfaceRegs,
                             const float lightColor[4], idVec4 matrix[2],
                             float color[4]);

  void DrawInteractions(const viewDef_t* _viewDef);
  void DrawSingleInteraction(drawInteraction_t* din, bool useFastPath,
                             bool useIBL, bool setInteractionShader);
  int DrawShaderPasses(const drawSurf_t* const* const drawSurfs,
                       const int numDrawSurfs,
                       const float guiStereoScreenOffset, const int stereoEye);

  void RenderInteractions(const drawSurf_t* surfList, const viewLight_t* vLight,
 ...
 private:
  ...
  void DBG_RenderDrawSurfListWithFunction(drawSurf_t** drawSurfs,
                                          int numDrawSurfs);
 ...
 private:
  ...
  idScreenRect
      currentScissor;  // for scissor clipping, local inside renderView viewport
 ...
 public:
  int GetCurrentTextureUnit() const { return currenttmu; }
