...
struct srfTriangles_t {
  ...
  bool referencedVerts;  // if true the 'verts' are referenced and should not be
                         // freed
  bool referencedIndexes;  // if true, indexes, silIndexes, mirrorVerts, and
                           // silEdges are
  ...
  int shadowCapPlaneBits;  // bits 0-5 are set when that plane of the
                           // interacting light has triangles
  ...
  idShadowVert* preLightShadowVertexes;  // shadow vertices in CPU memory for
                                         // pre-light shadow volumes
  idShadowVert* staticShadowVertexes;    // shadow vertices in CPU memory for
                                         // static shadow volumes

  srfTriangles_t* ambientSurface;  // for light interactions, point back at the
                                   // original surface that generated
...
class idRenderModel {
 public:
  ...
  virtual bool LoadBinaryModel(idFile* file,
                               const ID_TIME_T sourceTimeStamp) = 0;
  virtual void WriteBinaryModel(idFile* file,
                                ID_TIME_T* _timeStamp = NULL) const = 0;
  virtual bool SupportsBinaryModel() = 0;

  // RB begin
  virtual void ExportOBJ(idFile* objFile, idFile* mtlFile,
                         ID_TIME_T* _timeStamp = NULL) = 0;
  // RB end
