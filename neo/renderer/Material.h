...
// RB: define this to use the id Tech 4.5 UI interface for ImGui instead of
// OpenGL or Vulkan this allows to have the com_showFPS stats in screenshots
...
typedef enum {
  TF_LINEAR,
  TF_NEAREST,
  TF_NEAREST_MIPMAP,  // RB: no linear interpolation but explicit mip-map levels
                      // for hierarchical depth buffer
...
struct stencilStage_t {
  ...
  // What to do with the contents of the buffer if the stencil test(and the
  // depth test) passes.
...
const int MAX_GLOBAL_SHADER_PARMS =
    12;  // ? this looks like it should only be 8
...
typedef enum {
  ...
  MF_LOD_PERSISTENT = BIT(
      11),  // motorsep 11-24-2014; material flag for persistent LOD iteration
  MF_GUITARGET = BIT(12),  // Admer: this GUI surface is used to compute a GUI
                           // render map, but a GUI should NOT be drawn on it
  MF_AUTOGEN_TEMPLATE = BIT(
      13),  // Admer: this material is a template for auto-generated templates
...
static const char* predef_lightstylesinfo[] = {"Normal",
                                               "Flicker A",
                                               "Slow Strong Pulse",
                                               "Candle A",
                                               "Fast Strobe",
...
class idMaterial : public idDecl {
 public:
  ...
  // RB: characters and models that baked in Blender or Substance designer use
  // the newer Mikkelsen tangent space standard. see:
  ...
  // floats. FloatTime is passed in because different entities, which may be
  // running in parallel, can be in different time groups.
  ...
  // motorsep 11-23-2014; material LOD keys that define what LOD iteration the
  // surface falls into lod1 - lod4 defines several levels of LOD persistentLOD
  // specifies the LOD iteration that still being rendered, even after the
  // camera is beyond the distance at which LOD iteration should not be rendered
  ...
  bool IsLODVisibleForDistance(float distance, float lodBase) const {
    int bit = (materialFlags & (MF_LOD1 | MF_LOD2 | MF_LOD3 | MF_LOD4)) >>
              MF_LOD1_SHIFT;
    float m1 = lodBase * (bit >> 1);
    float m2 = lodBase * bit;
    return distance >= m1 &&
           (distance < m2 || (materialFlags & (MF_LOD_PERSISTENT)));
  }
 ...
 private:
  ...
  void ParseStencilOperation(const idToken& token,
  ...
  void MultiplyTextureMatrix(
      textureStage_t* ts, int registers[2][3]);  // FIXME: for some reason the
 ...
 private:
  ...
  bool mikktspace;  // RB: use Mikkelsen tangent space standard for normal
                    // mapping
