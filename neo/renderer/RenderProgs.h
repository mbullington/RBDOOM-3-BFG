...
NOTE: There is a PS3 dependency between the bit flag specified here and the
vertex attribute index and attribute semantic specified in DeclRenderProg.cpp
because the stored render prog vertexMask is initialized with
cellCgbGetVertexConfiguration(). The ATTRIB_INDEX_ defines are used to make sure
the vertexMask_t and attrib assignment in DeclRenderProg.cpp are in sync.

Even though VERTEX_MASK_XYZ_SHORT and VERTEX_MASK_ST_SHORT are not real
attributes, they come before the VERTEX_MASK_MORPH to reduce the range of vertex
program permutations defined by the vertexMask_t bits on the Xbox 360 (see
MAX_VERTEX_DECLARATIONS).
================================================
*/
enum vertexMask_t {
...
// This enum list corresponds to the global constant register indecies as
// defined in global.inc for all shaders.  We used a shared pool to keeps things
// simple.  If something changes here then it also needs to change in global.inc
// and vice versa
enum renderParm_t {
...
class idRenderProgManager {
 public:
  ...
  int FindShader(const char* name, rpStage_t stage, const char* nameOutSuffix,
                 uint32 features, bool builtin,
                 vertexLayoutType_t vertexLayout = LAYOUT_DRAW_VERT);
  ...
  bool ShaderUsesJoints() const { return renderProgs[current].usesJoints; }
  // the rpEnableSkinning render parm should only be set for vertex programs
  // that use it
 ...
 private:
  ...
  enum {
    ...
    BUILTIN_STEREO_WARP,
    BUILTIN_BINK,
    BUILTIN_BINK_SRGB,  // SRS - Added Bink shader without sRGB to linear
                        // conversion for testVideo cmd
    BUILTIN_BINK_GUI,
  ...
  void LoadGLSLProgram(const int programIndex, const int vertexShaderIndex,
  ...
  struct renderProg_t {
    ...
    VkPipeline GetPipeline(uint64 stateBits, VkShaderModule vertexShader,
  ...
  static void CreateDescriptorSetLayout(const shader_t& vertexShader,
                                        const shader_t& fragmentShader,
                                        renderProg_t& renderProg);
  void AllocParmBlockBuffer(const idList<int>& parmIndices,
                            idUniformBuffer& ubo);
#else
  struct shader_t {
    shader_t()
