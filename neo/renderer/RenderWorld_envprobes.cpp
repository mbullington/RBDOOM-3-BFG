...
viewEnvprobe_t* R_SetEnvprobeDefViewEnvprobe(RenderEnvprobeLocal* probe) {
  ...
  viewEnvprobe_t* vProbe = (viewEnvprobe_t*)R_ClearedFrameAlloc(
      sizeof(*vProbe), FRAME_ALLOC_VIEW_LIGHT);
  vProbe->envprobeDef = probe;

  // the scissorRect will be expanded as the envprobe bounds is accepted into
  // visible portal chains and the scissor will be reduced in
  // R_AddSingleEnvprobe based on the screen space projection
...
================
*/
bool idRenderWorldLocal::CullEnvprobeByPortals(const RenderEnvprobeLocal* probe,
                                               const portalStack_t* ps) {
  if (r_useLightPortalCulling.GetInteger() == 1) {
    ALIGNTYPE16 frustumCorners_t corners;
    idRenderMatrix::GetFrustumCorners(corners, probe->inverseBaseProbeProject,
                                      bounds_zeroOneCube);
    for (int i = 0; i < ps->numPortalPlanes; i++) {
      if (idRenderMatrix::CullFrustumCornersToPlane(
              corners, ps->portalPlanes[i]) == FRUSTUM_CULL_FRONT) {
        return true;
      }
    }
  }
...
Any lights that are visible through the current portalStack will have their
scissor rect updated.
===================
*/
void idRenderWorldLocal::AddAreaViewEnvprobes(int areaNum,
                                              const portalStack_t* ps) {
  portalArea_t* area = &portalAreas[areaNum];

  for (areaReference_t* lref = area->envprobeRefs.areaNext;
       lref != &area->envprobeRefs; lref = lref->areaNext) {
    RenderEnvprobeLocal* probe = lref->envprobe;

    // debug tool to allow viewing of only one light at a time
    if (r_singleEnvprobe.GetInteger() >= 0 &&
        r_singleEnvprobe.GetInteger() != probe->index) {
      continue;
    }
...
void R_SampleCubeMapHDR(const idVec3& dir, int size, byte* buffers[6],
  ...
  // uint32_t value = ( *( const uint32_t* )buffers[axis][( y * size + x ) * 4 +
  // 0] );
...
// To implement the Hammersley point set we only need an efficent way to
// implement the Van der Corput radical inverse phi2(i). Since it is in base 2
// we can use some basic bit operations to achieve this. The brilliant book
// Hacker's Delight [warren01] provides us a a simple way to reverse the bits in
// a given 32bit integer. Using this, the following code then implements phi2(i)
...
idVec3 ImportanceSampleGGX(const idVec2& Xi, const idVec3& N, float roughness) {
  ...
  // cosinus distributed direction (Z-up or tangent space) from the hammersley
  // point xi
...
idVec2 NormalizedOctCoord(int x, int y, const int probeWithBorderSide) {
  ...
  idVec2 octFragCoord = idVec2((x - margin) % probeWithBorderSide,
                               (y - margin) % probeWithBorderSide);

  // Add back the half pixel to get pixel center normalized coordinates
  return (idVec2(octFragCoord) + idVec2(0.5f, 0.5f)) *
             (2.0f / float(probeSideLength)) -
         idVec2(1.0f, 1.0f);

#endif
}

static inline idVec2 NormalizedOctCoordNoBorder(int x, int y,
                                                const int probeWithBorderSide) {
  int probeSideLength = probeWithBorderSide;

  idVec2 octFragCoord =
      idVec2(x % probeWithBorderSide, y % probeWithBorderSide);
...
/*
static inline float LatLongTexelArea( const idVec2i& pos, const idVec2i&
imageSize )
{
        idVec2 uv0;
...
void CalculateIrradianceJob(calcEnvprobeParms_t* parms) {
  ...
  CommandlineProgressBar progressBar(
      R_CalculateUsedAtlasPixels(parms->outHeight), parms->printWidth,
      parms->printHeight);
    ...
    for (int x = dstRect.x; x < (dstRect.x + dstRect.z); x++) {
      for (int y = dstRect.y; y < (dstRect.y + dstRect.w); y++) {
        ...
        if (mip > 0) {
          ...
          octCoord = NormalizedOctCoordNoBorder(x - dstRect.x, y - dstRect.y,
                                                dstRect.z);
        } else {
...
void CalculateRadianceJob(calcEnvprobeParms_t* parms) {
  ...
  const int numMips = idMath::BitsForInteger(parms->outHeight);
  const int numOctahedronMips =
      numMips - 3;  // the last 3 mips are too low quality for filtering
  ...
  for (int mip = 0; mip < numOctahedronMips; mip++) {
    ...
    for (int x = dstRect.x; x < (dstRect.x + dstRect.z); x++) {
      for (int y = dstRect.y; y < (dstRect.y + dstRect.w); y++) {
        ...
        // Epic Games makes a further approximation by assuming the view
        // direction (and thus the specular reflection direction) to be equal to
        // the output sample direction ωo. This translates itself to the
        // following code:
        const idVec3 R = N;
        const idVec3 V = R;
        ...
        for (int s = 0; s < parms->samples; s++) {
          ...
          if (NdotL > 0.0) {
            ...
            R_SampleCubeMapHDR16F(H, ENVPROBE_CAPTURE_SIZE, buffers, sample, u,
                                  v);
...
void R_MakeAmbientMap(const char* baseName, byte* buffers[6],
                      const char* suffix, int outSize, bool specular,
                      bool useThreads) {
  idStr fullname;
  renderView_t ref;
  viewDef_t primary;
  ...
  jobParms->outBuffer = (halfFloat_t*)R_StaticAlloc(
      idMath::Ceil(outSize * outSize * 3 * sizeof(halfFloat_t) * 1.5f),
      TAG_IMAGE);
...
CONSOLE_COMMAND(bakeEnvironmentProbes, "Bake environment probes", NULL) {
  ...
  idFileList* files =
      fileSystem->ListFilesTree(fullname, "envprobe*.exr", true);
  ...
  for (int i = 0; i < tr.primaryWorld->envprobeDefs.Num(); i++) {
    ...
    for (int j = 0; j < 6; j++) {
      ...
      glReadPixels(0, 0, captureSize, captureSize, GL_RGB, GL_HALF_FLOAT,
                   float16FRGB);
    ...
    fullname.Format("%s/area%i_envprobe_%i_%i_%i", baseName.c_str(), areaNum,
                    int(point.x), int(point.y), int(point.z));
    fullname.ReplaceChar('-', '_');

    // create 2 jobs
    R_MakeAmbientMap(fullname.c_str(), buffers, "_amb",
                     IRRADIANCE_OCTAHEDRON_SIZE, false, useThreads);
    R_MakeAmbientMap(fullname.c_str(), buffers, "_spec",
                     RADIANCE_OCTAHEDRON_SIZE, true, useThreads);
  }
  ...
  for (int j = 0; j < tr.envprobeJobs.Num(); j++) {
    ...
    R_WriteEXR(job->filename, (byte*)job->outBuffer, 3, job->outWidth,
               job->outHeight, "fs_basepath");
...
CONSOLE_COMMAND(makeBrdfLUT, "make a GGX BRDF lookup table", NULL) {
  ...
  // R_WriteEXR( "env/_brdfLut.exr", hdrBuffer, 4, outSize, outSize,
  // "fs_basepath" );

  idFileLocal headerFile(
      fileSystem->OpenFileWrite("env/Image_brdfLut.h", "fs_basepath"));
...
CONSOLE_COMMAND(makeImageHeader, "load an image and turn it into a .h file",
                NULL) {
  byte* buffer;
  int width = 0, height = 0;
