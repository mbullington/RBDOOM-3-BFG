...
struct drawSurf_t {
  const srfTriangles_t* frontEndGeo;  // don't use on the back end, it may be
                                      // updated by the front end!
  int numIndexes;
  ...
  uint64
      extraGLState;  // Extra GL state |'d with material->stage[].drawStateBits
  ...
  drawSurf_t** linkChain;  // defer linking to lights to a serial section to
                           // avoid a mutex
  idScreenRect
...
struct areaReference_t {
  ...
  idRenderEntityLocal*
      entity;  // only one of entity / light / envprobe will be non-NULL
  idRenderLightLocal*
      light;  // only one of entity / light / envprobe will be non-NULL
  RenderEnvprobeLocal*
      envprobe;  // only one of entity / light / envprobe will be non-NULL
...
// RB : RennderEnvprobe should become the new public interface replacing the
// qhandle_t to envprobe defs in the idRenderWorld interface
class RenderEnvprobe {
 public:
  virtual ~RenderEnvprobe() {}

  virtual void FreeRenderEnvprobe() = 0;
  virtual void UpdateRenderEnvprobe(const renderEnvironmentProbe_t* ep,
                                    bool forceUpdate = false) = 0;
...
class idRenderLightLocal : public idRenderLight {
 public:
  ...
  bool LightCastsShadows() const {
    return parms.forceShadows ||
           (!parms.noShadows && lightShader->LightCastsShadows());
  }
  ...
  // derived information
  idPlane lightProject[4];  // old style light projection where Z and W are
                            // flipped and projected lights lightProject[3] is
                            // divided by ( zNear + zFar )
  idRenderMatrix baseLightProject;  // global xyz1 to projected light strq
  idRenderMatrix
      inverseBaseLightProject;  // transforms the zero-to-one cube to exactly
                                // cover the light in world space
...
class RenderEnvprobeLocal : public RenderEnvprobe {
 public:
  ...
  // temporary helpers
  int viewCount;  // if == tr.viewCount, the envprobe is on the
                  // viewDef->viewEnvprobes list
...
class idRenderEntityLocal : public idRenderEntity {
 public:
  ...
  idRenderMatrix
      inverseBaseModelProject;  // transforms the unit cube to exactly cover the
  ...
  // the local bounds used to place entityRefs, either from parms for dynamic
  // entities, or a model bounds
...
struct viewLight_t {
  ...
  // RB end
  idRenderMatrix
      inverseBaseLightProject;  // the matrix for deforming the
                                // 'zeroOneCubeModel' to exactly cover the light
  ...
  drawSurf_t*
      translucentInteractions;  // translucent interactions don't get shadows

  // R_AddSingleLight will build a chain of parameters here to setup shadow
  // volumes
...
struct viewEntity_t {
  ...
  bool isGuiSurface;  // force two sided and vertex colors regardless of
                      // material setting
  ...
  // R_AddSingleModel will build a chain of parameters here to setup shadow
  // volumes
...
// a viewEnvprobe contains everything that the back end needs out of an
// RenderEnvprobeLocal, which the front end may be modifying simultaniously if
// running in SMP mode.

// this structure will be especially helpful when we switch RBDOOM-3-BFG to
// forward cluster shading because then we can evaluate all viewEnvprobes
...
struct viewEnvprobe_t {
  ...
  idRenderMatrix
      inverseBaseProbeProject;  // the matrix for deforming the
                                // 'zeroOneCubeModel' to exactly cover the light
                                // volume in world space
  idImage* irradianceImage;     // cubemap image used for diffuse IBL by backend
  idImage* radianceImage;  // cubemap image used for specular IBL by backend
...
static const int LIGHTGRID_IRRADIANCE_BORDER_SIZE =
    2;  // one pixel border all around the octahedron so 2 on each side
static const int LIGHTGRID_IRRADIANCE_SIZE =
    14 + LIGHTGRID_IRRADIANCE_BORDER_SIZE;
...
struct calcLightGridPointParms_t {
  ...
  halfFloat_t*
      outBuffer;  // HDR R11G11B11F octahedron LIGHTGRID_IRRADIANCE_SIZE^2
  int time;       // execution time in milliseconds
};
// RB end
...
struct viewDef_t {
  ...
  viewEntity_t* viewEntitys;  // chain of all viewEntities effecting view,
                              // including off screen ones casting shadows
  // we use viewEntities as a check to see if a given view consists solely
  // of 2D rendering, which we can optimize in certain ways.  A 2D view will
  // not have any viewEntities

  // RB begin
  frustum_t frustums[MAX_FRUSTUMS];  // positive sides face outward, [4] is the
                                     // front clip plane
  ...
  idBounds globalProbeBounds;
  idRenderMatrix
      inverseBaseEnvProbeProject;  // the matrix for deforming the
                                   // 'zeroOneCubeModel' to exactly cover the
                                   // environent probe volume in world space
  idImage* irradianceImage;    // cubemap image used for diffuse IBL by backend
...
class idRenderSystemLocal : public idRenderSystem {
 public:
  ...
  virtual void BeginAutomaticBackgroundSwaps(
      autoRenderIconType_t icon = AUTORENDER_DEFAULTICON);
  virtual void EndAutomaticBackgroundSwaps();
  virtual bool AreAutomaticBackgroundSwapsRunning(
      autoRenderIconType_t* usingAlternateIcon = NULL) const;
  ...
  virtual void DrawFilled(const idVec4& color, float x, float y, float w,
                          float h);
  virtual void DrawStretchPic(float x, float y, float w, float h, float s1,
                              float t1, float s2, float t2,
                              const idMaterial* material);
  virtual void DrawStretchPic(const idVec4& topLeft, const idVec4& topRight,
                              const idVec4& bottomRight,
                              const idVec4& bottomLeft,
                              const idMaterial* material);
  virtual void DrawStretchTri(const idVec2& p1, const idVec2& p2,
                              const idVec2& p3, const idVec2& t1,
                              const idVec2& t2, const idVec2& t3,
                              const idMaterial* material);
  virtual idDrawVert* AllocTris(
      int numVerts, const triIndex_t* indexes, int numIndexes,
      const idMaterial* material,
      const stereoDepthType_t stereoType = STEREO_DEPTH_TYPE_NONE);
  virtual void DrawSmallChar(int x, int y, int ch);
  virtual void DrawSmallStringExt(int x, int y, const char* string,
                                  const idVec4& setColor, bool forceColor);
  virtual void DrawBigChar(int x, int y, int ch);
  virtual void DrawBigStringExt(int x, int y, const char* string,
                                const idVec4& setColor, bool forceColor);

  virtual void WriteDemoPics();
  virtual void WriteEndFrame();
  virtual void DrawDemoPics();
  virtual const emptyCommand_t* SwapCommandBuffers(
      uint64* frontEndMicroSec, uint64* backEndMicroSec, uint64* shadowMicroSec,
      uint64* gpuMicroSec, backEndCounters_t* bc, performanceCounters_t* pc);

  virtual void SwapCommandBuffers_FinishRendering(
      uint64* frontEndMicroSec, uint64* backEndMicroSec, uint64* shadowMicroSec,
      uint64* gpuMicroSec, backEndCounters_t* bc, performanceCounters_t* pc);
  ...
  virtual void TakeScreenshot(int width, int height, const char* fileName,
                              int downSample, renderView_t* ref, int exten);
  ...
  virtual void CaptureRenderToImage(const char* imageName,
                                    bool clearColorAfterCopy = false);
  virtual void CaptureRenderToFile(const char* fileName, bool fixAlpha);
  virtual void UnCrop();
  virtual bool UploadImage(const char* imageName, const byte* data, int width,
                           int height);
...
// SRS - Added cvar to control workarounds for AMD OSX driver bugs when shadow
// mapping enabled
extern idCVar r_skipAMDWorkarounds;  // skip work arounds for AMD driver bugs
// SRS end
extern idCVar
    r_skipIntelWorkarounds;  // skip work arounds for Intel driver bugs
...
extern idCVar
    r_useLightPortalCulling;  // 0 = none, 1 = box, 2 = exact clip of polyhedron
                              // faces, 3 MVP to plane culling
...
extern idCVar
    r_skipPrelightShadows;  // 1 = skip the dmap generated static shadow volumes
extern idCVar
...
extern idCVar r_lightAllBackFaces;   // light all the back faces, even when they
                                     // would be shadowed
extern idCVar r_useLightDepthBounds;  // use depth bounds test on lights to
                                      // reduce both shadow and interaction fill
extern idCVar r_useShadowDepthBounds;  // use depth bounds test on individual
                                       // shadows to reduce shadow fill
// RB begin
extern idCVar
...
extern idCVar
    r_skipStaticInteractions;  // skip interactions created at level load
extern idCVar
    r_skipDynamicInteractions;     // skip interactions created after level load
...
struct glimpParms_t {
  ...
  int width;
  int height;
  int fullScreen;  // 0 = windowed, otherwise 1 based monitor number to go full
                   // screen on
...
// DG: R_GetModeListForDisplay is called before GLimp_Init(), but SDL needs
// SDL_Init() first. So add PreInit for platforms that need it, others can just
// stub it.
void VKimp_PreInit();

// If the desired mode can't be set satisfactorily, false will be returned.
// If succesful, sets glConfig.nativeScreenWidth, glConfig.nativeScreenHeight,
// and glConfig.pixelAspect The renderer will then reset the glimpParms to "safe
// mode" of 640x480 fullscreen and try again.  If that also fails, the error
// will be fatal.
...
void VKimp_SetGamma(unsigned short red[256], unsigned short green[256],
...
srfTriangles_t* R_PolytopeSurface(int numPlanes, const idPlane* planes,
                                  idWinding** windings);
...
void R_SampleCubeMapHDR(const idVec3& dir, int size, byte* buffers[6],
                        float result[3], float& u, float& v);
void R_SampleCubeMapHDR16F(const idVec3& dir, int size, halfFloat_t* buffers[6],
                           float result[3], float& u, float& v);
...
void* R_ClearedFrameAlloc(int bytes,
                          frameAllocType_t type = FRAME_ALLOC_UNKNOWN);

void* R_StaticAlloc(
    int bytes,
    const memTag_t tag = TAG_RENDER_STATIC);  // just malloc with error checking
...
void R_SurfaceToTextureAxis(const srfTriangles_t* tri, idVec3& origin,
                            idVec3 axis[3]);
void R_AddInGameGuis(const drawSurf_t* const drawSurfs[],
...
bool R_GenerateSubViews(const drawSurf_t* const drawSurfs[],
                        const int numDrawSurfs);
...
void R_CleanupTriangles(srfTriangles_t* tri, bool createNormals,
                        bool identifySilEdges, bool useUnsmoothedTangents,
                        bool useMikktspace);
...
// For static surfaces, the indexes, ambient, and shadow buffers can be
// pre-created at load time, rather than being re-created each frame in the
// frame temporary buffers.
void R_CreateStaticBuffersForTri(srfTriangles_t& tri);

// RB
idVec3 R_ClosestPointPointTriangle(const idVec3& point, const idVec3& vertex1,
                                   const idVec3& vertex2,
                                   const idVec3& vertex3);
...
struct deformInfo_t {
  ...
  triIndex_t* silIndexes;  // indexes changed to be the first vertex with same
...
// if outputVertexes is not NULL, it will point to a newly allocated set of
// verts that includes the mirrored ones
...
localTrace_t R_LocalTrace(const idVec3& start, const idVec3& end,
                          const float radius, const srfTriangles_t* tri);
