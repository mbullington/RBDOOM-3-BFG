...
bool MA_ParseHeaderIndex(maAttribHeader_t* header, int& minIndex, int& maxIndex,
                         const char* headerType, const char* skipString) {
  idParser miniParse;
  idToken token;
...
bool MA_ParseVertex(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->vertexes) {
    ...
    pMesh->vertexes =
        (idVec3*)Mem_Alloc(sizeof(idVec3) * pMesh->numVertexes, TAG_MODEL);
  }
...
bool MA_ParseVertexTransforms(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->vertTransforms) {
    ...
    pMesh->vertTransforms = (idVec4*)Mem_Alloc(
        sizeof(idVec4) * pMesh->numVertTransforms, TAG_MODEL);
...
bool MA_ParseEdge(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->edges) {
    ...
    pMesh->edges =
        (idVec3*)Mem_Alloc(sizeof(idVec3) * pMesh->numEdges, TAG_MODEL);
  }
...
bool MA_ParseNormal(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->normals) {
    ...
    pMesh->normals =
        (idVec3*)Mem_Alloc(sizeof(idVec3) * pMesh->numNormals, TAG_MODEL);
  }
...
bool MA_ParseFace(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->faces) {
    ...
    pMesh->faces =
        (maFace_t*)Mem_Alloc(sizeof(maFace_t) * pMesh->numFaces, TAG_MODEL);
  }
...
bool MA_ParseColor(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->colors) {
    ...
    pMesh->colors =
        (byte*)Mem_Alloc(sizeof(byte) * pMesh->numColors * 4, TAG_MODEL);
  }
...
bool MA_ParseTVert(idParser& parser, maAttribHeader_t* header) {
  ...
  if (!pMesh->tvertexes) {
    ...
    pMesh->tvertexes =
        (idVec2*)Mem_Alloc(sizeof(idVec2) * pMesh->numTVertexes, TAG_MODEL);
  }
...
void MA_GetSharedFace(int faceIndex, int vertIndex, int& sharedFace,
                      int& sharedVert) {
  maMesh_t* pMesh = &maGlobal.currentObject->mesh;
  int vertNum = pMesh->faces[faceIndex].vertexNum[vertIndex];
