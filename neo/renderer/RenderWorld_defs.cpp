...
void R_DeriveEntityData(idRenderEntityLocal* entity) {
  R_AxisToModelMatrix(entity->parms.axis, entity->parms.origin,
                      entity->modelMatrix);

  idRenderMatrix::CreateFromOriginAxis(entity->parms.origin, entity->parms.axis,
                                       entity->modelRenderMatrix);

  // calculate the matrix that transforms the unit cube to exactly cover the
  // model in world space
  idRenderMatrix::OffsetScaleForBounds(entity->modelRenderMatrix,
                                       entity->localReferenceBounds,
                                       entity->inverseBaseModelProject);

  // calculate the global model bounds by inverse projecting the unit cube with
  // the 'inverseBaseModelProject'
  idRenderMatrix::ProjectedBounds(entity->globalReferenceBounds,
                                  entity->inverseBaseModelProject,
                                  bounds_unitCube, false);
}
...
===================
*/
void R_FreeEntityDefDecals(idRenderEntityLocal* def) { def->decals = NULL; }
...
===================
*/
void R_FreeEntityDefOverlay(idRenderEntityLocal* def) { def->overlays = NULL; }
...
void R_CreateEntityRefs(idRenderEntityLocal* entity) {
  ...
  if (r_showUpdates.GetBool() &&
    ...
    common->Printf(
        "big entityRef: %f,%f\n",
        entity->localReferenceBounds[1][0] - entity->localReferenceBounds[0][0],
        entity->localReferenceBounds[1][1] -
            entity->localReferenceBounds[0][1]);
  }
  ...
  entity->world->PushFrustumIntoTree(
...
========================
*/
static float R_ComputePointLightProjectionMatrix(idRenderLightLocal* light,
                                                 idRenderMatrix& localProject) {
  assert(light->parms.pointLight);

  // A point light uses a box projection.
  // This projects into the 0.0 - 1.0 texture range instead of -1.0 to 1.0 clip
  // space range.
  localProject.Zero();
...
========================
*/
static float R_ComputeSpotLightProjectionMatrix(idRenderLightLocal* light,
                                                idRenderMatrix& localProject) {
  ...
  const idVec3 normalizedRight =
      light->parms.right * (0.5f * targetDist / light->parms.right.LengthSqr());
  const idVec3 normalizedUp =
      light->parms.up * (-0.5f * targetDist / light->parms.up.LengthSqr());
  ...
  // This is similar to the Z calculation for depth buffering, which means that
  // the mapped texture is going to be perspective distorted heavily towards the
  // zero end.
  const float zNear =
      Max(light->parms.start * normalizedTarget, SPOT_LIGHT_MIN_Z_NEAR);
  const float zFar =
      Max(light->parms.end * normalizedTarget, SPOT_LIGHT_MIN_Z_FAR);
  ...
  // now offset to the 0.0 - 1.0 texture range instead of -1.0 to 1.0 clip space
  // range
...
========================
*/
static float R_ComputeParallelLightProjectionMatrix(
    idRenderLightLocal* light, idRenderMatrix& localProject) {
...
void R_DeriveLightData(idRenderLightLocal* light) {
  ...
  if (light->falloffImage == NULL) {
    ...
    if (light->parms.pointLight) {
      ...
      // Touch the default shader. to make sure it's decl has been parsed ( it
      ...
      // Touch the light shader. to make sure it's decl has been parsed ( it
      // might have been purged ).
      declManager->Touch(static_cast<const idDecl*>(defaultShader));
  ...
  idRenderMatrix::CreateFromOriginAxis(light->parms.origin, light->parms.axis,
                                       lightMatrix);
  ...
  // 'baseLightProject' goes from global space -> light local space -> light
  // projective space
  idRenderMatrix::Multiply(localProject, inverseLightMatrix,
                           light->baseLightProject);

  // Invert the light projection so we can deform zero-to-one cubes into
  // the light model and calculate global bounds.
  if (!idRenderMatrix::Inverse(light->baseLightProject,
                               light->inverseBaseLightProject)) {
    idLib::Warning("baseLightProject invert failed");
  }

  // calculate the global light bounds by inverse projecting the zero to one
  // cube with the 'inverseBaseLightProject'
  idRenderMatrix::ProjectedBounds(light->globalLightBounds,
                                  light->inverseBaseLightProject,
                                  bounds_zeroOneCube, false);
}
...
void R_FreeLightDefDerivedData(idRenderLightLocal* ldef) {
  ...
  for (doublePortal_t* dp = ldef->foggedPortals; dp != NULL;
...
void R_RenderLightFrustum(const renderLight_t& renderLight,
  ...
  idRenderMatrix::GetFrustumPlanes(lightFrustum, fakeLight.baseLightProject,
                                   true, true);
...
===============
*/
static bool WindingCompletelyInsideLight(const idWinding* w,
                                         const idRenderLightLocal* ldef) {
  for (int i = 0; i < w->GetNumPoints(); i++) {
    if (idRenderMatrix::CullPointToMVP(ldef->baseLightProject, (*w)[i].ToVec3(),
                                       true)) {
      return false;
    }
  }
...
static void R_CreateLightDefFogPortals(idRenderLightLocal* ldef) {
  ...
  for (areaReference_t* lref = ldef->references; lref != NULL;
       lref = lref->ownerNext) {
...
void R_CreateLightRefs(idRenderLightLocal* light) {
  ...
  if (light->parms.prelightModel != NULL && r_useLightPortalFlow.GetBool() &&
    ...
    light->world->PushFrustumIntoTree(
        NULL, light, light->inverseBaseLightProject, bounds_zeroOneCube);
  }
...
void R_DeriveEnvprobeData(RenderEnvprobeLocal* probe) {
  ...
  fullname.Format("env/%s/area%i_envprobe_%i_%i_%i_amb", basename.c_str(),
                  areaNum, int(point.x), int(point.y), int(point.z));
  fullname.ReplaceChar('-', '_');

  probe->irradianceImage = globalImages->ImageFromFile(
      fullname, TF_LINEAR, TR_CLAMP, TD_R11G11B10F, CF_2D_PACKED_MIPCHAIN);

  fullname.Format("env/%s/area%i_envprobe_%i_%i_%i_spec", basename.c_str(),
                  areaNum, int(point.x), int(point.y), int(point.z));
  fullname.ReplaceChar('-', '_');

  probe->radianceImage = globalImages->ImageFromFile(
      fullname, TF_DEFAULT, TR_CLAMP, TD_R11G11B10F, CF_2D_PACKED_MIPCHAIN);
  ...
  // idRenderMatrix::CreateFromOriginAxis( probe->globalProbeBounds[0], axis,
  // modelRenderMatrix );
  ...
  // idRenderMatrix::CreateFromOriginAxis( -probe->globalProbeBounds[0] * 2,
  // axis, modelRenderMatrix );
#endif

  // calculate the matrix that transforms the unit cube to exactly cover the
  // model in world space
  idRenderMatrix::OffsetScaleForBounds(modelRenderMatrix, localBounds,
                                       probe->inverseBaseProbeProject);

  // calculate the global model bounds by inverse projecting the unit cube with
  // the 'inverseBaseModelProject'
  // idRenderMatrix::ProjectedBounds( probe->globalProbeBounds,
  // probe->inverseBaseProbeProject, bounds_unitCube, false );
}
...
void R_FreeEnvprobeDefDerivedData(RenderEnvprobeLocal* probe) {
  ...
  areaReference_t* nextRef = NULL;
  for (areaReference_t* lref = probe->references; lref != NULL;
       lref = nextRef) {
    nextRef = lref->ownerNext;
