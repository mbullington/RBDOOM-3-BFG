...
void idImage::MakeDefault() {
  ...
  GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT, TR_REPEAT,
                TD_DEFAULT);

  defaulted = true;
}

static void R_DefaultImage(idImage* image) { image->MakeDefault(); }
...
static void R_ChromeSpecImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT,
                       TR_REPEAT, TD_SPECULAR_PBR_RMAO);
}
...
static void R_RGBA8Image(idImage* image) {
  ...
  image->GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_DEFAULT,
                       TR_REPEAT, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_RGBA8LinearImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, DEFAULT_SIZE, DEFAULT_SIZE, TF_LINEAR,
                       TR_REPEAT, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_EnvprobeImage_Depth(idImage* image) {
  image->GenerateImage(NULL, ENVPROBE_CAPTURE_SIZE, ENVPROBE_CAPTURE_SIZE,
                       TF_NEAREST, TR_CLAMP, TD_DEPTH_STENCIL);
}

static void R_SMAAImage_ResNative(idImage* image) {
  image->GenerateImage(NULL, renderSystem->GetWidth(),
                       renderSystem->GetHeight(), TF_LINEAR, TR_CLAMP,
                       TD_LOOKUP_TABLE_RGBA);
}
...
static void R_SSAOImage_ResHalf(idImage* image) {
  image->GenerateImage(NULL, renderSystem->GetWidth() / 2,
                       renderSystem->GetHeight() / 2, TF_LINEAR, TR_CLAMP,
                       TD_LOOKUP_TABLE_RGBA);
}

static void R_HierarchicalZBufferImage_ResNative(idImage* image) {
  image->GenerateImage(NULL, renderSystem->GetWidth(),
                       renderSystem->GetHeight(), TF_NEAREST_MIPMAP, TR_CLAMP,
                       TD_R32F);
}
...
static void R_AlphaNotchImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 2, 1, TF_NEAREST, TR_CLAMP,
...
static void R_CreateNoFalloffImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FALLOFF_TEXTURE_SIZE, 16, TF_DEFAULT,
                       TR_CLAMP_TO_ZERO, TD_LOOKUP_TABLE_MONO);
}
...
void R_FogImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FOG_SIZE, FOG_SIZE, TF_LINEAR, TR_CLAMP,
                       TD_LOOKUP_TABLE_ALPHA);
}
...
void R_FogEnterImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, FOG_ENTER_SIZE, FOG_ENTER_SIZE, TF_LINEAR,
                       TR_CLAMP, TD_LOOKUP_TABLE_ALPHA);
}
...
void R_QuadraticImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, QUADRATIC_WIDTH, QUADRATIC_HEIGHT,
                       TF_DEFAULT, TR_CLAMP, TD_LOOKUP_TABLE_RGB1);
}

// RB begin
static void R_CreateShadowMapImage_Res0(idImage* image) {
  int size = shadowMapResolutions[0];
  image->GenerateShadowArray(size, size, TF_LINEAR, TR_CLAMP_TO_ZERO_ALPHA,
                             TD_SHADOW_ARRAY);
}
...
static void R_CreateJitterImage1(idImage* image) {
  ...
  image->GenerateImage((byte*)data, JITTER_SIZE, JITTER_SIZE, TF_NEAREST,
                       TR_REPEAT, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateRandom256Image(idImage* image) {
  ...
  image->GenerateImage((byte*)data, 256, 256, TF_NEAREST, TR_REPEAT,
                       TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateBlueNoise256Image(idImage* image) {
  ...
  image->GenerateImage((byte*)data, BLUENOISE_TEX_WIDTH, BLUENOISE_TEX_HEIGHT,
                       TF_NEAREST, TR_REPEAT, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateHeatmap5ColorsImage(idImage* image) {
  ...
  static idVec4 colors[numColors] = {colorBlue, colorCyan, colorGreen,
  ...
  image->GenerateImage((byte*)data, FALLOFF_TEXTURE_SIZE, 16, TF_LINEAR,
                       TR_CLAMP, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateGrainImage1(idImage* image) {
  ...
  image->GenerateImage((byte*)data, GRAIN_SIZE, GRAIN_SIZE, TF_NEAREST,
                       TR_REPEAT, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateSMAAAreaImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, AREATEX_WIDTH, AREATEX_HEIGHT, TF_LINEAR,
                       TR_CLAMP, TD_LOOKUP_TABLE_RGBA);
}
...
static void R_CreateSMAASearchImage(idImage* image) {
  ...
  image->GenerateImage((byte*)data, SEARCHTEX_WIDTH, SEARCHTEX_HEIGHT,
                       TF_LINEAR, TR_CLAMP, TD_LOOKUP_TABLE_MONO);
}
...
static void R_CreateImGuiFontImage(idImage* image) {
  ...
  image->GenerateImage((byte*)pixels, width, height, TF_LINEAR, TR_CLAMP,
                       TD_LOOKUP_TABLE_RGBA);
...
static void R_CreateBrdfLutImage(idImage* image) {
  image->GenerateImage((byte*)brfLutTexBytes, BRDFLUT_TEX_WIDTH,
                       BRDFLUT_TEX_HEIGHT, TF_LINEAR, TR_CLAMP, TD_RG16F);
}

static void R_CreateEnvprobeImage_UAC_lobby_irradiance(idImage* image) {
  image->GenerateImage(
      (byte*)IMAGE_ENV_UAC_LOBBY_AMB_H_Bytes,
      IMAGE_ENV_UAC_LOBBY_AMB_H_TEX_WIDTH, IMAGE_ENV_UAC_LOBBY_AMB_H_TEX_HEIGHT,
      TF_DEFAULT, TR_CLAMP, TD_R11G11B10F, SAMPLE_1, CF_2D_PACKED_MIPCHAIN);
}
...
static void R_CreateEnvprobeImage_UAC_lobby_radiance(idImage* image) {
  image->GenerateImage((byte*)IMAGE_ENV_UAC_LOBBY_SPEC_H_Bytes,
                       IMAGE_ENV_UAC_LOBBY_SPEC_H_TEX_WIDTH,
                       IMAGE_ENV_UAC_LOBBY_SPEC_H_TEX_HEIGHT, TF_DEFAULT,
                       TR_CLAMP, TD_R11G11B10F, SAMPLE_1,
...
void idImageManager::CreateIntrinsicImages() {
  ...
  // RB begin
  shadowImage[0] =
      ImageFromFunction(va("_shadowMapArray0_%i", shadowMapResolutions[0]),
                        R_CreateShadowMapImage_Res0);
  shadowImage[1] =
      ImageFromFunction(va("_shadowMapArray1_%i", shadowMapResolutions[1]),
                        R_CreateShadowMapImage_Res1);
  shadowImage[2] =
      ImageFromFunction(va("_shadowMapArray2_%i", shadowMapResolutions[2]),
                        R_CreateShadowMapImage_Res2);
  shadowImage[3] =
      ImageFromFunction(va("_shadowMapArray3_%i", shadowMapResolutions[3]),
                        R_CreateShadowMapImage_Res3);
  shadowImage[4] =
      ImageFromFunction(va("_shadowMapArray4_%i", shadowMapResolutions[4]),
                        R_CreateShadowMapImage_Res4);
  ...
  smaaAreaImage =
      globalImages->ImageFromFunction("_smaaArea", R_CreateSMAAAreaImage);
  smaaSearchImage =
      globalImages->ImageFromFunction("_smaaSearch", R_CreateSMAASearchImage);

  smaaEdgesImage =
      globalImages->ImageFromFunction("_smaaEdges", R_SMAAImage_ResNative);
  smaaBlendImage =
      globalImages->ImageFromFunction("_smaaBlend", R_SMAAImage_ResNative);
  ...
  hierarchicalZbufferImage =
      ImageFromFunction("_cszBuffer", R_HierarchicalZBufferImage_ResNative);
  ...
  // save a copy of this for material comparison, because currentRenderImage may
  // get reassigned during stereo rendering
  originalCurrentRenderImage = currentRenderImage;

  loadingIconImage = ImageFromFile("textures/loadingicon2", TF_DEFAULT,
                                   TR_CLAMP, TD_DEFAULT, CF_2D);
  hellLoadingIconImage = ImageFromFile("textures/loadingicon3", TF_DEFAULT,
                                       TR_CLAMP, TD_DEFAULT, CF_2D);
...
#else
  defaultUACIrradianceCube = ImageFromFunction(
      "_defaultUACIrradiance", R_CreateEnvprobeImage_UAC_lobby_irradiance);
  defaultUACRadianceCube = ImageFromFunction(
      "_defaultUACRadiance", R_CreateEnvprobeImage_UAC_lobby_radiance);
#endif
  // RB end
