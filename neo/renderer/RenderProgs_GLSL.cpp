...
attribInfo_t attribsPC[] = {
    // vertex attributes
    {"float4", "position", "POSITION", "in_Position", PC_ATTRIB_INDEX_VERTEX,
     AT_VS_IN, VERTEX_MASK_XYZ},
    {"float2", "texcoord", "TEXCOORD0", "in_TexCoord", PC_ATTRIB_INDEX_ST,
     AT_VS_IN, VERTEX_MASK_ST},
    {"float4", "normal", "NORMAL", "in_Normal", PC_ATTRIB_INDEX_NORMAL,
     AT_VS_IN, VERTEX_MASK_NORMAL},
    {"float4", "tangent", "TANGENT", "in_Tangent", PC_ATTRIB_INDEX_TANGENT,
     AT_VS_IN, VERTEX_MASK_TANGENT},
    {"float4", "color", "COLOR0", "in_Color", PC_ATTRIB_INDEX_COLOR, AT_VS_IN,
     VERTEX_MASK_COLOR},
    {"float4", "color2", "COLOR1", "in_Color2", PC_ATTRIB_INDEX_COLOR2,
     AT_VS_IN, VERTEX_MASK_COLOR2},

    // pre-defined vertex program output
    {"float4", "position", "POSITION", "gl_Position", 0,
     AT_VS_OUT | AT_VS_OUT_RESERVED, 0},
    ...
    {"float4", "position", "WPOS", "gl_FragCoord", 0,
     AT_PS_IN | AT_PS_IN_RESERVED, 0},
    {"half4", "hposition", "WPOS", "gl_FragCoord", 0,
     AT_PS_IN | AT_PS_IN_RESERVED, 0},
    {"float", "facing", "FACE", "gl_FrontFacing", 0,
     AT_PS_IN | AT_PS_IN_RESERVED, 0},

    // fragment program output
    {"float4", "color", "COLOR", "fo_FragColor", 0,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"half4", "hcolor", "COLOR", "fo_FragColor", 0,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"float4", "color0", "COLOR0", "fo_FragColor", 0,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"float4", "color1", "COLOR1", "fo_FragColor", 1,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"float4", "color2", "COLOR2", "fo_FragColor", 2,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"float4", "color3", "COLOR3", "fo_FragColor", 3,
     AT_PS_OUT /*| AT_PS_OUT_RESERVED*/, 0},
    {"float", "depth", "DEPTH", "gl_FragDepth", 4,
     AT_PS_OUT | AT_PS_OUT_RESERVED, 0},
    ...
    {"float4", "texcoord0", "TEXCOORD0", "vofi_TexCoord0", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord1", "TEXCOORD1", "vofi_TexCoord1", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord2", "TEXCOORD2", "vofi_TexCoord2", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord3", "TEXCOORD3", "vofi_TexCoord3", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord4", "TEXCOORD4", "vofi_TexCoord4", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord5", "TEXCOORD5", "vofi_TexCoord5", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord6", "TEXCOORD6", "vofi_TexCoord6", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord7", "TEXCOORD7", "vofi_TexCoord7", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord8", "TEXCOORD8", "vofi_TexCoord8", 0,
     AT_VS_OUT | AT_PS_IN, 0},
    {"float4", "texcoord9", "TEXCOORD9", "vofi_TexCoord9", 0,
     AT_VS_OUT | AT_PS_IN, 0},
...
const char* types[] = {"int",  "float", "half",   "fixed",
                       "bool", "cint",  "cfloat", "void"};
static const int numTypes = sizeof(types) / sizeof(types[0]);

const char* typePosts[] = {"1",   "2",   "3",   "4",   "1x1", "1x2", "1x3",
                           "1x4", "2x1", "2x2", "2x3", "2x4", "3x1", "3x2",
                           "3x3", "3x4", "4x1", "4x2", "4x3", "4x4"};
...
========================
*/
idStr idRenderProgManager::StripDeadCode(const idStr& in, const char* name,
  ...
  for (int i = 0; i < blocks.Num(); i++) {
    blocks[i].used =
        ((blocks[i].name == "main") || blocks[i].name.Right(4) == "_ubo");
...
========================
*/
void ParseInOutStruct(idLexer& src, int attribType, int attribIgnoreType,
                      idList<inOutVariable_t>& inOutVars, bool in_Position4) {
  ...
  while (!src.CheckTokenString("}")) {
    ...
    if (glConfig.driverType == GLDRV_VULKAN) {
      ...
      if (!in_Position4 && var.nameGLSL == "in_Position" &&
          var.type == "vec4") {
        var.type = "vec3";
      }
    }
...
idStr idRenderProgManager::ConvertCG2GLSL(const idStr& in, const char* name,
  ...
  bool in_Position4 = (vertexLayout == LAYOUT_DRAW_SHADOW_VERT) ||
                      (vertexLayout == LAYOUT_DRAW_SHADOW_VERT_SKINNED);

  bool inMain = false;
  bool justEnteredMain = false;
  const char* uniformArrayName = (stage == SHADER_STAGE_VERTEX)
                                     ? VERTEX_UNIFORM_ARRAY_NAME
  ...
  while (src.ReadToken(&token)) {
      ...
      if (src.CheckTokenString("VS_IN")) {
          ...
          if (varsIn[i].declareInOut) {
            ...
            if (vkGLSL) {
              program += va("layout( location = %i ) in %s %s;\n", i,
                            varsIn[i].type.c_str(), varsIn[i].nameGLSL.c_str());
            } else {
              program +=
                  "in " + varsIn[i].type + " " + varsIn[i].nameGLSL + ";\n";
            }
          }
        }
        continue;
        ...
        for (int i = 0; i < varsOut.Num(); i++) {
          if (varsOut[i].declareInOut) {
            ...
            if (vkGLSL) {
              program +=
                  va("layout( location = %i ) out %s %s;\n", numDeclareOut,
                     varsOut[i].type.c_str(), varsOut[i].nameGLSL.c_str());
            } else {
              program +=
                  "out " + varsOut[i].type + " " + varsOut[i].nameGLSL + ";\n";
            }
    ...
    if (token == "static") {
      ...
      src.SkipWhiteSpace(
          true);  // remove white space between 'static' and the next token
      continue;
    }
    ...
    if (token == "const" && inMain) {
      program += (token.linesCrossed > 0)
                     ...
                     : (token.WhiteSpaceBeforeToken() > 0 ? " " : "");
      src.SkipWhiteSpace(
          true);  // remove white space between 'const' and the next token
      continue;
    }
    ...
    if (token == "{") {
      ...
      if (inMain && !justEnteredMain && (stage == SHADER_STAGE_VERTEX) &&
          vkGLSL && !in_Position4) {
    ...
    for (int i = 0; i < uniformList->Num(); i++) {
      if (token == (*uniformList)[i]) {
        ...
        // RB: for some unknown reasons has the Nvidia driver problems with
        // regular uniforms when using glUniform4fv so we need these uniform
    ...
    if (src.CheckTokenString(".")) {
      if (token == "vertex" || token == "fragment") {
        ...
        for (int i = 0; i < varsIn.Num(); i++) {
          if (member.Cmp(varsIn[i].nameCg) == 0) {
            ...
            */
            if (!in_Position4 && vkGLSL &&
                (varsIn[i].nameGLSL == "in_Position")) {
              program += "position4";
            } else {
        ...
        if (!foundInOut) {
          src.Error("invalid input parameter %s.%s", token.c_str(),
                    member.c_str());
          program += (token.linesCrossed > 0)
                         ? newline
                         : (token.WhiteSpaceBeforeToken() > 0 ? " " : "");
      ...
      if (token == "result") {
        ...
        if (!foundInOut) {
          src.Error("invalid output parameter %s.%s", token.c_str(),
                    member.c_str());
          program += (token.linesCrossed > 0)
                         ? newline
                         : (token.WhiteSpaceBeforeToken() > 0 ? " " : "");
