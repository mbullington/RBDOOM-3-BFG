...
void R_SurfaceToTextureAxis(const srfTriangles_t* tri, idVec3& origin,
  ...
  const idVec2 boundsOrg(floor((boundsMin.x + boundsMax.x) * 0.5f),
                         floor((boundsMin.y + boundsMax.y) * 0.5f));
  ...
  const idVec3 aXYZ = idDrawVert::GetSkinnedDrawVertPosition(
      tri->verts[tri->indexes[0]], joints);
  const idVec3 bXYZ = idDrawVert::GetSkinnedDrawVertPosition(
      tri->verts[tri->indexes[1]], joints);
  const idVec3 cXYZ = idDrawVert::GetSkinnedDrawVertPosition(
      tri->verts[tri->indexes[2]], joints);
...
================
*/
void R_ListGuis_f(const idCmdArgs& args) { uiManager->ListGuis(); }
