...
int idRenderEntityLocal::GetIndex() { return index; }
...
int idRenderLightLocal::GetIndex() { return index; }
...
void RenderEnvprobeLocal::UpdateRenderEnvprobe(
    const renderEnvironmentProbe_t* ep, bool forceUpdate) {}
void RenderEnvprobeLocal::GetRenderEnvprobe(renderEnvironmentProbe_t* ep) {}
void RenderEnvprobeLocal::ForceUpdate() {}
int RenderEnvprobeLocal::GetIndex() { return index; }
...
void idRenderEntityLocal::ReadFromDemoFile(class idDemoFile* f) {
  ...
  ent.customShader =
      (declName[0] != 0) ? declManager->FindMaterial(declName) : NULL;
  ...
  ent.referenceSound =
      soundIndex != -1 ? common->SW()->EmitterForIndex(soundIndex) : NULL;

  const char* mdlName = f->ReadHashString();
  ent.hModel =
      (mdlName[0] != 0) ? renderModelManager->FindModel(mdlName) : NULL;
  ...
  */
  ...
  if (r_showDemo.GetBool()) {
    common->Printf("DC_UPDATE_ENTITYDEF: %i = %s\n", index,
                   parms.hModel ? parms.hModel->Name() : "NULL");
  }
}
