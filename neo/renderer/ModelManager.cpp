...
idCVar binaryLoadRenderModels("binaryLoadRenderModels", "1", 0,
                              "enable binary load/write of render models");
idCVar preload_MapModels("preload_MapModels", "1", CVAR_SYSTEM | CVAR_BOOL,
                         "preload models during begin or end levelload");

// RB begin
idCVar postLoadExportModels("postLoadExportModels", "0",
                            CVAR_BOOL | CVAR_RENDERER,
                            "export models after loading to OBJ model format");
// RB end
...
=================
*/
idRenderModel* idRenderModelManagerLocal::GetModel(const char* _modelName,
  ...
  for (int i = hash.First(key); i != -1; i = hash.Next(i)) {
    ...
    if (canonical.Icmp(model->Name()) == 0) {
      if (!model->IsLoaded()) {
        ...
        // Get the timestamp on the original file, if it's newer than what is
        // stored in binary model, regenerate it
  ...
  if (postLoadExportModels.GetBool() &&
      (model != defaultModel && model != beamModel && model != spriteModel)) {
    ...
    {
      idFileLocal objFile(
          fileSystem->OpenFileWrite(exportedFileName, "fs_basepath"));
...
void idRenderModelManagerLocal::Preload(const idPreloadManifest& manifest) {
  if (preload_MapModels.GetBool()) {
    ...
    common->Printf(
        "%05d models preloaded ( or were already loaded ) in %5.1f seconds\n",
