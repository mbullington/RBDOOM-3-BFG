...
viewLight_t* R_SetLightDefViewLight(idRenderLightLocal* light) {
  ...
  viewLight_t* vLight = (viewLight_t*)R_ClearedFrameAlloc(
      sizeof(*vLight), FRAME_ALLOC_VIEW_LIGHT);
  vLight->lightDef = light;

  // the scissorRect will be expanded as the light bounds is accepted into
  // visible portal chains and the scissor will be reduced in R_AddSingleLight
  // based on the screen space projection
...
viewEntity_t* R_SetEntityDefViewEntity(idRenderEntityLocal* def) {
  ...
  viewEntity_t* vModel = (viewEntity_t*)R_ClearedFrameAlloc(
      sizeof(*vModel), FRAME_ALLOC_VIEW_ENTITY);
  vModel->entityDef = def;

  // the scissorRect will be expanded as the model bounds is accepted into
  // visible portal chains It will remain clear if the model is only needed for
  // shadows.
...
bool idRenderWorldLocal::CullEntityByPortals(const idRenderEntityLocal* entity,
  ...
  if (r_useEntityPortalCulling.GetInteger() == 1) {
    ...
    idRenderMatrix::GetFrustumCorners(corners, entity->inverseBaseModelProject,
                                      bounds_unitCube);
    ...
    idRenderMatrix::GetFrustumPlanes(frustumPlanes, baseModelProject, false,
                                     true);

    // exact clip of light faces against all planes
    for (int i = 0; i < 6; i++) {
      // the entity frustum planes face inward, so the planes that have the
      // view origin on the positive side will be the "back" faces of the
      // entity, which must have some fragment inside the portal stack planes to
      // be visible
...
bool idRenderWorldLocal::CullLightByPortals(const idRenderLightLocal* light,
  ...
  if (r_useLightPortalCulling.GetInteger() == 1) {
    ALIGNTYPE16 frustumCorners_t corners;
    idRenderMatrix::GetFrustumCorners(corners, light->inverseBaseLightProject,
                                      bounds_zeroOneCube);
    for (int i = 0; i < ps->numPortalPlanes; i++) {
      if (idRenderMatrix::CullFrustumCornersToPlane(
              corners, ps->portalPlanes[i]) == FRUSTUM_CULL_FRONT) {
    ...
    idPlane frustumPlanes[6];
    idRenderMatrix::GetFrustumPlanes(frustumPlanes, light->baseLightProject,
                                     true, true);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      // which must have some fragment inside the the portal stack planes to be
      // visible
      if (frustumPlanes[i].Distance(tr.viewDef->renderView.vieworg) <= 0.0f) {
        continue;
      }
...
===================
*/
void idRenderWorldLocal::AddAreaViewLights(int areaNum,
                                           const portalStack_t* ps) {
  portalArea_t* area = &portalAreas[areaNum];

  for (areaReference_t* lref = area->lightRefs.areaNext;
       lref != &area->lightRefs; lref = lref->areaNext) {
...
===================
*/
idScreenRect idRenderWorldLocal::ScreenRectFromWinding(
    const idWinding* w, const viewEntity_t* space) {
...
bool idRenderWorldLocal::PortalIsFoggedOut(const portal_t* p) {
  ...
  lightShader->EvaluateRegisters(
      regs, ldef->parms.shaderParms, tr.viewDef->renderView.shaderParms,
      tr.viewDef->renderView.time[0] * 0.001f, ldef->parms.referenceSound);
...
portals that the origin point can see into. The planes array defines a volume
with the planes pointing outside the volume. Zero planes assumes an unbounded
volume.
=======================
*/
...
void idRenderWorldLocal::FindViewLightsAndEntities() {
  ...
  if (r_singleArea.GetBool()) {
    ...
    FlowViewThroughPortals(tr.viewDef->renderView.vieworg, 5,
                           tr.viewDef->frustums[FRUSTUM_PRIMARY]);
  }
}
...
void idRenderWorldLocal::FlowLightThroughPortals(idRenderLightLocal* light) {
  ...
  idPlane frustumPlanes[6];
  idRenderMatrix::GetFrustumPlanes(frustumPlanes, light->baseLightProject, true,
                                   true);
...
==============
*/
int idRenderWorldLocal::NumPortals() const { return numInterAreaPortals; }
...
bool idRenderWorldLocal::AreasAreConnected(
    int areaNum1, int areaNum2, portalConnection_t connection) const {
