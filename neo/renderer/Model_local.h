...
class idRenderModelStatic : public idRenderModel {
 ...
 public:
  ...
  // when an md5 is instantiated, the inverted joints array is stored to allow
  // GPU skinning
...
class idMD5Mesh {
 ...
 private:
  ...
  float
      maxJointVertDist;  // maximum distance a vertex is separated from a joint
...
class idRenderModelMD3 : public idRenderModelStatic {
 ...
 private:
  ...
  void LerpMeshVertexes(srfTriangles_t* tri, const struct md3Surface_s* surf,
                        const float backlerp, const int frame,
...
class idRenderModelTrail : public idRenderModelStatic {
 ...
 public:
  ...
  void DrawTrail(int index, const struct renderEntity_s* ent,
                 srfTriangles_t* tri, float globalAlpha);
};
