...
void R_CalcInteractionFacing(const idRenderEntityLocal* ent,
  ...
  cullInfo.facing = (byte*)R_StaticAlloc(
      (numFaces + 1) * sizeof(cullInfo.facing[0]), TAG_RENDER_INTERACTION);
...
void R_CalcInteractionCullBits(const idRenderEntityLocal* ent,
  ...
  for (int i = 0; i < 6; i++) {
    R_GlobalPlaneToLocal(ent->modelMatrix, frustumPlanes[i],
  ...
  cullInfo.cullBits = (byte*)R_StaticAlloc(
      tri->numVerts * sizeof(cullInfo.cullBits[0]), TAG_RENDER_INTERACTION);
...
====================
*/
static srfTriangles_t* R_CreateInteractionLightTris(
  ...
  // at the moment RB: now we do with r_useHalfLambert, so don't cull back faces
  // if we have smooth shadowing enabled
...
Note that dangling edges outside the light frustum don't make silhouette planes
because a triangle outside the light frustum is considered facing and the "fake
triangle" on the outside of the dangling edge is also set to facing:
cullInfo.facing[numFaces] = 1;
=====================
*/
static srfTriangles_t* R_CreateInteractionShadowVolume(
    const idRenderEntityLocal* ent, const srfTriangles_t* tri,
    const idRenderLightLocal* light) {
...
void idInteraction::UnlinkAndFree() {
  ...
  if (renderWorld->interactionTable != NULL) {
    ...
    if (renderWorld->interactionTable[index] != this &&
        renderWorld->interactionTable[index] != INTERACTION_EMPTY) {
      common->Error(
...
void idInteraction::MakeEmpty() {
  ...
  const int interactionIndex =
      lightDef->index * entityDef->world->interactionTableWidth +
      entityDef->index;
...
void idInteraction::CreateStaticInteraction() {
  ...
  if (R_CullModelBoundsToLight(lightDef, bounds,
  ...
  for (int c = 0; c < model->NumSurfaces(); c++) {
    ...
    const idMaterial* const shader =
        R_RemapShaderBySkin(surf->shader, entityDef->parms.customSkin,
    ...
    // generate a set of indexes for the lit surfaces, culling away triangles
    // that are not at least partially inside the light
    if (shader->ReceivesLighting()) {
      srfTriangles_t* lightTris =
          R_CreateInteractionLightTris(entityDef, tri, lightDef, shader);
      if (lightTris != NULL) {
        ...
        sint->lightTrisIndexCache = vertexCache.AllocStaticIndex(
            lightTris->indexes,
            ALIGN(lightTris->numIndexes * sizeof(lightTris->indexes[0]),
                  INDEX_CACHE_ALIGN));
    ...
    if (HasShadows() && shader->SurfaceCastsShadow() && tri->silEdges != NULL) {
      ...
      if (lightDef->parms.prelightModel == NULL ||
          !model->IsStaticWorldModel() || r_skipPrelightShadows.GetBool()) {
        srfTriangles_t* shadowTris =
            R_CreateInteractionShadowVolume(entityDef, tri, lightDef);
        if (shadowTris != NULL) {
          // make a static index cache
          sint->shadowIndexCache = vertexCache.AllocStaticIndex(
              shadowTris->indexes,
              ALIGN(shadowTris->numIndexes * sizeof(shadowTris->indexes[0]),
                    INDEX_CACHE_ALIGN));
...
void R_ShowInteractionMemory_f(const idCmdArgs& args) {
  ...
  for (int i = 0; i < tr.primaryWorld->lightDefs.Num(); i++) {
    ...
    for (idInteraction* inter = light->firstInteraction; inter != NULL;
