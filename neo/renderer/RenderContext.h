...
// This is for "official" HDMI 3D support with with the left eye above the right
// and a guard band in the middle Some displays which don't support this can
// still do stereo 3D by packing 2 eyes into a single (mono-sized) buffer
enum hdmi3DState_t {
  HDMI3D_NOT_SUPPORTED,  // The TV doesn't support it
  HDMI3D_NOT_ENABLED,    // The TV supports it, but the user disabled it
  HDMI3D_NOT_ACTIVE,  // The TV supports it, and the user enabled it, but it's
                      // not active
  HDMI3D_ACTIVE
};
...
class idRenderContext {
 public:
  ...
  void SetGamma(unsigned short red[256], unsigned short green[256],
  ...
  void ResolveTargetColor(idImage* image, int srcMinX, int srcMinY, int srcMaxX,
                          int srcMaxY, int dstX, int dstY);
  void ResolveTargetDepth(idImage* image, int srcMinX, int srcMinY, int srcMaxX,
                          int srcMaxY, int dstX, int dstY);
