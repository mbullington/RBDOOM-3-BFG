...
bool idRenderModelDecal::CreateProjectionParms(
    decalProjectionParms_t& parms, const idFixedWinding& winding,
    const idVec3& projectionOrigin, const bool parallel, const float fadeDepth,
    const idMaterial* material, const int startTime) {
  ...
  if (parallel) {
    ...
    for (int i = 0; i < winding.GetNumPoints(); i++) {
      parms.boundingPlanes[i].FromPoints(
  ...
  parms.textureAxis[0][3] =
      winding[0].s - (winding[0].ToVec3() * parms.textureAxis[0].Normal());
  ...
  parms.textureAxis[1][3] =
      winding[0].t - (winding[0].ToVec3() * parms.textureAxis[1].Normal());
...
=================
*/
void idRenderModelDecal::GlobalProjectionParmsToLocal(
    decalProjectionParms_t& localParms,
    const decalProjectionParms_t& globalParms, const idVec3& origin,
    const idMat3& axis) {
  ...
  for (int j = 0; j < NUM_DECAL_BOUNDING_PLANES; j++) {
    R_GlobalPlaneToLocal(modelMatrix, globalParms.boundingPlanes[j],
                         localParms.boundingPlanes[j]);
  }
  R_GlobalPlaneToLocal(modelMatrix, globalParms.fadePlanes[0],
                       localParms.fadePlanes[0]);
  R_GlobalPlaneToLocal(modelMatrix, globalParms.fadePlanes[1],
                       localParms.fadePlanes[1]);
  R_GlobalPlaneToLocal(modelMatrix, globalParms.textureAxis[0],
                       localParms.textureAxis[0]);
  R_GlobalPlaneToLocal(modelMatrix, globalParms.textureAxis[1],
                       localParms.textureAxis[1]);
  R_GlobalPointToLocal(modelMatrix, globalParms.projectionOrigin,
                       localParms.projectionOrigin);
...
=================
*/
void idRenderModelDecal::CreateDecalFromWinding(const idWinding& w,
                                                const idMaterial* decalMaterial,
                                                const idPlane fadePlanes[2],
                                                float fadeDepth,
                                                int startTime) {
  // Often we are appending a new triangle to an existing decal, so merge with
  // the previous decal if possible
  int decalIndex = (nextDecal - 1) & (MAX_DECALS - 1);
  if (decalIndex >= 0 && decals[decalIndex].material == decalMaterial &&
      decals[decalIndex].startTime == startTime &&
      decals[decalIndex].numVerts + w.GetNumPoints() <= MAX_DECAL_VERTS &&
      decals[decalIndex].numIndexes + 3 * (w.GetNumPoints() - 2) <=
          MAX_DECAL_INDEXES) {
  } else {
  ...
  for (; (((decal.numIndexes * sizeof(triIndex_t)) & 15) != 0);
       decal.numIndexes += 3) {
    assert(decal.numIndexes + 3 <= MAX_DECAL_INDEXES);
...
============
*/
static void R_DecalPointCullStatic(byte* cullBits, const idPlane* planes,
  ...
  for (int i = 0; i < numVerts;) {
    ...
    for (; i <= nextNumVerts; i += 4) {
      ...
      const __m128 d0 =
          _mm_madd_ps(vX, p0X, _mm_madd_ps(vY, p0Y, _mm_madd_ps(vZ, p0Z, p0W)));
      const __m128 d1 =
          _mm_madd_ps(vX, p1X, _mm_madd_ps(vY, p1Y, _mm_madd_ps(vZ, p1Z, p1W)));
      const __m128 d2 =
          _mm_madd_ps(vX, p2X, _mm_madd_ps(vY, p2Y, _mm_madd_ps(vZ, p2Z, p2W)));
      const __m128 d3 =
          _mm_madd_ps(vX, p3X, _mm_madd_ps(vY, p3Y, _mm_madd_ps(vZ, p3Z, p3W)));
      const __m128 d4 =
          _mm_madd_ps(vX, p4X, _mm_madd_ps(vY, p4Y, _mm_madd_ps(vZ, p4Z, p4W)));
      const __m128 d5 =
          _mm_madd_ps(vX, p5X, _mm_madd_ps(vY, p5Y, _mm_madd_ps(vZ, p5Z, p5W)));
...
void idRenderModelDecal::CreateDecal(const idRenderModel* model,
                                     const decalProjectionParms_t& localParms) {
  int maxVerts = 0;
  ...
  for (int surfNum = 0; surfNum < model->NumSurfaces(); surfNum++) {
    ...
    R_DecalPointCullStatic(cullBits.Ptr(), localParms.boundingPlanes,
                           tri->verts, tri->numVerts);

    // start streaming the indexes
    idODSStreamedArray<triIndex_t, 256, SBT_QUAD, 3> indexesODS(
    ...
    for (int i = 0; i < tri->numIndexes;) {
      ...
      for (; i <= nextNumIndexes; i += 3) {
        ...
        const idDrawVert* verts[3] = {&tri->verts[i0], &tri->verts[i1],
                                      &tri->verts[i2]};

        // skip back facing triangles
        const idPlane plane(verts[0]->xyz, verts[1]->xyz, verts[2]->xyz);
        if (plane.Normal() *
                localParms.boundingPlanes[NUM_DECAL_BOUNDING_PLANES - 2]
                    .Normal() <
            -0.1f) {
          continue;
        }
        ...
        if (localParms.parallel) {
          ...
          for (int j = 0; j < 3; j++) {
            ...
            float scale;
            localParms.boundingPlanes[NUM_DECAL_BOUNDING_PLANES - 1]
                .RayIntersection(verts[j]->xyz, dir, scale);
        ...
        // if there is a part of the triangle between the bounding planes then
        // clip the triangle based on depth and add decals for the depth faded
        // parts
        if (fw.GetNumPoints() != 0) {
          idFixedWinding back;

          if (fw.Split(&back, localParms.fadePlanes[0], 0.1f) == SIDE_CROSS) {
            CreateDecalFromWinding(back, localParms.material,
                                   localParms.fadePlanes, localParms.fadeDepth,
                                   localParms.startTime);
          }
          ...
          CreateDecalFromWinding(fw, localParms.material, localParms.fadePlanes,
                                 localParms.fadeDepth, localParms.startTime);
        }
      }
    }
  }
...
void idRenderModelDecal::CreateDeferredDecals(const idRenderModel* model) {
  for (unsigned int i = firstDeferredDecal; i < nextDeferredDecal; i++) {
    decalProjectionParms_t& parms =
        deferredDecals[i & (MAX_DEFERRED_DECALS - 1)];
    if (parms.startTime >
        tr.viewDef->renderView.time[0] - DEFFERED_DECAL_TIMEOUT) {
...
=====================
*/
void idRenderModelDecal::AddDeferredDecal(
    const decalProjectionParms_t& localParms) {
...
=====================
*/
static void R_CopyDecalSurface(idDrawVert* verts, int numVerts,
                               triIndex_t* indexes, int numIndexes,
  ...
  const __m128i vector_int_num_verts =
      _mm_shuffle_epi32(_mm_cvtsi32_si128(numVerts), 0);
  const __m128i vector_short_num_verts =
      _mm_packs_epi32(vector_int_num_verts, vector_int_num_verts);
  ...
  // copy vertices and apply depth/time based fading
  for (int i = 0; i < decal->numVerts; i++) {
    // NOTE: bad out-of-order write-combined write, SIMD code does the right
    // thing
    verts[numVerts + i] = decal->verts[i];
    for (int j = 0; j < 4; j++) {
      verts[numVerts + i].color[j] =
          idMath::Ftob(fadeColor[j] * decal->vertDepthFade[i]);
    }
  }
  ...
  for (int i = 0; i < decal->numIndexes; i += 2) {
    assert(decal->indexes[i + 0] < decal->numVerts &&
           decal->indexes[i + 1] < decal->numVerts);
    WriteIndexPair(&indexes[numIndexes + i], numVerts + decal->indexes[i + 0],
                   numVerts + decal->indexes[i + 1]);
  }
...
=====================
*/
drawSurf_t* idRenderModelDecal::CreateDecalDrawSurf(const viewEntity_t* space,
                                                    unsigned int index) {
  ...
  // create a new triangle surface in frame memory so it gets automatically
  // disposed of
  ...
  idDrawVert* mappedVerts =
      (idDrawVert*)vertexCache.MappedVertexBuffer(newTri->ambientCache);
  triIndex_t* mappedIndexes =
      (triIndex_t*)vertexCache.MappedIndexBuffer(newTri->indexCache);
  ...
  for (unsigned int i = firstDecal; i < nextDecal; i++) {
    ...
    const float f = (deltaTime > decalInfo.stayTime)
                        ? ((float)fadeTime / decalInfo.fadeTime)
                        : 0.0f;

    ALIGNTYPE16 float fadeColor[4];
    for (int j = 0; j < 4; j++) {
      fadeColor[j] = 255.0f * (decalInfo.start[j] +
                               (decalInfo.end[j] - decalInfo.start[j]) * f);
    }

    // use SIMD optimized routine to copy the vertices and indices directly to
    // write-combined memory this also applies any depth/time based fading while
    // copying
    R_CopyDecalSurface(mappedVerts, numVerts, mappedIndexes, numIndexes, &decal,
                       fadeColor);
...
void idRenderModelDecal::ReadFromDemoFile(idDemoFile* f) {
  ...
  for (unsigned int i = firstDeferredDecal; i < nextDeferredDecal; i++) {
    decalProjectionParms_t& deferredDecal =
        deferredDecals[i & (MAX_DEFERRED_DECALS - 1)];
    f->ReadInt(deferredDecal.startTime);
    f->ReadBool(deferredDecal.parallel);
    f->ReadBool(deferredDecal.force);
    f->Read(deferredDecal.boundingPlanes,
            sizeof(idPlane) * NUM_DECAL_BOUNDING_PLANES);
    ...
    const char* matName = f->ReadHashString();
    deferredDecal.material =
        matName[0] ? declManager->FindMaterial(matName) : NULL;
  }
...
void idRenderModelDecal::WriteToDemoFile(idDemoFile* f) const {
  ...
  for (i = firstDeferredDecal; i < nextDeferredDecal; i++) {
    ...
    f->WriteHashString(
        deferredDecal.material ? deferredDecal.material->GetName() : "");
  }
