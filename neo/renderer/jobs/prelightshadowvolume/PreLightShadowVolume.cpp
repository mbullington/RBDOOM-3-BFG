...
void PreLightShadowVolumeJob(const preLightShadowVolumeParms_t* parms) {
  ...
  if (shadowZMin < shadowZMax) {
    ...
    // render with Z-fail.
    if (parms->triangleBounds
            .Expand(parms->zNear * INSIDE_SHADOW_VOLUME_EXTRA_STRETCH)
            .ContainsPoint(parms->localViewOrigin)) {
    ...
    // must always draw the caps because pre-light shadows do not project to
    // infinity
