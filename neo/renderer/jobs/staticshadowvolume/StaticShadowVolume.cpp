...
void StaticShadowVolumeJob(const staticShadowVolumeParms_t* parms) {
  if (parms->tempCullBits == NULL) {
    *const_cast<byte**>(&parms->tempCullBits) =
        (byte*)_alloca16(TEMP_CULLBITS(parms->numVerts));
  }
  ...
  if (shadowZMin < shadowZMax) {
    ...
    if (R_ViewPotentiallyInsideInfiniteShadowVolume(
      ...
      if (parms->useShadowPreciseInsideTest && parms->verts != NULL &&
          parms->indexes != NULL) {
        renderZFail = R_ViewInsideShadowVolume(
            parms->tempCullBits, parms->verts, parms->numVerts, parms->indexes,
            parms->numIndexes, parms->localLightOrigin, parms->localViewOrigin,
            parms->zNear * INSIDE_SHADOW_VOLUME_EXTRA_STRETCH);
      } else {
        renderZFail = true;
      }
    ...
    numShadowIndices = (parms->forceShadowCaps || renderZFail)
                           ? parms->numShadowIndicesWithCaps
                           : parms->numShadowIndicesNoCaps;
  }
