...
======================
*/
bool R_ViewPotentiallyInsideInfiniteShadowVolume(const idBounds& occluderBounds,
  ...
  // If the light is inside the geometry bounding box then the shadow is
  // projected in all directions and any view position is inside the infinte
  // shadow projection.
...
====================
*/
static void R_ShadowVolumeCullBits(byte* cullBits, byte& totalOr,
                                   const float radius, const idPlane* planes,
                                   const idShadowVert* verts,
                                   const int numVerts) {
  ...
  vecTotalOrInt = _mm_or_si128(
      vecTotalOrInt, _mm_shuffle_epi32(vecTotalOrInt, _MM_SHUFFLE(1, 0, 3, 2)));
  vecTotalOrInt = _mm_or_si128(
      vecTotalOrInt, _mm_shuffle_epi32(vecTotalOrInt, _MM_SHUFFLE(2, 3, 0, 1)));
...
===================
*/
static float R_SegmentToSegmentDistanceSquare(const idVec3& start1,
                                              const idVec3& end1,
                                              const idVec3& start2,
                                              const idVec3& end2) {
  ...
  if (dotDir1Dir1 < idMath::FLT_SMALLEST_NON_DENORMAL ||
      dotDir2Dir2 < idMath::FLT_SMALLEST_NON_DENORMAL) {
    // At least one of the lines is degenerate.
    // The returned length is correct only if both lines are degenerate
    // otherwise the start point of the degenerate line has to be projected onto
    // the other line to calculate the shortest distance. The degenerate case is
...
===================
*/
bool R_LineIntersectsTriangleExpandedWithSphere(
    const idVec3& lineStart, const idVec3& lineEnd, const idVec3& lineDir,
    const float lineLength, const float sphereRadius, const idVec3& triVert0,
    const idVec3& triVert1, const idVec3& triVert2) {
  // edge directions
  ...
  if (R_SegmentToSegmentDistanceSquare(lineStart, lineEnd, triVert0, triVert1) <
      radiusSqr) {
    return true;
  }

  if (R_SegmentToSegmentDistanceSquare(lineStart, lineEnd, triVert1, triVert2) <
      radiusSqr) {
    return true;
  }

  if (R_SegmentToSegmentDistanceSquare(lineStart, lineEnd, triVert2, triVert0) <
      radiusSqr) {
    return true;
  }
...
===================
*/
bool R_ViewInsideShadowVolume(byte* cullBits, const idShadowVert* verts,
                              int numVerts, const triIndex_t* indexes,
                              int numIndexes, const idVec3& localLightOrigin,
  ...
  idODSStreamedArray<triIndex_t, 256, SBT_QUAD, 3> indexesODS(indexes,
                                                              numIndexes);
  ...
  for (int i = 0; i < numIndexes;) {
    ...
    for (; i <= nextNumIndexes; i += 3) {
      ...
      // If there are no points on both sides of both the ray planes, no
      // intersection.
      ...
      if (R_LineIntersectsTriangleExpandedWithSphere(
              lineStart, lineEnd, lineDir, lineLength, zNear,
