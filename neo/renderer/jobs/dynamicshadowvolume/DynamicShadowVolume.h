...
A dynamic shadow is cast from a model touching a light where either the model or
light is dynamic or both. A dynamic shadow volume extends to infinity which
allows the end caps to be omitted when the view is outside and far enough away
from the shadow volume.

A dynamic shadow volume is created at run-time in this job. This job also
determines whether or not end caps need to be generated and rendered, whether or
not the shadow volume needs to be rendered with Z-Fail, and optionally
calculates the shadow volume depth bounds.

The occluder triangles are optionally culled to the light projection matrix.
This is particularly important for spot lights that would otherwise cast shadows
in all directions. However, there can also be significant savings when a small
point light touches a large model like for instance a world model.

================================================================================================
*/

#define TEMP_ROUND16(x) ((x + 15) & ~15)
#define TEMP_FACING(numIndexes)              \
  TEMP_ROUND16(((numIndexes / 3 + 3) & ~3) + \
               1)  // rounded up for SIMD, plus 1 for dangling edges
#define TEMP_CULL(numIndexes) \
  TEMP_ROUND16(((numIndexes / 3 + 3) & ~3))  // rounded up for SIMD
#define TEMP_VERTS(numVerts) TEMP_ROUND16(numVerts * sizeof(idVec4))
#define OUTPUT_INDEX_BUFFER_SIZE 4096

struct silEdge_t {
  // NOTE: using triIndex_t for the planes is dubious, as there can be 2x the
  // faces as verts
...
struct dynamicShadowVolumeParms_t {
  ...
  volatile shadowVolumeState_t*
      shadowVolumeState;  // streamed out to main memory
