...
// RB: similar to Q3A - allow separate codepaths between OpenGL 3.x, OpenGL ES
// versions
enum graphicsDriverType_t {
  ...
  GLDRV_OPENGL_ES2,
  GLDRV_OPENGL_ES3,
  GLDRV_OPENGL_MESA,  // fear this, it is probably the best to disable GPU
                      // skinning and run shaders in GLSL ES 1.0
...
struct glconfig_t {
  ...
  // only true with uniform buffer support and an OpenGL driver that supports
  // GLSL >= 1.50
  bool gpuSkinningAvailable;
  // RB end

  stereo3DMode_t stereo3Dmode;
  int nativeScreenWidth;   // this is the native screen width resolution of the
                           // renderer
  int nativeScreenHeight;  // this is the native screen height resolution of the
                           // renderer
  ...
  // PC vid code sets this, converting from diagonals / inches / whatever as
  // needed. If the value can't be determined, set something reasonable, like
  // 50cm.
...
class idRenderSystem {
 public:
  ...
  // This is used to calculate stereoscopic screen offset for a given
  ...
  void DrawStretchPic(const idVec4& rect, const idVec4& st,
                      const idMaterial* material) {
    DrawStretchPic(rect.x, rect.y, rect.z, rect.w, st.x, st.y, st.z, st.w,
                   material);
  }
  ...
  // with the rendering of the closed off command buffers by
  // RenderCommandBuffers()
  virtual const emptyCommand_t* SwapCommandBuffers(
      uint64* frontEndMicroSec, uint64* backEndMicroSec, uint64* shadowMicroSec,
      uint64* gpuMicroSec, backEndCounters_t* bc,
      performanceCounters_t* pc) = 0;

  // SwapCommandBuffers operation can be split in two parts for non-smp
  // rendering where the GPU is idled intentionally for minimal latency.
  ...
  // by SwapCommandBuffers().  No references should be made to the current
  // frameData, so new scenes and GUIs can be built up in parallel with the
  // rendering.
  ...
  virtual void TakeScreenshot(int width, int height, const char* fileName,
                              int samples, struct renderView_s* ref,
                              int exten) = 0;

  // RB
  virtual byte* CaptureRenderToBuffer(int width, int height,
                                      renderView_t* ref) = 0;
  ...
  virtual void CaptureRenderToImage(const char* imageName,
                                    bool clearColorAfterCopy = false) = 0;
...
// display
void R_LightProjectionMatrix(const idVec3& origin, const idPlane& rearPlane,
                             idVec4 mat[4]);
