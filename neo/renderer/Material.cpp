...
idCVar r_forceSoundOpAmplitude(
    "r_forceSoundOpAmplitude", "0", CVAR_FLOAT,
    "Don't call into the sound system for amplitudes");
...
idImage* idMaterial::GetEditorImage() const {
  ...
  if (!editorImageName.Length()) {
    ...
    editorImage = globalImages->ImageFromFile(editorImageName, TF_DEFAULT,
                                              TR_REPEAT, TD_DIFFUSE);

    // look for the diffusemap alternative like TrenchBroom does
    // this is required to have the texture dimensions for the
    // convertMapToValve220 cmd
...
// RB - just look for first stage and fallback to editor image like D3Radiant
// does
...
=================
*/
int idMaterial::ParseEmitOp(idLexer& src, int a, expOpType_t opType,
                            int priority) {
  int b;
...
===============
*/
void idMaterial::ParseStencilCompare(const idToken& token,
                                     stencilComp_t* stencilComp) {
  ...
  common->Warning(
      "Material %s expected a valid stencil comparison function. Got %s",
      GetName(), token.c_str());
}
...
===============
*/
void idMaterial::ParseStencilOperation(const idToken& token,
                                       stencilOperation_t* stencilOp) {
  if (!token.Icmp("Keep")) {
  ...
  common->Warning(
      "Material %s expected a valid stencil operation function. Got %s",
      GetName(), token.c_str());
}
...
void idMaterial::ParseStencil(idLexer& src, stencilStage_t* stencilStage) {
  ...
  while (1) {
    ...
    if (!token.Icmp("Ref")) {
      ...
      if (!token.IsNumeric()) {
        common->Warning(
            "Material %s expected number for stencil ref value. Got %s",
            GetName(), token.c_str());
        continue;
      }
      ...
      if (token.GetIntValue() > 255 || token.GetIntValue() < 0) {
        common->Warning(
            "Material %s expected stencil ref value between 0 and 255. Got %s",
            GetName(), token.c_str());
        continue;
      }
    ...
    common->Warning("Material %s expected a valid stencil keyword. Got %s.",
                    GetName(), token.c_str());
  }
}
...
void idMaterial::ParseStage(idLexer& src, const textureRepeat_t trpDefault) {
  ...
  textureUsage_t td;
  cubeFiles_t cubeMap;
  int cubeMapSize = 0;  // SP: The size of the cubemap for subimage uploading to
                        // the cubemap targets.
  ...
  while (1) {
    ...
    if (!token.Icmp("uncompressedCubeMap")) {
      if (r_useHighQualitySky.GetBool()) {
        td = TD_HIGHQUALITY_CUBE;  // motorsep 05-17-2015; token to mark
                                   // cubemap/skybox to be uncompressed texture
      } else {
    ...
    if (!token.Icmp("program")) {
      if (src.ReadTokenOnLine(&token)) {
        newStage.vertexProgram = renderProgManager.FindShader(
            token.c_str(), SHADER_STAGE_VERTEX, "", 0, false);
        newStage.fragmentProgram = renderProgManager.FindShader(
            token.c_str(), SHADER_STAGE_FRAGMENT, "", 0, false);
      }
      continue;
    }
    ...
    if (!token.Icmp("stencil")) {
      ...
      ss->stencilStage =
          (stencilStage_t*)Mem_Alloc(sizeof(stencilStage_t), TAG_MATERIAL);
  ...
  if (newStage.fragmentProgram || newStage.vertexProgram) {
    newStage.glslProgram = renderProgManager.FindGLSLProgram(
        GetName(), newStage.vertexProgram, newStage.fragmentProgram);
  ...
  // create a new coverage stage on the fly - copy all data from the current
  // stage
  if ((td == TD_DIFFUSE) && ss->hasAlphaTest) {
    ...
    *newCoverageStage = *ss;
    ...
    // toggle alphatest off for the current stage so it doesn't get called
    // during the depth fill pass
    ...
    if (imageName[0]) {
      coverageTS->image = globalImages->ImageFromFile(
          imageName, tf, trp, TD_COVERAGE, cubeMap, cubeMapSize);
      if (!coverageTS->image) {
        coverageTS->image = globalImages->defaultImage;
      }
    } else if (!coverageTS->cinematic && !coverageTS->dynamic &&
               !ss->newStage) {
...
void idMaterial::ParseMaterial(idLexer& src) {
  ...
  while (1) {
    ...
    // twoSided
      ...
      if (!r_useShadowMapping
               .GetBool())  // motorsep 11-08-2014; when shadow mapping is on,
                            // we allow two-sided surfaces to cast shadows
      {
        SetMaterialFlag(MF_NOSHADOWS);
      }
    }
    ...
    // light volumes
      ...
      lightFalloffImage = globalImages->ImageFromFile(
          copy, TF_DEFAULT, TR_CLAMP /* TR_CLAMP_TO_ZERO */, TD_DEFAULT);
      continue;
    }
    ...
    else if (!token.Icmp("rmaomap") || !token.Icmp("reflectionmap") ||
             !token.Icmp("pbrmap")) {
      str = R_ParsePastImageProgram(src);
    ...
    // motorsep 11-23-2014; material LOD keys that define what LOD iteration the
    // surface falls into
...
bool idMaterial::Parse(const char* text, const int textLength,
    ...
    if (pStage->newStage) {
      for (int j = 0; j < pStage->newStage->numFragmentProgramImages; j++) {
        if (pStage->newStage->fragmentProgramImages[j] ==
            globalImages->originalCurrentRenderImage) {
          if (sort != SS_PORTAL_SKY) {
            sort = SS_POST_PROCESS;
            coverage = MC_TRANSLUCENT;
          }
  ...
  */
  ...
  if (numRegisters) {
    expressionRegisters = (float*)R_StaticAlloc(
        numRegisters * sizeof(expressionRegisters[0]), TAG_MATERIAL);
...
===============
*/
bool idMaterial::Save(const char* fileName) { return ReplaceSourceFileText(); }
...
void idMaterial::EvaluateRegisters(
    float* registers, const float localShaderParms[MAX_ENTITY_SHADER_PARMS],
    const float globalShaderParms[MAX_GLOBAL_SHADER_PARMS],
    const float floatTime, idSoundEmitter* soundEmitter) const {
  int i, b;
  expOp_t* op;
...
// RB: added because we can't rely on the FFmpeg feedback how long a video
// really is
...
void idMaterial::CheckForConstantRegisters() {
  ...
  EvaluateRegisters(constantRegisters, shaderParms,
                    viewDef.renderView.shaderParms, 0.0f, 0);
}
...
=================
*/
size_t idMaterial::Size() const { return sizeof(idMaterial); }
...
void idMaterial::SetFastPathImages() {
  ...
  for (int surfaceStageNum = 0; surfaceStageNum < GetNumStages();
       surfaceStageNum++) {
    ...
    for (int i = 0; i < 4; i++) {
      if (idMath::Fabs(constantRegisters[surfaceStage->color.registers[i]] -
                       1.0f) > 0.1f) {
        goto fail;
      }
    }
  ...
  // we also need a diffuse image, because we can't get a pure black with our
  // YCoCg conversion from 565 DXT.  The general-path code also sets the diffuse
  // color to 0 in the default case, but the fast path can't.
