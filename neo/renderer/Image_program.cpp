...
=================
*/
static void R_HeightmapToNormalMap(byte* data, int width, int height,
                                   float scale) {
  int i, j;
  byte* depth;
...
===================
*/
static void R_AddNormalMaps(byte* data1, int width1, int height1, byte* data2,
...
===================
*/
static void R_ImageAdd(byte* data1, int width1, int height1, byte* data2,
                       int width2, int height2) {
  int i, j;
  int c;
  byte* newMap;
...
// SP begin
static void R_CombineRgba(byte* data1, int width1, int height1, byte* data2,
                          int width2, int height2, byte* data3, int width3,
                          int height3) {
  ...
  for (int j = 0; j < 4 * height1; j += 4) {
    for (int i = 0; i < 4 * width1; i += 4) {
      // Assume that these textures are all grayscale images. just take the r
      // channel of each and set them to the respective rgb.
...
===================
*/
static bool R_ParseImageProgram_r(idLexer& src, byte** pic, int* width,
                                  int* height, ID_TIME_T* timestamps,
  ...
  // Since all interaction shaders now assume YCoCG diffuse textures.  We
  // replace all entries for the intrinsic _black texture to the black texture
  // on disk.  Doing this will cause a YCoCG compliant texture to be generated.
  // Without a YCoCG compliant black texture we will get color artifacts for any
  // interaction material that specifies the _black texture.
  if (token == "_black") {
  ...
  if (!token.Icmp("combineRgba")) {
    ...
    if (!R_ParseImageProgram_r(src, pic2 ? &pic3 : NULL, &width3, &height3,
                               timestamps, usage)) {
      if (pic) {
        R_StaticFree(*pic);
        *pic = NULL;
    ...
    // process it
    if (pic) {
      R_CombineRgba(*pic, *width, *height, pic2, width2, height2, pic3, width3,
                    height3);
      R_StaticFree(pic2);
      R_StaticFree(pic3);
    }
