...
static const unsigned int BLGRID_MAGIC =
    ('P' << 24) | ('R' << 16) | ('O' << 8) | BLGRID_VERSION;

static const int MAX_LIGHTGRID_ATLAS_SIZE = 2048;
static const int MAX_AREA_LIGHTGRID_POINTS =
    (MAX_LIGHTGRID_ATLAS_SIZE / LIGHTGRID_IRRADIANCE_SIZE) *
    (MAX_LIGHTGRID_ATLAS_SIZE / LIGHTGRID_IRRADIANCE_SIZE);
...
void LightGrid::SetupLightGrid(const idBounds& bounds, const char* mapName,
                               ...
                               const idVec3& gridSize, int _area, int numAreas,
                               int maxProbes, bool printToConsole) {
  ...
  while (numGridPoints > maxGridPoints) {
    for (int i = 0; i < 3; i++) {
      lightGridOrigin[i] =
          lightGridSize[i] * ceil(bounds[0][i] / lightGridSize[i]);
      maxs[i] = lightGridSize[i] * floor(bounds[1][i] / lightGridSize[i]);
      lightGridBounds[i] =
          (maxs[i] - lightGridOrigin[i]) / lightGridSize[i] + 1;
    }

    numGridPoints =
        lightGridBounds[0] * lightGridBounds[1] * lightGridBounds[2];
  ...
  if (numGridPoints > 0) {
    ...
    if (printToConsole) {
      idLib::Printf("\narea %i of %i (%i x %i x %i) = %i grid points \n", area,
                    numAreas, lightGridBounds[0], lightGridBounds[1],
                    lightGridBounds[2], numGridPoints);
      idLib::Printf("area %i grid size (%i %i %i)\n", area,
                    (int)lightGridSize[0], (int)lightGridSize[1],
                    (int)lightGridSize[2]);
      idLib::Printf("area %i grid bounds (%i %i %i)\n", area,
                    (int)lightGridBounds[0], (int)lightGridBounds[1],
                    (int)lightGridBounds[2]);
      // idLib::Printf( "area %i %9u x %" PRIuSIZE " = lightGridSize =
      // (%.2fMB)\n", area, numGridPoints, sizeof( lightGridPoint_t ), ( float
      // )( lightGridPoints.MemoryUsed() ) / ( 1024.0f * 1024.0f ) );
    }
...
int LightGrid::GridCoordToProbeIndex(int gridCoord[3]) {
  ...
  int gridPointIndex = gridCoord[0] * gridStep[0] + gridCoord[1] * gridStep[1] +
                       gridCoord[2] * gridStep[2];
...
idVec3 LightGrid::GetGridCoordDebugColor(int gridCoord[3]) {
  ...
  static idVec4 colors[numColors] = {colorBlack, colorBlue,   colorCyan,
...
void LightGrid::CalculateLightGridPointPositions(const idRenderWorld* world,
                                                 int area) {
    ...
    for (int j = 0; j < lightGridBounds[1]; j += 1) {
      for (int k = 0; k < lightGridBounds[2]; k += 1) {
        ...
        lightGridPoint_t* gridPoint =
            &lightGridPoints[pos[0] * gridStep[0] + pos[1] * gridStep[1] +
                             pos[2] * gridStep[2]];
...
#if 1
          // RB: do what q3map1 did - try to nudge the origin around to find a
          // valid point
  ...
  idLib::Printf("area %i: %i of %i grid points in empty space (%.2f%%)\n", area,
                invalidCount, lightGridPoints.Num(),
                ((float)invalidCount / lightGridPoints.Num()) * 100);
}
...
void idRenderWorldLocal::SetupLightGrid() {
  ...
  if (loaded) {
    ...
    for (int i = 0; i < numPortalAreas; i++) {
      ...
      area->lightGrid.SetupLightGrid(area->globalBounds, mapName, this,
                                     defaultLightGridSize, i, numPortalAreas,
                                     -1, true);
...
void idRenderWorldLocal::LoadLightGridImages() {
  ...
  for (int i = 0; i < numPortalAreas; i++) {
    ...
    if (!area->lightGrid.irradianceImage) {
      ...
      area->lightGrid.irradianceImage = globalImages->ImageFromFile(
          filename, TF_LINEAR, TR_CLAMP, TD_R11G11B10F, CF_2D);
    } else {
...
void idRenderWorldLocal::WriteLightGrid(idFile* fp,
                                        const LightGrid& lightGrid) {
  fp->WriteFloatString(
      "lightGridPoints { /* area = */ %i /* numLightGridPoints = */ %i /* "
      "imageSingleProbeSize = */ %i /* imageBorderSize = */ %i \n",
      lightGrid.area, lightGrid.lightGridPoints.Num(),
      lightGrid.imageSingleProbeSize, lightGrid.imageBorderSize);

  fp->WriteFloatString("/* gridMins */ ");
  fp->WriteFloatString("\t ( %f %f %f )\n", lightGrid.lightGridOrigin[0],
                       lightGrid.lightGridOrigin[1],
                       lightGrid.lightGridOrigin[2]);

  fp->WriteFloatString("/* gridSize */ ");
  fp->WriteFloatString("\t ( %f %f %f )\n", lightGrid.lightGridSize[0],
                       lightGrid.lightGridSize[1], lightGrid.lightGridSize[2]);

  fp->WriteFloatString("/* gridBounds */ ");
  fp->WriteFloatString("%i %i %i\n\n", lightGrid.lightGridBounds[0],
                       lightGrid.lightGridBounds[1],
                       lightGrid.lightGridBounds[2]);

  for (int i = 0; i < lightGrid.lightGridPoints.Num(); i++) {
    const lightGridPoint_t* gridPoint = &lightGrid.lightGridPoints[i];

    fp->WriteFloatString("/* lgp %i */ %d ( %f %f %f )", i,
                         (int)gridPoint->valid, gridPoint->origin[0],
                         gridPoint->origin[1], gridPoint->origin[2]);
    ...
    for (int j = 0; j < shSize(3); j++) {
      fp->WriteFloatString("%f %f %f ", gridPoint->shRadiance[j][0],
                           gridPoint->shRadiance[j][1],
                           gridPoint->shRadiance[j][2]);
    }
...
bool idRenderWorldLocal::LoadLightGridFile(const char* name) {
  ...
  if (file != NULL) {
    ...
    if ((magic == BLGRID_MAGIC) && (sourceTimeStamp == storedTimeStamp) &&
        (numEntries > 0)) {
      loaded = true;
  ...
  if (!loaded) {
    ...
    src = new (TAG_RENDER)
        idLexer(fileName, LEXFL_NOSTRINGCONCAT | LEXFL_NODOLLARPRECOMPILE);
    if (!src->IsLoaded()) {
      delete src;
      return false;
    }
    ...
    int numEntries = 0;
    idFileLocal outputFile(
        fileSystem->OpenFileWrite(generatedFileName, "fs_basepath"));
    if (outputFile != NULL) {
    ...
    } else if (token.type != TT_NUMBER && token.subtype != TT_STRING &&
               token.subtype != TT_NAME) {
    ...
    if (lightGridVersion != LGRID_VERSION && lightGridVersion != 4) {
      common->Warning("%s has version %i instead of %i", fileName.c_str(),
                      lightGridVersion, LGRID_VERSION);
      delete src;
      return false;
    }
    ...
    while (1) {
      ...
      src->Error("idRenderWorldLocal::LoadLightGridFile: bad token \"%s\"",
                 token.c_str());
    }
...
void idRenderWorldLocal::ParseLightGridPoints(idLexer* src, idFile* fileOut) {
  ...
  if (imageProbeSize < 8) {
    src->Error("ParseLightGridPoints: bad single image probe size %i",
               imageProbeSize);
    return;
  }
  ...
  if (imageBorderSize < 0) {
    src->Error("ParseLightGridPoints: bad probe border size %i",
               imageBorderSize);
    return;
  }
  ...
  idLib::Printf("\narea %i (%i x %i x %i) = %i grid points \n", areaIndex,
                area->lightGrid.lightGridBounds[0],
                area->lightGrid.lightGridBounds[1],
                area->lightGrid.lightGridBounds[2], numLightGridPoints);
  idLib::Printf("area %i grid size (%i %i %i)\n", areaIndex,
                (int)area->lightGrid.lightGridSize[0],
                (int)area->lightGrid.lightGridSize[1],
                (int)area->lightGrid.lightGridSize[2]);
  idLib::Printf("area %i grid bounds (%i %i %i)\n", areaIndex,
                (int)area->lightGrid.lightGridBounds[0],
                (int)area->lightGrid.lightGridBounds[1],
                (int)area->lightGrid.lightGridBounds[2]);
  // idLib::Printf( "area %i %9u x %" PRIuSIZE " = lightGridSize = (%.2fMB)\n",
  // areaIndex, numLightGridPoints, sizeof( lightGridPoint_t ), ( float )(
  // area->lightGrid.lightGridPoints.MemoryUsed() ) / ( 1024.0f * 1024.0f ) );
...
#if STORE_LIGHTGRID_SHDATA
      fileOut->WriteBigArray(gridPoint->shRadiance[0].ToFloatPtr(),
                             shSize(4) * 3);
#endif
    }
  }
...
void idRenderWorldLocal::ReadBinaryLightGridPoints(idFile* file) {
  ...
  if (imageProbeSize < 0) {
    idLib::Error("ReadBinaryLightGridPoints: bad imageProbeSize %i",
                 imageProbeSize);
    return;
  }
  ...
  if (imageBorderSize < 0) {
    idLib::Error("ReadBinaryLightGridPoints: bad imageBorderSize %i",
                 imageBorderSize);
    return;
  }
  ...
  for (int i = 0; i < numLightGridPoints; i++) {
    ...
    // file->ReadBigArray( gridPoint->shRadiance[0].ToFloatPtr(), shSize( 3 ) *
    // 3 );
  }
}
...
/// u and v should be center adressing and in [-1.0 + invSize.. 1.0 - invSize]
/// range.
static inline float CubemapTexelSolidAngle(float u, float v,
                                           float _invFaceSize) {
  // Specify texel area.
  ...
  const float solidAngle = AreaElement(x1, y1) - AreaElement(x0, y1) -
                           AreaElement(x1, y0) + AreaElement(x0, y0);

  return solidAngle;
}

static inline idVec3 MapXYSToDirection(uint64 x, uint64 y, uint64 s,
...
void CalculateLightGridPointJob(calcLightGridPointParms_t* parms) {
  ...
  for (int side = 0; side < 6; side++) {
    for (int x = 0; x < sourceImageSize.x; x++) {
      for (int y = 0; y < sourceImageSize.y; y++) {
        ...
        idVec3 dir =
            MapXYSToDirection(x, y, side, sourceImageSize.x, sourceImageSize.y);

        float u, v;
        idVec3 radiance;
        R_SampleCubeMapHDR16F(dir, ENVPROBE_CAPTURE_SIZE, buffers, &radiance[0],
                              u, v);
  ...
  for (int x = 0; x < parms->outWidth; x++) {
    for (int y = 0; y < parms->outHeight; y++) {
      ...
      idVec3 sampleIrradianceSh =
          shEvaluateDiffuse<idVec3, 4>(shRadiance, dir) / idMath::PI;
      ...
      parms->outBuffer[(y * parms->outWidth + x) * 3 + 0] =
          F32toF16(outColor[0]);
      parms->outBuffer[(y * parms->outWidth + x) * 3 + 1] =
          F32toF16(outColor[1]);
      parms->outBuffer[(y * parms->outWidth + x) * 3 + 2] =
          F32toF16(outColor[2]);
    }
  }
...
CONSOLE_COMMAND(bakeLightGrids, "Bake irradiance/vis light grid data", NULL) {
  ...
  if (helpRequested) {
    ...
    idLib::Printf(
        " bounce[num] : number of bounces or number of light reuse (default "
        "1)\n");
    idLib::Printf(
        " grid( xdim ydim zdim ) : light grid size steps into each direction "
        "(default 64 64 128)\n");
  ...
  for (int bounce = 0; bounce < bounces; bounce++) {
    for (int a = 0; a < tr.primaryWorld->NumAreas(); a++) {
      ...
      area->lightGrid.SetupLightGrid(
          area->globalBounds, tr.primaryWorld->mapName, tr.primaryWorld,
          gridSize, a, tr.primaryWorld->NumAreas(), limit, false);
  ...
  for (int bounce = 0; bounce < bounces; bounce++) {
    for (int a = 0; a < tr.primaryWorld->NumAreas(); a++) {
      ...
      area->lightGrid.SetupLightGrid(
          area->globalBounds, tr.primaryWorld->mapName, tr.primaryWorld,
          gridSize, a, tr.primaryWorld->NumAreas(), limit, true);
      ...
      gridStep[2] = area->lightGrid.lightGridBounds[0] *
                    area->lightGrid.lightGridBounds[1];
      ...
      for (int i = 0; i < area->lightGrid.lightGridBounds[0]; i += 1) {
        for (int j = 0; j < area->lightGrid.lightGridBounds[1]; j += 1) {
          for (int k = 0; k < area->lightGrid.lightGridBounds[2]; k += 1) {
            ...
            lightGridPoint_t* gridPoint =
                &area->lightGrid.lightGridPoints[gridCoord[0] * gridStep[0] +
                                                 gridCoord[1] * gridStep[1] +
                                                 gridCoord[2] * gridStep[2]];
            if (!gridPoint->valid) {
            ...
            for (int side = 0; side < 6; side++) {
              ...
              // discard anything currently on the list (this triggers
              // SwapBuffers)
              tr.SwapCommandBuffers(NULL, NULL, NULL, NULL, NULL, NULL);
              ...
              glReadPixels(0, 0, captureSize, captureSize, GL_RGB,
                           GL_HALF_FLOAT, float16FRGB);
      ...
      common->Printf(
          "captured light grid radiance for area %i in %5.1f seconds\n\n", a,
          (end - start) * 0.001f);
      ...
      for (int j = 0; j < tr.lightGridJobs.Num(); j++) {
        ...
        jobParms->outBuffer = (halfFloat_t*)R_StaticAlloc(
            idMath::Ceil(LIGHTGRID_IRRADIANCE_SIZE * LIGHTGRID_IRRADIANCE_SIZE *
                         3 * sizeof(halfFloat_t) * 1.5f),
            TAG_IMAGE);

        if (useThreads) {
          tr.envprobeJobList->AddJob((jobRun_t)CalculateLightGridPointJob,
                                     jobParms);
        } else {
      ...
      int atlasWidth = area->lightGrid.lightGridBounds[0] *
                       area->lightGrid.lightGridBounds[2] *
                       LIGHTGRID_IRRADIANCE_SIZE;
      int atlasHeight =
          area->lightGrid.lightGridBounds[1] * LIGHTGRID_IRRADIANCE_SIZE;
      ...
      for (int j = 0; j < tr.lightGridJobs.Num(); j++) {
        ...
        for (int x = 0; x < LIGHTGRID_IRRADIANCE_SIZE; x++) {
          for (int y = 0; y < LIGHTGRID_IRRADIANCE_SIZE; y++) {
            int xx = x + (job->gridCoord[0] * gridStep[0] +
                          job->gridCoord[2] * gridStep[1]) *
                             LIGHTGRID_IRRADIANCE_SIZE;
            int yy = y + job->gridCoord[1] * LIGHTGRID_IRRADIANCE_SIZE;

            irradianceAtlas[(yy * atlasWidth + xx) * 3 + 0] =
                job->outBuffer[(y * LIGHTGRID_IRRADIANCE_SIZE + x) * 3 + 0];
            irradianceAtlas[(yy * atlasWidth + xx) * 3 + 1] =
                job->outBuffer[(y * LIGHTGRID_IRRADIANCE_SIZE + x) * 3 + 1];
            irradianceAtlas[(yy * atlasWidth + xx) * 3 + 2] =
                job->outBuffer[(y * LIGHTGRID_IRRADIANCE_SIZE + x) * 3 + 2];
          }
        }
...
#if STORE_LIGHTGRID_SHDATA
        lightGridPoint_t* gridPoint =
            &area->lightGrid.lightGridPoints[job->gridCoord[0] * gridStep[0] +
                                             job->gridCoord[1] * gridStep[1] +
                                             job->gridCoord[2] * gridStep[2]];
        for (int i = 0; i < shSize(4); i++) {
      ...
      R_WriteEXR(filename.c_str(), irradianceAtlas.Ptr(), 3, atlasWidth,
                 atlasHeight, "fs_basepath");
      ...
      common->Printf(
          "computed light grid irradiance for area %i in %5.1f seconds\n\n", a,
          (end - start) * 0.001f);
    }
