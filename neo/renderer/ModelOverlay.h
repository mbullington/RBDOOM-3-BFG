...
static const int DEFFERED_OVERLAY_TIMEOUT =
    200;  // don't create a overlay if it wasn't visible within the first 200
          // milliseconds
...
class idRenderModelOverlay {
 public:
  ...
  struct drawSurf_t* CreateOverlayDrawSurf(const struct viewEntity_t* space,
                                           const idRenderModel* baseModel,
                                           unsigned int index);
