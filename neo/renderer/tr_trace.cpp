...
====================
*/
static void R_TracePointCullStatic(byte* cullBits, byte& totalOr,
...
====================
*/
static void R_TracePointCullSkinned(byte* cullBits, byte& totalOr,
                                    const float radius, const idPlane* planes,
...
static bool R_LineIntersectsTriangleExpandedWithCircle(
    localTrace_t& hit, const idVec3& start, const idVec3& end,
    const float circleRadius, const idVec3& triVert0, const idVec3& triVert1,
    const idVec3& triVert2) {
...
localTrace_t R_LocalTrace(const idVec3& start, const idVec3& end,
  ...
  byte* cullBits = (byte*)_alloca16(
      ALIGN(tri->numVerts, 4));  // round up to a multiple of 4 for SIMD
  byte totalOr = 0;
  ...
  if (joints != NULL) {
    R_TracePointCullSkinned(cullBits, totalOr, radius, planes, tri->verts,
                            tri->numVerts, joints);
  } else {
    R_TracePointCullStatic(cullBits, totalOr, radius, planes, tri->verts,
                           tri->numVerts);
  }
  ...
  idODSStreamedArray<triIndex_t, 256, SBT_QUAD, 3> indexesODS(tri->indexes,
                                                              tri->numIndexes);
  ...
  for (int i = 0; i < tri->numIndexes;) {
    ...
    for (; i <= nextNumIndexes; i += 3) {
      ...
      const idVec3 triVert0 = idDrawVert::GetSkinnedDrawVertPosition(
          idODSObject<idDrawVert>(&tri->verts[i0]), joints);
      const idVec3 triVert1 = idDrawVert::GetSkinnedDrawVertPosition(
          idODSObject<idDrawVert>(&tri->verts[i1]), joints);
      const idVec3 triVert2 = idDrawVert::GetSkinnedDrawVertPosition(
          idODSObject<idDrawVert>(&tri->verts[i2]), joints);

      if (R_LineIntersectsTriangleExpandedWithCircle(
              hit, start, end, radius, triVert0, triVert1, triVert2)) {
        hit.indexes[0] = i0;
        hit.indexes[1] = i1;
        hit.indexes[2] = i2;
      }
