...
idCVar stereoRender_defaultGuiDepth(
    "stereoRender_defaultGuiDepth", "0", CVAR_RENDERER,
    "Fraction of separation when not specified");
/*
================
EmitSurfaces
...
void idGuiModel::EmitSurfaces(float modelMatrix[16], float modelViewMatrix[16],
                              bool depthHack, bool allowFullScreenStereoDepth,
                              bool linkAsEntity) {
  viewEntity_t* guiSpace = (viewEntity_t*)R_ClearedFrameAlloc(
      sizeof(*guiSpace), FRAME_ALLOC_VIEW_ENTITY);
  ...
  // If this is an in-game gui, we need to be able to find the matrix again for
  // head mounted display bypass matrix fixup.
  if (linkAsEntity) {
  ...
  idRenderMatrix::Multiply(tr.viewDef->projectionRenderMatrix, viewMat,
                           guiSpace->mvp);
  if (depthHack) {
  ...
  // depth fractions between 0=screen and 1=infinity, which directly modulate
  // the screenSeparation parameter for an X offset. The value is stored in the
  // drawSurf sort value, which adjusts the matrix in the backend.
  float defaultStereoDepth =
      stereoRender_defaultGuiDepth.GetFloat();  // default to at-screen
  ...
  for (int i = 0; i < surfaces.Num(); i++) {
    ...
    drawSurf->indexCache =
        indexBlock + ((int64)(guiSurf.firstIndex * sizeof(triIndex_t))
                      << VERTCACHE_OFFSET_SHIFT);
    ...
    if (constRegs) {
      ...
      shader->EvaluateRegisters(regs, shaderParms,
                                tr.viewDef->renderView.shaderParms,
                                tr.viewDef->renderView.time[1] * 0.001f, NULL);
    }
...
void idGuiModel::EmitToCurrentView(float modelMatrix[16], bool depthHack) {
  ...
  EmitSurfaces(modelMatrix, modelViewMatrix, depthHack,
               false /* stereoDepthSort */, true /* link as entity */);
}

// DG: move function declaration here (=> out of EmitFullScreen() method)
// because it confused clang (and possibly other compilers that just didn't
// complain and silently made it a float variable initialized to something,
// probably 0.0f)
...
void idGuiModel::EmitFullScreen() {
  ...
  viewDef_t* viewDef =
      (viewDef_t*)R_ClearedFrameAlloc(sizeof(*viewDef), FRAME_ALLOC_VIEW_DEF);
  ...
  viewDef->projectionMatrix[1 * 4 + 1] =
      2.0f / renderSystem->GetVirtualHeight();
#else
  viewDef->projectionMatrix[1 * 4 + 1] =
      -2.0f / renderSystem->GetVirtualHeight();
#endif
  ...
  idRenderMatrix::Transpose(*(idRenderMatrix*)viewDef->projectionMatrix,
                            viewDef->projectionRenderMatrix);
  ...
  EmitSurfaces(viewDef->worldSpace.modelMatrix,
               viewDef->worldSpace.modelViewMatrix, false /* depthHack */,
               stereoEnabled /* stereoDepthSort */, false /* link as entity */);
...
void idGuiModel::EmitImGui(ImDrawData* drawData) {
  // NOTE: this implementation does not support scissor clipping for the
  // indivudal draw commands but it is sufficient for things like com_showFPS

  const float sysWidth = renderSystem->GetWidth();
  const float sysHeight = renderSystem->GetHeight();

  idVec2 scaleToVirtual((float)renderSystem->GetVirtualWidth() / sysWidth,
  ...
  for (int a = 0; a < drawData->CmdListsCount; a++) {
    ...
    for (int b = 0; b < cmd_list->CmdBuffer.size(); b++) {
      ...
      idDrawVert* verts = renderSystem->AllocTris(
          numVerts, indexBufferOffset, numIndexes, mat, STEREO_DEPTH_TYPE_NONE);
      if (verts == NULL) {
        continue;
      }
      ...
      if (pcmd->UserCallback) {
        ...
        for (int j = 0; j < numVerts; j++) {
          ...
          tempVert.xyz.ToVec2() =
              idVec2(imVert->pos.x, imVert->pos.y).Scale(scaleToVirtual);
          tempVert.xyz.z = 0.0f;
...
=============
*/
idDrawVert* idGuiModel::AllocTris(int vertCount, const triIndex_t* tempIndexes,
                                  int indexCount, const idMaterial* material,
  ...
  if ((startIndex & 1) || (indexCount & 1)) {
    ...
    for (int i = 0; i < indexCount; i += 2) {
      WriteIndexPair(indexPointer + startIndex + i, startVert + tempIndexes[i],
                     startVert + tempIndexes[i + 1]);
    }
  }
