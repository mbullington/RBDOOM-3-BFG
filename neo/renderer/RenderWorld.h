...
const int RDF_NOSHADOWS =
    BIT(0);  // force renderer to use faster lighting only path
const int RDF_NOAMBIENT = BIT(1);   // don't render indirect lighting
const int RDF_IRRADIANCE = BIT(2);  // render into 256^2 HDR render target for
                                    // irradiance/radiance GGX calculation
const int RDF_UNDERWATER =
    BIT(3);  // TODO enable automatic underwater caustics and fog
...
typedef struct renderView_s {
  ...
  idVec3
      vieworg_weapon;  // has already been adjusted for stereo world seperation
  idMat3
  ...
  // the viewEyeBuffer may be of a different polarity than
  // stereoScreenSeparation if the eyes have been swapped
  int viewEyeBuffer;  // -1 = left eye, 1 = right eye, 0 = monoscopic view or
                      // GUI
  float stereoScreenSeparation;  // projection matrix horizontal offset,
                                 // positive or negative based on camera eye
...
// Admer: Suppress Windows API DrawText, so IntelliSense can properly highlight
...
class idRenderWorld {
 public:
  ...
  // This fixes a crash when switching between expansion packs in the same game
  // session the modelManager gets reset, which deletes all render models
  // without resetting the localModels list inside renderWorldLocal. Now we'll
  // have a hook to reset the list from here.
  ...
  virtual bool AreasAreConnected(int areaNum1, int areaNum2,
                                 portalConnection_t connection) const = 0;
  ...
  virtual void DebugSphere(const idVec4& color, const idSphere& sphere,
                           const int lifetime = 0, bool depthTest = false) = 0;
