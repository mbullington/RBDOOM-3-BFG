...
================
*/
float RB_DrawTextLength(const char* text, float scale, int len) { return 0; }
...
================
*/
void RB_DrawText(const char* text, const idVec3& origin, float scale,
                 const idVec4& color, const idMat3& viewAxis, const int align) {
...
void idRenderBackend::DBG_TestImage() {
  ...
  // Set State
  GL_State(GLS_DEPTHFUNC_ALWAYS | GLS_CULL_TWOSIDED | GLS_SRCBLEND_ONE |
           GLS_DSTBLEND_ZERO);
  ...
  if ((imageCr != NULL) && (imageCb != NULL)) {
    ...
    // SRS - Use Bink shader with no sRGB to linear conversion, otherwise
