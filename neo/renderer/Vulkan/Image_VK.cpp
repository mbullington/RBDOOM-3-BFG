...
====================
*/
static VkComponentMapping VK_GetComponentMappingFromTextureFormat(
    const textureFormat_t format, textureColor_t color) {
...
void idImage::CreateSampler() {
  ...
  createInfo.compareOp = (opts.format == FMT_DEPTH)
  ...
  switch (filter) {
    case TF_DEFAULT:
      ...
      if (r_maxAnisotropicFiltering.GetInteger() > 0) {
        ...
        createInfo.maxAnisotropy =
            Min(r_maxAnisotropicFiltering.GetFloat(),
                vkcontext.gpu->props.limits.maxSamplerAnisotropy);
      }
      break;
    ...
    default:
      idLib::FatalError(
          "idImage::CreateSampler: unrecognized texture filter %d", filter);
  }
  ...
  switch (repeat) {
    ...
    default:
      idLib::FatalError(
          "idImage::CreateSampler: unrecognized texture repeat mode %d",
          repeat);
  }
...
void idImage::Bind() {
  ...
  vklocal.imageParms[vklocal.currentImageParm] = this;
}
...
void idImage::AllocImage() {
  ...
  imageCreateInfo.flags =
      (opts.textureType == TT_CUBIC) ? VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT : 0;
  ...
  viewCreateInfo.viewType = (opts.textureType == TT_CUBIC)
                                ? VK_IMAGE_VIEW_TYPE_CUBE
                                : VK_IMAGE_VIEW_TYPE_2D;
  viewCreateInfo.format = internalFormat;
  viewCreateInfo.components =
      VK_GetComponentMappingFromTextureFormat(opts.format, opts.colorFormat);
  ...
  viewCreateInfo.subresourceRange.layerCount =
      (opts.textureType == TT_CUBIC) ? 6 : 1;
  viewCreateInfo.subresourceRange.baseMipLevel = 0;

  ID_VK_CHECK(
      vkCreateImageView(vkcontext.device, &viewCreateInfo, NULL, &view));
}
...
void idImage::SubImageUpload(int mipLevel, int x, int y, int z, int width,
  ...
  // SRS - Calculate buffer size without changing original width and height
  ...
  if (opts.format == FMT_RGB565) {
    ...
    // individual rows of half-height chroma planes into full-height planes This
    // works around a stall that occurs with half-height planes when exiting
    // levels or after demo playback (possible issue in MoltenVK??)
    // ***IMPORTANT - Assumes that SubImageUpload() has been called with
    // half-width and full-height parameters and a packed pic buffer ***
    byte* imgData = (byte*)pic;
    int evenRow;
    for (int i = 0; i < size / 2; i++) {
      evenRow = (i / width) * 2;
      data[evenRow * width + i % width] =
          imgData[i];  // SRS - Copy image data into even-numbered rows of new
                       // chroma plane
      data[(evenRow + 1) * width + i % width] =
          imgData[i];  // SRS - Duplicate image data into odd-numbered rows of
                       // new chroma plane
    }
  }
#endif
  ...
  imgCopy.bufferImageHeight =
      bufferH;  // SRS - Use buffer height vs. image height to avoid vulkan
                // errors for compressed images
  ...
  imgCopy.imageExtent.width =
      width;  // SRS - Always use original width and height dimensions, even
  ...
  vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                       VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, 0, 0, NULL, 0, NULL,
                       1, &barrier);
...
====================
*/
void idImage::SetImageLayout(VkImage image,
