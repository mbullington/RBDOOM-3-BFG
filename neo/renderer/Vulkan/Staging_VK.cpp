...
idCVar r_vkUploadBufferSizeMB("r_vkUploadBufferSizeMB", "64",
                              CVAR_INTEGER | CVAR_INIT,
                              "Size of gpu upload buffer.");
idCVar r_vkStagingMaxCommands(
    "r_vkStagingMaxCommands", "-1", CVAR_INTEGER | CVAR_INIT,
    "Maximum amount of commands staged (-1 for no limit)");
...
void idVulkanStagingManager::Init() {
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ...
    ID_VK_CHECK(vkCreateBuffer(vkcontext.device, &bufferCreateInfo, NULL,
                               &buffers[i].buffer));
  }

  VkMemoryRequirements memoryRequirements;
  vkGetBufferMemoryRequirements(vkcontext.device, buffers[0].buffer,
                                &memoryRequirements);
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ID_VK_CHECK(vkBindBufferMemory(vkcontext.device, buffers[i].buffer, memory,
                                   i * alignedSize));
  }

  ID_VK_CHECK(vkMapMemory(vkcontext.device, memory, 0,
                          alignedSize * NUM_FRAME_DATA, 0,
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ...
    ID_VK_CHECK(vkCreateFence(vkcontext.device, &fenceCreateInfo, NULL,
                              &buffers[i].fence));
    ID_VK_CHECK(vkBeginCommandBuffer(buffers[i].commandBuffer,
...
void idVulkanStagingManager::Shutdown() {
  // SRS - use vkFreeMemory (with implicit unmap) vs. vkUnmapMemory to avoid
  // validation layer errors on shutdown
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ...
    vkFreeCommandBuffers(vkcontext.device, commandPool, 1,
                         &buffers[i].commandBuffer);
  }
...
=============
*/
byte* idVulkanStagingManager::Stage(const int size, const int alignment,
                                    VkCommandBuffer& commandBuffer,
                                    VkBuffer& buffer, int& bufferOffset) {
  if (size > maxBufferSize) {
    idLib::FatalError("Can't allocate %d MB in gpu transfer buffer",
                      (int)(size / 1024 / 1024));
  }
...
void idVulkanStagingManager::Flush() {
  ...
  vkCmdPipelineBarrier(stage.commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
...
void idVulkanStagingManager::Wait(stagingBuffer_t& stage) {
  ...
  ID_VK_CHECK(
      vkWaitForFences(vkcontext.device, 1, &stage.fence, VK_TRUE, UINT64_MAX));
  ...
  ID_VK_CHECK(
      vkBeginCommandBuffer(stage.commandBuffer, &commandBufferBeginInfo));
}