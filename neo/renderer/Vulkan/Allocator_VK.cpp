...
idCVar r_vkHostVisibleMemoryMB("r_vkHostVisibleMemoryMB", "64",
                               CVAR_INTEGER | CVAR_INIT, "");

static const char* memoryUsageStrings[VULKAN_MEMORY_USAGES] = {
    "VULKAN_MEMORY_USAGE_UNKNOWN",    "VULKAN_MEMORY_USAGE_GPU_ONLY",
    "VULKAN_MEMORY_USAGE_CPU_ONLY",   "VULKAN_MEMORY_USAGE_CPU_TO_GPU",
...
=============
*/
uint32 FindMemoryTypeIndex(const uint32 memoryTypeBits,
                           const vulkanMemoryUsage_t usage) {
  VkPhysicalDeviceMemoryProperties& physicalMemoryProperties =
      vkcontext.gpu->memProps;
  ...
  switch (usage) {
    ...
    default:
      idLib::FatalError(
          "idVulkanAllocator::AllocateFromPools: Unknown memory usage.");
  }
  ...
  for (uint32 i = 0; i < physicalMemoryProperties.memoryTypeCount; ++i) {
    ...
    if ((physicalMemoryProperties
             .memoryHeaps[physicalMemoryProperties.memoryTypes[i].heapIndex]
             .flags &
         avoid) != 0) {
      continue;
    }
...
=============
*/
idVulkanBlock::idVulkanBlock(const uint32 _memoryTypeIndex,
                             const VkDeviceSize _size,
                             vulkanMemoryUsage_t _usage)
    : nextBlockId(0),
      size(_size),
      allocated(0),
...
=============
*/
idVulkanBlock::~idVulkanBlock() { Shutdown(); }
...
bool idVulkanBlock::Init() {
  // SRS - Changed UINT64_MAX to UINT32_MAX for type consistency, otherwise test
  // is always false
  ...
  ID_VK_CHECK(vkAllocateMemory(vkcontext.device, &memoryAllocateInfo, NULL,
                               &deviceMemory))
  ...
  if (IsHostVisible()) {
    ID_VK_CHECK(
        vkMapMemory(vkcontext.device, deviceMemory, 0, size, 0, (void**)&data));
  }
...
=============
*/
static bool IsOnSamePage(VkDeviceSize rAOffset, VkDeviceSize rASize,
...
=============
*/
static bool HasGranularityConflict(vulkanAllocationType_t type1,
                                   vulkanAllocationType_t type2) {
  if (type1 > type2) {
...
=============
*/
bool idVulkanBlock::Allocate(const uint32 _size, const uint32 align,
...
void idVulkanBlock::Free(vulkanAllocation_t& allocation) {
  ...
  if (current == NULL) {
    idLib::Warning(
        "idVulkanBlock::Free: Tried to free an unknown allocation. %p - %u",
        this, allocation.id);
    return;
  }
...
void idVulkanBlock::Print() {
  ...
  // SRS - Changed %lu to %PRIu64 pre-defined macro to handle platform
  // differences
...
vulkanAllocation_t idVulkanAllocator::Allocate(
    const uint32 _size, const uint32 align, const uint32 memoryTypeBits,
    const vulkanMemoryUsage_t usage, const vulkanAllocationType_t allocType) {
  vulkanAllocation_t allocation;

  uint32 memoryTypeIndex = FindMemoryTypeIndex(memoryTypeBits, usage);
  if (memoryTypeIndex == UINT32_MAX) {
    idLib::FatalError(
        "idVulkanAllocator::Allocate: Unable to find a memoryTypeIndex for "
        "allocation request.");
  }
  ...
  for (int i = 0; i < numBlocks; ++i) {
    ...
    if (block->Allocate(_size, align, bufferImageGranularity, allocType,
                        allocation)) {
      return allocation;
    }
  }
  ...
  if (block->Init()) {
    ...
    idLib::FatalError(
        "idVulkanAllocator::Allocate: Could not allocate new memory block.");
  }
...
void idVulkanAllocator::Free(const vulkanAllocation_t allocation) {
  // SRS - Make sure we are trying to free an actual allocated block, otherwise
  // skip
...
CONSOLE_COMMAND(Vulkan_PrintHeapInfo,
                "Print out the heap information for this hardware.", 0) {
...
CONSOLE_COMMAND(Vulkan_PrintAllocations,
                "Print out all the current allocations.", 0) {
#if defined(USE_AMD_ALLOCATOR)
  // TODO
#else
