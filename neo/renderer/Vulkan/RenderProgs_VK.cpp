...
static const char* renderProgBindingStrings[BINDING_TYPE_MAX] = {"ubo",
                                                                 "sampler"};
...
void CreateDescriptorPools(VkDescriptorPool (&pools)[NUM_FRAME_DATA]) {
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ID_VK_CHECK(vkCreateDescriptorPool(vkcontext.device, &poolCreateInfo, NULL,
                                       &pools[i]));
  }
}
...
========================
*/
void idRenderProgManager::CreateDescriptorSetLayout(
    const shader_t& vertexShader, const shader_t& fragmentShader,
  ...
  // Descriptor Set Layout
  {
    ...
    vkCreateDescriptorSetLayout(vkcontext.device, &createInfo, NULL,
                                &renderProg.descriptorSetLayout);
  }
  ...
  // Pipeline Layout
  {
    ...
    vkCreatePipelineLayout(vkcontext.device, &createInfo, NULL,
                           &renderProg.pipelineLayout);
  }
}
...
================================================================================================
*/
void idRenderProgManager::Unbind() { current = -1; }
...
static int CompileGLSLtoSPIRV(const char* filename, const idStr& dataGLSL,
                              const rpStage_t stage, uint32** spirvBuffer) {
  ...
  shaderc::SpvCompilationResult module =
      compiler.CompileGlslToSpv(source, shaderKind, filename, options);
...
static int CompileGLSLtoSPIRV(const char* filename, const idStr& dataGLSL,
  ...
  // All that's left to do now is to convert the program's intermediate
  // representation into SpirV:
...
void idRenderProgManager::LoadShader(shader_t& shader) {
  ...
  outFileHLSL.Format("renderprogs/hlsl/%s%s", shader.name.c_str(),
                     shader.nameOutSuffix.c_str());
  outFileHLSL.StripFileExtension();

  outFileGLSL.Format("renderprogs/vkglsl/%s%s", shader.name.c_str(),
                     shader.nameOutSuffix.c_str());
  outFileLayout.Format("renderprogs/vkglsl/%s%s", shader.name.c_str(),
                       shader.nameOutSuffix.c_str());
  ...
  // first check whether we already have a valid GLSL file and compare it to the
  // hlsl timestamp;
  ID_TIME_T hlslTimeStamp;
  int hlslFileLength =
      fileSystem->ReadFile(inFile.c_str(), NULL, &hlslTimeStamp);

  ID_TIME_T glslTimeStamp;
  int glslFileLength =
      fileSystem->ReadFile(outFileGLSL.c_str(), NULL, &glslTimeStamp);

  // if the glsl file doesn't exist or we have a newer HLSL file we need to
  // recreate the glsl file.
  idStr programGLSL;
  idStr programLayout;
  // if( ( glslFileLength <= 0 ) || ( hlslTimeStamp != FILE_NOT_FOUND_TIMESTAMP
  // && hlslTimeStamp > glslTimeStamp ) || r_alwaysExportGLSL.GetBool() )
  {
    ...
    // FIXME: we should really scan the program source code for using
    // rpEnableSkinning but at this point we directly load a binary and the
    // program source code is not available on the consoles
    ...
    fileSystem->WriteFile(outFileGLSL, programGLSL.c_str(),
                          programGLSL.Length(), "fs_savepath");
    fileSystem->WriteFile(outFileLayout, programLayout.c_str(),
                          programLayout.Length(), "fs_savepath");
  }
  /*
  else
  {
          ...
          int lengthGLSL = fileSystem->ReadFile( outFileGLSL.c_str(),
  &fileBufferGLSL ); if( lengthGLSL <= 0 )
          {
                  idLib::Error( "GLSL file %s could not be loaded and may be
  corrupt", outFileGLSL.c_str() );
          }
...

          {
                  ...
                  int lengthUniforms = fileSystem->ReadFile(
  outFileUniforms.c_str(), &fileBufferUniforms ); if( lengthUniforms <= 0 )
                  {
                          idLib::Error( "uniform file %s could not be loaded and
  may be corrupt", outFileUniforms.c_str() );
                  }
  ...
  */
  ...
  // RB: find the uniforms locations in either the vertex or fragment uniform
  // array this uses the new layout structure
  {
    ...
    if (src.ExpectTokenString("uniforms")) {
      ...
      while (!src.CheckTokenString("]")) {
        ...
        if (index == -1) {
          idLib::Error("couldn't find uniform %s for %s", token.c_str(),
                       outFileGLSL.c_str());
        }
  ...
  int spirvLen = CompileGLSLtoSPIRV(outFileGLSL.c_str(), programGLSL,
                                    shader.stage, &spirvBuffer);
  ...
  ID_VK_CHECK(vkCreateShaderModule(vkcontext.device, &shaderModuleCreateInfo,
                                   NULL, &shader.module));
...
void idRenderProgManager::LoadGLSLProgram(const int programIndex,
                                          const int vertexShaderIndex,
                                          const int fragmentShaderIndex) {
...
void idRenderProgManager::KillAllShaders() {
  ...
  for (int i = 0; i < renderProgs.Num(); ++i) {
    ...
    vkDestroyDescriptorSetLayout(vkcontext.device, prog.descriptorSetLayout,
                                 NULL);
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    // vkFreeDescriptorSets( vkcontext.device, descriptorPools[ i ],
    // MAX_DESC_SETS, descriptorSets[ i ] );
...
========================
*/
void idRenderProgManager::AllocParmBlockBuffer(const idList<int>& parmIndices,
                                               idUniformBuffer& ubo) {
  // TODO support shadow matrices + 23 float4

  const int numParms = parmIndices.Num();
  const int bytes =
      ALIGN(numParms * sizeof(idVec4),
            vkcontext.gpu->props.limits.minUniformBufferOffsetAlignment);
...
========================
*/
static VkPipeline CreateGraphicsPipeline(vertexLayoutType_t vertexLayoutType,
  ...
  rasterizationState.frontFace = (stateBits & GLS_CLOCKWISE)
                                     ? VK_FRONT_FACE_CLOCKWISE
                                     : VK_FRONT_FACE_COUNTER_CLOCKWISE;
  rasterizationState.lineWidth = 1.0f;
  rasterizationState.polygonMode = (stateBits & GLS_POLYMODE_LINE)
  ...
  VkPipelineColorBlendAttachmentState attachmentState = {};
  {
    ...
    attachmentState.blendEnable =
        (srcFactor != VK_BLEND_FACTOR_ONE || dstFactor != VK_BLEND_FACTOR_ZERO);
    ...
    attachmentState.colorWriteMask |=
        (stateBits & GLS_REDMASK) ? 0 : VK_COLOR_COMPONENT_R_BIT;
    attachmentState.colorWriteMask |=
        (stateBits & GLS_GREENMASK) ? 0 : VK_COLOR_COMPONENT_G_BIT;
    attachmentState.colorWriteMask |=
        (stateBits & GLS_BLUEMASK) ? 0 : VK_COLOR_COMPONENT_B_BIT;
    attachmentState.colorWriteMask |=
        (stateBits & GLS_ALPHAMASK) ? 0 : VK_COLOR_COMPONENT_A_BIT;
  }
  ...
  VkPipelineDepthStencilStateCreateInfo depthStencilState = {};
  {
    ...
    depthStencilState.depthBoundsTestEnable =
        vkcontext.physicalDeviceFeatures.depthBounds &&
        (stateBits & GLS_DEPTH_TEST_MASK) != 0;
    depthStencilState.minDepthBounds = 0.0f;
    depthStencilState.maxDepthBounds = 1.0f;
    depthStencilState.stencilTestEnable =
        (stateBits & (GLS_STENCIL_FUNC_BITS | GLS_STENCIL_OP_BITS |
                      GLS_SEPARATE_STENCIL)) != 0;
    ...
    if (stateBits & GLS_SEPARATE_STENCIL) {
      depthStencilState.front =
          GetStencilOpState(stateBits & GLS_STENCIL_FRONT_OPS);
      ...
      depthStencilState.back =
          GetStencilOpState((stateBits & GLS_STENCIL_BACK_OPS) >> 12);
  ...
  ID_VK_CHECK(vkCreateGraphicsPipelines(vkcontext.device,
                                        vkcontext.pipelineCache, 1, &createInfo,
                                        NULL, &pipeline));
...
========================
*/
VkPipeline idRenderProgManager::renderProg_t::GetPipeline(
    uint64 stateBits, VkShaderModule vertexShader,
    VkShaderModule fragmentShader) {
  ...
  VkPipeline pipeline = CreateGraphicsPipeline(
      vertexLayout, vertexShader, fragmentShader, pipelineLayout, stateBits);
...
void idRenderProgManager::CommitUniforms(uint64 stateBits) {
  ...
  VkPipeline pipeline =
      prog.GetPipeline(stateBits, shaders[prog.vertexShaderIndex].module,
                       shaders[prog.fragmentShaderIndex].module);
  ...
  ID_VK_CHECK(
      vkAllocateDescriptorSets(vkcontext.device, &setAllocInfo,
                               &descriptorSets[currentData][currentDescSet]));
  ...
  if (prog.vertexShaderIndex > -1 &&
      shaders[prog.vertexShaderIndex].parmIndices.Num() > 0) {
    AllocParmBlockBuffer(shaders[prog.vertexShaderIndex].parmIndices,
                         vertParms);
  ...
  if (prog.fragmentShaderIndex > -1 &&
      shaders[prog.fragmentShaderIndex].parmIndices.Num() > 0) {
    AllocParmBlockBuffer(shaders[prog.fragmentShaderIndex].parmIndices,
                         fragParms);
...
CONSOLE_COMMAND(Vulkan_PrintPipelineStates,
                "Print the GLState bits associated with each pipeline.", 0) {
  renderProgManager.PrintPipelines();
}

CONSOLE_COMMAND(Vulkan_ClearPipelines,
                "Clear all existing pipelines, forcing them to be recreated.",
                0) {
