...
// VK_EXT_debug_utils
PFN_vkQueueBeginDebugUtilsLabelEXT qvkQueueBeginDebugUtilsLabelEXT =
    VK_NULL_HANDLE;
...
PFN_vkCmdInsertDebugUtilsLabelEXT qvkCmdInsertDebugUtilsLabelEXT =
    VK_NULL_HANDLE;
...
idCVar r_vkEnableValidationLayers("r_vkEnableValidationLayers", "0",
                                  CVAR_BOOL | CVAR_INIT, "");
...
// SRS - optionally needed for runtime access to fullImageViewSwizzle (instead
// of env var MVK_CONFIG_FULL_IMAGE_VIEW_SWIZZLE = 1)
...
// SRS - use MoltenVK validation layer on macOS when using libMoltenVK in place
// of libvulkan
#if defined(__APPLE__) && defined(USE_MoltenVK)
    "MoltenVK"
#else
...
=============
*/
VKAPI_ATTR VkBool32 VKAPI_CALL DebugCallback(VkDebugReportFlagsEXT msgFlags,
...
static void CreateDebugReportCallback() {
  ...
  PFN_vkCreateDebugReportCallbackEXT func =
      (PFN_vkCreateDebugReportCallbackEXT)vkGetInstanceProcAddr(
          vkcontext.instance, "vkCreateDebugReportCallbackEXT");
  ID_VK_VALIDATE(func != NULL, "Could not find vkCreateDebugReportCallbackEXT");
  ID_VK_CHECK(
      func(vkcontext.instance, &callbackInfo, NULL, &vkcontext.callback));
}
...
static void DestroyDebugReportCallback() {
  PFN_vkDestroyDebugReportCallbackEXT func =
      (PFN_vkDestroyDebugReportCallbackEXT)vkGetInstanceProcAddr(
          vkcontext.instance, "vkDestroyDebugReportCallbackEXT");
  ID_VK_VALIDATE(func != NULL,
                 "Could not find vkDestroyDebugReportCallbackEXT");
...
static void ValidateValidationLayers() {
  ...
  for (uint32 i = 0; i < g_numValidationLayers; ++i) {
    for (uint32 j = 0; j < instanceLayerCount; ++j) {
      ...
      if (idStr::Icmp(g_validationLayers[i], instanceLayers[j].layerName) ==
          0) {
        found = true;
        break;
      }
...
static void CreateVulkanInstance() {
  ...
  // SRS - Enumerate available Vulkan instance extensions and test for presence
  // of VK_KHR_get_physical_device_properties2 and VK_EXT_debug_utils
  idLib::Printf("Getting available vulkan instance extensions...\n");
  uint32 numInstanceExtensions;
  ID_VK_CHECK(vkEnumerateInstanceExtensionProperties(
      NULL, &numInstanceExtensions, NULL));
  ID_VK_VALIDATE(
      numInstanceExtensions > 0,
      "vkEnumerateInstanceExtensionProperties returned zero extensions.");

  idList<VkExtensionProperties> instanceExtensionProps;
  instanceExtensionProps.SetNum(numInstanceExtensions);
  ID_VK_CHECK(vkEnumerateInstanceExtensionProperties(
      NULL, &numInstanceExtensions, instanceExtensionProps.Ptr()));
  ID_VK_VALIDATE(
      numInstanceExtensions > 0,
      "vkEnumerateInstanceExtensionProperties returned zero extensions.");
  ...
  for (int i = 0; i < numInstanceExtensions; i++) {
    if (idStr::Icmp(instanceExtensionProps[i].extensionName,
    ...
    // SRS - Enable physical device enumeration when using the Vulkan loader on
    // macOS (MoltenVK portability driver)
    if (idStr::Icmp(instanceExtensionProps[i].extensionName,
...
static void EnumeratePhysicalDevices() {
  uint32 numDevices = 0;
  ID_VK_CHECK(
      vkEnumeratePhysicalDevices(vkcontext.instance, &numDevices, NULL));
  ID_VK_VALIDATE(numDevices > 0,
                 "vkEnumeratePhysicalDevices returned zero devices.");
  ...
  ID_VK_CHECK(vkEnumeratePhysicalDevices(vkcontext.instance, &numDevices,
                                         devices.Ptr()));
  ID_VK_VALIDATE(numDevices > 0,
                 "vkEnumeratePhysicalDevices returned zero devices.");
  ...
  for (uint32 i = 0; i < numDevices; ++i) {
    ...
    // get Queue family properties
    {
      ...
      ID_VK_VALIDATE(
          numQueues > 0,
          "vkGetPhysicalDeviceQueueFamilyProperties returned zero queues.");

      gpu.queueFamilyProps.SetNum(numQueues);
      vkGetPhysicalDeviceQueueFamilyProperties(gpu.device, &numQueues,
                                               gpu.queueFamilyProps.Ptr());
      ID_VK_VALIDATE(
          numQueues > 0,
          "vkGetPhysicalDeviceQueueFamilyProperties returned zero queues.");
    }
    ...
    // grab available Vulkan device extensions
    {
      ...
      uint32 numExtension;
      ID_VK_CHECK(vkEnumerateDeviceExtensionProperties(gpu.device, NULL,
                                                       &numExtension, NULL));
      ID_VK_VALIDATE(
          numExtension > 0,
          "vkEnumerateDeviceExtensionProperties returned zero extensions.");

      gpu.extensionProps.SetNum(numExtension);
      ID_VK_CHECK(vkEnumerateDeviceExtensionProperties(
          gpu.device, NULL, &numExtension, gpu.extensionProps.Ptr()));
      ID_VK_VALIDATE(
          numExtension > 0,
          "vkEnumerateDeviceExtensionProperties returned zero extensions.");
#if 0
    ...
    ID_VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        gpu.device, vkcontext.surface, &gpu.surfaceCaps));

    {
      uint32 numFormats;
      ID_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
          gpu.device, vkcontext.surface, &numFormats, NULL));
      ID_VK_VALIDATE(numFormats > 0,
                     "vkGetPhysicalDeviceSurfaceFormatsKHR returned zero "
                     "surface formats.");

      gpu.surfaceFormats.SetNum(numFormats);
      ID_VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
          gpu.device, vkcontext.surface, &numFormats,
          gpu.surfaceFormats.Ptr()));
      ID_VK_VALIDATE(numFormats > 0,
                     "vkGetPhysicalDeviceSurfaceFormatsKHR returned zero "
                     "surface formats.");
    }

    {
      ...
      ID_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
          gpu.device, vkcontext.surface, &numPresentModes, NULL));
      ID_VK_VALIDATE(numPresentModes > 0,
                     "vkGetPhysicalDeviceSurfacePresentModesKHR returned zero "
                     "present modes.");

      gpu.presentModes.SetNum(numPresentModes);
      ID_VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
          gpu.device, vkcontext.surface, &numPresentModes,
          gpu.presentModes.Ptr()));
      ID_VK_VALIDATE(numPresentModes > 0,
                     "vkGetPhysicalDeviceSurfacePresentModesKHR returned zero "
                     "present modes.");
    }
...
static void CreateSurface() {
  ...
  ID_VK_CHECK(vkCreateWin32SurfaceKHR(vkcontext.instance, &createInfo, NULL,
                                      &vkcontext.surface));

// SRS - Generalized Vulkan SDL platform
#elif defined(VULKAN_USE_PLATFORM_SDL)
  if (!SDL_Vulkan_CreateSurface(vkcontext.sdlWindow, vkcontext.instance,
                                &vkcontext.surface)) {
    idLib::FatalError("Error while creating Vulkan surface: %s",
                      SDL_GetError());
  }
#endif
}
...
*/
/*
static bool CheckPhysicalDeviceExtensionSupport( gpuInfo_t& gpu, idList< const
char* >& requiredExt )
{
        ...
        for( int i = 0; i < requiredExt.Num(); ++i )
        {
                ...
                {
                        if( idStr::Icmp( requiredExt[ i ], gpu.extensionProps[ j
].extensionName ) == 0 )
                        {
                                available++;
                                break;
...
=============
*/
static void PopulateDeviceExtensions(
    const idList<VkExtensionProperties>& extensionProps,
  ...
  for (int i = 0; i < numExtensions; ++i) {
    ...
    if (idStr::Icmp(extensionProps[i].extensionName,
    ...
    // SRS - Move this to CreateVulkanInstance(), since VK_EXT_debug_utils is an
    // instance extension not a device extension
    // if( idStr::Icmp( extensionProps[ i ].extensionName,
    // VK_EXT_DEBUG_UTILS_EXTENSION_NAME ) == 0 && enableLayers )
    //{
...
static bool CheckDeviceExtensionSupport(
    const idList<VkExtensionProperties>& extensionProps,
    const idList<const char*>& requiredExt) {
  int required = requiredExt.Num();
  int available = 0;
...
=============
*/
static void EnableDeviceExtensionFeatures(
    const idList<const char*>& extensions) {
  ...
  for (int i = 0; i < extensions.Num(); ++i) {
    ...
    // SRS - Move this to CreateVulkanInstance(), since VK_EXT_debug_utils is an
    // instance extension not a device extension
    // if( idStr::Icmp( extensions[ i ], VK_EXT_DEBUG_UTILS_EXTENSION_NAME ) ==
    // 0 )
    //{
    //	idLib::Printf( "Using Vulkan device extension [%s]\n",
    // VK_EXT_DEBUG_UTILS_EXTENSION_NAME );
...
static void SelectPhysicalDevice() {
  ...
  for (int i = 0; i < vkcontext.gpus.Num(); ++i) {
    ...
    for (int j = 0; j < gpu.queueFamilyProps.Num(); ++j) {
      ...
      vkGetPhysicalDeviceSurfaceSupportKHR(gpu.device, j, vkcontext.surface,
                                           &supportsPresent);
      if (supportsPresent) {
        presentIdx = j;
        break;
    ...
    if (graphicsIdx >= 0 && presentIdx >= 0) {
      ...
      vkGetPhysicalDeviceFeatures(vkcontext.physicalDevice,
      ...
      version_string.Append(va("Vulkan API %i.%i.%i",
                               VK_API_VERSION_MAJOR(gpu.props.apiVersion),
                               VK_API_VERSION_MINOR(gpu.props.apiVersion),
      ...
      for (int i = 0; i < gpu.extensionProps.Num(); i++) {
        extensions_string.Append(
            va("%s ", gpu.extensionProps[i].extensionName));

        if (idStr::Icmp(gpu.extensionProps[i].extensionName,
      ...
      if (vkcontext.deviceProperties2Available && driverPropertiesAvailable) {
        ...
        pDriverProperties.sType =
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DRIVER_PROPERTIES;
        vkGetPhysicalDeviceProperties2(vkcontext.physicalDevice, &pProperties);
        version_string.Append(va(" (%s %s)", pDriverProperties.driverName,
  ...
  idLib::FatalError(
      "Could not find a physical device which fits our desired profile");
}
...
static void CreateLogicalDeviceAndQueues() {
  ...
  ID_VK_CHECK(vkGetMoltenVKConfigurationMVK(vkcontext.instance, &pConfig,
                                            &pConfigSize));

  // SRS - If we don't have native image view swizzle, enable MoltenVK's image
  // view swizzle feature
  if (portabilityFeatures.imageViewFormatSwizzle == VK_FALSE) {
    ...
    ID_VK_CHECK(vkSetMoltenVKConfigurationMVK(vkcontext.instance, &pConfig,
                                              &pConfigSize));
  }

  // SRS - If MoltenVK's Metal argument buffer feature is on, disable it for
  // sampler scalability
  ...
  deviceFeatures.samplerAnisotropy =
      vkcontext.physicalDeviceFeatures.samplerAnisotropy;  // RB
#endif
  ...
  ID_VK_CHECK(
      vkCreateDevice(vkcontext.physicalDevice, &info, NULL, &vkcontext.device));

  vkGetDeviceQueue(vkcontext.device, vkcontext.graphicsFamilyIdx, 0,
                   &vkcontext.graphicsQueue);
  vkGetDeviceQueue(vkcontext.device, vkcontext.presentFamilyIdx, 0,
                   &vkcontext.presentQueue);

  if (vkcontext.debugMarkerSupportAvailable) {
    qvkDebugMarkerSetObjectTagEXT =
        (PFN_vkDebugMarkerSetObjectTagEXT)vkGetDeviceProcAddr(
            vkcontext.device, "vkDebugMarkerSetObjectTagEXT");
    qvkDebugMarkerSetObjectNameEXT =
        (PFN_vkDebugMarkerSetObjectNameEXT)vkGetDeviceProcAddr(
            vkcontext.device, "vkDebugMarkerSetObjectNameEXT");

    qvkCmdDebugMarkerBeginEXT =
        (PFN_vkCmdDebugMarkerBeginEXT)vkGetDeviceProcAddr(
            vkcontext.device, "vkCmdDebugMarkerBeginEXT");
    qvkCmdDebugMarkerEndEXT = (PFN_vkCmdDebugMarkerEndEXT)vkGetDeviceProcAddr(
        vkcontext.device, "vkCmdDebugMarkerEndEXT");
    qvkCmdDebugMarkerInsertEXT =
        (PFN_vkCmdDebugMarkerInsertEXT)vkGetDeviceProcAddr(
            vkcontext.device, "vkCmdDebugMarkerInsertEXT");

    // TODO

    // VK_RegisterObjectForDebug( reinterpret_cast< uint64 >(
    // vkcontext.graphicsQueue ), "graphics_queue",
    // VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT ); VK_RegisterObjectForDebug(
    // reinterpret_cast< uint64 >( vkcontext.presentQueue ), "present_queue",
    // VK_DEBUG_REPORT_OBJECT_TYPE_QUEUE_EXT );
  }

  if (vkcontext.debugUtilsSupportAvailable) {
    // SRS - Since VK_EXT_debug_utils is an instance extension, must use
    // vkGetInstanceProcAddr() vs vkGetDeviceProcAddr()
    qvkQueueBeginDebugUtilsLabelEXT =
        (PFN_vkQueueBeginDebugUtilsLabelEXT)vkGetInstanceProcAddr(
            vkcontext.instance, "vkQueueBeginDebugUtilsLabelEXT");
    qvkQueueEndDebugUtilsLabelEXT =
        (PFN_vkQueueEndDebugUtilsLabelEXT)vkGetInstanceProcAddr(
            vkcontext.instance, "vkQueueEndDebugUtilsLabelEXT");
    qvkCmdBeginDebugUtilsLabelEXT =
        (PFN_vkCmdBeginDebugUtilsLabelEXT)vkGetInstanceProcAddr(
            vkcontext.instance, "vkCmdBeginDebugUtilsLabelEXT");
    qvkCmdEndDebugUtilsLabelEXT =
        (PFN_vkCmdEndDebugUtilsLabelEXT)vkGetInstanceProcAddr(
            vkcontext.instance, "vkCmdEndDebugUtilsLabelEXT");
    qvkCmdInsertDebugUtilsLabelEXT =
        (PFN_vkCmdInsertDebugUtilsLabelEXT)vkGetInstanceProcAddr(
            vkcontext.instance, "vkCmdInsertDebugUtilsLabelEXT");
  }
}
...
// SRS - Generalized Vulkan SDL platform
  ...
  width = idMath::ClampInt(caps.minImageExtent.width, caps.maxImageExtent.width,
                           width);
  height = idMath::ClampInt(caps.minImageExtent.height,
                            caps.maxImageExtent.height, height);
#endif
...
static void CreateSwapChain() {
  ...
  if (vkcontext.graphicsFamilyIdx != vkcontext.presentFamilyIdx) {
    uint32 indices[] = {(uint32)vkcontext.graphicsFamilyIdx,
                        (uint32)vkcontext.presentFamilyIdx};
  ...
  ID_VK_CHECK(vkCreateSwapchainKHR(vkcontext.device, &info, NULL,
                                   &vkcontext.swapchain));
  ...
  uint32 numImages = 0;
  ID_VK_CHECK(vkGetSwapchainImagesKHR(vkcontext.device, vkcontext.swapchain,
                                      &numImages, NULL));
  ID_VK_VALIDATE(numImages > 0,
                 "vkGetSwapchainImagesKHR returned a zero image count.");

  ID_VK_CHECK(vkGetSwapchainImagesKHR(vkcontext.device, vkcontext.swapchain,
                                      &numImages,
                                      vkcontext.swapchainImages.Ptr()));
  ID_VK_VALIDATE(numImages > 0,
                 "vkGetSwapchainImagesKHR returned a zero image count.");
  ...
  for (uint32 i = 0; i < NUM_FRAME_DATA; ++i) {
    ...
    ID_VK_CHECK(vkCreateImageView(vkcontext.device, &imageViewCreateInfo, NULL,
                                  &vkcontext.swapchainViews[i]));
  }
}
...
static void CreateQueryPool() {
  ...
  for (int i = 0; i < NUM_FRAME_DATA; ++i) {
    ID_VK_CHECK(vkCreateQueryPool(vkcontext.device, &createInfo, NULL,
                                  &vkcontext.queryPools[i]));
  }
}
...
=============
*/
static VkFormat ChooseSupportedFormat(VkFormat* formats, int numFormats,
...
static void CreateRenderTargets() {
  ...
  vkGetPhysicalDeviceImageFormatProperties(
      vkcontext.physicalDevice, vkcontext.swapchainFormat, VK_IMAGE_TYPE_2D,
...
static void CreatePipelineCache() {
  ...
  ID_VK_CHECK(vkCreatePipelineCache(vkcontext.device, &pipelineCacheCreateInfo,
                                    NULL, &vkcontext.pipelineCache));
}
...
void idRenderBackend::Init() {
  ...
  // DG: make sure SDL has setup video so getting supported modes in
  // R_SetNewMode() works
// SRS - Generalized Vulkan SDL platform
#if defined(VULKAN_USE_PLATFORM_SDL)
  VKimp_PreInit();
  ...
  glConfig.timerQueryAvailable =
      true;  // SRS - Use glConfig.timerQueryAvailable flag to control Vulkan
             // timestamp capture
...
bool GL_CheckErrors_(const char* filename, int line) { return false; }
...
void idRenderBackend::ResizeImages() {
  ...
  ID_VK_CHECK(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      ...
      &vkcontext.gpu->surfaceCaps));

  // Recheck presentation support
  VkBool32 supportsPresent = VK_FALSE;
  ID_VK_CHECK(vkGetPhysicalDeviceSurfaceSupportKHR(
      vkcontext.physicalDevice, vkcontext.presentFamilyIdx, vkcontext.surface,
      &supportsPresent));
  if (supportsPresent == VK_FALSE) {
    idLib::FatalError(
        "idRenderBackend::ResizeImages: New surface does not support present?");
  }
...
void idRenderBackend::DrawElementsWithCounters(const drawSurf_t* surf) {
  ...
  if (vertexCache.CacheIsStatic(vbHandle)) {
    ...
    const uint64 frameNum =
        (int)(vbHandle >> VERTCACHE_FRAME_SHIFT) & VERTCACHE_FRAME_MASK;
  ...
  const int vertOffset =
      (int)(vbHandle >> VERTCACHE_OFFSET_SHIFT) & VERTCACHE_OFFSET_MASK;
  ...
  if (vertexCache.CacheIsStatic(ibHandle)) {
    ...
    const uint64 frameNum =
        (int)(ibHandle >> VERTCACHE_FRAME_SHIFT) & VERTCACHE_FRAME_MASK;
    if (frameNum != ((vertexCache.currentFrame - 1) & VERTCACHE_FRAME_MASK)) {
  ...
  const ptrdiff_t indexOffset =
      (ptrdiff_t)(ibHandle >> VERTCACHE_OFFSET_SHIFT) & VERTCACHE_OFFSET_MASK;
  ...
  if (surf->jointCache) {
    ...
    if (!verify(!renderProgManager.ShaderUsesJoints() ||
                renderProgManager.ShaderHasOptionalSkinning())) {
      return;
    }
  }
  ...
  {
          glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, ( GLintptr
  )indexBuffer->GetAPIObject() ); currentIndexBuffer = ( GLintptr
  ...
  */
  ...
  /*
  if( ( vertexLayout != LAYOUT_DRAW_VERT ) || ( currentVertexBuffer != (
  GLintptr )vertexBuffer->GetAPIObject() ) || !r_useStateCaching.GetBool() )
  {
          glBindBuffer( GL_ARRAY_BUFFER, ( GLintptr
  )vertexBuffer->GetAPIObject() ); currentVertexBuffer = ( GLintptr
  ...
  sizeof( idDrawVert ), ( void* )( DRAWVERT_XYZ_OFFSET ) );
          glVertexAttribPointer( PC_ATTRIB_INDEX_NORMAL, 4, GL_UNSIGNED_BYTE,
  GL_TRUE, sizeof( idDrawVert ), ( void* )( DRAWVERT_NORMAL_OFFSET ) );
          glVertexAttribPointer( PC_ATTRIB_INDEX_COLOR, 4, GL_UNSIGNED_BYTE,
  GL_TRUE, sizeof( idDrawVert ), ( void* )( DRAWVERT_COLOR_OFFSET ) );
          glVertexAttribPointer( PC_ATTRIB_INDEX_COLOR2, 4, GL_UNSIGNED_BYTE,
  GL_TRUE, sizeof( idDrawVert ), ( void* )( DRAWVERT_COLOR2_OFFSET ) );
          glVertexAttribPointer( PC_ATTRIB_INDEX_ST, 2, GL_HALF_FLOAT, GL_TRUE,
  sizeof( idDrawVert ), ( void* )( DRAWVERT_ST_OFFSET ) );
          glVertexAttribPointer( PC_ATTRIB_INDEX_TANGENT, 4, GL_UNSIGNED_BYTE,
  GL_TRUE, sizeof( idDrawVert ), ( void* )( DRAWVERT_TANGENT_OFFSET ) );
  ...
  */
  ...
  3 : surf->numIndexes, GL_INDEX_TYPE, ( triIndex_t* )indexOffset, vertOffset /
  sizeof( idDrawVert ) );
  */
...
id::Framebuffer* idRenderBackend::GL_StartFrame() {
  ...
  VkResult result =
      vkAcquireNextImageKHR(vkcontext.device, vkcontext.swapchain, UINT64_MAX,
                            vkcontext.acquireSemaphores[vkcontext.frameParity],
  ...
  switch (result) {
    ...
    default:
      idLib::FatalError(
          "VK: %s - %s", VK_ErrorToString(result),
...
void idRenderBackend::GL_BlockingSwapBuffers() {
  ...
  // Eric: // Eric: Since VK_SUBOPTIMAL_KHR and VK_ERROR_OUT_OF_DATE_KHR here
  // means the window is being resized (or other stuff) handle them instead of
  // killing the app.
...
=================
*/
uint64 idRenderBackend::GL_GetCurrentState() const { return glStateBits; }
...
uint64 idRenderBackend::GL_GetCurrentStateMinusStencil() const {
  return GL_GetCurrentState() &
         ~(GLS_STENCIL_OP_BITS | GLS_STENCIL_FUNC_BITS |
           GLS_STENCIL_FUNC_REF_BITS | GLS_STENCIL_FUNC_MASK_BITS);
}
...
void idRenderBackend::CheckCVars() {
  ...
  r_useTrilinearFiltering.IsModified() || r_lodBias.IsModified() )
  {
...
====================
*/
void idRenderBackend::StereoRenderExecuteBackEndCommands(
    const emptyCommand_t* const allCmds) {
  // RB: TODO ?
}
...
void idRenderBackend::DrawStencilShadowPass(const drawSurf_t* drawSurf,
  ...
  if (renderZPass) {
    ...
    uint64 stencil = GLS_STENCIL_OP_FAIL_KEEP | GLS_STENCIL_OP_ZFAIL_KEEP |
                     GLS_STENCIL_OP_PASS_INCR | GLS_BACK_STENCIL_OP_FAIL_KEEP |
                     GLS_BACK_STENCIL_OP_ZFAIL_KEEP |
                     GLS_BACK_STENCIL_OP_PASS_DECR;

    GL_State((glStateBits & ~GLS_STENCIL_OP_BITS) | stencil);
  } else if (r_useStencilShadowPreload.GetBool()) {
    // preload + Z-pass
    uint64 stencil = GLS_STENCIL_OP_FAIL_KEEP | GLS_STENCIL_OP_ZFAIL_DECR |
                     GLS_STENCIL_OP_PASS_DECR | GLS_BACK_STENCIL_OP_FAIL_KEEP |
                     GLS_BACK_STENCIL_OP_ZFAIL_INCR |
                     GLS_BACK_STENCIL_OP_PASS_INCR;

    GL_State((glStateBits & ~GLS_STENCIL_OP_BITS) | stencil);
  } else {
    // Z-fail (Carmack's Reverse)
    uint64 stencil = GLS_STENCIL_OP_FAIL_KEEP | GLS_STENCIL_OP_ZFAIL_DECR |
                     GLS_STENCIL_OP_PASS_KEEP | GLS_BACK_STENCIL_OP_FAIL_KEEP |
                     GLS_BACK_STENCIL_OP_ZFAIL_INCR |
                     GLS_BACK_STENCIL_OP_PASS_KEEP;
  ...
  const uint64 indexOffset =
      (int)(ibHandle >> VERTCACHE_OFFSET_SHIFT) & VERTCACHE_OFFSET_MASK;
