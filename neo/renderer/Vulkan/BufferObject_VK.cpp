...
========================
*/
idVertexBuffer::idVertexBuffer() { SetUnmapped(); }
...
bool idVertexBuffer::AllocBufferObject(const void* data, int allocSize,
  ...
  if (usage == BU_STATIC) {
    ...
    // SRS - VMA_MEMORY_USAGE_CPU_ONLY required for BU_DYNAMIC host coherency on
    // OSX, otherwise black screen
...
void idVertexBuffer::Update(const void* data, int size, int offset) const {
  ...
  if (usage == BU_DYNAMIC) {
    ...
    int stageOffset = 0;
    byte* stageData =
        stagingManager.Stage(size, 1, commandBuffer, stageBuffer, stageOffset);
...
void idVertexBuffer::UnmapBuffer() {
  ...
  if (usage == BU_STATIC) {
    idLib::FatalError(
        "idVertexBuffer::UnmapBuffer: Cannot unmap a buffer marked as "
        "BU_STATIC.");
  }
...
========================
*/
idIndexBuffer::idIndexBuffer() { SetUnmapped(); }
...
void idIndexBuffer::UnmapBuffer() {
  ...
  if (usage == BU_STATIC) {
    idLib::FatalError(
        "idIndexBuffer::UnmapBuffer: Cannot unmap a buffer marked as "
        "BU_STATIC.");
  }
...
void idUniformBuffer::UnmapBuffer() {
  ...
  if (usage == BU_STATIC) {
    idLib::FatalError(
        "idUniformBuffer::UnmapBuffer: Cannot unmap a buffer marked as "
        "BU_STATIC.");
  }
