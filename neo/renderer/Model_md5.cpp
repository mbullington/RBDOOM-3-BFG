...
static const __m128 vector_float_posInfinity = {
    idMath::INFINITUM, idMath::INFINITUM, idMath::INFINITUM, idMath::INFINITUM};
static const __m128 vector_float_negInfinity = {
    -idMath::INFINITUM, -idMath::INFINITUM, -idMath::INFINITUM,
    -idMath::INFINITUM};
#endif
...
static const unsigned int MD5B_MAGIC =
    ('5' << 24) | ('D' << 16) | ('M' << 8) | MD5B_VERSION;

idCVar r_useGPUSkinning("r_useGPUSkinning", "1", CVAR_INTEGER,
                        "animate normals and tangents instead of deriving");
...
====================
*/
void idMD5Mesh::ParseMesh(idLexer& parser, int numJoints,
  ...
  //
  idDrawVert* basePose = (idDrawVert*)Mem_ClearedAlloc(
      texCoords.Num() * sizeof(*basePose), TAG_MD5_BASE);
  for (int j = 0, i = 0; i < texCoords.Num(); i++) {
    idVec3 v = (*(idJointMat*)((byte*)joints + weightIndex[j * 2 + 0])) *
               scaledWeights[j];
    while (weightIndex[j * 2 + 1] == 0) {
      j++;
      v += (*(idJointMat*)((byte*)joints + weightIndex[j * 2 + 0])) *
           scaledWeights[j];
    }
    j++;
  ...
  for (int i = 0; i < texCoords.Num(); i++) {
    ...
    for (int j = 0; j < numWeights; j++) {
      for (int k = 0; k < numWeights - 1 - j; k++) {
        if (tempWeights[weights[k]].jointWeight <
            tempWeights[weights[k + 1]].jointWeight) {
    ...
    finalWeights[0] += Max(255 - finalWeights[0] - finalWeights[1] -
                               finalWeights[2] - finalWeights[3],
                           0);
  ...
  meshJoints =
      (byte*)Mem_Alloc(numMeshJoints * sizeof(meshJoints[0]), TAG_MODEL);
  numMeshJoints = 0;
  ...
  deformInfo = R_BuildDeformInfo(texCoords.Num(), basePose, tris.Num(),
                                 tris.Ptr(), shader->UseUnsmoothedTangents());
...
============
*/
void TransformVertsAndTangents(idDrawVert* targetVerts, const int numVerts,
  ...
  for (int i = 0; i < numVerts; i++) {
    ...
    targetVerts[i].xyz =
        accum * idVec4(base.xyz.x, base.xyz.y, base.xyz.z, 1.0f);
...
void idMD5Mesh::UpdateSurface(const struct renderEntity_s* ent,
  ...
  if (r_useGPUSkinning.GetBool() && glConfig.gpuSkinningAvailable) {
    ...
    if (tri->verts == NULL || tri->verts == deformInfo->verts) {
      ...
      memcpy(
          tri->verts, deformInfo->verts,
          deformInfo->numOutputVerts *
              sizeof(
                  deformInfo->verts[0]));  // copy over the texture coordinates
    }
    TransformVertsAndTangents(tri->verts, deformInfo->numOutputVerts,
                              deformInfo->verts, entJointsInverted);
...
====================
*/
void idMD5Mesh::CalculateBounds(const idJointMat* entJoints,
...
====================
*/
void idRenderModelMD5::ParseJoint(idLexer& parser, idMD5Joint* joint,
...
========================
*/
bool idRenderModelMD5::LoadBinaryModel(idFile* file,
  ...
  for (int i = 0; i < meshes.Num(); i++) {
    ...
    meshes[i].meshJoints = (byte*)Mem_Alloc(
        meshes[i].numMeshJoints * sizeof(meshes[i].meshJoints[0]), TAG_MODEL);
    file->ReadBigArray(meshes[i].meshJoints, meshes[i].numMeshJoints);
    file->ReadBig(meshes[i].maxJointVertDist);

    meshes[i].deformInfo =
        (deformInfo_t*)R_ClearedStaticAlloc(sizeof(deformInfo_t));
...
========================
*/
void idRenderModelMD5::WriteBinaryModel(idFile* file,
...
void idRenderModelMD5::LoadModel() {
  ...
  parser.ExpectTokenString("{");
  idJointMat* poseMat =
      (idJointMat*)_alloca16(joints.Num() * sizeof(poseMat[0]));
  for (int i = 0; i < joints.Num(); i++) {
    ...
    if (joint->parent) {
      ...
      pose->q = (poseMat[i].ToMat3() * poseMat[parentNum].ToMat3().Transpose())
                    .ToQuat();
      pose->t = (poseMat[i].ToVec3() - poseMat[parentNum].ToVec3()) *
                poseMat[parentNum].ToMat3().Transpose();
    }
  }
  parser.ExpectTokenString("}");
...
void idRenderModelMD5::DrawJoints(const renderEntity_t* ent,
  ...
  for (i = 0; i < num; i++, joint++, md5Joint++) {
    ...
    common->RW()->DebugLine(colorRed, pos,
                            pos + joint->ToMat3()[0] * 2.0f * ent->axis);
    common->RW()->DebugLine(colorGreen, pos,
                            pos + joint->ToMat3()[1] * 2.0f * ent->axis);
    common->RW()->DebugLine(colorBlue, pos,
  ...
  if ((r_jointNameScale.GetFloat() != 0.0f) &&
    ...
    for (i = 0; i < num; i++, joint++) {
      pos = ent->origin + joint->ToVec3() * ent->axis;
      common->RW()->DrawText(joints[i].name, pos + offset, scale, colorWhite,
...
static void TransformJoints(idJointMat* __restrict outJoints,
                            const int numJoints,
                            const idJointMat* __restrict inJoints1,
...
idRenderModel* idRenderModelMD5::InstantiateDynamicModel(
  ...
  if (staticModel->jointsInverted == NULL) {
    ...
    staticModel->jointsInverted = (idJointMat*)Mem_ClearedAlloc(
        numInvertedJoints * sizeof(idJointMat), TAG_JOINTMAT);
    staticModel->jointsInvertedBuffer = 0;
  } else {
    assert(staticModel->numInvertedJoints == numInvertedJoints);
  }

  TransformJoints(staticModel->jointsInverted, joints.Num(), ent->joints,
                  invertedDefaultPose.Ptr());
  ...
  for (int i = 0; i < meshes.Num(); i++, mesh++) {
    ...
    // the deformation of the tangents can be deferred until each surface is
    // added to the view
...
====================
*/
dynamicModel_t idRenderModelMD5::IsDynamicModel() const { return DM_CACHED; }
...
====================
*/
int idRenderModelMD5::NumJoints() const { return joints.Num(); }
...
====================
*/
const idMD5Joint* idRenderModelMD5::GetJoints() const { return joints.Ptr(); }
...
// RB begin
void idRenderModelMD5::ExportOBJ(idFile* objFile, idFile* mtlFile,
  ...
  if (ent.numJoints > 0) {
    ent.joints = (idJointMat*)Mem_ClearedAlloc(
