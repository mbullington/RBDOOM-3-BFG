...
static aseMesh_t* ASE_GetCurrentMesh() { return ase.currentMesh; }
...
static void ASE_SkipRestOfLine() { ASE_GetToken(true); }
...
static void ASE_KeyMATERIAL_LIST(const char* token) {
  if (!strcmp(token, "*MATERIAL_COUNT")) {
    ...
    ase.currentMaterial =
        (aseMaterial_t*)Mem_Alloc(sizeof(aseMaterial_t), TAG_MODEL);
...
static void ASE_KeyMESH(const char* token) {
  ...
  if (!strcmp(token, "*TIMEVALUE")) {
    ...
    ase.currentVertex = 0;
    pMesh->cvertexes =
        (idVec3*)Mem_Alloc(sizeof(idVec3) * pMesh->numCVertexes, TAG_MODEL);
    VERBOSE((".....parsing MESH_CVERTLIST\n"));
    ASE_ParseBracedBlock(ASE_KeyMESH_CVERTLIST);
  } else if (!strcmp(token, "*MESH_FACE_LIST")) {
    pMesh->faces =
        (aseFace_t*)Mem_Alloc(sizeof(aseFace_t) * pMesh->numFaces, TAG_MODEL);
    ase.currentFace = 0;
