...
void idDxtEncoder::NV4XHardwareBugFix(byte* minColor, byte* maxColor) const {
  int minq =
      ((minColor[0] << 16) | (minColor[1] << 8) | minColor[2]) & 0x00F8FCF8;
  int maxq =
      ((maxColor[0] << 16) | (maxColor[1] << 8) | maxColor[2]) & 0x00F8FCF8;
...
========================
*/
bool idDxtEncoder::HasConstantValuePer4x4Block(const byte* inBuf, int width,
...
========================
*/
void idDxtEncoder::WriteTinyColorDXT1(const byte* inBuf, int width,
                                      int height) {
  int numBlocks = ((width + 3) / 4) * ((height + 3) / 4);
  int stride = ((width * height) / numBlocks) *
               4;  // number of bytes from one block to the next
  // example: 2x8 pixels
  // numBlocks = 2
...
========================
*/
void idDxtEncoder::WriteTinyColorDXT5(const byte* inBuf, int width,
                                      int height) {
  int numBlocks = ((width + 3) / 4) * ((height + 3) / 4);
  int stride = ((width * height) / numBlocks) *
               4;  // number of bytes from one block to the next
...
========================
*/
void idDxtEncoder::WriteTinyColorCTX1DXT5A(const byte* inBuf, int width,
                                           int height) {
  int numBlocks = ((width + 3) / 4) * ((height + 3) / 4);
  int stride = ((width * height) / numBlocks) *
               4;  // number of bytes from one block to the next
...
========================
*/
void idDxtEncoder::WriteTinyNormalMapDXT5(const byte* inBuf, int width,
                                          int height) {
  int numBlocks = ((width + 3) / 4) * ((height + 3) / 4);
  int stride = ((width * height) / numBlocks) *
               4;  // number of bytes from one block to the next
...
========================
*/
void idDxtEncoder::WriteTinyNormalMapDXN(const byte* inBuf, int width,
                                         int height) {
  int numBlocks = ((width + 3) / 4) * ((height + 3) / 4);
  int stride = ((width * height) / numBlocks) *
               4;  // number of bytes from one block to the next
...
Finds the two RGB colors in a 4x4 block furthest apart. Also finds the two alpha
values furthest apart.
...
========================
*/
void idDxtEncoder::GetMinMaxColorsMaxDist(const byte* colorBlock,
                                          byte* minColor,
                                          byte* maxColor) const {
  int maxDistC = -1;
  int maxDistA = -1;
...
Finds the two RGB colors in a 4x4 block furthest apart based on luminance. Also
finds the two alpha values furthest apart.
...
========================
*/
void idDxtEncoder::GetMinMaxColorsLuminance(const byte* colorBlock,
                                            byte* minColor,
                                            byte* maxColor) const {
  ...
  for (int i = 0; i < 16; i++) {
    int luminance = colorBlock[i * 4 + 0] + colorBlock[i * 4 + 1] * 2 +
                    colorBlock[i * 4 + 2];
...
========================
*/
int idDxtEncoder::GetSquareAlphaError(const byte* colorBlock,
...
========================
*/
int idDxtEncoder::GetMinMaxAlphaHQ(const byte* colorBlock,
                                   const int alphaOffset, byte* minColor,
                                   byte* maxColor) const {
  int i, j;
  ...
  for (i = alphaMin; i <= alphaMax; i++) {
    for (j = alphaMax; j >= i; j--) {
      error = GetSquareAlphaError(colorBlock, alphaOffset, (byte)i, (byte)j,
                                  bestError);
      if (error < bestError) {
        bestError = error;
      ...
      error = GetSquareAlphaError(colorBlock, alphaOffset, (byte)j, (byte)i,
                                  bestError);
      if (error < bestError) {
        bestError = error;
        minColor[alphaOffset] = (byte)i;
...
========================
*/
int idDxtEncoder::GetSquareColorsError(const byte* colorBlock,
                                       const unsigned short color0,
                                       const unsigned short color1,
                                       int lastError) const {
  int i, j;
  byte colors[4][4];
...
========================
*/
int idDxtEncoder::GetSquareNormalYError(const byte* colorBlock,
                                        const unsigned short color0,
                                        const unsigned short color1,
                                        int lastError, int scale) const {
  int i, j;
  byte colors[4][4];
...
Uses an exhaustive search to find the two RGB colors that produce the least
error when used to compress the 4x4 block. Also finds the minimum and maximum
alpha values.
...
========================
*/
int idDxtEncoder::GetMinMaxColorsHQ(const byte* colorBlock, byte* minColor,
                                    byte* maxColor, bool noBlack) const {
  int i;
  ...
  // get the minimum distance the end points of the line must be apart along
  // each axis
  for (i = 0; i < 3; i++) {
  ...
  bboxMin[0] =
      (bboxMin[0] <= C565_BBOX_EXPAND) ? 0 : bboxMin[0] - C565_BBOX_EXPAND;
  bboxMin[1] =
      (bboxMin[1] <= C565_BBOX_EXPAND) ? 0 : bboxMin[1] - C565_BBOX_EXPAND;
  bboxMin[2] =
      (bboxMin[2] <= C565_BBOX_EXPAND) ? 0 : bboxMin[2] - C565_BBOX_EXPAND;
  bboxMax[0] = (bboxMax[0] >= (255 >> 3) - C565_BBOX_EXPAND)
                   ? (255 >> 3)
                   : bboxMax[0] + C565_BBOX_EXPAND;
  bboxMax[1] = (bboxMax[1] >= (255 >> 2) - C565_BBOX_EXPAND)
                   ? (255 >> 2)
                   : bboxMax[1] + C565_BBOX_EXPAND;
  bboxMax[2] = (bboxMax[2] >= (255 >> 3) - C565_BBOX_EXPAND)
                   ? (255 >> 3)
                   : bboxMax[2] + C565_BBOX_EXPAND;
        ...
        for (j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
          ...
          for (i2 = bboxMin[2]; i2 <= bboxMax[2]; i2++) {
            for (j2 = bboxMax[2]; j2 >= bboxMin[2]; j2--) {
              ...
              minColor565 =
                  (unsigned short)((i0 << 11) | (i1 << 5) | (i2 << 0));
              maxColor565 =
                  (unsigned short)((j0 << 11) | (j1 << 5) | (j2 << 0));

              if (!noBlack) {
                error = GetSquareColorsError(colorBlock, maxColor565,
                                             minColor565, bestError);
                if (error < bestError) {
                  bestError = error;
              ...
              error = GetSquareColorsError(colorBlock, minColor565, maxColor565,
                                           bestError);
              if (error < bestError) {
                bestError = error;
                bestMinColor565 = minColor565;
...
========================
*/
int idDxtEncoder::GetSquareCTX1Error(const byte* colorBlock, const byte* color0,
...
========================
*/
int idDxtEncoder::GetMinMaxCTX1HQ(const byte* colorBlock, byte* minColor,
                                  byte* maxColor) const {
  int i;
  int i0, i1, j0, j1;
  ...
  bboxMin[0] =
      (bboxMin[0] <= CXT1_BBOX_EXPAND) ? 0 : bboxMin[0] - CXT1_BBOX_EXPAND;
  bboxMin[1] =
      (bboxMin[1] <= CXT1_BBOX_EXPAND) ? 0 : bboxMin[1] - CXT1_BBOX_EXPAND;
  bboxMax[0] = (bboxMax[0] >= 255 - CXT1_BBOX_EXPAND)
                   ? 255
                   : bboxMax[0] + CXT1_BBOX_EXPAND;
  bboxMax[1] = (bboxMax[1] >= 255 - CXT1_BBOX_EXPAND)
                   ? 255
                   : bboxMax[1] + CXT1_BBOX_EXPAND;
    ...
    for (j0 = bboxMax[0]; j0 >= bboxMin[0]; j0--) {
      ...
      for (i1 = bboxMin[1]; i1 <= bboxMax[1]; i1++) {
        for (j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
          ...
          error = GetSquareCTX1Error(colorBlock, curMinColor, curMaxColor,
                                     bestError);
          if (error < bestError) {
            bestError = error;
...
========================
*/
int idDxtEncoder::GetMinMaxNormalYHQ(const byte* colorBlock, byte* minColor,
                                     byte* maxColor, bool noBlack,
                                     int scale) const {
  ...
  for (int i1 = bboxMin[1]; i1 <= bboxMax[1]; i1++) {
    for (int j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
      ...
      if (!noBlack) {
        error = GetSquareNormalYError(colorBlock, maxColor565, minColor565,
                                      bestError, scale);
        if (error < bestError) {
          bestError = error;
          bestMinColor565 = minColor565;
      ...
      error = GetSquareNormalYError(colorBlock, minColor565, maxColor565,
                                    bestError, scale);
      if (error < bestError) {
        bestError = error;
        bestMinColor565 = minColor565;
...
ALIGN16(static float SIMD_SSE2_float_scale[4]) = {2.0f / 255.0f, 2.0f / 255.0f,
                                                  2.0f / 255.0f, 2.0f / 255.0f};
ALIGN16(static float SIMD_SSE2_float_descale[4]) = {
    255.0f / 2.0f, 255.0f / 2.0f, 255.0f / 2.0f, 255.0f / 2.0f};
...
ALIGN16(static dword SIMD_SSE2_dword_maskFirstThree[4]) = {
    0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x00000000};
ALIGN16(static dword SIMD_SSE2_dword_maskWords[4]) = {0x0000FFFF, 0x0000FFFF,
...
int NormalDistanceDXT1(const int* vector, const int* normalized) {
  ...
  float rcplen = idMath::InvSqrt(floatNormal[0] * floatNormal[0] +
                                 floatNormal[1] * floatNormal[1] +
                                 floatNormal[2] * floatNormal[2]);
  ...
  int result =
      ((intNormal[0] - normalized[0]) * (intNormal[0] - normalized[0])) +
...
int NormalDistanceDXT5(const int* vector, const int* normalized) {
  ...
  int result =
      ((intNormal[c0] - normalized[c0]) * (intNormal[c0] - normalized[c0])) +
...
========================
*/
int idDxtEncoder::GetSquareNormalsDXT1Error(const int* colorBlock,
                                            const unsigned short color0,
                                            const unsigned short color1,
                                            int lastError,
...
========================
*/
int idDxtEncoder::GetMinMaxNormalsDXT1HQ(const byte* colorBlock, byte* minColor,
                                         byte* maxColor,
        ...
        for (j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
          ...
          for (i2 = bboxMin[2]; i2 <= bboxMax[2]; i2++) {
            for (j2 = bboxMax[2]; j2 >= bboxMin[2]; j2--) {
              ...
              unsigned short minColor565 =
                  (unsigned short)((i0 << 11) | (i1 << 5) | (i2 << 0));
              unsigned short maxColor565 =
                  (unsigned short)((j0 << 11) | (j1 << 5) | (j2 << 0));

              if (!noBlack) {
                error = GetSquareNormalsDXT1Error(intColorBlock, maxColor565,
                                                  minColor565, bestError,
                                                  tempColorIndices);
                if (error < bestError) {
                  bestError = error;
              ...
              error = GetSquareNormalsDXT1Error(intColorBlock, minColor565,
                                                maxColor565, bestError,
                                                tempColorIndices);
              if (error < bestError) {
                bestError = error;
                bestMinColor565 = minColor565;
...
========================
*/
int idDxtEncoder::GetSquareNormalsDXT5Error(
    const int* normalBlock, const byte* minNormal, const byte* maxNormal,
    int lastError, unsigned int& colorIndices, byte* alphaIndices) const {
  byte alphas[8];
  byte colors[4][4];
  ...
  alphaIndices[0] =
      byte((tempAlphaIndices[0] >> 0) | (tempAlphaIndices[1] << 3) |
           (tempAlphaIndices[2] << 6));
  alphaIndices[1] =
      byte((tempAlphaIndices[2] >> 2) | (tempAlphaIndices[3] << 1) |
           (tempAlphaIndices[4] << 4) | (tempAlphaIndices[5] << 7));
  alphaIndices[2] =
      byte((tempAlphaIndices[5] >> 1) | (tempAlphaIndices[6] << 2) |
           (tempAlphaIndices[7] << 5));

  alphaIndices[3] =
      byte((tempAlphaIndices[8] >> 0) | (tempAlphaIndices[9] << 3) |
           (tempAlphaIndices[10] << 6));
  alphaIndices[4] =
      byte((tempAlphaIndices[10] >> 2) | (tempAlphaIndices[11] << 1) |
           (tempAlphaIndices[12] << 4) | (tempAlphaIndices[13] << 7));
  alphaIndices[5] =
      byte((tempAlphaIndices[13] >> 1) | (tempAlphaIndices[14] << 2) |
           (tempAlphaIndices[15] << 5));
...
========================
*/
int idDxtEncoder::GetMinMaxNormalsDXT5HQ(const byte* colorBlock, byte* minColor,
                                         byte* maxColor,
                                         unsigned int& colorIndices,
  ...
  bboxMin[1] =
      (bboxMin[1] <= C565_BBOX_EXPAND) ? 0 : bboxMin[1] - C565_BBOX_EXPAND;
  bboxMin[3] =
      (bboxMin[3] <= ALPHA_BBOX_EXPAND) ? 0 : bboxMin[3] - ALPHA_BBOX_EXPAND;
  bboxMax[0] = (bboxMax[0] >= (255 >> 3) - C565_BBOX_EXPAND)
                   ? (255 >> 3)
                   : bboxMax[0] + C565_BBOX_EXPAND;
  bboxMax[1] = (bboxMax[1] >= (255 >> 2) - C565_BBOX_EXPAND)
                   ? (255 >> 2)
                   : bboxMax[1] + C565_BBOX_EXPAND;
  bboxMax[3] = (bboxMax[3] >= (255) - ALPHA_BBOX_EXPAND)
                   ? (255)
                   : bboxMax[3] + ALPHA_BBOX_EXPAND;
        ...
        for (j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
          ...
          for (i3 = bboxMin[3]; i3 <= bboxMax[3]; i3++) {
            for (j3 = bboxMax[3]; j3 >= bboxMin[3]; j3--) {
              ...
              error = GetSquareNormalsDXT5Error(intColorBlock, tmin, tmax,
                                                bestError, tempColorIndices,
                                                tempAlphaIndices);
              if (error < bestError) {
                bestError = error;
...
========================
*/
int idDxtEncoder::GetMinMaxNormalsDXT5HQFast(const byte* colorBlock,
                                             byte* minColor, byte* maxColor,
                                             unsigned int& colorIndices,
                                             byte* alphaIndices) const {
  ...
  bboxMin[2] =
      (bboxMin[2] <= C565_BBOX_EXPAND) ? 0 : bboxMin[2] - C565_BBOX_EXPAND;
  bboxMax[2] = (bboxMax[2] >= (255 >> 2) - C565_BBOX_EXPAND)
                   ? (255 >> 2)
                   : bboxMax[2] + C565_BBOX_EXPAND;
#endif
        ...
        for (j1 = bboxMax[1]; j1 >= bboxMin[1]; j1--) {
          ...
          for (i2 = bboxMin[2]; i2 <= bboxMax[2]; i2++) {
            for (j2 = bboxMax[2]; j2 >= bboxMin[2]; j2--) {
              ...
              unsigned short minColor565 =
                  (unsigned short)((i0 << 11) | (i1 << 5) | i2);
              unsigned short maxColor565 =
                  (unsigned short)((j0 << 11) | (j1 << 5) | j2);
  ...
  for (i3 = bboxMin[3]; i3 <= bboxMax[3]; i3++) {
    for (j3 = bboxMax[3]; j3 >= bboxMin[3]; j3--) {
      ...
      error = GetSquareNormalsDXT5Error(intColorBlock, tmin, tmax, bestError,
                                        tempColorIndices, tempAlphaIndices);
      if (error < bestError) {
        bestError = error;
        memcpy(minColor, tmin, 4);
        memcpy(maxColor, tmax, 4);
...
========================
*/
int idDxtEncoder::FindColorIndices(const byte* colorBlock,
                                   const unsigned short color0,
                                   const unsigned short color1,
...
========================
*/
int idDxtEncoder::FindAlphaIndices(const byte* colorBlock,
                                   const int alphaOffset, const byte alpha0,
                                   const byte alpha1, byte* rindexes) const {
  int i, j;
  unsigned int indexes[16];
  byte alphas[8];
  ...
  rindexes[1] = byte((indexes[2] >> 2) | (indexes[3] << 1) | (indexes[4] << 4) |
                     (indexes[5] << 7));
  rindexes[2] = byte((indexes[5] >> 1) | (indexes[6] << 2) | (indexes[7] << 5));

  rindexes[3] =
      byte((indexes[8] >> 0) | (indexes[9] << 3) | (indexes[10] << 6));
  rindexes[4] = byte((indexes[10] >> 2) | (indexes[11] << 1) |
                     (indexes[12] << 4) | (indexes[13] << 7));
  rindexes[5] =
      byte((indexes[13] >> 1) | (indexes[14] << 2) | (indexes[15] << 5));
...
========================
*/
int idDxtEncoder::FindCTX1Indices(const byte* colorBlock, const byte* color0,
                                  const byte* color1,
                                  unsigned int& result) const {
  int i, j;
  unsigned int indexes[16];
...
========================
*/
void idDxtEncoder::CompressImageDXT1HQ(const byte* inBuf, byte* outBuf,
  ...
  for (int j = 0; j < height; j += 4, inBuf += width * 4 * 4) {
    for (int i = 0; i < width; i += 4) {
      ...
      // Write out color data. Try and find minimum error for the two encoding
      // methods.
...
========================
*/
void idDxtEncoder::CompressImageDXT5HQ(const byte* inBuf, byte* outBuf,
                                       int width, int height) {
  ALIGN16(byte block[64]);
  ...
  for (int j = 0; j < height; j += 4, inBuf += width * 4 * 4) {
    for (int i = 0; i < width; i += 4) {
      ...
      // Write out alpha data. Try and find minimum error for the two encoding
      // methods.
...
========================
*/
void idDxtEncoder::CompressImageCTX1HQ(const byte* inBuf, byte* outBuf,
                                       int width, int height) {
  ALIGN16(byte block[64]);
  unsigned int colorIndices;
  byte col1[4];
...
========================
*/
void idDxtEncoder::ExtractBlockGimpDDS(const byte* src, int x, int y, int w,
                                       int h, byte* block) {
  int i, j;
  int bw = MIN(w - x, 4);
  int bh = MIN(h - y, 4);
...
========================
*/
void idDxtEncoder::EncodeAlphaBlockBC3GimpDDS(byte* dst, const byte* block,
                                              const int offset) {
  int i, v, mn, mx;
...
========================
*/
void idDxtEncoder::GetMinMaxYCoCgGimpDDS(const byte* block, byte* mincolor,
                                         byte* maxcolor) {
  int i;
...
========================
*/
void idDxtEncoder::ScaleYCoCgGimpDDS(byte* block, byte* mincolor,
                                     byte* maxcolor) {
  const int s0 = 128 / 2 - 1;
  const int s1 = 128 / 4 - 1;
  int m0, m1, m2, m3;
...
========================
*/
void idDxtEncoder::SelectDiagonalYCoCgGimpDDS(const byte* block, byte* mincolor,
                                              byte* maxcolor) {
...
inline unsigned short idDxtEncoder::PackRGB565GimpDDS(const byte* c) {
  return ((Mul8BitGimpDDS(c[2], 31) << 11) | (Mul8BitGimpDDS(c[1], 63) << 5) |
...
========================
*/
void idDxtEncoder::CompressYCoCgDXT5HQ(const byte* inBuf, byte* outBuf,
                                       int width, int height) {
  ALIGN16(byte block[64]);
  byte alphaIndices1[6];
  byte alphaIndices2[6];
...
========================
*/
void idDxtEncoder::CompressYCoCgCTX1DXT5AHQ(const byte* inBuf, byte* outBuf,
                                            int width, int height) {
  ALIGN16(byte block[64]);
  byte alphaIndices1[6];
  byte alphaIndices2[6];
...
========================
*/
void idDxtEncoder::CompressNormalMapDXT1HQ(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  ALIGN16(byte block[64]);
  unsigned int colorIndices;
  byte col1[4];
...
========================
*/
void idDxtEncoder::CompressNormalMapDXT1RenormalizeHQ(const byte* inBuf,
...
void idDxtEncoder::BiasScaleNormalY(byte* colorBlock) const {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    colorBlock[i * 4 + 1] =
        byte((colorBlock[i * 4 + 1] - bestBias) * scale + 128);
    colorBlock[i * 4 + 2] = byte((scale - 1) << 3);
  }
}
...
void idDxtEncoder::RotateNormalsDXT5(byte* block) const {
  ...
  for (int i = 0; i < 32; i += 1) {
    ...
    for (int j = 0; j < 16; j++) {
      rotatedBlock[j * 4 + 1] =
          byte((rotatedBlock[j * 4 + 1] - 128) * scale + 128);
    }
#endif
...
========================
*/
void idDxtEncoder::CompressNormalMapDXT5HQ(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  ALIGN16(byte block[64]);
  byte alphaIndices1[6];
  byte alphaIndices2[6];
...
========================
*/
void idDxtEncoder::CompressNormalMapDXT5RenormalizeHQ(const byte* inBuf,
                                                      byte* outBuf, int width,
                                                      int height) {
  ALIGN16(byte block[64]);
  unsigned int colorIndices;
...
========================
*/
void idDxtEncoder::CompressNormalMapDXN2HQ(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  ALIGN16(byte block[64]);
  byte alphaIndices1[6];
  byte alphaIndices2[6];
...
Takes the extents of the bounding box of the colors in the 4x4 block in RGB
space. Also finds the minimum and maximum alpha values.
...
========================
*/
void idDxtEncoder::SelectColorsDiagonal(const byte* colorBlock, byte* minColor,
                                        byte* maxColor) const {
  ...
  // calculating the covariance of just the sign bits is much faster and gives
...
========================
*/
void idDxtEncoder::EmitColorIndices(const byte* colorBlock,
                                    const byte* minColor,
                                    const byte* maxColor) {
#if 1
  ...
  // uses sum of absolute differences instead of squared distance to find the
  // best match
  for (int i = 15; i >= 0; i--) {
...
========================
*/
void idDxtEncoder::EmitColorAlphaIndices(const byte* colorBlock,
                                         const byte* minColor,
                                         const byte* maxColor) {
  ALIGN16(uint16 colors[4][4]);
  unsigned int result = 0;
...
========================
*/
void idDxtEncoder::EmitCTX1Indices(const byte* colorBlock, const byte* minColor,
                                   const byte* maxColor) {
...
========================
*/
void idDxtEncoder::EmitAlphaIndices(const byte* colorBlock, const int offset,
                                    const byte minAlpha, const byte maxAlpha) {
  ...
  EmitByte((indexes[2] >> 2) | (indexes[3] << 1) | (indexes[4] << 4) |
           (indexes[5] << 7));
  EmitByte((indexes[5] >> 1) | (indexes[6] << 2) | (indexes[7] << 5));

  EmitByte((indexes[8] >> 0) | (indexes[9] << 3) | (indexes[10] << 6));
  EmitByte((indexes[10] >> 2) | (indexes[11] << 1) | (indexes[12] << 4) |
           (indexes[13] << 7));
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (a <= bottom) {
      ...
      indexes[i] =
          (ALPHA_RANGE + 1) + ((minAlpha - a) * ALPHA_RANGE - half) / delta;
    }
  }
...
========================
*/
void idDxtEncoder::CompressImageDXT1Fast_Generic(const byte* inBuf,
                                                 byte* outBuf, int width,
                                                 int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
...
========================
*/
void idDxtEncoder::CompressImageDXT1AlphaFast_Generic(const byte* inBuf,
                                                      byte* outBuf, int width,
                                                      int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
...
========================
*/
void idDxtEncoder::CompressImageDXT5Fast_Generic(const byte* inBuf,
                                                 byte* outBuf, int width,
                                                 int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
...
========================
*/
void idDxtEncoder::ScaleYCoCg(byte* colorBlock, byte* minColor,
                              byte* maxColor) const {
  int m0 = abs(minColor[0] - 128);
  int m1 = abs(minColor[1] - 128);
  int m2 = abs(maxColor[0] - 128);
...
inline void idDxtEncoder::InsetYCoCgBBox(byte* minColor, byte* maxColor) const {
  ...
  mini[0] =
      ((minColor[0] << INSET_COLOR_SHIFT) + inset[0]) >> INSET_COLOR_SHIFT;
  mini[1] =
      ((minColor[1] << INSET_COLOR_SHIFT) + inset[1]) >> INSET_COLOR_SHIFT;
  mini[3] =
      ((minColor[3] << INSET_ALPHA_SHIFT) + inset[3]) >> INSET_ALPHA_SHIFT;

  maxi[0] =
      ((maxColor[0] << INSET_COLOR_SHIFT) - inset[0]) >> INSET_COLOR_SHIFT;
  maxi[1] =
      ((maxColor[1] << INSET_COLOR_SHIFT) - inset[1]) >> INSET_COLOR_SHIFT;
  maxi[3] =
      ((maxColor[3] << INSET_ALPHA_SHIFT) - inset[3]) >> INSET_ALPHA_SHIFT;
...
inline void idDxtEncoder::InsetYCoCgAlpaBBox(byte* minColor,
  ...
  mini[1] =
      ((minColor[1] << INSET_COLOR_SHIFT) + inset[1]) >> INSET_COLOR_SHIFT;
  mini[2] =
      ((minColor[2] << INSET_COLOR_SHIFT) + inset[2]) >> INSET_COLOR_SHIFT;
  mini[3] =
      ((minColor[3] << INSET_ALPHA_SHIFT) + inset[3]) >> INSET_ALPHA_SHIFT;
  ...
  maxi[1] =
      ((maxColor[1] << INSET_COLOR_SHIFT) - inset[1]) >> INSET_COLOR_SHIFT;
  maxi[2] =
      ((maxColor[2] << INSET_COLOR_SHIFT) - inset[2]) >> INSET_COLOR_SHIFT;
  maxi[3] =
      ((maxColor[3] << INSET_ALPHA_SHIFT) - inset[3]) >> INSET_ALPHA_SHIFT;
...
========================
*/
void idDxtEncoder::SelectYCoCgDiagonal(const byte* colorBlock, byte* minColor,
                                       byte* maxColor) const {
  byte side = 0;
...
========================
*/
void idDxtEncoder::CompressYCoCgDXT5Fast_Generic(const byte* inBuf,
                                                 byte* outBuf, int width,
                                                 int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
...
========================
*/
void idDxtEncoder::CompressYCoCgAlphaDXT5Fast(const byte* inBuf, byte* outBuf,
                                              int width, int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
  ALIGN16(byte maxColor[4]);
  ...
  for (int j = 0; j < height; j += 4, inBuf += width * 4 * 4) {
    for (int i = 0; i < width; i += 4) {
      ...
      // scale down the chroma of texels that are close to gray with low
      // luminance
      for (int k = 0; k < 16; k++) {
        if (abs(block[k * 4 + 0] - 132) <= 8 &&
            abs(block[k * 4 + 2] - 132) <= 8 && block[k * 4 + 3] < 96) {
...
========================
*/
void idDxtEncoder::CompressYCoCgCTX1DXT5AFast_Generic(const byte* inBuf,
                                                      byte* outBuf, int width,
                                                      int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte minColor[4]);
...
========================
*/
void idDxtEncoder::EmitGreenIndices(const byte* block, const int offset,
...
========================
*/
void idDxtEncoder::InsetNormalsBBoxDXT5(byte* minNormal,
                                        byte* maxNormal) const {
  int inset[4];
  int mini[4];
  int maxi[4];

  inset[3] =
      (maxNormal[3] - minNormal[3]) - ((1 << (INSET_ALPHA_SHIFT - 1)) - 1);
  inset[1] =
      (maxNormal[1] - minNormal[1]) - ((1 << (INSET_COLOR_SHIFT - 1)) - 1);

  mini[3] =
      ((minNormal[3] << INSET_ALPHA_SHIFT) + inset[3]) >> INSET_ALPHA_SHIFT;
  mini[1] =
      ((minNormal[1] << INSET_COLOR_SHIFT) + inset[1]) >> INSET_COLOR_SHIFT;

  maxi[3] =
      ((maxNormal[3] << INSET_ALPHA_SHIFT) - inset[3]) >> INSET_ALPHA_SHIFT;
  maxi[1] =
      ((maxNormal[1] << INSET_COLOR_SHIFT) - inset[1]) >> INSET_COLOR_SHIFT;
...
void idDxtEncoder::InsetNormalsBBox3Dc(byte* minNormal, byte* maxNormal) const {
  ...
  inset[0] =
      (maxNormal[0] - minNormal[0]) - ((1 << (INSET_ALPHA_SHIFT - 1)) - 1);
  inset[1] =
      (maxNormal[1] - minNormal[1]) - ((1 << (INSET_ALPHA_SHIFT - 1)) - 1);

  mini[0] =
      ((minNormal[0] << INSET_ALPHA_SHIFT) + inset[0]) >> INSET_ALPHA_SHIFT;
  mini[1] =
      ((minNormal[1] << INSET_ALPHA_SHIFT) + inset[1]) >> INSET_ALPHA_SHIFT;

  maxi[0] =
      ((maxNormal[0] << INSET_ALPHA_SHIFT) - inset[0]) >> INSET_ALPHA_SHIFT;
  maxi[1] =
      ((maxNormal[1] << INSET_ALPHA_SHIFT) - inset[1]) >> INSET_ALPHA_SHIFT;
...
========================
*/
void idDxtEncoder::CompressNormalMapDXT5Fast_Generic(const byte* inBuf,
                                                     byte* outBuf, int width,
                                                     int height) {
  ALIGN16(byte block[64]);
...
========================
*/
void idDxtEncoder::CompressImageDXN1Fast_Generic(const byte* inBuf,
                                                 byte* outBuf, int width,
                                                 int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte min[4]);
...
========================
*/
void idDxtEncoder::CompressNormalMapDXN2Fast_Generic(const byte* inBuf,
                                                     byte* outBuf, int width,
                                                     int height) {
  ALIGN16(byte block[64]);
  ALIGN16(byte normal1[4]);
...
========================
*/
void idDxtEncoder::EncodeNormalRGBIndices(byte* outBuf, const byte min,
                                          const byte max, const byte* values) {
  const int COLOR_RANGE = 3;
...
========================
*/
void idDxtEncoder::ConvertNormalMapDXN2_DXT5(const byte* inBuf, byte* outBuf,
                                             int width, int height) {
...
========================
*/
void idDxtEncoder::DecodeNormalYValues(const byte* inBuf, byte& min, byte& max,
                                       byte* values) {
  int i;
  ...
  indexes = (unsigned int)inBuf[4] | ((unsigned int)inBuf[5] << 8) |
            ((unsigned int)inBuf[6] << 16) | ((unsigned int)inBuf[7] << 24);
  for (i = 0; i < 16; i++) {
...
========================
*/
void idDxtEncoder::EncodeDXNAlphaValues(byte* outBuf, const byte min,
                                        const byte max, const byte* values) {
  int i;
  byte alphas[8];
  int j;
...
========================
*/
void idDxtEncoder::ConvertNormalMapDXT5_DXN2(const byte* inBuf, byte* outBuf,
                                             int width, int height) {
  ALIGN16(byte values[16]);
...
========================
*/
void idDxtEncoder::ConvertImageDXN1_DXT1(const byte* inBuf, byte* outBuf,
                                         int width, int height) {
  ALIGN16(byte values[16]);
