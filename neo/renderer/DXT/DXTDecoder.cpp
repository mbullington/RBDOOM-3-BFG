...
========================
*/
void idDxtDecoder::EmitBlock(byte* outPtr, int x, int y,
                             const byte* colorBlock) {
  outPtr += (y * width + x) * 4;
  for (int j = 0; j < 4; j++) {
...
========================
*/
void idDxtDecoder::DecodeColorValues(byte* colorBlock, bool noBlack,
                                     bool writeAlpha) {
  byte colors[4][4];
...
========================
*/
void idDxtDecoder::DecompressImageDXT1(const byte* inBuf, byte* outBuf,
...
========================
*/
void idDxtDecoder::DecompressImageDXT5(const byte* inBuf, byte* outBuf,
                                       int width, int height) {
  byte block[64];
...
========================
*/
void idDxtDecoder::DecompressImageDXT5_nVidia7x(const byte* inBuf, byte* outBuf,
                                                int width, int height) {
  byte block[64];
...
========================
*/
void idDxtDecoder::DecompressYCoCgDXT5(const byte* inBuf, byte* outBuf,
                                       int width, int height) {
  ...
  for (int i = 0; i < width * height; i++) {
    ...
    outBuf[i * 4 + 2] =
        0;  // this translates to a scale factor of 1 for uncompressed
  }
}
...
========================
*/
void idDxtDecoder::DecompressYCoCgCTX1DXT5A(const byte* inBuf, byte* outBuf,
                                            int width, int height) {
  byte block[64];
...
========================
*/
void idDxtDecoder::DecodeNormalYValues(byte* normalBlock, const int offsetY,
                                       byte& c0, byte& c1) {
  int i;
  unsigned int indexes;
...
void idDxtDecoder::DeriveNormalZValues(byte* normalBlock) {
  ...
  for (i = 0; i < 16; i++) {
    ...
    normalBlock[i * 4 + 2] =
        128 + UShortSqrt((unsigned short)(16383 - x * x - y * y));
  }
}
...
========================
*/
void idDxtDecoder::DecompressNormalMapDXT1(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  byte block[64];
  ...
  for (int j = 0; j < height; j += 4) {
    for (int i = 0; i < width; i += 4) {
      ...
      */
      ...
      for (int k = 0; k < 16; k++) {
        block[k * 4 + 0] =
            idMath::Ftob((normals[k * 4 + 0] + 1.0f) / 2.0f * 255.0f);
        block[k * 4 + 1] =
            idMath::Ftob((normals[k * 4 + 1] + 1.0f) / 2.0f * 255.0f);
        block[k * 4 + 2] =
            idMath::Ftob((normals[k * 4 + 2] + 1.0f) / 2.0f * 255.0f);
      }
#else
...
========================
*/
void idDxtDecoder::DecompressNormalMapDXT1Renormalize(const byte* inBuf,
  ...
  for (int j = 0; j < height; j += 4) {
    for (int i = 0; i < width; i += 4) {
      ...
      for (int k = 0; k < 16; k++) {
        ...
        float rsq =
            idMath::InvSqrt(normal[0] * normal[0] + normal[1] * normal[1] +
                            normal[2] * normal[2]);
        normal[0] *= rsq;
        normal[1] *= rsq;
        normal[2] *= rsq;
        block[k * 4 + 0] =
            idMath::Ftob((normal[0] + 1.0f) / 2.0f * 255.0f + 0.5f);
        block[k * 4 + 1] =
            idMath::Ftob((normal[1] + 1.0f) / 2.0f * 255.0f + 0.5f);
        block[k * 4 + 2] =
            idMath::Ftob((normal[2] + 1.0f) / 2.0f * 255.0f + 0.5f);
      }
...
========================
*/
void idDxtDecoder::DecompressNormalMapDXT5Renormalize(const byte* inBuf,
                                                      byte* outBuf, int width,
                                                      int height) {
  byte block[64];
...
========================
*/
void BiasScaleNormalY(byte* normals, const int offsetY, const byte c0,
                      const byte c1) {
  int bias = c0 - 4;
  int scale = (c1 >> 3) + 1;
  for (int i = 0; i < 16; i++) {
    normals[i * 4 + offsetY] =
        byte((normals[i * 4 + offsetY] - 128) / scale + bias);
  }
}
...
========================
*/
void BiasScaleNormals(const byte* block, float* normals, const byte c0,
                      const byte c1) {
  int bias = c0 - 4;
  int scale = (c1 >> 3) + 1;
  for (int i = 0; i < 16; i++) {
    ...
    normals[i * 4 + 1] =
        ((block[i * 4 + 1] - 128.0f) / scale + bias) / 255.0f * 2.0f - 1.0f;
  }
}
...
========================
*/
void idDxtDecoder::DecompressNormalMapDXT5(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  byte block[64];
  byte c0, c1;
...
========================
*/
void idDxtDecoder::DecompressNormalMapDXN2(const byte* inBuf, byte* outBuf,
                                           int width, int height) {
  byte block[64];
...
========================
*/
void idDxtDecoder::DecomposeColorBlock(byte colors[2][4], byte colorIndices[16],
                                       bool noBlack) {
  int i;
...
========================
*/
void idDxtDecoder::DecomposeAlphaBlock(byte colors[2][4],
                                       byte alphaIndices[16]) {
  int i;
...
========================
*/
void idDxtDecoder::DecomposeImageDXT1(const byte* inBuf, byte* colorIndices,
                                      byte* pic1, byte* pic2, int width,
                                      int height) {
  byte colors[2][4];
  byte indices[16];
...
========================
*/
void idDxtDecoder::DecomposeImageDXT5(const byte* inBuf, byte* colorIndices,
                                      byte* alphaIndices, byte* pic1,
