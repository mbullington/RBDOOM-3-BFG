...
idCVar r_writeDemoDecals("r_writeDemoDecals", "1", CVAR_BOOL | CVAR_SYSTEM,
                         "enable Writing of entity decals to demo files.");
idCVar r_writeDemoOverlays("r_writeDemoOverlays", "1", CVAR_BOOL | CVAR_SYSTEM,
                           "enable Writing of entity Overlays to demo files.");

// idCVar r_writeDemoGUI( "r_writeDemoGUI", "0", CVAR_BOOL | CVAR_SYSTEM,
// "enable Writing of GUI elements to demo files." ); #define WRITE_GUIS
...
================
*/
void idRenderWorldLocal::WriteRenderLight(idDemoFile* f, qhandle_t handle,
...
void idRenderWorldLocal::ReadRenderLight() {
  ...
  if (i) {
    light.prelightModel =
        renderModelManager->FindModel(common->ReadDemo()->ReadHashString());
  }
  common->ReadDemo()->ReadInt(i);
  if (i) {
    light.shader =
        declManager->FindMaterial(common->ReadDemo()->ReadHashString());
  }
...
================
*/
void idRenderWorldLocal::WriteRenderEntity(idDemoFile* f,
  ...
  if (entity->decals &&
      entity->decals->demoSerialCurrent != entity->decals->demoSerialWrite) {
    entity->decals->demoSerialWrite = entity->decals->demoSerialCurrent;
    WriteRenderDecal(f, entity->decals->index);
  }

  if (entity->overlays && entity->overlays->demoSerialCurrent !=
                              entity->overlays->demoSerialWrite) {
