...
static const int DEFFERED_DECAL_TIMEOUT =
    1000;  // don't create a decal if it wasn't visible within the first second
static const int MAX_DECALS = 128;
#else
static const int MAX_DEFERRED_DECALS = 16;
static const int DEFFERED_DECAL_TIMEOUT =
    200;  // don't create a decal if it wasn't visible within the first 200
          // milliseconds
static const int MAX_DECALS = 128;
#endif
static const int MAX_DECAL_VERTS =
    3 + NUM_DECAL_BOUNDING_PLANES + 3 +
    6;  // 3 triangle verts clipped NUM_DECAL_BOUNDING_PLANES + 3 times (plus 6
        // for safety)
static const int MAX_DECAL_INDEXES = (MAX_DECAL_VERTS - 2) * 3;

compile_time_assert(CONST_ISPOWEROFTWO(MAX_DECALS));
// the max indices must be a multiple of 2 for copying indices to write-combined
// memory
...
class idRenderModelDecal {
 public:
  ...
  static bool CreateProjectionParms(decalProjectionParms_t& parms,
  ...
  static void GlobalProjectionParmsToLocal(
  ...
  // Save the parameters for the renderer front-end to actually create the
  // decal.
  ...
  struct drawSurf_t* CreateDecalDrawSurf(const struct viewEntity_t* space,
                                         unsigned int index);
 ...
 private:
  ...
  void CreateDecalFromWinding(const idWinding& w,
                              const idMaterial* decalMaterial,
                              const idPlane fadePlanes[2], float fadeDepth,
                              int startTime);
