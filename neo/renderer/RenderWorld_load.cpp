...
================
*/
idRenderModel* idRenderWorldLocal::ParseModel(idLexer* src, const char* mapName,
  ...
  for (int i = 0; i < numSurfaces; i++) {
    ...
    for (int j = 0; j < tri->numIndexes; j += 3) {
      ...
      for (int n = 0; n < queue.Num(); n++) {
        ...
        for (int k = 0; k < tri->numIndexes; k += 3) {
          ...
          bool connected = indexes[t + 0] == indexes[k + 0] ||
                           indexes[t + 0] == indexes[k + 1] ||
                           indexes[t + 0] == indexes[k + 2] ||
                           indexes[t + 1] == indexes[k + 0] ||
                           indexes[t + 1] == indexes[k + 1] ||
                           indexes[t + 1] == indexes[k + 2] ||
                           indexes[t + 2] == indexes[k + 0] ||
                           indexes[t + 2] == indexes[k + 1] ||
                           indexes[t + 2] == indexes[k + 2];
          if (connected) {
    ...
    // center the texture coordinates for each island for maximum 16-bit
    // precision
    ...
    for (int j = 0; j < tri->numVerts; j++) {
      ...
      tri->verts[j].SetNormal(verts[j * 8 + 5], verts[j * 8 + 6],
                              verts[j * 8 + 7]);
    }
  ...
  if (fileOut != NULL && model->SupportsBinaryModel() &&
      binaryLoadRenderModels.GetBool()) {
...
idRenderModel* idRenderWorldLocal::ParseShadowModel(idLexer* src,
  ...
  // to be consistent set the number of vertices to half the number of shadow
  // vertices
  ...
  // NOTE: we do NOT do a model->FinishSurfaceces, because we don't need sil
  // edges, planes, tangents, etc.
...
bool idRenderWorldLocal::InitFromMap(const char* name) {
  ...
  static const unsigned int BPROC_MAGIC =
      ('P' << 24) | ('R' << 16) | ('O' << 8) | BPROC_VERSION;
  bool loaded = false;
...
bool idRenderWorldLocal::CheckAreaForPortalSky(int areaNum) {
  ...
  for (areaReference_t* ref = portalAreas[areaNum].entityRefs.areaNext;
       ref->entity; ref = ref->areaNext) {
...
void idRenderWorldLocal::ResetLocalRenderModels() {
  localModels.Clear();  // Clear out the list when switching between expansion
                        // packs, so InitFromMap doesn't try to delete the list
                        // whose content has already been deleted by the model
                        // manager being re-started
}