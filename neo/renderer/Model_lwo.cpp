...
void lwFree(void* ptr) { Mem_Free(ptr); }
...
lwObject* lwGetObject(const char* filename, unsigned int* failID,
                      int* failpos) {
  idFile* fp = NULL;
  lwObject* object;
  lwLayer* layer;
...
lwObject* lwGetObject5(const char* filename, unsigned int* failID,
                       int* failpos) {
  idFile* fp = NULL;
  lwObject* object;
  lwLayer* layer;
...
int lwAllocPolygons(lwPolygonList* plist, int npols, int nverts) {
  ...
  plist->pol =
      (lwPolygon*)Mem_Alloc(plist->count * sizeof(lwPolygon), TAG_MODEL);
  if (!plist->pol) {
    return 0;
  }
  if (oldpol) {
  ...
  plist->pol[0].v =
      (lwPolVert*)Mem_Alloc(plist->vcount * sizeof(lwPolVert), TAG_MODEL);
  if (!plist->pol[0].v) {
    return 0;
  }
  if (oldpolv) {
...
int lwGetPointPolygons(lwPointList* point, lwPolygonList* polygon) {
  ...
  for (i = 0; i < point->count; i++) {
    ...
    point->pt[i].pol =
        (int*)Mem_ClearedAlloc(point->pt[i].npols * sizeof(int), TAG_MODEL);
    if (!point->pt[i].pol) {
      return 0;
    }
    point->pt[i].npols = 0;
...
int lwResolvePolySurfaces(lwPolygonList* polygon, lwTagList* tlist,
  ...
  for (i = 0; i < polygon->count; i++) {
    ...
    if (!s[index]) {
      ...
      s[index]->name =
          (char*)Mem_ClearedAlloc(strlen(tlist->tag[index]) + 1, TAG_MODEL);
      if (!s[index]->name) {
        return 0;
      }
  ...
  while (1) {
    ...
    switch (id) {
      ...
      case ID_FKEY:
        ...
        tex->param.grad.key =
            (lwGradKey*)Mem_ClearedAlloc(nkeys * sizeof(lwGradKey), TAG_MODEL);
      ...
      case ID_IKEY:
        nkeys = sz / 2;
        tex->param.grad.ikey =
            (short*)Mem_ClearedAlloc(nkeys * sizeof(short), TAG_MODEL);
...
int lwGetPointVMaps(lwPointList* point, lwVMap* vmap) {
  ...
  for (i = 0; i < point->count; i++) {
    if (point->pt[i].nvmaps) {
      point->pt[i].vm = (lwVMapPt*)Mem_ClearedAlloc(
          point->pt[i].nvmaps * sizeof(lwVMapPt), TAG_MODEL);
      if (!point->pt[i].vm) {
        return 0;
      }
      point->pt[i].nvmaps = 0;
