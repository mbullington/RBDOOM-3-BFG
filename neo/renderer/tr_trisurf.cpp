...
// Mikktspace is a standard that should be used for new assets. If you'd like to
// use the original method of calculating tangent spaces for the original game's
// normal maps, disable mikktspace before loading in the model. see
// http://www.mikktspace.com/
// idCVar r_useMikktspace( "r_useMikktspace", "1", CVAR_RENDERER | CVAR_BOOL,
// "Use the mikktspace standard to derive tangents" );

static void* mkAlloc(int bytes);
static void mkFree(void* mem);
static int mkGetNumFaces(const SMikkTSpaceContext* pContext);
static int mkGetNumVerticesOfFace(const SMikkTSpaceContext* pContext,
                                  const int iFace);
static void mkGetPosition(const SMikkTSpaceContext* pContext, float fvPosOut[],
                          const int iFace, const int iVert);
static void mkGetNormal(const SMikkTSpaceContext* pContext, float fvNormOut[],
                        const int iFace, const int iVert);
static void mkGetTexCoord(const SMikkTSpaceContext* pContext, float fvTexcOut[],
                          const int iFace, const int iVert);
static void mkSetTSpaceBasic(const SMikkTSpaceContext* pContext,
...
srfTriangles_t* R_AllocStaticTriSurf() {
  srfTriangles_t* tris =
      (srfTriangles_t*)Mem_ClearedAlloc(sizeof(srfTriangles_t), TAG_SRFTRIS);
  return tris;
}
...
void R_CreateDupVerts(srfTriangles_t* tri) {
  ...
  memcpy(tri->dupVerts, tempDupVerts.Ptr(),
...
static void R_DefineEdge(const int v1, const int v2, const int planeNum,
                         const int numPlanes, idList<silEdge_t>& silEdges,
...
void R_IdentifySilEdges(srfTriangles_t* tri, bool omitCoplanarEdges) {
    ...
    for (i = 0; i < silEdges.Num(); i++) {
      ...
      if (j == 3) {
        ...
        memmove(&silEdges[i], &silEdges[i + 1],
                (silEdges.Num() - i - 1) * sizeof(silEdges[i]));
        c_coplanarCulled++;
  ...
  memcpy(tri->silEdges, silEdges.Ptr(),
         silEdges.Num() * sizeof(tri->silEdges[0]));
}
...
void R_DeriveNormalsAndTangents(srfTriangles_t* tri) {
  ...
  for (int i = 0; i < tri->numIndexes; i += 3) {
    ...
    const float f0 = idMath::InvSqrt(normal.x * normal.x + normal.y * normal.y +
                                     normal.z * normal.z);
    ...
    *(unsigned int*)&f1 ^= signBit;
    ...
    const float f2 =
        idMath::InvSqrt(bitangent.x * bitangent.x + bitangent.y * bitangent.y +
  ...
  for (int i = 0; i < tri->numDupVerts; i++) {
    vertexNormals[tri->dupVerts[i * 2 + 0]] +=
        vertexNormals[tri->dupVerts[i * 2 + 1]];
  }
  ...
  for (int i = 0; i < tri->numVerts; i++) {
    const float normalScale =
        idMath::InvSqrt(vertexNormals[i].x * vertexNormals[i].x +
    ...
    vertexTangents[i] -=
        (vertexTangents[i] * vertexNormals[i]) * vertexNormals[i];
    vertexBitangents[i] -=
        (vertexBitangents[i] * vertexNormals[i]) * vertexNormals[i];

    const float tangentScale =
        idMath::InvSqrt(vertexTangents[i].x * vertexTangents[i].x +
                        vertexTangents[i].y * vertexTangents[i].y +
                        vertexTangents[i].z * vertexTangents[i].z);
    vertexTangents[i].x *= tangentScale;
    vertexTangents[i].y *= tangentScale;
    vertexTangents[i].z *= tangentScale;

    const float bitangentScale =
        idMath::InvSqrt(vertexBitangents[i].x * vertexBitangents[i].x +
                        vertexBitangents[i].y * vertexBitangents[i].y +
                        vertexBitangents[i].z * vertexBitangents[i].z);
...
void R_BuildDominantTris(srfTriangles_t* tri) {
  ...
  indexSort_t* ind =
      (indexSort_t*)R_StaticAlloc(numIndexes * sizeof(indexSort_t));
  if (ind == NULL) {
...
void R_TestDegenerateTextureSpace(srfTriangles_t* tri) {
  ...
  for (i = 0; i < tri->numIndexes; i += 3) {
    ...
    if (a.GetTexCoord() == b.GetTexCoord() ||
        b.GetTexCoord() == c.GetTexCoord() ||
        c.GetTexCoord() == a.GetTexCoord()) {
      c_degenerate++;
    }
    // RB end
  }
...
deformInfo_t* R_BuildDeformInfo(int numVerts, const idDrawVert* verts,
  ...
  idShadowVertSkinned::CreateShadowCache(shadowVerts, deform->verts,
                                         deform->numOutputVerts);

  deform->staticAmbientCache = vertexCache.AllocStaticVertex(
      deform->verts,
      ALIGN(deform->numOutputVerts * sizeof(idDrawVert), VERTEX_CACHE_ALIGN));
  deform->staticIndexCache = vertexCache.AllocStaticIndex(
      deform->indexes,
      ALIGN(deform->numIndexes * sizeof(triIndex_t), INDEX_CACHE_ALIGN));
  deform->staticShadowCache = vertexCache.AllocStaticVertex(
      shadowVerts,
      ALIGN(deform->numOutputVerts * 2 * sizeof(idShadowVertSkinned),
            VERTEX_CACHE_ALIGN));
...
void R_InitDrawSurfFromTri(drawSurf_t& ds, srfTriangles_t& tri) {
  ...
  // copy verts and indexes to this frame's hardware memory if they aren't
  // already there
  //
  // deformed surfaces will not have any vertices but the ambient cache will
  // have already been created for them.
...
For static surfaces, the indexes, ambient, and shadow buffers can be pre-created
at load time, rather than being re-created each frame in the frame temporary
buffers.
===================
*/
void R_CreateStaticBuffersForTri(srfTriangles_t& tri) {
  ...
  // index cache
  if (tri.indexes != NULL) {
    tri.indexCache = vertexCache.AllocStaticIndex(
        tri.indexes,
        ALIGN(tri.numIndexes * sizeof(tri.indexes[0]), INDEX_CACHE_ALIGN));
  }

  // vertex cache
  if (tri.verts != NULL) {
    tri.ambientCache = vertexCache.AllocStaticVertex(
        tri.verts,
        ALIGN(tri.numVerts * sizeof(tri.verts[0]), VERTEX_CACHE_ALIGN));
  }

  // shadow cache
  if (tri.preLightShadowVertexes != NULL) {
    // this should only be true for the _prelight<NAME> pre-calculated shadow
    // volumes
    assert(
        tri.verts ==
        NULL);  // pre-light shadow volume surfaces don't have ambient vertices
    const int shadowSize =
        ALIGN(tri.numVerts * 2 * sizeof(idShadowVert), VERTEX_CACHE_ALIGN);
    tri.shadowCache =
        vertexCache.AllocStaticVertex(tri.preLightShadowVertexes, shadowSize);
...
// SP begin
static void* mkAlloc(int bytes) { return R_StaticAlloc(bytes); }

static void mkFree(void* mem) { R_StaticFree(mem); }

static int mkGetNumFaces(const SMikkTSpaceContext* pContext) {
  srfTriangles_t* tris =
      reinterpret_cast<srfTriangles_t*>(pContext->m_pUserData);
...
idVec3 R_ClosestPointPointTriangle(const idVec3& point, const idVec3& vertex1,
  ...
  if (va <= 0.0f && (d4 - d3) >= 0.0f && (d5 - d6) >= 0.0f) {
    ...
    result =
        vertex2 + w * (vertex3 - vertex2);  // Barycentric coordinates (0,1-w,w)
    return result;
  }
