...
void idRenderSystemLocal::PrintPerformanceCounters() {
  ...
  if (r_showDynamic.GetBool()) {
    common->Printf(
        ...
        pc.c_entityDefCallbacks, pc.c_generateMd5, pc.c_deformedVerts,
        pc.c_deformedIndexes / 3, pc.c_tangentIndexes / 3, pc.c_guiSurfs);
  }
  ...
  if (r_showAddModel.GetBool()) {
    common->Printf("callback:%i createInteractions:%i createShadowVolumes:%i\n",
                   pc.c_entityDefCallbacks, pc.c_createInteractions,
                   pc.c_createShadowVolumes);
    common->Printf("viewEntities:%i  shadowEntities:%i  viewLights:%i\n",
                   pc.c_visibleViewEntities, pc.c_shadowViewEntities,
                   pc.c_viewLights);
  }
  if (r_showUpdates.GetBool()) {
    common->Printf(
        "entityUpdates:%i  entityRefs:%i  lightUpdates:%i  lightRefs:%i\n",
        pc.c_entityUpdates, pc.c_entityReferences, pc.c_lightUpdates,
        pc.c_lightReferences);
  }
  if (r_showMemory.GetBool()) {
    common->Printf("frameData: %i (%i)\n",
                   frameData->frameMemoryAllocated.GetValue(),
                   frameData->highWaterAllocated);
  }
...
void idRenderSystemLocal::RenderCommandBuffers(
    const emptyCommand_t* const cmdHead) {
  // if there isn't a draw view command, do nothing to avoid swapping a bad
  // frame
  bool hasView = false;
  for (const emptyCommand_t* cmd = cmdHead; cmd;
       cmd = (const emptyCommand_t*)cmd->next) {
    if (cmd->commandId == RC_DRAW_VIEW_3D ||
        cmd->commandId == RC_DRAW_VIEW_GUI) {
      hasView = true;
      break;
    }
  }
...
void R_AddDrawPostProcess(viewDef_t* parms) {
  postProcessCommand_t* cmd =
      (postProcessCommand_t*)R_GetCommandBuffer(sizeof(*cmd));
...
=============
*/
void idRenderSystemLocal::DrawFilled(const idVec4& color, float x, float y,
                                     float w, float h) {
  SetColor(color);
...
void idRenderSystemLocal::DrawStretchPic(float x, float y, float w, float h,
  ...
  DrawStretchPic(idVec4(x, y, s1, t1), idVec4(x + w, y, s2, t1),
                 idVec4(x + w, y + h, s2, t2), idVec4(x, y + h, s1, t2),
                 material);
}
...
void idRenderSystemLocal::DrawStretchPic(const idVec4& topLeft,
  ...
  idDrawVert* verts = guiModel->AllocTris(
      4, quadPicIndexes, 6, material, currentGLState, STEREO_DEPTH_TYPE_NONE);
  if (verts == NULL) {
    return;
  }
...
=============
*/
void idRenderSystemLocal::DrawStretchTri(const idVec2& p1, const idVec2& p2,
  ...
  idDrawVert* verts = guiModel->AllocTris(
      3, tempIndexes, 3, material, currentGLState, STEREO_DEPTH_TYPE_NONE);
  if (verts == NULL) {
    return;
  }
...
=============
*/
idDrawVert* idRenderSystemLocal::AllocTris(int numVerts,
                                           const triIndex_t* indexes,
                                           int numIndexes,
                                           const idMaterial* material,
                                           const stereoDepthType_t stereoType) {
  return guiModel->AllocTris(numVerts, indexes, numIndexes, material,
                             currentGLState, stereoType);
}
...
==================
*/
void idRenderSystemLocal::DrawSmallStringExt(int x, int y, const char* string,
...
==================
*/
void idRenderSystemLocal::DrawBigStringExt(int x, int y, const char* string,
                                           const idVec4& setColor,
                                           bool forceColor) {
  idVec4 color;
  const char* s;
...
const emptyCommand_t* idRenderSystemLocal::SwapCommandBuffers(
  ...
  SwapCommandBuffers_FinishRendering(frontEndMicroSec, backEndMicroSec,
                                     shadowMicroSec, gpuMicroSec, bc, pc);
...
void idRenderSystemLocal::SwapCommandBuffers_FinishRendering(
  ...
  if (glConfig.timerQueryAvailable) {
    ...
    if (glcontext.renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
                                                   [MRB_GPU_TIME * 2 + 1] > 0) {
      glGetQueryObjectui64vEXT(
          glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                                  [MRB_GPU_TIME * 2 + 0],
          GL_QUERY_RESULT, &gpuStartNanoseconds);
      glGetQueryObjectui64vEXT(
          glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                                  [MRB_GPU_TIME * 2 + 1],
          GL_QUERY_RESULT, &gpuEndNanoseconds);
    ...
    if (glcontext
            .renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
                                              [MRB_FILL_DEPTH_BUFFER * 2 + 1] >
        0) {
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                             [MRB_FILL_DEPTH_BUFFER * 2 + 0],
          GL_QUERY_RESULT, &gpuStartNanoseconds);
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
      ...
      backend.pc.gpuDepthMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }
    ...
    if (glcontext.renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
      ...
      backend.pc.gpuScreenSpaceAmbientOcclusionMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }

    if (glcontext.renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
                                                   [MRB_AMBIENT_PASS * 2 + 1] >
        0) {
      glGetQueryObjectui64vEXT(
          glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                                  [MRB_AMBIENT_PASS * 2 + 0],
          GL_QUERY_RESULT, &gpuStartNanoseconds);
      glGetQueryObjectui64vEXT(
          glcontext.renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                                  [MRB_AMBIENT_PASS * 2 + 1],
      ...
      backend.pc.gpuAmbientPassMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }

    if (glcontext
            .renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
                                              [MRB_DRAW_INTERACTIONS * 2 + 1] >
        0) {
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                             [MRB_DRAW_INTERACTIONS * 2 + 0],
          GL_QUERY_RESULT, &gpuStartNanoseconds);
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
      ...
      backend.pc.gpuInteractionsMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }

    if (glcontext
            .renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
                                              [MRB_DRAW_SHADER_PASSES * 2 + 1] >
        0) {
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                             [MRB_DRAW_SHADER_PASSES * 2 + 0],
          GL_QUERY_RESULT, &gpuStartNanoseconds);
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
      ...
      backend.pc.gpuShaderPassMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }
    ...
    if (glcontext.renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1]
      ...
      backend.pc.gpuPostProcessingMicroSec =
          (gpuEndNanoseconds - gpuStartNanoseconds) / 1000;
    }

// SRS - For OSX OpenGL calculate total rendering time vs direct measurement due
// to missing GL_TIMESTAMP support in Apple OpenGL 4.1
#if defined(__APPLE__)
    // SRS - On OSX gpuMicroSec starts with only a portion of the GPU rendering
    // time, must add additional components for more accurate estimate
    backend.pc.gpuMicroSec +=
        backend.pc.gpuDepthMicroSec +
        backend.pc.gpuScreenSpaceAmbientOcclusionMicroSec +
        backend.pc.gpuScreenSpaceReflectionsMicroSec +
        backend.pc.gpuAmbientPassMicroSec + backend.pc.gpuInteractionsMicroSec +
        backend.pc.gpuShaderPassMicroSec + backend.pc.gpuPostProcessingMicroSec;

    // SRS - For OSX elapsed time queries, no need to perform unnecessary calls
    // to glGetQueryObjectui64vEXT since gpuStartNanoseconds will always equal 0
    if (glcontext.renderLogMainBlockTimeQueryIssued
            [glcontext.frameParity ^ 1][MRB_FILL_GEOMETRY_BUFFER * 2 + 1] > 0) {
      glGetQueryObjectui64vEXT(
          glcontext
              .renderLogMainBlockTimeQueryIds[glcontext.frameParity ^ 1]
                                             [MRB_FILL_GEOMETRY_BUFFER * 2 + 1],
          GL_QUERY_RESULT, &gpuEndNanoseconds);
    ...
    if (glcontext.renderLogMainBlockTimeQueryIssued
            [glcontext.frameParity ^ 1][MRB_DRAW_SHADER_PASSES_POST * 2 + 1] >
        0) {
      glGetQueryObjectui64vEXT(
    ...
    for (int i = 0; i < MRB_TOTAL_QUERIES; i++) {
      glcontext
          .renderLogMainBlockTimeQueryIssued[glcontext.frameParity ^ 1][i] = 0;
    }
  }
#endif
  ...
  // RB: TODO clean up the above and just pass entire backend and performance
  // stats before they get cleared
  if (bc != NULL) {
    *bc = backend.pc;
  }
...
idRenderSystemLocal::SwapCommandBuffers_FinishCommandBuffers() {
  ...
  // RB: general GUI system path to treat ImGui surfaces in the renderer
  // frontend like SWF this calls io.RenderDrawListsFn
#if IMGUI_BFGUI
  ImGuiHook::Render();
#endif
  ...
  // allocated at the start of the buffer memory to the backEnd referenced
  // locations
  ...
  setBufferCommand_t* cmd2 =
      (setBufferCommand_t*)R_GetCommandBuffer(sizeof(*cmd2));
...
========================
*/
void idRenderSystemLocal::PerformResolutionScaling(int& newWidth,
                                                   int& newHeight) {
  float xScale = 1.0f;
  float yScale = 1.0f;
...
void idRenderSystemLocal::CaptureRenderToFile(const char* fileName,
  ...
  glReadPixels(rc.x1, rc.y1, rc.GetWidth(), rc.GetHeight(), GL_RGB,
...
===============
*/
bool idRenderSystemLocal::UploadImage(const char* imageName, const byte* data,
