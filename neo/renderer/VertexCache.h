...
// there are a lot more static indexes than vertexes, because interactions are
// just new index lists that reference existing vertexes
const int STATIC_INDEX_MEMORY = 4 * 31 * 1024 * 1024;
const int STATIC_VERTEX_MEMORY =
    4 * 31 * 1024 * 1024;  // make sure it fits in VERTCACHE_OFFSET_MASK!
...
const int VERTCACHE_FRAME_MASK =
    0x1fff;  // 13 bits = 8191 frames to wrap around

#else

// RB: original values which are still good for low spec hardware and
// performance
...
const int STATIC_VERTEX_MEMORY =
    31 * 1024 * 1024;  // make sure it fits in VERTCACHE_OFFSET_MASK!
...
class idVertexCache {
 public:
  ...
  vertCacheHandle_t AllocVertex(const void* data, int num,
                                size_t size = sizeof(idDrawVert));
  vertCacheHandle_t AllocIndex(const void* data, int num,
                               size_t size = sizeof(triIndex_t));
  vertCacheHandle_t AllocJoint(const void* data, int num,
                               size_t size = sizeof(idJointMat));
  ...
  // This can only be called by the front end, the back end should only be
  // looking at vertCacheHandle_t that are already validated.
 ...
 public:
  ...
  vertCacheHandle_t ActuallyAlloc(geoBufferSet_t& vcs, const void* data,
                                  int bytes, cacheType_t type);
};
