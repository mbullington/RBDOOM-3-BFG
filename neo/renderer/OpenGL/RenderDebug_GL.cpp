...
idCVar r_showCenterOfProjection(
    "r_showCenterOfProjection", "0", CVAR_RENDERER | CVAR_BOOL,
    "Draw a cross to show the center of projection");
idCVar r_showLines(
    "r_showLines", "0", CVAR_RENDERER | CVAR_INTEGER,
    "1 = draw alternate horizontal lines, 2 = draw alternate vertical lines");
...
static void RB_DrawText(const char* text, const idVec3& origin, float scale,
                        const idVec4& color, const idMat3& viewAxis,
                        const int align);
...
void idRenderBackend::DBG_ScanStencilBuffer() {
  ...
  stencilReadback = (byte*)R_StaticAlloc(
      renderSystem->GetWidth() * renderSystem->GetHeight(), TAG_RENDER_TOOLS);
  glReadPixels(0, 0, renderSystem->GetWidth(), renderSystem->GetHeight(),
...
void idRenderBackend::DBG_ColorByStencilBuffer() {
  ...
  static idVec3 colors[8] = {
      idVec3(0, 0, 0), idVec3(1, 0, 0), idVec3(0, 1, 0), idVec3(0, 0, 1),
      idVec3(0, 1, 1), idVec3(1, 0, 1), idVec3(1, 1, 0), idVec3(1, 1, 1),
  };
...
void idRenderBackend::DBG_ShowIntensity() {
  ...
  colorReadback = (byte*)R_StaticAlloc(
      renderSystem->GetWidth() * renderSystem->GetHeight() * 4,
      TAG_RENDER_TOOLS);
  glReadPixels(0, 0, renderSystem->GetWidth(), renderSystem->GetHeight(),
               GL_RGBA, GL_UNSIGNED_BYTE, colorReadback);
  ...
  glDrawPixels(renderSystem->GetWidth(), renderSystem->GetHeight(), GL_RGBA,
...
void idRenderBackend::DBG_ShowDepthBuffer() {
  ...
  depthReadback =
      R_StaticAlloc(renderSystem->GetWidth() * renderSystem->GetHeight() * 4,
                    TAG_RENDER_TOOLS);
  memset(depthReadback, 0,
         renderSystem->GetWidth() * renderSystem->GetHeight() * 4);
...
void idRenderBackend::DBG_ShowLightCount() {
  ...
  if (r_showLightCount.GetInteger() >= 2) {
    GL_State(GLS_DEPTHFUNC_EQUAL | GLS_STENCIL_OP_FAIL_KEEP |
...
====================
*/
void idRenderBackend::DBG_RenderDrawSurfListWithFunction(drawSurf_t** drawSurfs,
                                                         int numDrawSurfs) {
  currentSpace = NULL;
  ...
  for (int i = 0; i < numDrawSurfs; i++) {
    ...
    {
      // Set these values ahead of time so we don't have to reconstruct the
      // matrices on the consoles
    ...
    if (r_useScissor.GetBool() &&
        !currentScissor.Equals(drawSurf->scissorRect)) {
      currentScissor = drawSurf->scissorRect;
...
void idRenderBackend::DBG_ShowTris(drawSurf_t** drawSurfs, int numDrawSurfs) {
  ...
  switch (r_showTris.GetInteger()) {
    case 1:  // only draw visible ones
      GL_State(GLS_DEPTHMASK | GLS_ALPHAMASK | GLS_POLYMODE_LINE |
               GLS_POLYGON_OFFSET);
      break;
  ...
  if (r_showTris.GetInteger() == 3) {
    GL_State(
        (glStateBits & ~(GLS_CULL_MASK)) |
        GLS_CULL_TWOSIDED);  // SRS - Added parens to silence build warnings
  }
  ...
  if (r_showTris.GetInteger() == 3) {
    GL_State(
        (glStateBits & ~(GLS_CULL_MASK)) |
        GLS_CULL_FRONTSIDED);  // SRS - Added parens to silence build warnings
  }
}
...
void idRenderSystemLocal::OnFrame() {
  ...
  start = tr.primaryView->renderView.vieworg +
          tr.primaryView->renderView.viewaxis[0] * 32;
...
void idRenderBackend::DBG_ShowSurfaceInfo(drawSurf_t** drawSurfs,
                                          int numDrawSurfs) {
  ...
  tr.primaryWorld->DrawText(
      surfModelName, surfPoint + tr.primaryView->renderView.viewaxis[2] * 12,
      0.35f, colorRed, tr.primaryView->renderView.viewaxis);
  tr.primaryWorld->DrawText(surfMatName, surfPoint, 0.35f, colorBlue,
...
void idRenderBackend::DBG_ShowViewEntitys(viewEntity_t* vModels) {
  ...
  for (const viewEntity_t* vModel = vModels; vModel; vModel = vModel->next) {
    ...
    if (r_showViewEntitys.GetInteger() >= 2) {
      idVec3 corner;
      R_LocalPointToGlobal(vModel->modelMatrix, edef->localReferenceBounds[1],
                           corner);

      tr.primaryWorld->DrawText(
          va("%i:%s", edef->index, edef->parms.hModel->Name()), corner, 0.25f,
          color, tr.primaryView->renderView.viewaxis);
    }
...
=====================
*/
void idRenderBackend::DBG_ShowTexturePolarity(drawSurf_t** drawSurfs,
                                              int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
...
=====================
*/
void idRenderBackend::DBG_ShowUnsmoothedTangents(drawSurf_t** drawSurfs,
                                                 int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
  const srfTriangles_t* tri;
...
=====================
*/
void idRenderBackend::DBG_ShowTangentSpace(drawSurf_t** drawSurfs,
                                           int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
  const srfTriangles_t* tri;
...
=====================
*/
void idRenderBackend::DBG_ShowVertexColor(drawSurf_t** drawSurfs,
                                          int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
  const srfTriangles_t* tri;
...
=====================
*/
void idRenderBackend::DBG_ShowNormals(drawSurf_t** drawSurfs,
                                      int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
  idVec3 end;
    ...
    for (i = 0; i < numDrawSurfs; i++) {
      ...
      for (j = 0; j < tri->numVerts; j++) {
        ...
        R_LocalPointToGlobal(drawSurf->space->modelMatrix,
                             tri->verts[j].xyz + tangent + normal * 0.2f, pos);
        RB_DrawText(va("%d", j), pos, 0.01f, colorWhite,
                    viewDef->renderView.viewaxis, 1);
      }
...
=====================
*/
void idRenderBackend::DBG_ShowTextureVectors(drawSurf_t** drawSurfs,
                                             int numDrawSurfs) {
  ...
  for (int i = 0; i < numDrawSurfs; i++) {
    ...
    for (int j = 0; j < tri->numIndexes; j += 3) {
      ...
      const idVec3 mid =
          (a->xyz + b->xyz + c->xyz) * (1.0f / 3.0f) + 0.1f * plane.Normal();
...
=====================
*/
void idRenderBackend::DBG_ShowDominantTris(drawSurf_t** drawSurfs,
                                           int numDrawSurfs) {
  int i, j;
  drawSurf_t* drawSurf;
  const srfTriangles_t* tri;
...
view r_showLights 2	: also draw planes of each volume
...
class idSort_DebugCompareViewEnvprobe
    : public idSort_Quick<RenderEnvprobeLocal*,
                          idSort_DebugCompareViewEnvprobe> {
  idVec3 viewOrigin;

 public:
  idSort_DebugCompareViewEnvprobe(const idVec3& origin) { viewOrigin = origin; }

  int Compare(RenderEnvprobeLocal* const& a,
              RenderEnvprobeLocal* const& b) const {
...
void idRenderBackend::DBG_ShowViewEnvprobes() {
  ...
  int count = 0;
  for (viewEnvprobe_t* vProbe = viewDef->viewEnvprobes; vProbe != NULL;
       vProbe = vProbe->next) {
    count++;
    ...
    idRenderMatrix::CreateFromOriginAxis(vProbe->globalOrigin, axis,
                                         modelRenderMatrix);

    // calculate the matrix that transforms the unit cube to exactly cover the
    // model in world space
    const float size = 16.0f;
    idBounds debugBounds(idVec3(-size), idVec3(size));

    idRenderMatrix inverseBaseModelProject;
    idRenderMatrix::OffsetScaleForBounds(modelRenderMatrix, debugBounds,
                                         inverseBaseModelProject);

    idRenderMatrix invProjectMVPMatrix;
    idRenderMatrix::Multiply(viewDef->worldSpace.mvp, inverseBaseModelProject,
                             invProjectMVPMatrix);
    RB_SetMVP(invProjectMVPMatrix);
    ...
    // );
    R_GlobalPointToLocal(modelMatrix, viewDef->renderView.vieworg,
                         localViewOrigin.ToVec3());

    renderProgManager.SetUniformValue(
        RENDERPARM_LOCALVIEWORIGIN,
        localViewOrigin.ToFloatPtr());  // rpLocalViewOrigin
  ...
  if (tr.primaryWorld && r_showViewEnvprobes.GetInteger() == 3) {
    ...
    for( viewEnvprobe_t* vProbe = viewDef->viewEnvprobes; vProbe != NULL; vProbe
    = vProbe->next )
    {
    ...
    */
    ...
    for (int i = 0; i < tr.primaryWorld->envprobeDefs.Num(); i++) {
      ...
      if (vProbe) {
        ...
        if (r_useLightAreaCulling.GetBool() && vProbe->areaNum != -1 &&
            !viewDef->connectedAreas[vProbe->areaNum]) {
          continue;
        }
    ...
    idVec3 closest =
        R_ClosestPointPointTriangle(testOrigin, verts[0], verts[1], verts[2]);
    ...
    idLib::Printf("bary weights %.2f %.2f %.2f\n", barycentricWeights.x,
                  barycentricWeights.y, barycentricWeights.z);
    ...
    for (int i = 0; i < viewEnvprobes.Num() && i < 3; i++) {
      ...
      idRenderMatrix modelRenderMatrix;
      idRenderMatrix::CreateFromOriginAxis(vProbe->parms.origin, axis,
                                           modelRenderMatrix);
...
void idRenderBackend::DBG_ShowLightGrid() {
  ...
  static idVec4 colors[numColors] = {colorBrown, colorBlue,   colorCyan,
  ...
  for (int a = 0; a < tr.primaryWorld->NumAreas(); a++) {
    ...
    idVec4 lightGridOrigin(area->lightGrid.lightGridOrigin.x,
                           area->lightGrid.lightGridOrigin.y,
                           area->lightGrid.lightGridOrigin.z, 1.0f);
    idVec4 lightGridSize(area->lightGrid.lightGridSize.x,
                         area->lightGrid.lightGridSize.y,
                         area->lightGrid.lightGridSize.z, 1.0f);
    idVec4 lightGridBounds(area->lightGrid.lightGridBounds[0],
                           area->lightGrid.lightGridBounds[1],
                           area->lightGrid.lightGridBounds[2], 1.0f);
    ...
    idVec4 probeSize;
    probeSize[0] =
        area->lightGrid.imageSingleProbeSize - area->lightGrid.imageBorderSize;
    probeSize[1] = area->lightGrid.imageSingleProbeSize;
    probeSize[2] = area->lightGrid.imageBorderSize;
    probeSize[3] = float(area->lightGrid.imageSingleProbeSize -
                         area->lightGrid.imageBorderSize) /
                   area->lightGrid.imageSingleProbeSize;
    renderProgManager.SetUniformValue(
    ...
    for (int i = 0; i < area->lightGrid.lightGridPoints.Num(); i++) {
      ...
      // move center into the cube so we can void using negative results with
      // GetBaseGridCoord
      ...
      // RB: if we want to get the normals in world space so we need the model
      // -> world matrix
      idRenderMatrix modelMatrix2;
      idRenderMatrix::Transpose(*(idRenderMatrix*)modelMatrix, modelMatrix2);

      renderProgManager.SetUniformValue(RENDERPARM_MODELMATRIX_X,
                                        &modelMatrix2[0][0]);
      renderProgManager.SetUniformValue(RENDERPARM_MODELMATRIX_Y,
                                        &modelMatrix2[1][0]);
      renderProgManager.SetUniformValue(RENDERPARM_MODELMATRIX_Z,
                                        &modelMatrix2[2][0]);
      renderProgManager.SetUniformValue(RENDERPARM_MODELMATRIX_W,
                                        &modelMatrix2[3][0]);
      ...
      if (r_showLightGrid.GetInteger() == 4 ||
          !area->lightGrid.GetIrradianceImage()) {
        renderProgManager.BindShader_Color();
        ...
        idVec2i res =
            area->lightGrid.GetIrradianceImage()->GetUploadResolution();
      ...
      idRenderMatrix modelRenderMatrix;
      idRenderMatrix::CreateFromOriginAxis(gridPoint->origin, axis,
                                           modelRenderMatrix);
  ...
  if (r_showLightGrid.GetInteger() == 2) {
    // show 8 nearest grid points around the camera and illustrate how the
    // trilerping works
    ...
    // draw sample origin we want to test the grid with
    {
      ...
      idRenderMatrix modelRenderMatrix;
      idRenderMatrix::CreateFromOriginAxis(lightOrigin, axis,
                                           modelRenderMatrix);
    ...
    gridStep[2] =
        area->lightGrid.lightGridBounds[0] * area->lightGrid.lightGridBounds[1];
    ...
    for (int i = 0; i < 8; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        if (cornerOffsets[i][j] > 0.0f) {
          ...
          if (gridPointIndex2 < 0 ||
              gridPointIndex2 >= area->lightGrid.lightGridPoints.Num()) {
      ...
      idRenderMatrix modelRenderMatrix;
      idRenderMatrix::CreateFromOriginAxis(gridPoint2->origin, axis,
                                           modelRenderMatrix);
...
================
*/
void RB_AddDebugText(const char* text, const idVec3& origin, float scale,
                     const idVec4& color, const idMat3& viewAxis,
                     const int align, const int lifetime,
...
================
*/
void RB_AddDebugLine(const idVec4& color, const idVec3& start,
                     const idVec3& end, const int lifeTime,
                     const bool depthTest) {
  debugLine_t* line;
...
================
*/
void RB_AddDebugPolygon(const idVec4& color, const idWinding& winding,
...
void idRenderBackend::DBG_ShowDebugPolygons() {
  ...
  if (r_debugPolygonFilled.GetBool()) {
    GL_State(GLS_POLYGON_OFFSET | GLS_SRCBLEND_SRC_ALPHA |
             GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA | GLS_DEPTHMASK);
    GL_PolygonOffset(-1, -2);
  } else {
...
void idRenderBackend::DBG_ShowLines() {
  ...
  const int start =
      (r_showLines.GetInteger() > 2);  // 1,3 = horizontal, 2,4 = vertical
...
void idRenderBackend::DBG_TestImage() {
  ...
  if (tr.testVideo) {
    ...
    // SRS - Don't need calibrated time for testing cinematics, so just call
    // ImageForTime( 0 ) for current system time This simplification allows
    // cinematic test playback to work over both 2D and 3D background scenes
    cin = tr.testVideo->ImageForTime(
        0 /*viewDef->renderView.time[1] - tr.testVideoStartTime*/);
    if (cin.imageY != NULL) {
      image = cin.imageY;
      imageCr = cin.imageCr;
      imageCb = cin.imageCb;
    ...
    max = image->GetUploadWidth() > image->GetUploadHeight()
  ...
  float scale[16] = {0};
  scale[0] = w;  // scale
  scale[5] = h;  // scale			(SRS - changed h from -ve to +ve so video
                 // plays right side up)
  scale[12] = halfScreenWidth - (halfScreenWidth * w);  // translate
  scale[13] = halfScreenHeight - (halfScreenHeight * h) -
              h;  // translate (SRS - moved up by h)
  scale[10] = 1.0f;
  scale[15] = 1.0f;
  ...
  renderProgManager.SetRenderParms(RENDERPARM_MVPMATRIX_X, projMatrixTranspose,
                                   4);
#else
  ...
  if ((imageCr != NULL) && (imageCb != NULL)) {
    ...
    GL_SelectTexture(2);
    imageCb->Bind();
    // SRS - Use Bink shader without sRGB to linear conversion, otherwise
    // cinematic colours may be wrong BindShader_BinkGUI() does not seem to work
    // here - perhaps due to vertex shader input dependencies?
...
void idRenderBackend::DBG_ShowShadowMaps() {
  ...
  for (int i = 0; i < (r_shadowMapSplits.GetInteger() + 1); i++) {
    ...
    float projMatrixTranspose[16];
    R_MatrixTranspose(finalOrtho, projMatrixTranspose);
    renderProgManager.SetRenderParms(RENDERPARM_MVPMATRIX_X,
                                     projMatrixTranspose, 4);
    ...
    renderProgManager.SetRenderParm(
        RENDERPARM_SCREENCORRECTIONFACTOR,
...
=================
*/
static void RB_DrawExpandedTriangles(const srfTriangles_t* tri,
                                     const float radius,
...
=================
*/
void idRenderBackend::DBG_RenderDebugTools(drawSurf_t** drawSurfs,
                                           int numDrawSurfs) {
