...
====================
*/
bool idImage::IsLoaded() const { return texnum != TEXTURE_NOT_LOADED; }
...
#if !defined(USE_GLES2) && !defined(USE_GLES3)
      if (glConfig.directStateAccess) {
        glBindMultiTextureEXT(GL_TEXTURE0 + texUnit, GL_TEXTURE_CUBE_MAP,
                              texnum);
      } else
#endif
      {
...
#if !defined(USE_GLES2) && !defined(USE_GLES3)
      if (glConfig.directStateAccess) {
        glBindMultiTextureEXT(GL_TEXTURE0 + texUnit, GL_TEXTURE_2D_ARRAY,
                              texnum);
      } else
#endif
      {
...
#if !defined(USE_GLES2) && !defined(USE_GLES3)
      if (glConfig.directStateAccess) {
        glBindMultiTextureEXT(GL_TEXTURE0 + texUnit, GL_TEXTURE_2D_MULTISAMPLE,
                              texnum);
      } else
#endif
      {
...
void idImage::CopyFramebuffer(int x, int y, int imageWidth, int imageHeight) {
  ...
  switch (opts.textureType) {
    ...
    case TT_2D_MULTISAMPLE:
      ...
      break;
    default:
      // idLib::FatalError( "%s: bad texture type %d", GetName(),
      // opts.textureType );
      return;
  }
...
void idImage::CopyDepthbuffer(int x, int y, int imageWidth, int imageHeight) {
  glBindTexture(
      (opts.textureType == TT_CUBIC) ? GL_TEXTURE_CUBE_MAP : GL_TEXTURE_2D,
      texnum);

  opts.width = imageWidth;
  opts.height = imageHeight;
  glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT, x, y, imageWidth,
                   imageHeight, 0);
...
========================
*/
void idImage::SubImageUpload(int mipLevel, int x, int y, int z, int width,
                             int height, const void* pic, int pixelPitch) {
  ...
  if (IsCompressed()) {
    ...
    // make sure the pixel store alignment is correct so that lower mips get
    // created properly for odd shaped textures - this fixes the mip mapping
    // issues with fonts
    int unpackAlignment =
        width * BitsForFormat((textureFormat_t)opts.format) / 8;
    ...
    glTexSubImage2D(uploadTarget, mipLevel, x, y, width, height, dataFormat,
                    dataType, pic);
  }
...
void idImage::SetTexParameters() {
  ...
  // but doesn't save any VRAM
  /*
  if( glConfig.textureLODBiasAvailable && ( usage != TD_FONT ) )
  {
          // use a blurring LOD bias in combination with high anisotropy to fix
  our aliasing grate textures... glTexParameterf( target,
  GL_TEXTURE_LOD_BIAS_EXT, 0.5 ); //r_lodBias.GetFloat() );
  }
  */
  // RB end
...
Every image will pass through this function. Allocates all the necessary MipMap
levels for the Image, but doesn't put anything in them.
...
void idImage::AllocImage() {
  ...
  if (opts.textureType == TT_2D_ARRAY) {
    glTexImage3D(uploadTarget, 0, internalFormat, opts.width, opts.height,
                 numSides, 0, dataFormat, GL_UNSIGNED_BYTE, NULL);
  } else if (opts.textureType == TT_2D_MULTISAMPLE) {
    glTexImage2DMultisample(uploadTarget, opts.samples, internalFormat,
                            opts.width, opts.height, GL_FALSE);
  } else {
      ...
      for (int level = 0; level < opts.numLevels; level++) {
        ...
        if (IsCompressed()) {
          int compressedSize = (((w + 3) / 4) * ((h + 3) / 4) * int64(16) *
                                BitsForFormat(opts.format)) /
                               8;

          // Even though the OpenGL specification allows the 'data' pointer to
          // be NULL, for some drivers we actually need to upload data to get it
          // to allocate the texture. However, on 32-bit systems we may fail to
          // allocate a large block of memory for large textures. We handle this
          // case by using HeapAlloc directly and allowing the allocation to
          // fail in which case we simply pass down NULL to
          // glCompressedTexImage2D and hope for the best. As of 2011-10-6 using
          // NVIDIA hardware and drivers we have to allocate the memory with
          // HeapAlloc with the exact size otherwise large image allocation (for
          // instance for physical page textures) may fail on Vista 32-bit.

          // RB begin
#if defined(_WIN32)
          void* data = HeapAlloc(GetProcessHeap(), 0, compressedSize);
          glCompressedTexImage2D(uploadTarget + side, level, internalFormat, w,
                                 h, 0, compressedSize, data);
          if (data != NULL) {
...
void idImage::PurgeImage() {
  if (texnum != TEXTURE_NOT_LOADED) {
    glDeleteTextures(
        1,
        (GLuint*)&texnum);  // this should be the ONLY place it is ever called!
    texnum = TEXTURE_NOT_LOADED;
  }
