...
bool idVertexBuffer::AllocBufferObject(const void* data, int allocSize,
                                       bufferUsageType_t _usage) {
  assert(apiObject == 0xFFFF);
  assert_16_byte_aligned(data);

  if (allocSize <= 0) {
    idLib::Error("idVertexBuffer::AllocBufferObject: allocSize = %i",
                 allocSize);
  }
...
void idVertexBuffer::FreeBufferObject() {
  ...
  // if this is a sub-allocation inside a larger buffer, don't actually free
  // anything.
  if (OwnsBuffer() == false) {
    ClearWithoutFreeing();
    return;
...
========================
*/
void idVertexBuffer::Update(const void* data, int updateSize,
                            int offset) const {
  assert(apiObject != 0xFFFF);
  assert_16_byte_aligned(data);
  ...
  if (usage == BU_DYNAMIC) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, apiObject);
    glBufferSubData(GL_ARRAY_BUFFER, GetOffset() + offset,
                    (GLsizeiptrARB)numBytes, data);
  }
}
...
void* idVertexBuffer::MapBuffer(bufferMapType_t mapType) {
  ...
  if (mapType == BM_READ) {
    ...
    // RB: removed GL_MAP_INVALIDATE_RANGE_BIT as it breaks with an optimization
    // in the Nvidia WHQL drivers >= 344.11
    buffer =
        glMapBufferRange(GL_ARRAY_BUFFER, 0, GetAllocedSize(),
                         GL_MAP_WRITE_BIT /*| GL_MAP_INVALIDATE_RANGE_BIT*/ |
                             GL_MAP_UNSYNCHRONIZED_BIT);
    if (buffer != NULL) {
      buffer = (byte*)buffer + GetOffset();
    }
    // assert( IsWriteCombined( buffer ) ); // commented out because it spams
    // the console
  } else {
    assert(false);
  }
...
bool idIndexBuffer::AllocBufferObject(const void* data, int allocSize,
  ...
  if (apiObject == 0xFFFF) {
    ...
    idLib::FatalError("idIndexBuffer::AllocBufferObject: failed - GL_Error %d",
                      error);
  }
...
void idIndexBuffer::Update(const void* data, int updateSize, int offset) const {
  ...
  if (usage == BU_DYNAMIC) {
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, apiObject);
    glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, GetOffset() + offset,
                    (GLsizeiptrARB)numBytes, data);
  }
}
...
void* idIndexBuffer::MapBuffer(bufferMapType_t mapType) {
  ...
  if (mapType == BM_READ) {
    ...
    // in the Nvidia WHQL drivers >= 344.11
    buffer =
        glMapBufferRange(GL_ELEMENT_ARRAY_BUFFER, 0, GetAllocedSize(),
                         GL_MAP_WRITE_BIT /*| GL_MAP_INVALIDATE_RANGE_BIT*/ |
                             GL_MAP_UNSYNCHRONIZED_BIT);
    if (buffer != NULL) {
      buffer = (byte*)buffer + GetOffset();
...
bool idUniformBuffer::AllocBufferObject(const void* data, int allocSize,
  ...
  if (allocSize <= 0) {
    idLib::Error("idUniformBuffer::AllocBufferObject: allocSize = %i",
                 allocSize);
  }
...
========================
*/
void idUniformBuffer::Update(const void* data, int updateSize,
                             int offset) const {
  assert(apiObject != 0xFFFF);
  assert_16_byte_aligned(data);
  assert((GetOffset() & 15) == 0);
  ...
  if (usage == BU_DYNAMIC) {
    ...
    glBindBuffer(GL_ARRAY_BUFFER, apiObject);
    glBufferSubData(GL_ARRAY_BUFFER, GetOffset() + offset, (GLsizeiptr)numBytes,
                    data);
  }
}
