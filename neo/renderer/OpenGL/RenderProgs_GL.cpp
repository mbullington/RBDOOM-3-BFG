...
idCVar r_displayGLSLCompilerMessages(
    "r_displayGLSLCompilerMessages", "1", CVAR_BOOL | CVAR_ARCHIVE,
    "Show info messages the GPU driver outputs when compiling the shaders");
...
void idRenderProgManager::LoadShader(shader_t& shader) {
  ...
  switch (glConfig.driverType) {
    case GLDRV_OPENGL_MESA: {
      outFileGLSL.Format("renderprogs/glsles-3_00/%s%s", shader.name.c_str(),
                         shader.nameOutSuffix.c_str());
      outFileUniforms.Format("renderprogs/glsles-3_00/%s%s",
                             shader.name.c_str(), shader.nameOutSuffix.c_str());
      break;
    }
    ...
    case GLDRV_VULKAN: {
      outFileGLSL.Format("renderprogs/vkglsl/%s%s", shader.name.c_str(),
                         shader.nameOutSuffix.c_str());
      outFileUniforms.Format("renderprogs/vkglsl/%s%s", shader.name.c_str(),
                             shader.nameOutSuffix.c_str());
      break;
    }
    ...
    default: {
      ...
      outFileGLSL.Format("renderprogs/glsl-4_10/%s%s", shader.name.c_str(),
                         shader.nameOutSuffix.c_str());
      outFileUniforms.Format("renderprogs/glsl-4_10/%s%s", shader.name.c_str(),
                             shader.nameOutSuffix.c_str());
#else
      outFileGLSL.Format("renderprogs/glsl-4_50/%s%s", shader.name.c_str(),
                         shader.nameOutSuffix.c_str());
      outFileUniforms.Format("renderprogs/glsl-4_50/%s%s", shader.name.c_str(),
                             shader.nameOutSuffix.c_str());
#endif
    }
  }
  ...
  if ((glslFileLength <= 0) ||
      (hlslTimeStamp != FILE_NOT_FOUND_TIMESTAMP &&
       hlslTimeStamp > glslTimeStamp) ||
    ...
    programGLSL = ConvertCG2GLSL(programHLSL, inFile.c_str(), shader.stage,
                                 programUniforms, false, hasGPUSkinning,
                                 shader.vertexLayout);
    ...
    fileSystem->WriteFile(outFileUniforms, programUniforms.c_str(),
                          programUniforms.Length(), "fs_savepath");
  } else {
...

    {
      ...
      int lengthUniforms =
          fileSystem->ReadFile(outFileUniforms.c_str(), &fileBufferUniforms);
      if (lengthUniforms <= 0) {
        idLib::Error("uniform file %s could not be loaded and may be corrupt",
                     outFileUniforms.c_str());
      }
  ...
  if (shader.progId) {
    ...
    if (infologLength > 1) {
      ...
      int charsWritten = 0;
      glGetShaderInfoLog(shader.progId, infologLength, &charsWritten,
                         infoLog.Ptr());

      // catch the strings the ATI and Intel drivers output on success
      if (strstr(infoLog.Ptr(), "successfully compiled to run on hardware") !=
              NULL ||
          strstr(infoLog.Ptr(), "No errors.") != NULL) {
        // idLib::Printf( "%s program %s from %s compiled to run on hardware\n",
        // typeName, GetName(), GetFileName() );
      } else if (r_displayGLSLCompilerMessages
                     .GetBool())  // DG:  check for the CVar I added above
      {
        idLib::Printf(
            "While compiling %s program %s\n",
            (shader.stage == SHADER_STAGE_FRAGMENT) ? "fragment" : "vertex",
            inFile.c_str());
        ...
        lines.Append(source);
        for (int index = 0, ofs = lines[index].Find(separator); ofs != -1;
             index++, ofs = lines[index].Find(separator)) {
...
void idRenderProgManager::LoadGLSLProgram(const int programIndex,
  ...
  GLuint vertexProgID = (vertexShaderIndex != -1)
                            ? shaders[vertexShaderIndex].progId
                            : INVALID_PROGID;
  GLuint fragmentProgID = (fragmentShaderIndex != -1)
                              ? shaders[fragmentShaderIndex].progId
                              : INVALID_PROGID;
  ...
  if (program) {
    ...
    if (infologLength > 1) {
      ...
      if (strstr(infoLog,
              ...
              NULL ||
          strstr(infoLog, "No errors.") != NULL) {
        // idLib::Printf( "render prog %s from %s linked\n", GetName(),
        // GetFileName() );
      } else {
        idLib::Printf(
            "While linking GLSL program %d with vertexShader %s and "
            "fragmentShader %s\n",
            programIndex,
            (vertexShaderIndex >= 0) ? shaders[vertexShaderIndex].name.c_str()
                                     : "<Invalid>",
            (fragmentShaderIndex >= 0)
                ? shaders[fragmentShaderIndex].name.c_str()
                : "<Invalid>");
        idLib::Printf("%s\n", infoLog);
      }
  ...
  if (linked == GL_FALSE) {
    ...
    idLib::Error(
        "While linking GLSL program %d with vertexShader %s and fragmentShader "
        "%s\n",
        programIndex,
        (vertexShaderIndex >= 0) ? shaders[vertexShaderIndex].name.c_str()
                                 : "<Invalid>",
        (fragmentShaderIndex >= 0) ? shaders[fragmentShaderIndex].name.c_str()
                                   : "<Invalid>");
    return;
  }

  // shaders[ vertexShaderIndex ].uniformArray = glGetUniformLocation( program,
  // VERTEX_UNIFORM_ARRAY_NAME ); shaders[ fragmentShaderIndex ].uniformArray =
  // glGetUniformLocation( program, FRAGMENT_UNIFORM_ARRAY_NAME );

  if (vertexShaderIndex > -1 && shaders[vertexShaderIndex].uniforms.Num() > 0) {
    shader_t& vertexShader = shaders[vertexShaderIndex];
    vertexShader.uniformArray =
        glGetUniformLocation(program, VERTEX_UNIFORM_ARRAY_NAME);
  }

  if (fragmentShaderIndex > -1 &&
      shaders[fragmentShaderIndex].uniforms.Num() > 0) {
    shader_t& fragmentShader = shaders[fragmentShaderIndex];
    fragmentShader.uniformArray =
        glGetUniformLocation(program, FRAGMENT_UNIFORM_ARRAY_NAME);
  }

  assert(shaders[vertexShaderIndex].uniformArray != -1 ||
         vertexShaderIndex > -1 ||
         shaders[vertexShaderIndex].uniforms.Num() == 0);
  assert(shaders[fragmentShaderIndex].uniformArray != -1 ||
  ...
  // set the texture unit locations once for the render program. We only need to
  // do this once since we only link the program once
...
void idRenderProgManager::CommitUniforms(uint64 stateBits) {
  ...
  auto commitarray = [&](idVec4(&vectors)[RENDERPARM_TOTAL], shader_t& shader) {
    ...
    if (shader.uniformArray != -1 && numUniforms > 0) {
      ...
      glUniform4fv(shader.uniformArray, totalUniforms,
                   localVectors->ToFloatPtr());
    }
  };
