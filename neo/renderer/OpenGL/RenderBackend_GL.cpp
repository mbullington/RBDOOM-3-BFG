...
// SRS - Include SDL headers to enable vsync changes without restart for
// UNIX-like OSs
#if defined(__linux__) || defined(__FreeBSD__) || defined(__APPLE__)
// SRS - Don't seem to need these #undefs (at least on macOS), are they needed
// for Linux, etc? DG: SDL.h somehow needs the following functions, so #undef
// those silly
...
idCVar stereoRender_warpStrength("stereoRender_warpStrength", "1.45",
                                 CVAR_RENDERER | CVAR_ARCHIVE | CVAR_FLOAT,
                                 "amount of pre-distortion");

idCVar stereoRender_warpCenterX(
    "stereoRender_warpCenterX", "0.5",
    CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE,
    "center for left eye, right eye will be 1.0 - this");
idCVar stereoRender_warpCenterY("stereoRender_warpCenterY", "0.5",
                                CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE,
                                "center for both eyes");
idCVar stereoRender_warpParmZ("stereoRender_warpParmZ", "0",
                              CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE,
                              "development parm");
idCVar stereoRender_warpParmW("stereoRender_warpParmW", "0",
                              CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE,
                              "development parm");
idCVar stereoRender_warpTargetFraction(
    "stereoRender_warpTargetFraction", "1.0",
    CVAR_RENDERER | CVAR_FLOAT | CVAR_ARCHIVE,
    "fraction of half-width the through-lens view covers");
...
static void R_CheckPortableExtensions() {
  ...
  if (idStr::Icmpn(glConfig.vendor_string, "ATI ", 4) == 0 ||
      idStr::Icmpn(glConfig.renderer_string, "ATI ", 4) == 0 ||
    ...
    glConfig.vendor = VENDOR_AMD;
             ...
             idStr::Icmpn(glConfig.renderer_string, "NVIDIA", 6) == 0) {
    glConfig.vendor = VENDOR_NVIDIA;
  } else if (idStr::Icmpn(glConfig.vendor_string, "Intel", 5) == 0 ||
             idStr::Icmpn(glConfig.renderer_string, "Intel", 5) == 0) {
    glConfig.vendor = VENDOR_INTEL;
  }
  // SRS - Added support for Apple GPUs
  else if (idStr::Icmpn(glConfig.vendor_string, "Apple", 5) == 0 ||
           idStr::Icmpn(glConfig.renderer_string, "Apple", 5) == 0) {
  ...
  if (glConfig.driverType == GLDRV_OPENGL_MESA_CORE_PROFILE) {
    ...
    glConfig.textureCompressionAvailable =
        GLEW_ARB_texture_compression != 0 &&
        GLEW_EXT_texture_compression_s3tc != 0;
  }
  // GL_EXT_texture_filter_anisotropic
  glConfig.anisotropicFilterAvailable =
  ...
  glConfig.textureLODBiasAvailable =
      (glConfig.glVersion >= 1.4 || GLEW_EXT_texture_lod_bias != 0);
  ...
  r_useSeamlessCubeMap
      .SetModified();  // the CheckCvars() next frame will enable / disable it
  ...
  glConfig.drawElementsBaseVertexAvailable =
      GLEW_ARB_draw_elements_base_vertex != 0;
  ...
  glConfig.gpuSkinningAvailable =
      glConfig.uniformBufferAvailable &&
  ...
  glConfig.twoSidedStencilAvailable =
      (glConfig.glVersion >= 2.0f) || GLEW_ATI_separate_stencil != 0;
  ...
  // SRS - DSA not available in Apple OpenGL 4.1, but enable for OSX anyways
  // since elapsed time query will be used to get timing info instead
  glConfig.timerQueryAvailable =
      (GLEW_ARB_timer_query != 0 || GLEW_EXT_timer_query != 0);
#else
  // GL_ARB_timer_query using the DSA interface
  glConfig.timerQueryAvailable =
      (GLEW_ARB_direct_state_access != 0 && GLEW_ARB_timer_query != 0);
#endif
  ...
  if (!glConfig.textureCompressionAvailable) {
    idLib::Error(
        "GL_ARB_texture_compression or GL_EXT_texture_compression_s3tc not "
        "available");
  }
  // GL_ARB_vertex_buffer_object
...
void idRenderBackend::Init() {
  ...
  if (glConfig.extensions_string == NULL) {
    // As of OpenGL 3.2, glGetStringi is required to obtain the available
    // extensions
    // glGetStringi = ( PFNGLGETSTRINGIPROC )GLimp_ExtensionPointer(
    // "glGetStringi" );
    ...
    for (int i = 0; i < numExtensions; i++) {
      ...
      // the now deprecated glGetString method usaed to create a single string
      // with each extension separated by a space
...
void idRenderBackend::DrawElementsWithCounters(const drawSurf_t* surf) {
  ...
  const GLintptr indexOffset =
      (GLintptr)(ibHandle >> VERTCACHE_OFFSET_SHIFT) & VERTCACHE_OFFSET_MASK;
  // RB end
  ...
  if (surf->jointCache) {
    ...
    assert((jointBuffer.GetOffset() &
            (glConfig.uniformBufferOffsetAlignment - 1)) == 0);
  ...
  if (currentIndexBuffer != (GLintptr)indexBuffer->GetAPIObject() ||
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,
                 (GLintptr)indexBuffer->GetAPIObject());
    currentIndexBuffer = (GLintptr)indexBuffer->GetAPIObject();
  }

  if ((vertexLayout != LAYOUT_DRAW_VERT) ||
  ...
  glDrawElementsBaseVertex(
      GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : surf->numIndexes,
      GL_INDEX_TYPE, (triIndex_t*)indexOffset, vertOffset / sizeof(idDrawVert));
...
void idRenderBackend::GL_EndFrame() {
  // Fix for the steam overlay not showing up while in game without
  // Shell/Debug/Console/Menu also rendering
  glColorMask(1, 1, 1, 1);
  ...
  if (glConfig.syncAvailable) {
    ...
    if (glIsSync(syncToWaitOn)) {
      for (GLenum r = GL_TIMEOUT_EXPIRED; r == GL_TIMEOUT_EXPIRED;) {
        r = glClientWaitSync(syncToWaitOn, GL_SYNC_FLUSH_COMMANDS_BIT,
                             1000 * 1000);
      }
    }
  }
...
void idRenderBackend::GL_SetDefaultState() {
  ...
  // DG: deprecated in opengl 3.2 and not needed because we don't do fixed
  // function pipeline glShadeModel( GL_SMOOTH ); DG end
...
void idRenderBackend::GL_State(uint64 stateBits, bool forceGlState) {
  ...
  if (diff & (GLS_STENCIL_FUNC_BITS | GLS_STENCIL_FUNC_REF_BITS |
              GLS_STENCIL_FUNC_MASK_BITS)) {
    GLuint ref = GLuint((stateBits & GLS_STENCIL_FUNC_REF_BITS) >>
                        GLS_STENCIL_FUNC_REF_SHIFT);
    GLuint mask = GLuint((stateBits & GLS_STENCIL_FUNC_MASK_BITS) >>
  ...
  if (diff & (GLS_STENCIL_OP_FAIL_BITS | GLS_STENCIL_OP_ZFAIL_BITS |
              GLS_STENCIL_OP_PASS_BITS)) {
    GLenum sFail = 0;
    GLenum zFail = 0;
    GLenum pass = 0;
...
void idRenderBackend::DrawStencilShadowPass(const drawSurf_t* drawSurf,
  ...
  if (drawSurf->jointCache) {
    ...
    if ((vertexLayout != LAYOUT_DRAW_SHADOW_VERT_SKINNED) ||
        (currentVertexBuffer != (GLintptr)vertexBuffer->GetAPIObject()) ||
        !r_useStateCaching.GetBool()) {
      glBindBuffer(GL_ARRAY_BUFFER, (GLintptr)vertexBuffer->GetAPIObject());
      currentVertexBuffer = (GLintptr)vertexBuffer->GetAPIObject();
    ...
    if ((vertexLayout != LAYOUT_DRAW_SHADOW_VERT) ||
        (currentVertexBuffer != (GLintptr)vertexBuffer->GetAPIObject()) ||
        !r_useStateCaching.GetBool()) {
      glBindBuffer(GL_ARRAY_BUFFER, (GLintptr)vertexBuffer->GetAPIObject());
      currentVertexBuffer = (GLintptr)vertexBuffer->GetAPIObject();
...
#else
    glDrawElementsBaseVertex(
        GL_TRIANGLES, r_singleTriangle.GetBool() ? 3 : drawSurf->numIndexes,
        GL_INDEX_TYPE, (triIndex_t*)indexOffset,
...
idRenderBackend::idRenderBackend() {
  ...
  memset(glcontext.renderLogMainBlockTimeQueryIds, 0,
         sizeof(glcontext.renderLogMainBlockTimeQueryIds));
  memset(glcontext.renderLogMainBlockTimeQueryIssued, 0,
         sizeof(glcontext.renderLogMainBlockTimeQueryIssued));
}
...
void idRenderBackend::StereoRenderExecuteBackEndCommands(
  ...
  for (int i = 0; i < 2; i++) {
    if (stereoRenderImages[i] == NULL) {
      stereoRenderImages[i] = globalImages->ImageFromFunction(
          va("_stereoRender%i", i), R_MakeStereoRenderImage);
    }
    ...
    if (stereoRenderImages[i]->GetUploadWidth() != renderSystem->GetWidth() ||
      ...
      stereoRenderImages[i]->Resize(renderSystem->GetWidth(),
  ...
  // TODO: only do the copy after the final view has been rendered, not mirror
  // subviews?
  ...
  for (int stereoEye = 1; stereoEye >= -1; stereoEye -= 2) {
    ...
    // Set the back end into a known default state to fix any stale render state
    // issues
    GL_SetDefaultState();
    ...
    for (const emptyCommand_t* cmds = allCmds; cmds != NULL;
    ...
    // copy to the target
    stereoRenderImages[targetEye]->CopyFramebuffer(
        0, 0, renderSystem->GetWidth(), renderSystem->GetHeight());
  }

  // perform the final compositing / warping / deghosting to the actual
  // framebuffer(s)
  ...
  switch (renderSystem->GetStereo3DMode()) {
    ...
    case STEREO3D_SIDE_BY_SIDE:
      if (stereoRender_warp.GetBool()) {
        ...
        // renderSystem->GetWidth() / GetHeight() have returned equal values
        // (640 for initial Rift) and we are going to warp them onto a symetric
        // square region of each half of the screen

        renderProgManager.BindShader_StereoWarp();

        // clear the entire screen to black
        // we could be smart and only clear the areas we aren't going to draw
        // on, but clears are fast...
        glScissor(0, 0, glConfig.nativeScreenWidth,
        ...
        const int pixelDimensions = (glConfig.nativeScreenWidth >> 1) *
                                    stereoRender_warpTargetFraction.GetFloat();
        ...
        glScissor(0, 0, glConfig.nativeScreenWidth >> 1,
                  glConfig.nativeScreenHeight);

        idVec4 color(stereoRender_warpCenterX.GetFloat(),
                     stereoRender_warpCenterY.GetFloat(),
                     stereoRender_warpParmZ.GetFloat(),
                     stereoRender_warpParmW.GetFloat());
        ...
        idVec4 color2(stereoRender_warpCenterX.GetFloat(),
                      stereoRender_warpCenterY.GetFloat(),
                      stereoRender_warpParmZ.GetFloat(),
                      stereoRender_warpParmW.GetFloat());
        // don't use GL_Color(), because we don't want to clamp
        ...
        glViewport((glConfig.nativeScreenWidth >> 1),
                   (glConfig.nativeScreenHeight >> 1) - (pixelDimensions >> 1),
                   pixelDimensions, pixelDimensions);
        glScissor(glConfig.nativeScreenWidth >> 1, 0,
                  glConfig.nativeScreenWidth >> 1, glConfig.nativeScreenHeight);
    ...
    case STEREO3D_SIDE_BY_SIDE_COMPRESSED:
      ...
      GL_SelectTexture(1);
      stereoRenderImages[1]->Bind();
      GL_ViewportAndScissor(0, 0, renderSystem->GetWidth(),
                            renderSystem->GetHeight());
      DrawElementsWithCounters(&unitSquareSurface);
      ...
      GL_SelectTexture(1);
      stereoRenderImages[0]->Bind();
      GL_ViewportAndScissor(renderSystem->GetWidth(), 0,
    ...
    case STEREO3D_TOP_AND_BOTTOM_COMPRESSED:
      ...
      GL_SelectTexture(0);
      stereoRenderImages[0]->Bind();
      GL_ViewportAndScissor(0, renderSystem->GetHeight(),
                            renderSystem->GetWidth(),
                            renderSystem->GetHeight());
      DrawElementsWithCounters(&unitSquareSurface);
      break;
