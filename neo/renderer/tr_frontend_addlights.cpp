...
idCVar r_useAreasConnectedForShadowCulling(
    "r_useAreasConnectedForShadowCulling", "2", CVAR_RENDERER | CVAR_INTEGER,
    "cull entities cut off by doors");
idCVar r_useParallelAddLights("r_useParallelAddLights", "1",
                              CVAR_RENDERER | CVAR_BOOL,
                              "aadd all lights in parallel with jobs");

/*
============================
R_ShadowBounds

Even though the extruded shadows are drawn projected to infinity, their effects
are limited to a fraction of the light's volume.  An extruded box would require
12 faces to specify and be a lot of trouble, but an axial bounding box is quick
and easy to determine.

If the light is completely contained in the view, there is no value in trying to
cull the shadows, as they will all pass.
...
void R_ShadowBounds(const idBounds& modelBounds, const idBounds& lightBounds,
  ...
  for (int i = 0; i < 3; i++) {
    shadowBounds[0][i] = __fsels(modelBounds[0][i] - lightOrigin[i],
                                 modelBounds[0][i], lightBounds[0][i]);
    shadowBounds[1][i] = __fsels(lightOrigin[i] - modelBounds[1][i],
                                 modelBounds[1][i], lightBounds[1][i]);
  }
}
...
Sets vLight->removeFromList to true if the light should be removed from the
list. Builds a chain of entities that need to be added for shadows only off
vLight->shadowOnlyViewEntities. Allocates and fills in
vLight->entityInteractionState.
...
static void R_AddSingleLight(viewLight_t* vLight) {
  ...
  if (!r_skipSuppress.GetBool()) {
    if (light->parms.suppressLightInViewID &&
        light->parms.suppressLightInViewID == viewDef->renderView.viewID) {
      return;
    }
    if (light->parms.allowLightInViewID &&
        light->parms.allowLightInViewID != viewDef->renderView.viewID) {
      return;
    }
  }
  ...
  lightShader->EvaluateRegisters(
      lightRegs, light->parms.shaderParms, viewDef->renderView.shaderParms,
  ...
  idPlane fogPlane(
      light->baseLightProject[2][0] - light->baseLightProject[3][0],
  ...
  // copy the matrix for deforming the 'zeroOneCubeModel' to exactly cover the
  // light volume in world space
  ...
  if (r_useLightScissors.GetInteger() != 0) {
    // Calculate the matrix that projects the zero-to-one cube to exactly cover
    // the light frustum in clip space.
    idRenderMatrix invProjectMVPMatrix;
    idRenderMatrix::Multiply(viewDef->worldSpace.mvp,
                             light->inverseBaseLightProject,
                             invProjectMVPMatrix);

    // Calculate the projected bounds, either not clipped at all, near clipped,
    // or fully clipped.
    idBounds projected;
    if (r_useLightScissors.GetInteger() == 1) {
      idRenderMatrix::ProjectedBounds(projected, invProjectMVPMatrix,
                                      bounds_zeroOneCube);
      ...
      idRenderMatrix::ProjectedFullyClippedBounds(
          projected, invProjectMVPMatrix, bounds_zeroOneCube);
    }
    ...
    float screenWidth =
        (float)viewDef->viewport.x2 - (float)viewDef->viewport.x1;
    float screenHeight =
        (float)viewDef->viewport.y2 - (float)viewDef->viewport.y1;
    ...
    if (r_useShadowMapping.GetBool() && lightCastsShadows) {
      ...
      // 2048^2 ultra quality is only for cascaded shadow mapping with sun
      // lights
  ...
  // this bool array will be set true whenever the entity will visibly interact
  // with the light
  vLight->entityInteractionState =
      (byte*)R_ClearedFrameAlloc(light->world->entityDefs.Num() *
                                     sizeof(vLight->entityInteractionState[0]),
                                 FRAME_ALLOC_INTERACTION_STATE);

  idInteraction** const interactionTableRow =
      light->world->interactionTable +
      light->index * light->world->interactionTableWidth;

  for (areaReference_t* lref = light->references; lref != NULL;
       lref = lref->ownerNext) {
    portalArea_t* area = lref->area;

    // some lights have their center of projection outside the world, but
    // otherwise we want to ignore areas that are not connected to the light
    // center due to a closed door
    if (light->areaNum != -1 &&
        r_useAreasConnectedForShadowCulling.GetInteger() == 2) {
      if (!light->world->AreasAreConnected(light->areaNum, area->areaNum,
                                           PS_BLOCK_VIEW)) {
    ...
    for (areaReference_t* eref = area->entityRefs.areaNext;
         eref != &area->entityRefs; eref = eref->areaNext) {
      idRenderEntityLocal* edef = eref->entity;

      if (vLight->entityInteractionState[edef->index] !=
          viewLight_t::INTERACTION_UNCHECKED) {
        continue;
      }
      // until proven otherwise
      vLight->entityInteractionState[edef->index] = viewLight_t::INTERACTION_NO;

      // The table is updated at interaction::AllocAndLink() and
      // interaction::UnlinkAndFree()

      // TODO(Stephen): interactionTableRow is null if renderDef is used in a
      // gui.sub
      ...
      // a large fraction of static entity / light pairs will still have no
      // interactions even though they are both present in the same area(s)
      if (eModel != NULL && !eModel->IsDynamicModel() &&
          inter == INTERACTION_EMPTY) {
        // the interaction was statically checked, and it didn't generate any
        // surfaces, so there is no need to force the entity onto the view list
      ...
      // HACK: the armor-mounted flashlight is a private spot light, which is
      // probably wrong -- you would expect to see them in multiplayer.
      //	if( light->parms.allowLightInViewID && light->parms.pointLight
      //&& !eParms.weaponDepthHack )
      //	{
      //		continue;
      //	}
      ...
      // directly visible
      if ((eParms.noShadow ||
           (eModel && !eModel->ModelHasShadowCastingSurfaces())) &&
          !edef->IsDirectlyVisible()) {
        continue;
      }

      // if the model doesn't accept lighting or cast shadows, it doesn't need
      // to be added
      if (eModel && !eModel->ModelHasInteractingSurfaces() &&
      ...
      if (inter == NULL) {
        ...
        // to see if they can't possibly interact, despite sharing one or more
        // world areas
        if (R_CullModelBoundsToLight(light, edef->localReferenceBounds,
      ...
      if (edef->IsDirectlyVisible()) {
        ...
        vLight->entityInteractionState[edef->index] =
      ...
      // if we are suppressing its shadow in this view (player shadows, etc),
      // skip
      if (!r_skipSuppress.GetBool()) {
        if (eParms.suppressShadowInViewID &&
            eParms.suppressShadowInViewID == renderViewID) {
          continue;
        }
        if (eParms.suppressShadowInLightID &&
            eParms.suppressShadowInLightID == light->parms.lightId) {
          continue;
        }
      }
      ...
      R_ShadowBounds(edef->globalReferenceBounds, light->globalLightBounds,
                     light->globalLightOrigin, shadowBounds);

      // this test is pointless if we knew the light was completely contained
      // in the view frustum, but the entity would also be directly visible in
      // most of those cases.

      // this doesn't say that the shadow can't effect anything, only that it
      // can't effect anything in the view, so we shouldn't set up a view entity
      ...
      if (r_singleEntity.GetInteger() >= 0 &&
          r_singleEntity.GetInteger() != edef->index) {
        continue;
      }
      ...
      shadowOnlyEntity_t* shadEnt = (shadowOnlyEntity_t*)R_FrameAlloc(
          sizeof(shadowOnlyEntity_t), FRAME_ALLOC_SHADOW_ONLY_ENTITY);
  ...
  if (light->parms.prelightModel != NULL && !r_useShadowMapping.GetBool()) {
    ...
    shadowDrawSurf->scissorRect =
        vLight->scissorRect;  // default to the light scissor and light depth
                              // bounds
    shadowDrawSurf->shadowVolumeState =
        SHADOWVOLUME_DONE;  // assume the shadow volume is done in case
                            // r_skipPrelightShadows is set

    if (!r_skipPrelightShadows.GetBool()) {
      preLightShadowVolumeParms_t* shadowParms =
          (preLightShadowVolumeParms_t*)R_FrameAlloc(
              sizeof(shadowParms[0]), FRAME_ALLOC_SHADOW_VOLUME_PARMS);
      ...
      shadowParms->useShadowPreciseInsideTest =
      ...
      // the pre-light shadow volume "_prelight_light_3297" in "d3xpdm2" is
      // malformed in that it contains the light origin so the precise inside
      // test always fails
      if (tr.primaryWorld->mapName.IcmpPath("maps/game/mp/d3xpdm2.map") == 0 &&
          idStr::Icmp(light->parms.prelightModel->Name(),
                      "_prelight_light_3297") == 0) {
...
void R_AddLights() {
  ...
  if (r_useParallelAddLights.GetBool()) {
    for (viewLight_t* vLight = tr.viewDef->viewLights; vLight != NULL;
         vLight = vLight->next) {
  ...
  while (*ptr != NULL) {
    ...
    if (vLight->removeFromList) {
      vLight->lightDef->viewCount =
          -1;  // this probably doesn't matter with current code
      *ptr = vLight->next;
      continue;
    }
    ...
    for (shadowOnlyEntity_t* shadEnt = vLight->shadowOnlyViewEntities;
         shadEnt != NULL; shadEnt = shadEnt->next) {
      // this will add it to the viewEntities list, but with an empty scissor
      // rect
  ...
  if (r_useParallelAddShadows.GetInteger() == 1) {
    for (viewLight_t* vLight = tr.viewDef->viewLights; vLight != NULL;
         vLight = vLight->next) {
      for (preLightShadowVolumeParms_t* shadowParms =
               vLight->preLightShadowVolumes;
           shadowParms != NULL; shadowParms = shadowParms->next) {
        tr.frontEndJobList->AddJob((jobRun_t)PreLightShadowVolumeJob,
                                   shadowParms);
      }
      vLight->preLightShadowVolumes = NULL;
    }
...
void R_OptimizeViewLightsList() {
  ...
  if (r_useShadowSurfaceScissor.GetBool()) {
    ...
    for (viewLight_t* vLight = tr.viewDef->viewLights; vLight;
      ...
      for (surf = vLight->globalInteractions; surf != NULL;
           surf = surf->nextOnLight) {
        surfRect.Union(surf->scissorRect);
      }
      for (surf = vLight->localShadows; surf != NULL;
           surf = surf->nextOnLight) {
        surf->scissorRect.Intersect(surfRect);
      }

      for (surf = vLight->localInteractions; surf != NULL;
           surf = surf->nextOnLight) {
        surfRect.Union(surf->scissorRect);
      }
      for (surf = vLight->globalShadows; surf != NULL;
           surf = surf->nextOnLight) {
        surf->scissorRect.Intersect(surfRect);
      }

      for (surf = vLight->translucentInteractions; surf != NULL;
           surf = surf->nextOnLight) {
        surfRect.Union(surf->scissorRect);
      }
  ...
  // sort the viewLights list so the largest lights come first, which will
  // reduce the chance of GPU pipeline bubbles
  struct sortLight_t {
    viewLight_t* vLight;
    int screenArea;
  ...
  sortLight_t* sortLights =
      (sortLight_t*)_alloca(sizeof(sortLight_t) * numViewLights);
  ...
  qsort(sortLights, numSortLightsFilled, sizeof(sortLights[0]),
        sortLight_t::sort);
