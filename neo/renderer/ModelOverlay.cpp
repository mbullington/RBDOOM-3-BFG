...
====================
*/
static void R_OverlayPointCullStatic(byte* cullBits, halfFloat_t* texCoordS,
                                     halfFloat_t* texCoordT,
...
====================
*/
static void R_OverlayPointCullSkinned(byte* cullBits, halfFloat_t* texCoordS,
                                      halfFloat_t* texCoordT,
                                      const idPlane* planes,
                                      const idDrawVert* verts,
                                      const int numVerts,
...
void idRenderModelOverlay::CreateOverlay(const idRenderModel* model,
  ...
  maxIndexes +=
      3 * 16 /
      sizeof(
          triIndex_t);  // to allow the index size to be a multiple of 16 bytes
  ...
  for (int surfNum = 0; surfNum < model->NumBaseSurfaces(); surfNum++) {
    ...
    if (tri->staticModelWithJoints != NULL && r_useGPUSkinning.GetBool() &&
      ...
      R_OverlayPointCullSkinned(cullBits.Ptr(), texCoordS.Ptr(),
                                texCoordT.Ptr(), localTextureAxis, tri->verts,
                                tri->numVerts,
                                tri->staticModelWithJoints->jointsInverted);
    } else {
      R_OverlayPointCullStatic(cullBits.Ptr(), texCoordS.Ptr(), texCoordT.Ptr(),
                               localTextureAxis, tri->verts, tri->numVerts);
    }
    // RB end
    ...
    for (int i = 0; i < tri->numIndexes;) {
      ...
      for (; i <= nextNumIndexes; i += 3) {
        ...
        // we could do more precise triangle culling, like a light interaction
        // does, but it's not worth it
    ...
    overlay.indexes = (triIndex_t*)Mem_Alloc(
        numIndexes * sizeof(overlay.indexes[0]), TAG_MODEL);
    memcpy(overlay.indexes, overlayIndexes.Ptr(),
           numIndexes * sizeof(overlay.indexes[0]));
    overlay.numVerts = numVerts;
    overlay.verts = (overlayVertex_t*)Mem_Alloc(
        numVerts * sizeof(overlay.verts[0]), TAG_MODEL);
    memcpy(overlay.verts, overlayVerts.Ptr(),
           numVerts * sizeof(overlay.verts[0]));
...
void idRenderModelOverlay::CreateDeferredOverlays(const idRenderModel* model) {
  for (unsigned int i = firstDeferredOverlay; i < nextDeferredOverlay; i++) {
    const overlayProjectionParms_t& parms =
        deferredOverlays[i & (MAX_DEFERRED_OVERLAYS - 1)];
    if (parms.startTime >
        tr.viewDef->renderView.time[0] - DEFFERED_OVERLAY_TIMEOUT) {
...
void idRenderModelOverlay::AddDeferredOverlay(
    const overlayProjectionParms_t& localParms) {
  deferredOverlays[nextDeferredOverlay++ & (MAX_DEFERRED_OVERLAYS - 1)] =
      localParms;
...
====================
*/
static void R_CopyOverlaySurface(idDrawVert* verts, int numVerts,
                                 triIndex_t* indexes, int numIndexes,
                                 const overlay_t* overlay,
  ...
  for (int i = 0; i < overlay->numIndexes; i += 2) {
    assert(overlay->indexes[i + 0] < overlay->numVerts &&
           overlay->indexes[i + 1] < overlay->numVerts);
    WriteIndexPair(&indexes[numIndexes + i], numVerts + overlay->indexes[i + 0],
                   numVerts + overlay->indexes[i + 1]);
  }
...
====================
*/
drawSurf_t* idRenderModelOverlay::CreateOverlayDrawSurf(
    const viewEntity_t* space, const idRenderModel* baseModel,
    unsigned int index) {
  ...
  if (baseModel == NULL || baseModel->IsDefaultModel() ||
      baseModel->NumSurfaces() == 0) {
    return NULL;
  }

  assert(baseModel->IsDynamicModel() == DM_STATIC);

  const idRenderModelStatic* staticModel =
      static_cast<const idRenderModelStatic*>(baseModel);
  ...
  newTri->staticModelWithJoints =
      (staticModel->jointsInverted != NULL)
          ? const_cast<idRenderModelStatic*>(staticModel)
          : NULL;  // allow GPU skinning
  ...
  for (unsigned int i = firstOverlay; i < nextOverlay; i++) {
    ...
    const modelSurface_t* baseSurf =
        (overlay.surfaceNum < staticModel->NumSurfaces())
            ? staticModel->Surface(overlay.surfaceNum)
            : NULL;

    // if the surface ids no longer match
    if (baseSurf == NULL || baseSurf->id != overlay.surfaceId) {
      // find the surface with the correct id
      if (staticModel->FindSurfaceWithId(overlay.surfaceId,
                                         overlay.surfaceNum)) {
    ...
    if (overlay.maxReferencedVertex >= baseTri->numVerts) {
      // This can happen when playing a demofile and a model has been changed
      // since it was recorded, so just issue a warning and go on.
      common->Warning(
          "idRenderModelOverlay::CreateOverlayDrawSurf: overlay vertex out of "
          "range.  Model has probably changed since generating the overlay.");
      FreeOverlay(overlay);
      if (i == firstOverlay) {
        firstOverlay++;
      }
    ...
    R_CopyOverlaySurface(mappedVerts, numVerts, mappedIndexes, numIndexes,
                         &overlay, baseTri->verts);
  ...
  for (unsigned int i = firstOverlay; i < nextOverlay; i++) {
    ...
    if (numVerts > 0) {
      if (overlay.numVerts != numVerts) {
        ...
        overlay.numVerts = numVerts;
        overlay.verts = (overlayVertex_t*)Mem_Alloc(
            overlay.numVerts * sizeof(overlayVertex_t), TAG_MODEL);
      }
    ...
    if (numIndices > 0) {
      if (overlay.numIndexes != numIndices) {
        ...
        overlay.indexes = (triIndex_t*)Mem_Alloc(
            overlay.numIndexes * sizeof(triIndex_t), TAG_MODEL);
      }
