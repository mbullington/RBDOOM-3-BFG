...
==============================
*/
bool LoadOldGlyphData(const char* filename,
                      oldGlyphInfo_t glyphInfo[GLYPHS_PER_FONT]) {
  ...
  for (int i = 0; i < GLYPHS_PER_FONT; i++) {
    ...
    assert(glyphInfo[i].imageHeight ==
           (glyphInfo[i].t2 - glyphInfo[i].t) * 256);
...
bool idFont::LoadFont() {
  ...
  const int FONT_INFO_MAGIC =
      (FONT_INFO_VERSION | ('i' << 24) | ('d' << 16) | ('f' << 8));
  ...
  fontInfo->glyphData = (glyphInfo_t*)Mem_Alloc(
      sizeof(glyphInfo_t) * fontInfo->numGlyphs, TAG_FONT);
  fontInfo->charIndex =
      (uint32*)Mem_Alloc(sizeof(uint32) * fontInfo->numGlyphs, TAG_FONT);
  ...
  for (int i = 0; i < fontInfo->numGlyphs; i++) {
    if (fontInfo->charIndex[i] < 128) {
      ...
      // Since the characters are sorted, as soon as we find a non-ascii
  ...
  // Load the old glyph data because we want our new fonts to fit in the old
  // glyph metrics
  ...
  for (int i = 0; i < 3; i++) {
    ...
    const char* oldFileName =
        va("newfonts/%s/old_%d.dat", GetName(), pointSizes[i]);
...
float idFont::GetLineHeight(float scale) const {
  ...
  if (fontInfo != NULL) {
    return scale * Old_SelectValueForScale(scale,
                                           fontInfo->oldInfo[0].maxHeight,
                                           fontInfo->oldInfo[1].maxHeight,
                                           fontInfo->oldInfo[2].maxHeight);
  }
  return 0.0f;
}
...
float idFont::GetMaxCharWidth(float scale) const {
  ...
  if (fontInfo != NULL) {
    return scale * Old_SelectValueForScale(scale, fontInfo->oldInfo[0].maxWidth,
                                           fontInfo->oldInfo[1].maxWidth,
                                           fontInfo->oldInfo[2].maxWidth);
  }
  return 0.0f;
}
...
==============================
*/
void idFont::GetScaledGlyph(float scale, uint32 idx,
                            scaledGlyphInfo_t& glyphInfo) const {
  if (alias != NULL) {
