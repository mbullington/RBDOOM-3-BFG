...
// Pre-generated shadow volumes from dmap are not present in
// surfaceInteraction_t, they are added separately.
struct surfaceInteraction_t {
  ...
  int numShadowIndexesNoCaps;  // if the view is outside the shadow, this can be
                               // used
  triIndex_t*
      shadowIndexes;  // only != NULL if KEEP_INTERACTION_CPU_DATA is defined
...
class idInteraction {
 public:
  ...
  bool staticInteraction;  // true if the interaction was created at map load
                           // time in static buffer space
