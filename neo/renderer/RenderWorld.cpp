...
void idRenderWorldLocal::ResizeInteractionTable() {
  ...
  const int size = interactionTableWidth * interactionTableHeight *
  ...
  for (int l = 0; l < oldIinteractionTableHeight; l++) {
    for (int e = 0; e < oldInteractionTableWidth; e++) {
      interactionTable[l * interactionTableWidth + e] =
          oldInteractionTable[l * oldInteractionTableWidth + e];
    }
  }
...
void idRenderWorldLocal::UpdateEntityDef(qhandle_t entityHandle,
  ...
  if (def->parms.hModel != NULL &&
      !def->parms.hModel->ModelHasDrawingSurfaces()) {
    return;
  }
...
// RB begin
qhandle_t idRenderWorldLocal::AddEnvprobeDef(
...
=================
*/
void idRenderWorldLocal::UpdateEnvprobeDef(qhandle_t envprobeHandle,
                                           const renderEnvironmentProbe_t* ep) {
  if (r_skipUpdates.GetBool()) {
    return;
  }
  ...
  if (envprobeHandle < 0 || envprobeHandle > LUDICROUS_INDEX) {
    common->Error("idRenderWorld::UpdateEnvprobeDef: index = %i",
                  envprobeHandle);
  }
...
void idRenderWorldLocal::FreeEnvprobeDef(qhandle_t envprobeHandle) {
  ...
  if (envprobeHandle < 0 || envprobeHandle >= envprobeDefs.Num()) {
    common->Printf(
        "idRenderWorld::FreeEnvprobeDef: invalid handle %i [0, %i]\n",
...
const renderEnvironmentProbe_t* idRenderWorldLocal::GetRenderEnvprobe(
...
void idRenderWorldLocal::ProjectDecalOntoWorld(const idFixedWinding& winding,
  ...
  if (!idRenderModelDecal::CreateProjectionParms(
          globalParms, winding, projectionOrigin, parallel, fadeDepth, material,
          startTime)) {
    return;
  }
  ...
  for (int i = 0; i < numAreas; i++) {
    ...
    for (const areaReference_t* ref = area->entityRefs.areaNext;
         ref != &area->entityRefs; ref = ref->areaNext) {
      ...
      if (def->parms.callback != NULL || model == NULL ||
      ...
      idRenderModelDecal::GlobalProjectionParmsToLocal(
          localParms, globalParms, def->parms.origin, def->parms.axis);
...
void idRenderWorldLocal::ProjectDecal(
    qhandle_t entityHandle, const idFixedWinding& winding,
...
====================
*/
idRenderModelDecal* idRenderWorldLocal::AllocDecal(qhandle_t newEntityHandle,
                                                   int startTime) {
  int oldest = 0;
  ...
  if (decals[oldest].entityHandle >= 0 &&
      decals[oldest].entityHandle < entityDefs.Num()) {
...
====================
*/
idRenderModelOverlay* idRenderWorldLocal::AllocOverlay(
    qhandle_t newEntityHandle, int startTime) {
  int oldest = 0;
  int oldestTime = MAX_TYPE(oldestTime);
  for (int i = 0; i < overlays.Num(); i++) {
  ...
  if (overlays[oldest].entityHandle >= 0 &&
      overlays[oldest].entityHandle < entityDefs.Num()) {
...
void idRenderWorldLocal::RenderScene(const renderView_t* renderView) {
  ...
  // render any post processing after the view and all its subviews has been
  // draw
...
===================
*/
int idRenderWorldLocal::NumAreas() const { return numPortalAreas; }
...
===================
*/
void idRenderWorldLocal::BoundsInAreas_r(int nodeNum, const idBounds& bounds,
                                         int* areas, int* numAreas,
                                         int maxAreas) const {
  int side, i;
  areaNode_t* node;
...
===================
*/
int idRenderWorldLocal::BoundsInAreas(const idBounds& bounds, int* areas,
                                      int maxAreas) const {
  int numAreas = 0;
...
guiPoint_t idRenderWorldLocal::GuiTrace(qhandle_t entityHandle,
  ...
  for (int i = 0; i < model->NumSurfaces(); i++) {
    ...
    const idMaterial* shader = R_RemapShaderBySkin(
        surf->shader, def->parms.customSkin, def->parms.customShader);
    if (shader == NULL) {
      continue;
    }
...
===================
*/
bool idRenderWorldLocal::ModelTrace(modelTrace_t& trace, qhandle_t entityHandle,
  ...
  for (int i = 0; i < model->NumBaseSurfaces(); i++) {
    ...
    localTrace_t localTrace =
        R_LocalTrace(localStart, localEnd, radius, surf->geometry);
...
bool idRenderWorldLocal::Trace(modelTrace_t& trace, const idVec3& start,
      ...
      for (int j = 0; j < model->NumSurfaces(); j++) {
        ...
        if (skipPlayer) {
          ...
          for (int k = 0; playerMaterialExcludeList[k] != NULL; k++) {
            if (idStr::Cmp(shader->GetName(), playerMaterialExcludeList[k]) ==
                0) {
              exclude = true;
              break;
            }
...
// RB begin
void idRenderWorldLocal::AddEnvprobeRefToArea(RenderEnvprobeLocal* probe,
                                              portalArea_t* area) {
  areaReference_t* lref;
...
void idRenderWorldLocal::GenerateAllInteractions() {
  ...
  for (int i = 0; i < this->lightDefs.Num(); i++) {
    ...
    for (areaReference_t* lref = ldef->references; lref;
         lref = lref->ownerNext) {
...
void idRenderWorldLocal::PushFrustumIntoTree_r(idRenderEntityLocal* def,
                                               idRenderLightLocal* light,
                                               const frustumCorners_t& corners,
                                               int nodeNum) {
  if (nodeNum < 0) {
  ...
  if (node->commonChildrenArea != CHILDREN_HAVE_MULTIPLE_AREAS &&
      r_useNodeCommonChildren.GetBool()) {
  ...
  frustumCull_t cull =
      idRenderMatrix::CullFrustumCornersToPlane(corners, node->plane);
...
==============
*/
void idRenderWorldLocal::PushFrustumIntoTree(
    idRenderEntityLocal* def, idRenderLightLocal* light,
    const idRenderMatrix& frustumTransform, const idBounds& frustumBounds) {
  if (areaNodes == NULL) {
    return;
  }
...
// RB begin
void idRenderWorldLocal::PushEnvprobeIntoTree_r(RenderEnvprobeLocal* probe,
                                                int nodeNum) {
  if (nodeNum < 0) {
    int areaNum = -1 - nodeNum;
    portalArea_t* area = &portalAreas[areaNum];
...
====================
*/
void idRenderWorldLocal::DebugLine(const idVec4& color, const idVec3& start,
...
================
*/
void idRenderWorldLocal::DebugArrow(const idVec4& color, const idVec3& start,
                                    const idVec3& end, int size,
                                    const int lifetime) {
...
====================
*/
void idRenderWorldLocal::DebugWinding(const idVec4& color, const idWinding& w,
...
====================
*/
void idRenderWorldLocal::DebugCircle(const idVec4& color, const idVec3& origin,
                                     const idVec3& dir, const float radius,
                                     const int numSteps, const int lifetime,
                                     const bool depthTest) {
  int i;
  float a;
...
void idRenderWorldLocal::DebugSphere(const idVec4& color,
                                     const idSphere& sphere, const int lifetime,
...
void idRenderWorldLocal::DebugBounds(const idVec4& color,
                                     const idBounds& bounds, const idVec3& org,
                                     const int lifetime) {
  int i;
  idVec3 v[8];
...
============
*/
void idRenderWorldLocal::DebugCone(const idVec4& color, const idVec3& apex,
                                   const idVec3& dir, float radius1,
                                   float radius2, const int lifetime) {
  int i;
  idMat3 axis;
...
================
*/
float idRenderWorldLocal::DrawTextLength(const char* text, float scale,
                                         int len) {
...
================
*/
void idRenderWorldLocal::DrawText(const char* text, const idVec3& origin,
                                  float scale, const idVec4& color,
                                  const idMat3& viewAxis, const int align,
