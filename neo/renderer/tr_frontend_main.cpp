...
static const unsigned int MAX_FRAME_MEMORY =
    64 * 1024 * 1024;  // larger so that we can noclip on PC for dev purposes
...
void R_ToggleSmpFrame() {
  ...
  if (frameData->frameMemoryAllocated.GetValue() >
  ...
  const uintptr_t bytesNeededForAlignment =
      FRAME_ALLOC_ALIGNMENT -
      ((uintptr_t)frameData->frameMemory & (FRAME_ALLOC_ALIGNMENT - 1));
  // RB end
  ...
  // clear the command chain and make a RC_NOP command the only thing on the
  // list
  frameData->cmdHead = frameData->cmdTail = (emptyCommand_t*)R_FrameAlloc(
      sizeof(*frameData->cmdHead), FRAME_ALLOC_DRAW_COMMAND);
...
void* R_FrameAlloc(int bytes, frameAllocType_t type) {
  ...
  if (end > MAX_FRAME_MEMORY) {
    idLib::Error(
        "R_FrameAlloc ran out of memory. bytes = %d, end = %d, "
        "highWaterAllocated = %d\n",
...
static void R_SortDrawSurfs(drawSurf_t** drawSurfs, const int numDrawSurfs) {
  ...
  for (int i = 0; i < numDrawSurfs; i++) {
    ...
    if (drawSurfs[i]->frontEndGeo != NULL) {
      float min = 0.0f;
      float max = 1.0f;
      idRenderMatrix::DepthBoundsForBounds(min, max, drawSurfs[i]->space->mvp,
                                           drawSurfs[i]->frontEndGeo->bounds);
      dist = idMath::Ftoui16(min * 0xFFFF);
    }

    indices[i] = ((numDrawSurfs - i) & 0xFFFF) | (dist << 16) |
                 ((uint64)(*(uint32*)&sort) << 32);
  }
  ...
  // NOTE: not allowed in C++17 :(
  // we'll just rely on the compiler to optimize this out
  ...
  for (;;) {
    ...
    if (j - i >= 4 && level < MAX_LEVELS - 1) {
      ...
      // No need for these iterations because we are always sorting unique
      // values.
  ...
  // Add a sort offset so surfaces with equal sort orders still
  // deterministically draw in the order they were added, at least within a
  // given model.
  ...
  // sort the drawsurfs
  qsort(drawSurfs, numDrawSurfs, sizeof(drawSurfs[0]),
        local_t::R_QsortSurfaces);
...
static void R_SetupSplitFrustums(viewDef_t* viewDef) {
  ...
  const float zNearStart = (viewDef->renderView.cramZNear)
  ...
  for (int i = 1; i <= (r_shadowMapSplits.GetInteger() + 1) && i < MAX_FRUSTUMS;
       i++) {
    ...
    zFar = 1.005f * lambda * (zNearStart * powf(ratio, si)) +
           (1 - lambda) * (zNearStart + (zFarEnd - zNearStart) * si);
    ...
    idRenderMatrix::Transpose(*(idRenderMatrix*)projectionMatrix,
                              projectionRenderMatrix);
    idRenderMatrix viewRenderMatrix;
    idRenderMatrix::Transpose(
        *(idRenderMatrix*)tr.viewDef->worldSpace.modelViewMatrix,
        viewRenderMatrix);
    idRenderMatrix::Multiply(projectionRenderMatrix, viewRenderMatrix,
                             tr.viewDef->frustumMVPs[i]);

    // the planes of the view frustum are needed for portal visibility culling
    idRenderMatrix::GetFrustumPlanes(tr.viewDef->frustums[i],
                                     tr.viewDef->frustumMVPs[i], false, true);
...
class idSort_CompareEnvprobe
    : public idSort_Quick<RenderEnvprobeLocal*, idSort_CompareEnvprobe> {
  idVec3 viewOrigin;

 public:
  idSort_CompareEnvprobe(const idVec3& origin) { viewOrigin = origin; }
...
static void R_FindClosestEnvironmentProbes() {
  ...
  for (int i = 0; i < tr.primaryWorld->envprobeDefs.Num(); i++) {
    ...
    if (vProbe) {
      ...
      if (r_useLightAreaCulling.GetBool() && vProbe->areaNum != -1 &&
          !tr.viewDef->connectedAreas[vProbe->areaNum]) {
        continue;
      }
  ...
  // sort by distance
  // RB: each Doom 3 level has ~50 - 150 probes so this should be ok for each
  // frame
...
void R_RenderView(viewDef_t* parms) {
  ...
  // RB: we need a unprojection matrix to calculate the vertex position based on
  // the depth image value for some post process shaders
  R_SetupUnprojection(tr.viewDef);
  // RB end

  // setup render matrices for faster culling
  idRenderMatrix::Transpose(*(idRenderMatrix*)tr.viewDef->projectionMatrix,
                            tr.viewDef->projectionRenderMatrix);
  idRenderMatrix viewRenderMatrix;
  idRenderMatrix::Transpose(
      *(idRenderMatrix*)tr.viewDef->worldSpace.modelViewMatrix,
      viewRenderMatrix);
  idRenderMatrix::Multiply(tr.viewDef->projectionRenderMatrix, viewRenderMatrix,
                           tr.viewDef->worldSpace.mvp);

  // the planes of the view frustum are needed for portal visibility culling
  idRenderMatrix::GetFrustumPlanes(tr.viewDef->frustums[FRUSTUM_PRIMARY],
                                   tr.viewDef->worldSpace.mvp, false, true);

  // the DOOM 3 frustum planes point outside the frustum
  for (int i = 0; i < 6; i++) {
    tr.viewDef->frustums[FRUSTUM_PRIMARY][i] =
        -tr.viewDef->frustums[FRUSTUM_PRIMARY][i];
  }
  // remove the Z-near to avoid portals from being near clipped
  ...
  // make sure that interactions exist for all light / entity combinations that
  // are visible add any pre-generated light shadows, and calculate the light
  // shader values
  R_AddLights();
  ...
  // RB: find closest environment probes so we can interpolate between them in
  // the ambient shaders
...
Because R_RenderView may be called by subviews we have to make sure the post
process pass happens after the active view and its subviews is done rendering.
================
*/
