...
================
*/
idRenderModelStatic::~idRenderModelStatic() { PurgeModel(); }
...
================
*/
bool idRenderModelStatic::IsDefaultModel() const { return defaulted; }
...
================
*/
static void AddCubeFace(srfTriangles_t* tri, idVec3 v1, idVec3 v2, idVec3 v3,
                        idVec3 v4) {
...
void idRenderModelStatic::InitFromFile(const char* fileName) {
  ...
  if (sourceTimeStamp > min2016) {
    // HACK: assume this is a newer asset not by id Software and its normalmaps
    // are baked using the mikktspace standard
    useMikktspace = true;
  }
...
bool idRenderModelStatic::LoadBinaryModel(idFile* file,
                                          const ID_TIME_T sourceTimeStamp) {
  if (file == NULL) {
    return false;
  }
  ...
  // RB: source might be from .resources, so we ignore the time stamp and assume
  // a release build
  if (!fileSystem->InProductionMode() &&
      (sourceTimeStamp != FILE_NOT_FOUND_TIMESTAMP) && (sourceTimeStamp != 0) &&
    ...
    if (isGeometry) {
      ...
      if (numInFile > 0) {
        ...
        for (int j = 0; j < tri.numVerts; j++) {
          ...
          file->ReadBigArray(
              tri.verts[j].color,
              sizeof(tri.verts[j].color) / sizeof(tri.verts[j].color[0]));
          file->ReadBigArray(
              tri.verts[j].color2,
              sizeof(tri.verts[j].color2) / sizeof(tri.verts[j].color2[0]));
        }
      }
...
========================
*/
void idRenderModelStatic::WriteBinaryModel(idFile* file,
                                           ID_TIME_T* _timeStamp) const {
  if (file == NULL) {
...
// RB begin
void idRenderModelStatic::ExportOBJ(idFile* objFile, idFile* mtlFile,
  ...
  for (int i = 0; i < surfaces.Num(); i++) {
    // shadow models use numVerts but have no verts
    if ((surfaces[i].geometry != NULL) &&
        (surfaces[i].geometry->numVerts > 0) &&
        (surfaces[i].geometry->numIndexes > 0) &&
        (surfaces[i].geometry->verts != NULL)) {
      ...
      if (tri.numVerts > 0 && tri.verts != NULL) {
        for (int j = 0; j < tri.numVerts; j++) {
          objFile->Printf("v %1.6f %1.6f %1.6f\n", tri.verts[j].xyz.x,
                          tri.verts[j].xyz.y, tri.verts[j].xyz.z);
        }
        ...
        // file->WriteBigArray( tri.verts[j].normal, 4 );
        // file->WriteBigArray( tri.verts[j].tangent, 4 );
        // file->WriteBigArray( tri.verts[j].color, sizeof( tri.verts[j].color )
        // / sizeof( tri.verts[j].color[0] ) ); file->WriteBigArray(
        // tri.verts[j].color2, sizeof( tri.verts[j].color2 ) / sizeof(
        // tri.verts[j].color2[0] ) );
      }
...
================
*/
const char* idRenderModelStatic::Name() const { return name; }
...
================
*/
ID_TIME_T idRenderModelStatic::Timestamp() const { return timeStamp; }
...
================
*/
int idRenderModelStatic::NumSurfaces() const { return surfaces.Num(); }
...
srfTriangles_t* idRenderModelStatic::AllocSurfaceTriangles(
    int numVerts, int numIndexes) const {
  srfTriangles_t* tri = R_AllocStaticTriSurf();
...
================
*/
bool idRenderModelStatic::IsReloadable() const { return reloadable; }
...
================
*/
float idRenderModelStatic::DepthHack() const { return 0.0f; }
...
idRenderModel* idRenderModelStatic::InstantiateDynamicModel(
    const struct renderEntity_s* ent, const viewDef_t* view,
...
================
*/
int idRenderModelStatic::NumJoints() const { return 0; }
...
================
*/
const idMD5Joint* idRenderModelStatic::GetJoints() const { return NULL; }
...
================
*/
const idJointQuat* idRenderModelStatic::GetDefaultPose() const { return NULL; }
...
================
*/
int idRenderModelStatic::NearestJoint(int surfaceNum, int a, int b,
                                      int c) const {
  return INVALID_JOINT;
}
...
void idRenderModelStatic::FinishSurfaces(bool useMikktspace) {
  ...
  for (i = 0; i < surfaces.Num(); i++) {
    ...
    R_CleanupTriangles(surf->geometry, surf->geometry->generateNormals, true,
                       surf->shader->UseUnsmoothedTangents(), mikktspace);
...
bool idRenderModelStatic::ConvertDAEToModelSurfaces(const ColladaParser* dae) {
  ...
  for (objectNum = 0; objectNum < dae->mNodeLibrary.Num(); objectNum++) {
    object = (dae->mNodeLibrary.GetIndex(
        objectNum));  // object = ase->objects[objectNum];
    ...
    if (meshInstance->mMaterials.Num() > 0) {
      matName =
          meshInstance->mMaterials[0];  // material =
                                        // ase->materials[object->materialRef];
    ...
    idMat4 nodeTransform =
        dae->CalculateResultTransform((*object)->mTransforms);
    ...
    if (fastLoad) {
      ...
      SIMDProcessor->MinMax(mins, maxs, &(*mesh)->mPositions[0],
                            (*mesh)->mPositions.Num());
      ...
      for (j = 0; j < (*mesh)->mPositions.Num(); j++) {
        vRemap[j] =
            vertexSubset.FindVector(&(*mesh)->mPositions[0], j, vertexEpsilon);
      }
    }
#endif

    tvRemap =
        (int*)R_StaticAlloc((*mesh)->mTexCoords.Num() * sizeof(tvRemap[0]));
    ...
    if (fastLoad) {
      ...
      SIMDProcessor->MinMax(mins, maxs, &(*mesh)->mTexCoords[0],
                            (*mesh)->mTexCoords.Num());
      mins -= idVec2(expand, expand);
      maxs += idVec2(expand, expand);
      texCoordSubset.Init(mins, maxs, 32, 1024);
      for (j = 0; j < (*mesh)->mTexCoords.Num(); j++) {
        tvRemap[j] = texCoordSubset.FindVector(&(*mesh)->mTexCoords[0], j,
                                               texCoordEpsilon);
      }
    }
#endif
    ...
    mvTable = (matchVert_t*)R_ClearedStaticAlloc(
        (*mesh)->mFacePosIndices.Num() * sizeof(mvTable[0]));

    // we will have a hash chain based on the xyz values
    mvHash = (matchVert_t**)R_ClearedStaticAlloc((*mesh)->mPositions.Num() *
                                                 sizeof(mvHash[0]));
    ...
    R_AllocStaticTriSurfIndexes(
        tri, (*mesh)->mFacePosIndices.Num());  // mesh->numFaces * 3 );
    ...
    // for( j = 0; j < ( *mesh )->mFacePosIndices.Num(); j++ )
    {
      ...
      // for( k = 0; k < 3; k++ )
      {
        ...
        if (index < 0 || index >= (*mesh)->mPositions.Num()) {
          common->Error(
              "ConvertDAEToModelSurfaces: bad vertex index in DAE file %s",
              name.c_str());
        }
        ...
        if ((*mesh)->mTexCoords.Num()) {
          ...
          if (tv < 0 || tv >= (*mesh)->mTexCoords.Num()) {
            common->Error(
                "ConvertDAEToModelSurfaces: bad tex coord index in DAE file %s",
                name.c_str());
          }
        ...
        if ((*mesh)->mColors.Num() > 0) {
          color = (byte*)&(*mesh)
                      ->mColors[index];  // mesh->faces[j].vertexColors[k];
        }
#endif
    ...
    if (tri->numIndexes >
        (*mesh)->mFacePosIndices.Num())  // mesh->numFaces * 3 )
    {
      common->FatalError(
          "ConvertDAEToModelSurfaces: index miscount in DAE file %s",
          name.c_str());
    }
    if (tri->numVerts > (*mesh)->mFacePosIndices.Num())  // mesh->numFaces * 3 )
    {
      common->FatalError(
          "ConvertDAEToModelSurfaces: vertex miscount in DAE file %s",
          name.c_str());
    }
...
bool idRenderModelStatic::ConvertOBJToModelSurfaces(const objModel_t* model) {
  ...
  {
          // if we don't have any materials, dump everything into a single
  surface surf.shader = tr.defaultMaterial; surf.id = 0; this->AddSurface( surf
  ...
  */
  ...
  for (objectNum = 0; objectNum < model->objects.Num(); objectNum++) {
    ...
    if (fastLoad) {
      ...
      SIMDProcessor->MinMax(mins, maxs, &mesh->vertexes[0],
                            mesh->vertexes.Num());
      ...
      for (j = 0; j < mesh->vertexes.Num(); j++) {
        vRemap[j] =
            vertexSubset.FindVector(&mesh->vertexes[0], j, vertexEpsilon);
      }
    }
    ...
    if (fastLoad) {
      ...
      SIMDProcessor->MinMax(mins, maxs, &mesh->texcoords[0],
                            mesh->texcoords.Num());
      ...
      for (j = 0; j < mesh->texcoords.Num(); j++) {
        tvRemap[j] =
            texCoordSubset.FindVector(&mesh->texcoords[0], j, texCoordEpsilon);
      }
    }
    ...
    // the maximum possible number of combined vertexes is the number of indexes
    mvTable = (matchVert_t*)R_ClearedStaticAlloc(mesh->indexes.Num() *
                                                 sizeof(mvTable[0]));

    // we will have a hash chain based on the xyz values
    mvHash = (matchVert_t**)R_ClearedStaticAlloc(mesh->vertexes.Num() *
                                                 sizeof(mvHash[0]));
    ...
    for (j = 0; j < (mesh->indexes.Num() / 3); j++) {
      ...
      for (int k = 2; k >= 0; k--) {
        ...
        if (index < 0 || index >= mesh->vertexes.Num()) {
          common->Error(
              "ConvertOBJToModelSurfaces: bad vertex index in ASE file %s",
              name.c_str());
        }
        ...
        if (mesh->texcoords.Num() != 0) {
          ...
          if (tv < 0 || tv >= mesh->texcoords.Num()) {
            common->Error(
                "ConvertOBJToModelSurfaces: bad tex coord index in ASE file %s",
                name.c_str());
          }
          // collapse the tex coord if it was slightly offset
          tv = tvRemap[tv];
    ...
    if (tri->numIndexes > mesh->indexes.Num()) {
      common->FatalError(
          "ConvertOBJToModelSurfaces: index miscount in OBJ file %s",
          name.c_str());
    }
    if (tri->numVerts > mesh->indexes.Num()) {
      common->FatalError(
          "ConvertOBJToModelSurfaces: vertex miscount in OBJ file %s",
          name.c_str());
    }
...
bool idRenderModelStatic::ConvertASEToModelSurfaces(
  ...
  for (objectNum = 0; objectNum < ase->objects.Num(); objectNum++) {
    ...
    // im1 = declManager->FindMaterial( material->name );
    //
    //  caedes dhewm3 fix for ASE meshes without materials (a lot of Doom 3 mods
    //  have this issue) 05-18-2021
    material = (ase->materials.Num() > object->materialRef)
                   ? ase->materials[object->materialRef]
                   : NULL;
    ...
    vRemap =
        (int*)R_StaticAlloc(mesh->numVertexes * sizeof(vRemap[0]), TAG_MODEL);
    ...
    tvRemap =
        (int*)R_StaticAlloc(mesh->numTVertexes * sizeof(tvRemap[0]), TAG_MODEL);
    ...
    for (j = 0; j < tri->numVerts; j++) {
      ...
      *(unsigned*)tri->verts[j].color = *(unsigned*)mv->color;
      if (mesh->numTVFaces == mesh->numFaces && mesh->numTVertexes != 0) {
        ...
        tri->verts[j].SetTexCoord(u * textureCos + v * textureSin,
                                  u * -textureSin + v * textureCos);
      }
    }
...
bool idRenderModelStatic::ConvertLWOToModelSurfaces(
  ...
  vList =
      (idVec3*)R_StaticAlloc(layer->point.count * sizeof(vList[0]), TAG_MODEL);
  ...
  if (numTVertexes) {
    ...
    numTVertexes = 1;
    tvList =
        (idVec2*)Mem_ClearedAlloc(numTVertexes * sizeof(tvList[0]), TAG_MODEL);
  }
  ...
  vRemap =
      (int*)R_StaticAlloc(layer->point.count * sizeof(vRemap[0]), TAG_MODEL);
...
struct aseModel_s* idRenderModelStatic::ConvertLWOToASE(
    const struct st_lwObject* obj, const char* fileName) {
  int j, k;
  aseModel_t* ase;
  ...
  for (lwSurface* surf = obj->surf; surf; surf = surf->next) {
    aseMaterial_t* mat =
        (aseMaterial_t*)Mem_ClearedAlloc(sizeof(*mat), TAG_MODEL);
    ...
    aseObject_t* object =
        (aseObject_t*)Mem_ClearedAlloc(sizeof(*object), TAG_MODEL);
    ...
    mesh->vertexes = (idVec3*)Mem_Alloc(
        mesh->numVertexes * sizeof(mesh->vertexes[0]), TAG_MODEL);
    ...
    if (mesh->numTVertexes) {
      mesh->tvertexes = (idVec2*)Mem_Alloc(
          mesh->numTVertexes * sizeof(mesh->tvertexes[0]), TAG_MODEL);
      int offset = 0;
      for (lwVMap* vm = layer->vmap; vm; vm = vm->next) {
        if (vm->type == LWID_('T', 'X', 'U', 'V')) {
          vm->offset = offset;
      ...
      mesh->numTVertexes = 1;
      mesh->tvertexes = (idVec2*)Mem_ClearedAlloc(
          mesh->numTVertexes * sizeof(mesh->tvertexes[0]), TAG_MODEL);
    }
    ...
    aseFace_t* newFaces = (aseFace_t*)Mem_Alloc(
        mesh->numFaces * sizeof(mesh->faces[0]), TAG_MODEL);
...
================
*/
bool idRenderModelStatic::IsLoaded() { return !purged; }
