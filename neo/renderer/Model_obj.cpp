...
======================================================================
        ...
        Parses Wavefront export files. This parser is designed to work with
...
=================
*/
objModel_t* OBJ_Parse(const char* fileName, const char* objFileBuffer,
                      int length) {
  ...
  while (true) {
    ...
    if (token == "v") {
      ...
      common->FatalError(
          "idRenderModelStatic::ParseOBJ: Unknown or unexpected token %s",
          token.c_str());
    }
  }
...
objModel_t* OBJ_Load(const char* fileName) {
  ...
  int objBufferLen =
      fileSystem->ReadFile(fileName, (void**)&objBuffer, &timeStamp);
