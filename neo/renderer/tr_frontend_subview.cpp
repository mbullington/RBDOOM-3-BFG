...
=================
*/
static void R_MirrorPoint(const idVec3 in, orientation_t* surface,
                          orientation_t* camera, idVec3& out) {
...
=================
*/
static void R_MirrorVector(const idVec3 in, orientation_t* surface,
                           orientation_t* camera, idVec3& out) {
  out.Zero();
  for (int i = 0; i < 3; i++) {
...
bool R_PreciseCullSurface(const drawSurf_t* drawSurf, idBounds& ndcBounds) {
  ...
  for (int i = 0; i < tri->numVerts; i++) {
    const idVec3 vXYZ =
        idDrawVert::GetSkinnedDrawVertPosition(tri->verts[i], joints);

    idPlane eye, clip;
    R_TransformModelToClip(vXYZ, drawSurf->space->modelViewMatrix,
    ...
    for (int j = 0; j < 3; j++) {
      if (clip[j] >= clip[3]) {
        pointFlags |= (1 << (j * 2 + 0));
      } else if (clip[j] <= -clip[3])  // FIXME: the D3D near clip plane is at
                                       // zero instead of -1
      {
        pointFlags |= (1 << (j * 2 + 1));
      }
  ...
  R_GlobalPointToLocal(drawSurf->space->modelMatrix,
                       tr.viewDef->renderView.vieworg, localViewOrigin);

  for (int i = 0; i < tri->numIndexes; i += 3) {
    const idVec3 v1 = idDrawVert::GetSkinnedDrawVertPosition(
        tri->verts[tri->indexes[i + 0]], joints);
    const idVec3 v2 = idDrawVert::GetSkinnedDrawVertPosition(
        tri->verts[tri->indexes[i + 1]], joints);
    const idVec3 v3 = idDrawVert::GetSkinnedDrawVertPosition(
        tri->verts[tri->indexes[i + 2]], joints);
...
static viewDef_t* R_MirrorViewBySurface(const drawSurf_t* drawSurf) {
  ...
  *parms = *tr.viewDef;
  ...
  const idVec3 center =
      (drawSurf->frontEndGeo->bounds[0] + drawSurf->frontEndGeo->bounds[1]) *
      0.5f;
...
static viewDef_t* R_PortalViewBySurface(const drawSurf_t* surf) {
  ...
  *parms = *tr.viewDef;
  ...
  idMat3 remoteViewAxis =
      surf->space->entityDef->parms.remoteRenderView->viewaxis;
  ...
  idVec3 dirToPortal =
      (surf->space->entityDef->parms.origin - orig) * inverseSurfView;
  ...
  parms->clipPlanes[0][3] =
      -(surf->space->entityDef->parms.remoteRenderView->vieworg *
        parms->clipPlanes[0].Normal());
...
=================
*/
void R_MirrorRender(const drawSurf_t* surf, textureStage_t* stage,
...
=================
*/
void R_XrayRender(const drawSurf_t* surf, textureStage_t* stage,
                  idScreenRect scissor) {
  // remote views can be reused in a single frame
  if (stage->dynamicFrameCount == tr.frameCount) {
    return;
...
bool R_GenerateSurfaceSubview(const drawSurf_t* drawSurf) {
  ...
  idScreenRect scissor;
  scissor.x1 = v->x1 + idMath::Ftoi((v->x2 - v->x1 + 1) * 0.5f *
                                    (ndcBounds[0][0] + 1.0f));
  scissor.y1 = v->y1 + idMath::Ftoi((v->y2 - v->y1 + 1) * 0.5f *
                                    (ndcBounds[0][1] + 1.0f));
  scissor.x2 = v->x1 + idMath::Ftoi((v->x2 - v->x1 + 1) * 0.5f *
                                    (ndcBounds[1][0] + 1.0f));
  scissor.y2 = v->y1 + idMath::Ftoi((v->y2 - v->y1 + 1) * 0.5f *
                                    (ndcBounds[1][1] + 1.0f));
