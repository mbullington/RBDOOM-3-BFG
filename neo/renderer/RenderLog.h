...
// these are used to make sure each Indent() is properly paired with an
// Outdent()
...
// using this macro avoids printf parameter overhead if the renderlog isn't
// active
#define RENDERLOG_PRINTF(...) \
  if (renderLog.activeLevel) renderLog.Printf(__VA_ARGS__);
