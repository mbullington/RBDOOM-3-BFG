...
========================
*/
void idColorSpace::ConvertRGBToYCoCg(byte* dst, const byte* src, int width,
                                     int height) {
...
========================
*/
void idColorSpace::ConvertYCoCgToRGB(byte* dst, const byte* src, int width,
                                     int height) {
  for (int i = 0; i < width * height; i++) {
    int y = src[i * 4 + 0];
...
========================
*/
void idColorSpace::ConvertRGBToCoCg_Y(byte* dst, const byte* src, int width,
                                      int height) {
  for (int i = 0; i < width * height; i++) {
    int r = src[i * 4 + 0];
    int g = src[i * 4 + 1];
...
========================
*/
void idColorSpace::ConvertCoCg_YToRGB(byte* dst, const byte* src, int width,
                                      int height) {
  for (int i = 0; i < width * height; i++) {
...
========================
*/
void idColorSpace::ConvertCoCgSYToRGB(byte* dst, const byte* src, int width,
                                      int height) {
  for (int i = 0; i < width * height; i++) {
    int co = src[i * 4 + 0] - 128;
    int cg = src[i * 4 + 1] - 128;
...
========================
*/
void idColorSpace::ConvertRGBToYCoCg420(byte* dst, const byte* src, int width,
                                        int height) {
  int numSamples = 0;
...
========================
*/
void idColorSpace::ConvertYCoCg420ToRGB(byte* dst, const byte* src, int width,
                                        int height) {
...
CCIR 601-1, except that Cb and Cr are normalized to the range 0 -> 255 rather
than -0.5 -> 0.5. The conversion equations to be implemented are therefore:
...
These numbers are derived from TIFF 6.0 section 21, dated 3-June-92. To avoid
floating-point arithmetic, we represent the fractional constants as integers
scaled up by 2^16 (about 4 digits precision); we have to divide the products by
2^16, with appropriate rounding, to get the correct answer.
================================================================================================
*/
...
#define RGB_TO_YCBCR_Y(r, g, b) \
  (((r * r029900 + g * g058700 + b * b011400) + ycbcr_round) >> ycbcr_shift)
#define RGB_TO_YCBCR_CB(r, g, b) \
  (((-r * r016874 - g * g033126 + b * b050000) + ycbcr_round) >> ycbcr_shift)
#define RGB_TO_YCBCR_CR(r, g, b) \
  (((r * r050000 - g * g041869 - b * b008131) + ycbcr_round) >> ycbcr_shift)

#define CBCR_TO_R(cb, cr) ((ycbcr_round + cr * r140200) >> ycbcr_shift)
#define CBCR_TO_G(cb, cr) \
  ((ycbcr_round - cb * g034414 - cr * g071414) >> ycbcr_shift)
...
========================
*/
void idColorSpace::ConvertRGBToYCbCr(byte* dst, const byte* src, int width,
                                     int height) {
  for (int i = 0; i < width * height; i++) {
    int r = src[i * 4 + 0];
    int g = src[i * 4 + 1];
...
========================
*/
void idColorSpace::ConvertYCbCrToRGB(byte* dst, const byte* src, int width,
                                     int height) {
  for (int i = 0; i < width * height; i++) {
    int y = src[i * 4 + 0];
...
========================
*/
void idColorSpace::ConvertRGBToCbCr_Y(byte* dst, const byte* src, int width,
                                      int height) {
  for (int i = 0; i < width * height; i++) {
    int r = src[i * 4 + 0];
    int g = src[i * 4 + 1];
...
========================
*/
void idColorSpace::ConvertCbCr_YToRGB(byte* dst, const byte* src, int width,
                                      int height) {
  for (int i = 0; i < width * height; i++) {
...
========================
*/
void idColorSpace::ConvertRGBToYCbCr420(byte* dst, const byte* src, int width,
                                        int height) {
  int numSamples = 0;
  for (int j = 0; j < height; j += 2) {
    for (int i = 0; i < width; i += 2) {
...
========================
*/
void idColorSpace::ConvertYCbCr420ToRGB(byte* dst, const byte* src, int width,
                                        int height) {
  int numSamples = width * height * 2 - width;
  for (int j = height - 2; j >= 0; j -= 2) {
    for (int i = width - 2; i >= 0; i -= 2) {
...
The iterative algorithm is pretty crappy but it's reasonably fast and good
enough for testing purposes. The algorithm uses a stereographic projection of
the normals to reduce the entropy and preserve significantly more detail.
...
Where: w = width, h = height, H(i,j) = height, Nx(i,j) = (normal.x/(1+normal.z),
Ny(i,j) = (normal.y/(1+normal.z) The c(i,j) are setup such that:
...
========================
*/
void idColorSpace::ConvertNormalMapToStereographicHeightMap(
    byte* heightMap, const byte* normalMap, int width, int height,
    float& scale) {
  ...
  for (int n = 0; n < NUM_ITERATIONS; n++) {
    for (int i = 0; i < height; i++) {
      for (int j = 1; j < width; j++) {
        float x = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j + 0)) * 4 + 0]);
        float z = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j + 0)) * 4 + 2]);
        temp[i * width + j] =
            scale0 * temp[i * width + j] +
            scale1 * (temp[(i + 0) * width + (j - 1)] - (x / (1 + z)));
      }
    }
    for (int i = 1; i < height; i++) {
      for (int j = 0; j < width; j++) {
        float y = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j + 0)) * 4 + 1]);
        float z = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j + 0)) * 4 + 2]);
        temp[i * width + j] =
            scale0 * temp[i * width + j] +
            scale1 * (temp[(i - 1) * width + (j + 0)] - (y / (1 + z)));
      }
    }
    for (int i = 0; i < height; i++) {
      for (int j = width - 1; j > 0; j--) {
        float x = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j - 1)) * 4 + 0]);
        float z = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i + 0) * width + (j - 1)) * 4 + 2]);
        temp[i * width + (j - 1)] =
            scale0 * temp[i * width + (j - 1)] +
            scale1 * (temp[(i + 0) * width + (j + 0)] + (x / (1 + z)));
      }
    }
    for (int i = height - 1; i > 0; i--) {
      for (int j = 0; j < width; j++) {
        float y = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i - 1) * width + (j + 0)) * 4 + 1]);
        float z = NORMALMAP_BYTE_TO_FLOAT(
            normalMap[((i - 1) * width + (j + 0)) * 4 + 2]);
        temp[(i - 1) * width + j] =
            scale0 * temp[(i - 1) * width + j] +
            scale1 * (temp[(i + 0) * width + (j + 0)] + (y / (1 + z)));
      }
    }
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width; i++) {
      heightMap[j * width + i] =
          idMath::Ftob((temp[j * width + i] - minHeight) * s);
    }
  }
}
...
This converts a heightmap of a stereographically projected normal map back into
a regular normal map.
========================
*/
void idColorSpace::ConvertStereographicHeightMapToNormalMap(
    byte* normalMap, const byte* heightMap, int width, int height,
    float scale) {
  for (int i = 0; i < height; i++) {
    int previ = Max(i, 0);
    ...
    for (int j = 0; j < width; j++) {
      ...
      idVec3 normal;
      float pX = scale *
                 (heightMap[i * width + prevj] - heightMap[i * width + nextj]) /
                 255.0f;
      float pY = scale *
                 (heightMap[previ * width + j] - heightMap[nexti * width + j]) /
                 255.0f;
...
========================
*/
void idColorSpace::ConvertRGBToMonochrome(byte* mono, const byte* rgb,
                                          int width, int height) {
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
      mono[i * width + j] =
          (rgb[(i * width + j) * 4 + 0] + rgb[(i * width + j) * 4 + 1] +
           rgb[(i * width + j) * 4 + 2]) /
          3;
    }
  }
...
========================
*/
void idColorSpace::ConvertMonochromeToRGB(byte* rgb, const byte* mono,
                                          int width, int height) {
  for (int i = 0; i < height; i++) {
    for (int j = 0; j < width; j++) {
