...
===============
*/
static void AddTriListToArea(uEntity_t* e, mapTri_t* triList, int planeNum,
                             int areaNum, textureVectors_t* texVec) {
  uArea_t* area;
...
static void TexVecForTri(textureVectors_t* texVec, mapTri_t* tri) {
  ...
  texVec->v[0][3] =
      tri->v[0].xyz * texVec->v[0].ToVec3() - tri->v[0].GetTexCoord().x;
  ...
  texVec->v[1][3] =
      tri->v[0].xyz * texVec->v[0].ToVec3() - tri->v[0].GetTexCoord().y;
  // RB end
}
  ...
  if (1) {
    ...
    for (i = 0; i < w->GetNumPoints(); i++) {
      ...
      for (j = 0; j < 3; j++) {
        ...
        // copy normal
        dv->SetNormal(dmapGlobals.mapPlanes[s->planenum].Normal());
        if (dv->GetNormal().Length() < 0.9f ||
            dv->GetNormal().Length() > 1.1f) {
          common->Error("Bad normal in TriListForSide");
        }
      }
    }
...
====================
*/
void ClipTriIntoTree_r(idWinding* w, mapTri_t* originalTri, uEntity_t* e,
                       node_t* node) {
  idWinding *front, *back;
  ...
  if (node->planenum != PLANENUM_LEAF) {
    // common->Printf( "ClipTriIntoTree_r: splitting triangle with splitplane
    // %i\n", node->nodeNumber );
  ...
  // common->Printf( "ClipTriIntoTree_r: leaf area = %i, opaque = %i, occupied =
...
====================
*/
static void PutWindingIntoAreas_r(uEntity_t* e, const idWinding* w,
                                  side_t* side, node_t* node) {
  idWinding *front, *back;
  int area;
      ...
      for (i = 0; i < model->NumSurfaces(); i++) {
        ...
        for (int j = 0; j < tri->numIndexes; j += 3) {
          for (int k = 0; k < 3; k++) {
            ...
            mapTri.v[k].SetNormal(tri->verts[tri->indexes[j + k]].GetNormal() *
                                  axis);
            mapTri.v[k].SetTexCoord(
                tri->verts[tri->indexes[j + k]].GetTexCoord());
          }
