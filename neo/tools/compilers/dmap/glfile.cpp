...
void OutputNode(const node_t* node, idList<OBJGroup>& groups) {
  ...
  if (node->planenum == PLANENUM_LEAF) {
    if (node->occupied) {
      group = &groups.Alloc();
      group->name.Format("area%i_leaf_occupied.%i", node->area,
                         node->nodeNumber);
    } else if (node->opaque) {
      group = &groups.Alloc();

      if (node->area != -1) {
        group->name.Format("area%i_leaf_opaque.%i", node->area,
                           node->nodeNumber);
      } else {
...
void WriteGLView(bspface_t* list, const char* source) {
  ...
  idStrStatic<MAX_OSPATH> path;
  path.Format("%s_BSP_%s_%i.obj", dmapGlobals.mapFileBase, source,
