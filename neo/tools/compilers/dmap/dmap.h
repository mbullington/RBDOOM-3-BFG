...
typedef struct mapTri_s {
  ...
  // RB begin
  int polygonId;  // n-gon number from original face used for area portal
                  // construction

  const MapPolygonMesh* originalMapMesh;
  //	idWinding* 			visibleHull;	// also clipped to the solid
  //parts of the world
...
typedef struct {
  ...
  idPlane
      frustumPlanes[6];  // RB: should be calculated after R_DeriveLightData()
} mapLight_t;
...
void SplitBrush(uBrush_t* brush, int planenum, uBrush_t** front,
                uBrush_t** back);
node_t* AllocNode();
...
void WriteGLView(tree_t* tree, const char* source, int entityNum,
                 bool force = false);
