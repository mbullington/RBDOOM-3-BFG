...
static void AdjustEntityForOrigin( uEntity_t* ent )
{
        ...
        for( prim = ent->primitives ; prim ; prim = prim->next )
        {
                ...
                for( i = 0; i < b->numsides; i++ )
                {
                        ...
                        s->texVec.v[0][3] += DotProduct( ent->origin,
s->texVec.v[0] ); s->texVec.v[1][3] += DotProduct( ent->origin, s->texVec.v[1]
);
...
static void ParseBrush(const idMapBrush* mapBrush, int primitiveNum) {
  ...
  for (i = 0; i < mapBrush->GetNumSides(); i++) {
    ...
    s->texValve220 =
        (ms->GetProjectionType() == idMapBrushSide::PROJECTION_VALVE220);
...
================
*/
static void ParseSurface(const idMapPatch* patch, const idSurface* surface,
...
// RB begin
static int ParsePolygonMesh(const MapPolygonMesh* mesh, int primitiveNum,
                            int numPolygons) {
...
static bool ProcessMapEntity(idMapEntity* mapEnt) {
  ...
  for (entityPrimitive = 0; entityPrimitive < mapEnt->GetNumPrimitives();
    ...
    // RB begin
      ...
      numPolygons += ParsePolygonMesh(static_cast<MapPolygonMesh*>(prim),
                                      entityPrimitive, numPolygons);
    }
    // RB end
  }
...
static void CreateMapLight(const idMapEntity* mapEnt) {
  ...
  // RB begin
  idRenderMatrix::GetFrustumPlanes(light->frustumPlanes,
                                   light->def.baseLightProject, true, true);
