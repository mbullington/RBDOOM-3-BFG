...
============
*/
idAASBuild::~idAASBuild() { Shutdown(); }
...
void idAASBuild::ParseProcNodes(idLexer* src) {
  ...
  idAASBuild::procNodes = (aasProcNode_t*)Mem_ClearedAlloc(
      idAASBuild::numProcNodes * sizeof(aasProcNode_t), TAG_TOOLS);
...
============
*/
bool idAASBuild::ChoppedAwayByProcBSP(int nodeNum, idFixedWinding* w,
                                      const idVec3& normal,
                                      const idVec3& origin,
                                      const float radius) {
  int res;
...
idBrushList idAASBuild::AddBrushesForMapBrush(const idMapBrush* mapBrush,
                                              const idVec3& origin,
                                              const idMat3& axis, int entityNum,
                                              int primitiveNum,
