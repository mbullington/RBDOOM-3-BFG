...
================
*/
bool idAASBuild::GetEdge(const idVec3& v1, const idVec3& v2, int* edgeNum,
                         int v1num) {
  int v2num, hashKey, e;
  int* vertexNum;
  aasEdge_t edge;
...
================
*/
bool idAASBuild::GetFaceForPortal(idBrushBSPPortal* portal, int side,
                                  int* faceNum) {
  int i, j, v1num;
  ...
  for (i = 0; i < w->GetNumPoints(); i++) {
    ...
    if (faceEdges[numFaceEdges]) {
      ...
      v1num = file->edges[abs(faceEdges[numFaceEdges])]
                  .vertexNum[INT32_SIGNBITNOTSET(faceEdges[numFaceEdges])];
...
void idAASBuild::GetSizeEstimate_r(idBrushBSPNode* parent, idBrushBSPNode* node,
