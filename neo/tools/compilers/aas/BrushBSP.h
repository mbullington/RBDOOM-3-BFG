...
class idBrushBSPPortal {
 ...
 public:
  ...
  void Flip();
  int Split(const idPlane& splitPlane, idBrushBSPPortal** front,
...
class idBrushBSP {
 public:
  ...
  bool RemoveOutside(const idMapFile* mapFile, int contents,
 ...
 private:
  ...
  int BrushSplitterStats(const idBrush* brush, int planeNum,
                         const idPlaneSet& planeList, bool* testedPlanes,
                         struct splitterStats_s& stats);
  int FindSplitter(idBrushBSPNode* node, const idPlaneSet& planeList,
                   bool* testedPlanes, struct splitterStats_s& bestStats);
  ...
  idBrushBSPNode* BuildBrushBSP_r(idBrushBSPNode* node,
                                  ...
                                  bool* testedPlanes, int skipContents);
  ...
  bool FloodFromEntities(const idMapFile* mapFile, int contents,
                         const idStrList& classNames);
  ...
  void UpdateTreeAfterMerge_r(idBrushBSPNode* node, const idBounds& bounds,
                              idBrushBSPNode* oldNode, idBrushBSPNode* newNode);
  void RemoveLeafNodeColinearPoints(idBrushBSPNode* node);
  void RemoveColinearPoints_r(idBrushBSPNode* node, int skipContents);
  void MeltFlood_r(idBrushBSPNode* node, int skipContents, idBounds& bounds,
                   idVectorSet<idVec3, 3>& vertexList);
  void MeltLeafNodePortals(idBrushBSPNode* node, int skipContents,
                           idVectorSet<idVec3, 3>& vertexList);
  void MeltPortals_r(idBrushBSPNode* node, int skipContents,
                     idVectorSet<idVec3, 3>& vertexList);
};
