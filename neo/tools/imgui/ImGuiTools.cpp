...
// things in impl need to be used in at least one other file, but should
// generally not be touched
namespace impl {

void SetReleaseToolMouse(bool doRelease) { releaseMouse = doRelease; }

}  // namespace impl

bool AreEditorsActive() {
  // FIXME: this is not exactly clean and must be changed if we ever support
  // game dlls
  return g_editEntityMode.GetInteger() > 0;
}

bool ReleaseMouseForTools() { return AreEditorsActive() && releaseMouse; }
...
void LightEditorInit(const idDict* dict, idEntity* ent) {
  ...
  // be game/mod specific. but we can at least check the spawnclass from the
  // dict.
  idassert(
      idStr::Icmp(dict->GetString("spawnclass"), "idLight") == 0 &&
