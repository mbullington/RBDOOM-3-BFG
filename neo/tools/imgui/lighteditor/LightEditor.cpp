...
void LightInfo::FromDict(const idDict* e) {
  ...
  if (e->GetVector("light_right", "", lightRight)) {
    ...
    if (e->GetVector("light_radius", "", lightRadius)) {
      equalRadius =
          (lightRadius.x == lightRadius.y && lightRadius.x == lightRadius.z);
    } else {
...
// the returned idDict is supposed to be used by
// idGameEdit::EntityChangeSpawnArgs() and thus will contain pairs with value ""
// if the key should be removed from entity
void LightInfo::ToDict(idDict* e) {
  ...
  e->Set("_color", color.ToString(4));  // NOTE: e->SetVector() uses precision
                                        // of 2, not enough for color
...
void LightEditor::ReInit(const idDict* dict, idEntity* light) {
  // TODO: if the lighteditor is currently shown, show a warning first about
  // saving current changes to the last light?
  TheLightEditor.Init(dict, light);
}

// static
void LightEditor::Draw() { TheLightEditor.DrawWindow(); }
...
void LightEditor::Init(const idDict* dict, idEntity* light) {
  ...
  if (dict) {
    ...
    if (name) {
      ...
      // idassert( 0 && "LightEditor::Init(): Given entity has no 'name'
      // property?!" );
      entityName =
          "";  // TODO: generate name or handle gracefully or something?
      title.Format("Light Editor: <unnamed> light at (%s)",
                   entityPos.ToString());
    }
...
void LightEditor::LoadLightTextures() {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (matName.Icmpn("lights/", strlen("lights/")) == 0 ||
        matName.Icmpn("fogs/", strlen("fogs/")) == 0) {
      // actually load the material
      const idMaterial* material = declManager->FindMaterial(matName, false);
      if (material != NULL) {
        // check if the material has textures or is just a leftover from the
        // development
...
// static
bool LightEditor::TextureItemsGetter(void* data, int idx,
                                     const char** outText) {
...
void LightEditor::LoadLightStyles() {
  ...
  const idDeclEntityDef* decl = static_cast<const idDeclEntityDef*>(
      declManager->FindType(DECL_ENTITYDEF, "light", false));
  if (decl == NULL) {
    return;
  }
...
static float* vecToArr(idVec3& v) { return &v.x; }
...
void LightEditor::DrawWindow() {
  ...
  if (ImGui::Begin(title, &showWindow))  //, ImGuiWindowFlags_ShowBorders ) )
  {
    ...
    ImGui::SameLine();
    changes |=
        ImGui::RadioButton("Projected Light", &lightSelectionRadioBtn, 1);
    ...
    if (lightSelectionRadioBtn == 0) {
      ImGui::PushItemWidth(
          -1.0f);  // align end of Drag* with right window border

      changes |= ImGui::Checkbox("Equilateral Radius", &cur.equalRadius);
      ImGui::Text("Radius:");
      ImGui::Indent();
      if (cur.equalRadius) {
        if (ImGui::DragFloat("##radEquil", &cur.lightRadius.x, 1.0f, 0.0f,
                             10000.0f, "%.1f")) {
      ...
      // TODO: this could as well be a slider or something, if 0/0.5/1 is too
      // restricting
      ...
      // a slider is easier than radiobuttons.. does it really have to be
      // radiobuttons?
      changes |=
          ImGui::SliderFloat("##FallOff", &cur.fallOff, 0.0f, 1.0f, "%.1f");
      ...
      if (cur.hasCenter) {
        ImGui::Indent();
        changes |= ImGui::DragVec3("##centerXYZ", cur.lightCenter, 1.0f, 0.0f,
                                   10000.0f, "%.1f");
        ImGui::Unindent();
      }
      ...
      changes |=
          ImGui::DragVec3("Target", cur.lightTarget, 1.0f, 0.0f, 0.0f, "%.1f");
      changes |=
          ImGui::DragVec3("Right", cur.lightRight, 1.0f, 0.0f, 0.0f, "%.1f");
      changes |= ImGui::DragVec3("Up", cur.lightUp, 1.0f, 0.0f, 0.0f, "%.1f");

      ImGui::Spacing();

      changes |=
          ImGui::Checkbox("Explicit start/end points", &cur.explicitStartEnd);

      ImGui::Spacing();
      if (cur.explicitStartEnd) {
        changes |=
            ImGui::DragVec3("Start", cur.lightStart, 1.0f, 0.0f, 0.0f, "%.1f");
        changes |=
            ImGui::DragVec3("End", cur.lightEnd, 1.0f, 0.0f, 0.0f, "%.1f");
      }
    }

    ImGui::Unindent();

    if (ImGui::Combo("Style", &currentStyleIndex, StyleItemsGetter, this,
                     styleNames.Num() + 1)) {
      changes = true;
    ...
    if (ImGui::Combo("Texture", &currentTextureIndex, TextureItemsGetter, this,
                     textureNames.Num() + 1)) {
      changes = true;

      // -1 because 0 is "<No Texture>"
      cur.strTexture = (currentTextureIndex > 0)
                           ? textureNames[currentTextureIndex - 1]
                           : "";
      LoadCurrentTexture();
    }

    if (currentTextureMaterial != nullptr && currentTexture != nullptr) {
      ImVec2 size(currentTexture->GetUploadWidth(),
                  currentTexture->GetUploadHeight());

#if !IMGUI_BFGUI
      ImGui::Image((void*)currentTexture->GetImGuiTextureID(), size,
                   ImVec2(0, 0), ImVec2(1, 1),
#else
      ImGui::Image(
          (void*)currentTextureMaterial, size, ImVec2(0, 0), ImVec2(1, 1),
#endif
                   ImColor(255, 255, 255, 255), ImColor(255, 255, 255, 128));
    }

    // TODO: allow multiple lights selected at the same time + "apply different"
    // button?
    //       then only the changed attribute (e.g. color) would be set to all
    //       lights, but they'd keep their other individual properties (eg
    //       radius)
