...
idCVar com_version("si_version", version.string,
                   CVAR_SYSTEM | CVAR_ROM | CVAR_SERVERINFO, "engine version");
...
#ifdef ID_RETAIL
idCVar com_allowConsole("com_allowConsole", "0",
...
idCVar com_developer("developer", "0", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
                     "developer mode");
idCVar com_speeds("com_speeds", "0", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
                  "show engine timings");
// DG: support "com_showFPS 1" for fps-only view like in classic doom3 => make
// it CVAR_INTEGER
idCVar com_showFPS("com_showFPS", "0",
                   CVAR_INTEGER | CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_NOCHEAT,
                   "show frames rendered per second. 0: off, 1: only show FPS "
                   "(classic view), 2: default bfg values");
// DG end
idCVar com_showMemoryUsage("com_showMemoryUsage", "0",
                           CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
                           "show total and per frame memory usage");
idCVar com_updateLoadSize("com_updateLoadSize", "0",
                          CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
                          "update the load size after loading a map");

idCVar com_productionMode("com_productionMode", "0", CVAR_SYSTEM | CVAR_BOOL,
                          "0 - no special behavior, 1 - building a production "
                          "build, 2 - running a production build");

idCVar com_japaneseCensorship("com_japaneseCensorship", "0", CVAR_NOCHEAT,
                              "Enable Japanese censorship");

idCVar preload_CommonAssets("preload_CommonAssets", "1",
                            CVAR_SYSTEM | CVAR_BOOL, "preload common assets");

idCVar net_inviteOnly("net_inviteOnly", "1", CVAR_BOOL | CVAR_ARCHIVE,
                      "whether or not the private server you create allows "
                      "friends to join or invite only");

// DG: add cvar for pause
idCVar com_pause("com_pause", "0", CVAR_BOOL | CVAR_SYSTEM | CVAR_NOCHEAT,
                 "set to 1 to pause game, to 0 to unpause again");
// DG end
...
float com_engineHz_latched =
    60.0f;  // Latched version of cvar, updated between map loads
...
// RB: defaulted this to 1 because we don't have a sound for the intro .bik
// video
idCVar com_skipIntroVideos("com_skipIntroVideos", "1", CVAR_BOOL,
                           "skips intro videos");
...
=================
*/
CONSOLE_COMMAND_SHIP(quit, "quits the game", NULL) { commonLocal.Quit(); }
CONSOLE_COMMAND_SHIP(exit, "exits the game", NULL) { commonLocal.Quit(); }
...
void idCommonLocal::CheckStartupStorageRequirements() {
  // RB: disabled savegame and profile storage checks, because it fails
...
void idCommonLocal::RenderSplash() {
  ...
  const float sysWidth =
      renderSystem->GetWidth() * renderSystem->GetPixelAspect();
  const float sysHeight = renderSystem->GetHeight();
  const float sysAspect = sysWidth / sysHeight;
  const float splashAspect = 16.0f / 9.0f;
  const float adjustment = sysAspect / splashAspect;
  const float barHeight = (adjustment >= 1.0f)
                              ? 0.0f
                              : (1.0f - adjustment) *
                                    (float)renderSystem->GetVirtualHeight() *
                                    0.25f;
  const float barWidth =
      (adjustment <= 1.0f) ? 0.0f
                           : (adjustment - 1.0f) *
                                 (float)renderSystem->GetVirtualWidth() * 0.25f;
  if (barHeight > 0.0f) {
    renderSystem->SetColor(colorBlack);
    renderSystem->DrawStretchPic(0, 0, renderSystem->GetVirtualWidth(),
                                 barHeight, 0, 0, 1, 1, whiteMaterial);
    renderSystem->DrawStretchPic(
        0, renderSystem->GetVirtualHeight() - barHeight,
        renderSystem->GetVirtualWidth(), barHeight, 0, 0, 1, 1, whiteMaterial);
  }
  if (barWidth > 0.0f) {
    renderSystem->SetColor(colorBlack);
    renderSystem->DrawStretchPic(0, 0, barWidth,
                                 renderSystem->GetVirtualHeight(), 0, 0, 1, 1,
                                 whiteMaterial);
    renderSystem->DrawStretchPic(renderSystem->GetVirtualWidth() - barWidth, 0,
                                 barWidth, renderSystem->GetVirtualHeight(), 0,
                                 0, 1, 1, whiteMaterial);
  }
  renderSystem->SetColor4(1, 1, 1, 1);
  renderSystem->DrawStretchPic(
      barWidth, barHeight, renderSystem->GetVirtualWidth() - barWidth * 2.0f,
      renderSystem->GetVirtualHeight() - barHeight * 2.0f, 0, 0, 1, 1,
      splashScreen);
  ...
  // renderSystem->SwapCommandBuffers_FinishRendering( &time_frontend,
  // &time_backend, &time_shadows, &time_gpu );
}
...
void idCommonLocal::RenderBink(const char* path) {
  ...
  const float imageWidth =
      renderSystem->GetVirtualWidth() * movieAspect / sysAspect;
  ...
  idMaterial* material =
      const_cast<idMaterial*>(declManager->FindMaterial("splashbink"));
  material->Parse(materialText.c_str(), materialText.Length(), false);
  material->ResetCinematicTime(Sys_Milliseconds());

  // SRS - Restored original calculation after implementing
  // idCinematicLocal::GetStartTime() and fixing animationLength in
  // idCinematicLocal::InitFromBinkDecFile()
  int cinematicLength = material->CinematicLength();
  int mouseEvents[MAX_MOUSE_EVENTS][2];

  bool escapeEvent = false;
  while ((Sys_Milliseconds() <=
          (material->GetCinematicStartTime() + cinematicLength)) &&
         material->CinematicIsPlaying()) {
    renderSystem->DrawStretchPic(chop, 0, imageWidth,
                                 renderSystem->GetVirtualHeight(), 0, 0, 1, 1,
                                 material);
    const emptyCommand_t* cmd = renderSystem->SwapCommandBuffers(
        &time_frontend, &time_backend, &time_shadows, &time_gpu, &stats_backend,
...
=================
*/
bool idCommonLocal::IsInitialized() const { return com_fullyInitialized; }
...
void idCommonLocal::Init(int argc, const char* const* argv,
  ...
  try {
    ...
    // Pre-allocate our 20 MB save buffer here on time, instead of on-demand for
    // each save....
    ...
    fileSystem->BeginLevelLoad("_startup", saveFile.GetDataPtr(),
                               saveFile.GetAllocated());
    ...
    const bool showVideo =
        (!com_skipIntroVideos.GetBool() && fileSystem->UsingResourceFiles());
    if (showVideo) {
      ...
      RenderSplash();
      // SRS - OSX needs this for some OpenGL drivers, otherwise renders
      // leftover image before splash
      RenderSplash();
    }
    ...
    // core
    gameThread.StartWorkerThread("Game/Draw", CORE_1B, THREAD_BELOW_NORMAL,
                                 0x100000);
    ...
    if (!fileSystem->UsingResourceFiles()) {
      ...
      // Make sure all videos get touched because you can bring videos from one
      // map to another, they need to be included in all maps
    ...
    // leaderboards need to be initialized after InitializeMPMapsModes, which
    // populates the MP Map list.
    if (game != NULL) {
    ...
    StartMenu(true);
// SRS - changed ifndef to ifdef since legalMinTime should apply to retail
// builds, not dev builds
#ifdef ID_RETAIL
...
void idCommonLocal::Shutdown() {
  ...
  // SRS - Call FreeRenderWorld() vs. delete, otherwise worlds list not updated
  // on shutdown
  renderSystem->FreeRenderWorld(renderWorld);
  renderWorld = NULL;

  printf("delete soundWorld;\n");
  // SRS - Call FreeSoundWorld() vs. delete, otherwise soundWorlds list not
  // updated and can segfault in soundSystem->Shutdown()
  ...
  // SRS - Note this also shuts down all cinematic resources, including
  // cinematic audio voices
  printf("declManager->Shutdown();\n");
  declManager->Shutdown();

  // shut down the sound system
  // SRS - Shut down sound system after decl manager so cinematic audio voices
  // are destroyed first Important for XAudio2 where the mastering voice cannot
  // be destroyed if any other voices exist
...
bool idCommonLocal::WaitForSessionState(
  ...
  while (true) {
    ...
    if (sessionState != idSession::LOADING &&
        ...
        sessionState != idSession::BUSY && sessionState != desiredState) {
      return false;
    }
...
bool idCommonLocal::ProcessEvent(const sysEvent_t* event) {
  ...
  if (game && (game->IsInGame() || readDemo)) {
    if (event->evType == SE_KEY && event->evValue2 == 1 &&
        (event->evValue == K_ESCAPE || event->evValue == K_JOY9)) {
...
CONSOLE_COMMAND(testSIMD, "test SIMD code", NULL) { idSIMD::Test_f(args); }
