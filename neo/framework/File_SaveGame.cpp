...
========================
*/
void ZlibFree(void* opaque, void* address) { Mem_Free(address); }

idCVar sgf_threads("sgf_threads", "2", CVAR_INTEGER,
                   "0 = all foreground, 1 = background write, 2 = background "
                   "write + compress");
idCVar sgf_checksums("sgf_checksums", "1", CVAR_BOOL,
                     "enable save game file checksums");
idCVar sgf_testCorruption("sgf_testCorruption", "-1", CVAR_INTEGER,
                          "test corruption at the 128 kB compressed block");

// this is supposed to get faster going from -15 to -9, but it gets slower as
// well as worse compression
idCVar sgf_windowBits("sgf_windowBits", "-15", CVAR_INTEGER,
                      "zlib window bits");
...
int idFile_SaveGamePipelined::GetPointerSize() const {
  if (pointerSize == 0) {
    // in original savegames we weren't saving the pointer size, so the 2 high
    // bytes of the save version will be 0
    return 4;
  } else {
    return pointerSize;
  }
...
============================
*/
bool idFile_SaveGamePipelined::OpenForWriting(const char* const filename,
                                              bool useNativeFile) {
  assert(mode == CLOSED);
  ...
  // mem.PushHeap();
  int status = deflateInit2(&zStream, Z_BEST_SPEED, Z_DEFLATED,
                            sgf_windowBits.GetInteger(), 9, Z_DEFAULT_STRATEGY);
  // mem.PopHeap();
  if (status != Z_OK) {
    idLib::FatalError(
        "idFile_SaveGamePipelined::OpenForWriting: deflateInit2() error %i",
        status);
  }
  ...
  if (sgf_threads.GetInteger() >= 1) {
    ...
    // DG: change threadname from "SGF_CompressThread" to "SGF_Compress",
    // because Linux has a 15 (+ \0) char limit for threadnames. also, "thread"
    // in a threadname is redundant
...
Called when a compressed block fills up, and also to flush the final partial
block. Flushes everything from [compressedConsumedBytes ->
...
Called when an uncompressed block fills up, and also to flush the final partial
block. Flushes everything from [uncompressedConsumedBytes ->
...
void idFile_SaveGamePipelined::CompressBlock() {
  ...
  while (zStream.avail_in > 0 || zLibFlushType == Z_FINISH) {
    ...
    if (zstat != Z_OK && zstat != Z_STREAM_END) {
      idLib::FatalError(
          "idFile_SaveGamePipelined::CompressBlock: deflate() returned %i",
          zstat);
    }
    ...
    if (zStream.avail_out == 0 || zLibFlushType == Z_FINISH) {
      ...
      compressedProducedBytes =
          zStream.total_out + numChecksums * sizeof(uint32);
      ...
      zStream.avail_out = COMPRESSED_BLOCK_SIZE;
      zStream.next_out = (Bytef*)&compressed[compressedProducedBytes &
                                             (COMPRESSED_BUFFER_SIZE - 1)];
...
void idFile_SaveGamePipelined::FlushUncompressedBlock() {
  ...
  dataZlib =
      &uncompressed[uncompressedConsumedBytes & (UNCOMPRESSED_BUFFER_SIZE - 1)];
...
int idFile_SaveGamePipelined::Write(const void* buffer, int length) {
...
#if 1  // quick and dirty fix for user-initiated forced shutdown during a
       // savegame
  ...
  while (lengthRemaining > 0) {
    const size_t ofsInBuffer =
        uncompressedProducedBytes & (UNCOMPRESSED_BUFFER_SIZE - 1);
    const size_t ofsInBlock =
        uncompressedProducedBytes & (UNCOMPRESSED_BLOCK_SIZE - 1);
    const size_t remainingInBlock = UNCOMPRESSED_BLOCK_SIZE - ofsInBlock;
    const size_t copyToBlock = (lengthRemaining < remainingInBlock)
                                   ? lengthRemaining
                                   : remainingInBlock;
...
============================
*/
bool idFile_SaveGamePipelined::OpenForReading(const char* const filename,
                                              bool useNativeFile) {
  assert(mode == CLOSED);
  ...
  if (status != Z_OK) {
    idLib::FatalError(
        "idFile_SaveGamePipelined::OpenForReading: inflateInit2() error %i",
        status);
  }
  ...
  if (sgf_threads.GetInteger() >= 1) {
    ...
    // DG: change threadname from "SGF_DecompressThread" to "SGF_Decompress",
    // because Linux has a 15 (+ \0) char limit for threadnames. also, "thread"
    // in a threadname is redundant
    decompressThread->StartWorkerThread("SGF_Decompress", CORE_2B,
                                        THREAD_NORMAL);
    // DG end
  }
  if (nativeFile != NULL && sgf_threads.GetInteger() >= 2) {
...
bool idFile_SaveGamePipelined::OpenForReading(idFile* file) {
  ...
  if (sgf_threads.GetInteger() >= 1) {
    ...
    // because Linux has a 15 (+ \0) char limit for threadnames. also, "thread"
    // in a threadname is redundant
    decompressThread->StartWorkerThread("SGF_Decompress", CORE_1B,
                                        THREAD_NORMAL);
    // DG end
  }
  if (nativeFile != NULL && sgf_threads.GetInteger() >= 2) {
...
============================
*/
bool idFile_SaveGamePipelined::NextReadBlock(blockForIO_t* block,
                                             size_t lastReadBytes) {
  assert(mode == READ);
  ...
  blockAvailable
      .Raise();  // a new block is available for the pipeline to consume
  ...
  blockRequested
      .Wait();  // wait for the last block to be consumed by the pipeline
  ...
  assert(0 == (compressedProducedBytes & (COMPRESSED_BLOCK_SIZE - 1)));
  block->data =
      &compressed[compressedProducedBytes & (COMPRESSED_BUFFER_SIZE - 1)];
...
void idFile_SaveGamePipelined::ReadBlock() {
  ...
  // when we are reading the last block of the file, we may not fill the entire
  // block
  assert(0 == (compressedProducedBytes & (COMPRESSED_BLOCK_SIZE - 1)));
  byte* dest =
      &compressed[compressedProducedBytes & (COMPRESSED_BUFFER_SIZE - 1)];
...
void idFile_SaveGamePipelined::DecompressBlock() {
  ...
  zStream.next_out = (Bytef*)&uncompressed[uncompressedProducedBytes &
                                           (UNCOMPRESSED_BUFFER_SIZE - 1)];
  ...
  while (zStream.avail_out > 0) {
      ...
      if (sgf_checksums.GetBool()) {
        ...
        if (!verify(zStream.next_in[zStream.avail_in + 0] ==
            ...
            !verify(zStream.next_in[zStream.avail_in + 1] ==
                    ((checksum >> 8) & 0xFF)) ||
            !verify(zStream.next_in[zStream.avail_in + 2] ==
                    ((checksum >> 16) & 0xFF)) ||
            !verify(zStream.next_in[zStream.avail_in + 3] ==
                    ((checksum >> 24) & 0xFF))) {
    ...
    if (zstat != Z_OK) {
      idLib::Warning(
          "idFile_SaveGamePipelined::DecompressBlock: inflate() returned %i",
          zstat);
      zStreamEndHit = true;
      return;
    }
...
int idFile_SaveGamePipelined::Read(void* buffer, int length) {
  ...
  while (lengthRemaining > 0) {
    ...
    const size_t copyFromBlock =
        (lengthRemaining < bytesZlib) ? lengthRemaining : bytesZlib;
...
static void TestProcessFile(const char* const filename) {
  ...
  idFile_SaveGamePipelined* saveFile =
      new (TAG_IDFILE) idFile_SaveGamePipelined;
  ...
  const uint64 writeMicroseconds =
      endWriteMicroseconds - startWriteMicroseconds;

  idLib::Printf(
      "%lld microseconds to compress %i bytes to %i written bytes = %4.1f "
      "MB/s\n",
      writeMicroseconds, testDataLength, readDataLength,
      (float)readDataLength / writeMicroseconds);
...
CONSOLE_COMMAND(TestCompressionSpeeds, "Compares zlib and our code", 0) {
  ...
  idLib::Printf(
      "%i microseconds to compress %i bytes to %i written bytes = %4.1f MB/s\n",
      writeMicroseconds, testDataLength, readDataLength,
      (float)readDataLength / writeMicroseconds);
}
