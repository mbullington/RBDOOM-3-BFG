...
keyname_t keynames[] = {
    ...
    // DG: adding names for keys from cegui/directinput I added in enum keyNum_t
    // in sys_public.h
    //     (they're really valid directinput scancodes, they just haven't been
    //     handled before in d3bfg)
    NAMEKEY2(OEM_102),
    NAMEKEY2(ABNT_C1),
    NAMEKEY2(NEXTTRACK),
    NAMEKEY2(MUTE),
...
===================
*/
bool idKeyInput::GetOverstrikeMode() { return key_overstrikeMode; }
...
===================
*/
void idKeyInput::SetOverstrikeMode(bool state) { key_overstrikeMode = state; }
...
const char* idKeyInput::LocalizedKeyName(keyNum_t keynum) {
  ...
  if (keynum < K_JOY1) {
    // On the PC, we want to turn the scan code in to a key label that matches
    ...
    int result = -1;
    while (result < 0) {
      result = ToUnicode(vkey, scancode, keystate, temp,
                         sizeof(temp) / sizeof(temp[0]), 0);
    }
...
#else  // DG: for !Windows I introduced Sys_GetKeyName() to get key label for
...
void Key_ListBinds_f(const idCmdArgs& args) {
  for (int i = 0; i < K_LAST_KEY; i++) {
    if (keys[i].binding.Length()) {
      common->Printf("%s \"%s\"\n", idKeyInput::KeyNumToString((keyNum_t)i),
...
const char* idKeyInput::KeysFromBinding(const char* bind) {
      ...
      if (keys[i].binding.Icmp(bind) == 0) {
        if (keyName[0] != '\0') {
          idStr::Append(keyName, sizeof(keyName),
                        idLocalization::GetString("#str_07183"));
        }
  ...
  if (keyName[0] == '\0') {
    idStr::Copynz(keyName, idLocalization::GetString("#str_07133"),
                  sizeof(keyName));
  }
...
========================
*/
keyBindings_t idKeyInput::KeyBindingsFromBinding(const char* bind,
                                                 bool firstOnly,
                                                 bool localized) {
  idStr keyboard;
  idStr mouse;
...
int Key_CovertHIDCode(int hid) {
  if (hid >= 0 && hid <= 106) {
    int table[] = {
        K_NONE,        K_NONE,      K_NONE,       K_NONE,      K_A,
        K_B,           K_C,         K_D,          K_E,         K_F,
        K_G,           K_H,         K_I,          K_J,         K_K,
        K_L,           K_M,         K_N,          K_O,         K_P,
        K_Q,           K_R,         K_S,          K_T,         K_U,
        ...
        K_MINUS,       K_EQUALS,    K_LBRACKET,   K_RBRACKET,  K_BACKSLASH,
        K_NONE,        K_SEMICOLON, K_APOSTROPHE, K_GRAVE,     K_COMMA,
        K_PERIOD,      K_SLASH,     K_CAPSLOCK,   K_F1,        K_F2,
        K_F3,          K_F4,        K_F5,         K_F6,        K_F7,
        K_F8,          K_F9,        K_F10,        K_F11,       K_F12,
        K_PRINTSCREEN, K_SCROLL,    K_PAUSE,      K_INS,       K_HOME,
        K_PGUP,        K_DEL,       K_END,        K_PGDN,      K_RIGHTARROW,
        K_LEFTARROW,   K_DOWNARROW, K_UPARROW,    K_NUMLOCK,   K_KP_SLASH,
        K_KP_STAR,     K_KP_MINUS,  K_KP_PLUS,    K_KP_ENTER,  K_KP_1,
        K_KP_2,        K_KP_3,      K_KP_4,       K_KP_5,      K_KP_6,
        K_KP_7,        K_KP_8,      K_KP_9,       K_KP_0,      K_KP_DOT,
        K_NONE,        K_APPS,      K_POWER,      K_KP_EQUALS, K_F13,
        K_F14,         K_F15};
    return table[hid];
  }
  if (hid >= 224 && hid <= 231) {
    int table[] = {K_LCTRL, K_LSHIFT, K_LALT, K_LWIN,
