...
idCVar memcpyImpl(
    "memcpyImpl", "0", 0,
    "Which implementation of memcpy to use for idFile_Memory::Write() [0/1 - "
    "standard (1 eliminates branch misprediction), 2 - auto-vectorized]");
...
CONSOLE_COMMAND(clearHistogram, "", 0) { histogram.Clear(); }
...
int idFile_Memory::Write(const void* buffer, int len) {
  ...
  if (alloc > 0) {
    if (maxSize != 0) {
      common->Error("idFile_Memory::Write: exceeded maximum size %" PRIuSIZE "",
                    maxSize);
      return 0;
    }
...
void idFile_Memory::PreAllocate(size_t len) {
  if (len > allocated) {
    if (maxSize != 0) {
      idLib::Error("idFile_Memory::SetLength: exceeded maximum size %" PRIuSIZE
                   "",
                   maxSize);
    }
...
void idFile_Memory::TruncateData(size_t len) {
  if (len > allocated) {
    idLib::Error("idFile_Memory::TruncateData: len (%" PRIuSIZE
                 ") exceeded allocated size (%" PRIuSIZE ")",
                 len, allocated);
  } else {
    fileSize = len;
  }
...
=================
*/
int idFile_BitMsg::Length() const { return msg->GetSize(); }
...
=================
*/
ID_TIME_T idFile_BitMsg::Timestamp() const { return 0; }
...
=================
*/
int idFile_BitMsg::Seek(long offset, fsOrigin_t origin) { return -1; }
...
int idFile_Permanent::Read(void* buffer, int len) {
  ...
  while (remaining) {
    ...
    if (!ReadFile(o, buf, block, &bytesRead, NULL)) {
      idLib::Warning("idFile_Permanent::Read failed with %d from %s",
...
================
*/
int idFile_Permanent::Length() const { return fileSize; }
...
=================
*/
idFile_Cached::~idFile_Cached() { Mem_Free(buffered); }
...
int idFile_Cached::Read(void* buffer, int len) {
  if (internalFilePos >= bufferedStartOffset &&
      internalFilePos + len < bufferedEndOffset) {
    // this is in the buffer
    memcpy(buffer, (void*)&buffered[internalFilePos - bufferedStartOffset],
           len);
    internalFilePos += len;
    return len;
  }
...
=================
*/
int idFile_Cached::Tell() const { return internalFilePos; }
...
int idFile_Cached::Seek(long offset, fsOrigin_t origin) {
  if (origin == FS_SEEK_SET && offset >= bufferedStartOffset &&
      offset < bufferedEndOffset) {
    // don't do anything to the actual file ptr, just update or internal
    // position
...
int idFile_InZip::Tell() const {
  ...
  // OTOH idFile in general seems to assume file-length <= INT_MAX so it may be
  // ok..
...
================
*/
int idFile_InZip::Length() const { return fileSize; }
...
================
*/
ID_TIME_T idFile_InZip::Timestamp() const { return 0; }
...
int idFile_InZip::Seek(long offset, fsOrigin_t origin) {
  ...
  switch (origin) {
    ...
    case FS_SEEK_SET: {
      ...
      // DG use standard unzip.h function instead of custom one (not needed
      // anymore with minizip 1.1)
...
=================
*/
idFile_InnerResource::idFile_InnerResource(const char* _name, idFile* rezFile,
...
int idFile_InnerResource::Read(void* buffer, int len) {
  ...
  int read = 0;  // fileSystem->ReadFromBGL( resourceFile, (byte*)buffer, offset
                 // + internalFilePos, len );
  ...
  if (read != len) {
    if (resourceBuffer != NULL) {
      ...
      read = fileSystem->ReadFromBGL(resourceFile, buffer,
                                     offset + internalFilePos, len);
    }
  }
...
=================
*/
int idFile_InnerResource::Tell() const { return internalFilePos; }
...
CONSOLE_COMMAND(testEndianNessWrite,
                "Tests the read/write compatibility between platforms", 0) {
...
CONSOLE_COMMAND(testEndianNessRead,
                "Tests the read/write compatibility between platforms", 0) {
...
CONSOLE_COMMAND(testEndianNessReset,
                "Tests the read/write compatibility between platforms", 0) {
