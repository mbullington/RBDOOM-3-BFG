...
================
*/
float idCompressor_None::GetCompressionRatio() const { return 0.0f; }
...
class idCompressor_BitStream : public idCompressor_None {
 ...
 protected:
  ...
  int Compare(const byte* src1, int bitPtr1, const byte* src2, int bitPtr2,
              int maxBits) const;
};
...
================
*/
inline void idCompressor_BitStream::InitCompress(const void* inData,
                                                 const int inLength) {
  readLength = inLength;
  readByte = 0;
  readBit = 0;
...
================
*/
inline void idCompressor_BitStream::InitDecompress(void* outData,
                                                   int outLength) {
  if (!readLength) {
...
================
*/
int idCompressor_BitStream::Compare(const byte* src1, int bitPtr1,
                                    const byte* src2, int bitPtr2,
                                    int maxBits) const {
  int i;
...
================
*/
void idCompressor_Huffman::Send(huffmanNode_t* node, huffmanNode_t* child,
                                byte* fout) {
  if (node->parent) {
...
int idCompressor_Arithmetic::GetCurrentCount() {
  ...
  return (unsigned int)(((((int)code - low) + 1) * scale - 1) /
                        (((int)high - low) + 1));
  // DG end
}
...
================
*/
bool idCompressor_LZSS::FindMatch(int startWord, int startValue,
...
================
*/
void idCompressor_LZSS_WordAligned::Init(idFile* f, bool compress,
                                         int wordLength) {
