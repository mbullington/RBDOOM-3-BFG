...
bool idTokenParser::ReadToken(idToken* tok) {
  if (currentToken >= 0 &&
      currentToken < guiTokenIndexes[currentTokenList].Num()) {
    tok->Clear();
    idBinaryToken& btok =
        tokens[guiTokenIndexes[currentTokenList][currentToken]];
    *tok = btok.token;
...
int idTokenParser::ExpectTokenType(int type, int subtype, idToken* token) {
  ...
  if (token->type == TT_NUMBER) {
    ...
    if (token->subtype != subtype) {
      // Error( "expected '%s' but found '%s'", idLexer::GetPunctuationFromId(
      // subtype ), token->c_str() );
      return 0;
    }
  }
...
void idTokenParser::UnreadToken(const idToken* token) {
  if (currentToken == 0 ||
    ...
    idLib::common->FatalError(
        "idTokenParser::unreadToken, unread token twice\n");
  }
  currentToken--;
}
...
// issue an Error().  If it isn't NULL, it will issue a Warning() and set
// *errorFlag = true
