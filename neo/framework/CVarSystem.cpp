...
============
*/
idInternalCVar::idInternalCVar(const char* newName, const char* newValue,
                               int newFlags) {
  nameString = newName;
...
const char** idInternalCVar::CopyValueStrings(const char** strings) {
  ...
  ptr =
      (const char**)Mem_Alloc((i + 1) * sizeof(char*) + totalLength, TAG_CVAR);
...
void idInternalCVar::UpdateCheat() {
  ...
  if (flags & (CVAR_NOCHEAT | CVAR_INIT | CVAR_ROM | CVAR_ARCHIVE |
               CVAR_SERVERINFO | CVAR_NETWORKSYNC)) {
    flags &= ~CVAR_CHEAT;
  } else {
    flags |= CVAR_CHEAT;
  }
...
void idInternalCVar::Set(const char* newValue, bool force, bool fromServer) {
  ...
  if (!force) {
    ...
    if (flags & CVAR_INIT) {
      common->Printf(
          "%s is write protected and can only be set from the cmdline (or "
          "autoexec.cfg).\n",
          nameString.c_str());
      return;
    }
...
class idCVarSystemLocal : public idCVarSystem {
 public:
  ...
  virtual void WriteFlaggedVariables(int flags, const char* setCmd,
                                     idFile* f) const;

  virtual void MoveCVarsToDict(int flags, idDict& dict,
...
const char* CreateColumn(const char* text, int columnWidth, const char* indent,
                         idStr& string) {
  int i, lastLine;
...
============
*/
void idCVarSystemLocal::SetInternal(const char* name, const char* value,
                                    int flags) {
  int hash;
...
void idCVarSystemLocal::Init() {
  ...
  cmdSystem->AddCommand("cvarAdd", CvarAdd_f, CMD_FL_SYSTEM,
                        "adds a value to a numeric cvar");
...
============
*/
bool idCVarSystemLocal::IsInitialized() const { return initialized; }
...
============
*/
int idCVarSystemLocal::GetModifiedFlags() const { return modifiedFlags; }
...
============
*/
void idCVarSystemLocal::WriteFlaggedVariables(int flags, const char* setCmd,
                                              idFile* f) const {
  for (int i = 0; i < cvars.Num(); i++) {
    idInternalCVar* cvar = cvars[i];
    if (cvar->GetFlags() & flags) {
...
============
*/
void idCVarSystemLocal::MoveCVarsToDict(int flags, idDict& dict,
                                        bool onlyModified) const {
  dict.Clear();
  for (int i = 0; i < cvars.Num(); i++) {
    idCVar* cvar = cvars[i];
    if (cvar->GetFlags() & flags) {
      if (onlyModified &&
          idStr::Icmp(cvar->GetString(), cvar->GetDefaultString()) == 0) {
        continue;
      }
...
//================================================
//*/
// class idSort_InternalCVar : public idSort_Quick< const idInternalCVar *,
// idSort_InternalCVar > { public: 	int Compare( const idInternalCVar * & a,
//const idInternalCVar * & b ) const { return idStr::Icmp( a.GetName(),
//b.GetName() ); }
//};
