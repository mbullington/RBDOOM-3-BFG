...
class idFileSystem {
 public:
  ...
  virtual const char* RelativePathToOSPath(
      const char* relativePath, const char* basePath = "fs_basepath") = 0;
  ...
  virtual idFile* OpenFileAppend(const char* filename, bool sync = false,
  ...
  virtual bool GetResourceCacheEntry(const char* fileName,
                                     idResourceCacheEntry& rc) = 0;
  virtual void FreeResourceBuffer() = 0;
  virtual void AddImagePreload(const char* resName, int filter, int repeat,
                               int usage, int cube) = 0;
