...
static const int MAX_DIALOGS =
    4;  // maximum dialogs that can be open at one time
...
enum gameDialogMessages_t {
  ...
  GDM_NEW_HOST_GAME_STATS_DROPPED,  // Host left/DC'd, someone else is new host,
                                    // ranked game so stats were dropped
  GDM_BECAME_HOST_GAME,  // Host left/DC'd, you became host, unranked game
  GDM_BECAME_HOST_GAME_STATS_DROPPED,  // Host left/DC'd, you became host,
                                       // ranked game so stats were dropped
  GDM_LOBBY_DISBANDED,
...
class idCommonDialog {
 public:
  ...
  void AddDialog(gameDialogMessages_t msg, dialogType_t type,
                 ...
                 const char* location = NULL, int lineNumber = 0,
                 bool leaveOnMapHeapReset = false, bool waitOnAtlas = false,
  ...
  bool IsDialogActive();
  void ClearDialog(gameDialogMessages_t msg, const char* location = NULL,
                   int lineNumber = 0);
 ...
 private:
  ...
  int startSaveTime;  // with stopSaveTime, useful to pass 360 TCR# 047.  Need
                      // to keep the dialog on the screen for a minimum amount
                      // of time
  int stopSaveTime;
  bool dialogInUse;  // this is to prevent an active msg getting lost during a
                     // map heap reset
};
