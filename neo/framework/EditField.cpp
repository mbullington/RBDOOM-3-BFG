...
===============
*/
int idEditField::GetCursor() const { return cursor; }
...
===============
*/
int idEditField::GetAutoCompleteLength() const { return autoComplete.length; }
...
void idEditField::KeyDownEvent(int key) {
  ...
  if (key == K_HOME || (key == K_A && (idKeyInput::IsDown(K_LCTRL) ||
  ...
  if (key == K_END || (key == K_E && (idKeyInput::IsDown(K_LCTRL) ||
                                      idKeyInput::IsDown(K_RCTRL)))) {
    cursor = len;
    if (cursor >= scroll + widthInChars) {
      scroll = cursor - widthInChars + 1;
...
===============
*/
char* idEditField::GetBuffer() { return buffer; }
