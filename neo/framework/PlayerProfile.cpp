...
// After releasing a version to the market, here are limitations for
// compatibility:
//	- the major version should not ever change
//	- always add new items to the bottom of the save/load routine
//	- never remove or change the size of items, just stop using them or add
//new items to the end
//
// The biggest reason these limitations exist is because if a newer profile is
// created and then loaded with a GMC version, we have to support it.
const int16 PROFILE_TAG = ('D' << 8) | '3';
const int8 PROFILE_VER_MAJOR =
    10;  // If this is changed, you should reset the minor version and remove
         // all backward compatible code
const int8 PROFILE_VER_MINOR =
    0;  // Within each major version, minor versions can be supported for
...
bool idPlayerProfile::Serialize(idSerializer& ser) {
  ...
  if (ser.IsReading()) {
    // Never sync these cvars with Steam because they require an engine or video
    // restart
  ...
  // New setting to save to make sure that we have or haven't seen this
  // achievement before used to pass TRC R149d
  ser.Serialize(achievementBits);
  ser.Serialize(achievementBits2);

  // Check to map sure we are on a valid map before we save, this helps prevent
  // someone from creating a test map and gaining a bunch of achievements from
  // it
...
========================
*/
int idPlayerProfile::StatGetInt(int s) const { return stats[s].i; }
...
========================
*/
float idPlayerProfile::StatGetFloat(int s) const { return stats[s].f; }
...
========================
*/
void idPlayerProfile::RestoreDefault() { ExecConfig(true, true); }
...
CONSOLE_COMMAND(setProfileDefaults,
                "sets profile settings to default and saves", 0) {
  if (session->GetSignInManager().GetMasterLocalUser() == NULL) {
    return;
  }
  idPlayerProfile* profile =
      session->GetSignInManager().GetMasterLocalUser()->GetProfile();
