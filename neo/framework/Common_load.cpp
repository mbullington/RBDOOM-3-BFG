...
idCVar com_disableAutoSaves("com_disableAutoSaves", "0",
...
// This is for the dirty hack to get a dialog to show up before we capture the
// screen for autorender.
...
===============
*/
void idCommonLocal::StartNewGame(const char* mapName, bool devmap,
                                 int gameMode) {
  ...
  if (gameMode == GAME_MODE_SINGLEPLAYER) {
    ...
    matchParameters.gameMap =
        mpGameMaps.Num();  // If this map isn't found in mpGameMaps, then set it
                           // to some undefined value (this happens when, for
                           // example, we load a box map with netmap)
...
void idCommonLocal::LoadLoadingGui(const char* mapName, bool& hellMap) {
  ...
  if (g_demoMode.GetBool()) {
    hellMap = false;
    if (loadGUI != NULL) {
      const idMaterial* defaultMat =
          declManager->FindMaterial("guis/assets/loadscreens/default");
      renderSystem->LoadLevelImages();

      loadGUI->Activate(true);
      idSWFSpriteInstance* bgImg =
          loadGUI->GetRootObject().GetSprite("bgImage");
      if (bgImg != NULL) {
  ...
  if (loadGUI != NULL) {
    ...
    idSWFSpriteInstance* overlay =
        loadGUI->GetRootObject().GetSprite("overlay");
    ...
    if (mapDef != NULL) {
      ...
      displayName =
          idLocalization::GetString(mapDef->dict.GetString("name", mapName));
      ...
      if (matchParameters.gameMode == GAME_MODE_SINGLEPLAYER) {
        ...
        subTitle =
            idLocalization::GetString(mapDef->dict.GetString("subTitle", ""));
      } else {
        const idStrList& modes = common->GetModeDisplayList();
        subTitle = modes[idMath::ClampInt(0, modes.Num() - 1,
                                          matchParameters.gameMode)];

        const char* modeDescs[] = {
            "#str_swf_deathmatch_desc", "#str_swf_tourney_desc",
            "#str_swf_team_deathmatch_desc", "#str_swf_lastman_desc",
            "#str_swf_ctf_desc"};
...
void idCommonLocal::ExecuteMapChange() {
  ...
  // This is so we can tell if we had a new loadmap request from within an
  // existing loadmap call
  ...
  // this fixes issues with the projection matrix getting messed up when
  // switching maps or loading a saved game while an in-game cinematic is
  // playing.
  ...
  if (mapSpawnData.savegameFile) {
    ...
    fileSystem->BeginLevelLoad(currentMapName, saveFile.GetDataPtr(),
  ...
  if (mapSpawnData.savegameFile) {
    if (!game->InitFromSaveGame(
            fullMapName, renderWorld, soundWorld, mapSpawnData.savegameFile,
            mapSpawnData.stringTableFile, mapSpawnData.savegameVersion)) {
      // If the loadgame failed, end the session, which will force us to go back
      // to the main menu
      session->QuitMatchToTitle();
    }
  } else {
    ...
    game->InitFromNewMap(fullMapName, renderWorld, soundWorld,
                         matchParameters.gameMode, Sys_Milliseconds());
  }
  ...
  if (session->GetState() == idSession::LOADING &&
      session->GetLoadingID() == cachedLoadingID) {
  ...
  if (session->GetState() != idSession::INGAME) {
    // Something went wrong, don't process stale reliables that have been queued
    // up.
    reliableQueue.Clear();
  }
  ...
  // Issue a render at the very end of the load process to update soundTime
...
Pumps the session and if multiplayer, displays dialogs during the loading
process.
===============
*/
void idCommonLocal::UpdateLevelLoadPacifier() {
  autoRenderIconType_t icon = AUTORENDER_DEFAULTICON;
  bool autoswapsRunning =
      renderSystem->AreAutomaticBackgroundSwapsRunning(&icon);
  ...
  if (autoswapsRunning) {
    ...
    if (lastPacifierDialogState != dialogState) {
      ...
      if (dialogState) {
        icon = AUTORENDER_DIALOGICON;  // Done this way to handle the rare case
                                       // of a tip changing at the same time a
                                       // dialog comes up
  ...
  if (time >= nextLoadTip && loadGUI != NULL && loadTipList.Num() > 0 &&
      !defaultLoadscreen) {
    if (autoswapsRunning) {
      renderSystem->EndAutomaticBackgroundSwaps();
    }
    ...
    idSWFTextInstance* txtVal =
        loadGUI->GetRootObject().GetNestedText("txtDesc");
    if (txtVal != NULL) {
      if (isHellMap) {
...
void idCommonLocal::LoadPacifierBinarizeFilename(const char* filename,
                                                 const char* reason) {
  idLib::Printf("Binarize File: '%s' - reason '%s'\n", filename, reason);

  // we won't actually show updates on very quick files (<16ms), so keep this
  // false until the first progress
...
void idCommonLocal::LoadPacifierBinarizeProgressIncrement(int step) {
  ...
  LoadPacifierBinarizeProgress((float)loadPacifierBinarizeProgressCurrent /
                               loadPacifierBinarizeProgressTotal);
}
...
bool idCommonLocal::SaveGame(const char* saveName) {
  ...
  if (com_disableAllSaves.GetBool() ||
      (com_disableAutoSaves.GetBool() &&
       (idStr::Icmp(saveName, "autosave") == 0))) {
    return false;
  }
  ...
  // Make sure the file is writable and the contents are cleared out (Set to
  // write from the start of file)
  ...
  gameDetails.descriptors.Set(SAVEGAME_DETAIL_FIELD_LANGUAGE,
                              sys_lang.GetString());
  gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_CHECKSUM,
                                 (int)gameDetails.descriptors.Checksum());
...
bool idCommonLocal::LoadGame(const char* saveName) {
  ...
  bool found = false;
  const saveGameDetailsList_t& sgdl =
  ...
  for (int i = 0; i < sgdl.Num(); i++) {
    if (sgdl[i].slotName == saveName) {
      if (sgdl[i].GetLanguage() != sys_lang.GetString()) {
        ...
        idStrStatic<256> msg;
        msg.Format(idLocalization::GetString("#str_dlg_wrong_language"),
                   idLocalization::GetString(langName));
        Dialog().AddDynamicDialog(GDM_SAVEGAME_WRONG_LANGUAGE, callbacks,
                                  optionText, true, msg, false, true);
        if (wipeForced) {
          ClearWipe();
        }
        return false;
...
void HandleInsufficientStorage(const idSaveLoadParms& parms) {
  session->GetSaveGameManager().ShowRetySaveDialog(parms.directory,
                                                   parms.requiredSpaceInBytes);
}
...
bool HandleCommonErrors(const idSaveLoadParms& parms) {
  ...
  if (parms.GetError() & SAVEGAME_E_CORRUPTED) {
    ...
    common->Dialog().AddDialog(GDM_CORRUPT_CONTINUE, DIALOG_CONTINUE, NULL,
                               NULL, false);

    // Find the game in the enumerated details, mark as corrupt so the menus can
    // show as corrupt
    saveGameDetailsList_t& list =
        session->GetSaveGameManager().GetEnumeratedSavegamesNonConst();
    ...
    return true;
  } else if (parms.GetError() & SAVEGAME_E_UNABLE_TO_SELECT_STORAGE_DEVICE &&
             saveGame_enable.GetBool()) {
    common->Dialog().AddDialog(GDM_UNABLE_TO_USE_SELECTED_STORAGE_DEVICE,
                               DIALOG_CONTINUE, NULL, NULL, false);
    return true;
    ...
    common->Dialog().AddDialog(GDM_DLC_ERROR_MISSING_GENERIC, DIALOG_CONTINUE,
                               NULL, NULL, false);
    return true;
  } else if (parms.GetError() & SAVEGAME_E_DISC_SWAP) {
    common->Dialog().AddDialog(GDM_DISC_SWAP, DIALOG_CONTINUE, NULL, NULL,
                               false);
    return true;
  } else if (parms.GetError() & SAVEGAME_E_INCOMPATIBLE_NEWER_VERSION) {
    common->Dialog().AddDialog(GDM_INCOMPATIBLE_NEWER_SAVE, DIALOG_CONTINUE,
                               NULL, NULL, false);
    return true;
  }
...
void idCommonLocal::OnSaveCompleted(idSaveLoadParms& parms) {
  ...
  if (!HandleCommonErrors(parms)) {
    common->Dialog().AddDialog(GDM_ERROR_SAVING_SAVEGAME, DIALOG_CONTINUE, NULL,
                               NULL, false);
  }
}
...
void idCommonLocal::OnLoadCompleted(idSaveLoadParms& parms) {
  if (!HandleCommonErrors(parms)) {
    common->Dialog().AddDialog(GDM_ERROR_LOADING_SAVEGAME, DIALOG_CONTINUE,
                               NULL, NULL, false);
  }
}
...
void idCommonLocal::OnLoadFilesCompleted(idSaveLoadParms& parms) {
  if ((mapSpawnData.savegameFile != NULL) &&
      (parms.GetError() == SAVEGAME_E_NONE)) {
...
===============
*/
CONSOLE_COMMAND_SHIP(loadGame, "loads a game",
...
==================
*/
CONSOLE_COMMAND_SHIP(devmap, "loads a map in developer mode",
...
==================
*/
CONSOLE_COMMAND_SHIP(netmap, "loads a map in multiplayer mode",
                     idCmdSystem::ArgCompletion_MapName) {
...
==================
*/
CONSOLE_COMMAND(bakemap, "loads a map and bakes environment probes",
                idCmdSystem::ArgCompletion_MapName) {
  idStr map, string;
