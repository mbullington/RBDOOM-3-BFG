...
the base path, it will be copied over to the save path. This is a development
aid to help build test releases and to copy working sets of files.
...
class idFileSystemLocal : public idFileSystem {
 public:
  ...
  virtual idFileList* ListFiles(const char* relativePath, const char* extension,
                                bool sort = false,
  ...
  virtual idFileList* ListFilesTree(const char* relativePath,
                                    ...
                                    bool allowSubdirsForResourcePaks = false,
                                    const char* gamedir = NULL);
  ...
  virtual const char* RelativePathToOSPath(const char* relativePath,
                                           const char* basePath);
  virtual const char* BuildOSPath(const char* base, const char* game,
  ...
  virtual int WriteFile(const char* relativePath, const void* buffer, int size,
  ...
  virtual bool RenameFile(const char* relativePath, const char* newName,
                          const char* basePath = "fs_savepath");
  virtual idFile* OpenFileReadFlags(const char* relativePath, int searchFlags,
                                    bool allowCopyFiles = true,
                                    const char* gamedir = NULL);
  virtual idFile* OpenFileRead(const char* relativePath,
                               bool allowCopyFiles = true,
                               const char* gamedir = NULL);
  virtual idFile* OpenFileReadMemory(const char* relativePath,
                                     bool allowCopyFiles = true,
                                     const char* gamedir = NULL);
  virtual idFile* OpenFileWrite(const char* relativePath,
                                const char* basePath = "fs_savepath");
  virtual idFile* OpenFileAppend(const char* relativePath, bool sync = false,
  ...
  virtual void BeginLevelLoad(const char* name, char* _blockBuffer,
                              int _blockBufferSize);
  ...
  virtual int ReadFromBGL(idFile* _resourceFile, void* _buffer, int _offset,
                          int _len);
  ...
  virtual void AddImagePreload(const char* resName, int _filter, int _repeat,
                               int _usage, int _cube) {
 ...
 private:
  ...
  int ListOSFiles(const char* directory, const char* extension,
                  idStrList& list);
  ...
  int AddUnique(const char* name, idStrList& list,
  ...
  int GetFileList(const char* relativePath, const idStrList& extensions,
                  idStrList& list, idHashIndex& hashIndex,
                  bool fullRelativePath,
                  bool allowSubdirsForResourcePaks = false,
                  const char* gamedir = NULL);
  int GetFileListTree(const char* relativePath, const idStrList& extensions,
                      idStrList& list, idHashIndex& hashIndex,
                      bool allowSubdirsForResourcePaks = false,
                      const char* gamedir = NULL);
...
idCVar idFileSystemLocal::fs_debug("fs_debug", "0", CVAR_SYSTEM | CVAR_INTEGER,
                                   "", 0, 2,
                                   idCmdSystem::ArgCompletion_Integer<0, 2>);
idCVar idFileSystemLocal::fs_debugResources("fs_debugResources", "0",
                                            CVAR_SYSTEM | CVAR_BOOL, "");
idCVar idFileSystemLocal::fs_enableBGL("fs_enableBGL", "0",
                                       CVAR_SYSTEM | CVAR_BOOL, "");
idCVar idFileSystemLocal::fs_debugBGL("fs_debugBGL", "0",
                                      CVAR_SYSTEM | CVAR_BOOL, "");
idCVar idFileSystemLocal::fs_copyfiles(
    "fs_copyfiles", "0", CVAR_SYSTEM | CVAR_INIT | CVAR_BOOL,
    "Copy every file touched to fs_savepath");
idCVar idFileSystemLocal::fs_buildResources(
    "fs_buildresources", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_INIT,
    "Copy every file touched to a resource file");
...
idCVar fs_resourceLoadPriority(
    "fs_resourceLoadPriority", "0", CVAR_SYSTEM,
    "if 1, open requests will be honored from resource files first; if 0, the "
    "resource files are checked after normal search paths");
idCVar fs_enableBackgroundCaching(
    "fs_enableBackgroundCaching", "1", CVAR_SYSTEM,
    "if 1 allow the 360 to precache game files in the background");
...
================
*/
int idFileSystemLocal::ReadFromBGL(idFile* _resourceFile, void* _buffer,
                                   int _offset, int _len) {
...
================
*/
idFileHandle idFileSystemLocal::OpenOSFile(const char* fileName,
                                           fsMode_t mode) {
  idFileHandle fp;
  ...
  {
    // RB: really any proper OS other than Windows should have a case sensitive
    // filesystem
    idStr fpath, entry;
    idStrList list;
...
=================
*/
void idFileSystemLocal::BeginLevelLoad(const char* name, char* _blockBuffer,
                                       int _blockBufferSize) {
  if (name == NULL || *name == '\0') {
    return;
  }
...
bool idFileSystemLocal::InProductionMode() {
  // return fs_resourceLoadPriority.GetBool() && ( resourceFiles.Num() > 0 ) ||
  // ( com_productionMode.GetInteger() != 0 );

  return (com_productionMode.GetInteger() != 0);
}
// RB end

bool FileExistsInAllManifests(const char* filename,
                              idList<idFileManifest>& manifests) {
...
bool FileExistsInAllPreloadManifests(const char* filename,
                                     idList<idPreloadManifest>& manifests) {
  for (int i = 0; i < manifests.Num(); i++) {
    if (strstr(manifests[i].GetManifestName(), "_startup") != NULL) {
      continue;
    }
...
void RemoveFileFromAllManifests(const char* filename,
                                idList<idFileManifest>& manifests) {
  for (int i = 0; i < manifests.Num(); i++) {
    if (strstr(manifests[i].GetManifestName(), "_startup") != NULL) {
      continue;
...
void idFileSystemLocal::AddFonts(idStrList& files) {
  ...
  idFileList* fl =
      ListFilesTree("generated/images/newfonts", "*.bimage", false);
...
const char* excludeExtensions[] = {".idxma", ".idmsf", ".idwav",   ".xma",
                                   ".msf",   ".wav",   ".resource"};
const int numExcludeExtensions =
    sizeof(excludeExtensions) / sizeof(excludeExtensions[0]);
...
bool idFileSystemLocal::IsBinaryModel(const idStr& resName) const {
  ...
  if ((ext.Icmp("base") == 0) || (ext.Icmp("blwo") == 0) ||
      (ext.Icmp("bflt") == 0) || (ext.Icmp("bma") == 0)) {
    return true;
  }
  return false;
}
...
bool idFileSystemLocal::IsSoundSample(const idStr& resName) const {
  idStrStatic<32> ext;
  resName.ExtractFileExtension(ext);
  if ((ext.Icmp("idxma") == 0) || (ext.Icmp("idwav") == 0) ||
      (ext.Icmp("idmsf") == 0) || (ext.Icmp("xma") == 0) ||
      (ext.Icmp("wav") == 0) || (ext.Icmp("msf") == 0) ||
      (ext.Icmp("msadpcm") == 0)) {
    return true;
  }
  return false;
...
void idFileSystemLocal::BuildOrderedStartupContainer() {
  ...
  idResourceContainer::WriteResourceFile("_ordered.resources", orderedFiles,
                                         false);
}
...
void idFileSystemLocal::WriteResourcePacks() {
  idStrList filesNotCommonToAllMaps(
      16384);  // files that are not shared by all maps, used to trim the common
               // list
  idStrList filesCommonToAllMaps(
      16384);  // files that are shared by all maps, will include startup files,
               // renderprogs etc..
  idPreloadManifest
      commonPreloads;  // preload entries that exist in all map preload files
  ...
  for (int i = 0; i < manifestFiles.Num(); i++) {
    ...
    if (manifest.LoadManifest(path)) {
      // manifest.Print();
      manifest.RemoveAll(
          va("strings/%s", ID_LANG_ENGLISH));  // remove all .lang files
  ...
  idList<idPreloadManifest>
      preloadManifests;  // list of all preload manifest files
  ...
  for (int i = 0; i < manifests.Num(); i++) {
    if ((strstr(manifests[i].GetManifestName(), "_startup") != NULL) ||
        (strstr(manifests[i].GetManifestName(), "_pc") != NULL)) {
      continue;
    }
    // idLib::Printf( "%04d referenced files for %s\n", manifests[ i
    // ].GetReferencedFileCount(), manifests[ i ].GetManifestName() );
  ...
  for (int i = 0; i < manifests.Num(); i++) {
    ...
    idStaticList<idStr, 16384> mapFiles;  // map files from the manifest, these
                                          // are static for easy debugging
    idStaticList<idStr, 16384>
        mapFilesTwo;       // accumulates non bimage, bmodel and sample files
    commonImages.Clear();  // collect images and models separately so they can
                           // be added in linear preload order
    commonModels.Clear();
    commonAnims.Clear();
    ...
    idResourceContainer::WriteResourceFile(resourceFileName, mapFilesToWrite,
                                           false);
  }
  ...
  const soundVOInfo_t soundFileInfo[] = {
      ...
      {"en", "sound/vo/", &soundOutputFiles[5]}  // english last so the other
                                                 // langs are culled first
  };
  ...
  for (int k = 0; k < numSoundFiles; k++) {
    ...
    idResourceContainer::WriteResourceFile(
        va("_sound_pc_%s", soundFileInfo[k].filename), sampleList, false);
  ...
  if (strstr(fromOSPath, ".wav") != NULL) {
    ...
    if (newFromPath.Find("/vo/", false) >= 0) {
      for (int i = 0; i < Sys_NumLangs(); i++) {
        ...
        src = OpenExplicitFileRead(newFromPath);
        if (src == NULL) {
          idLib::Warning("LOCALIZATION PROBLEM: Could not open %s for read",
                         newFromPath.c_str());
        } else {
          CopyFile(src, newToPath);
          delete src;
...
===================
*/
const char* idFileSystemLocal::BuildOSPath(const char* base, const char* game,
                                           const char* relativePath) {
...
const char* idFileSystemLocal::OSPathToRelativePath(const char* OSPath) {
  if ((OSPath[0] != '/') && (OSPath[0] != '\\') &&
      (idStr::FindChar(OSPath, ':') < 0)) {
    // No colon and it doesn't start with a slash... it must already be a
    // relative path
    return OSPath;
  }
  ...
  for (int n = 0; n < slashes.Num() - 1; n++) {
    ...
    for (int i = 0; i < basePaths.Num(); i++) {
      ...
      if (basePaths[i].Icmpn(start, componentLength) == 0) {
        ...
        // So instead we check for 2 base paths next to each other and take the
        // 2nd in that case
        if (n < slashes.Num() - 2) {
...
============
*/
int idFileSystemLocal::WriteFile(const char* relativePath, const void* buffer,
...
bool idFileSystemLocal::RenameFile(const char* relativePath,
                                   const char* newName, const char* basePath) {
  const char* path = cvarSystem->GetCVarString(basePath);
  if (!path[0]) {
    path = fs_savepath.GetString();
  }
  ...
  // There is a MoveFileTransacted() on vista and above, not sure if that means
  // there is a race condition inside MoveFileEx...
  const bool success = (MoveFileEx(oldOSPath.c_str(), newOSPath.c_str(),
                                   MOVEFILE_REPLACE_EXISTING) != 0);

  if (!success) {
    const int err = GetLastError();
    idLib::Warning("RenameFile( %s, %s ) error %i", newOSPath.c_str(),
                   oldOSPath.c_str(), err);
  }
#else
  ...
  if (!success) {
    const int err = errno;
    idLib::Warning("rename( %s, %s ) error %s", newOSPath.c_str(),
                   oldOSPath.c_str(), strerror(errno));
  }
#endif
  // RB end
...
===============
*/
int idFileSystemLocal::AddUnique(const char* name, idStrList& list,
                                 idHashIndex& hashIndex) const {
  int i, hashKey;
...
int idFileSystemLocal::GetFileList(const char* relativePath,
                                   const idStrList& extensions, idStrList& list,
                                   idHashIndex& hashIndex,
                                   bool fullRelativePath,
                                   bool allowSubdirsForResourcePaks,
                                   const char* gamedir) {
  if (!IsInitialized()) {
    common->FatalError("Filesystem call made without initialization\n");
  }
  ...
  if (resourceFiles.Num() > 0) {
    ...
    while (idx >= 0) {
      for (int i = 0; i < resourceFiles[idx]->cacheTable.Num(); i++) {
        ...
        if (pathLength &&
            idStr::Icmpn(rt.filename, relativePath, pathLength - 1) != 0) {
          continue;
        }

        // ensure we have a path, and not just a filename containing the path
        if (rt.filename[pathLength] == '\0' ||
            (pathLength && rt.filename[pathLength - 1] != '/')) {
          continue;
        }
        ...
        for (j = 0; j < extensions.Num(); j++) {
          if (rt.filename.Length() >= extensions[j].Length() &&
              extensions[j].Icmp(rt.filename.c_str() + rt.filename.Length() -
                                 extensions[j].Length()) == 0) {
            break;
          }
        }
        if (j >= extensions.Num()) {
  ...
  for (int sp = searchPaths.Num() - 1; sp >= 0; sp--) {
    ...
    idStr netpath = BuildOSPath(searchPaths[sp].path, searchPaths[sp].gamedir,
                                relativePath);
...
idFileList* idFileSystemLocal::ListFiles(const char* relativePath,
                                         const char* extension, bool sort,
                                         bool fullRelativePath,
                                         const char* gamedir) {
  ...
  GetFileList(relativePath, extensionList, fileList->list, hashIndex,
              fullRelativePath, false, gamedir);
...
int idFileSystemLocal::GetFileListTree(const char* relativePath,
  ...
  slash.Append("/");
  GetFileList(relativePath, slash, folders, folderHashIndex, true,
              allowSubdirsForResourcePaks, gamedir);
  for (i = 0; i < folders.Num(); i++) {
    ...
    GetFileListTree(folders[i], extensions, list, hashIndex,
                    allowSubdirsForResourcePaks, gamedir);
  }

  // list files in the current directory
  GetFileList(relativePath, extensions, list, hashIndex, true,
              allowSubdirsForResourcePaks, gamedir);
...
===============
*/
void idFileSystemLocal::FreeFileList(idFileList* fileList) { delete fileList; }
...
===============
*/
int idFileSystemLocal::ListOSFiles(const char* directory, const char* extension,
                                   idStrList& list) {
  if (!extension) {
    extension = "";
  }
...
void idFileSystemLocal::ExtractResourceFile_f(const idCmdArgs& args) {
  if (args.Argc() < 3) {
    common->Printf(
        "Usage: extractResourceFile <resource file> <outpath> <copysound>\n");
    return;
  }
...
void idFileSystemLocal::Path_f(const idCmdArgs& args) {
  ...
  for (int sp = fileSystemLocal.searchPaths.Num() - 1; sp >= 0; sp--) {
    common->Printf("%s/%s\n", fileSystemLocal.searchPaths[sp].path.c_str(),
                   fileSystemLocal.searchPaths[sp].gamedir.c_str());
  }
...
void idFileSystemLocal::CreateCRCsForResourceFileList(const idFileList& list) {
  for (int fileIndex = 0; fileIndex < list.GetNumFiles(); ++fileIndex) {
    ...
    std::unique_ptr<idFile_Memory> currentFile(static_cast<idFile_Memory*>(
        fileSystem->OpenFileReadMemory(list.GetFile(fileIndex))));
...
void idFileSystemLocal::AddGameDirectory(const char* path, const char* dir) {
  ...
  for (int i = 0; i < searchPaths.Num(); i++) {
    if (searchPaths[i].path.Cmp(path) == 0 &&
        searchPaths[i].gamedir.Cmp(dir) == 0) {
      return;
    }
  }
...
void idFileSystemLocal::Startup() {
  ...
  cmdSystem->AddCommand("buildGame", BuildGame_f, CMD_FL_SYSTEM,
                        "builds game pak files");
  cmdSystem->AddCommand("writeResourceFile", WriteResourceFile_f, CMD_FL_SYSTEM,
                        "writes a .resources file from a supplied manifest");
  cmdSystem->AddCommand(
      "extractResourceFile", ExtractResourceFile_f, CMD_FL_SYSTEM,
      "extracts to the supplied resource file to the supplied path");
  cmdSystem->AddCommand(
...
========================
*/
bool idFileSystemLocal::GetResourceCacheEntry(const char* fileName,
                                              idResourceCacheEntry& rc) {
  ...
  while (idx >= 0) {
    ...
    for (int index = resourceFiles[idx]->cacheHash.GetFirst(key);
         index != idHashIndex::NULL_INDEX;
         index = resourceFiles[idx]->cacheHash.GetNext(index)) {
...
idFile* idFileSystemLocal::GetResourceFile(const char* fileName, bool memFile) {
  ...
  if (GetResourceCacheEntry(fileName, rc)) {
    ...
    idFile_InnerResource* file = new idFile_InnerResource(
        rc.filename, resourceFiles[rc.containerIndex]->resourceFile, rc.offset,
        rc.length);
    // DG: add parenthesis to make sure this block is only entered when file !=
    // NULL - bug found by clang.
    if (file != NULL && ((memFile || rc.length <= resourceBufferAvailable) ||
                         rc.length < 8 * 1024 * 1024)) {
      byte* buf = NULL;
      ...
      if (buf == resourceBufferPtr) {
        ...
        idFile_Memory* mfile =
            new idFile_Memory(rc.filename, (const char*)buf, rc.length);
        if (mfile != NULL) {
...
idFile* idFileSystemLocal::OpenFileReadFlags(const char* relativePath,
  ...
  if (resourceFiles.Num() > 0 && fs_resourceLoadPriority.GetInteger() == 1) {
    idFile* rf = GetResourceFile(relativePath,
                                 (searchFlags & FSFLAG_RETURN_FILE_MEM) != 0);
    if (rf != NULL) {
      return rf;
    }
  }
    ...
    for (int sp = searchPaths.Num() - 1; sp >= 0; sp--) {
      ...
      if (fs_debug.GetInteger()) {
        common->Printf("idFileSystem::OpenFileRead: %s (found in '%s/%s')\n",
                       relativePath, searchPaths[sp].path.c_str(),
                       searchPaths[sp].gamedir.c_str());
      }
      ...
      if (allowCopyFiles) {
        idStr copypath;
        idStr name;
        copypath = BuildOSPath(fs_savepath.GetString(), searchPaths[sp].gamedir,
                               relativePath);
...
===========
*/
idFile* idFileSystemLocal::OpenFileReadMemory(const char* relativePath,
                                              bool allowCopyFiles,
                                              const char* gamedir) {
...
===========
*/
idFile* idFileSystemLocal::OpenFileAppend(const char* relativePath, bool sync,
                                          const char* basePath) {
  const char* path;
  idStr OSpath;
  idFile_Permanent* f;
