...
class OBJExporter {
 public:
  ...
  void ConvertBrushToOBJ(OBJGroup& group, const idMapBrush* mapBrush,
                         int entityNum, int primitiveNum,
                         const idMat4& transform);
  void ConvertPatchToOBJ(OBJGroup& group, const idMapPatch* patch,
                         int entityNum, int primitiveNum,
                         const idMat4& transform);
  void ConvertMeshToOBJ(OBJGroup& group, const MapPolygonMesh* mesh,
                        int entityNum, int primitiveNum,
                        const idMat4& transform);
    ...
    for (int o = 0; o < group.objects.Num(); o++) {
      ...
      for (int i = 0; i < geometry.faces.Num(); i++) {
        ...
        for (int j = face.indexes.Num() - 1; j >= 0; j--) {
          objFile->Printf("%i/%i/%i ", face.indexes[j] + 1 + totalVerts,
...
void OBJExporter::ConvertBrushToOBJ(OBJGroup& group, const idMapBrush* mapBrush,
                                    int entityNum, int primitiveNum,
                                    const idMat4& transform) {
  ...
  for (int i = 0; i < mapBrush->GetNumSides(); i++) {
    ...
    for (int j = 0; j < w.GetNumPoints(); j++) {
      ...
      if ((mapSide->GetProjectionType() ==
           idMapBrushSide::PROJECTION_VALVE220)) {
        const idMaterial* material =
            declManager->FindMaterial(mapSide->GetMaterial());
...
void OBJExporter::ConvertPatchToOBJ(OBJGroup& group, const idMapPatch* patch,
                                    int entityNum, int primitiveNum,
                                    const idMat4& transform) {
  OBJExporter::OBJObject& geometry = group.objects.Alloc();
...
void OBJExporter::ConvertMeshToOBJ(OBJGroup& group, const MapPolygonMesh* mesh,
                                   int entityNum, int primitiveNum,
                                   const idMat4& transform) {
  OBJExporter::OBJObject& geometry = group.objects.Alloc();
...
CONSOLE_COMMAND(exportMapToOBJ, "Convert .map file to .obj/.mtl ",
        ...
        if (ent->GetNumPrimitives()) {
          ...
          for (int i = 0; i < ent->GetNumPrimitives(); i++) {
            ...
            if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
              exporter.ConvertBrushToOBJ(
                  group, static_cast<idMapBrush*>(mapPrim), j, i, transform);
              continue;
            }

            if (mapPrim->GetType() == idMapPrimitive::TYPE_PATCH) {
              exporter.ConvertPatchToOBJ(
                  group, static_cast<idMapPatch*>(mapPrim), j, i, transform);
              continue;
            }

            if (mapPrim->GetType() == idMapPrimitive::TYPE_MESH) {
              exporter.ConvertMeshToOBJ(group,
                                        static_cast<MapPolygonMesh*>(mapPrim),
                                        j, i, transform);
              continue;
            }
          }
        ...
        *list )[k] == "location" )
                        ...
                        if( val.Length() && TestMapVal( val ) )
                        {
                                ...
                                if( !hasLocation || ( *list )[k] == "location" )
                                {
                                        //Localize it!!!
                                        strCount++;
                                        ent->epairs.Set( ( *list )[k],
        langDict.AddString( val ) );
                                }
                        }
                }
        }
...
CONSOLE_COMMAND(
    convertMap,
    "Convert .map file to new map format with polygons instead of brushes",
    idCmdSystem::ArgCompletion_MapNameNoJson) {
  common->SetRefreshOnPrint(true);
...
CONSOLE_COMMAND(convertMapToValve220,
                "Convert .map file to the Valve 220 map format for TrenchBroom",
                idCmdSystem::ArgCompletion_MapNameNoJson) {
  common->SetRefreshOnPrint(true);
...
CONSOLE_COMMAND(convertMapQuakeToDoom,
                "Convert Quake .map in Valve 220 map format for Doom 3 BFG",
                idCmdSystem::ArgCompletion_MapNameNoJson) {
  common->SetRefreshOnPrint(true);
