...
class idCommonLocal : public idCommon {
 ...
 public:
  ...
  virtual void BeginRedirect(char* buffer, int buffersize,
 ...
 public:
  ...
  uint64 GetRendererEndFrameSyncMicroseconds() const {
    return mainFrameTiming.finishSyncTime_EndFrame -
           mainFrameTiming.startRenderTime;
  }
  // SRS end
  ...
  // for images in particular we can measure more accurately this way (to deal
  // with mipmaps)
 ...
 public:  // These are public because they are called directly by static
 ...
 private:
  ...
  // This additional information is required for ExecuteMapChange for SP games
  // ONLY This data is cleared after ExecuteMapChange
  struct mapSpawnData_t {
    idFile_SaveGame* savegameFile;  // Used for loading a save game
    idFile_SaveGame*
        stringTableFile;  // String table read from save game loaded
    idFile_SaveGamePipelined* pipelineFile;
  ...
  int wipeStopTime;
  bool wipeHold;
  bool wipeForced;  // used for the PS3 to start an early wipe while we are
                    // accessing saved game data
  ...
  uint64 time_shadows;      // renderer backend waiting for shadow volumes to be
                            // created
  ...
  // foresthale 2014-05-30: a special binarize pacifier has to be shown in some
  // cases, which includes filename and ETA information
