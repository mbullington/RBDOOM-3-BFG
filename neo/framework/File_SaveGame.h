...
enum saveGameType_t {
  ...
  SAVEGAMEFILE_BKGRND_IMAGE =
      BIT(5),  // for special processing on certain platforms, large background
               // used on PS3
  SAVEGAMEFILE_AUTO_DELETE =
      BIT(6),  // to be deleted automatically after completed
  SAVEGAMEFILE_OPTIONAL =
      BIT(7)  // if this flag is not set and missing, there is an error
};
...
class idFile_SaveGame : public idFile_Memory {
 public:
  ...
  idFile_SaveGame(const char* _name)
      : idFile_Memory(_name), type(SAVEGAMEFILE_NONE), error(false) {}
  idFile_SaveGame(const char* _name, int type_)
      : idFile_Memory(_name), type(type_), error(false) {}
 ...
 public:       // TODO_KC_CR for now...
  int type;    // helps platform determine what to do with the file (encrypt,
               // checksum, etc.)
...
class idFile_SaveGamePipelined : public idFile {
 public:
  // The buffers each hold two blocks of data, so one block can be operated on
  // by the next part of the generate / compress / IO pipeline.  The factor of
  // two size difference between the uncompressed and compressed blocks is
  // unrelated to the fact that there are two blocks in each buffer.
  ...
  // Called by a background thread to get the next block to read data into and
  // to report the number of bytes written to the previous block. This may block
  // until space is available to place the next block. Pass in NULL to notify
  // the end of the file was reached. Returns false if there are no more blocks.
 ...
 private:
  ...
  //------------------------
  // These variables are used to pass data between threads in a thread-safe
  // manner.
  //------------------------
  ...
  //------------------------
  // The background threads and signals for NextWriteBlock() and
  // NextReadBlock().
  //------------------------
  ...
  int16
      pointerSize;  // the number of bytes in a pointer, because different
                    // pointer sizes mean different offsets into objects a 64
                    // bit build cannot load games saved from a 32 bit build or
                    // vice version (a value of 0 is interpreted as 4 bytes)
  int16 saveFormatVersion;  // version number specific to save games (for
                            // maintaining save compatibility across builds)

  //------------------------
  // These variables are used when we want to abort due to the termination of
  // the application
  //------------------------
