...
void idDebugGraph::Render(idRenderSystem* gui) {
  ...
  if (sideways) {
    ...
    for (int i = 0; i < bars.Num(); i++) {
      ...
      if (mode == GRAPH_LINE) {
        rect.Set(barLeft + barWidth * bars[i].value, barTop + i * barHeight,
                 1.0f, barHeight - border);
      } else if (mode == GRAPH_FILL) {
        rect.Set(barLeft, barTop + i * barHeight, barWidth * bars[i].value,
                 barHeight - border);
      } else if (mode == GRAPH_FILL_REVERSE) {
        rect.Set(barLeft + barWidth, barTop + i * barHeight,
                 barWidth - barWidth * bars[i].value, barHeight - border);
      }
    ...
    if (labels.Num() > 0) {
      ...
      for (int i = 0; i < labels.Num(); i++) {
        ...
        gui->DrawSmallStringExt(idMath::Ftoi(pos.x), idMath::Ftoi(pos.y),
                                labels[i], fontColor, true);
      }
    }
  } else {
    ...
    for (int i = 0; i < grid.Num(); i++) {
      idVec4 rect(position.x, barBottom - barHeight * grid[i].value, position.z,
                  1.0f);
    ...
    for (int i = 0; i < bars.Num(); i++) {
      idVec4 rect;
      if (mode == GRAPH_LINE) {
        rect.Set(barLeft + i * barWidth, barBottom - barHeight * bars[i].value,
                 barWidth - border, 1.0f);
      } else if (mode == GRAPH_FILL) {
        rect.Set(barLeft + i * barWidth, barBottom - barHeight * bars[i].value,
                 barWidth - border, barHeight * bars[i].value);
      } else if (mode == GRAPH_FILL_REVERSE) {
        rect.Set(barLeft + i * barWidth, barTop, barWidth - border,
                 barHeight * bars[i].value);
      }
      gui->DrawFilled(bars[i].color, rect.x, rect.y, rect.z, rect.w);
    }
    if (labels.Num() > 0) {
