...
static const unsigned int BPRT_MAGIC =
    ('B' << 24) | ('P' << 16) | ('R' << 8) | BPRT_VERSION;
...
=================
*/
size_t idDeclParticle::Size() const { return sizeof(idDeclParticle); }
...
bool idDeclParticle::Parse(const char* text, const int textLength,
  ...
  // don't calculate bounds or write binary files for defaulted ( non-existent )
  // particles in resource builds
...
bool idDeclParticle::LoadBinary(idFile* file, unsigned int checksum) {
  ...
  struct local {
    static void LoadParticleParm(idFile* file, idParticleParm& parm) {
      ...
      if (name.IsEmpty()) {
        ...
        parm.table =
            (idDeclTable*)declManager->FindType(DECL_TABLE, name, false);
      }
  ...
  for (int i = 0; i < numStages; i++) {
    ...
    file->ReadBigArray(
        s->distributionParms,
        sizeof(s->distributionParms) / sizeof(s->distributionParms[0]));
    file->ReadBig(s->directionType);
    file->ReadBigArray(s->directionParms, sizeof(s->directionParms) /
                                              sizeof(s->directionParms[0]));
    ...
    file->ReadBigArray(s->customPathParms, sizeof(s->customPathParms) /
                                               sizeof(s->customPathParms[0]));
    ...
    file->ReadBigArray(s->orientationParms, sizeof(s->orientationParms) /
                                                sizeof(s->orientationParms[0]));
...
void idDeclParticle::WriteBinary(idFile* file, unsigned int checksum) {
  ...
  for (int i = 0; i < stages.Num(); i++) {
    ...
    file->WriteBigArray(s->directionParms, sizeof(s->directionParms) /
                                               sizeof(s->directionParms[0]));
    ...
    file->WriteBigArray(s->customPathParms, sizeof(s->customPathParms) /
                                                sizeof(s->customPathParms[0]));
    ...
    file->WriteBigArray(
        s->orientationParms,
        sizeof(s->orientationParms) / sizeof(s->orientationParms[0]));
...
================
*/
void idDeclParticle::FreeData() { stages.DeleteContents(true); }
...
================
*/
void idDeclParticle::WriteParticleParm(idFile* f, idParticleParm* parm,
                                       const char* name) {
...
==================
*/
int idParticleStage::ParticleVerts(particleGen_t* g, idVec3 origin,
