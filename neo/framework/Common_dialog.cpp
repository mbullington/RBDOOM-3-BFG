...
idCVar popupDialog_debug("popupDialog_debug", "0", CVAR_BOOL | CVAR_ARCHIVE,
                         "display debug spam");
...
idCVar dialog_saveClearLevel("dialog_saveClearLevel", "1000", CVAR_INTEGER,
                             "Time required to show long message");

/*
========================
bool DialogMsgShouldWait

There are a few dialog types that should pause so the user has the ability to
...
================================================
*/
void idCommonDialog::AddDialog(gameDialogMessages_t msg, dialogType_t type,
                               idSWFScriptFunction* acceptCallback,
                               idSWFScriptFunction* cancelCallback, bool pause,
                               const char* location, int lineNumber,
                               bool leaveOnMapHeapReset, bool waitOnAtlas,
  ...
  idLib::PrintfIf(popupDialog_debug.GetBool(),
                  "[%s] msg: %s, pause: %d from: %s:%d\n", __FUNCTION__,
                  dialogStateToString[msg], pause,
                  location == NULL ? "NULL" : location, lineNumber);
...
========================
*/
void idCommonDialog::AddDynamicDialog(
    gameDialogMessages_t msg,
    const idStaticList<idSWFScriptFunction*, 4>& callbacks,
    const idStaticList<idStrId, 4>& optionText, bool pause,
    idStrStatic<256> overrideMsg, bool leaveOnMapHeapReset, bool waitOnAtlas,
    bool renderDuringLoad) {
  if (dialog == NULL) {
    return;
  }
...
void idCommonDialog::AddDialogInternal(idDialogInfo& info) {
  // don't add the dialog if it's already in the list, we never want to show a
  // duplicate dialog
  if (HasDialogMsg(info.msg, NULL)) {
    return;
  }

  // Remove the delete confirmation if we remove the device and ask for a
  ...
  if (messageList.Num() == 0) {
    ...
    if (messageList.Num() > 0) {
      idLib::PrintfIf(popupDialog_debug.GetBool(),
                      "[%s] msg: %s new dialog added over old\n", __FUNCTION__,
  ...
  if (info.type == DIALOG_QUICK_SAVE || info.type == DIALOG_CRAWL_SAVE ||
      messageList[0].msg == GDM_CALCULATING_BENCHMARK) {
    ShowNextDialog();
  }
}
...
void idCommonDialog::ShowDialog(const idDialogInfo& info) {
  idLib::PrintfIf(
      popupDialog_debug.GetBool(),
      "[%s] msg: %s, m.clear = %d, m.waitClear = %d, m.killTime = %d\n",
      __FUNCTION__, dialogStateToString[info.msg], info.clear, info.waitClear,
      info.killTime);

  // here instead of add dialog to make sure we meet the TCR, otherwise it has a
  // chance to be visible for less than 1 second
  ...
  if (info.acceptCB == NULL &&
      (info.type != DIALOG_WAIT && info.type != DIALOG_WAIT_BLACKOUT)) {
    class idSWFScriptFunction_Accept : public idSWFScriptFunction_RefCounted {
     public:
      idSWFScriptFunction_Accept(gameDialogMessages_t _msg) { msg = _msg; }
    ...
    dialog->SetGlobal("acceptCallBack",
                      new (TAG_SWF) idSWFScriptFunction_Accept(info.msg));
...
void idCommonDialog::ShowSaveIndicator(bool show) {
  ...
  if (show) {
    ...
    common->Dialog().AddDialog(GDM_SAVING, DIALOG_WAIT, NULL, NULL, true, "", 0,
                               false, true, true);
  } else {
...
From TCR# 047
Games must display a message during storage writes for the following conditions
and the respective amount of time:
- Writes longer than one second require the standard message be displayed for
three seconds.
- Writes longer than three seconds require the standard message be displayed for
the length of the write.
- Writes that last one second or less require the shorter message be displayed
for one second or the standard message for three seconds.
...
void idCommonDialog::RemoveWaitDialogs() {
  ...
  for (int index = 0; index < messageList.Num(); ++index) {
    if (DialogMsgShouldWait(messageList[index].msg)) {
      if (Sys_Milliseconds() >= messageList[index].killTime &&
          messageList[index].waitClear) {
...
================================================
*/
void idCommonDialog::ClearDialog(gameDialogMessages_t msg, const char* location,
                                 int lineNumber) {
  bool topMessageCleared = false;
  ...
  for (int index = 0; index < messageList.Num(); ++index) {
    ...
    if (info.msg == msg && !info.clear) {
      if (DialogMsgShouldWait(info.msg)) {
        // you can have 2 saving dialogs simultaneously, if you clear
        // back-to-back, we need to let the 2nd dialog get the clear message
        ...
        if (timeShown < dialog_saveClearLevel.GetInteger()) {
          messageList[index].killTime =
              Sys_Milliseconds() +
              (dialog_saveClearLevel.GetInteger() - timeShown);
      ...
      assert(info.msg >= GDM_INVALID &&
             info.msg < GDM_MAX);  // not sure why /analyze complains about this
      idLib::PrintfIf(popupDialog_debug.GetBool(),
                      "[%s] msg: %s, from: %s:%d, topMessageCleared = %d, "
                      "m.clear = %d, m.waitClear = %d, m.killTime = %d\n",
...
void idCommonDialog::Render(bool loading) {
  ...
  for (int index = 0; index < messageList.Num(); ++index) {
    if (messageList[index].clear) {
      idLib::PrintfIf(popupDialog_debug.GetBool(), "[%s] removing %s\n",
                      __FUNCTION__,
                      dialogStateToString[messageList[index].msg]);
      ReleaseCallBacks(index);
      messageList.RemoveIndex(index);
      index--;
    } else {
  ...
  if (messageList.Num() > 0 &&
    ...
    if (timeRemaining <= 0) {
      ...
      dialog->SetGlobal("countdownInfo",
                        va(txtTime.GetLocalizedString(), timeRemaining));
    }
  }

  if (messageList.Num() > 0 && loading &&
      (messageList[0].renderDuringLoad == false)) {
    return;
  }
...
================================================
*/
idStr idCommonDialog::GetDialogMsg(gameDialogMessages_t msg, idStr& message,
                                   idStr& title) {
  message = "#str_dlg_pc_";
...
CONSOLE_COMMAND(testShowDialog, "show a dialog", 0) {
  ...
  common->Dialog().AddDialog((gameDialogMessages_t)dialogId, DIALOG_ACCEPT,
                             NULL, NULL, false);
}
...
CONSOLE_COMMAND(testShowDialogBug, "show a dynamic dialog", 0) {
  ...
  // This locks the game because it thinks it's paused because we're passing in
  // pause = true but the dialog isn't ever added because of the abuse of
  // dialog->isActive when the save indicator is shown.
  int dialogId = atoi(args.Argv(1));
  common->Dialog().AddDialog((gameDialogMessages_t)dialogId, DIALOG_ACCEPT,
                             NULL, NULL, true);
}
