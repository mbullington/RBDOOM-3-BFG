...
class idConsoleLocal : public idConsole {
 public:
  ...
  virtual void PrintOverlay(idOverlayHandle& handle, justify_t justify,
                            const char* text, ...);
...
void idConsoleLocal::DrawTextRightAlign(float x, float& y, const char* text,
  ...
  va_end(argptr);
  renderSystem->DrawSmallStringExt(x - i * SMALLCHAR_WIDTH, y + 2, string,
                                   colorWhite, true);
  y += SMALLCHAR_HEIGHT + 4;
}
...
float idConsoleLocal::DrawFPS(float y) {
  ...
  bool renderImGuiPerfWindow =
      ImGuiHook::IsReadyToRender() && (com_showFPS.GetInteger() > 1);
  ...
  previousTimesNormalized[index % FPS_FRAMES_HISTORY] =
      frameTime / milliSecondsPerFrame;
  ...
  if (index > FPS_FRAMES) {
    ...
    if (com_showFPS.GetInteger() == 1) {
      renderSystem->DrawBigStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                     s, colorWhite, true);
    }
  }
  ...
  // SRS - Shouldn't use these getters since they access and return int-sized
  // variables measured in milliseconds
  ...
  const uint64 gameThreadTotalTime =
      commonLocal.mainFrameTiming.finishDrawTime -
      commonLocal.mainFrameTiming.startGameTime;
  const uint64 gameThreadGameTime = commonLocal.mainFrameTiming.finishGameTime -
                                    commonLocal.mainFrameTiming.startGameTime;
  const uint64 gameThreadRenderTime =
      commonLocal.mainFrameTiming.finishDrawTime -
      commonLocal.mainFrameTiming.finishGameTime;

  const uint64 rendererBackEndTime =
      commonLocal.GetRendererBackEndMicroseconds();
  const uint64 rendererShadowsTime =
      commonLocal.GetRendererShadowsMicroseconds();
  // SRS - GPU idle time calculation depends on whether game is operating in smp
  // mode or not
  const uint64 rendererGPUIdleTime =
      commonLocal.GetRendererIdleMicroseconds() -
      (com_smp.GetInteger() > 0 && com_editors == 0 ? 0 : gameThreadTotalTime);
  const uint64 rendererGPUTime = commonLocal.GetRendererGPUMicroseconds();
  const uint64 rendererGPUEarlyZTime =
      commonLocal.GetRendererGpuEarlyZMicroseconds();
  const uint64 rendererGPU_SSAOTime =
      commonLocal.GetRendererGpuSSAOMicroseconds();
  const uint64 rendererGPU_SSRTime =
      commonLocal.GetRendererGpuSSRMicroseconds();
  const uint64 rendererGPUAmbientPassTime =
      commonLocal.GetRendererGpuAmbientPassMicroseconds();
  const uint64 rendererGPUInteractionsTime =
      commonLocal.GetRendererGpuInteractionsMicroseconds();
  const uint64 rendererGPUShaderPassesTime =
      commonLocal.GetRendererGpuShaderPassMicroseconds();
  const uint64 rendererGPUPostProcessingTime =
      commonLocal.GetRendererGpuPostProcessingMicroseconds();
  const int maxTime = int(1000 / com_engineHz_latched) * 1000;

  // SRS - Get GPU sync time at the start of a frame (com_smp = 1 or 0) and at
  // the end of a frame (com_smp = -1)
  const uint64 rendererStartFrameSyncTime =
      commonLocal.GetRendererStartFrameSyncMicroseconds();
  const uint64 rendererEndFrameSyncTime =
      commonLocal.GetRendererEndFrameSyncMicroseconds();

  // SRS - Total CPU and Frame time calculations depend on whether game is
  // operating in smp mode or not
  const uint64 totalCPUTime =
      (com_smp.GetInteger() > 0 && com_editors == 0
           ? std::max(gameThreadTotalTime, rendererBackEndTime)
           : gameThreadTotalTime + rendererBackEndTime);
  const uint64 totalFrameTime =
      (com_smp.GetInteger() > 0 && com_editors == 0
           ? std::max(gameThreadTotalTime, rendererEndFrameSyncTime)
           : gameThreadTotalTime + rendererEndFrameSyncTime) +
      rendererStartFrameSyncTime;
  ...
  if (ImGuiHook::IsReadyToRender()) {
    ...
    ImGui::TextColored(colorCyan, "API: %s, AA[%i, %i]: %s, %s", API, width,
                       height, aaMode, resolutionText.c_str());

    ImGui::TextColored(colorLtGrey,
                       "GENERAL: views:%i draws:%i tris:%i (shdw:%i)",
                       commonLocal.stats_frontend.c_numViews,
                       commonLocal.stats_backend.c_drawElements +
                           commonLocal.stats_backend.c_shadowElements,
                       (commonLocal.stats_backend.c_drawIndexes +
    ...
    if (com_showFPS.GetInteger() > 2) {
      ImGui::TextColored(colorLtGrey,
                         "DYNAMIC: callback:%-2i md5:%i dfrmVerts:%i "
                         "dfrmTris:%i tangTris:%i guis:%i",
      ...
      ImGui::TextColored(
          colorLtGrey,
          "viewEntities:%-3i  shadowEntities:%-3i  viewLights:%i\n",
      ...
      ImGui::TextColored(colorLtGrey,
                         "UPDATES: entityUpdates:%-3i  entityRefs:%-3i  "
                         "lightUpdates:%-2i  lightRefs:%i\n",
                         commonLocal.stats_frontend.c_entityUpdates,
                         commonLocal.stats_frontend.c_entityReferences,
                         commonLocal.stats_frontend.c_lightUpdates,
                         commonLocal.stats_frontend.c_lightReferences);
    }
    ...
    if (com_showFPS.GetInteger() > 2) {
      ...
      ImGui::PlotLines("Relative\nFrametime ms", previousTimesNormalized,
                       FPS_FRAMES_HISTORY, valuesOffset, overlay, -10.0f, 10.0f,
                       ImVec2(0, 50));
    } else {
      ImGui::TextColored(fps < com_engineHz_latched ? colorRed : colorYellow,
                         "Average FPS %i", fps);
    }
    ...
    ImGui::TextColored(gameThreadTotalTime > maxTime ? colorRed : colorWhite,
                       "Game+RF: %5llu us   EarlyZ:       %5llu us",
                       gameThreadTotalTime, rendererGPUEarlyZTime);
    ImGui::TextColored(gameThreadGameTime > maxTime ? colorRed : colorWhite,
                       "Game:    %5llu us   SSAO:         %5llu us",
                       gameThreadGameTime, rendererGPU_SSAOTime);
    ImGui::TextColored(gameThreadRenderTime > maxTime ? colorRed : colorWhite,
                       "RF:      %5llu us   SSR:          %5llu us",
                       gameThreadRenderTime, rendererGPU_SSRTime);
    ImGui::TextColored(rendererBackEndTime > maxTime ? colorRed : colorWhite,
                       "RB:      %5llu us   AmbientPass:  %5llu us",
                       rendererBackEndTime, rendererGPUAmbientPassTime);
    ImGui::TextColored(rendererShadowsTime > maxTime ? colorRed : colorWhite,
                       "Shadows: %5llu us   Interactions: %5llu us",
                       rendererShadowsTime, rendererGPUInteractionsTime);
    ImGui::TextColored(
        rendererGPUShaderPassesTime > maxTime ? colorRed : colorWhite,
        "                    ShaderPass:   %5llu us",
        rendererGPUShaderPassesTime);
    ImGui::TextColored(
        rendererGPUPostProcessingTime > maxTime ? colorRed : colorWhite,
        "                    PostFX:       %5llu us",
        rendererGPUPostProcessingTime);
    ImGui::TextColored(totalCPUTime > maxTime || rendererGPUTime > maxTime
                           ? colorRed
                           : colorWhite,
                       "Total:   %5llu us   Total:        %5llu us",
                       totalCPUTime, rendererGPUTime);
    ImGui::TextColored(totalFrameTime > maxTime ? colorRed : colorWhite,
                       "Frame:   %5llu us   Idle:         %5llu us",
                       totalFrameTime, rendererGPUIdleTime);
  ...
  renderSystem->DrawBigStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                 resolutionText.c_str(), colorWhite, true);

  y += SMALLCHAR_HEIGHT + 4;
  idStr timeStr;
  timeStr.Format("%sG+RF: %4d",
                 gameThreadTotalTime > maxTime ? S_COLOR_RED : "",
                 gameThreadTotalTime);
  w = timeStr.LengthWithoutColors() * SMALLCHAR_WIDTH;
  renderSystem->DrawSmallStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                   timeStr.c_str(), colorWhite, false);
  y += SMALLCHAR_HEIGHT + 4;

  timeStr.Format("%sG: %4d", gameThreadGameTime > maxTime ? S_COLOR_RED : "",
                 gameThreadGameTime);
  w = timeStr.LengthWithoutColors() * SMALLCHAR_WIDTH;
  renderSystem->DrawSmallStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                   timeStr.c_str(), colorWhite, false);
  ...
  timeStr.Format("%sRF: %4d", gameThreadRenderTime > maxTime ? S_COLOR_RED : "",
                 gameThreadRenderTime);
  w = timeStr.LengthWithoutColors() * SMALLCHAR_WIDTH;
  renderSystem->DrawSmallStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                   timeStr.c_str(), colorWhite, false);
  y += SMALLCHAR_HEIGHT + 4;

  timeStr.Format("%sRB: %4.1f",
                 rendererBackEndTime > maxTime * 1000 ? S_COLOR_RED : "",
  ...
  timeStr.Format("%sSV: %4.1f",
                 rendererShadowsTime > maxTime * 1000 ? S_COLOR_RED : "",
  ...
  timeStr.Format("%sIDLE: %4.1f",
                 rendererGPUIdleTime > maxTime * 1000 ? S_COLOR_RED : "",
  ...
  timeStr.Format("%sGPU: %4.1f",
                 rendererGPUTime > maxTime * 1000 ? S_COLOR_RED : "",
                 rendererGPUTime / 1000.0f);
  w = timeStr.LengthWithoutColors() * SMALLCHAR_WIDTH;
  renderSystem->DrawSmallStringExt(LOCALSAFE_RIGHT - w, idMath::Ftoi(y) + 2,
                                   timeStr.c_str(), colorWhite, false);
...
==================
*/
float idConsoleLocal::DrawMemoryUsage(float y) { return y; }
...
================
*/
bool idConsoleLocal::Active() { return keyCatching; }
...
================
*/
void idConsoleLocal::Top() { display = 0; }
...
void idConsoleLocal::KeyDownEvent(int key) {
  ...
  if (key == K_ENTER || key == K_KP_ENTER) {
    ...
    common->UpdateScreen(
        captureToImage);  // force an update, because the command
    // may take some time
    return;
  }
  ...
  if ((key == K_UPARROW) || (key == K_P && (idKeyInput::IsDown(K_LCTRL) ||
                                            idKeyInput::IsDown(K_RCTRL)))) {
  ...
  if ((key == K_DOWNARROW) || (key == K_N && (idKeyInput::IsDown(K_LCTRL) ||
                                              idKeyInput::IsDown(K_RCTRL)))) {
...
void idConsoleLocal::UpdateDisplayFraction() {
  ...
  if (finalFrac < displayFrac) {
    displayFrac -=
        con_speed.GetFloat() * (Sys_Milliseconds() - fracTime) * 0.001f;
    if (finalFrac > displayFrac) {
      displayFrac = finalFrac;
    }
    fracTime = Sys_Milliseconds();
...
bool idConsoleLocal::ProcessEvent(const sysEvent_t* event, bool forceAccept) {
  const bool consoleKey = event->evType == SE_KEY &&
                          event->evValue == K_GRAVE &&
...
void idConsoleLocal::DrawInput() {
  ...
  if (consoleField.GetAutoCompleteLength() != 0) {
    ...
    if (autoCompleteLength > 0) {
      renderSystem->DrawFilled(
          idVec4(0.8f, 0.2f, 0.2f, 0.45f),
          LOCALSAFE_LEFT + 2 * SMALLCHAR_WIDTH +
  ...
  consoleField.Draw(LOCALSAFE_LEFT + 2 * SMALLCHAR_WIDTH, y,
                    renderSystem->GetVirtualWidth() - 3 * SMALLCHAR_WIDTH,
                    true);
}
...
void idConsoleLocal::DrawNotify() {
  ...
  for (i = current - NUM_CON_TIMES + 1; i <= current; i++) {
    ...
    for (x = 0; x < LINE_WIDTH; x++) {
      ...
      renderSystem->DrawSmallChar(LOCALSAFE_LEFT + (x + 1) * SMALLCHAR_WIDTH, v,
                                  text_p[x] & 0xff);
    }
...
void idConsoleLocal::DrawSolidConsole(float frac) {
  ...
  if (y < 1.0f) {
    ...
    renderSystem->DrawFilled(idVec4(0.0f, 0.0f, 0.0f, 0.75f), 0, 0,
                             renderSystem->GetVirtualWidth(), y);
  }
  ...
  // RB begin
  // idStr version = va( "%s.%i.%i", ENGINE_VERSION, BUILD_NUMBER,
  // BUILD_NUMBER_MINOR );
...
#define VERSION_LINE_SPACE (SMALLCHAR_HEIGHT + 4)
  ...
  for (x = 0; x < i; x++) {
    renderSystem->DrawSmallChar(
        ...
        (lines - (SMALLCHAR_HEIGHT + SMALLCHAR_HEIGHT / 4)) -
            VERSION_LINE_SPACE - VERSION_LINE_SPACE,
        version[x]);
  }
  // jmarshall
  ...
  if (display != current) {
    ...
    for (x = 0; x < LINE_WIDTH; x += 4) {
      renderSystem->DrawSmallChar(LOCALSAFE_LEFT + (x + 1) * SMALLCHAR_WIDTH,
                                  idMath::Ftoi(y), '^');
    }
    y -= SMALLCHAR_HEIGHT;
    rows--;
...
========================
*/
void idConsoleLocal::PrintOverlay(idOverlayHandle& handle, justify_t justify,
                                  const char* text, ...) {
...
========================
*/
void idConsoleLocal::DrawOverlayText(float& leftY, float& rightY,
                                     float& centerY) {
  for (int i = 0; i < overlayText.Num(); i++) {
    ...
    if (overlayText[i].justify == JUSTIFY_LEFT) {
      renderSystem->DrawFilled(bgColor, LOCALSAFE_LEFT + bgAdjust, leftY, width,
                               height);
    } else if (overlayText[i].justify == JUSTIFY_RIGHT) {
      renderSystem->DrawFilled(bgColor, LOCALSAFE_RIGHT - width + bgAdjust,
                               rightY, width, height);
    } else if (overlayText[i].justify == JUSTIFY_CENTER_LEFT ||
               overlayText[i].justify == JUSTIFY_CENTER_RIGHT) {
      renderSystem->DrawFilled(
          bgColor, LOCALSAFE_LEFT + (LOCALSAFE_WIDTH - width + bgAdjust) * 0.5f,
    ...
    for (int j = 0; j < text.Length(); j += singleLine.Length() + 1) {
      ...
      if (overlayText[i].justify == JUSTIFY_LEFT) {
        ...
        DrawTextLeftAlign(LOCALSAFE_LEFT + (LOCALSAFE_WIDTH - width) * 0.5f,
                          centerY, "%s", singleLine.c_str());
      } else if (overlayText[i].justify == JUSTIFY_CENTER_RIGHT) {
        DrawTextRightAlign(LOCALSAFE_LEFT + (LOCALSAFE_WIDTH + width) * 0.5f,
                           centerY, "%s", singleLine.c_str());
      } else {
        assert(false);
      }
