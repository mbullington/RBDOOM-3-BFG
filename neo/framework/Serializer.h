...
#define SERIALIZE_ENUM(ser, x, type, max) \
  ((x) = (type)ser.SerializeUMaxNonRef(x, max))
...
class idSerializer {
 public:
  ...
  void SerializeRange(int& value, int minSize,
                      int maxSize)  // Supports signed types
  {
    SanityCheck();
    if (writing) {
      msg->WriteBits(value - minSize,
                     idMath::BitsForInteger(maxSize - minSize));
    } else {
      value =
          minSize + msg->ReadBits(idMath::BitsForInteger(maxSize - minSize));
    }
  ...
  // void SerializeBitMsg( idBitMsg & inOutMsg, int numBytes ) { SanityCheck();
  // if ( writing ) { msg->WriteBitMsg( inOutMsg, numBytes ); } else {
  // msg->ReadBitMsg( inOutMsg, numBytes ); } }
  ...
  bool SerializeBoolNonRef(bool value) {
    SanityCheck();  // We return a value so we can support bit fields (can't
                    // pass by reference)
    if (writing) {
      msg->WriteBool(value);
    } else {
...
#else
  // SerializeQ - Quantizes a float to a variable number of bits (assumes
  // signed, uses simple quantization)
  template <int _max_, int _numBits_>
  void SerializeQ(idVec3& value) {
    SanityCheck();
    if (writing) {
  ...
  void SerializeQ(idMat3& axis,
                  int bits = 15);  // Default to 15 bits per component, which
                                   // has almost unnoticeable quantization
#endif
  ...
  void SerializeListElement(const idList<_type_*>& list,
  ...
  void SerializeCheckpoint(const char* file, int line) {
    ...
    if (tag != tagValue) {
      idLib::Error(
          "SERIALIZE_CHECKPOINT: tag != tagValue (file: %s - line: %i)", file,
          line);
    }
#endif
  }
...
class idSerializerScopedBlock {
 public:
  idSerializerScopedBlock(idSerializer& ser_, int maxSizeBytes_) {
    ...
    startByte = ser->IsReading() ? ser->GetMsg().GetReadCount()
                                 : ser->GetMsg().GetSize();
  ...
  ~idSerializerScopedBlock() {
    ...
    int endByte = ser->IsReading() ? ser->GetMsg().GetReadCount()
                                   : ser->GetMsg().GetSize();
    int sizeBytes = endByte - startByte;
    if (!verify(sizeBytes <= maxSizeBytes)) {
      idLib::Warning("idSerializerScopedBlock went over maxSize (%d > %d)",
...
inline void idSerializer::SerializeQ(idMat3& axis, int bits) {
  ...
  if (IsWriting()) {
    ...
    quat[maxIndex] = idMath::Sqrt(
        idMath::Fabs(1.0f - in.x * in.x - in.y * in.y - in.z * in.z));
...
(this function supports negative values, but will use 2 bytes for values greater
than 63)
========================
*/
