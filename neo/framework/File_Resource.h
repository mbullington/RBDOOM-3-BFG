...
class idResourceContainer {
 ...
 public:
  ...
  static void WriteResourceFile(const char* fileName, const idStrList& manifest,
  ...
  static void ExtractResourceFile(const char* fileName, const char* outPath,
                                  bool copyWavs);
  static void UpdateResourceFile(const char* filename,
                                 const idStrList& filesToAdd);
 ...
 private:
  ...
  // offset should probably be a 64 bit value for development, but 4 gigs won't
  // fit on a DVD layer, so it isn't a retail limitation.
