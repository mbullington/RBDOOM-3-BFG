...
idCVar net_ucmdRate("net_ucmdRate", "40", CVAR_SYSTEM | CVAR_INTEGER,
                    "How many milliseconds between sending usercmds");

idCVar net_debug_snapShotTime("net_debug_snapShotTime", "0",
                              CVAR_BOOL | CVAR_ARCHIVE, "");
idCVar com_forceLatestSnap("com_forceLatestSnap", "0", CVAR_BOOL, "");

// Enables effective snap rate: dynamically adjust the client snap rate based
// on:
//	-client FPS
//	-server FPS (interpolated game time received / interval it was received
//over)
//  -local buffered time (leave a cushion to absorb spikes, slow down when
//  infront of it, speed up when behind it) ie: net_minBufferedSnapPCT_Static
idCVar net_effectiveSnapRateEnable("net_effectiveSnapRateEnable", "1",
                                   CVAR_BOOL,
                                   "Dynamically adjust client snaprate");
idCVar net_effectiveSnapRateDebug("net_effectiveSnapRateDebug", "0", CVAR_BOOL,
                                  "Debug");

// Min buffered snapshot time to keep as a percentage of the effective snaprate
//	-ie we want to keep 50% of the amount of time difference between last
//two snaps. 	-we need to scale this because we may get throttled at the snaprate
//may change
//  -Acts as a buffer to absorb spikes
idCVar net_minBufferedSnapPCT_Static(
    "net_minBufferedSnapPCT_Static", "1.0", CVAR_FLOAT,
    "Min amount of snapshot buffer time we want need to buffer");
idCVar net_maxBufferedSnapMS("net_maxBufferedSnapMS", "336", CVAR_INTEGER,
                             "Max time to allow for interpolation cushion");
idCVar net_minBufferedSnapWinPCT_Static(
...
// Enabled a dynamic ideal snap buffer window: we will scale the distance and
// size
idCVar net_optimalDynamic(
    "net_optimalDynamic", "1", CVAR_BOOL,
    "How fast to add to our optimal time buffer when we are playing snapshots "
    "faster than server is feeding them to us");

// These values are used instead if net_optimalDynamic is 0 (don't scale by
// actual snap rate/interval)
idCVar net_optimalSnapWindow("net_optimalSnapWindow", "112", CVAR_FLOAT, "");
idCVar net_optimalSnapTime(
    "net_optimalSnapTime", "112", CVAR_FLOAT,
    "How fast to add to our optimal time buffer when we are playing snapshots "
    "faster than server is feeding them to us");

// this is at what percentage of being ahead of the interpolation buffer that we
// start slowing down (we ramp down from 1.0 to 0.0 starting here) this is a
// percentage of the total cushion time.
idCVar net_interpolationSlowdownStart(
    "net_interpolationSlowdownStart", "0.5", CVAR_FLOAT,
    "Scale interpolation rate when we fall behind");
...
bool idCommonLocal::IsMultiplayer() {
  ...
  return (((lobby.GetMatchParms().matchFlags & MATCH_ONLINE) != 0) &&
          (session->GetState() > idSession::IDLE));
}
...
===============
*/
int idCommonLocal::GetSnapRate() { return net_snapRate.GetInteger(); }
...
void idCommonLocal::NetReceiveSnapshot(class idSnapShot& ss) {
  ...
  // If we are about to overwrite the oldest snap, then force a read, which will
  // cause a pop on screen, but we have to do this.
  if (writeSnapshotIndex - readSnapshotIndex >= RECEIVE_SNAPSHOT_BUFFER_SIZE) {
    ...
    assert(writeSnapshotIndex % RECEIVE_SNAPSHOT_BUFFER_SIZE ==
...
void idCommonLocal::SendUsercmds(int localClientNum) {
  ...
  // Which may result in duplicate usercmds being sent in the case of a low
  // net_ucmdRate But the LZW compressor means the extra usercmds are not large
  // and the redundancy can smooth packet loss
  ...
  const int numCmds =
      userCmdMgr.GetPlayerCmds(localClientNum, cmdBuffer, NUM_USERCMD_SEND);
  ...
  nextUsercmdSendTime =
      MSEC_ALIGN_TO_FRAME(currentTime + net_ucmdRate.GetInteger());
}
...
void idCommonLocal::NetReceiveUsercmds(int peer, idBitMsg& msg) {
  ...
  if (clientNum == -1) {
    idLib::Warning("NetReceiveUsercmds: Could not find client for peer %d",
                   peer);
    return;
  }
...
void idCommonLocal::NetReceiveReliable(int peer, int type, idBitMsg& msg) {
  int clientNum = Game()->MapPeerToClient(peer);
  // Only servers care about the client num. Band-aid for problems related to
  // the host's peerIndex being -1 on clients.
  if (common->IsServer() && clientNum == -1) {
    idLib::Warning("NetReceiveReliable: Could not find client for peer %d",
                   peer);
    return;
  }
  ...
  for ( int i = 0; i < MAX_PLAYERS; i++ ) {
          ...
          if ( ss.GetObjectMsgByID( idSession::SS_PLAYER + i, msg ) ) {
                  ...
                  if ( com_drawSnapshots.GetInteger() == 3 ) {
                          console->AddSnapObject( "players", msg.GetSize(),
  ss.CompareObject( &oldss, idSession::SS_PLAYER + i ) );
                  }
          }
  }
  */
...
void idCommonLocal::NetReadUsercmds(int clientNum, idBitMsg& msg) {
  if (clientNum == -1) {
    idLib::Warning(
        "NetReadUsercmds: Trying to read commands from invalid clientNum %d",
        clientNum);
    return;
  }
...
void idCommonLocal::RunNetworkSnapshotFrame() {
  ...
  for (int i = 0; i < reliableQueue.Num(); i++) {
    game->ProcessReliableMessage(reliableQueue[i].client, reliableQueue[i].type,
                                 idBitMsg((const byte*)reliableQueue[i].data,
                                          reliableQueue[i].dataSize));
  ...
  if (snapPrevious.serverTime >= 0) {
    ...
    do {
      ...
      snapCurrentTime = idMath::ClampInt(
          0,
          snapRate +
              Min((int)snapRate, (int)net_maxExtrapolationInMS.GetInteger()),
          snapCurrentTime);
      ...
      if (!IsValid(fraction)) {
        idLib::Warning(
            "Interpolation Fraction invalid: snapCurrentTime %d / snapRate %d",
      ...
      effectiveSnapRate = static_cast<float>(totalBufferedTime) /
                          static_cast<float>(totalRecvTime);

      if (net_minBufferedSnapPCT_Static.GetFloat() > 0.0f) {
        optimalPCTBuffer = session->GetTitleStorageFloat(
      ...
      if (net_optimalDynamic.GetBool()) {
        optimalTimeBuffered = idMath::ClampInt(
            0, net_maxBufferedSnapMS.GetInteger(), snapRate * optimalPCTBuffer);
        optimalTimeBufferedWindow =
            snapRate * net_minBufferedSnapWinPCT_Static.GetFloat();
      } else {
      ...
      if (snapTimeBuffered <= optimalTimeBuffered) {
        if (snapTimeBuffered <= idMath::FLT_SMALLEST_NON_DENORMAL) {
          ...
          // When we interpolate past our cushion of buffered snapshot, we want
          // to slow smoothly slow the rate of interpolation. frac will go
          // from 1.0 to 0.0 (if snapshots stop coming in).
          float startSlowdown =
          ...
          if (startSlowdown > 0 && snapTimeBuffered < startSlowdown) {
            float frac = idMath::ClampFloat(0.0f, 1.0f,
                                            snapTimeBuffered / startSlowdown);
            if (!IsValid(frac)) {
              frac = 0.0f;
            }
      ...
      } else if (snapTimeBuffered >
                 optimalTimeBuffered + optimalTimeBufferedWindow) {
        // Go faster
      ...
      if (net_effectiveSnapRateEnable.GetBool()) {
        float deltaFrameGameMS = static_cast<float>(initialBaseTicksPerSec) *
                                 static_cast<float>(deltaFrameTime / 1000.0f);
        delta_interpolate =
        ...
        snapCurrentResidual = idMath::Frac(
            delta_interpolate);  // fixme: snapCurrentTime should just be a
                                 // float, but would require changes in d4 too
        ...
        if (net_effectiveSnapRateDebug.GetBool()) {
          idLib::Printf(
              "%d/%.2f snapRateScale: %.2f effectiveSR: %.2f d.interp: %.2f "
              "snapTimeBuffered: %.2f res: %.2f\n",
              deltaFrameTime, deltaFrameGameMS, snapRateScale,
              effectiveSnapRate, delta_interpolate, snapTimeBuffered,
              snapCurrentResidual);
        }
      }
      ...
      snapCurrentTime +=
          interpolate_interval;  // advance interpolation time by the scaled
...
void idCommonLocal::ExecuteReliableMessages() {
  ...
  for (int i = 0; i < reliableQueue.Num(); i++) {
    ...
    game->ProcessReliableMessage(
        reliable.client, reliable.type,
        idBitMsg((const byte*)reliable.data, reliable.dataSize));
