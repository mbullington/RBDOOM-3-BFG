...
bool idResourceContainer::Init(const char* _fileName, uint8 containerIndex) {
  ...
  for (int i = 0; i < numFileResources; i++) {
    ...
    bool found = false;
    // for ( int index = cacheHash.GetFirst( key ); index !=
    // idHashIndex::NULL_INDEX; index = cacheHash.GetNext( index ) ) {
...
========================
*/
void idResourceContainer::WriteManifestFile(const char* name,
...
========================
*/
void idResourceContainer::UpdateResourceFile(const char* _filename,
                                             const idStrList& _filesToUpdate) {
  idFile* outFile = fileSystem->OpenFileWrite(va("%s.new", _filename));
  if (outFile == NULL) {
    idLib::Warning("Unable to open resource file %s or new output file",
                   _filename);
    return;
  }
  ...
  // go back and write the header offsets again, now that we have file offsets
  // and lengths
...
========================
*/
void idResourceContainer::ExtractResourceFile(const char* _fileName,
                                              const char* _outPath,
                                              bool _copyWavs) {
  idFile* inFile = fileSystem->OpenFileRead(_fileName);
  ...
  for (int i = 0; i < _numFileResources; i++) {
    ...
    byte* fbuf = NULL;
    if (_copyWavs &&
        (rt.filename.Find(".idwav") >= 0 || rt.filename.Find(".idxma") >= 0 ||
         rt.filename.Find(".idmsf") >= 0)) {
...
========================
*/
void idResourceContainer::WriteResourceFile(const char* manifestName,
                                            const idStrList& manifest,
                                            const bool& _writeManifest) {
  if (manifest.Num() == 0) {
    return;
  }
  ...
  for (int idx = 0; idx < outPutFiles.Num(); idx++) {
    ...
    for (int i = 0; i < fileList.Num(); i++) {
      ...
      // if the entry is uncompressed, align the file pointer to a 16 byte
      // boundary so it will be usable if memory mapped
