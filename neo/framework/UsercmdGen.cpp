...
idCVar joy_mergedThreshold(
    "joy_mergedThreshold", "1", CVAR_BOOL | CVAR_ARCHIVE,
    "If the thresholds aren't merged, you drift more off center");
idCVar joy_newCode("joy_newCode", "1", CVAR_BOOL | CVAR_ARCHIVE,
                   "Use the new codepath");
idCVar joy_triggerThreshold("joy_triggerThreshold", "0.05",
                            CVAR_FLOAT | CVAR_ARCHIVE,
                            "how far the joystick triggers have to be pressed "
                            "before they register as down");
idCVar joy_deadZone("joy_deadZone", "0.2", CVAR_FLOAT | CVAR_ARCHIVE,
                    "specifies how large the dead-zone is on the joystick");
idCVar joy_range("joy_range", "1.0", CVAR_FLOAT | CVAR_ARCHIVE,
                 "allow full range to be mapped to a smaller offset");
idCVar joy_gammaLook("joy_gammaLook", "1", CVAR_INTEGER | CVAR_ARCHIVE,
                     "use a log curve instead of a power curve for movement");
idCVar joy_powerScale("joy_powerScale", "2", CVAR_FLOAT | CVAR_ARCHIVE,
                      "Raise joystick values to this power");
idCVar joy_pitchSpeed("joy_pitchSpeed", "100", CVAR_ARCHIVE | CVAR_FLOAT,
...
idCVar joy_dampenLook("joy_dampenLook", "1", CVAR_BOOL | CVAR_ARCHIVE,
                      "Do not allow full acceleration on look");
idCVar joy_deltaPerMSLook("joy_deltaPerMSLook", "0.003",
                          CVAR_FLOAT | CVAR_ARCHIVE,
                          "Max amount to be added on look per MS");

idCVar in_mouseSpeed("in_mouseSpeed", "1", CVAR_ARCHIVE | CVAR_FLOAT,
                     "speed at which the mouse moves", 0.25f, 4.0f);
idCVar in_alwaysRun("in_alwaysRun", "1", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL,
                    "always run (reverse _speed button) - only in MP");

idCVar in_useJoystick("in_useJoystick", "0", CVAR_ARCHIVE | CVAR_BOOL,
                      "enables/disables the gamepad for PC use");
idCVar in_joystickRumble("in_joystickRumble", "1",
                         CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL,
                         "enable joystick rumble");
idCVar in_invertLook("in_invertLook", "0", CVAR_ARCHIVE | CVAR_BOOL,
                     "inverts the look controls so the forward looks up "
                     "(flight controls) - the proper way to play games!");
idCVar in_mouseInvertLook("in_mouseInvertLook", "0", CVAR_ARCHIVE | CVAR_BOOL,
                          "inverts the look controls so the forward looks up "
                          "(flight controls) - the proper way to play games!");
...
bool usercmd_t::operator==(const usercmd_t& rhs) const {
  return (buttons == rhs.buttons && forwardmove == rhs.forwardmove &&
          rightmove == rhs.rightmove && angles[0] == rhs.angles[0] &&
          angles[1] == rhs.angles[1] && angles[2] == rhs.angles[2] &&
          impulse == rhs.impulse && impulseSequence == rhs.impulseSequence &&
          mx == rhs.mx && my == rhs.my && fireCount == rhs.fireCount &&
...
class idUsercmdGenLocal : public idUsercmdGen {
 ...
 private:
  ...
  void HandleJoystickAxis(int keyNum, float unclampedValue, float threshold,
                          bool positive);
  void JoystickMove();
  void JoystickMove2();
  void MouseMove();
...
idCVar idUsercmdGenLocal::in_yawSpeed(
    "in_yawspeed", "140", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT,
...
idCVar idUsercmdGenLocal::in_angleSpeedKey(
    "in_anglespeedkey", "1.5", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT,
...
idCVar idUsercmdGenLocal::in_toggleCrouch(
    "in_toggleCrouch", "0", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL,
    "pressing _movedown button toggles player crouching/standing");
idCVar idUsercmdGenLocal::in_toggleZoom(
    "in_toggleZoom", "0", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_BOOL,
    "pressing _zoom button toggles zoom on/off");
idCVar idUsercmdGenLocal::sensitivity("sensitivity", "5",
                                      CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT,
                                      "mouse view sensitivity");
idCVar idUsercmdGenLocal::m_pitch("m_pitch", "0.022",
                                  CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT,
                                  "mouse pitch scale");
idCVar idUsercmdGenLocal::m_yaw("m_yaw", "0.022",
                                CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_FLOAT,
                                "mouse yaw scale");
idCVar idUsercmdGenLocal::m_smooth(
    "m_smooth", "1", CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_INTEGER,
    "number of samples blended for mouse viewing", 1, 8,
    idCmdSystem::ArgCompletion_Integer<1, 8>);
idCVar idUsercmdGenLocal::m_showMouseRate("m_showMouseRate", "0",
...
void idUsercmdGenLocal::MouseMove() {
  ...
  viewangles[PITCH] += m_pitch.GetFloat() * in_mouseSpeed.GetFloat() *
                       (in_mouseInvertLook.GetBool() ? -my : my);
}
...
========================
*/
void idUsercmdGenLocal::HandleJoystickAxis(int keyNum, float unclampedValue,
                                           float threshold, bool positive) {
  ...
  switch (action) {
    ...
    case UB_LOOKUP: {
      if (joy_dampenLook.GetBool()) {
        lookValue = Min(lookValue, (pollTime - lastPollTime) *
                                           joy_deltaPerMSLook.GetFloat() +
                                       lastLookValuePitch);
        lastLookValuePitch = lookValue;
      }

      float invertPitch = in_invertLook.GetBool() ? -1.0f : 1.0f;
      viewangles[PITCH] -= MS2SEC(pollTime - lastPollTime) * lookValue *
    ...
    case UB_LOOKLEFT: {
      ...
      viewangles[YAW] +=
          MS2SEC(pollTime - lastPollTime) * lookValue * joy_yawSpeed.GetFloat();
      break;
    }
    case UB_LOOKRIGHT: {
      if (joy_dampenLook.GetBool()) {
...
void idUsercmdGenLocal::JoystickMove() {
  ...
  HandleJoystickAxis(K_JOY_TRIGGER1, joystickAxis[AXIS_LEFT_TRIG],
                     triggerThreshold, true);
  HandleJoystickAxis(K_JOY_TRIGGER2, joystickAxis[AXIS_RIGHT_TRIG],
                     triggerThreshold, true);
}
...
=================
*/
idVec2 JoypadFunction(const idVec2 raw, const float aimAssistScale,
                      const float threshold, const float range,
...
void DrawJoypadTexture(const int size, byte image[],
                       ...
                       const transferFunction_t shape,
                       const bool mergedThreshold) {
  //	assert( raw.x >= -1.0f && raw.x <= 1.0f && raw.y >= -1.0f && raw.y
  //<= 1.0f );
  idVec2 clamped;
  for (int i = 0; i < 2; i++) {
  ...
  for (int i = 1; i < size; i++) {
    ...
    const idVec2 mapped = JoypadFunction(idVec2(v, 0.0f), 1.0f, threshold,
                                         range, shape, mergedThreshold);
...
#define CPLOT(x, y) \
  ((int*)image)[(int)(halfSize + y) * size + (int)(halfSize + x)] = 0xffffffff
...
void DrawJoypadTexture(const int size, byte image[]) {
  ...
  const transferFunction_t shape =
      (transferFunction_t)joy_gammaLook.GetInteger();

  DrawJoypadTexture(size, image, lastLookJoypad, threshold, range, shape,
                    mergedThreshold);
}
...
void idUsercmdGenLocal::JoystickMove2() {
  ...
  idVec2 leftMapped =
      JoypadFunction(leftRaw, 1.0f, threshold, range, shape, mergedThreshold);
  idVec2 rightMapped = JoypadFunction(rightRaw, aimAssist, threshold, range,
                                      shape, mergedThreshold);

  // because idPhysics_Player::CmdScale scales mvoement values down so that 1,1
  // = sqrt(2), sqrt(2), we need to expand our circular values out to a square
  CircleToSquare(leftMapped.x, leftMapped.y);

  // add on top of mouse / keyboard move values
  cmd.forwardmove =
      idMath::ClampChar(cmd.forwardmove + KEY_MOVESPEED * -leftMapped.y);
  cmd.rightmove =
      idMath::ClampChar(cmd.rightmove + KEY_MOVESPEED * leftMapped.x);

  viewangles[PITCH] +=
      MS2SEC(pollTime - lastPollTime) * rightMapped.y * pitchSpeed;
  viewangles[YAW] +=
      MS2SEC(pollTime - lastPollTime) * -rightMapped.x * yawSpeed;
...
void idUsercmdGenLocal::InitCurrent() {
  ...
  cmd.impulse = impulse;
  cmd.buttons |=
      (in_alwaysRun.GetBool() && common->IsMultiplayer()) ? BUTTON_RUN : 0;
}
...
void idUsercmdGenLocal::MakeCurrent() {
  ...
  if (!Inhibited()) {
    ...
    toggled_crouch.SetKeyState(ButtonState(UB_MOVEDOWN),
                               in_toggleCrouch.GetBool());
    toggled_run.SetKeyState(ButtonState(UB_SPEED),
                            in_toggleRun.GetBool() && common->IsMultiplayer());
...
================
*/
void idUsercmdGenLocal::Init() { initialized = true; }
...
================
*/
void idUsercmdGenLocal::Shutdown() { initialized = false; }
...
================
*/
void idUsercmdGenLocal::ClearAngles() { viewangles.Zero(); }
...
void idUsercmdGenLocal::Mouse() {
  ...
  for (int i = 0; i < numEvents; i++) {
    ...
    switch (action) {
      ...
      {
        // DG: this seems like a good place to inject mousewheel deltas into
        // ImGui
...
void idUsercmdGenLocal::Joystick(int deviceNum) {
  ...
  for (int i = 0; i < numEvents; i++) {
    ...
    if (Sys_ReturnJoystickInputEvent(i, action, value)) {
      //		common->Printf("idUsercmdGenLocal::Joystick: i = %i /
      //action = %i / value = %i\n", i, action, value);
      ...
      if (action >= J_ACTION1 && action <= J_ACTION_MAX) {
        ...
        joystickAxis[action - J_AXIS_MIN] =
