...
class idFile {
 public:
  ...
  template <class type>
  inline size_t ReadBigArray(type* c, int count) {
  ...
  template <class type>
  inline size_t WriteBig(const type& c) {
    type b = c;
    idSwap::Big(b);
  ...
  template <class type>
  inline size_t WriteBigArray(const type* c, int count) {
    size_t r = 0;
...
class idFile_Memory : public idFile {
 ...
 public:
  ...
  // Doesn't change how much is allocated, but allows you to set the size of the
  // file to smaller than it should be. Useful for stripping off a checksum at
  // the end of the file
...
class idFile_InnerResource : public idFile {
 ...
 public:
  idFile_InnerResource(const char* _name, idFile* rezFile, int _offset,
                       int _len);
