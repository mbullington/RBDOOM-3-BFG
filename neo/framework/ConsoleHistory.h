...
class idConsoleHistory {
 public:
  idConsoleHistory() : upPoint(0), downPoint(0), returnLine(0), numHistory(0) {
    ClearHistory();
  }
  ...
  // If the command is the same as the last returned history line, nothing is
  // changed.
 ...
 private:
  ...
  compile_time_assert(CONST_ISPOWEROFTWO(
      COMMAND_HISTORY));  // we use the binary 'and' operator for wrapping
};
