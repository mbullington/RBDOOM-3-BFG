...
class idUserCmdMgr {
 public:
  ...
  bool HasUserCmdForClientTimeBuffer(int playerIndex, int millisecondBuffer) {
    ...
    const usercmd_t& firstCmd =
        cmdBuffer[index % USERCMD_BUFFER_SIZE][playerIndex];
    const usercmd_t& lastCmd = NewestUserCmdForPlayer(playerIndex);

    const int timeDelta =
        lastCmd.clientGameMilliseconds - firstCmd.clientGameMilliseconds;
  ...
  void SkipBufferedCmdsForPlayer(int playerIndex) {
    ...
    // -After calling this, HasUserCmdForPlayer() will return TRUE iff there was
    // >= 1 fresh cmd in the buffer Also, If there are no fresh frames, we wont
    // roll the readFrame back
    readFrame[playerIndex] =
        Max(readFrame[playerIndex], writeFrame[playerIndex] - 2);
  }
