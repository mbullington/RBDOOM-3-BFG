...
#include "vendor/zlib/minizip/zip.h"
#endif
...
class idZipBuilder {
 ...
 public:
  // adds a list of file extensions ( e.g. "bcm|bmodel|" ) to be compressed in
  // the zip file
  void AddFileFilters(const char* filters);
  // adds a list of file extensions ( e.g. "genmodel|" ) to be added to the zip
  // file, in an uncompressed form
  ...
  static bool ExtractFiles(idFile_Memory*& srcFile,
                           idList<idFile_Memory*>& destFiles);

  void CleanSourceFolder();

  bool CreateZipFileFromFileList(const char* name,
 ...
 private:
  ...
  idStrList
      uncompressedFilterExts;  // file extensions we don't want to compress
};
