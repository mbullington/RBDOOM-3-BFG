...
Unlike previous SMP work, the actual GPU command drawing is done in the main
thread, which avoids the OpenGL problems with needing windows to be created by
the same thread that creates the context, as well as the issues with passing
context ownership back and forth on the 360.

The game tic and the generation of the draw command list is now run in a
separate thread, and overlapped with the interpretation of the previous draw
command list.

While the game tic should be nicely contained, the draw command generation winds
through the user interface code, and is potentially hazardous.  For now, the
overlap will be restricted to the renderer back end, which should also be nicely
contained.
...
idCVar com_fixedTic("com_fixedTic", DEFAULT_FIXED_TIC, CVAR_BOOL,
                    "run a single game frame per render frame");
idCVar com_noSleep("com_noSleep", DEFAULT_NO_SLEEP, CVAR_BOOL,
                   "don't sleep if the game is running too fast");
idCVar com_smp("com_smp", "1", CVAR_INTEGER | CVAR_SYSTEM | CVAR_NOCHEAT,
...
idCVar com_sleepGame("com_sleepGame", "0", CVAR_SYSTEM | CVAR_INTEGER,
                     "intentionally add a sleep in the game time");
idCVar com_sleepDraw("com_sleepDraw", "0", CVAR_SYSTEM | CVAR_INTEGER,
                     "intentionally add a sleep in the draw time");
idCVar com_sleepRender("com_sleepRender", "0", CVAR_SYSTEM | CVAR_INTEGER,
                       "intentionally add a sleep in the render time");

idCVar net_drawDebugHud("net_drawDebugHud", "0", CVAR_SYSTEM | CVAR_INTEGER,
                        "0 = None, 1 = Hud 1, 2 = Hud 2, 3 = Snapshots");
...
int idGameThread::Run() {
  ...
  SetThreadGameTime((commonLocal.frameTiming.finishGameTime -
                     commonLocal.frameTiming.startGameTime) /
                    1000);
  ...
  SetThreadRenderTime((commonLocal.frameTiming.finishDrawTime -
                       commonLocal.frameTiming.finishGameTime) /
                      1000);
...
gameReturn_t idGameThread::RunGameAndDraw(int numGameFrames_,
  ...
  // foresthale 2014-05-12: also check com_editors as many of them are not
  // particularly thread-safe (editLights for example)
...
void idCommonLocal::DrawWipeModel() {
  ...
  float fade =
      (float)(currentTime - wipeStartTime) / (wipeStopTime - wipeStartTime);
  renderSystem->SetColor4(1, 1, 1, fade);
  renderSystem->DrawStretchPic(0, 0, renderSystem->GetVirtualWidth(),
                               renderSystem->GetVirtualHeight(), 0, 0, 1, 1,
                               wipeMaterial);
}
...
void idCommonLocal::Draw() {
  ...
  if (loadPacifierBinarizeActive) {
    ...
    if (loadPacifierBinarizeProgress > 0.0f) {
      loadPacifierBinarizeTimeLeft = (1.0 - loadPacifierBinarizeProgress) *
                                     (time - loadPacifierBinarizeStartTime) *
                                     0.001f / loadPacifierBinarizeProgress;
    } else {
    ...
    if (loadPacifierBinarizeTimeLeft >= 99.5f) {
      text = va("Binarizing %3.0f%% ETA %2.0f minutes",
                loadPacifierBinarizeProgress * 100.0f,
                loadPacifierBinarizeTimeLeft / 60.0f);
    } else if (loadPacifierBinarizeTimeLeft) {
      text = va("Binarizing %3.0f%% ETA %2.0f seconds",
                loadPacifierBinarizeProgress * 100.0f,
    ...
    renderSystem->DrawStretchPic(0, renderSystem->GetVirtualHeight() - 48,
                                 renderSystem->GetVirtualWidth(), 48, 0, 0, 1,
                                 1, whiteMaterial);
    renderSystem->SetColor(idVec4(0.0f, 0.5f, 0.8f, 1.0f));
    renderSystem->DrawStretchPic(
        0, renderSystem->GetVirtualHeight() - 48,
        loadPacifierBinarizeProgress * renderSystem->GetVirtualWidth(), 32, 0,
        0, 1, 1, whiteMaterial);
    renderSystem->DrawSmallStringExt(0, renderSystem->GetVirtualHeight() - 48,
                                     loadPacifierBinarizeFilename.c_str(),
                                     idVec4(1.0f, 1.0f, 1.0f, 1.0f), true);
    renderSystem->DrawSmallStringExt(
        0, renderSystem->GetVirtualHeight() - 32,
        va("%s %d/%d lvls", loadPacifierBinarizeInfo.c_str(),
           loadPacifierBinarizeMiplevel, loadPacifierBinarizeMiplevelTotal),
        idVec4(1.0f, 1.0f, 1.0f, 1.0f), true);
    renderSystem->DrawSmallStringExt(0, renderSystem->GetVirtualHeight() - 16,
                                     text, idVec4(1.0f, 1.0f, 1.0f, 1.0f),
                                     true);
  } else if (loadGUI != NULL) {
    // foresthale 2014-05-30: showing a black background looks better than
    // flickering in widescreen
    renderSystem->SetColor(colorBlack);
    renderSystem->DrawStretchPic(0, 0, renderSystem->GetVirtualWidth(),
                                 renderSystem->GetVirtualHeight(), 0, 0, 1, 1,
                                 whiteMaterial);
    ...
    // SRS - Advance demo inside Frame() instead of Draw() to support smp mode
    // playback AdvanceRenderDemo( true );
...
===============
*/
// DG: added possibility to *not* release mouse in UpdateScreen(), it fucks up
// the view angle for screenshots
void idCommonLocal::UpdateScreen(bool captureToImage, bool releaseMouse) {
  ...
  frameTiming.finishDrawTime =
      Sys_Microseconds();  // SRS - Added frame timing for out-of-sequence
                           // updates (e.g. used in timedemo "twice" mode)

  // foresthale 2014-03-01: note: the only place that has captureToImage=true is
  ...
  const emptyCommand_t* cmd = renderSystem->SwapCommandBuffers(
      &time_frontend, &time_backend, &time_shadows, &time_gpu, &stats_backend,
      &stats_frontend);

  // get the GPU busy with new commands
  frameTiming.startRenderTime =
      Sys_Microseconds();  // SRS - Added frame timing for out-of-sequence
                           // updates (e.g. used in timedemo "twice" mode)
  renderSystem->RenderCommandBuffers(cmd);
  frameTiming.finishRenderTime =
      Sys_Microseconds();  // SRS - Added frame timing for out-of-sequence
                           // updates (e.g. used in timedemo "twice" mode)
...
void idCommonLocal::ProcessGameReturn(const gameReturn_t& ret) {
  // set joystick rumble
  if (in_useJoystick.GetBool() && in_joystickRumble.GetBool() &&
      !game->Shell_IsActive() &&
      session->GetSignInManager().GetMasterInputDevice() >= 0) {
    Sys_SetRumble(
        session->GetSignInManager().GetMasterInputDevice(), ret.vibrationLow,
        ret.vibrationHigh);  // Only set the rumble on the active controller
  } else {
...
void idCommonLocal::Frame() {
  try {
    ...
    // if the console or another gui is down, we don't need to hold the mouse
    // cursor
    bool chatting = false;

    // DG: Add pause from com_pause cvar
    if (com_pause.GetInteger() || console->Active() ||
        Dialog().IsDialogActive() || session->IsSystemUIShowing() ||
    ...
    if (aviCaptureMode) {
      idStr name;
      name.Format("demos/%s/%s_%05i", aviDemoShortName.c_str(),
                  aviDemoShortName.c_str(), aviDemoFrameCount++);
      renderSystem->TakeScreenshot(com_aviDemoWidth.GetInteger(),
                                   com_aviDemoHeight.GetInteger(), name,
                                   com_aviDemoSamples.GetInteger(), NULL, TGA);

      // remove any printed lines at the top before taking the screenshot
      console->ClearNotifyLines();

      // this will call Draw, possibly multiple times if com_aviDemoSamples is >
      // 1
      renderSystem->TakeScreenshot(com_aviDemoWidth.GetInteger(),
                                   com_aviDemoHeight.GetInteger(), name,
                                   com_aviDemoSamples.GetInteger(), NULL, TGA);
    ...
    // particularly thread-safe (editLights for example)
    if (com_smp.GetInteger() > 0 && com_editors == 0) {
      renderCommands = renderSystem->SwapCommandBuffers(
          &time_frontend, &time_backend, &time_shadows, &time_gpu,
          &stats_backend, &stats_frontend);
    ...
    for (;;) {
      ...
      const int clampedDeltaMilliseconds =
          Min(deltaMilliseconds, com_deltaTimeClamp.GetInteger());

      gameTimeResidual += clampedDeltaMilliseconds * timescale.GetFloat();

      // don't run any frames when paused
      // jpcy: the game is paused when playing a demo, but playDemo should wait
      // like the game does SRS - don't wait if window not in focus and playDemo
      // itself paused
      if (pauseGame &&
          (!(readDemo && !timeDemo) || session->IsSystemUIShowing() ||
    ...
    // jpcy: playDemo uses the game frame wait logic, but shouldn't run any game
    // frames
    ...
    if (session->GetState() == idSession::LOADING) {
      ...
      // SRS - If in Doom 3 mode (com_smp = -1) on map change, must obey fence
      // before returning to avoid command buffer sync issues
    ...
    // SRS - Advance demos inside Frame() vs. Draw() to support smp mode
    // playback SRS - Pause playDemo (but not timeDemo) when window not in focus
    if (readDemo &&
        (!(session->IsSystemUIShowing() || com_pause.GetInteger()) ||
         timeDemo)) {
      AdvanceRenderDemo(true);
      if (!readDemo) {
        // SRS - Important to return after demo playback is finished to avoid
        // command buffer sync issues
        return;
      }
    }
    ...
    for (int i = 0; i < numGameFrames; i++) {
      newCmd.clientGameMilliseconds =
          FRAME_TO_MSEC(gameFrame - numGameFrames + i + 1);
    ...
    // foresthale 2014-05-12: also check com_editors as many of them are not
    // particularly thread-safe (editLights for example) SRS - if com_editors is
    // active make sure com_smp != -1, otherwise skip and call
    // SwapCommandBuffers_FinishRendering later
    ...
    // SRS - If in Doom 3 mode (com_smp = -1), must sync after
    // RenderCommandBuffers() otherwise get artifacts due to improper command
    // buffer swap timing
    if (com_smp.GetInteger() < 0) {
    ...
    // SRS - Use finishSyncTime_EndFrame to record timing after sync for com_smp
    // = -1, and just before gameThread.WaitForThread() for com_smp = 1
    ...
    // SRS - Enable sound during normal playDemo playback but not during
    // timeDemo
    ...
    // SRS - Mute all sound output when dialog waiting or window not in focus
    // (mutes Doom3, Classic, Cinematic Audio)
    if (Dialog().IsDialogPausing() || session->IsSystemUIShowing() ||
        com_pause.GetInteger()) {
    ...
    if (lobby.HasActivePeers()) {
      ...
      lobby.DrawDebugNetworkHUD_ServerSnapshotMetrics(
          net_drawDebugHud.GetInteger() == 3);
    }
    ...
    if (com_speeds.GetBool()) {
      ...
      Printf("frame:%d all:%3d gfr:%3d rf:%3lld bk:%3lld\n", idLib::frameNumber,
             com_frameMsec, time_gameFrame, time_frontend / 1000,
             time_backend / 1000);
      time_gameFrame = 0;
      time_gameDraw = 0;
    }
