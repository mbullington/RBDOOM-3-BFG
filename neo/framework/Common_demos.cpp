...
// extern idCVar com_smp;            // SRS - No longer require non-smp mode for
// demos

void WriteDeclCache(idDemoFile* f, int demoCategory, int demoCode,
                    declType_t declType) {
...
void idCommonLocal::StartRecordingRenderDemo(const char* demoName) {
  ...
  if (!demoName[0]) {
    common->Printf(
        "idCommonLocal::StartRecordingRenderDemo: no name specified\n");
    return;
  }

  console->Close();

  //  com_smp.SetInteger( 0 );        // SRS - No longer require non-smp mode
  //  for demos
...
void idCommonLocal::StopRecordingRenderDemo() {
  ...
  delete writeDemo;
  writeDemo = NULL;
  //	com_smp.SetInteger( 1 ); // motorsep 12-30-2014; turn multithreading
  //back on;  SRS - No longer require non-smp mode for demos
}
...
void idCommonLocal::StartPlayingRenderDemo(idStr demoName) {
  if (!demoName[0]) {
    common->Printf(
        "idCommonLocal::StartPlayingRenderDemo: no name specified\n");
    return;
  }
  ...
  if (demoVersion != RENDERDEMO_VERSION) {
    common->Printf(
        "StartPlayingRenderDemo got version %d, expected version %d\n",
  ...
  numDemoFrames = 0;  // SRS - Moved ahead of first call to AdvanceRenderDemo to
                      // properly handle demoshots
  numShotFrames = 0;  // SRS - Initialize count of demoShot frames to play
                      // before timeout to main menu

  renderSystem->BeginLevelLoad();  // SRS - Free static data from previous level
                                   // before loading demo assets
  soundSystem->BeginLevelLoad();   // SRS - Free sound media from previous level
                                   // before loading demo assets
  declManager->BeginLevelLoad();  // SRS - Clear declaration manager data before
                                  // loading demo assets
  uiManager->BeginLevelLoad();    // SRS - Clear gui manager data before loading
                                  // demo assets

  AdvanceRenderDemo(true);  // SRS - Call AdvanceRenderDemo() once to load map
                            // and initial assets (like level load)

  renderSystem
      ->EndLevelLoad();  // SRS - Define static data for use by
                         // RB_StencilShadowPass if stencil shadows enabled
  soundSystem->EndLevelLoad();
  declManager->EndLevelLoad();
  uiManager->EndLevelLoad("");  // SRS - FIXME: No gui assets are currently
                                // saved/reloaded in demo file, fix later?
...
================
*/
void idCommonLocal::TimeRenderDemo(const char* demoName, bool twice,
                                   bool quit) {
  ...
  if (twice && readDemo) {
    timeDemo = TD_YES;  // SRS - Set timeDemo to TD_YES to disable time demo
                        // playback pause when window not in focus

    int smp_mode = com_smp.GetInteger();
    com_smp.SetInteger(
        0);  // SRS - First pass of timedemo is effectively in com_smp == 0
             // mode, so set this for ImGui timings to be correct

    while (readDemo) {
      BusyWait();  // SRS - BusyWait() calls UpdateScreen() which draws and
                   // renders out-of-sequence but still supports frame timing
      ...
      commonLocal.frameTiming.finishSyncTime =
          commonLocal.frameTiming.startSyncTime;
      commonLocal.frameTiming.startGameTime =
          commonLocal.frameTiming.finishSyncTime;

      AdvanceRenderDemo(
          true);  // SRS - Advance demo commands to manually run the next game
                  // frame during first pass of the timedemo
      commonLocal.frameTiming.finishGameTime = Sys_Microseconds();

      eventLoop->RunEventLoop(
          false);  // SRS - Run event loop (with no commands) to allow keyboard
                   // escape to cancel first pass of the timedemo
    }

    com_smp.SetInteger(
        smp_mode);  // SRS - Restore original com_smp mode before second pass of
                    // timedemo which runs within normal rendering loop
  ...
  if (!readDemo) {
    timeDemo =
        TD_NO;  // SRS - Make sure timeDemo flag is off if readDemo is NULL
    return;
  }
...
void idCommonLocal::BeginAVICapture(const char* demoName) {
  ...
  aviCaptureMode = true;
  aviDemoFrameCount = 0;
  soundWorld->AVIOpen(va("demos/%s/", aviDemoShortName.c_str()),
    ...
    switch (ds) {
      case DS_FINISHED:
        if (numDemoFrames == 1) {
          ...
          if (numShotFrames++ <
              com_engineHz_latched *
                  10)  // SRS - play demoShot for min 10 sec then timeout
          {
            return;
          }
        }
        LeaveGame();  // SRS - drop back to main menu after demo playback is
                      // finished
        return;
      case DS_RENDER:
        if (renderWorld->ProcessDemoCommand(readDemo, &currentDemoRenderView,
                                            &demoTimeOffset)) {
...
// SRS - Changed macro from CONSOLE_COMMAND to CONSOLE_COMMAND_SHIP for
// demo-related commands - i.e. include in release builds
...
================
*/
CONSOLE_COMMAND_SHIP(compressDemo, "compresses a demo file",
...
================
*/
CONSOLE_COMMAND_SHIP(playDemo, "plays back a demo",
                     idCmdSystem::ArgCompletion_DemoName) {
  if (args.Argc() >= 2) {
...
================
*/
CONSOLE_COMMAND_SHIP(timeDemo, "times a demo",
                     idCmdSystem::ArgCompletion_DemoName) {
  if (args.Argc() >= 2) {
    commonLocal.TimeRenderDemo(va("demos/%s", args.Argv(1)), (args.Argc() > 2),
                               false);
  }
}
...
================
*/
CONSOLE_COMMAND_SHIP(timeDemoQuit, "times a demo and quits",
                     idCmdSystem::ArgCompletion_DemoName) {
  commonLocal.TimeRenderDemo(va("demos/%s", args.Argv(1)), (args.Argc() > 2),
                             true);  // SRS - fixed missing "twice" argument
}
...
================
*/
CONSOLE_COMMAND_SHIP(aviDemo, "writes AVIs for a demo",
                     idCmdSystem::ArgCompletion_DemoName) {
