...
typedef enum {
  ...
  LEXFL_NOSTRINGCONCAT =
      BIT(3),  // multiple strings separated by whitespaces are not concatenated
  ...
  LEXFL_ALLOWBACKSLASHSTRINGCONCAT =
      BIT(12),  // allow multiple strings separated by '\' to be concatenated
...
class idLexer {
 ...
 public:
  ...
  // skips spaces, tabs, C-like comments etc. Returns false if there is no token
  // left to read.
