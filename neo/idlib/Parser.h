...
class idParser {
 public:
  ...
  const char* ParseBracedSection(idStr& out, int tabs, bool parseFirstBrace,
                                 char intro, char outro);
  ...
  // RB: made protected to have custom #include behaviours for embedded
  // resources
 protected:
  // RB end
  ...
  int ExpandBuiltinDefine(idToken* deftoken, define_t* define,
                          idToken** firsttoken, idToken** lasttoken);
  int ExpandDefine(idToken* deftoken, define_t* define, idToken** firsttoken,
                   idToken** lasttoken);
  ...
  int EvaluateTokens(idToken* tokens, signed int* intvalue, double* floatvalue,
                     int integer);
...
inline const char* idParser::GetFileName() const {
...
inline const int idParser::GetFileOffset() const {
  if (idParser::scriptstack) {
...
inline const ID_TIME_T idParser::GetFileTime() const {
  if (idParser::scriptstack) {
...
inline const int idParser::GetLineNum() const {
  if (idParser::scriptstack) {
