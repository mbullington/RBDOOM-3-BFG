...
================
*/
bool idDict::GetFloat(const char* key, const char* defaultString,
                      float& out) const {
  const char* s;
  bool found;
...
================
*/
bool idDict::GetInt(const char* key, const char* defaultString,
                    int& out) const {
  const char* s;
  bool found;
...
================
*/
bool idDict::GetBool(const char* key, const char* defaultString,
                     bool& out) const {
  const char* s;
  bool found;
...
================
*/
bool idDict::GetFloat(const char* key, const float defaultFloat,
                      float& out) const {
...
================
*/
bool idDict::GetAngles(const char* key, const char* defaultString,
                       idAngles& out) const {
  bool found;
  const char* s;
...
================
*/
bool idDict::GetVector(const char* key, const char* defaultString,
                       idVec3& out) const {
  bool found;
  const char* s;
...
================
*/
bool idDict::GetVec2(const char* key, const char* defaultString,
                     idVec2& out) const {
  bool found;
  const char* s;
...
================
*/
bool idDict::GetVec4(const char* key, const char* defaultString,
                     idVec4& out) const {
  bool found;
  const char* s;
...
================
*/
bool idDict::GetMatrix(const char* key, const char* defaultString,
                       idMat3& out) const {
  const char* s;
  bool found;
...
const char* idDict::RandomPrefix(const char* prefix, idRandom& random) const {
  ...
  list[0] = "";
  for (count = 0, kv = MatchPrefix(prefix);
       kv != NULL && count < MAX_RANDOM_KEYS; kv = MatchPrefix(prefix, kv)) {
...
void idDict::WriteToIniFile(idFile* f) const {
  ...
  idTempArray<int> prefixIndex(
      sortedArgs.Num());  // for each keyValue in the args, this is an index
                          // into which prefix it uses.
  ...
  for (int i = 0; i < sortedArgs.Num(); i++) {
    ...
    if (prefixIndex[i] != prevPrefixIndex) {
      ...
      prefixLength = prefixList[prevPrefixIndex - 1].Length() +
                     1;  // to skip past the '/' too

      // output prefix
      idStr str = va("%s[%s]\n", skipFirstLine.c_str(),
                     prefixList[prevPrefixIndex - 1].c_str());
      f->Write((void*)str.c_str(), str.Length());
    }

    const idKeyValue* kv = &sortedArgs[i];
    idStr str = va("%s=%s\n", kv->GetKey().c_str() + prefixLength,
                   idStr::CStyleQuote(kv->GetValue()));
    f->Write((void*)str.c_str(), str.Length());
  }
}
...
bool idDict::ReadFromIniFile(idFile* f) {
  ...
  buffer[length - 1] =
      '\0';  // Since the .ini files are not null terminated, make sure we mark
             // where the end of the .ini file is in our read buffer

  idLexer parser(LEXFL_NOFATALERRORS |
                 LEXFL_ALLOWPATHNAMES /*| LEXFL_ONLYSTRINGS */);
  ...
  const punctuation_t ini_punctuations[] = {{"[", P_SQBRACKETOPEN},
                                            {"]", P_SQBRACKETCLOSE},
                                            {"=", P_ASSIGN},
                                            {NULL, 0}};
  ...
  while (success && !parser.EndOfFile()) {
    if (parser.PeekTokenType(TT_PUNCTUATION, P_SQBRACKETOPEN, &token)) {
      success = success &&
                parser.ExpectTokenType(TT_PUNCTUATION, P_SQBRACKETOPEN, &token);
      success = success && parser.ReadToken(&token);
      prefix = token.c_str();
      prefix.Append('/');
      success = success && parser.ExpectTokenType(TT_PUNCTUATION,
                                                  P_SQBRACKETCLOSE, &token);
    }
    ...
    success =
        success && parser.ExpectTokenType(TT_PUNCTUATION, P_ASSIGN, &token2);
...
CONSOLE_COMMAND(
    TestDictIniFile,
    "Tests the writing/reading of various items in a dict to/from an ini file",
    0) {
  // Write to the file
