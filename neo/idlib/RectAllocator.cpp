...
#include "vendor/binpack2d/binpack2d.h"

/*

This routine performs a tight packing of a list of rectangles, attempting to
minimize the area of the rectangle that encloses all of them.  Algorithm order
is N^2, so it is not apropriate for lists with many thousands of elements.
...
void RectAllocator(const idList<idVec2i>& inputSizes,
                   ...
                   const int START_MAX, const int imageMax) {
  ...
  // Sort the rects from largest to smallest (it makes allocating them in the
  // image better)
  ...
  // For each image try to fit it at a corner of one of the already fitted
  // images while minimizing the total area. Somewhat better allocation could be
  // had by checking all the combinations of x and y edges in the allocated
  // rectangles, rather than just the corners of each rectangle, but it still
  ...
  for (int i = 1; i < inputSizes.Num(); i++) {
    ...
    for (int j = 0; j < i; j++) {
      for (int k = 1; k < 4; k++) {
        idVec2i test;
        for (int n = 0; n < 2; n++) {
          test[n] = outputPositions[sizeRemap[j]][n] +
                    ((k >> n) & 1) * inputSizes[sizeRemap[j]][n];
        }
        ...
        // if we have already found a spot that keeps the image smaller, don't
        // bother checking here This calculation biases the rect towards more
        // square shapes instead of allowing it to extend in one dimension for a
        // long time.
        ...
        // if the image isn't required to grow, favor the location closest to
        // the origin
        ...
        for (; n < i; n++) {
          ...
          if (test.x + size.x > check.x && test.y + size.y > check.y &&
...
void RectAllocatorBinPack2D(const idList<idVec2i>& inputSizes,
  ...
  for (int i = 0; i < inputSizes.Num(); i++) {
    ...
    inputContent += BinPack2D::Content<MyContent>(
        mycontent, BinPack2D::Coord(), BinPack2D::Size(width, height), false);
  }
  ...
  BinPack2D::CanvasArray<MyContent> canvasArray =
      BinPack2D::UniformCanvasArrayBuilder<MyContent>(START_MAX, START_MAX, 2)
          .Build();
  ...
  int i = 0;
  for (binpack2d_iterator itor = outputContent.Get().begin();
       itor != outputContent.Get().end(); itor++, i++) {
  ...
  for (binpack2d_iterator itor = remainder.Get().begin();
       itor != remainder.Get().end(); itor++) {
    const BinPack2D::Content<MyContent>& content = *itor;
    ...
    idLib::Printf("\tFailed to place %9s of size %3dx%3d\n",
                  myContent.str.c_str(), content.size.w, content.size.h);
  }
}