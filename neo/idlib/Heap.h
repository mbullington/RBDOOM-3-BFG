...
// memory tag names are used to sort allocations for sys_dumpMemory and other
// reporting functions
enum memTag_t {
...
===============================================================================
        ...
        All objects of the 'type' are properly constructed and destructed when
reused.
...
// Define this to force all block allocators to act like normal new/delete
// allocation for tool checking.
...
class idBlockAlloc {
 ...
 private:
  union element_t {
    _type_* data;  // this is a hack to make sure the save game system marks
                   // _type_ as saveable
    element_t* next;
    byte buffer[(CONST_MAX(sizeof(_type_), sizeof(element_t*)) +
                 (BLOCK_ALLOC_ALIGNMENT - 1)) &
                ~(BLOCK_ALLOC_ALIGNMENT - 1)];
  };
  ...
  class idBlock {
   public:
    ...
    idBlock* next;
    element_t* free;  // list with free elements in this block (temp used only
                      // by FreeEmptyBlocks)
    int freeCount;  // number of free elements in this block (temp used only by
                    // FreeEmptyBlocks)
  };
...
inline _type_* idBlockAlloc<_type_, _blockSize_, memTag>::Alloc() {
  ...
  if (clearAllocs) {
    memset(
        (void*)t, 0,
        sizeof(
            _type_));  // SRS - Added (void*) cast to silence build-time warning
  }
  new (t) _type_;
  return t;
#endif
  ...
  for (element_t* element = free; element != NULL;) {
    ...
    for (idBlock* block = blocks; block != NULL; block = block->next) {
      if (element >= block->elements &&
          element < block->elements + _blockSize_) {
...
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                         _tag_>::SetFixedBlocks(int numBlocks) {
  idDynamicBlock<type>* block;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                         _tag_>::SetLockMemory(bool lock) {
  lockMemory = lock;
}

template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                         _tag_>::FreeEmptyBaseBlocks() {
...
int idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                        _tag_>::GetNumEmptyBaseBlocks() const {
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
type* idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Alloc(
    const int num) {
  idDynamicBlock<type>* block;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
type* idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Resize(
    type* ptr, const int num) {
  numResizes++;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Free(
    type* ptr) {
  numFrees++;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
const char* idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                                _tag_>::CheckMemory(const type* ptr) const {
  idDynamicBlock<type>* block;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
idDynamicBlock<type>* idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                                          _tag_>::AllocInternal(const int num) {
  idDynamicBlock<type>* block;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
idDynamicBlock<type>*
idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::ResizeInternal(
    idDynamicBlock<type>* block, const int num) {
  int alignedBytes = (num * sizeof(type) + 15) & ~15;
...
template <class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize,
                         _tag_>::FreeInternal(idDynamicBlock<type>* block) {
