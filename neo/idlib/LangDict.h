...
class idLangDict {
 public:
  ...
  const char* GetString(
      const char* str) const;  // returns str if string not found
  const char* FindString(
      const char* str) const;  // returns NULL if string not found
  ...
  bool SetString(const char* key,
                 const char* val);  // Returns false if the key doesn't exist
  void AddKeyVal(const char* key,
                 const char* val);  // Like SetString, but adds it if it doesn't
                                    // already exist
...
class idLocalization {
 public:
  static const char* GetString(
      const char* inString);  // returns inString if string not found
  static const char* FindString(
      const char* inString);  // Returns NULL if string not found

  static void ClearDictionary();
  static bool LoadDictionary(const byte* buffer, const int bufferLen,
                             const char* name);
  ...
  static utf8Encoding_t VerifyUTF8(const uint8* buffer, const int bufferLen,
                                   const char* name);
