  ...
  if (numBits > GetRemainingWriteBits()) {
    if (!allowOverflow) {
      idLib::FatalError(
          ...
          "numBits=%i numRemainingWriteBits=%i",
  ...
  if (numBits != 32) {
    if (numBits > 0) {
      if (value > (1 << numBits) - 1) {
        idLib::FatalError("idBitMsg::WriteBits: value overflow %d %d", value,
                          numBits);
...
int idBitMsg::ReadBits(int numBits) const {
  ...
  while (valueBits < numBits) {
    ...
    fraction >>= readBit;
    // this doesn't really do anything: the bits set to 0 here are already
    // shifted to 0
    fraction &= (1 << get) - 1;
