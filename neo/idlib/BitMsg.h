...
avoids alignment errors. It allows concurrent writing and reading. The data set
with Init is never free-d.
================================================
*/
class idBitMsg {
 public:
  ...
  void WriteDeltaFloat(float oldValue, float newValue, int exponentBits,
                       int mantissaBits) {
  ...
  template <int _max_, int _numBits_>
  void WriteQuantizedUFloat(
      float value);  // Quantize a float to a variable number of bits (assumes
                     // unsigned, uses simple quantization)
  ...
  // Compress a vector to a variable number of bits (assumes signed, uses simple
  // quantization)
  ...
  float ReadDeltaFloat(float oldValue, int exponentBits,
 ...
 private:
  ...
  bool
      allowOverflow;  // if false, generate error when the message is overflowed
...
inline void idBitMsg::WriteByteAlign() {
  ...
  // bits into the stream, however idBitMsg::WriteBits is properly initializing
  // the byte to 0 so hopefully we are still safe adding this extra check just
  // in case
  curSize += writeBit != 0;
  assert(writeBit == 0 ||
         ((writeData[curSize - 1] >> writeBit) ==
          0));  // had to early out writeBit == 0 because when writeBit == 0
                // writeData[curSize - 1] may be the previous byte written and
...
inline void idBitMsg::WriteQuantizedFloat(float value) {
  ...
  if (_max_ > storeMax) {
    ...
    WriteBits(
        idMath::ClampInt(-storeMax, storeMax, idMath::Ftoi(value * scale)),
        -_numBits_);
  } else {
...
inline void idBitMsg::WriteQuantizedUFloat(float value) {
  ...
  if (_max_ > storeMax) {
    // Scaling down (scale should be < 1)
    const float scale = (float)storeMax / (float)_max_;
    WriteBits(idMath::ClampInt(0, storeMax, idMath::Ftoi(value * scale)),
              _numBits_);
  } else {
    // Scaling up (scale should be >= 1) (Preserve whole numbers when possible)
    enum { scale = storeMax / _max_ };
...
inline float idBitMsg::ReadQuantizedFloat() const {
  ...
  if (_max_ > storeMax) {
    ...
    // We use a float to get rid of (potential divide by zero) which is handled
    // above, but the compiler is dumb
...
*/
template <class _arrayType_>
void WriteDeltaFloatArray(idBitMsg& message, const _arrayType_& oldArray,
                          const _arrayType_& newArray) {
  for (int i = 0; i < idTupleSize<_arrayType_>::value; ++i) {
...
*/
template <class _arrayType_>
_arrayType_ ReadDeltaFloatArray(const idBitMsg& message,
                                const _arrayType_& oldArray) {
  _arrayType_ result;
