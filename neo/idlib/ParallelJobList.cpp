...
========================
*/
idParallelJobRegistration::idParallelJobRegistration(jobRun_t function,
                                                     const char* name) {
...
static idCVar jobs_longJobMicroSec(
    "jobs_longJobMicroSec", "10000", CVAR_INTEGER,
    "print a warning for jobs that take more than this number of microseconds");
...
class idParallelJobList_Threads {
 public:
  idParallelJobList_Threads(jobListId_t id, jobListPriority_t priority,
  ...
  int RunJobs(unsigned int threadNum, threadJobListState_t& state,
              bool singleJob);

 private:
  static const int NUM_DONE_GUARDS =
      4;  // cycle through 4 guards so we can cyclicly chain job lists
  ...
  int RunJobsInternal(unsigned int threadNum, threadJobListState_t& state,
                      bool singleJob);
...
========================
*/
idParallelJobList_Threads::idParallelJobList_Threads(jobListId_t id,
  ...
  jobList.AssureSize(
      maxJobs + maxSyncs * 2 +
      1);  // syncs go in as dummy jobs and one more to update the doneCount
  jobList.SetNum(0);
...
========================
*/
idParallelJobList_Threads::~idParallelJobList_Threads() { Wait(); }
  ...
  if (1)  // JDC: this never worked in tech5!  !jobList.IsFull() ) {
  {
    ...
    for (int i = 0; i < numRegisteredJobs; ++i) {
      if (currentJobCount[i] > 0) {
        idLib::Printf("Job: %s, # %d", registeredJobs[i].name,
                      currentJobCount[i]);
      }
    }
    idLib::Error("Can't add job '%s', too many jobs %d", GetJobName(function),
                 jobList.Num());
  }
}
  ...
  switch (syncType) {
    case SYNC_SIGNAL: {
      ...
      if (jobList.Num()) {
        ...
        signalJobCount[signalJobCount.Num() - 1].SetValue(jobList.Num() -
                                                          lastSignalJob);
...
========================
*/
void idParallelJobList_Threads::Submit(
    idParallelJobList_Threads* waitForJobList, int parallelism) {
  assert(done);
  ...
  if (waitForJobList != NULL) {
    waitForGuard =
        &waitForJobList->doneGuards[waitForJobList->currentDoneGuard];
  } else {
    waitForGuard = NULL;
  }
...
bool idParallelJobList_Threads::TryWait() {
  if (jobList.Num() == 0 ||
      signalJobCount[signalJobCount.Num() - 1].GetValue() <= 0) {
    Wait();
    return true;
  }
  return false;
...
========================
*/
bool idParallelJobList_Threads::IsSubmitted() const { return !done; }
...
uint64 idParallelJobList_Threads::GetTotalWastedTimeMicroSec() const {
  ...
  for (int unit = 0; unit < MAX_THREADS; unit++) {
    total +=
        threadStats.threadTotalTime[unit] - threadStats.threadExecTime[unit];
  }
  return total;
}
...
========================
*/
uint64 idParallelJobList_Threads::GetUnitProcessingTimeMicroSec(
    int unit) const {
...
========================
*/
int idParallelJobList_Threads::RunJobsInternal(unsigned int threadNum,
  ...
  if (deferredThreadStats.startTime == 0) {
    deferredThreadStats.startTime =
        Sys_Microseconds();  // first time any thread is running jobs from this
                             // list
  }
  ...
  do {
    // run through all signals and syncs before the last job that has been or is
    // being executed this loop is really an optimization to minimize the time
    // spent in the fetchLock section below
    for (; state.lastJobIndex < (int)currentJob.GetValue() &&
           state.lastJobIndex < jobList.Num();
         state.lastJobIndex++) {
    ...
    if (fetchLock.Increment() == 1) {
      ...
      // run through any remaining signals and syncs (this should rarely iterate
      // more than once)
    ...
    // execute the next job
    {
      ...
      if (jobs_longJobMicroSec.GetInteger() > 0) {
        if (jobEnd - jobStart > jobs_longJobMicroSec.GetInteger() &&
          ...
          const char* jobName =
              GetJobName(jobList[state.nextJobIndex].function);
          const char* jobListName = GetJobListName(GetId());
          idLib::Printf(
              "%1.1f milliseconds for a single '%s' job from job list %s on "
              "thread %d\n",
...
========================
*/
int idParallelJobList_Threads::RunJobs(unsigned int threadNum,
                                       threadJobListState_t& state,
                                       bool singleJob) {
...
========================
*/
idParallelJobList::idParallelJobList(jobListId_t id, jobListPriority_t priority,
                                     unsigned int maxJobs,
                                     unsigned int maxSyncs,
                                     const idColor* color) {
  assert(priority > JOBLIST_PRIORITY_NONE);
  this->jobListThreads = new (TAG_JOBLIST)
      idParallelJobList_Threads(id, priority, maxJobs, maxSyncs);
  this->color = color;
}
...
========================
*/
idParallelJobList::~idParallelJobList() { delete jobListThreads; }
...
========================
*/
CellSpursJob128* idParallelJobList::AddJobSPURS() { return NULL; }
...
========================
*/
void idParallelJobList::Submit(idParallelJobList* waitForJobList,
                               int parallelism) {
  assert(waitForJobList != this);
  jobListThreads->Submit(
      (waitForJobList != NULL) ? waitForJobList->jobListThreads : NULL,
      parallelism);
}
...
========================
*/
jobListId_t idParallelJobList::GetId() const { return jobListThreads->GetId(); }
...
const int JOB_THREAD_STACK_SIZE =
    256 * 1024;  // same size as the SPU local store
...
static idCVar jobs_prioritize("jobs_prioritize", "1", CVAR_BOOL | CVAR_NOCHEAT,
                              "prioritize job lists");
...
class idJobThread : public idSysThread {
 ...
 private:
  ...
  unsigned int
      lastJobList;  // index where the next job list to work on will be added
...
========================
*/
idJobThread::idJobThread() : firstJobList(0), lastJobList(0), threadNum(0) {}
...
void idJobThread::Start(core_t core, unsigned int threadNum) {
  ...
  // DG: change threadname from "JobListProcessor_%d" to "JLProc_%d", because
  // Linux has a 15 (+ \0) char limit for threadnames. furthermore: va is not
  // thread safe, use snPrintf instead
  char name[16];
...
void idJobThread::AddJobList(idParallelJobList_Threads* jobList) {
  // must lock because multiple threads may try to add new job lists at the same
  // time
  addJobMutex.Lock();
  // wait until there is space available because in rare cases multiple versions
  // of the same job lists may still be queued
...
int idJobThread::Run() {
  ...
  while (!IsTerminating()) {
    ...
    if (numJobLists < MAX_JOBLISTS && firstJobList < lastJobList) {
      threadJobListState[numJobLists].jobList =
          jobLists[firstJobList & (MAX_JOBLISTS - 1)].jobList;
      threadJobListState[numJobLists].version =
          jobLists[firstJobList & (MAX_JOBLISTS - 1)].version;
    ...
    if (lastStalledJobList < 0) {
      ...
      // try to hide the stall with a job from a list that has equal or higher
      // priority
      currentJobList = lastStalledJobList;
      priority = threadJobListState[lastStalledJobList].jobList->GetPriority();
      for (int i = 0; i < numJobLists; i++) {
        if (i != lastStalledJobList &&
            threadJobListState[i].jobList->GetPriority() >= priority &&
            !threadJobListState[i].jobList->WaitForOtherJobList()) {
          priority = threadJobListState[i].jobList->GetPriority();
          currentJobList = i;
        }
    ...
    // if the priority is high then try to run through the whole list to reduce
    // the overhead otherwise run a single job and re-evaluate priorities for
    // the next job
    bool singleJob =
        (priority == JOBLIST_PRIORITY_HIGH) ? false : jobs_prioritize.GetBool();

    // try running one or more jobs from the current job list
    int result = threadJobListState[currentJobList].jobList->RunJobs(
        threadNum, threadJobListState[currentJobList], singleJob);
    ...
    if ((result & idParallelJobList_Threads::RUN_DONE) != 0) {
      ...
      // yield when stalled on the same job list again without making any
      // progress
...
//
// Windows 7 Starter						1
// 32/64 Windows 7 Home Basic						1
// 32/64 Windows 7 Professional					2
// 32/64
//
//
// Hyperthreading is not dead yet.  Intel's Core i7 Processor is quad-core with
// HT for 8 logicals.

// DOOM3: We don't have that many jobs, so just set this fairly low so we don't
// spin up a ton of idle threads
#define MAX_JOB_THREADS 32
#define NUM_JOB_THREADS "2"
#define JOB_THREAD_CORES                                                      \
  {                                                                           \
    CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY,     \
        CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, \
        CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, \
        CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, CORE_ANY, \
...
idCVar jobs_numThreads("jobs_numThreads", NUM_JOB_THREADS,
                       CVAR_INTEGER | CVAR_NOCHEAT,
                       "number of threads used to crunch through jobs", 0,
                       MAX_JOB_THREADS);
...
class idParallelJobManagerLocal : public idParallelJobManager {
 public:
  ...
  virtual idParallelJobList* AllocJobList(jobListId_t id,
                                          jobListPriority_t priority,
                                          unsigned int maxJobs,
                                          unsigned int maxSyncs,
                                          const idColor* color);
...
void idParallelJobManagerLocal::Init() {
  // on consoles this will have specific cores for the threads, but on PC they
  // will all be CORE_ANY
...
========================
*/
idParallelJobList* idParallelJobManagerLocal::AllocJobList(
    jobListId_t id, jobListPriority_t priority, unsigned int maxJobs,
    unsigned int maxSyncs, const idColor* color) {
  for (int i = 0; i < jobLists.Num(); i++) {
    if (jobLists[i]->GetId() == id) {
      // idStudio may cause job lists to be allocated multiple times
    }
  }
  idParallelJobList* jobList = new (TAG_JOBLIST)
      idParallelJobList(id, priority, maxJobs, maxSyncs, color);
...
void idParallelJobManagerLocal::FreeJobList(idParallelJobList* jobList) {
  ...
  // wait for all job threads to finish because job list deletion is not thread
  // safe
...
========================
*/
int idParallelJobManagerLocal::GetNumJobLists() const { return jobLists.Num(); }
...
========================
*/
int idParallelJobManagerLocal::GetNumProcessingUnits() { return maxThreads; }
...
========================
*/
void idParallelJobManagerLocal::Submit(idParallelJobList_Threads* jobList,
                                       int parallelism) {
  if (jobs_numThreads.IsModified()) {
    maxThreads =
        idMath::ClampInt(0, MAX_JOB_THREADS, jobs_numThreads.GetInteger());
