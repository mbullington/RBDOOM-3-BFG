...
inline void* idListArrayNew(int num, bool zeroBuffer) {
  _type_* ptr = NULL;
  if (zeroBuffer) {
...
inline void idListArrayDelete(void* ptr, int num) {
...
*/
template <typename _type_, memTag_t _tag_>
inline void* idListArrayResize(void* voldptr, int oldNum, int newNum,
                               bool zeroBuffer) {
...
class idList {
 public:
  ...
  size_t Size() const;  // returns total size of allocated memory including size
                        // of list _type_
  ...
  void
  Condense();  // resizes list to exactly the number of elements it contains
  ...
  void SetNum(int newnum);       // set number of elements in list and resize to
                                 // exactly this number if needed
  void AssureSize(int newSize);  // assure list has given number of elements,
                                 // but leave them uninitialized
  void AssureSize(
      int newSize,
      const _type_& initValue);  // assure list has given number of elements and
  ...
  int FindIndex(
      const _type_& obj) const;  // find the index for the given element
  _type_* Find(_type_ const& obj) const;  // find pointer to the given element
  int FindNull()
      const;  // find the index for the first NULL pointer in the list
  int IndexOf(const _type_* obj)
      const;  // returns the index for the pointer to an element in the list
  bool RemoveIndex(int index);  // remove the element at the given index
  // removes the element at the given index and places the last element into its
...
*/
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>::idList(int newgranularity) {
...
*/
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>::idList(idList&& other) {
  list = NULL;
  *this = std::move(other);
}
...
*/
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>::idList(const idList& other) {
  list = NULL;
  *this = other;
}
...
*/
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>::~idList() {
  Clear();
}
...
Frees up the memory allocated by the list.  Assumes that _type_ automatically
handles freeing up memory.
================
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::Clear() {
  if (list) {
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::DeleteContents(bool clear) {
  int i;
...
account additional memory allocated by _type_
================
*/
template <typename _type_, memTag_t _tag_>
inline size_t idList<_type_, _tag_>::Allocated() const {
...
memory allocated by _type_
================
*/
template <typename _type_, memTag_t _tag_>
inline size_t idList<_type_, _tag_>::Size() const {
...
*/
template <typename _type_, memTag_t _tag_>
inline size_t idList<_type_, _tag_>::MemoryUsed() const {
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::Num() const {
  return num;
}
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::NumAllocated() const {
  return size;
}
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::SetNum(int newnum) {
  assert(newnum >= 0);
  if (newnum > size) {
    Resize(newnum);
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::SetGranularity(int newgranularity) {
  int newsize;
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::GetGranularity() const {
  return granularity;
}
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::Condense() {
  if (list) {
    if (num) {
      Resize(num);
    } else {
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::Resize(int newsize) {
  assert(newsize >= 0);
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::Resize(int newsize, int newgranularity) {
  assert(newsize >= 0);
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::AssureSize(int newSize) {
  int newNum = newSize;
...
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::AssureSizeAlloc(int newSize,
                                                   new_t* allocator) {
  int newNum = newSize;
...
*/
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>& idList<_type_, _tag_>::operator=(
...
template <typename _type_, memTag_t _tag_>
inline idList<_type_, _tag_>& idList<_type_, _tag_>::operator=(
    const idList<_type_, _tag_>& other) {
  int i;
...
*/
template <typename _type_, memTag_t _tag_>
inline const _type_& idList<_type_, _tag_>::operator[](int index) const {
  assert(index >= 0);
  assert(index < num);
...
*/
template <typename _type_, memTag_t _tag_>
inline _type_& idList<_type_, _tag_>::operator[](int index) {
  assert(index >= 0);
  assert(index < num);
...
*/
template <typename _type_, memTag_t _tag_>
inline _type_* idList<_type_, _tag_>::Ptr() {
  return list;
}
...
*/
template <typename _type_, memTag_t _tag_>
const inline _type_* idList<_type_, _tag_>::Ptr() const {
  return list;
}
...
*/
template <typename _type_, memTag_t _tag_>
inline _type_& idList<_type_, _tag_>::Alloc() {
  if (!list) {
    Resize(granularity);
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::Append(_type_ const& obj) {
  if (!list) {
    Resize(granularity);
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::Insert(_type_ const& obj, int index) {
  if (!list) {
    Resize(granularity);
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::Append(const idList<_type_, _tag_>& other) {
  if (!list) {
    if (granularity == 0)  // this is a hack to fix our memset classes
    {
      granularity = 16;
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::Append(const _type_* other, int n) {
  int newNum = Num() + n;
  if (newNum > Size()) {
    Resize(newNum);
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::AddUnique(_type_ const& obj) {
  int index;
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::FindIndex(_type_ const& obj) const {
  int i;
...
*/
template <typename _type_, memTag_t _tag_>
inline _type_* idList<_type_, _tag_>::Find(_type_ const& obj) const {
  int i;
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::FindNull() const {
  int i;
...
*/
template <typename _type_, memTag_t _tag_>
inline int idList<_type_, _tag_>::IndexOf(_type_ const* objptr) const {
  int index;
...
*/
template <typename _type_, memTag_t _tag_>
inline bool idList<_type_, _tag_>::RemoveIndex(int index) {
  int i;
...
maintain the order of elements! The number of elements in the list is reduced by
one.

return:	bool	- false if the data is not found in the list.

NOTE:	The element is not destroyed, so any memory used by it may not be freed
until the destruction of the list.
========================
*/
template <typename _type_, memTag_t _tag_>
inline bool idList<_type_, _tag_>::RemoveIndexFast(int index) {
  if ((index < 0) || (index >= num)) {
    return false;
  }
...
*/
template <typename _type_, memTag_t _tag_>
inline bool idList<_type_, _tag_>::Remove(_type_ const& obj) {
  int index;
...
//*/
// template< typename _type_, memTag_t _tag_ >
// inline void idList<_type_,_tag_>::Sort( cmp_t *compare ) {
//	if ( !list ) {
//		return;
//	}
...
Note:	The data is merely moved around the list, so any pointers to data within
the list may no longer be valid.
========================
*/
template <typename _type_, memTag_t _tag_>
inline void idList<_type_, _tag_>::SortWithTemplate(
    const idSort<_type_>& sort) {
  if (list == NULL) {
    return;
  }
  sort.Sort(Ptr(), Num());
...
//*/
// template< typename _type_, memTag_t _tag_ >
// inline void idList<_type_,_tag_>::SortSubSection( int startIndex, int
// endIndex, cmp_t *compare ) { 	if ( !list ) { 		return;
//	}
//	if ( startIndex < 0 ) {
//		startIndex = 0;
//	}
...
Finds an item in a list based on any another datatype.  Your _type_ must
overload operator()== for the _type_. If your _type_ is a ptr, use the
FindFromGenericPtr function instead.
========================
*/
template <typename _type_, memTag_t _tag_, typename _compare_type_>
_type_* FindFromGeneric(idList<_type_, _tag_>& list,
                        const _compare_type_& other) {
...
*/
template <typename _type_, memTag_t _tag_, typename _compare_type_>
_type_* FindFromGenericPtr(idList<_type_, _tag_>& list,
                           const _compare_type_& other) {
  for (int i = 0; i < list.Num(); i++) {
    if (*list[i] == other) {
      return &list[i];
