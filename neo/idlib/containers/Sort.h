  ...
  virtual void Sort(_type_* base, unsigned int num) const {
    ...
    for (int64 level = 0; level >= 0;) {
      ...
      if (((j - i) >= 4) && (level < (MAX_LEVELS - 1))) {
        ...
        do {
          while (static_cast<const _derived_*>(this)->Compare(base[i], pivot) <
                 0) {
            if (++i >= j) {
              break;
            }
          }
          while (static_cast<const _derived_*>(this)->Compare(base[j], pivot) >
                 0) {
            if (--j <= i) {
              break;
            }
        ...
        for (; i < j; j--) {
          ...
          for (int64 k = i + 1; k <= j; k++) {
            if (static_cast<const _derived_*>(this)->Compare(base[k], base[m]) >
                0) {
              m = k;
            }
          }
 ...
 public:
  virtual void Sort(_type_* base, unsigned int num) const {
    ...
    for (unsigned int i = num / 2; i > 0; i--) {
      ...
      for (unsigned int child = parent * 2 + 1; child < num;
           child = parent * 2 + 1) {
        if (child + 1 < num && static_cast<const _derived_*>(this)->Compare(
                                   base[child + 1], base[child]) > 0) {
          child++;
        }
        if (static_cast<const _derived_*>(this)->Compare(base[child],
                                                         base[parent]) <= 0) {
          break;
        }
    ...
    for (unsigned int i = 1; i < num; i++) {
      ...
      for (unsigned int child = i; child > 0;) {
        ...
        if (static_cast<const _derived_*>(this)->Compare(base[parent],
                                                         base[child]) > 0) {
          break;
        }
    ...
    for (unsigned int i = num - 1; i > 0; i--) {
      ...
      for (unsigned int child = parent * 2 + 1; child < i;
           child = parent * 2 + 1) {
        if (child + 1 < i && static_cast<const _derived_*>(this)->Compare(
                                 base[child + 1], base[child]) > 0) {
          child++;
        }
        if (static_cast<const _derived_*>(this)->Compare(base[child],
...
class idSort_HeapDefault
    : public idSort_Heap<_type_, idSort_HeapDefault<_type_> > {
 public:
  int Compare(const _type_& a, const _type_& b) const { return a - b; }
};
...
class idSort_InsertionDefault
    : public idSort_Insertion<_type_, idSort_InsertionDefault<_type_> > {
 public:
  int Compare(const _type_& a, const _type_& b) const { return a - b; }
};
