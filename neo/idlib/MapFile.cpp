...
// RB begin
inline bool BrushPrimitive_Degenerate(const idVec3& bpTexMatX,
                                      const idVec3& bpTexMatY) {
  // 2D cross product
...
void idMapBrushSide::ConvertToValve220Format(const idMat4& entityTransform,
  ...
  // move planepts from entity space to world space because TrenchBroom can only
  // handle brushes in world space
  ...
  if (BrushPrimitive_Degenerate(texX, texY)) {
    // idLib::Warning( "non orthogonal texture matrix in
    // ConvertToValve220Format" );
  }
  ...
  texScale[0] = 1.0f / idMath::Sqrt(localMat[0][0] * localMat[0][0] +
                                    localMat[1][0] * localMat[1][0]);
  texScale[1] = 1.0f / idMath::Sqrt(localMat[0][1] * localMat[0][1] +
                                    localMat[1][1] * localMat[1][1]);
...
=================
*/
idMapPatch* idMapPatch::Parse(idLexer& src, const idVec3& origin,
                              bool patchDef3, float version) {
  float info[7];
  idDrawVert* vert;
  idToken token;
  int i, j;
...
============
*/
bool idMapPatch::Write(idFile* fp, int primitiveNum,
  ...
  for (i = 0; i < GetWidth(); i++) {
    ...
    for (j = 0; j < GetHeight(); j++) {
      ...
      fp->WriteFloatString(" ( %f %f %f %f %f )", v->xyz[0] + origin[0],
                           v->xyz[1] + origin[1], v->xyz[2] + origin[2], st[0],
                           st[1]);
    }
    fp->WriteFloatString(" )\n");
  }
...
=================
*/
idMapBrush* idMapBrush::Parse(idLexer& src, const idVec3& origin,
...
idMapBrush* idMapBrush::ParseValve220(idLexer& src, const idVec3& origin) {
  ...
  do {
    ...
    if (!src.Parse1DMatrix(3, planepts[0].ToFloatPtr()) ||
        ...
        !src.Parse1DMatrix(3, planepts[2].ToFloatPtr())) {
      src.Error(
          "idMapBrush::ParseValve220: unable to read brush side plane "
          "definition");
      sides.DeleteContents(true);
      return NULL;
    }
    ...
    if (!src.ReadTokenOnLine(&token)) {
      src.Error(
          "idMapBrush::ParseValve220: unable to read brush side material");
      sides.DeleteContents(true);
      return NULL;
    }
    ...
    if (token == "*" || token == "+" || token.type == TT_NUMBER) {
      ...
      if (token == "+") {
        ...
        if (numberToken.type == TT_NUMBER) {
          if (!src.ReadTokenOnLine(&token)) {
            src.Error(
                "idMapBrush::ParseValve220: unable to read brush side "
                "material");
            sides.DeleteContents(true);
            return NULL;
          }
    ...
    /*
    RB: negative values seem to be valid and the q3map2 implementation in
    netradiant-custom is faulty if( scale[0] < idMath::FLOAT_EPSILON )
    {
            scale[0] = 1.0f;
    }
...
============
*/
bool idMapBrush::Write(idFile* fp, int primitiveNum,
                       const idVec3& origin) const {
  int i;
  idMapBrushSide* side;
...
============
*/
bool idMapBrush::WriteValve220(idFile* fp, int primitiveNum,
                               const idVec3& origin) const {
  int i;
  idMapBrushSide* side;
  ...
  for (i = 0; i < GetNumSides(); i++) {
    side = GetSide(i);
    fp->WriteFloatString(
        "( %f %f %f ) ( %f %f %f ) ( %f %f %f )", side->planepts[0][0],
        side->planepts[0][1], side->planepts[0][2], side->planepts[1][0],
        side->planepts[1][1], side->planepts[1][2], side->planepts[2][0],
    ...
    fp->WriteFloatString(
        "[ %f %f %f %f ] [ %f %f %f %f ] 0 %f %f 0 0 0\n", side->texValve[0][0],
        side->texValve[0][1], side->texValve[0][2], side->texValve[0][3],
        side->texValve[1][0], side->texValve[1][1], side->texValve[1][2],
        side->texValve[1][3], side->texScale[0], side->texScale[1]);
  }
...
bool idMapBrush::IsOriginBrush() const {
  for (int i = 0; i < GetNumSides(); i++) {
    const idMaterial* material =
        declManager->FindMaterial(sides[i]->GetMaterial());
...
bool idMapEntity::WriteJSON(idFile* fp, int entityNum, int numEntities) const {
  ...
  for (int i = 0; i < epairs.GetNumKeyVals(); i++) {
    ...
    fp->WriteFloatString(
        ...
        ((i == (epairs.GetNumKeyVals() - 1)) && !GetNumPrimitives()) ? ""
                                                                     : ",");
  }
...
===============
*/
void idMapEntity::RemovePrimitiveData() { primitives.DeleteContents(true); }
...
class idSort_CompareMapEntity
    : public idSort_Quick<idMapEntity*, idSort_CompareMapEntity> {
 public:
  int Compare(idMapEntity* const& a, idMapEntity* const& b) const {
    ...
    return idStr::Icmp(a->epairs.GetString("name"),
                       b->epairs.GetString("name"));
  }
};
...
bool idMapFile::Parse(const char* filename, bool ignoreRegion, bool osPath) {
  ...
  if (isJSON) {
    ...
    if (entities.Num() > 0 &&
        (idStr::Icmp(entities[0]->epairs.GetString("name"), "worldspawn") !=
         0)) {
      // move world spawn to first place
      for (int i = 1; i < entities.Num(); i++) {
        if (idStr::Icmp(entities[i]->epairs.GetString("name"), "worldspawn") ==
            0) {
...
============
*/
bool idMapFile::Write(const char* fileName, const char* ext,
                      bool fromBasePath) {
  int i;
  idStr qpath;
  idFile* fp;
...
// RB begin
bool idMapFile::WriteJSON(const char* fileName, const char* ext,
                          bool fromBasePath) {
  int i;
  idStr qpath;
  idFile* fp;
...
void MapPolygonMesh::ConvertFromBrush(const idMapBrush* mapBrush, int entityNum,
                                      int primitiveNum) {
  originalType = TYPE_BRUSH;

  // fix degenerate planes
  idPlane* planes =
      (idPlane*)_alloca16(mapBrush->GetNumSides() * sizeof(planes[0]));
  ...
  for (int i = 0; i < mapBrush->GetNumSides(); i++) {
    ...
    const idMaterial* material =
        declManager->FindMaterial(mapSide->GetMaterial());
    ...
    for (int j = 0; j < mapBrush->GetNumSides() && w.GetNumPoints(); j++) {
      ...
      if (!w.ClipInPlace(-planes[j], 0)) {
        ...
        // badBrush = true;
        common->Printf(
            "Entity %i, Brush %i: no intersection with other brush plane\n",
            entityNum, primitiveNum);
        // break;
      }
    }
...
void MapPolygonMesh::ConvertFromPatch(const idMapPatch* patch, int entityNum,
                                      int primitiveNum) {
...
bool MapPolygonMesh::Write(idFile* fp, int primitiveNum,
                           const idVec3& origin) const {
  fp->WriteFloatString("// primitive %d\n{\n meshDef\n {\n", primitiveNum);
  // fp->WriteFloatString( "  \"%s\"\n  ( %d %d 0 0 0 )\n", GetMaterial(),
  // GetWidth(), GetHeight() );
  ...
  for (int i = 0; i < verts.Num(); i++) {
    ...
    // origin[0], v->xyz[1] + origin[1], v->xyz[2] + origin[2], st[0], st[1],
    // n[0], n[1], n[2] );
    fp->WriteFloatString("   ( %f %f %f %f %f %f %f %f )\n", v->xyz[0],
                         v->xyz[1], v->xyz[2], st[0], st[1], n[0], n[1], n[2]);
  }
  fp->WriteFloatString("  )\n");
  ...
  for (int i = 0; i < polygons.Num(); i++) {
    ...
    fp->WriteFloatString("   \"%s\" %d = ", poly.GetMaterial(),
                         poly.indexes.Num());
...
bool MapPolygonMesh::WriteJSON(idFile* fp, int primitiveNum,
                               const idVec3& origin) const {
  fp->WriteFloatString("\t\t\t\t{\n\t\t\t\t\t\"primitive\": \"%d\",\n",
                       primitiveNum);
  ...
  for (int i = 0; i < polygons.Num(); i++) {
    ...
    fp->WriteFloatString("\t\t\t\t\t\t{ \"material\": \"%s\", \"indices\": [",
                         poly.GetMaterial());
...
MapPolygonMesh* MapPolygonMesh::Parse(idLexer& src, const idVec3& origin,
                                      float version) {
  float info[7];
  idToken token;
  int i;
...
bool idMapFile::ConvertToValve220Format() {
    ...
    if (ent) {
...
#if 1
      // HACK: convert every old .lwo, .ase model to an .obj proxy model so it
      // can be displayed properly in TrenchBroom
      idStr ext;
      model.ExtractFileExtension(ext);

      if (ext.Icmp("lwo") == 0 || ext.Icmp("ase") == 0 ||
          ext.Icmp("dae") == 0) {
      ...
      if (ent->GetNumPrimitives() > 0) {
        ...
        if (idStr::Icmp(classname, "func_static") == 0 &&
            idStr::Icmp(model.c_str(), classname.c_str()) == 0) {
        ...
        for (int i = 0; i < ent->GetNumPrimitives(); i++) {
          ...
          if (mapPrim->GetType() == idMapPrimitive::TYPE_BRUSH) {
            ...
            idMapFile::AddMaterialToCollection(patch->GetMaterial(),
                                               textureCollections);
          }
        }
        ...
        if (kv && kv->GetValue().Length()) {
          if (idStr::Icmp(kv->GetValue().c_str(), "PointClass") == 0 &&
              idStr::Icmp(kv->GetValue().c_str(), "Mixed") != 0) {
        ...
        // TrenchBroom doesn't allow models for SolidClasses so use helper
        // classes instead
        if (idStr::Icmp(classname, "func_static") == 0 &&
            idStr::Icmp(model.c_str(), classname.c_str()) != 0) {
          ent->epairs.Set("classname", "misc_model");
        } else if (idStr::Icmp(classname, "func_door") == 0 &&
                   idStr::Icmp(model.c_str(), classname.c_str()) != 0) {
          ent->epairs.Set("classname", "func_door_model");
        } else if (idStr::Icmp(classname, "func_mover") == 0 &&
                   idStr::Icmp(model.c_str(), classname.c_str()) != 0) {
          ent->epairs.Set("classname", "func_mover_amodel");
        } else if (idStr::Icmp(classname, "func_rotating") == 0 &&
                   idStr::Icmp(model.c_str(), classname.c_str()) != 0) {
          ent->epairs.Set("classname", "func_rotating_model");
        } else if (idStr::Icmp(classname, "func_plat") == 0 &&
                   idStr::Icmp(model.c_str(), classname.c_str()) != 0) {
          ent->epairs.Set("classname", "func_plat_model");
        }

        // replace "rotation" with angles because it is not supported by
        // TrenchBroom
        ...
        const idKeyValue* kv = classTypeOverview.FindKey(classname);
        if (kv && kv->GetValue().Length()) {
          if (idStr::Icmp(kv->GetValue().c_str(), "BrushClass") == 0 &&
              idStr::Icmp(kv->GetValue().c_str(), "Mixed") != 0) {
            classTypeOverview.Set(classname, "Mixed");
          }
        } else {
  ...
  if (worldspawn) {
    // worldspawn->epairs.Set( "_tb_textures",
    // "textures/common;textures/editor;textures/decals;textures/decals2" );
    ...
    if (ent) {
      ...
      if (!namePair) {
        ...
        for (int i = 1; i < count; i++) {
          ...
          if (otherNamePair && !otherNamePair->GetValue().IsEmpty() &&
              idStr::Cmp(namePair->GetValue(), otherNamePair->GetValue()) ==
                  0) {
...
#if 1
    worldspawn->epairs.Set("_tb_textures",
                           "textures/common;textures/editor;textures/id1");
#else
    TODO
...
void idMapFile::AddMaterialToCollection(const char* material,
                                        idStrList& textureCollections) {
...
static const int numTextureConvertNames =
    sizeof(textureConvertNames) / sizeof(textureConvertNames[0]);
