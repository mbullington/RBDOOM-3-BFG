...
void idSurface_SweptSpline::SetSweptCircle(const float radius) {
  idCurve_NURBS<idVec4>* nurbs =
      new (TAG_IDLIB_SURFACE) idCurve_NURBS<idVec4>();
  nurbs->Clear();
...
void idSurface_SweptSpline::GetFrame(const idMat3& previousFrame,
                                     const idVec3 dir, idMat3& newFrame) {
  float wx, wy, wz;
  float xx, yy, yz;
  float xy, xz, zz;
  float x2, y2, z2;
...
void idSurface_SweptSpline::Tessellate(const int splineSubdivisions,
  ...
  sweptSplineDiv =
      sweptSpline->GetBoundaryType() == idCurve_Spline<idVec4>::BT_CLOSED
  ...
  totalTime = spline->GetTime(spline->GetNumValues() - 1) - spline->GetTime(0) +
              spline->GetCloseTime();
  splineDiv = spline->GetBoundaryType() == idCurve_Spline<idVec4>::BT_CLOSED
                  ? splineSubdivisions
