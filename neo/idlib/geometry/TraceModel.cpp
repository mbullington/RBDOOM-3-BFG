...
============
*/
void idTraceModel::SetupCylinder(const float height, const float width,
                                 const int numSides) {
  idBounds cylBounds;
...
============
*/
void idTraceModel::SetupCone(const float height, const float width,
                             const int numSides) {
  idBounds coneBounds;
  float halfWidth;
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    for (j = 0; j < polys[i].numEdges; j++) {
      ...
      polys[i].bounds.AddPoint(
          verts[edges[abs(edgeNum)].v[INT32_SIGNBITSET(edgeNum)]]);
    }
    polys[i].dist = polys[i].normal *
                    verts[edges[abs(edgeNum)].v[INT32_SIGNBITSET(edgeNum)]];
...
void idTraceModel::Shrink(const float m) {
  ...
  if (type == TRM_POLYGON) {
    for (i = 0; i < numEdges; i++) {
      ...
      edge = &edges[abs(edgeNum)];
      dir = verts[edge->v[INT32_SIGNBITSET(edgeNum)]] -
            verts[edge->v[INT32_SIGNBITNOTSET(edgeNum)]];
...
float idTraceModel::GetPolygonArea(int polyNum) const {
  ...
  for (i = 0; i < poly->numEdges; i++) {
    v1 = verts[edges[abs(poly->edges[i])].v[INT32_SIGNBITSET(poly->edges[i])]] -
         base;
    v2 = verts[edges[abs(poly->edges[i])]
                   .v[INT32_SIGNBITNOTSET(poly->edges[i])]] -
         base;
    cross = v1.Cross(v2);
    total += cross.Length();
  }
...
int idTraceModel::GetProjectionSilhouetteEdges(
    const idVec3& projectionOrigin, int silEdges[MAX_TRACEMODEL_EDGES]) const {
  int i, j, edgeNum;
  ...
  for (i = 0; i < numPolys; i++) {
    ...
    dir = verts[edges[abs(edgeNum)].v[INT32_SIGNBITSET(edgeNum)]] -
          projectionOrigin;
...
int idTraceModel::GetParallelProjectionSilhouetteEdges(
    const idVec3& projectionDir, int silEdges[MAX_TRACEMODEL_EDGES]) const {
  int i, j, edgeNum;
  int edgeIsSilEdge[MAX_TRACEMODEL_EDGES + 1];
  const traceModelPoly_t* poly;
...
void idTraceModel::ProjectionIntegrals(
    int polyNum, int a, int b, struct projectionIntegrals_s& integrals) const {
  const traceModelPoly_t* poly;
  int i, edgeNum;
  idVec3 v1, v2;
  float a0, a1, da;
...
============
*/
void idTraceModel::GetMassProperties(const float density, float& mass,
                                     idVec3& centerOfMass,
