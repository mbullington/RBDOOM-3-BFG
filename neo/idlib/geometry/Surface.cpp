...
int idSurface::Split(const idPlane& plane, const float epsilon,
  ...
  *back = surface[1] = new (TAG_IDLIB_SURFACE) idSurface();
  ...
  for (i = 0; i < edgeIndexes.Num(); i += 3) {
    ...
    switch ((INT32_SIGNBITSET(edgeSplitVertex[e0]) |
             (INT32_SIGNBITSET(edgeSplitVertex[e1]) << 1) |
             (INT32_SIGNBITSET(edgeSplitVertex[e2]) << 2)) ^
            7) {
...
=================
*/
bool idSurface::ClipInPlace(const idPlane& plane, const float epsilon,
                            const bool keepOn) {
  float* dists;
  float f;
  byte* sides;
...
=================
*/
bool idSurface::LineIntersection(const idVec3& start, const idVec3& end,
...
=================
*/
bool idSurface::RayIntersection(const idVec3& start, const idVec3& dir,
