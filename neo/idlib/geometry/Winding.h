...
class idWinding {
 public:
  ...
  int Split(const idPlane& plane, const float epsilon, idWinding** front,
  ...
  // is returned
  idWinding* Clip(const idPlane& plane, const float epsilon = ON_EPSILON,
  ...
  bool InsertPointIfOnEdge(const idVec5& point, const idPlane& plane,
                           const float epsilon = ON_EPSILON);
  bool InsertPointIfOnEdge(const idVec3& point, const idPlane& plane,
                           const float epsilon = ON_EPSILON);
  // add a winding to the convex hull
  void AddToConvexHull(const idWinding* winding, const idVec3& normal,
                       const float epsilon = ON_EPSILON);
  ...
  idWinding* TryMerge(const idWinding& w, const idVec3& normal,
  ...
  bool PlanesConcave(const idWinding& w2, const idVec3& normal1,
                     const idVec3& normal2, float dist1, float dist2) const;

  bool PointInside(const idVec3& normal, const idVec3& point,
                   const float epsilon) const;
  // returns true if the line or ray intersects the winding
  bool LineIntersection(const idPlane& windingPlane, const idVec3& start,
                        const idVec3& end, bool backFaceCull = false) const;
  // intersection point is start + dir * scale
  bool RayIntersection(const idPlane& windingPlane, const idVec3& start,
                       const idVec3& dir, float& scale,
...
inline int idWinding::GetNumPoints() const { return numPoints; }
...
class idFixedWinding : public idWinding {
 public:
  ...
  // returns a SIDE_?
  int Split(idFixedWinding* back, const idPlane& plane,
            const float epsilon = ON_EPSILON);
...
inline idFixedWinding::idFixedWinding() {
  numPoints = 0;
  p = data;
...
inline void idFixedWinding::Clear() { numPoints = 0; }
#endif /* !__WINDING_H__ */
