...
void idLexer::CreatePunctuationTable(const punctuation_t* punctuations) {
  ...
  if (punctuations == default_punctuations) {
    ...
    if (!idLexer::punctuationtable ||
        idLexer::punctuationtable == default_punctuationtable) {
      idLexer::punctuationtable =
          (int*)Mem_Alloc(256 * sizeof(int), TAG_IDLIB_LEXER);
    }
    ...
    idLexer::nextpunctuation =
        (int*)Mem_Alloc(i * sizeof(int), TAG_IDLIB_LEXER);
  }
...
Reads spaces, tabs, C-like comments etc. When a newline character is found, the
scripts line counter is increased. Returns false if there is no token left to be
read.
========================
*/
...
Returns a string up to the \n, but doesn't eat any whitespace at the beginning
of the next line.
========================
*/
...
================
*/
int idLexer::GetLastWhiteSpaceEnd() const { return whiteSpaceEnd_p - buffer; }
...
================
*/
bool idLexer::EndOfFile() { return idLexer::script_p >= idLexer::end_p; }
...
================
*/
int idLexer::NumLinesCrossed() { return idLexer::line - idLexer::lastline; }
...
================
*/
int idLexer::LoadMemory(const char* ptr, int length, const char* name,
                        int startLine) {
  if (idLexer::loaded) {
