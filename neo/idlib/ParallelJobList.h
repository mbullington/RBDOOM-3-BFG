...
enum jobListParallelism_t {
  ...
  JOBLIST_PARALLELISM_MAX_CORES =
      -2,  // use a thread for each logical core (includes hyperthreads)
  JOBLIST_PARALLELISM_MAX_THREADS =
      -3  // use the maximum number of job threads, which can help if there is
          // IO to overlap
};
...
// Admer: prevent Visual Studio from incorrectly highlighting AddJob, as the
// same is defined in winspool.h
#ifdef AddJob
#undef AddJob
#endif
...
class idParallelJobList {
 ...
 public:
  ...
  // Wait for the jobs in this list to finish. Will spin in place if any jobs
  // are not done.
  void Wait();
 ...
 private:
  ...
  idParallelJobList(jobListId_t id, jobListPriority_t priority,
