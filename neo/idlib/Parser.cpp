...
================
*/
void idParser::SetBaseFolder(const char* path) { idLexer::SetBaseFolder(path); }
...
void idParser::AddBuiltinDefines() {
  ...
  for (i = 0; builtin[i].string; i++) {
    define = (define_t*)Mem_Alloc(
        sizeof(define_t) + strlen(builtin[i].string) + 1, TAG_IDLIB_PARSER);
...
================
*/
int idParser::ExpandBuiltinDefine(idToken* deftoken, define_t* define,
...
================
*/
int idParser::ExpandDefine(idToken* deftoken, define_t* define,
                           idToken** firsttoken, idToken** lasttoken) {
...
int idParser::EvaluateTokens(idToken* tokens, signed int* intvalue,
...
================
*/
int idParser::DollarEvaluate(signed int* intvalue, double* floatvalue,
                             int integer) {
...
int idParser::DollarDirective_evalfloat() {
  ...
  token.intvalue = (unsigned int)fabs(
      value);  // DG: use int instead of long for 64bit compatibility
...
The next token should be an open brace. Parses until a matching close brace is
found. Internal brace depths are properly skipped.
========================
*/
const char* idParser::ParseBracedSection(idStr& out, int tabs,
                                         bool parseFirstBrace, char intro,
                                         char outro) {
  idToken token;
  int i, depth;
  bool doTabs;
...
================
*/
void idParser::SetMarker() { marker_p = NULL; }
...
================
*/
int idParser::GetFlags() const { return idParser::flags; }
...
int idParser::LoadFile(const char* filename, bool OSPath) {
  ...
  if (!idParser::definehash) {
    ...
    idParser::definehash = (define_t**)Mem_ClearedAlloc(
        DEFINEHASHSIZE * sizeof(define_t*), TAG_IDLIB_PARSER);
    idParser::AddGlobalDefinesToSource();
  }
  return true;
}
...
================
*/
idParser::~idParser() { idParser::FreeSource(false); }
