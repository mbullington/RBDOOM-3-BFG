...
class idMapBrush : public idMapPrimitive {
 public:
  ...
  static idMapBrush* Parse(idLexer& src, const idVec3& origin,
                           bool newFormat = true,
  ...
  bool WriteValve220(idFile* fp, int primitiveNum,
                     const idVec3& origin) const;  // RB
...
class idMapPatch : public idMapPrimitive, public idSurface_Patch {
 public:
  ...
  ~idMapPatch() {}
  static idMapPatch* Parse(idLexer& src, const idVec3& origin,
                           bool patchDef3 = true,
                           float version = CURRENT_MAP_VERSION);
  bool Write(idFile* fp, int primitiveNum, const idVec3& origin) const;
  const char* GetMaterial() const { return material; }
...
class MapPolygonMesh : public idMapPrimitive {
 public:
  ...
  void ConvertFromBrush(const idMapBrush* brush, int entityNum,
                        int primitiveNum);
  void ConvertFromPatch(const idMapPatch* patch, int entityNum,
                        int primitiveNum);

  static MapPolygonMesh* Parse(idLexer& src, const idVec3& origin,
                               float version = CURRENT_MAP_VERSION);
  bool Write(idFile* fp, int primitiveNum, const idVec3& origin) const;
...
class idMapEntity {
 ...
 public:
  ...
  static idMapEntity* Parse(idLexer& src, bool worldSpawn = false,
                            float version = CURRENT_MAP_VERSION);
...
class idMapFile {
 public:
  ...
  // load a .map file
  bool Parse(const char* filename, bool ignoreRegion = false,
             bool osPath = false);
  bool Write(const char* fileName, const char* ext, bool fromBasePath = true);

  // RB begin
  bool WriteJSON(const char* fileName, const char* ext,
                 bool fromBasePath = true);
  bool ConvertToPolygonMeshFormat();
  bool ConvertToValve220Format();

  // converts Wad texture names to valid Doom 3 materials and gives every entity
  // a unique name
  ...
  static void AddMaterialToCollection(const char* material,
                                      idStrList& textureCollections);
