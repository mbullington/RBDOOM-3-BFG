...
/*
================================================
The *Swap* static template class, idSwap, is used by the SwapClass template
class for performing EndianSwapping.
================================================
*/
class idSwap {
 public:
...
#define SwapBytes(x, y) \
  ...
  static void Little(type& c) {
    // byte swapping pointers is pointless because we should never store
    // pointers on disk
    assert(!IsPointer(c));
  }
...
#define BIG32(v)                                             \
  ((((uint32)(v)) >> 24) | (((uint32)(v)&0x00FF0000) >> 8) | \
   (((uint32)(v)&0x0000FF00) << 8) | ((uint32)(v) << 24))
