...
// This is the default language dict that the entire system uses, but you can
// instantiate your own idLangDict classes to manipulate a language dictionary
// in a tool
idLangDict idLocalization::languageDict;

idCVar lang_maskLocalizedStrings(
    "lang_maskLocalizedStrings", "0", CVAR_BOOL,
    "Masks all localized strings to help debugging.  When set will replace "
    "strings with an equal length of W's and ending in an X.  Note: The "
...
========================
*/
void idLocalization::ClearDictionary() { languageDict.Clear(); }
...
========================
*/
bool idLocalization::LoadDictionary(const byte* data, int dataLen,
                                    const char* fileName) {
...
========================
*/
utf8Encoding_t idLocalization::VerifyUTF8(const uint8* buffer,
                                          const int bufferLen,
                                          const char* name) {
  utf8Encoding_t encoding;
  idStr::IsValidUTF8(buffer, bufferLen, encoding);
  if (encoding == UTF8_INVALID) {
    idLib::FatalError("Language file %s is not valid UTF-8 or plain ASCII.",
                      name);
  } else if (encoding == UTF8_INVALID_BOM) {
    idLib::FatalError(
        "Language file %s is marked as UTF-8 but has invalid encoding.", name);
  } else if (encoding == UTF8_ENCODED_NO_BOM) {
    idLib::FatalError(
        "Language file %s has no byte order marker. Fix this or roll back to a "
        "version that has the marker.",
        name);
...
// recursing up to this many times before we decided someone did something
// stupid
const char* idLangDict::KEY_PREFIX =
    "#str_";  // all keys should be prefixed with this for redirection to work
...
========================
*/
bool idLangDict::Load(const byte* buffer, const int bufferLen,
                      const char* name) {
  ...
  // in all but retail builds, ensure that the byte-order mark is NOT MISSING so
  // that we can avoid debugging UTF-8 code
#ifndef ID_RETAIL
  ...
  if (encoding == UTF8_ENCODED_BOM) {
    ...
    assert(false);  // this should have been handled in VerifyUTF8 with a
                    // FatalError
    return false;
  }
#else
  // in release we just check the BOM so we're not scanning the lang file twice
  // on startup
  if (bufferLen > 3 && buffer[0] == 0xEF && buffer[1] == 0xBB &&
      buffer[2] == 0xBF) {
    utf8 = true;
  }
#endif
  ...
  while (i < bufferLen) {
    ...
    if (c == '/')  // comment, read until new line
    {
      ...
      if (keyEnd < keyStart) {
        idLib::FatalError("%s File ended while reading key at line %d", name,
                          line);
      }
      ...
      if (valStart < 0) {
        idLib::FatalError("%s File ended while reading value at line %d", name,
                          line);
      }
      int valEnd = -1;
      tempVal.CapLength(0);
      while (i < bufferLen) {
        ...
        if (!utf8 && c >= 0x80) {
          // this is a serious error and we must check this to avoid
          // accidentally shipping a file where someone squased UTF-8 encodings
          idLib::FatalError(
              "Language file %s is supposed to be plain ASCII, but has byte "
              "values > 127!",
              name);
        }
        if (c == '\"') {
      ...
      if (lang_maskLocalizedStrings.GetBool() && tempVal.Length() > 0 &&
          tempKey.Find("#font_") == -1) {
...
const char* idLangDict::FindString_r(const char* str, int& depth) const {
  ...
  if (depth > MAX_REDIRECTION_DEPTH) {
    // This isn't an error because we assume the error will be obvious somewhere
    // in a GUI or something, and the whole point of tracking the depth is to
    // avoid a crash.
    idLib::Warning("String '%s', indirection depth > %d", str,
...
========================
*/
int idLangDict::GetNumKeyVals() const { return keyVals.Num(); }
...
========================
*/
const idLangKeyValue* idLangDict::GetKeyVal(int i) const { return &keyVals[i]; }
...
void idStrId::Set(const char* key) {
  if (key == NULL || key[0] == 0) {
    ...
    if (index < 0) {
      // don't allow setting of string ID's to an unknown ID... this should only
      // be allowed from the string table tool because additions from anywhere
      // else are not guaranteed to be saved to the .lang file.
