...
ID_INLINE idBox::idBox(const idVec3& center, const idVec3& extents,
                       const idMat3& axis) {
  this->center = center;
...
ID_INLINE idBox::idBox(const idBounds& bounds, const idVec3& origin,
                       const idMat3& axis) {
  this->center = (bounds[0] + bounds[1]) * 0.5f;
  this->extents = bounds[1] - this->center;
...
ID_INLINE bool idBox::operator==(const idBox& a) const { return Compare(a); }
...
ID_INLINE const idVec3& idBox::GetCenter() const { return center; }

ID_INLINE const idVec3& idBox::GetExtents() const { return extents; }

ID_INLINE const idMat3& idBox::GetAxis() const { return axis; }

ID_INLINE float idBox::GetVolume() const {
  return (extents * 2.0f).LengthSqr();
}

ID_INLINE bool idBox::IsCleared() const { return extents[0] < 0.0f; }
...
ID_INLINE void idBox::AxisProjection(const idVec3& dir, float& min,
                                     float& max) const {
  float d1 = dir * center;
