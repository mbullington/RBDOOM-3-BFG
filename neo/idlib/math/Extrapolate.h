...
class idExtrapolate {
 public:
  ...
  void Init(const int startTime, const int duration, const type& startValue,
            const type& baseSpeed, const type& speed,
...
*/
template <class type>
inline void idExtrapolate<type>::Init(const int startTime, const int duration,
...
*/
template <class type>
inline type idExtrapolate<type>::GetCurrentValue(int time) const {
  if (time < startTime) {
    return startValue;
  }
  ...
  switch (extrapolationType & ~EXTRAPOLATION_NOSTOP) {
    ...
    case EXTRAPOLATION_ACCELLINEAR: {
      if (duration == 0) {
        ...
        const float s =
            (0.5f * deltaTime * deltaTime) * ((float)duration * 0.001f);
    ...
    case EXTRAPOLATION_ACCELSINE: {
      if (duration == 0) {
        ...
        const float deltaTime = (time - startTime) / (float)duration;
        const float s = (1.0f - idMath::Cos(deltaTime * idMath::HALF_PI)) *
                        (float)duration * 0.001f * idMath::SQRT_1OVER2;
        return startValue + deltaTime * baseSpeed + s * speed;
      }
    }
...
*/
template <class type>
inline type idExtrapolate<type>::GetCurrentSpeed(int time) const {
