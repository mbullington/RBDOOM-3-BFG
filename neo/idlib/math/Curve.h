...
*/
template <class type>
inline float idCurve<type>::RombergIntegral(const float t0, const float t1,
...
*/
template <class type>
inline float idCurve<type>::GetLengthBetweenKnots(const int i0,
                                                  const int i1) const {
  float length = 0.0f;
  for (int i = i0; i < i1; i++) {
...
*/
template <class type>
inline float idCurve<type>::GetTimeForLength(const float length,
...
class idCurve_Bezier : public idCurve<type> {
 ...
 protected:
  ...
  void BasisSecondDerivative(const int order, const float t,
                             float* bvals) const;
};
...
*/
template <class type>
inline idCurve_Bezier<type>::idCurve_Bezier() {}
...
*/
template <class type>
inline type idCurve_Bezier<type>::GetCurrentFirstDerivative(
...
*/
template <class type>
inline type idCurve_Bezier<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i;
  float *bvals, d;
  type v;
...
*/
template <class type>
inline void idCurve_Bezier<type>::Basis(const int order, const float t,
                                        float* bvals) const {
  int i, j, d;
...
*/
template <class type>
inline void idCurve_Bezier<type>::BasisFirstDerivative(const int order,
                                                       const float t,
                                                       float* bvals) const {
  int i;
...
*/
template <class type>
inline void idCurve_Bezier<type>::BasisSecondDerivative(const int order,
                                                        const float t,
                                                        float* bvals) const {
  int i;
...
*/
template <class type>
inline idCurve_QuadraticBezier<type>::idCurve_QuadraticBezier() {}
...
*/
template <class type>
inline type idCurve_QuadraticBezier<type>::GetCurrentValue(
    const float time) const {
  float bvals[3];
...
*/
template <class type>
inline type idCurve_QuadraticBezier<type>::GetCurrentFirstDerivative(
    const float time) const {
  float bvals[3], d;
  assert(this->values.Num() == 3);
...
*/
template <class type>
inline type idCurve_QuadraticBezier<type>::GetCurrentSecondDerivative(
    const float time) const {
  float bvals[3], d;
  assert(this->values.Num() == 3);
...
*/
template <class type>
inline void idCurve_QuadraticBezier<type>::Basis(const float t,
                                                 float* bvals) const {
...
*/
template <class type>
inline void idCurve_QuadraticBezier<type>::BasisFirstDerivative(
    const float t, float* bvals) const {
  float s1 = (float)(t - this->times[0]) / (this->times[2] - this->times[0]);
...
*/
template <class type>
inline void idCurve_QuadraticBezier<type>::BasisSecondDerivative(
    const float t, float* bvals) const {
  float s1 = (float)(t - this->times[0]) / (this->times[2] - this->times[0]);
  bvals[0] = 2.0f;
  bvals[1] = -4.0f;
...
*/
template <class type>
inline idCurve_CubicBezier<type>::idCurve_CubicBezier() {}
...
*/
template <class type>
inline type idCurve_CubicBezier<type>::GetCurrentFirstDerivative(
    const float time) const {
  float bvals[4], d;
  assert(this->values.Num() == 4);
  BasisFirstDerivative(time, bvals);
...
*/
template <class type>
inline type idCurve_CubicBezier<type>::GetCurrentSecondDerivative(
    const float time) const {
  float bvals[4], d;
  assert(this->values.Num() == 4);
  BasisSecondDerivative(time, bvals);
...
*/
template <class type>
inline void idCurve_CubicBezier<type>::Basis(const float t,
                                             float* bvals) const {
...
*/
template <class type>
inline void idCurve_CubicBezier<type>::BasisFirstDerivative(
    const float t, float* bvals) const {
  float s1 = (float)(t - this->times[0]) / (this->times[3] - this->times[0]);
  float s2 = s1 * s1;
...
*/
template <class type>
inline void idCurve_CubicBezier<type>::BasisSecondDerivative(
    const float t, float* bvals) const {
  float s1 = (float)(t - this->times[0]) / (this->times[3] - this->times[0]);
...
*/
template <class type>
inline idCurve_Spline<type>::idCurve_Spline() {
...
*/
template <class type>
inline idCurve_NaturalCubicSpline<type>::idCurve_NaturalCubicSpline() {}
...
*/
template <class type>
inline type idCurve_NaturalCubicSpline<type>::GetCurrentValue(
    const float time) const {
...
*/
template <class type>
inline type idCurve_NaturalCubicSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  float clampedTime = this->ClampedTime(time);
  int i = this->IndexForTime(clampedTime);
  float s = time - this->TimeForIndex(i);
...
*/
template <class type>
inline type idCurve_NaturalCubicSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  float clampedTime = this->ClampedTime(time);
  int i = this->IndexForTime(clampedTime);
  float s = time - this->TimeForIndex(i);
...
*/
template <class type>
inline void idCurve_NaturalCubicSpline<type>::Setup() const {
  if (this->changed) {
...
*/
template <class type>
inline void idCurve_NaturalCubicSpline<type>::SetupFree() const {
  int i;
  float inv;
...
*/
template <class type>
inline void idCurve_NaturalCubicSpline<type>::SetupClamped() const {
  int i;
  float inv;
  float *d0, *d1, *beta, *gamma;
  type *alpha, *delta;
...
*/
template <class type>
inline void idCurve_NaturalCubicSpline<type>::SetupClosed() const {
  int i, j;
  float c0, c1;
  float* d0;
  idMatX mat;
...
class idCurve_CatmullRomSpline : public idCurve_Spline<type> {
 ...
 protected:
  ...
  void BasisSecondDerivative(const int index, const float t,
                             float* bvals) const;
};
...
*/
template <class type>
inline idCurve_CatmullRomSpline<type>::idCurve_CatmullRomSpline() {}
...
*/
template <class type>
inline type idCurve_CatmullRomSpline<type>::GetCurrentValue(
    const float time) const {
  int i, j, k;
...
*/
template <class type>
inline type idCurve_CatmullRomSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i, j, k;
...
*/
template <class type>
inline type idCurve_CatmullRomSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i, j, k;
  float bvals[4], d, clampedTime;
  type v;
...
*/
template <class type>
inline void idCurve_CatmullRomSpline<type>::Basis(const int index,
                                                  const float t,
                                                  float* bvals) const {
...
*/
template <class type>
inline void idCurve_CatmullRomSpline<type>::BasisFirstDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
*/
template <class type>
inline void idCurve_CatmullRomSpline<type>::BasisSecondDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
  bvals[0] = -3.0f * s + 2.0f;
...
class idCurve_KochanekBartelsSpline : public idCurve_Spline<type> {
 public:
  ...
  virtual int AddValue(const float time, const type& value);
  virtual int AddValue(const float time, const type& value, const float tension,
                       const float continuity, const float bias);
  virtual void RemoveIndex(const int index) {
...
*/
template <class type>
inline idCurve_KochanekBartelsSpline<type>::idCurve_KochanekBartelsSpline() {}
...
*/
template <class type>
inline int idCurve_KochanekBartelsSpline<type>::AddValue(const float time,
                                                         const type& value) {
  int i;
...
*/
template <class type>
inline type idCurve_KochanekBartelsSpline<type>::GetCurrentValue(
    const float time) const {
  int i;
  float bvals[4], clampedTime;
  type v, t0, t1;
...
*/
template <class type>
inline type idCurve_KochanekBartelsSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i;
  float bvals[4], d, clampedTime;
  type v, t0, t1;
...
*/
template <class type>
inline type idCurve_KochanekBartelsSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i;
  float bvals[4], d, clampedTime;
  type v, t0, t1;
...
*/
template <class type>
inline void idCurve_KochanekBartelsSpline<type>::TangentsForIndex(
    const int index, type& t0, type& t1) const {
...
*/
template <class type>
inline void idCurve_KochanekBartelsSpline<type>::Basis(const int index,
                                                       const float t,
                                                       float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
*/
template <class type>
inline void idCurve_KochanekBartelsSpline<type>::BasisFirstDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
*/
template <class type>
inline void idCurve_KochanekBartelsSpline<type>::BasisSecondDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
class idCurve_BSpline : public idCurve_Spline<type> {
 ...
 protected:
  ...
  float BasisFirstDerivative(const int index, const int order,
                             const float t) const;
  float BasisSecondDerivative(const int index, const int order,
                              const float t) const;
};
...
*/
template <class type>
inline idCurve_BSpline<type>::idCurve_BSpline() {
...
*/
template <class type>
inline type idCurve_BSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i, j, k;
  float clampedTime;
  type v;
...
*/
template <class type>
inline type idCurve_BSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i, j, k;
  float clampedTime;
  type v;
...
*/
template <class type>
inline float idCurve_BSpline<type>::Basis(const int index, const int order,
                                          const float t) const {
  if (order <= 1) {
...
*/
template <class type>
inline float idCurve_BSpline<type>::BasisFirstDerivative(const int index,
                                                         const int order,
                                                         const float t) const {
...
*/
template <class type>
inline float idCurve_BSpline<type>::BasisSecondDerivative(const int index,
                                                          const int order,
                                                          const float t) const {
...
*/
template <class type>
inline idCurve_UniformCubicBSpline<type>::idCurve_UniformCubicBSpline() {
...
*/
template <class type>
inline type idCurve_UniformCubicBSpline<type>::GetCurrentValue(
    const float time) const {
  int i, j, k;
  float bvals[4], clampedTime;
  type v;
...
*/
template <class type>
inline type idCurve_UniformCubicBSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i, j, k;
  float bvals[4], d, clampedTime;
  type v;
...
*/
template <class type>
inline type idCurve_UniformCubicBSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i, j, k;
  float bvals[4], d, clampedTime;
  type v;
...
*/
template <class type>
inline void idCurve_UniformCubicBSpline<type>::Basis(const int index,
                                                     const float t,
                                                     float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
*/
template <class type>
inline void idCurve_UniformCubicBSpline<type>::BasisFirstDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
...
*/
template <class type>
inline void idCurve_UniformCubicBSpline<type>::BasisSecondDerivative(
    const int index, const float t, float* bvals) const {
  float s = (float)(t - this->TimeForIndex(index)) /
            (this->TimeForIndex(index + 1) - this->TimeForIndex(index));
  bvals[0] = -s + 1.0f;
...
class idCurve_NonUniformBSpline : public idCurve_BSpline<type> {
 ...
 protected:
  void Basis(const int index, const int order, const float t,
             float* bvals) const;
  void BasisFirstDerivative(const int index, const int order, const float t,
                            float* bvals) const;
  void BasisSecondDerivative(const int index, const int order, const float t,
                             float* bvals) const;
};
...
*/
template <class type>
inline idCurve_NonUniformBSpline<type>::idCurve_NonUniformBSpline() {}
...
*/
template <class type>
inline type idCurve_NonUniformBSpline<type>::GetCurrentValue(
    const float time) const {
  int i, j, k;
  float clampedTime;
  type v;
...
*/
template <class type>
inline type idCurve_NonUniformBSpline<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i, j, k;
  float clampedTime;
  type v;
...
*/
template <class type>
inline type idCurve_NonUniformBSpline<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i, j, k;
  float clampedTime;
  type v;
...
*/
template <class type>
inline void idCurve_NonUniformBSpline<type>::Basis(const int index,
                                                   const int order,
                                                   const float t,
                                                   float* bvals) const {
  int r, s, i;
...
*/
template <class type>
inline void idCurve_NonUniformBSpline<type>::BasisFirstDerivative(
    const int index, const int order, const float t, float* bvals) const {
  int i;
...
*/
template <class type>
inline void idCurve_NonUniformBSpline<type>::BasisSecondDerivative(
    const int index, const int order, const float t, float* bvals) const {
  int i;
...
*/
template <class type>
inline idCurve_NURBS<type>::idCurve_NURBS() {}
...
*/
template <class type>
inline type idCurve_NURBS<type>::GetCurrentFirstDerivative(
    const float time) const {
  int i, j, k;
...
*/
template <class type>
inline type idCurve_NURBS<type>::GetCurrentSecondDerivative(
    const float time) const {
  int i, j, k;
