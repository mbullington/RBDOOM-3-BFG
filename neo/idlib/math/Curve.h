...
*/
template <class type>
ID_INLINE float idCurve<type>::RombergIntegral(const float t0, const float t1,
...
*/
template <class type>
ID_INLINE void idCurve_Bezier<type>::Basis(const int order, const float t,
                                           float* bvals) const {
  int i, j, d;
...
ID_INLINE int idCurve_KochanekBartelsSpline<type>::AddValue(
    const float time, const type& value, const float tension,
    const float continuity, const float bias) {
  int i;
...
*/
template <class type>
ID_INLINE float idCurve_BSpline<type>::Basis(const int index, const int order,
                                             const float t) const {
  if (order <= 1) {
...
ID_INLINE float idCurve_BSpline<type>::BasisFirstDerivative(
    const int index, const int order, const float t) const {
