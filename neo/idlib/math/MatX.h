...
#define MATX_ALLOCA_CACHE_LINES(n)                                  \
  ((float*)_alloca128(((n) * sizeof(float) + CACHE_LINE_SIZE - 1) & \
                      ~(CACHE_LINE_SIZE - 1)))
...
class idMatX {
 public:
  ...
  bool LU_UpdateRankOne(const idVecX& v, const idVecX& w, float alpha,
                        int* index);
  ...
  bool LU_UpdateDecrement(const idVecX& v, const idVecX& w, const idVecX& u,
                          int r, int* index);
  ...
  bool QR_UpdateRankOne(idMatX& R, const idVecX& v, const idVecX& w,
                        float alpha);
  ...
  void QR_Solve(idVecX& x, const idVecX& b, const idVecX& c,
  ...
  void QR_UnpackFactors(idMatX& Q, idMatX& R, const idVecX& c,
                        const idVecX& d) const;
  ...
  void SVD_Solve(idVecX& x, const idVecX& b, const idVecX& w,
 ...
 private:
  ...
  void ComplexDivision(float xr, float xi, float yr, float yi, float& cdivr,
                       float& cdivi);
  bool HessenbergToRealSchur(idMatX& H, idVecX& realEigenValues,
...
#ifdef MATX_SIMD
  for (int i = 0; i < s; i += 4) {
    _mm_store_ps(m.mat + i,
                 _mm_add_ps(_mm_load_ps(mat + i), _mm_load_ps(a.mat + i)));
  }
#else
  for (int i = 0; i < s; i++) {
...
inline idMatX idMatX::operator-(const idMatX& a) const {
  ...
  for (int i = 0; i < s; i += 4) {
    _mm_store_ps(m.mat + i,
                 _mm_sub_ps(_mm_load_ps(mat + i), _mm_load_ps(a.mat + i)));
  }
#else
  for (int i = 0; i < s; i++) {
    m.mat[i] = mat[i] - a.mat[i];
...
inline void idMatX::Negate() {
  ...
  for (int i = 0; i < s; i += 4) {
    _mm_store_ps(mat + i,
                 _mm_xor_ps(_mm_load_ps(mat + i), (__m128&)signBit[0]));
  }
#else
  for (int i = 0; i < s; i++) {
    mat[i] = -mat[i];
