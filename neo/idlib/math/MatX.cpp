...
void idMatX::CopyLowerToUpperTriangle() {
  ...
  // const __m128 bottomMask[2] = { __m128c( _mm_set1_epi32( 0 ) ), __m128c(
  // _mm_set1_epi32( -1 ) ) }; compilers (e.g. MCST lcc, Intel icc) that use the
  // C++ Front End from EDG (Edison Design Group) have a bug
  // - no suitable conversion function from "__m128c" to "float"
  // an explicit type conversion is used as a solution to this bug
  const __m128 bottomMask[2] = {(__m128)__m128c(_mm_set1_epi32(0)),
                                (__m128)__m128c(_mm_set1_epi32(-1))};
  ...
  if (m & 3) {
    __m128 r0 = _mm_and_ps(_mm_load_ps(basePtr + n0), mask0);
    __m128 r1 = _mm_and_ps(_mm_load_ps(basePtr + n1_masked),
                           _mm_and_ps(mask1, bottomMask[b1]));
    __m128 r2 = _mm_and_ps(_mm_load_ps(basePtr + n2_masked),
                           _mm_and_ps(mask2, bottomMask[b2]));
    __m128 r3 = _mm_setzero_ps();
...
============
*/
void idMatX::Update_Decrement(int r) { RemoveRowColumn(r); }
...
============
*/
bool idMatX::Inverse_UpdateRankOne(const idVecX& v, const idVecX& w,
                                   float alpha) {
  int i, j;
  float beta, s;
  idVecX y, z;
...
============
*/
void idMatX::Inverse_Solve(idVecX& x, const idVecX& b) const { Multiply(x, b); }
...
============
*/
bool idMatX::LU_UpdateRankOne(const idVecX& v, const idVecX& w, float alpha,
                              int* index) {
  int i, j, max;
  float *y, *z;
...
============
*/
bool idMatX::LU_UpdateRowColumn(const idVecX& v, const idVecX& w, int r,
                                int* index) {
#if 0
...
============
*/
bool idMatX::LU_UpdateDecrement(const idVecX& v, const idVecX& w,
                                const idVecX& u, int r, int* index) {
  int i, p;
  idVecX v1, w1;
...
============
*/
bool idMatX::QR_UpdateRankOne(idMatX& R, const idVecX& v, const idVecX& w,
                              float alpha) {
  int i, k;
  float f;
  idVecX u;
...
============
*/
bool idMatX::QR_UpdateRowColumn(idMatX& R, const idVecX& v, const idVecX& w,
                                int r) {
  idVecX s;
...
============
*/
bool idMatX::QR_UpdateDecrement(idMatX& R, const idVecX& v, const idVecX& w,
                                int r) {
  idVecX v1, w1;
...
============
*/
void idMatX::QR_Solve(idVecX& x, const idVecX& b, const idVecX& c,
...
============
*/
void idMatX::QR_UnpackFactors(idMatX& Q, idMatX& R, const idVecX& c,
                              const idVecX& d) const {
  int i, j, k;
  double sum;
...
============
*/
void idMatX::QR_MultiplyFactors(idMatX& m, const idVecX& c,
                                const idVecX& d) const {
  int i, j, k;
  double sum;
  idMatX Q;
    ...
    for (its = 1; its <= 30; its++) {
      ...
      for (l = k; l >= 0; l--) {
        ...
        if ((idMath::Fabs(rv1[l]) + anorm) ==
            anorm /* idMath::Fabs( rv1[l] ) < idMath::FLOAT_EPSILON */) {
          flag = 0;
          break;
        }
        if ((idMath::Fabs(w[nm]) + anorm) ==
            anorm /* idMath::Fabs( w[nm] ) < idMath::FLOAT_EPSILON */) {
          break;
        }
      }
      if (flag) {
        ...
        for (i = l; i <= k; i++) {
          ...
          if ((idMath::Fabs(f) + anorm) !=
              anorm /* idMath::Fabs( f ) > idMath::FLOAT_EPSILON */) {
            g = w[i];
            h = Pythag(f, g);
            w[i] = h;
            h = 1.0f / h;
...
============
*/
void idMatX::SVD_Solve(idVecX& x, const idVecX& b, const idVecX& w,
...
============
*/
void idMatX::SVD_MultiplyFactors(idMatX& m, const idVecX& w,
                                 const idMatX& V) const {
  int r, i, j;
  double sum;
...
============
*/
void idMatX::ComplexDivision(float xr, float xi, float yr, float yi,
                             float& cdivr, float& cdivi) {
  float r, d;
...
============
*/
bool idMatX::HessenbergToRealSchur(idMatX& H, idVecX& realEigenValues,
