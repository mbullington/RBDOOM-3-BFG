...
============
*/
void PrintClocks(const char* string, int dataCount, int clocks,
                 int otherClocks = 0) {
  int i;
...
void TestMemcpy() {
  ...
  PrintClocks(va("   simd->Memcpy() %s", result), BIG_COUNT, bestClocksSIMD,
              bestClocksGeneric);
}
...
void TestMemset() {
  ...
  result = (i >= BIG_COUNT) ? "ok" : S_COLOR_RED "X";
  PrintClocks(va("   simd->Memset() %s", result), BIG_COUNT, bestClocksSIMD,
              bestClocksGeneric);
  ...
  result = (i >= BIG_COUNT) ? "ok" : S_COLOR_RED "X";
  PrintClocks(va("   simd->Memset( 0 ) %s", result), BIG_COUNT, bestClocksSIMD,
              bestClocksGeneric);
}
...
void TestBlendJoints() {
  ...
  for (i = 0; i < NUMTESTS; i++) {
    ...
    StartRecordTime(start);
    p_generic->BlendJoints(joints1.Ptr(), blendJoints.Ptr(), lerp, index.Ptr(),
                           COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksGeneric);
  }
  ...
  for (i = 0; i < NUMTESTS; i++) {
    ...
    StartRecordTime(start);
    p_simd->BlendJoints(joints2.Ptr(), blendJoints.Ptr(), lerp, index.Ptr(),
                        COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksSIMD);
  }
...
void TestBlendJointsFast() {
  ...
  for (i = 0; i < NUMTESTS; i++) {
    ...
    StartRecordTime(start);
    p_generic->BlendJointsFast(joints1.Ptr(), blendJoints.Ptr(), lerp,
                               index.Ptr(), COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksGeneric);
  }
  ...
  for (i = 0; i < NUMTESTS; i++) {
    ...
    StartRecordTime(start);
    p_simd->BlendJointsFast(joints2.Ptr(), blendJoints.Ptr(), lerp, index.Ptr(),
                            COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksSIMD);
  }
  ...
  result = (i >= COUNT) ? "ok" : S_COLOR_RED "X";
  PrintClocks(va("   simd->BlendJointsFast() %s", result), COUNT,
              bestClocksSIMD, bestClocksGeneric);
}
...
void TestConvertJointQuatsToJointMats() {
  ...
  for (i = 0; i < NUMTESTS; i++) {
    StartRecordTime(start);
    p_generic->ConvertJointQuatsToJointMats(joints1.Ptr(), baseJoints.Ptr(),
                                            COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksGeneric);
  }
  ...
  for (i = 0; i < NUMTESTS; i++) {
    StartRecordTime(start);
    p_simd->ConvertJointQuatsToJointMats(joints2.Ptr(), baseJoints.Ptr(),
                                         COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksSIMD);
  }
...
void TestConvertJointMatsToJointQuats() {
  ...
  for (i = 0; i < NUMTESTS; i++) {
    StartRecordTime(start);
    p_generic->ConvertJointMatsToJointQuats(joints1.Ptr(), baseJoints.Ptr(),
                                            COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksGeneric);
  }
  ...
  for (i = 0; i < NUMTESTS; i++) {
    StartRecordTime(start);
    p_simd->ConvertJointMatsToJointQuats(joints2.Ptr(), baseJoints.Ptr(),
                                         COUNT);
    StopRecordTime(end);
    GetBest(start, end, bestClocksSIMD);
  }
