...
static idCVar lcp_showFailures("lcp_showFailures", "0", CVAR_BOOL,
...
ALIGN16(const __m128 SIMD_SP_infinity) = {idMath::INFINITUM, idMath::INFINITUM,
                                          idMath::INFINITUM, idMath::INFINITUM};
ALIGN16(const __m128 SIMD_SP_LCP_DELTA_ACCEL_EPSILON) = {
    LCP_DELTA_ACCEL_EPSILON, LCP_DELTA_ACCEL_EPSILON, LCP_DELTA_ACCEL_EPSILON,
    LCP_DELTA_ACCEL_EPSILON};
ALIGN16(const __m128 SIMD_SP_LCP_DELTA_FORCE_EPSILON) = {
    LCP_DELTA_FORCE_EPSILON, LCP_DELTA_FORCE_EPSILON, LCP_DELTA_FORCE_EPSILON,
    LCP_DELTA_FORCE_EPSILON};
ALIGN16(const __m128 SIMD_SP_LCP_BOUND_EPSILON) = {
    LCP_BOUND_EPSILON, LCP_BOUND_EPSILON, LCP_BOUND_EPSILON, LCP_BOUND_EPSILON};
ALIGN16(const __m128 SIMD_SP_neg_LCP_BOUND_EPSILON) = {
    -LCP_BOUND_EPSILON, -LCP_BOUND_EPSILON, -LCP_BOUND_EPSILON,
    -LCP_BOUND_EPSILON};
ALIGN16(const unsigned int SIMD_SP_signBit[4]) = {
    IEEE_FLT_SIGN_MASK, IEEE_FLT_SIGN_MASK, IEEE_FLT_SIGN_MASK,
    IEEE_FLT_SIGN_MASK};
// DG: cast IEEE_FLT_SIGN_MASK to uint before interting it, as uint and not long
// values are expected here
#define INV_IEEE_FLT_SIGN_MASK (~((unsigned int)IEEE_FLT_SIGN_MASK))
ALIGN16(const unsigned int SIMD_SP_absMask[4]) = {
    INV_IEEE_FLT_SIGN_MASK, INV_IEEE_FLT_SIGN_MASK, INV_IEEE_FLT_SIGN_MASK,
...
ALIGN16(const unsigned int SIMD_SP_indexedStartMask[4][4]) = {
    ...
    {0, 0, 0, 0xFFFFFFFF}};
ALIGN16(const unsigned int SIMD_SP_indexedEndMask[4][4]) = {
    {0, 0, 0, 0},
    {0xFFFFFFFF, 0, 0, 0},
    {0xFFFFFFFF, 0xFFFFFFFF, 0, 0},
    {0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0}};
ALIGN16(const unsigned int SIMD_SP_clearLast1[4]) = {0xFFFFFFFF, 0xFFFFFFFF,
                                                     0xFFFFFFFF, 0};
ALIGN16(const unsigned int SIMD_SP_clearLast2[4]) = {0xFFFFFFFF, 0xFFFFFFFF, 0,
                                                     0};
...
========================
*/
static void Multiply_SIMD(float* dst, const float* src0, const float* src1,
                          const int count) {
  int i = 0;
...
========================
*/
static void MultiplyAdd_SIMD(float* dst, const float constant, const float* src,
                             const int count) {
  int i = 0;
...
========================
*/
static float DotProduct_SIMD(const float* src0, const float* src1,
                             const int count) {
...
Solves x in Lx = b for the n * n sub-matrix of L.
        * if skip > 0 the first skip elements of x are assumed to be valid
already
        * L has to be a lower triangular matrix with (implicit) ones on the
diagonal
        * x == b is allowed
========================
*/
static void LowerTriangularSolve_SIMD(const idMatX& L, float* x, const float* b,
...
========================
*/
static void LowerTriangularSolveTranspose_SIMD(const idMatX& L, float* x,
  ...
  for (int i = m; i >= 4; i -= 4) {
    ...
    for (int j = i; j < m; j += 4) {
      ...
      s0 = _mm_sub_ps(
          s0, _mm_mul_ps(_mm_splat_ps(xj, 0), _mm_load_ps(lptr2 + 0 * nc)));
      s1 = _mm_sub_ps(
          s1, _mm_mul_ps(_mm_splat_ps(xj, 1), _mm_load_ps(lptr2 + 1 * nc)));
      s2 = _mm_sub_ps(
          s2, _mm_mul_ps(_mm_splat_ps(xj, 2), _mm_load_ps(lptr2 + 2 * nc)));
      s3 = _mm_sub_ps(
          s3, _mm_mul_ps(_mm_splat_ps(xj, 3), _mm_load_ps(lptr2 + 3 * nc)));
    ...
    for (int j = 0; j < r; j++) {
      s0 = _mm_sub_ps(s0,
                      _mm_mul_ps(_mm_load_ps(lptr2), _mm_load1_ps(&xptr2[j])));
      lptr2 += nc;
    }
    s0 = _mm_add_ps(s0, s1);
    s2 = _mm_add_ps(s2, s3);
    ...
    lptr -= 4 * nc;
    __m128 t0 =
        _mm_and_ps(_mm_load_ps(lptr + 3 * nc), (__m128&)SIMD_SP_clearLast1);
    __m128 t1 =
        _mm_and_ps(_mm_load_ps(lptr + 2 * nc), (__m128&)SIMD_SP_clearLast2);
...
Solves x in Ux = b for the n * n sub-matrix of U.
        ...
        * invDiag is the reciprical of the diagonal of the upper triangular
matrix.
        * x == b is allowed
========================
*/
...
In-place factorization LU of the n * n sub-matrix of mat. The reciprocal of the
diagonal elements of U are stored in invDiag. No pivoting is used.
========================
*/
...
In-place factorization LDL' of the n * n sub-matrix of mat. The reciprocal of
the diagonal elements are stored in invDiag.
...
static bool LDLT_Factor_SIMD(idMatX& mat, idVecX& invDiag, const int n) {
  ...
  for (int i = 4; i < n; i++) {
    ...
    for (; j < n - 3; j += 4) {
      ...
      va =
          _mm_sub_ps(va, _mm_mul_ps(_mm_and_ps(_mm_load_ps(ra + k), mask), v0));
      vb =
          _mm_sub_ps(vb, _mm_mul_ps(_mm_and_ps(_mm_load_ps(rb + k), mask), v0));
      vc =
          _mm_sub_ps(vc, _mm_mul_ps(_mm_and_ps(_mm_load_ps(rc + k), mask), v0));
      vd =
          _mm_sub_ps(vd, _mm_mul_ps(_mm_and_ps(_mm_load_ps(rd + k), mask), v0));
    ...
    for (; j < n; j++) {
      ...
      v0 = _mm_load_ps(v + k);
      va = _mm_sub_ps(va,
                      _mm_mul_ps(_mm_and_ps(_mm_load_ps(mptr + k), mask), v0));
...
========================
*/
static void GetMaxStep_SIMD(const float* f, const float* a,
                            const float* delta_f, const float* delta_a,
                            const float* lo, const float* hi, const int* side,
                            int numUnbounded, int numClamped, int d, float dir,
                            float& maxStep, int& limit, int& limitSide) {
#if defined(LCP_SIMD)
  __m128 vMaxStep;
  __m128i vLimit;
  __m128i vLimitSide;
  ...
  {
    __m128 vNegAccel =
        _mm_xor_ps(_mm_load1_ps(a + d), (__m128&)SIMD_SP_signBit);
    __m128 vDeltaAccel = _mm_load1_ps(delta_a + d);
    __m128 vM0 = _mm_cmpgt_ps(_mm_and_ps(vDeltaAccel, (__m128&)SIMD_SP_absMask),
                              SIMD_SP_LCP_DELTA_ACCEL_EPSILON);
    __m128 vStep =
        _mm_div32_ps(vNegAccel, _mm_sel_ps(SIMD_SP_one, vDeltaAccel, vM0));
  ...
  // test the current variable
  {
    ...
    __m128 vForceLimit =
        _mm_sel_ps(_mm_load1_ps(hi + d), _mm_load1_ps(lo + d), vSign);
    __m128 vStep =
        _mm_div32_ps(_mm_sub_ps(vForceLimit, _mm_load1_ps(f + d)), vDeltaForce);
    __m128i vSetSide = _mm_or_si128(__m128c(vSign), (__m128i&)SIMD_DW_one);
    __m128 vM0 = _mm_cmpgt_ps(_mm_and_ps(vDeltaForce, (__m128&)SIMD_SP_absMask),
                              SIMD_SP_LCP_DELTA_FORCE_EPSILON);
    __m128 vM1 = _mm_cmpneq_ps(
        _mm_and_ps(vForceLimit, (__m128&)SIMD_SP_absMask), SIMD_SP_infinity);
  ...
  // test the clamped bounded variables
  {
    ...
    __m128i index = _mm_add_epi32(
        _mm_and_si128(_mm_shuffle_epi32(_mm_cvtsi32_si128(numUnbounded), 0),
                      (__m128i&)SIMD_DW_not3),
        (__m128i&)SIMD_DW_index);
    int i = numUnbounded & ~3;
    for (; i < numClamped - 3; i += 4) {
      __m128 vDeltaForce = _mm_and_ps(_mm_load_ps(delta_f + i), mask);
      __m128 vSign = _mm_cmplt_ps(vDeltaForce, SIMD_SP_zero);
      __m128 vForceLimit =
          _mm_sel_ps(_mm_load_ps(hi + i), _mm_load_ps(lo + i), vSign);
      __m128 vM0 =
          _mm_cmpgt_ps(_mm_and_ps(vDeltaForce, (__m128&)SIMD_SP_absMask),
                       SIMD_SP_LCP_DELTA_FORCE_EPSILON);
      __m128 vStep = _mm_div32_ps(_mm_sub_ps(vForceLimit, _mm_load_ps(f + i)),
                                  _mm_sel_ps(SIMD_SP_one, vDeltaForce, vM0));
      __m128i vSetSide = _mm_or_si128(__m128c(vSign), (__m128i&)SIMD_DW_one);
      __m128 vM1 = _mm_cmpneq_ps(
          _mm_and_ps(vForceLimit, (__m128&)SIMD_SP_absMask), SIMD_SP_infinity);
      __m128 vM2 = _mm_cmplt_ps(vStep, vMaxStep);
    ...
    __m128 vDeltaForce = _mm_and_ps(
        _mm_load_ps(delta_f + i),
        _mm_and_ps(mask, (__m128&)SIMD_SP_indexedEndMask[numClamped & 3]));
    __m128 vSign = _mm_cmplt_ps(vDeltaForce, SIMD_SP_zero);
    __m128 vForceLimit =
        _mm_sel_ps(_mm_load_ps(hi + i), _mm_load_ps(lo + i), vSign);
    __m128 vM0 = _mm_cmpgt_ps(_mm_and_ps(vDeltaForce, (__m128&)SIMD_SP_absMask),
  ...
  // test the not clamped bounded variables
  {
    ...
    __m128i index = _mm_add_epi32(
        _mm_and_si128(_mm_shuffle_epi32(_mm_cvtsi32_si128(numClamped), 0),
                      (__m128i&)SIMD_DW_not3),
        (__m128i&)SIMD_DW_index);
    int i = numClamped & ~3;
    for (; i < d - 3; i += 4) {
      __m128 vNegAccel =
          _mm_xor_ps(_mm_load_ps(a + i), (__m128&)SIMD_SP_signBit);
      __m128 vDeltaAccel = _mm_and_ps(_mm_load_ps(delta_a + i), mask);
      __m128 vSide = _mm_cvtepi32_ps(_mm_load_si128((__m128i*)(side + i)));
      __m128 vM0 = _mm_cmpgt_ps(_mm_mul_ps(vSide, vDeltaAccel),
                                SIMD_SP_LCP_DELTA_ACCEL_EPSILON);
      __m128 vStep =
          _mm_div32_ps(vNegAccel, _mm_sel_ps(SIMD_SP_one, vDeltaAccel, vM0));
      __m128 vM1 = _mm_or_ps(
          _mm_cmplt_ps(_mm_load_ps(lo + i), SIMD_SP_neg_LCP_BOUND_EPSILON),
          _mm_cmpgt_ps(_mm_load_ps(hi + i), SIMD_SP_LCP_BOUND_EPSILON));
      __m128 vM2 = _mm_cmplt_ps(vStep, vMaxStep);
      __m128 vM3 = _mm_and_ps(_mm_and_ps(vM0, vM1), vM2);
      vMaxStep = _mm_sel_ps(vMaxStep, vStep, vM3);
      vLimit = _mm_sel_si128(vLimit, index, vM3);
      vLimitSide =
          _mm_sel_si128(vLimitSide, (__m128i&)SIMD_DW_zero, __m128c(vM3));
      mask = (__m128&)SIMD_SP_indexedStartMask[0];
      index = _mm_add_epi32(index, (__m128i&)SIMD_DW_four);
    }
    __m128 vNegAccel = _mm_xor_ps(_mm_load_ps(a + i), (__m128&)SIMD_SP_signBit);
    __m128 vDeltaAccel =
        _mm_and_ps(_mm_load_ps(delta_a + i),
                   _mm_and_ps(mask, (__m128&)SIMD_SP_indexedEndMask[d & 3]));
    __m128 vSide = _mm_cvtepi32_ps(_mm_load_si128((__m128i*)(side + i)));
    __m128 vM0 = _mm_cmpgt_ps(_mm_mul_ps(vSide, vDeltaAccel),
                              SIMD_SP_LCP_DELTA_ACCEL_EPSILON);
    __m128 vStep =
  ...
  {
    __m128 tMaxStep =
        _mm_shuffle_ps(vMaxStep, vMaxStep, _MM_SHUFFLE(1, 0, 3, 2));
  ...
  {
    __m128 tMaxStep =
        _mm_shuffle_ps(vMaxStep, vMaxStep, _MM_SHUFFLE(2, 3, 0, 1));
...
========================
*/
static void PrintClocks(const char* string, int dataCount, int64 clocks,
                        int64 otherClocks = 0) {
  idLib::Printf(string);
  ...
  if (clocks && otherClocks) {
    int p = 0;
    if (clocks <= otherClocks) {
      p = idMath::Ftoi((float)(otherClocks - clocks) * 100.0f /
                       (float)otherClocks);
    } else {
...
static void DotProduct_Test() {
  ...
  for (int i = 0; i < TEST_TRIANGULAR_SOLVE_SIZE; i++) {
    ...
    const char* result =
        idMath::Fabs(dot1 - dot2) < 1e-4f ? "ok" : S_COLOR_RED "X";
    PrintClocks(va("DotProduct_SIMD    %d %s", i, result), 1, clocksSIMD,
                clocksGeneric);
  }
}
...
static void LowerTriangularSolve_Test() {
  ...
  for (int i = 1; i < TEST_TRIANGULAR_SOLVE_SIZE; i++) {
    ...
    PrintClocks(va("LowerTriangularSolve_Generic %dx%d", i, i), 1,
                clocksGeneric);
    ...
    const char* result = x.Compare(tst, TEST_TRIANGULAR_SOLVE_SIMD_EPSILON)
                             ? "ok"
                             : S_COLOR_RED "X";
    PrintClocks(va("LowerTriangularSolve_SIMD    %dx%d %s", i, i, result), 1,
                clocksSIMD, clocksGeneric);
  }
}
...
static void LowerTriangularSolveTranspose_Test() {
  ...
  for (int i = 1; i < TEST_TRIANGULAR_SOLVE_SIZE; i++) {
    ...
    for (int j = 0; j < NUM_TESTS; j++) {
      timer.Clear();
      timer.Start();
      LowerTriangularSolveTranspose_Generic(L, x.ToFloatPtr(), b.ToFloatPtr(),
                                            i);
      timer.Stop();
      clocksGeneric = Min(clocksGeneric, timer.ClockTicks());
    }
    ...
    PrintClocks(va("LowerTriangularSolveTranspose_Generic %dx%d", i, i), 1,
                clocksGeneric);
    ...
    const char* result = x.Compare(tst, TEST_TRIANGULAR_SOLVE_SIMD_EPSILON)
                             ...
                             : S_COLOR_RED "X";
    PrintClocks(
        va("LowerTriangularSolveTranspose_SIMD    %dx%d %s", i, i, result), 1,
        clocksSIMD, clocksGeneric);
  }
}
...
static void LDLT_Factor_Test() {
  ...
  for (int i = 1; i < TEST_FACTOR_SOLVE_SIZE; i++) {
    ...
    const char* result =
        mat1.Compare(mat2, TEST_FACTOR_SIMD_EPSILON) &&
                invDiag1.Compare(invDiag2, TEST_FACTOR_SIMD_EPSILON)
            ? "ok"
            : S_COLOR_RED "X";
    PrintClocks(va("LDLT_Factor_SIMD    %dx%d %s", i, i, result), 1, clocksSIMD,
                clocksGeneric);
  }
}
#endif
...
class idLCP_Square : public idLCP {
 public:
  virtual bool Solve(const idMatX& o_m, idVecX& o_x, const idVecX& o_b,
                     const idVecX& o_lo, const idVecX& o_hi,
...
void idLCP_Square::SolveClamped(idVecX& x, const float* b) {
  ...
  // solve U
  UpperTriangularSolve(clamped, diagonal.ToFloatPtr(), x.ToFloatPtr(),
                       x.ToFloatPtr(), numClamped);
}
...
========================
*/
inline void idLCP_Square::CalcAccelDelta(int d) {
...
========================
*/
inline void idLCP_Square::ChangeForce(int d, float step) {
...
========================
*/
bool idLCP_Square::Solve(const idMatX& o_m, idVecX& o_x, const idVecX& o_b,
                         const idVecX& o_lo, const idVecX& o_hi,
                         const int* o_boxIndex) {
  ...
  for (int i = numUnbounded; i < m.GetNumRows(); i++) {
    ...
    for (; n < maxIterations; n++) {
      ...
      GetMaxStep(f.ToFloatPtr(), a.ToFloatPtr(), delta_f.ToFloatPtr(),
                 delta_a.ToFloatPtr(), lo.ToFloatPtr(), hi.ToFloatPtr(), side,
                 numUnbounded, numClamped, i, dir, maxStep, limit, limitSide);
...
void idLCP_Symmetric::SolveClamped(idVecX& x, const float* b) {
  // solve L
  LowerTriangularSolve(clamped, solveCache1.ToFloatPtr(), b, numClamped,
                       clampedChangeStart);

  // scale with D
  Multiply(solveCache2.ToFloatPtr(), solveCache1.ToFloatPtr(),
           diagonal.ToFloatPtr(), numClamped);

  // solve Lt
  LowerTriangularSolveTranspose(clamped, x.ToFloatPtr(),
                                solveCache2.ToFloatPtr(), numClamped);
...
void idLCP_Symmetric::AddClamped(int r, bool useSolveCache) {
  ...
  if (useSolveCache) {
    ...
    dot = BigDotProduct(solveCache2.ToFloatPtr(), solveCache1.ToFloatPtr(),
                        numClamped);
...
========================
*/
inline void idLCP_Symmetric::CalcForceDelta(int d, float dir) {
  delta_f[d] = dir;
...
========================
*/
inline void idLCP_Symmetric::CalcAccelDelta(int d) {
  // only the not clamped variables, including the current variable, can have a
  // change in acceleration
  for (int j = numClamped; j <= d; j++) {
    // only the clamped variables and the current variable have a force delta
...
========================
*/
inline void idLCP_Symmetric::ChangeForce(int d, float step) {
  // only the clamped variables and current variable have a force delta unequal
  // zero
  MultiplyAdd(f.ToFloatPtr(), step, delta_f.ToFloatPtr(), numClamped);
  f[d] += step * delta_f[d];
...
========================
*/
inline void idLCP_Symmetric::ChangeAccel(int d, float step) {
  // only the not clamped variables, including the current variable, can have an
  // acceleration unequal zero
  MultiplyAdd(a.ToFloatPtr() + numClamped, step,
              delta_a.ToFloatPtr() + numClamped, d - numClamped + 1);
...
========================
*/
bool idLCP_Symmetric::Solve(const idMatX& o_m, idVecX& o_x, const idVecX& o_b,
                            const idVecX& o_lo, const idVecX& o_hi,
                            const int* o_boxIndex) {
  // true when the matrix rows are 16 byte padded
  padded = ((o_m.GetNumRows() + 3) & ~3) == o_m.GetNumColumns();
  ...
  // sub matrix for factorization
  clamped.SetDataCacheLines(
      m.GetNumRows(), m.GetNumColumns(),
      MATX_ALLOCA_CACHE_LINES(m.GetNumRows() * m.GetNumColumns()), true);
  diagonal.SetData(m.GetNumRows(), VECX_ALLOCA(m.GetNumRows()));
...
========================
*/
void idLCP::SetMaxIterations(int max) { maxIterations = max; }
...
========================
*/
int idLCP::GetMaxIterations() { return maxIterations; }
