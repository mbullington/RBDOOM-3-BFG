...
idODSCachedObject			// DMA in a single object through a
software cache
idODSArray					// DMA in an array with objects
idODSIndexedArray			// DMA gather from an array with objects
idODSStreamedArray			// overlapped DMA streaming of an array
with objects idODSStreamedIndexedArray	// overlapped DMA gather from an array
with objects

On the SPU the 'idODSObject' streams the data into temporary memory using the
DMA controller and the object constructor immediately waits for the DMA transfer
to complete. In other words there is no caching and every random memory access
incurs full memory latency. This should be used to stream in objects that are
only used once at unpredictable times.

The 'idODSCachedObject' uses an object based software cache on the SPU which is
useful for streaming in objects that may be used repeatedly or which usage can
be predicted allowing the objects to be prefetched.
...
The 'idSoftwareCache' implements a Prefetch() function that can be used to
prefetch whole objects into the cache well before they are needed. However, any
idODSObject, idODSArray, idODSIndexedArray etc. after calling the Prefetch()
function will have to wait for the prefetch to complete. In other words, make
sure there is enough "work" done in between a Prefetch() call and the first next
idODS* object.

The 'idODSArray' streams in a block of objects that are tightly packed in
memory.

The 'idODSIndexedArray' is used to gather a number of objects that are not
necessarily contiguous in memory. On the SPU a DMA-list is used in the
'idODSIndexedArray' constructor to efficiently gather all the objects.

The 'idODSStreamedArray' is used for sequentially reading a large input array.
Overlapped streaming is used where one batch of array elements can be accessed
while the next batch is being streamed in.

The 'idODSStreamedIndexedArray' is used for gathering elements from an array
using a sequentially read index. Overlapped streaming is used for both the index
and the array elements where one batch of array elements can be accessed while
the next batch of indices/array elements is being streamed in.

Outside the SPU, data is never copied to temporary memory because this would
cause significant load-hit-store penalties. Instead, the object constructor
issues prefetch instructions where appropriate and only maintains pointers to
the actual data. In the case of 'idODSObject' or 'idODSCachedObject' the class
is no more than a simple wrapper of a pointer and the class should completely
compile away with zero overhead.

COMMON MISTAKES:

1. When using ODS objects do not forget to set the "globalDmaTag" that is used
to issue and wait for DMAs.

   void cellSpursJobMain2( CellSpursJobContext2 * stInfo, CellSpursJob256 * job
) { globalDmaTag = stInfo->dmaTag;	// for ODS objects
   }

2. ODS objects can consume quite a bit of stack space. You may have to increase
the SPU job stack size. For instance:

   job->header.sizeStack = SPURS_QUADWORDS( 16 * 1024 );   // the ODS objects
get pretty large

   Make sure you measure the size of each ODS object and if there are recursive
functions using ODS objects make sure the recursion is bounded. When the stack
overflows the scratch and output memory may get overwritten and the results will
be undefined. Finding stack overflows is painful.

3. While you can setup a regular DMA list entry to use a NULL pointer with zero
size, do not use a NULL pointer for a cache DMA list entry. This confuses SPURS
and can cause your SPU binary to get corrupted.
...
================================================
*/
template <typename _type_, int _entries_ = 8, int _associativity_ = 4,
          bool aligned = false>
...
class ALIGNTYPE16 idODSStreamedOutputArray {
 public:
  idODSStreamedOutputArray(_type_* array, int* numElements, int maxElements)
      : localNum(0), outArray(array), outNum(numElements), outMax(maxElements) {
...
*/
template <typename _type_, int _bufferSize_,
          streamBufferType_t _sbt_ = SBT_DOUBLE, int _roundUpToMultiple_ = 1>
class ALIGNTYPE16 idODSStreamedArray {
 public:
  idODSStreamedArray(const _type_* array, const int numElements)
    ...
    FetchNextBatch();
    // Calculate the rounded up size here making the mod effectively for free
    // because we have to wait for memory access anyway while the above
    // FetchNextBatch() does not need the rounded up size yet.
    inArrayNumRoundedUp += _roundUpToMultiple_ - 1;
    inArrayNumRoundedUp -=
        inArrayNumRoundedUp %
        ((_roundUpToMultiple_ > 1) ? _roundUpToMultiple_ : 1);
  }
  ~idODSStreamedArray() {
    // Flush the accessible part of the array.
    FlushArray(inArray, cachedArrayStart * sizeof(_type_),
               cachedArrayEnd * sizeof(_type_));
  }

  // Fetches a new batch of array elements and returns the first index after
  // this new batch. After calling this, the elements starting at the index
  // returned by the previous call to FetchNextBach() (or zero if not yet
  // called) up to (excluding) the index returned by this call to
  // FetchNextBatch() can be accessed through the [] operator. When
  // quad-buffering, the elements starting at the index returned by the
  // second-from-last call to FetchNextBatch() can still be accessed. This is
  // useful when the algorithm needs to successively access an odd number of
  // elements at the same time that may cross a single buffer boundary.
  int FetchNextBatch() {
    ...
    if (cachedArrayEnd < inArrayNum) {
      ...
      FlushArray(inArray, (cachedArrayStart - _bufferSize_) * sizeof(_type_),
                 cachedArrayStart * sizeof(_type_));

      // Prefetch the next batch of elements.
      if (streamArrayEnd < inArrayNum) {
        streamArrayEnd = Min(streamArrayEnd + _bufferSize_, inArrayNum);
        for (unsigned int offset = cachedArrayEnd * sizeof(_type_);
             offset < streamArrayEnd * sizeof(_type_);
             offset += CACHE_LINE_SIZE) {
          Prefetch(inArray, offset);
        }
      }
    }
    return (cachedArrayEnd == inArrayNum) ? inArrayNumRoundedUp
                                          : cachedArrayEnd;
  }

  // Provides access to the elements starting at the index returned by the
  // next-to-last call to FetchNextBach() (or zero if only called once so far)
  // up to (excluding) the index returned by the last call to FetchNextBatch().
  // When quad-buffering, the elements starting at the index returned by the
  // second-from-last call to FetchNextBatch() can still be accessed. This is
  // useful when the algorithm needs to successively access an odd number of
  // elements at the same time that may cross a single buffer boundary.
  const _type_& operator[](int index) const {
    assert((index >= cachedArrayStart && index < cachedArrayEnd) ||
           (cachedArrayEnd == inArrayNum && index >= inArrayNum &&
            index < inArrayNumRoundedUp));
...
================================================
*/
template <typename _elemType_, typename _indexType_, int _bufferSize_,
          streamBufferType_t _sbt_ = SBT_DOUBLE, int _roundUpToMultiple_ = 1>
class ALIGNTYPE16 idODSStreamedIndexedArray {
 public:
  idODSStreamedIndexedArray(const _elemType_* array, const int numElements,
                            const _indexType_* index, const int numIndices)
      : cachedArrayStart(0),
        cachedArrayEnd(0),
        streamArrayEnd(0),
        cachedIndexStart(0),
    ...
    compile_time_assert((sizeof(_elemType_) & 15) ==
                        0);  // to avoid complexity due to cellDmaListGet
    compile_time_assert(_roundUpToMultiple_ >= 1);
    ...
    // Calculate the rounded up size here making the mod effectively for free
    // because we have to wait for memory access anyway while the above
    // FetchNextBatch() do not need the rounded up size yet.
  ...
  ~idODSStreamedIndexedArray() {
    ...
    FlushArray(inIndex, cachedIndexStart * sizeof(_indexType_),
               cachedIndexEnd * sizeof(_indexType_));
    // Flush the accessible part of the array.
    FlushArray(inArray, cachedArrayStart * sizeof(_elemType_),
               cachedArrayEnd * sizeof(_elemType_));
  }
  ...
  int FetchNextBatch() {
    ...
    if (cachedArrayEnd < inIndexNum) {
      if (streamIndexEnd > 0) {
        ...
        FlushArray(inIndex,
                   (cachedIndexStart - _bufferSize_) * sizeof(_indexType_),
                   cachedIndexStart * sizeof(_indexType_));
        // Flush the last batch of elements that is no longer accessible.
        FlushArray(inArray,
                   (cachedArrayStart - _bufferSize_) * sizeof(_elemType_),
                   cachedArrayStart * sizeof(_elemType_));
      ...
      if (streamIndexEnd < inIndexNum) {
        ...
        for (unsigned int offset = cachedIndexEnd * sizeof(_indexType_);
             offset < streamIndexEnd * sizeof(_indexType_);
             offset += CACHE_LINE_SIZE) {
          Prefetch(inIndex, offset);
        }
      }
    }
    return (cachedArrayEnd == inIndexNum) ? inIndexNumRoundedUp
                                          : cachedArrayEnd;
  }
  ...
  const _elemType_& operator[](int index) const {
    assert((index >= cachedArrayStart && index < cachedArrayEnd) ||
           (cachedArrayEnd == inIndexNum && index >= inIndexNum &&
            index < inIndexNumRoundedUp));
    if (_roundUpToMultiple_ > 1) {
