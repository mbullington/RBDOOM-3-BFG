...
// We specify the cache line size as 128 here to make the code consistent with
// the consoles.
...
// if the pointer is not on a cache line boundary this assumes the cache line
// the pointer starts in was already cleared RB: changed UINT_PTR to uintptr_t
#define CACHE_LINE_CLEAR_BLOCK(ptr, size)                               \
  byte* startPtr = (byte*)((((uintptr_t)(ptr)) + CACHE_LINE_SIZE - 1) & \
                           ~(CACHE_LINE_SIZE - 1));                     \
  byte* endPtr =                                                        \
      (byte*)(((uintptr_t)(ptr) + (size)-1) & ~(CACHE_LINE_SIZE - 1));  \
...
#endif
#endif
// DG: make sure __declspec(intrin_type) is only used on MSVC (it's not
// available on GCC etc
#ifdef _MSC_VER
...
#define _mm_splat_ps(x, i) \
  __m128c(_mm_shuffle_epi32(__m128c(x), _MM_SHUFFLE(i, i, i, i)))
#define _mm_perm_ps(x, perm) __m128c(_mm_shuffle_epi32(__m128c(x), perm))
#define _mm_sel_ps(a, b, c) \
  _mm_or_ps(_mm_andnot_ps(__m128c(c), a), _mm_and_ps(__m128c(c), b))
#define _mm_sel_si128(a, b, c) \
  _mm_or_si128(_mm_andnot_si128(__m128c(c), a), _mm_and_si128(__m128c(c), b))
#define _mm_sld_ps(x, y, imm)                           \
  __m128c(_mm_or_si128(_mm_srli_si128(__m128c(x), imm), \
                       _mm_slli_si128(__m128c(y), 16 - imm)))
#define _mm_sld_si128(x, y, imm) \
  _mm_or_si128(_mm_srli_si128(x, imm), _mm_slli_si128(y, 16 - imm))

ID_FORCE_INLINE_EXTERN __m128 _mm_msum3_ps(__m128 a, __m128 b) {
  __m128 c = _mm_mul_ps(a, b);
  return _mm_add_ps(_mm_splat_ps(c, 0),
                    _mm_add_ps(_mm_splat_ps(c, 1), _mm_splat_ps(c, 2)));
}
...
#define _mm_shufmix_epi32(x, y, perm) \
  __m128c(_mm_shuffle_ps(__m128c(x), __m128c(y), perm))
#define _mm_loadh_epi64(x, address) \
  __m128c(_mm_loadh_pi(__m128c(x), (__m64*)address))
...
#define _mm_loadu_bounds_0(bounds)                                           \
  _mm_perm_ps(_mm_loadh_pi(_mm_load_ss(&bounds[0].x), (__m64*)&bounds[0].y), \
              _MM_SHUFFLE(1, 3, 2, 0))
// load idBounds::GetMaxs()
#define _mm_loadu_bounds_1(bounds)                                           \
  _mm_perm_ps(_mm_loadh_pi(_mm_load_ss(&bounds[1].x), (__m64*)&bounds[1].y), \
              _MM_SHUFFLE(1, 3, 2, 0))
