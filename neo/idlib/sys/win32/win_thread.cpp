...
static void Sys_SetThreadName(DWORD threadID, const char* name) {
  ...
  __except (GetExceptionCode() == MS_VC_EXCEPTION ? EXCEPTION_EXECUTE_HANDLER
...
uintptr_t Sys_CreateThread(xthread_t function, void* parms,
  ...
  // Without this flag the 'dwStackSize' parameter to CreateThread specifies the
  // "Stack Commit Size" and the "Stack Reserve Size" is set to the value
  ...
  // settings. By setting this flag and by specifying 64 kB for the "Stack
  // Commit Size" in the project settings we can create new threads with a much
  // smaller reserved (and committed) stack space. It is very important that the
  // "Stack Commit Size" is set to a small value in the project settings. If it
  // is set to a large value we may be both reserving and committing a lot of
  // memory by setting the STACK_SIZE_PARAM_IS_A_RESERVATION flag. There are
  // some 50 threads allocated for normal game play. If, for instance, the
  // commit size is set to 16 MB then by adding this flag we would be reserving
  // and committing 50 x 16 = 800 MB of memory. On the other hand, if this flag
  // is not set and the "Stack Reserve Size" is set to 16 MB in the project
  // settings, then we would still be reserving 50 x 16 = 800 MB of virtual
  // address space.
  flags |= STACK_SIZE_PARAM_IS_A_RESERVATION;

  DWORD threadId;
  HANDLE handle = CreateThread(NULL,  // LPSECURITY_ATTRIBUTES lpsa, //-V513
                               stackSize, (LPTHREAD_START_ROUTINE)function,
                               parms, flags, &threadId);
  if (handle == 0) {
    idLib::common->FatalError("CreateThread error: %i", GetLastError());
    return (uintptr_t)0;
  }
  // TODO: when writing the SDL backend, just use this name when creating the
  // thread
  ...
  // Under Windows, we don't set the thread affinity and let the OS deal with
  // scheduling
...
========================
*/
void Sys_SignalDestroy(signalHandle_t& handle) { CloseHandle(handle); }
...
========================
*/
void Sys_SignalRaise(signalHandle_t& handle) { SetEvent(handle); }
...
bool Sys_SignalWait(signalHandle_t& handle, int timeout) {
  DWORD result = WaitForSingleObject(
      handle, timeout == idSysSignal::WAIT_INFINITE ? INFINITE : timeout);
  assert(result == WAIT_OBJECT_0 ||
         (timeout != idSysSignal::WAIT_INFINITE && result == WAIT_TIMEOUT));
...
========================
*/
void Sys_MutexDestroy(mutexHandle_t& handle) { DeleteCriticalSection(&handle); }
...
========================
*/
void Sys_MutexUnlock(mutexHandle_t& handle) { LeaveCriticalSection(&handle); }
