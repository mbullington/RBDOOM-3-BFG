...
// DG: Note: On Linux you need at least (e)glibc 2.12 to be able to set the
// threadname
...
static int Sys_SetThreadName(pthread_t handle, const char* name) {
  int ret = 0;
#ifdef __linux__
  // NOTE: linux only supports threadnames up to 16chars *including* terminating
  // NULL http://man7.org/linux/man-pages/man3/pthread_setname_np.3.html on my
  // machine a longer name (eg "JobListProcessor_0") caused an ENOENT error
  // (instead of ERANGE)
  ...
  // according to
  // http://www.freebsd.org/cgi/man.cgi?query=pthread_set_name_np&sektion=3 the
  // interface is void pthread_set_name_np(pthread_t tid, const char *name);
  ...
  /* TODO: OSX:
          ...
          // so we'd have to wrap the xthread_t function in Sys_CreateThread and
     set the name in the wrapping function...
  */
...
uintptr_t Sys_CreateThread(xthread_t function, void* parms,
  ...
  pthread_t handle;
  if (pthread_create((pthread_t*)&handle, &attr, (pthread_function_t)function,
                     parms) != 0) {
  ...
  // Under Linux, we don't set the thread affinity and let the OS deal with
  // scheduling
...
void Sys_SignalCreate(signalHandle_t& handle, bool manualReset) {
  ...
  // if this is true, the signal is only set to nonsignaled when Clear() is
  // called, else it's "auto-reset" and the state is set to !signaled after a
...
bool Sys_SignalWait(signalHandle_t& handle, int timeout) {
  ...
  // idSysSignal::WAIT_INFINITE ? INFINITE : timeout ); assert( result ==
  // WAIT_OBJECT_0 || ( timeout != idSysSignal::WAIT_INFINITE && result ==
  // WAIT_TIMEOUT ) ); return ( result == WAIT_OBJECT_0 );
  ...
  {
    if (!handle.manualReset)  // for auto-mode only one thread may be released -
                              // this one.
    {
      handle.signaled = false;
    }
  ...
  assert(status == 0 ||
         (timeout != idSysSignal::WAIT_INFINITE && status == ETIMEDOUT));
...
========================
*/
void Sys_MutexDestroy(mutexHandle_t& handle) { pthread_mutex_destroy(&handle); }
...
========================
*/
void Sys_MutexUnlock(mutexHandle_t& handle) { pthread_mutex_unlock(&handle); }
