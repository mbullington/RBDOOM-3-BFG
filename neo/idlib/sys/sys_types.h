...
// if writing to write-combined memory, always write indexes as pairs for 32 bit
// writes
ID_INLINE void WriteIndexPair(triIndex_t* dest, const triIndex_t a,
                              const triIndex_t b) {
...
// TODO: is that __assume an important optimization? if so, is there a gcc
// equivalent? SRS - The gcc equivalent is __builtin_unreachable()
#define NODEFAULT \
  default:        \
...
The CONST_* defines can be used to create constant expressions that	can be
evaluated at compile time. The parameters to these defines need to be compile
time constants such as literals or sizeof(). NEVER use an actual variable as a
parameter to one of these defines.

================================================================================================
*/

#ifdef _lint  // lint has problems with CONST_BITSFORINTEGER(), so just make it
              // something simple for analysis
...
#define CONST_COSINE_POLY(a)                                     \
  (((((-2.605e-07f * ((a) * (a)) + 2.47609e-05f) * ((a) * (a)) - \
      1.3888397e-03f) *                                          \
         ((a) * (a)) +                                           \
     4.16666418e-02f) *                                          \
        ((a) * (a)) -                                            \
...
#define CONST_SINE(a)                                                    \
  (((a) < CONST_PI)                                                      \
       ? (((a) > CONST_PI * 0.5f) ? CONST_SINE_POLY(CONST_PI - (a))      \
                                  : CONST_SINE_POLY((a)))                \
...
#define CONST_COSINE(a)                                                    \
  (((a) < CONST_PI)                                                        \
       ? (((a) > CONST_PI * 0.5f) ? -CONST_COSINE_POLY(CONST_PI - (a))     \
                                  : CONST_COSINE_POLY((a)))                \
