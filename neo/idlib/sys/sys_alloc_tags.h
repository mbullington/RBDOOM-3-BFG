...
MEM_TAG(STATIC_EXE)  // The static exe, generally how much memory we are using
                     // before our main() function ever runs
MEM_TAG(
    DEBUG)    // Crap we don't care about, because it won't be in a retail build
MEM_TAG(NEW)  // Crap allocated with new which hasn't been given an explicit tag
MEM_TAG(BLOCKALLOC)  // Crap allocated with idBlockAlloc which hasn't been given
                     // an explicit tag
MEM_TAG(PHYSICAL)
MEM_TAG(TRI_VERTS)
MEM_TAG(TRI_INDEXES)
...
MEM_TAG(SRFTRIS)
MEM_TAG(TEMP)  // Temp data which should be automatically freed at the end of
               // the function
MEM_TAG(PAGE)
...
MEM_TAG(CVAR)
MEM_TAG(
    CRAP)  // Crap allocated with new which hasn't been given an explicit tag
MEM_TAG(CINEMATIC)
MEM_TAG(FONT)
MEM_TAG(MATERIAL)
MEM_TAG(MODEL)
