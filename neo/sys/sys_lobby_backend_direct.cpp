...
========================
*/
idLobbyBackendDirect::idLobbyBackendDirect() { state = STATE_INVALID; }
...
void idLobbyBackendDirect::JoinFromConnectInfo(
  ...
  if (lobbyToSessionCB->CanJoinLocalHost()) {
    ...
    NET_VERBOSE_PRINT(
        "NET: idLobbyBackendDirect::JoinFromConnectInfo(): canJoinLocalHost\n");
  } else {
...
========================
*/
void idLobbyBackendDirect::Shutdown() { state = STATE_SHUTDOWN; }
...
========================
*/
void idLobbyBackendDirect::FinishBecomeHost() { isHost = true; }
...
lobbyConnectInfo_t idLobbyBackendDirect::GetConnectInfo() {
  ...
  if (IsHost()) {
    ...
    // DG: always using the first IP doesn't work, because on linux that's
    // 127.0.0.1 and even if not, this causes trouble with NAT. So either use
    // net_ip or, if it's not set ("localhost"), use 0.0.0.0 which is a special
    // case the client will treat as "just use the IP I used for the lobby"
    ...
    if (ip == NULL || idStr::Length(ip) == 0 ||
        idStr::Icmp(ip, "localhost") == 0) {
      ip = "0.0.0.0";
    }
    // DG end
...
========================
*/
bool idLobbyBackendDirect::IsOwnerOfConnectInfo(
