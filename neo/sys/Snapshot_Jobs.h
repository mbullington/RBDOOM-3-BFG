...
static const objectSize_t SIZE_STALE =
    MAX_TYPE(objectSize_t);  // Special size to indicate object went stale
static const objectSize_t SIZE_NOT_STALE =
    MAX_TYPE(objectSize_t) -
    1;  // Special size to indicate object is no longer stale

static const int RLE_COMPRESSION_PADDING =
    16;  // Padding to accommodate possible enlargement due to zlre compression

// OBJ_DEST_SIZE_ALIGN16 returns the total space needed to store an object for
// reading/writing during jobs
...
static const uint32 OBJ_DELETED =
    (1 << 3);  // Object was deleted (not going to be in the new snap)
static const uint32 OBJ_DIFFERENT =
    (1 << 4);  // Objects are in both snaps, but different
static const uint32 OBJ_SAME =
    (1 << 5);  // Objects are in both snaps, and are the same (we don't send
               // these, which means ack)
...
struct ALIGNTYPE16 objHeader_t {
  ...
  uint32
      flags;    // Flags used to communicate state from obj job to lzw delta job
...
// Input to initial jobs that produce delta'd zrle compressed versions of all
// the snap obj's
...
// This struct contains the start of where the final delta packet data is within
// lzwMem
...
// Input to the job that takes the results of the delta'd zrle obj's, and turns
// them into lzw delta packets
struct ALIGNTYPE16 lzwParm_t {
  ...
  int baseSequence;
  bool saveDictionary;
  bool fragmented;  // This lzw stream should continue where the last one left
                    // off
