...
idCVar saveGame_verbose("saveGame_verbose", "0", CVAR_BOOL | CVAR_ARCHIVE,
                        "debug spam");
idCVar saveGame_checksum("saveGame_checksum", "1", CVAR_BOOL,
                         "data integrity check");
idCVar saveGame_enable("saveGame_enable", "1", CVAR_BOOL,
...
void Sys_ExecuteSavegameCommandAsync(idSaveLoadParms* savegameParms) {
  ...
  if (!saveGame_enable.GetBool()) {
    idLib::Warning(
        "Savegames are disabled (saveGame_enable = 0). Skipping physical save "
        "to media.");
...
#define ASSERT_ENUM_STRING_BITFIELD(string, index) \
  (1 / (int)!(string - (1 << index))) ? #string : ""
...
CONSOLE_COMMAND(savegamePrintErrors,
                "Prints error code corresponding to each bit", 0) {
  idLib::Printf(
      "Bit  Description\n"
      "---  -----------\n");
...
This returns a comma delimited string of all the errors within the bitfield.  We
don't ever expect more than one error, the errors are bitfields so that they can
be used to mask which errors we want to handle in the engine or leave up to the
game.
...
idStr GetSaveGameErrorString(int errorMask) {
  ...
  for (int i = 0; i < SAVEGAME_E_NUM && continueProcessing; ++i) {
    ...
    if (localError & mask) {
      localError ^=
          mask;  // turn off this error so we can quickly see if we are done
...
const idStr& GetSaveFolder(idSaveGameManager::packageType_t type) {
  ...
  static idStrStatic<MAX_FOLDER_NAME_LENGTH>
      saveFolder[idSaveGameManager::PACKAGE_NUM];
  ...
  if (!initialized) {
    ...
    saveFolder[idSaveGameManager::PACKAGE_GAME].Format(
        "%s%s", ps3Header.c_str(), SAVEGAME_GAME_DIRECTORY_PREFIX);
    saveFolder[idSaveGameManager::PACKAGE_PROFILE].Format(
        "%s%s", ps3Header.c_str(), SAVEGAME_PROFILE_DIRECTORY_PREFIX);
    saveFolder[idSaveGameManager::PACKAGE_RAW].Format(
        "%s%s", ps3Header.c_str(), SAVEGAME_RAW_DIRECTORY_PREFIX);
  }
...
bool SavegameReadDetailsFromFile(idFile* file, idSaveGameDetails& details) {
  ...
  if (!ignoreChecksum) {
    ...
    int readChecksum =
        details.descriptors.GetInt(SAVEGAME_DETAIL_FIELD_CHECKSUM, 0);
...
*/
idSaveLoadParms::idSaveLoadParms() {
  // These are not done when we set defaults because SetDefaults is called
  // internally within the execution of the processor and these are set once and
  // shouldn't be touched until the processor is re-initialized
  cancelled = false;
...
========================
*/
void idSaveLoadParms::ResetCancelled() { cancelled = false; }
...
void idSaveLoadParms::Init() {
  ...
  // These are not done when we set defaults because SetDefaults is called
  // internally within the execution of the processor and these are set once and
  // shouldn't be touched until the processor is re-initialized cancelled =
  // false;
}
...
void idSaveLoadParms::SetDefaults(int newInputDevice) {
  // These are pulled out so SetDefaults() isn't called during global
  // instantiation of objects that have savegame processors in them that then
  // require a session reference.
  Init();
  ...
  if (newInputDevice != -1) {
    user =
        session->GetSignInManager().GetLocalUserByInputDevice(newInputDevice);
  ...
  if (user != NULL) {
    ...
    idLib::PrintfIf(saveGame_verbose.GetBool(),
                    "profile userId/gamertag: %s (%d)\n",
...
void idSaveLoadParms::CancelSaveGameFilePipelines() {
  for (int i = 0; i < files.Num(); i++) {
    if ((files[i]->type & SAVEGAMEFILE_PIPELINED) != 0) {
      idFile_SaveGamePipelined* file =
          dynamic_cast<idFile_SaveGamePipelined*>(files[i]);
      assert(file != NULL);
...
bool idSaveGameProcessor::Init() {
  if (!verify(!IsWorking())) {
    idLib::Warning(
        "[%s] Someone is trying to execute this processor twice, this is "
        "really bad!",
        this->Name());
    return false;
  }
...
saveGameHandle_t idSaveGameManager::ExecuteProcessor(
  ...
  // may not be running yet, but if we've init'd successfuly, the IsWorking()
  // call should return true if this method has been called.  You have problems
  // when callees are asking if the processor is done working by using
  // IsWorking() the next frame after they've executed the processor.
  processor->working = true;

  if (this->processor != NULL) {
    if (!verify(this->processor != processor)) {
      idLib::Warning(
          "[idSaveGameManager::ExecuteProcessor]:1 Someone is trying to "
          "execute this processor twice, this is really bad, learn patience "
          "padawan!");
      ...
      if (!verify(localProcessor == NULL)) {
        idLib::Warning(
            "[idSaveGameManager::ExecuteProcessor]:2 Someone is trying to "
            "execute this processor twice, this is really bad, learn patience "
            "padawan!");
...
========================
*/
saveGameHandle_t idSaveGameManager::ExecuteProcessorAndWait(
    idSaveGameProcessor* processor) {
...
Since the load & nextMap processors calls execute map change, we can't wait if
they are the only ones in the queue If there are only resettable processors in
the queue or no items in the queue, don't wait.

We would need to overrideSimpleProcessorCheck if we were sure we had done
something that would cause the processors to bail out nicely.  Something like
canceling a disc swap during a loading disc swap dialog...
========================
*/
void idSaveGameManager::WaitForAllProcessors(
    bool overrideSimpleProcessorCheck) {
  assert(idLib::IsMainThread());

  while (IsWorking() || (processorQueue.Num() > 0)) {
    if (!overrideSimpleProcessorCheck) {
      // BEFORE WE WAIT, and potentially hang everything, make sure processors
      // about to be executed won't sit and wait for themselves to complete.
      // Since we pull off simple processors first, we can stop waiting when the
      // processor being executed is not simple
      if (processor != NULL) {
...
========================
*/
void idSaveGameManager::CancelAllProcessors(
...
========================
*/
void idSaveGameManager::Set360RetrySaveAfterDeviceSelected(const char* folder,
                                                           const int64 bytes) {
  retryFolder = folder;
  retryBytes = bytes;
}
...
void idSaveGameManager::RetrySave() {
  if (DeviceSelectorWaitingOnSaveRetry() &&
      !common->Dialog().HasDialogMsg(
          GDM_WARNING_FOR_NEW_DEVICE_ABOUT_TO_LOSE_PROGRESS, NULL)) {
...
========================
*/
void idSaveGameManager::ShowRetySaveDialog(const char* folder,
                                           const int64 bytes) {
  ...
  // #str_dlg_space_required ~= "There is insufficient storage available. Please
  // free %s and try again."
  ...
  common->Dialog().AddDynamicDialog(GDM_INSUFFICENT_STORAGE_SPACE, callbacks,
                                    optionText, true, msg, true);
}
...
void idSaveGameManager::StartNextProcessor() {
  ...
  if (processorQueue.Num() > 0) {
    ...
    processor->parms.callbackSignal
        .Raise();  // signal that the thread is ready for work
...
void idSaveGameManager::FinishProcessor(idSaveGameProcessor* localProcessor) {
  ...
  idLib::PrintfIf(saveGame_verbose.GetBool(), "[%s] : %s, %d ms\n",
                  __FUNCTION__, localProcessor->Name(),
                  Sys_Milliseconds() - startTime);

  // This will delete from the files set for auto-deletion
  // Don't remove files not set for auto-deletion, they may be used outside of
  // the savegame manager by game-side callbacks for example
...
========================
*/
void idSaveGameManager::Clear() { processorQueue.Clear(); }
...
========================
*/
bool idSaveGameManager::IsWorking() const { return processor != NULL; }
...
void idSaveGameManager::Pump() {
  // After a processor is done, the next is pulled off the queue so the only way
  // the manager isn't working is if there isn't something executing or in the
  // queue.
  if (!IsWorking()) {
    // Unified start to initialize system on PS3 and do appropriate checks for
    // system combination issues
    // ------------------------------------
    // START
    // ------------------------------------
    StartNextProcessor();
  ...
  // Quickly checks to see if the savegame thread is done, otherwise, exit and
  // continue frame commands
  if (processor->IsThreadFinished()) {
    idLib::PrintfIf(saveGame_verbose.GetBool(),
                    "%s waited on processor [%s], error = 0x%08X, %s\n",
                    __FUNCTION__, processor->Name(), processor->GetError(),
                    GetSaveGameErrorString(processor->GetError()).c_str());

    if (!cancel && continueProcessing) {
      // Check for available storage unit
      if (session->GetSignInManager().GetMasterLocalUser() != NULL) {
        if (!session->GetSignInManager()
                 .GetMasterLocalUser()
                 ->IsStorageDeviceAvailable()) {
          // this will not allow further processing
          processor->parms.errorCode =
              SAVEGAME_E_UNABLE_TO_SELECT_STORAGE_DEVICE;
        }
      }

      // Execute Process() on the processor, if there was an error in a previous
      // Process() call, give the processor the chance to validate that error
      // and either clean itself up or convert it to another error or none.
      if (processor->GetError() == SAVEGAME_E_NONE ||
          processor->ValidateLastError()) {
        idLib::PrintfIf(saveGame_verbose.GetBool(),
                        "%s calling %s::Process(), error = 0x%08X, %s\n",
                        __FUNCTION__, processor->Name(), processor->GetError(),
                        GetSaveGameErrorString(processor->GetError()).c_str());
        ...
        // If we don't return here, the completedCallback will be executed
        // before it's done with it's async operation during it's last process
        // stage.
        return;
      } else {
        continueProcessing = false;
    ...
    if (!continueProcessing) {
      // Clear out details if we detect corruption but keep directory/slot
      // information
      ...
      idLib::PrintfIf(saveGame_verbose.GetBool(),
                      "%s calling %s::CompletedCallback()\n", __FUNCTION__,
                      processor->Name());
      processor->working = false;

      // This ensures that the savegame manager will believe the processor is
      // done when there is a potentially catastrophic thing that will happen
      // within CompletedCallback which might try to sync all threads The most
      // common case of this is executing a map change (which we no longer do).
      // We flush the heap and wait for all background processes to finish.
      // After all this is called, we will cleanup the old processor within
      // FinishProcessor()
    ...
    class idSWFScriptFunction_TryAgain : public idSWFScriptFunction_RefCounted {
     public:
      idSWFScriptFunction_TryAgain(idSaveGameManager* manager,
                                   idSaveGameProcessor* processor) {
    ...
    callbacks.Append(new (TAG_SWF)
                         idSWFScriptFunction_TryAgain(this, tempProcessor));
    optionText.Append(idStrId("#STR_SWF_RETRY"));
    common->Dialog().AddDynamicDialog(GDM_ERROR_SAVING_SAVEGAME, callbacks,
                                      optionText, true, "");
  }
}
