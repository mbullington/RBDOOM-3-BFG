...
class idNetSessionPort {
 ...
 private:
  float forcePacketDropCurr;  // Used with net_forceDrop and
                              // net_forceDropCorrelation
...
struct lobbyUser_t {
  ...
  bool disconnecting;  // true if we've sent a msg to disconnect this user from
                       // the session
  int level;
  int teamNumber;
  ...
  int migrationGameData;  // index into the local migration gamedata array that
                          // is associated with this user. -1=no migration game
                          // data available
  ...
  void WriteClientMutableData(idBitMsg& msg) {
    ...
    msg.WriteBits(
        teamNumber,
        2);  // We need two bits since we use team value of 2 for spectating
...
class idLobbyBackend {
 public:
  ...
  static const char* GetStateString(lobbyBackendState_t state_) {
    static const char* stateToString[NUM_STATES] = {"STATE_INVALID",
                                                    "STATE_READY",
                                                    "STATE_CREATING",
                                                    "STATE_SEARCHING",
                                                    "STATE_OBTAINING_ADDRESS",
  ...
  idLobbyBackend(lobbyBackendType_t lobbyType)
      : type(lobbyType), isLocal(false), isHost(false) {}
  ...
  virtual void FillMsgWithPostConnectInfo(
      idBitMsg& msg) = 0;  // Passed itno PostConnectFromMsg
  virtual void PostConnectFromMsg(
      idBitMsg& msg) = 0;  // Uses results from FillMsgWithPostConnectInfo
  virtual bool IsOwnerOfConnectInfo(
      const lobbyConnectInfo_t& connectInfo) const {
    return false;
  }
  ...
  }  // Called after becoming a new host, to register old addresses to send
     // invites to
