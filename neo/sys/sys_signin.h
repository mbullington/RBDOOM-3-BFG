...
class idSignInManagerBase {
 public:
  ...
  virtual void RegisterLocalUser(
      int inputDevice) = 0;  // Register a local controller user to the passed
                             // in input device
  virtual idLocalUser* GetRegisteringUser() {
    return NULL;  // This is a user that has started the registration process
                  // but is not yet a local user.
  }
  ...
  void SetDesiredLocalUsers(int minDesiredLocalUsers,
  ...
  bool IsMasterLocalUserPersistent() const {
    return (GetMasterLocalUser() != NULL) ? GetMasterLocalUser()->IsPersistent()
                                          : false;
  }
  bool IsMasterLocalUserOnline() const {
    return (GetMasterLocalUser() != NULL) ? GetMasterLocalUser()->IsOnline()
                                          : false;
  }
  int GetMasterInputDevice() const {
    return (GetMasterLocalUser() != NULL)
               ? GetMasterLocalUser()->GetInputDevice()
               : -1;
  }
  localUserHandle_t GetMasterLocalUserHandle() const {
    return (GetMasterLocalUser() != NULL)
               ? GetMasterLocalUser()->GetLocalUserHandle()
               : localUserHandle_t();
  }
