...
void idLZWCompressor::WriteBits(uint32 value, int bits) {
  ...
  // Flush 8 bits (1 byte) at a time ( leftovers will get caught in
  // idLZWCompressor::End() )
...
int idLZWCompressor::WriteChain(int code) {
  ...
  int i = 0;
  do {
    assert(i < lzwCompressionData_t::LZW_DICT_SIZE &&
           code < lzwCompressionData_t::LZW_DICT_SIZE && code >= 0);
...
int idLZWCompressor::ReadByte(bool ignoreOverflow) {
  ...
  if (blockIndex == blockSize)  //-V581 DecompressBlock() updates these values,
                                //the if() isn't redundant
  {
    if (!ignoreOverflow) {
      overflowed = true;
...
void idLZWCompressor::WriteByte(uint8 value) {
  ...
  if (lzwData->bytesWritten >=
      maxSize - (lzwData->codeBits + lzwData->tempBits + 7) / 8) {
    overflowed = true;  // At any point, if we can't perform an End call, then
                        // trigger an overflow
    return;
  }
}
...
int idLZWCompressor::End() {
  ...
  if (lzwData->tempBits > 0) {
    ...
    data[lzwData->bytesWritten++] =
        (uint8)lzwData->tempValue & ((1 << lzwData->tempBits) - 1);
  }
...
========================
*/
void idLZWCompressor::ClearHash() { memset(hash, 0xFF, sizeof(hash)); }
...
void idZeroRunLengthCompressor::Start(uint8* dest_, idLZWCompressor* comp_,
                                      int maxSize_) {
  zeroCount = 0;
  dest = dest_;
  comp = comp_;
