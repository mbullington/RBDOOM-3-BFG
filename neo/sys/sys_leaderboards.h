...
completely up to each game.  The system needs to deal with some stat indexes for
things like the level for matchmaking, etc.
================================================
*/
...
enum aggregationMethod_t {
  ...
  AGGREGATE_MAX,   // Write the new value if it is greater than the existing
                   // value.
  AGGREGATE_SUM,   // Add the new value to the existing value and write the
                   // result.
  AGGREGATE_LAST,  // Write the new value.
};

enum rankOrder_t {
  RANK_GREATEST_FIRST,  // Rank the in descending order, greatest score is best
                        // score
...
struct leaderboardDefinition_t {
  ...
  leaderboardDefinition_t(int id_, int numColumns_,
                          const columnDef_t* columnDefs_,
  ...
  bool checkAgainstCurrent;  // Compare column 0 with the currently stored
                             // leaderboard, and only submit the new leaderboard
                             // if the new column 0 is better
};
...
enum leaderboardError_t {
  ...
  LEADERBOARD_ERROR_BUSY,          // Unable to download leaderboards right now
                                   // (download already in progress)
  LEADERBOARD_ERROR_INVALID_USER,  // Unable to request leaderboards as the
                                   // given user
  LEADERBOARD_ERROR_INVALID_REQUEST,  // The leaderboard request was invalid
  LEADERBOARD_ERROR_DOWNLOAD,         // An error occurred while downloading the
                                      // leaderboard
...
class idLeaderboardCallback : public idCallback {
 ...
 protected:
  ...
  int localIndex;  // if player is in the rows, this is the offset of him within
                   // the returned rows
  ...
  leaderboardError_t
      errorCode;  // error, if any, that occurred during last operation
};
