...
const int DefaultPublicGameFlags = MATCH_JOIN_IN_PROGRESS |
                                   MATCH_REQUIRE_PARTY_LOBBY | MATCH_RANKED |
                                   MATCH_STATS;
const int DefaultPrivateGameFlags =
    MATCH_JOIN_IN_PROGRESS | MATCH_REQUIRE_PARTY_LOBBY | MATCH_PRIVATE;
...
enum leaderboardDisplayError_t {
  ...
  LEADERBOARD_DISPLAY_ERROR_NOT_RANKED,  // Attempting to view a "My Score"
                                         // leaderboard you aren't ranked on
...
class idLobbyBase {
 public:
  ...
  virtual int GetLobbyUserIndexFromLobbyUserID(
      lobbyUserID_t lobbyUserID) const = 0;

  virtual void SendReliable(
      int type, idBitMsg& msg, bool callReceiveReliable = true,
      peerMask_t sessionUserMask = MAX_UNSIGNED_TYPE(peerMask_t)) = 0;
  ...
  virtual bool GetLobbyUserWeaponAutoReload(
      lobbyUserID_t lobbyUserID) const = 0;
  virtual bool GetLobbyUserWeaponAutoSwitch(
      lobbyUserID_t lobbyUserID) const = 0;
...
class idSession {
 public:
  ...
  enum sessionOption_t {
    OPTION_LEAVE_WITH_PARTY =
        BIT(0),  // As a party leader, whether or not to drag your party members
                 // with you when you leave a game lobby
  ...
  virtual void MatchFinished() = 0;  // this is for when the game is over before
                                     // we go back to lobby. Need this incase
                                     // the host leaves during this time
  virtual void
  QuitMatch() = 0;  // Meant for host or peer to quit the match before it ends,
                    // will instigate host migration, etc
  ...
  virtual float GetTitleStorageFloat(const char* name,
                                     float defaultFloat) const = 0;
  virtual int GetTitleStorageInt(const char* name, int defaultInt) const = 0;
  virtual bool GetTitleStorageBool(const char* name,
                                   bool defaultBool) const = 0;
  virtual const char* GetTitleStorageString(
      const char* name, const char* defaultString) const = 0;
  ...
  // Savegames
  //
  // Default async implementations, saves to a folder, uses game.details file to
  // describe each save. Files saved are up to the game and provide through a
  // callback mechanism.  If you want to be notified when one of these
  // operations have completed, either modify the framework's completedCallback
  // of each of the savegame processors or create your own processors and
  // execute with the savegameManager.
  //=====================================================================================================
  virtual saveGameHandle_t SaveGameSync(
      const char* name, const saveFileEntryList_t& files,
      const idSaveGameDetails& description) = 0;
  virtual saveGameHandle_t SaveGameAsync(
      const char* name, const saveFileEntryList_t& files,
      const idSaveGameDetails& description) = 0;
  virtual saveGameHandle_t LoadGameSync(const char* name,
                                        saveFileEntryList_t& files) = 0;
  ...
  virtual bool IsSaveGameCompletedFromHandle(
      const saveGameHandle_t& handle) const = 0;
  ...
  // Returns the known list of savegames, must first enumerate for the savegames
  // ( via session->Enumerate() )
  virtual const saveGameDetailsList_t& GetEnumeratedSavegames() const = 0;

  // These are on session and not idGame so it can persist across game
  // deallocations
  ...
  virtual voiceStateDisplay_t GetDisplayStateFromVoiceState(
      voiceState_t voiceState) const = 0;
 ...
 protected:
  idSignInManagerBase* signInManager;  // pointer so we can treat dynamically
                                       // bind platform-specific impl
  idSaveGameManager* saveGameManager;
  idAchievementSystem*
      achievementSystem;  // pointer so we can treat dynamically bind
