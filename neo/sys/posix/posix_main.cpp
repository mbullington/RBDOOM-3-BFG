...
// terminal support
idCVar in_tty("in_tty", "1", CVAR_BOOL | CVAR_INIT | CVAR_SYSTEM,
...
idCVar com_pid("com_pid", "0", CVAR_INTEGER | CVAR_INIT | CVAR_SYSTEM,
               "process id");
...
================
*/
void Posix_SetExit(int ret) { set_exit = 0; }
...
================
*/
void Sys_Quit() { Posix_Exit(EXIT_SUCCESS); }
...
- binary image path
- MacOS app bundle resources directory path       // SRS - Added MacOS app
...
const char* Sys_DefaultBasePath() {
  ...
  if (basepath.Length()) {
...
#if defined(__APPLE__)  // SRS - - Added check for MacOS app bundle resources
                        // path
    basepath += "/../Resources";
    testbase = basepath;
    testbase += "/";
    ...
    if (stat(testbase.c_str(), &st) != -1 && S_ISDIR(st.st_mode)) {
      ...
      common->Printf(
          "no '%s' directory in MacOS app bundle resources path %s, skipping\n",
          BASE_GAMEDIR, basepath.c_str());
    }
#endif
  }
...
================
*/
bool Sys_Rmdir(const char* path) { return (rmdir(path) == 0); }
...
int Sys_ListFiles(const char* directory, const char* extension,
  ...
  while (readdir_r(fdir, entry, &d) == 0 && d != NULL) {
    ...
    if (!dironly) {
      ...
      // to match whole filenames and patterns in the savegame-code, not just
      // file extensions... so just use fnmatch() which supports matching shell
      // wildcard patterns ("*.foo" etc) if we should ever need case
      // insensitivity, use FNM_CASEFOLD as third flag
...
================
*/
bool Sys_LockMemory(void* ptr, int bytes) { return true; }
...
================
*/
bool Sys_UnlockMemory(void* ptr, int bytes) { return true; }
...
================
*/
bool Sys_AlreadyRunning() { return false; }
