...
numLogicalCPUCores	- the total number of logical CPU cores (equal to the
total number of threads from all CPU)
  ...
  if (fd != -1) {
    ...
    if (CPUCoresIsFound == false && SiblingsIsFound == false) {
      ...
      s_numPhysicalCPUCores =
          sysconf(_SC_NPROCESSORS_CONF);  // _SC_NPROCESSORS_ONLN may not be
                                          // reliable on Android
      s_numLogicalCPUCores =
          s_numPhysicalCPUCores;  // hack for CPU without Hyper-Threading (HT)
                                  // technology
...
void Sys_ReLaunch() {
  ...
  // NOTE: this function used to have parameters: the commandline arguments, but
  // as one string..
  //       for Linux/Unix we want one char* per argument so we'll just add the
  //       friggin' " +set com_skipIntroVideos 1" to the other commandline
  //       arguments in this function.

  int ret = fork();
  if (ret < 0) {
    idLib::Error("Sys_ReLaunch(): Couldn't fork(), reason: %s ",
                 strerror(errno));
  }
  ...
  if (ret == 0) {
    ...
    pid_t sId = setsid();
    if (sId == (pid_t)-1) {
      idLib::Error("Sys_ReLaunch(): setsid() failed! Reason: %s ",
                   strerror(errno));
    }
    ...
    if (devfd != NULL) {
      ...
      idLib::Warning(
          "Sys_ReLaunch(): Couldn't open /dev/fd/ - will leak file "
          "descriptors. Reason: %s",
          strerror(errno));
    }

    // + 3 because "+set" "com_skipIntroVideos" "1" - and note that while we'll
    // skip one (the first) cmdargv argument, we need one more pointer for NULL
    // at the end.
    int argc = cmdargc + 3;
    ...
    idLib::Error("Sys_ReLaunch(): WTF exec() failed! Reason: %s ",
                 strerror(errno));
