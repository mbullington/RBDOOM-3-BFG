...
// DG: Sys_ReLaunch() doesn't need any options (and the old way is painful for
// POSIX systems)
void Sys_ReLaunch();
// DG end
void Sys_Launch(const char* path, idCmdArgs& args, void* launchData,
                unsigned int launchDataSize);
...
// DG: currently this is only used by idKeyInput::LocalizedKeyName() for
// !windows
#ifndef _WIN32
// return a human readable name for the key in the current keyboard layout
// (keynum is a directinput scancode)
...
// Execute the specified process and wait until it's done, calling workFn every
// waitMS milliseconds. If showOutput == true, std IO from the executed process
// will be output to the console. Note that the return value is not an
// indication of the exit code of the process, but is false only if the process
// could not be created at all. If you wish to check the exit code of the
...
class idUDP {
 public:
  ...
  uint32 GetUIntAdr() const {
    return (bound_to.ip[0] | bound_to.ip[1] << 8 | bound_to.ip[2] << 16 |
            bound_to.ip[3] << 24);
  }
  void Close();
