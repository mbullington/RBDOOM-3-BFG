...
class idSnapshotProcessor {
 public:
  ...
  void PeekDeltaSequence(const char* deltaMem, int deltaSize,
                         int& deltaSequence, int& deltaBaseSequence);
  // Apply a delta to the supplied snapshot
  bool ApplyDeltaToSnapshot(idSnapShot& snap, const char* deltaMem,
  ...
  void SubmitPendingSnap(int visIndex, uint8* objMemory, int objMemorySize,
  ...
  bool ReceiveSnapshotDelta(const byte* deltaData, int deltaLength,
                            int visIndex, int& outSeq, int& outBaseSeq,
                            idSnapShot& outSnap, bool& fullSnap);
  ...
  // We know we can remove them, because we will never be able to apply them,
  // since the basestate needed to generate a full snap from these deltas is
  // gone.
  void RemoveDeltasForOldBaseSequence();
  // Make sure delta sequence and basesequence values are valid, and in order,
  // etc
  ...
  // This is used to ack the latest delta we have.  If we have no deltas, we
  // sent -1 to make sure Server knows we don't want to ack, since we are as up
  // to date as we can be
 ...
 private:
  ...
  int snapSequence;
  int baseSequence;
  int lastFullSnapBaseSequence;  // Latest base sequence number that is a full
                                 // snap
  ...
  struct jobMemory_t {
    static const int MAX_LZW_DELTAS =
        1;  // FIXME: cleanup the old multiple delta support completely

    // @TODO this is a hack fix to allow online to load into coop (where there
    // are lots of entities). The real solution should be coming soon. Doom MP:
    // we encountered the same problem, going from 1024 to 4096 as well until a
    // better solution is in place (initial, useless, exchange of func_statics
    // is killing us)
    static const int MAX_OBJ_PARMS = 4096;

    static const int MAX_LZW_PARMS = 32;
    static const int MAX_OBJ_HEADERS = 256;
    static const int MAX_LZW_MEM =
        1024 * 8;  // 8k in the byte * lzwMem buffers, must be <= PS3_DMA_MAX
    ...
    idArray<lzwDelta_t, MAX_LZW_DELTAS>
        lzwDeltas;  // Info about each pending delta output from jobs
    idArray<byte, MAX_LZW_MEM> lzwMem;  // Memory for output from lzw jobs

    lzwInOutData_t lzwInOutData;  // In/Out data used so lzw data can persist
                                  // across lzw jobs
  };
  ...
  idSnapShot templateStates;  // holds default snapshot states for some newly
                              // spawned object
