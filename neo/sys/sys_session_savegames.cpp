...
idCVar savegame_error("savegame_error", "0", CVAR_INTEGER,
                      "Combination of bits that will simulate and error, see "
...
========================
*/
bool idSaveGameProcessorLoadFiles::InitLoadFiles(
    const char* folder_, const saveFileEntryList_t& files,
...
========================
*/
bool idSaveGameProcessorDelete::InitDelete(
    const char* folder_, idSaveGameManager::packageType_t type) {
  if (!idSaveGameProcessor::Init()) {
    return false;
  }
...
========================
*/
bool idSaveGameProcessorSaveFiles::InitSave(
    const char* folder, const saveFileEntryList_t& files,
...
========================
*/
saveGameHandle_t idSessionLocal::SaveGameSync(
    const char* name, const saveFileEntryList_t& files,
  ...
  idFile_SaveGame* gameDetailsFile = new (TAG_SAVEGAMES) idFile_SaveGame(
      SAVEGAME_DETAILS_FILENAME, SAVEGAMEFILE_TEXT | SAVEGAMEFILE_AUTO_DELETE);
  ...
  if (processorSaveFiles->InitSave(name, filesWithDetails, description)) {
    processorSaveFiles->AddCompletedCallback(
        MakeCallback(this, &idSessionLocal::OnSaveCompleted,
...
========================
*/
saveGameHandle_t idSessionLocal::SaveGameAsync(
    const char* name, const saveFileEntryList_t& files,
    const idSaveGameDetails& description) {
  saveGameHandle_t handle = 0;

  // Done this way so we know it will be shutdown properly on early exit or
  // exception
  struct local_t {
...
void idSessionLocal::OnSaveCompleted(idSaveLoadParms* parms) {
  ...
  // Only turn off the indicator if we're not also going to save the profile
  // settings
  if (master != NULL && master->GetProfile() != NULL &&
      !master->GetProfile()->IsDirty()) {
    common->Dialog().ShowSaveIndicator(false);
  }
  ...
  if (parms->GetError() == SAVEGAME_E_NONE) {
    ...
    saveGameDetailsList_t& detailList =
...
We still want to use the savegame manager because we could have file system
operations in flight and need to
========================
*/
saveGameHandle_t idSessionLocal::LoadGameSync(const char* name,
  ...
  {
    // Put in a local block so everything will go in the global heap before the
    // map change, but the heap is automatically popped out on early return or
    // exception You cannot be in the global heap during a map change...
    ...
    saveFileEntryList_t filesWithDetails(files);
    std::unique_ptr<idFile_SaveGame> gameDetailsFile(
        new (TAG_SAVEGAMES)
            idFile_SaveGame(SAVEGAME_DETAILS_FILENAME, SAVEGAMEFILE_TEXT));
    filesWithDetails.Append(gameDetailsFile.get());

    // Check the cached save details from the enumeration and make sure we don't
    // load a save from a newer version of the game!
    const saveGameDetailsList_t details =
    ...
    // SRS - check details file for compatibility before removing it from
    // parms.files list
    if (parms.GetError() == SAVEGAME_E_NONE) {
      // Checks the details file to see if corrupted or if it's from a newer
      // savegame
      checkDetailsFile = LoadGameCheckDescriptionFile(parms);
    }

    // tomgreen66 - remove details file we added via auto_ptr to parms.files in
    // InitLoadFiles above
    for (int i = 0; i < parms.files.Num(); ++i) {
      if (parms.files[i] == gameDetailsFile.get()) {
        // details file reference will be deleted by auto_ptr so remove it from
        // list and update file count
        parms.files.RemoveIndexFast(i);
      }
    }

    // SRS - return if savegame error or description file corrupt or not
    // compatible
...
saveGameHandle_t idSessionLocal::EnumerateSaveGamesSync() {
  ...
  if (processorEnumerate->Init()) {
    processorEnumerate->AddCompletedCallback(
        MakeCallback(this, &idSessionLocal::OnEnumerationCompleted,
                     &processorEnumerate->GetParmsNonConst()));
...
int idSort_EnumeratedSavegames(const idSaveGameDetails* a,
                               const idSaveGameDetails* b) {
...
void idSessionLocal::OnEnumerationCompleted(idSaveLoadParms* parms) {
  ...
  std::sort(parms->detailList.Ptr(),
            parms->detailList.Ptr() + parms->detailList.Num());

  if (parms->GetError() == SAVEGAME_E_NONE) {
    // Copy into the maintained list
    saveGameDetailsList_t& detailsList =
        session->GetSaveGameManager().GetEnumeratedSavegamesNonConst();
    // mem.PushHeap();
    detailsList = parms->detailList;  // copies new list into the savegame
                                      // manager's reference
    // mem.PopHeap();

    // The platform-specific implementations don't know about the prefixes
    // If we don't do this here, we will end up with slots like:
    // GAME-GAME-GAME-GAME-AUTOSAVE...
    for (int i = 0; i < detailsList.Num(); i++) {
      idSaveGameDetails& details = detailsList[i];

      const idStr original = details.slotName;
      const idStr stripped =
          RemoveSaveFolderPrefix(original, idSaveGameManager::PACKAGE_GAME);
...
saveGameHandle_t idSessionLocal::DeleteSaveGameSync(const char* name) {
  ...
  if (processorDelete->InitDelete(name)) {
    processorDelete->AddCompletedCallback(
        MakeCallback(this, &idSessionLocal::OnDeleteCompleted,
                     &processorDelete->GetParmsNonConst()));
...
bool idSessionLocal::IsEnumerating() const {
  return !session->IsSaveGameCompletedFromHandle(
      processorEnumerate->GetHandle());
}
...
bool idSessionLocal::LoadGameCheckDescriptionFile(idSaveLoadParms& parms) {
  idFile_SaveGame** detailsFile =
      FindFromGenericPtr(parms.files, SAVEGAME_DETAILS_FILENAME);
  if (detailsFile == NULL) {
...
================================================================================================
*/
CONSOLE_COMMAND(testSavegameDeleteAll,
                "delete all savegames without confirmation", 0) {
  if (session == NULL) {
  ...
  idLib::Printf("Error = 0x%08X, %s\n", parms.GetError(),
                GetSaveGameErrorString(parms.GetError()).c_str());
}

CONSOLE_COMMAND(testSavegameDelete, "deletes a savegames without confirmation",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
    return;
  ...
  if (testDeleteSaveGamesProc.InitDelete(folder)) {
    session->GetSaveGameManager().ExecuteProcessorAndWait(
        &testDeleteSaveGamesProc);
  }

  idLib::Printf("Completed process.\n");
  idLib::Printf(
      "Error = 0x%08X, %s\n", testDeleteSaveGamesProc.GetParms().GetError(),
      GetSaveGameErrorString(testDeleteSaveGamesProc.GetParms().GetError())
          .c_str());
}

CONSOLE_COMMAND(testSavegameEnumerateFiles,
                "enumerates all the files in a folder (blank for 'current "
                "slot' folder, use 'autosave' for the autosave slot)",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
    return;
...
void OutputDetailList(const saveGameDetailsList_t& savegameList) {
  for (int i = 0; i < savegameList.Num(); ++i) {
    idLib::Printf(
        ...
        savegameList[i].damaged ? "?"
                                : savegameList[i].descriptors.GetString(
                                      SAVEGAME_DETAIL_FIELD_MAP, ""),
        Sys_TimeStampToStr(savegameList[i].date));
  }
}

CONSOLE_COMMAND(testSavegameEnumerate, "enumerates the savegames available",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
    return;
  ...
  const saveGameDetailsList_t gameList =
...
CONSOLE_COMMAND(testSaveGameOutputEnumeratedSavegames,
                "outputs the list of savegames already enumerated, this does "
                "not re-enumerate",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
  ...
  const saveGameDetailsList_t& savegames =
      session->GetSaveGameManager().GetEnumeratedSavegames();
  OutputDetailList(savegames);
}

CONSOLE_COMMAND(testSavegameGetCurrentSlot, "returns the current slot in use",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
    return;
...
CONSOLE_COMMAND(testSavegameSetCurrentSlot, "returns the current slot in use",
                0) {
  if (session == NULL) {
    idLib::Printf("Invalid session.\n");
    return;
...
CONSOLE_COMMAND(
    savegameSetErrorBit,
    "Allows you to set savegame_error by bit instead of integer value", 0) {
