...
================================================
*/
void idVoiceChatMgr::RegisterTalker(lobbyUser_t* user, int lobbyType,
                                    bool isLocal) {
  ...
  if (!newTalker.IsLocal())  // If this is a remote talker, register his machine
                             // address
  {
  ...
  // Since we added a new talker, make sure he is registered.
  // UpdateRegisteredTalkers will catch all users, including this one.
...
================================================
*/
void idVoiceChatMgr::UnregisterTalker(lobbyUser_t* user, int lobbyType,
                                      bool isLocal) {
  int i = FindTalkerIndex(user, lobbyType);
  ...
  UpdateRegisteredTalkers();  // Make sure the user gets unregistered before we
                              // remove him/her
...
================================================
*/
void idVoiceChatMgr::GetActiveLocalTalkers(
...
================================================
*/
void idVoiceChatMgr::GetRecipientsForTalker(
    int talkerIndex,
...
================================================
*/
void idVoiceChatMgr::SetTalkerGroup(const lobbyUser_t* user, int lobbyType,
                                    int groupIndex) {
  int i = FindTalkerIndex(user, lobbyType);
  ...
  // Since the group index of this player changed, call UpdateRegisteredTalkers,
  // which will register the appropriate users based on the current active group
  // and session
  UpdateRegisteredTalkers();
}
...
void idVoiceChatMgr::SetActiveLobby(int lobbyType) {
  if (activeLobbyType != lobbyType) {
    ...
    // When the active session changes, we need to immediately call
    // UpdateRegisteredTalkers, which will make sure the appropriate talkers are
...
void idVoiceChatMgr::SetActiveChatGroup(int groupIndex) {
  if (activeGroupIndex != groupIndex) {
    ...
    // When the active group changes, we need to immediately call
    // UpdateRegisteredTalkers, which will make sure the appropriate talkers are
    // registered depending on the activeGroup.
    UpdateRegisteredTalkers();
  }
}
...
int idVoiceChatMgr::FindTalkerByUserId(lobbyUserID_t userID, int lobbyType) {
  for (int i = 0; i < talkers.Num(); i++) {
    if (talkers[i].user->lobbyUserID == userID &&
        talkers[i].lobbyType == lobbyType) {
      return i;
    }
  }
...
================================================
*/
bool idVoiceChatMgr::GetLocalChatData(int talkerIndex, byte* data,
                                      int& dataSize) {
  talker_t& talker = talkers[talkerIndex];
  ...
  if (!GetLocalChatDataInternal(talkerIndex,
                                voiceMsg.GetWriteData() + voiceMsg.GetSize(),
                                voiceDataSize)) {
    dataSize = 0;
    return false;
  }
...
void idVoiceChatMgr::SubmitIncomingChatData(const byte* data, int dataSize) {
  ...
  if (talker.registeredSuccess && !talker.isMuted) {
    ...
    SubmitIncomingChatDataInternal(
        i, voiceMsg.GetReadData() + voiceMsg.GetReadCount(),
        voiceMsg.GetRemainingData());
  }
}
...
voiceState_t idVoiceChatMgr::GetVoiceState(const lobbyUser_t* user) {
  ...
  return talker.talking ? (talker.talkingGlobal ? VOICECHAT_STATE_TALKING_GLOBAL
...
========================
*/
void idVoiceChatMgr::ToggleMuteLocal(const lobbyUser_t* src,
...
int idVoiceChatMgr::FindMachine(const lobbyAddress_t& address, int lobbyType) {
  for (int i = 0; i < remoteMachines.Num(); i++) {
    ...
    if (remoteMachines[i].lobbyType == lobbyType &&
        remoteMachines[i].address.Compare(address)) {
      return i;
    }
  }
  return -1;  // Not found
...
void idVoiceChatMgr::RemoveMachine(int machineIndex, int lobbyType) {
  ...
  // Don't remove the machine.  refCount will eventually reach 0, which will
  // free up the slot to reclaim. We don't want to remove it, because that would
  // invalidate users machineIndex handles into the array
  remoteMachines[machineIndex].refCount--;
}
  ...
  for (int pass = 0; pass < 2; pass++) {
    for (int i = 0; i < talkers.Num(); i++) {
      ...
      bool shouldBeRegistered =
          (talker.lobbyType != -1 && disableVoiceReasons == 0 &&
           talker.lobbyType == activeLobbyType);

      if (shouldBeRegistered && pass == 0) {
        continue;  // Only unregister on first pass to make room for when the
                   // second pass will possibly register new talkers
      }
