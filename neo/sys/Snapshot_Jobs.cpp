...
========================
SnapshotObjectJob
This job processes objects by delta comparing them, and then zrle encoding them
to the dest stream The dest stream is then eventually read by the lzw job, and
then lzw compressed into the final delta packet ready to be sent to peers.
========================
*/
void SnapshotObjectJob(objParms_t* parms) {
  ...
  if (!newState.valid) {
    ...
    if (header->csize == -1) {
      // Not enough space, don't compress, have lzw job do zrle compression
      // instead
    ...
    if (!visChange || visSendState) {
      ...
      rleCompressor.Start(dataStart, NULL,
                          OBJ_DEST_SIZE_ALIGN16(newState.size));
      ...
      if (header->csize == -1) {
        ...
        for (int b = 0; b < compareSize; b++) {
          *dataStart++ =
              ((0xFF + 1 + (newState.data[b] - oldState.data[b])) & 0xFF);
        }
        // Get leftover
        int leftOver = newState.size - compareSize;
...
static void FinishLZWStream(lzwParm_t* parm, idLZWCompressor* lzwCompressor) {
  ...
  lzwDelta_t& pendingDelta =
      parm->ioData->lzwDeltas[parm->ioData->numlzwDeltas];
  ...
  pendingDelta.snapSequence =
      parm->ioData
          ->snapSequence;  // Remember which snap sequence this delta belongs to
...
========================
LZWJobInternal
This job takes a stream of objects, which should already be zrle compressed, and
then lzw compresses them and builds a final delta packet ready to be sent to
peers.
========================
*/
void LZWJobInternal(lzwParm_t* parm, unsigned int dmaTag) {
  ...
  if (parm->ioData->numlzwDeltas > 0) {
    ...
    // We only send the first one and rely on a full snap being sent to get the
    // whole snap across
...
#ifdef __GNUC__
  // DG: remove ALIGN16 for GCC/clang, as they can't use it here and clang gets
  // an error
  ...
  if (parm->fragmented) {
    // This packet was partially written out, we need to continue writing, using
    // previous lzw dictionary values
  ...
  for (int i = 0; i < parm->numObjects; i++) {
    ...
    // It's nice to know right when it happens though, so you can inspect the
    // situation.
    assert(!lzwCompressor.IsOverflowed() || numChangedObjProcessed > 1);

    // First, see if we need to finish the current lzw stream
    if (lzwCompressor.IsOverflowed() ||
        lzwCompressor.Length() >= parm->ioData->optimalLength) {
...
#else
      // Currently, we don't use fragmented deltas.
      // We only send the first one and rely on a full snap being sent to get
      // the whole snap across
      assert(!parm->ioData->fullSnap);
      assert(parm->ioData->numlzwDeltas == 1);
      return;
#endif
    ...
    if (numChangedObjProcessed > 0) {
      // We should be at a good spot in the stream if we've written at least one
      // obj without overflowing, so save it
      lzwCompressor.Save();
    }
    ...
    lzwCompressor.WriteAgnostic<uint16>(
        (uint16)(header->objID - parm->ioData->lastObjId));
    ...
    if (header->flags & (OBJ_VIS_STALE | OBJ_VIS_NOT_STALE)) {
      ...
      objectSize_t value =
          (header->flags & OBJ_VIS_STALE) ? SIZE_STALE : SIZE_NOT_STALE;
  ...
  if (!parm->saveDictionary) {
    ...
    // the compressor did some work, wrote data to lzwMem, but since we didn't
    // call FinishLZWStream to end the compression, we need to figure how much
    // needs to be DMA'ed back out
    assert(parm->ioData->lzwBytes ==
           0);  // I don't think we ever hit this with lzwBytes != 0, but adding
                // it just in case
...
========================
*/
void LZWJob(lzwParm_t* parm) { LZWJobInternal(parm, 0); }
