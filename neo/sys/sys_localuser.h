...
They may not necessarily be in a game (which would be a session user of
TYPE_GAME). A controller user references an input device (which is a gamepad,
keyboard, etc).
================================================
*/
class idLocalUser {
 public:
  ...
  virtual bool IsPersistent() const {
    return IsProfileReady();  // True if this user is a persistent user, and can
                              // save stats, etc (signed in)
  }
  virtual bool IsProfileReady()
      const = 0;  // True if IsPersistent is true AND profile is signed into
                  // LIVE service
  virtual bool IsOnline()
      const = 0;  // True if this user has online capabilities
  virtual uint32 GetOnlineCaps()
      const = 0;  // Returns combination of onlineCaps_t flags
  virtual bool HasOwnerChanged() const {
    return false;  // Whether or not the original persistent owner has changed
  ...
  virtual bool IsInParty() const = 0;  // True if the user is in a party (do we
                                       // support this on pc and ps3? )
  virtual int GetPartyCount()
      const = 0;  // Gets the amount of users in the party

  // Storage related
  virtual bool IsStorageDeviceAvailable()
      const;  // Only false if the player has chosen to play without a storage
              // device, only possible on 360, if available, everything needs to
  ...
  // virtual bool				StorageSizeAvailable( uint64 minSizeInBytes, int64&
  // neededBytes );
  // RB end
  ...
  // Will attempt to sync the achievement bits between the server and the
  // localUser when the achievement system is ready
