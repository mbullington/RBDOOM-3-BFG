...
idCVar net_verboseSnapshotCompression(
    "net_verboseSnapshotCompression", "0", CVAR_INTEGER | CVAR_NOCHEAT,
    "Verbose snapshot code to help debug snapshot problems. Greater the number "
    "greater the spam");
idCVar net_verboseSnapshotReport(
    "net_verboseSnapshotReport", "0", CVAR_INTEGER | CVAR_NOCHEAT,
    "Verbose snapshot code to help debug snapshot problems. Greater the number "
    "greater the spam");

idCVar net_ssTemplateDebug("net_ssTemplateDebug", "0", CVAR_BOOL,
                           "Debug snapshot template states");
idCVar net_ssTemplateDebug_len("net_ssTemplateDebug_len", "32", CVAR_INTEGER,
                               "Offset to start template state debugging");
idCVar net_ssTemplateDebug_start(
    "net_ssTemplateDebug_start", "0", CVAR_INTEGER,
    "length of template state to print in debugging");
...
bool InDebugRange(int i) {
  return (i >= net_ssTemplateDebug_start.GetInteger() &&
          i < net_ssTemplateDebug_start.GetInteger() +
                  net_ssTemplateDebug_len.GetInteger());
}
/*
========================
PrintAlign
...
========================
*/
void PrintAlign(const char* text) { idLib::Printf("%25s: 0x", text); }
...
void idSnapShot::objectState_t::Print(const char* name) {
  ...
  unsigned int end = Min((unsigned int)buffer.Size(),
                         start + net_ssTemplateDebug_len.GetInteger());
...
========================
*/
void idSnapShot::objectBuffer_t::operator=(
...
========================
*/
void idSnapShot::PeekDeltaSequence(const char* deltaMem, int deltaSize,
                                   int& sequence, int& baseSequence) {
...
========================
*/
bool idSnapShot::ReadDeltaForJob(const char* deltaMem, int deltaSize,
                                 int visIndex, idSnapShot* templateStates) {
  ...
  idLZWCompressor lzwCompressor(&lzwData);
  int bytesRead = 0;  // how many uncompressed bytes we read in. Used to figure
  ...
  while (lzwCompressor.ReadAgnostic(delta, true) == sizeof(delta)) {
    ...
    if (objectNum >= 0xFFFF) {
      ...
      if (net_verboseSnapshotCompression.GetBool()) {
        float compRatio =
            static_cast<float>(deltaSize) / static_cast<float>(bytesRead);
        idLib::Printf(
            "Snapshot (%d/%d). ReadSize: %d DeltaSize: %d Ratio: %.3f\n",
    ...
    if (newsize == SIZE_STALE) {
      ...
      state.visMask |= (1 << visIndex);
      state.stale = false;
      // the latest state is packed in, get the new size and continue reading
      // the new state
      lzwCompressor.ReadAgnostic(newsize);
      bytesRead += sizeof(newsize);
    }
    ...
    if (newsize == 0) {
      // object deleted: reset state now so next one to use it doesn't have old
      // data
      state.deleted = false;
      state.stale = false;
      ...
      if (objTemplateState != NULL && objTemplateState->buffer.Size() &&
          objTemplateState->expectedSequence < baseSequence) {
        idLib::PrintfIf(net_ssTemplateDebug.GetBool(),
                        "Clearing old template state[%d] [%d<%d]\n", objectNum,
                        objTemplateState->expectedSequence, baseSequence);
      ...
      if (state.buffer.Size() == 0) {
        ...
        // Brand new state
        if (objTemplateState != NULL && objTemplateState->buffer.Size() > 0 &&
            sequence >= objTemplateState->expectedSequence) {
          idLib::PrintfIf(net_ssTemplateDebug.GetBool(),
                          "\nAdding basestate for new object %d (for SS %d/%d. "
                          "obj base created in ss %d) deltaSize: %d\n",
                          objectNum, sequence, baseSequence,
                          objTemplateState->expectedSequence, deltaSize);
          ...
          idLib::Printf(
              "\nNew snapobject[%d] in snapshot %d/%d but no basestate found "
              "locally so creating new\n",
              objectNum, sequence, baseSequence);
        }
      } else {
        state.createdFromTemplate = false;
      ...
      if (newsize > compareSize) {
        rleCompressor.ReadBytes(newbuffer.Ptr() + compareSize,
                                newsize - compareSize);
    ...
    if (state.buffer.Size() > 0) {
      ...
      if (!verify(checksum ==
                  SnapObjChecksum(state.buffer.Ptr(), state.buffer.Size()))) {
...
void idSnapShot::SubmitObjectJob(
    const submitDeltaJobsInfo_t& submitDeltaJobsInfo, objectState_t* newState,
    objectState_t* oldState, objParms_t*& baseObjParm, objParms_t*& curObjParm,
    objHeader_t*& curHeader, uint8*& curObjDest, lzwParm_t*& curlzwParm) {
  ...
  if (curObjParm - submitDeltaJobsInfo.objParms >=
      submitDeltaJobsInfo.maxObjParms) {
    idLib::Error("Out of parms for snapshot jobs.\n");
  }

  // Check to see if we are out of dest write space, and need to flush the jobs
  bool needToSubmit =
      (curObjDest - submitDeltaJobsInfo.objMemory) + totalSize >=
      submitDeltaJobsInfo.maxObjMemory;
  needToSubmit |= (curHeader - submitDeltaJobsInfo.headers >=
                   submitDeltaJobsInfo.maxHeaders);

  if (needToSubmit) {
    // If this obj will put us over the limit, then submit the jobs now, and
    // start over re-using the same buffers
    SubmitLZWJob(submitDeltaJobsInfo, baseObjParm, curObjParm, curlzwParm,
                 true);
...
Take the current list of delta'd + zlre processed objects, and write them to the
lzw stream.
========================
*/
void idSnapShot::SubmitLZWJob(
    ...
    objParms_t*&
        baseObjParm,  // Pointer to the first obj parm for the current stream
  ...
  curlzwParm->headers =
      writeDeltaInfo.headers;  // We always start grabbing from the beggining of
                               // the memory (it's reused, with fences to
                               // protect memory sharing)
...
newState parameter is optional and is just used for debugging/printf comparison
of the template and actual state
========================
*/
idSnapShot::objectState_t* idSnapShot::GetTemplateState(
    int objNum, idSnapShot* templateStates,
    idSnapShot::objectState_t* newState /*=NULL*/) {
...
========================
*/
void idSnapShot::SubmitWriteDeltaToJobs(
    const submitDeltaJobsInfo_t& submitDeltaJobInfo) {
  ...
  for (int i = 0; i < objectStates.Num(); i++) {
    ...
    if (!verify(newState.buffer.Size() > 0)) {
      // you CANNOT have a valid ss obj state w/ size = 0: this will be
      // interpreted as a delete in ::ReadDelta and this will completely throw
      // off delta compression, eventually resulting in a checksum error that is
      // a pain to track down.
      idLib::Warning("Snap obj [%d] state.size <= 0... skipping ",
                     newState.objectNum);
      continue;
    }
    ...
    if (j >= numOldStates) {
      ...
      objectState_t* oldState = GetTemplateState(
          newState.objectNum, submitDeltaJobInfo.templateStates, &newState);
      SubmitObjectJob(submitDeltaJobInfo, &newState, oldState, baseObjParms,
                      curObjParms, curHeader, curObjMemory, curlzwParms);
      continue;
    }
    ...
    for (; j < numOldStates &&
           newState.objectNum >
               submitDeltaJobInfo.oldSnap->objectStates[j]->objectNum;
         j++) {
      objectState_t& oldState = *submitDeltaJobInfo.oldSnap->objectStates[j];

      if ((oldState.stale && !oldState.deleted) ||
          oldState.buffer.Size() <= 0) {
        continue;  // Don't delete objects that are stale and not marked as
                   // deleted
      }

      SubmitObjectJob(submitDeltaJobInfo, NULL, &oldState, baseObjParms,
                      curObjParms, curHeader, curObjMemory, curlzwParms);
    }
    ...
    objectState_t& submittedOldState =
        *submitDeltaJobInfo.oldSnap->objectStates[j];
    ...
    if (newState.objectNum == oldState->objectNum) {
      ...
      // Spawned
      oldState = GetTemplateState(newState.objectNum,
                                  submitDeltaJobInfo.templateStates, &newState);
      SubmitObjectJob(submitDeltaJobInfo, &newState, oldState, baseObjParms,
                      curObjParms, curHeader, curObjMemory, curlzwParms);
    }
  }
  ...
  // Submit any objects that are left over (will be all if they all fit up to
  // this point)
  SubmitLZWJob(submitDeltaJobInfo, baseObjParms, curObjParms, curlzwParms,
               false);
}
...
bool idSnapShot::ReadDelta(idFile* file, int visIndex) {
  ...
  while (file->ReadBig(delta) == sizeof(delta)) {
    ...
    if (state.buffer.Size() > 0) {
      ...
      assert(checksum ==
             MD5_BlockChecksum(state.buffer.Ptr(), state.buffer.Size()));
    }
#endif
  }
...
========================
*/
void idSnapShot::WriteObject(idFile* file, int visIndex,
                             objectState_t* newState, objectState_t* oldState,
                             int& lastobjectNum) {
  assert(newState != NULL || oldState != NULL);

  bool visChange =
      false;  // visibility changes will be signified with a 0xffff state size
  bool visSendState =
      false;  // the state is sent when an entity is no longer stale

  // Compute visibility changes
  // (we need to do this before writing out object id, because we may not need
  // to write out the id if we early out) (when we don't write out the id, we
  // assume this is an "ack" when we deserialize the objects)
  if (newState != NULL && oldState != NULL) {
    ...
    if (visIndex > 0) {
      ...
      newVisible |=
          (newState->buffer.Size() == 0) != (oldState->buffer.Size() == 0);

      if (!oldVisible && !newVisible) {
        // object is stale and ack'ed for this client, write nothing (see 'same
        // object' below)
        return;
    ...
    if (!visChange && newState->buffer.Size() == oldState->buffer.Size() &&
        ((newState->buffer.Ptr() == oldState->buffer.Ptr()) ||
         memcmp(newState->buffer.Ptr(), oldState->buffer.Ptr(),
                newState->buffer.Size()) == 0)) {
  ...
  if (newState == NULL) {
    ...
    if (visChange) {
      ...
      // NOTE: we may still send a real size and a state below, for 'no longer
      // stale' transitions TMP: send 0xFFFF for going stale and 0xFFFF - 1 for
      // no longer stale
      file->WriteBig<objectSize_t>(visSendState ? SIZE_NOT_STALE : SIZE_STALE);
    }
    if (!visChange || visSendState) {
      objectSize_t compareSize =
          Min(newState->buffer.Size(),
              oldState->buffer.Size());  // Get the number of bytes that overlap
      ...
      for (objectSize_t b = 0; b < compareSize; b++) {
        file->WriteBig<byte>(
            (0xFF + 1 + newState->buffer[b] - oldState->buffer[b]) & 0xFF);
      }

      // Write leftover
      if (newState->buffer.Size() > compareSize) {
        file->Write(newState->buffer.Ptr() + oldState->buffer.Size(),
                    newState->buffer.Size() - compareSize);
      }
    }
  }
  ...
  if ((!visChange || visSendState) && newState != NULL) {
    ...
    unsigned int checksum =
        MD5_BlockChecksum(newState->buffer.Ptr(), newState->buffer.Size());
...
========================
*/
bool idSnapShot::WriteDelta(idSnapShot& old, int visIndex, idFile* file,
                            int maxLength, int optimalLength) {
  file->WriteBig(time);
  ...
  for (int i = 0; i < objectStates.Num(); i++) {
    ...
    if (!verify(newState.buffer.Size() < maxLength)) {
      // If the new state's size is > the max packet size, we'll never be able
      // to send it!
      idLib::Warning(
          "Snap obj [%d] state.size > max packet length. Skipping... ",
          newState.objectNum);
      continue;
    ...
    if (file->Length() + objectHeaderSize + newState.buffer.Size() >=
        maxLength) {
      return false;
    }
    ...
    if (newState.objectNum == oldState.objectNum) {
      // FIXME: We don't need to early out if WriteObject determines that we
      // won't send the object due to being stale
      if (file->Length() + objectHeaderSize + newState.buffer.Size() >=
          maxLength) {
        return false;
      }
...
bool idSnapShot::CopyObject(const idSnapShot& oldss, int objectNum,
                            bool forceStale) {
...
start, end, and oldStart can optionally be passed in to compare subsections of
the object default parameters will compare entire object
========================
*/
int idSnapShot::CompareObject(const idSnapShot* oldss, int objectNum, int start,
                              int end, int oldStart) {
  if (oldss == NULL) {
    return 0;
  }
  ...
  int oldIndex = oldss->FindObjectIndexByID(objectNum);
  if (oldIndex == -1) {
    return (end == 0 ? newState.buffer.Size()
                     : end - start);  // Didn't exist in the old state, so we
                                      // take the hit on the entire size
  }

  objectState_t& oldState =
      const_cast<objectState_t&>(*oldss->objectStates[oldIndex]);
  ...
  int commonSize =
      (newState.buffer.Size() <= oldState.buffer.Size() - oldOffset)
          ? newState.buffer.Size()
          : oldState.buffer.Size() - oldOffset;
  if (end == 0) {
    // default 0 means compare the whole thing
    end = commonSize;

    // Get leftover (if any)
    bytes = (newState.buffer.Size() > oldState.buffer.Size())
                ? (newState.buffer.Size() - oldState.buffer.Size())
                : 0;
  } else {
  ...
  for (int b = start; b < end; b++) {
    if (verify(b >= 0 && b < (int)newState.buffer.Size() &&
               b + oldOffset >= 0 &&
               b + oldOffset < (int)oldState.buffer.Size())) {
...
int idSnapShot::FindObjectIndexByID(int objectNum) const {
  ...
  if (i >= 0 && i < objectStates.Num() &&
      objectStates[i]->objectNum == objectNum) {
    return i;
  }
  return -1;
}
...
void idSnapShot::ApplyToExistingState(int objId, idBitMsg& msg) {
  ...
  if (!objectState->createdFromTemplate) {
    // We were created this from a template, so we shouldn't be applying it
    // again
    if (net_ssTemplateDebug.GetBool()) {
      idLib::Printf(
          "NOT ApplyToExistingState[%d] because object was created from "
          "existing base state. %d\n",
  ...
  // Actually apply it
  for (objectSize_t i = 0; i < Min(objectState->buffer.Size(), msg.GetSize());
       i++) {
