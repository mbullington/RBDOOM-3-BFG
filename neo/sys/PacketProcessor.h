...
class idPacketProcessor {
 public:
  // DG: workaround for GCC bug (can't link when compiling with -O0): put
  // definitions in PacketProcessor.cpp
  ...
  // static const int MAX_PACKET_SIZE		= MAX_FINAL_PACKET_SIZE - 6 - sizeof(
  // sessionId_t );	// Largest possible packet before headers and such applied
  // (subtract some for various internal header data, and session id)
  static const int MAX_PACKET_SIZE =
      MAX_FINAL_PACKET_SIZE - 6 -
 ...
 public:
  ...
  // decrease a fragmentation counter, so we reflect how much we're maxing the
  // MTU
 ...
 private:
  ...
  int numReliable;
  byte
      reliableBuffer[MAX_MSG_SIZE];  // We shouldn't have to hold more than this
  ...
  idBitMsg unsentMsg;
  byte unsentBuffer[MAX_MSG_SIZE];  // Buffer used hold the current msg until
                                    // it's all sent
  ...
  int fragmentAccumulator;  // counts max size packets we are sending for the
                            // net debug hud
};
