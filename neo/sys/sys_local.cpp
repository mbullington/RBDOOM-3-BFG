...
const char* sysLanguageNames[] = {ID_LANG_ENGLISH,
                                  ID_LANG_FRENCH,
                                  ID_LANG_ITALIAN,
                                  ID_LANG_GERMAN,
                                  ID_LANG_SPANISH,
...
const int numLanguages =
    sizeof(sysLanguageNames) / sizeof sysLanguageNames[0] - 1;

// RB: allow sys_lang to be saved to config so it has to be set per cmdline only
// a single time
idCVar sys_lang("sys_lang", ID_LANG_ENGLISH,
                CVAR_SYSTEM | CVAR_INIT | CVAR_ARCHIVE, "", sysLanguageNames,
...
double idSysLocal::ClockTicksPerSecond() { return Sys_ClockTicksPerSecond(); }

cpuid_t idSysLocal::GetProcessorId() { return Sys_GetProcessorId(); }
...
bool idSysLocal::FPU_StackIsEmpty() { return Sys_FPU_StackIsEmpty(); }

void idSysLocal::FPU_SetFTZ(bool enable) { Sys_FPU_SetFTZ(enable); }

void idSysLocal::FPU_SetDAZ(bool enable) { Sys_FPU_SetDAZ(enable); }
...
int idSysLocal::DLL_Load(const char* dllName) { return Sys_DLL_Load(dllName); }

void* idSysLocal::DLL_GetProcAddress(int dllHandle, const char* procName) {
  return Sys_DLL_GetProcAddress(dllHandle, procName);
}

void idSysLocal::DLL_Unload(int dllHandle) { Sys_DLL_Unload(dllHandle); }

void idSysLocal::DLL_GetFileName(const char* baseName, char* dllName,
                                 int maxLength) {
...
int Sys_NumLangs() { return numLanguages; }
