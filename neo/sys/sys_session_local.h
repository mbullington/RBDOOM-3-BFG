...
// DG: achievements and signin is the same on windows, linux => put them in
// common dir
...
class idLobbyStub : public idLobbyBase {
 public:
  ...
  virtual int GetLobbyUserIndexFromLobbyUserID(
      lobbyUserID_t lobbyUserID) const {
    return -1;
  }
...
class idSessionLocal : public idSession {
 ...
 public:
  ...
  virtual bool ShouldShowMigratingDialog()
      const;  // Note this is not in sys_session.h
  ...
  virtual bool GetMigrationGameDataUser(lobbyUserID_t lobbyUserID,
  ...
  virtual bool IsAboutToLoad() const {
    return GetGameLobby().IsLobbyActive() &&
           GetGameLobby().startLoadingFromHost;
  }
  virtual bool GetMatchParamUpdate(int& peer, int& msg);
  virtual int GetInputRouting(int inputRouting[MAX_INPUT_DEVICES]);
  virtual void EndMatch(
      bool premature = false);  // Meant for host to end match gracefully, go
                                // back to lobby, tally scores, etc
  virtual void
  MatchFinished();  // this is for when the game is over before we go back to
                    // lobby. Need this incase the host leaves during this time
  virtual void QuitMatch();  // Meant for host or peer to quit the match before
                             // it ends, will instigate host migration, etc
  virtual void QuitMatchToTitle();  // Will forcefully quit the match and return
                                    // to the title screen.
  ...
  // Title Storage Vars
  //=====================================================================================================
  virtual float GetTitleStorageFloat(const char* name,
  ...
  virtual const char* GetTitleStorageString(const char* name,
                                            const char* defaultString) const {
    return titleStorageVars.GetString(name, defaultString);
  }

  virtual bool GetTitleStorageFloat(const char* name, float defaultFloat,
                                    float& out) const {
    return titleStorageVars.GetFloat(name, defaultFloat, out);
  }
  virtual bool GetTitleStorageInt(const char* name, int defaultInt,
                                  int& out) const {
    return titleStorageVars.GetInt(name, defaultInt, out);
  }
  virtual bool GetTitleStorageBool(const char* name, bool defaultBool,
                                   bool& out) const {
    return titleStorageVars.GetBool(name, defaultBool, out);
  }
  virtual bool GetTitleStorageString(const char* name,
  ...
  virtual voiceStateDisplay_t GetDisplayStateFromVoiceState(
  ...
  virtual void SetDiscSwapMPInvite(
      void* parm) = 0;  // call to request a discSwap multiplayer invite
  ...
  // This is called when the master LocalUser is signed in/out, these are called
  // after OnLocalUserSignin/out()
  ...
  // Leaderboards
  virtual void LeaderboardUpload(lobbyUserID_t lobbyUserID,
                                 const leaderboardDefinition_t* leaderboard,
                                 const column_t* stats,
                                 const idFile_Memory* attachment = NULL) = 0;
  virtual void LeaderboardDownload(int sessionUserIndex,
                                   const leaderboardDefinition_t* leaderboard,
                                   int startingRank, int numRows,
                                   const idLeaderboardCallback& callback) = 0;
  virtual void LeaderboardDownloadAttachment(
      int sessionUserIndex, const leaderboardDefinition_t* leaderboard,
      int64 attachmentID) = 0;

  // Scoring (currently just for TrueSkill)
  virtual void SetLobbyUserRelativeScore(lobbyUserID_t lobbyUserID,
                                         int relativeScore, int team) = 0;
  ...
  //	Savegames
  //=====================================================================================================
  virtual saveGameHandle_t SaveGameSync(const char* name,
                                        const saveFileEntryList_t& files,
                                        const idSaveGameDetails& description);
  virtual saveGameHandle_t SaveGameAsync(const char* name,
                                         const saveFileEntryList_t& files,
                                         const idSaveGameDetails& description);
  virtual saveGameHandle_t LoadGameSync(const char* name,
  ...
  virtual bool IsSaveGameCompletedFromHandle(
      const saveGameHandle_t& handle) const {
 ...
 protected:
  ...
  idLobbyStub stubLobby;  // We use this when we request the active lobby when
                          // we are not in a lobby (i.e at press start)

  int currentID;  // The host used this to send out a unique id to all users so
  ...
  float upstreamDropRate;  // instant rate in B/s at which we are dropping
                           // packets due to simulated upstream saturation
  int upstreamDropRateTime;

  float upstreamQueueRate;  // instant rate in B/s at which queued packets are
                            // coming out after local buffering due to upstream
                            // saturation
  ...
  void QueuePacket(idQueue<idQueuePacket, &idQueuePacket::queueNode>& queue,
                   int time, const lobbyAddress_t& to, const void* data,
                   int size, bool dedicated);
  bool ReadRawPacketFromQueue(int time, lobbyAddress_t& from, void* data,
                              int& size, bool& outDedicated, int maxSize);

  void SendRawPacket(const lobbyAddress_t& to, const void* data, int size,
                     bool dedicated);
  bool ReadRawPacket(lobbyAddress_t& from, void* data, int& size,
                     bool& outDedicated, int maxSize);

  void ConnectAndMoveToLobby(idLobby& lobby,
  ...
  void WriteLeaderboardToMsg(idBitMsg& msg,
                             const leaderboardDefinition_t* leaderboard,
                             const column_t* stats);
  void SendLeaderboardStatsToPlayer(lobbyUserID_t lobbyUserID,
                                    const leaderboardDefinition_t* leaderboard,
                                    const column_t* stats);
  void RecvLeaderboardStatsForPlayer(idBitMsg& msg);

  const leaderboardDefinition_t* ReadLeaderboardFromMsg(idBitMsg& msg,
                                                        column_t* stats);
  ...
  virtual bool GetLobbyAddressFromNetAddress(const netadr_t& netAddr,
                                             lobbyAddress_t& outAddr) const = 0;
  virtual bool GetNetAddressFromLobbyAddress(const lobbyAddress_t& lobbyAddress,
                                             netadr_t& outNetAddr) const = 0;

  void HandleDedicatedServerQueryRequest(lobbyAddress_t& remoteAddr,
                                         idBitMsg& msg, int msgType);
