...
// RB: FIXME this shit. We need the OpenGL alpha channel for advanced rendering
// effects
idCVar r_waylandcompat("r_waylandcompat", "0",
                       CVAR_SYSTEM | CVAR_NOCHEAT | CVAR_ARCHIVE,
                       "wayland compatible framebuffer");

// RB: only relevant if using SDL 2.0
#if defined(__APPLE__)
// only core profile is supported on OS X
idCVar r_useOpenGL32("r_useOpenGL32", "2", CVAR_INTEGER,
                     "0 = OpenGL 3.x, 1 = OpenGL 3.2 compatibility profile, 2 "
                     "= OpenGL 3.2 core profile",
                     0, 2);
#elif defined(__linux__)
// Linux open source drivers suck
idCVar r_useOpenGL32("r_useOpenGL32", "0", CVAR_INTEGER,
                     "0 = OpenGL 3.x, 1 = OpenGL 3.2 compatibility profile, 2 "
                     "= OpenGL 3.2 core profile",
                     0, 2);
#else
...
GLimp_PreInit
 ...
 R_GetModeListForDisplay is called before GLimp_Init(), but SDL needs SDL_Init()
first. So do that in GLimp_PreInit() Calling that function more than once
...
bool GLimp_Init(glimpParms_t parms) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (parms.fullScreen > 0) {
      if (parms.fullScreen > SDL_GetNumVideoDisplays()) {
        common->Warning(
            "Couldn't set display to num %i because we only have %i displays",
            parms.fullScreen, SDL_GetNumVideoDisplays());
      } else {
        // -1 because SDL starts counting displays at 0, while parms.fullScreen
        // starts at 1
        windowPos = SDL_WINDOWPOS_UNDEFINED_DISPLAY((parms.fullScreen - 1));
      }
    }
    // TODO: if parms.fullScreen == -1 there should be a borderless window
    // spanning multiple displays
    /*
     * NOTE that this implicitly handles parms.fullScreen == -2 (from
     * r_fullscreen -2) meaning "do fullscreen, but I don't care on what
    ...
    if (!window) {
      common->DPrintf("Couldn't set GL mode %d/%d/%d: %s", channelcolorbits,
                      tdepthbits, tstencilbits, SDL_GetError());
      continue;
    }
    ...
    // RB begin
    SDL_GetWindowSize(window, &glConfig.nativeScreenWidth,
                      &glConfig.nativeScreenHeight);
    // RB end

    glConfig.isFullscreen = (SDL_GetWindowFlags(window) &
  ...
  if (GLEW_OK != glewResult) {
    ...
    common->Printf("^3GLimp_Init() - GLEW could not load OpenGL subsystem: %s",
  ...
  // SRS - On OSX enable SDL2 relative mouse mode warping to capture mouse
  // properly if outside of window
  SDL_SetHintWithPriority(SDL_HINT_MOUSE_RELATIVE_MODE_WARP, "1",
                          SDL_HINT_OVERRIDE);
#endif

  // DG: disable cursor, we have two cursors in menu (because mouse isn't
  // grabbed in menu)
...
#if SDL_VERSION_ATLEAST(2, 0, 0)
// SDL1 doesn't support multiple displays, so the source is much shorter and
// doesn't need separate functions makes sure the window will be full-screened
// on the right display and returns the SDL display index
static int ScreenParmsHandleDisplayIndex(glimpParms_t parms) {
  int displayIdx;
  if (parms.fullScreen > 0) {
    displayIdx =
        parms.fullScreen - 1;  // first display for SDL is 0, in parms it's 1
  } else                       // -2 == use current display
  {
    displayIdx = SDL_GetWindowDisplayIndex(window);
    if (displayIdx <
        0)  // for some reason the display for the window couldn't be detected
    {
      displayIdx = 0;
    }
  }
  ...
  if (parms.fullScreen > SDL_GetNumVideoDisplays()) {
    common->Warning(
        "Can't set fullscreen mode to display number %i, because SDL2 only "
        "knows about %i displays!",
        parms.fullScreen, SDL_GetNumVideoDisplays());
    return -1;
  }
  ...
  if (parms.fullScreen != glConfig.isFullscreen) {
    ...
    if (glConfig.isFullscreen) {
      // if we're already in fullscreen mode but want to switch to another
      // monitor we have to go to windowed mode first to move the window..
      // SDL-oddity.
...
static bool SetScreenParmsFullscreen(glimpParms_t parms) {
  ...
  if (SDL_SetWindowDisplayMode(window, &m) < 0) {
    common->Warning("Couldn't set window mode for fullscreen, reason: %s",
                    SDL_GetError());
    return false;
  }
  ...
  if (!(SDL_GetWindowFlags(window) & SDL_WINDOW_FULLSCREEN)) {
    if (SDL_SetWindowFullscreen(window, SDL_TRUE) < 0) {
      common->Warning("Couldn't switch to fullscreen mode, reason: %s!",
                      SDL_GetError());
      return false;
    }
  }
...
static bool SetScreenParmsWindowed(glimpParms_t parms) {
  ...
  if (SDL_GetWindowFlags(window) & SDL_WINDOW_FULLSCREEN) {
    if (SDL_SetWindowFullscreen(window, SDL_FALSE) < 0) {
      common->Warning("Couldn't switch to windowed mode, reason: %s!",
                      SDL_GetError());
      return false;
    }
  }
...
bool GLimp_SetScreenParms(glimpParms_t parms) {
  ...
  } else if (parms.fullScreen == 0)  // windowed mode
  {
    ...
    common->Warning(
        "GLimp_SetScreenParms: fullScreen -1 (borderless window for multiple "
        "displays) currently unsupported!");
    return false;
  }
  ...
  if (s == NULL) {
    common->Warning(
        "GLimp_SetScreenParms: Couldn't get video information, reason: %s",
        SDL_GetError());
    return false;
  }
  ...
  if (s == NULL) {
    common->Warning(
        "GLimp_SetScreenParms: Couldn't set video information, reason: %s",
        SDL_GetError());
    return false;
  }
#endif  // SDL_VERSION_ATLEAST(2, 0, 0)
...
#if SDL_VERSION_ATLEAST(2, 0, 0)
  // DG: disabling the cursor is now done once in GLimp_Init() because it should
  // always be disabled

  // DG: check for GRAB_ENABLE instead of GRAB_HIDECURSOR because we always
  // wanna hide it
...
====================
*/
bool R_GetModeListForDisplay(const int requestedDisplayNum,
  ...
  if (numModes > 0) {
    for (int i = 0; i < numModes; i++) {
      ...
      mode.height = m.h;
      mode.displayHz =
          m.refresh_rate ? m.refresh_rate : 60;  // default to 60 if unknown (0)
      modeList.AddUnique(mode);
    }

    if (modeList.Num() < 1) {
      common->Warning(
          "Couldn't get a single video mode for display %i, using default "
          "ones..!\n",
          requestedDisplayNum);
  ...
  // R_GetModeListForDisplay() returning false for invalid displaynum to iterate
  // all displays
  ...
  if (videoInfo == NULL) {
    // DG: yes, this can actually fail, e.g. if SDL_Init( SDL_INIT_VIDEO )
    // wasn't called
    common->Warning("Can't get Video Info, using default modes...\n");
    FillStaticVidModes(modeList);
    return true;
  ...
  SDL_Rect** modes =
      SDL_ListModes(videoInfo->vfmt, SDL_OPENGL | SDL_FULLSCREEN);

  if (!modes) {
    common->Warning(
        "Can't get list of available modes, using default ones...\n");
    FillStaticVidModes(modeList);
    return true;
  }
