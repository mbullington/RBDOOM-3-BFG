...
// vulkanContext_t vkcontext; // Eric: I added this to pass SDL_Window* window
// to the SDL_Vulkan_* methods that are used else were.
...
std::vector<const char*> get_required_extensions() {
  ...
  SDL_Vulkan_GetInstanceExtensions(nullptr, &sdlCount,
                                   sdlInstanceExtensions.data());

  // SRS - Report enabled instance extensions in CreateVulkanInstance() vs.
  // doing it here
  /*
  ...
  */
  ...
  // SRS - needed for MoltenVK portability implementation and optionally for
  // MoltenVK configuration on OSX
#if defined(__APPLE__)
  sdlInstanceExtensions.push_back(
  ...
  // SRS - Add debug instance extensions in CreateVulkanInstance() vs.
...
VKimp_PreInit
 ...
 R_GetModeListForDisplay is called before VKimp_Init(), but SDL needs SDL_Init()
first. So do that in VKimp_PreInit() Calling that function more than once
...
/* Eric: Is the majority of this function not needed since switching from GL to
Vulkan?
===================
VKimp_Init
===================
...
bool VKimp_Init(glimpParms_t parms) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (!window) {
      common->DPrintf("Couldn't set Vulkan mode %d/%d/%d: %s", channelcolorbits,
...
bool VKimp_SetScreenParms(glimpParms_t parms) {
  ...
  } else if (parms.fullScreen == 0)  // windowed mode
  {
    ...
    common->Warning(
        "VKimp_SetScreenParms: fullScreen -1 (borderless window for multiple "
...
void VKimp_GrabInput(int flags) {
  ...
  // DG: disabling the cursor is now done once in VKimp_Init() because it should
  // always be disabled
