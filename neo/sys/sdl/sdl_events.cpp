...
// DG: SDL1 doesn't seem to have defines for scancodes.. add the (only) one we
// need
#define SDL_SCANCODE_GRAVE \
  49  // in SDL2 this is 53.. but according to two different systems and
      // keyboards this works for SDL1
      // DG end
#endif
...
const char* kbdNames[] = {"english", "french",  "german",    "italian",
                          "spanish", "turkish", "norwegian", NULL};

idCVar in_keyboard("in_keyboard", "english",
                   CVAR_SYSTEM | CVAR_ARCHIVE | CVAR_NOCHEAT, "keyboard layout",
                   kbdNames, idCmdSystem::ArgCompletion_String<kbdNames>);
...
// both strings are expected to have at most SDL_TEXTINPUTEVENT_TEXT_SIZE
// chars/ints (including terminating null)
static void ConvertUTF8toUTF32(const char* utf8str, int32* utf32buf) {
  static SDL_iconv_t cd = SDL_iconv_t(-1);

  if (cd == SDL_iconv_t(-1)) {
    const char* toFormat =
        "UTF-32LE";  // TODO: what does d3bfg expect on big endian machines?
    cd = SDL_iconv_open(toFormat, "UTF-8");
    if (cd == SDL_iconv_t(-1)) {
      common->Warning(
          "Couldn't initialize SDL_iconv for UTF-8 to UTF-32!");  // TODO: or
                                                                  // error?
      return;
    }
  }
  ...
  size_t outbytesleft =
      4 * SDL_TEXTINPUTEVENT_TEXT_SIZE;  // *4 because utf-32 needs 4x as much
                                         // space as utf-8
  ...
  {
    common->Warning(
        "Converting UTF-8 string \"%s\" from SDL_TEXTINPUT to UTF-32 failed!",
        utf8str);
...
===============
*/
unsigned char Sys_MapCharForKey(int key) { return key & 0xff; }
...
void Sys_GrabMouseCursor(bool grabIt) {
  ...
  if (grabIt) {
    // DG: disabling the cursor is now done once in GLimp_Init() because it
...
sysEvent_t Sys_GetEvent() {
  ...
  // loop until there is an event we care about (will return then) or no more
  // events
  while (SDL_PollEvent(&ev)) {
    switch (ev.type) {
      ...
      case SDL_WINDOWEVENT:
        switch (ev.window.event) {
          case SDL_WINDOWEVENT_FOCUS_GAINED: {
            // unset modifier, in case alt-tab was used to leave window and ALT
            // is still set as that can cause fullscreen-toggling when pressing
            // enter...
            ...
            // DG: un-pause the game when focus is gained, that also re-grabs
            // the input
            //     disabling the cursor is now done once in GLimp_Init() because
          ...
          case SDL_WINDOWEVENT_FOCUS_LOST:
            // DG: pause the game when focus is lost, that also un-grabs the
            // input
      ...
      case SDL_ACTIVEEVENT: {
        // DG: (un-)pause the game when focus is gained, that also (un-)grabs
        // the input
        bool pause = true;
        ...
        if (ev.active.gain) {
          ...
          // unset modifier, in case alt-tab was used to leave window and ALT is
          // still set as that can cause fullscreen-toggling when pressing
          // enter...
      ...
      case SDL_VIDEORESIZE: {
        ...
        // for some reason this needs a vid_restart in SDL1 but not SDL2 so
        // GLimp_SetScreenParms() is called
      ...
      case SDL_KEYDOWN:
        if (ev.key.keysym.sym == SDLK_RETURN &&
          ...
          // DG: go to fullscreen on current display, instead of always first
          // display
          int fullscreen = 0;
        ...
        // DG: ctrl-g to un-grab mouse - yeah, left ctrl shoots, then just use
        // right ctrl :)
...
#if !SDL_VERSION_ATLEAST(2, 0, 0)
        // DG: only do this for key-down, don't care about isChar from
        // SDL_KeyToDoom3Key.
      ...
      case SDL_KEYUP: {
        ...
        if (ev.key.keysym.scancode == SDL_SCANCODE_GRAVE) {
          ...
          if (key == 0) {
            // SDL2 has no ev.key.keysym.unicode anymore.. but the scancode
            // should work well enough for console
            if (ev.type == SDL_KEYDOWN)  // FIXME: don't complain if this was an
                                         // ASCII char and the console is open?
            {
          ...
          if (key == 0) {
            ...
            if (uc == Sys_GetConsoleKey(false) ||
                uc == Sys_GetConsoleKey(true)) {
              key = K_GRAVE;
              uniChar =
                  K_BACKSPACE;  // bad hack to get empty console inputline..
            } else {
              if (uniChar) {
                res.evType = SE_CHAR;
                res.evValue = uniChar;
              ...
              if (ev.type ==
                  SDL_KEYDOWN)  // FIXME: don't complain if this was an ASCII
                                // char and the console is open?
              {
                common->Warning("unmapped SDL key %d (0x%x) scancode %d",
      ...
      case SDL_FINGERMOTION:
        continue;  // Avoid 'unknown event' spam when testing with touchpad by
                   // skipping this
      ...
      case SDL_MOUSEBUTTONUP:
        ...
        switch (ev.button.button) {
          case SDL_BUTTON_LEFT:
            ...
            mouse_polls.Append(mouse_poll_t(
                M_ACTION1, ev.button.state == SDL_PRESSED ? 1 : 0));
            break;
          case SDL_BUTTON_MIDDLE:
            res.evValue = K_MOUSE3;
            mouse_polls.Append(mouse_poll_t(
                M_ACTION3, ev.button.state == SDL_PRESSED ? 1 : 0));
            break;
          case SDL_BUTTON_RIGHT:
            res.evValue = K_MOUSE2;
            mouse_polls.Append(mouse_poll_t(
                M_ACTION2, ev.button.state == SDL_PRESSED ? 1 : 0));
            break;
          ...
          default:
            ...
            if (ev.button.button <=
                16)  // d3bfg doesn't support more than 16 mouse buttons
            {
              int buttonIndex = ev.button.button - SDL_BUTTON_LEFT;
              res.evValue = K_MOUSE1 + buttonIndex;
              mouse_polls.Append(
                  mouse_poll_t(M_ACTION1 + buttonIndex,
                               ev.button.state == SDL_PRESSED ? 1 : 0));
      ...
      case SDL_CONTROLLERAXISMOTION:
        ...
        res.evValue =
            J_AXIS_LEFT_X + (ev.caxis.axis - SDL_CONTROLLER_AXIS_LEFTX);
      ...
      case SDL_CONTROLLERBUTTONUP:
        ...
        joystick_polls.Append(
            joystick_poll_t(controllerButtonRemap[ev.cbutton.button][1],
      ...
      // totally independant. I think the event returned by this function seems
      // to work on the GUI and the event returned by
      // Sys_ReturnJoystickInputEvent() works on the game. Also, remember that
      // joystick keys must be binded to actions in order to work!
      case SDL_JOYBUTTONDOWN:
      case SDL_JOYBUTTONUP:
        // sys_public.h: evValue is an axis number and evValue2 is the current
        // state (-127 to 127) WM0110: joystick ranges must be between (-32769,
        // 32768)!
        res.evType = SE_KEY;
        switch (ev.jbutton.button) {
          case 0:
            res.evValue = K_JOY1;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION1, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 1:
            res.evValue = K_JOY2;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION2, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 2:
            res.evValue = K_JOY3;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION3, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 3:
            res.evValue = K_JOY4;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION4, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 4:
            res.evValue = K_JOY5;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION5, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 5:
            res.evValue = K_JOY6;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION6, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 6:
            res.evValue = K_JOY7;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION7, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 7:
            res.evValue = K_JOY8;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION8, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 8:
            res.evValue = K_JOY9;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION9, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 9:
            res.evValue = K_JOY10;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION10, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          case 10:
            res.evValue = K_JOY11;
            joystick_polls.Append(joystick_poll_t(
                J_ACTION11, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            break;

          // D-PAD left (XBox 360 wireless)
          case 11:
            // If joystick has a hat, then use the hat as D-PAD. If not, D-PAD
            // is mapped to buttons.
            if (SDL_joystick_has_hat) {
              res.evValue = K_JOY12;
              joystick_polls.Append(joystick_poll_t(
                  J_ACTION12, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            } else {
              res.evValue = K_JOY_DPAD_LEFT;
              joystick_polls.Append(joystick_poll_t(
                  J_DPAD_LEFT, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            }
            break;
          ...
          case 12:
            if (SDL_joystick_has_hat) {
              res.evValue = K_JOY13;
              joystick_polls.Append(joystick_poll_t(
                  J_ACTION13, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            } else {
              res.evValue = K_JOY_DPAD_RIGHT;
              joystick_polls.Append(joystick_poll_t(
                  J_DPAD_RIGHT, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            }
            break;
          ...
          case 13:
            if (SDL_joystick_has_hat) {
              res.evValue = K_JOY14;
              joystick_polls.Append(joystick_poll_t(
                  J_ACTION14, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            } else {
              res.evValue = K_JOY_DPAD_UP;
              joystick_polls.Append(joystick_poll_t(
                  J_DPAD_UP, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            }
            break;
          ...
          case 14:
            if (SDL_joystick_has_hat) {
              res.evValue = K_JOY15;
              joystick_polls.Append(joystick_poll_t(
                  J_ACTION15, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            } else {
              res.evValue = K_JOY_DPAD_DOWN;
              joystick_polls.Append(joystick_poll_t(
                  J_DPAD_DOWN, ev.jbutton.state == SDL_PRESSED ? 1 : 0));
            }
            break;

          default:
            common->Warning(
                "Sys_GetEvent(): Unknown joystick button number %i\n",
                ev.jbutton.button);
      ...
      case SDL_JOYHATMOTION:
        ...
        // SDL_HAT_CENTERED is defined as 0
          ...
          if (previous_hat_state == J_DPAD_UP) {
            ...
            common->Warning(
                "Sys_GetEvent(): SDL_JOYHATMOTION: previous state "
                "SDL_HAT_CENTERED repeated!\n");
            continue;  // just try next event
          } else {
            common->Warning(
                "Sys_GetEvent(): SDL_JOYHATMOTION: unknown previous hat state "
                "%i\n",
                previous_hat_state);
            continue;  // just try next event
          }
      ...
      case SDL_JOYAXISMOTION:
        ...
        switch (ev.jaxis.axis) {
          ...
          case 2:
            ...
            // common->Printf("Sys_GetEvent: LEFT trigger value = %i / converted
            // value = %i\n", ev.jaxis.value, trigger_value);
            res.evValue = J_AXIS_LEFT_TRIG;

            joystick_polls.Append(
                joystick_poll_t(J_AXIS_LEFT_TRIG, triggerValue));
            break;

          // Right trigger
          case 5:
            triggerValue = (ev.jaxis.value + 32768) / 2;
            // common->Printf("Sys_GetEvent: RIGHT trigger value = %i /
            // converted value = %i\n", ev.jaxis.value, trigger_value);
            res.evValue = J_AXIS_RIGHT_TRIG;

            joystick_polls.Append(
                joystick_poll_t(J_AXIS_RIGHT_TRIG, triggerValue));
            break;
          ...
          case 0:
            ...
            joystick_polls.Append(
                joystick_poll_t(J_AXIS_LEFT_X, ev.jaxis.value));
          ...
          case 1:
            ...
            joystick_polls.Append(
                joystick_poll_t(J_AXIS_LEFT_Y, ev.jaxis.value));
          ...
          case 3:
            ...
            joystick_polls.Append(
                joystick_poll_t(J_AXIS_RIGHT_X, ev.jaxis.value));
          ...
          case 4:
            ...
            joystick_polls.Append(
                joystick_poll_t(J_AXIS_RIGHT_Y, ev.jaxis.value));
...
================
*/
int Sys_PollKeyboardInputEvents() { return kbd_polls.Num(); }
...
================
*/
void Sys_EndKeyboardInputEvents() { kbd_polls.SetNum(0); }
...
const char* Sys_GetKeyName(keyNum_t keynum) {
  // unfortunately, in SDL1.2 there is no way to get the keycode for a scancode,
  // so this doesn't work there. so this is SDL2-only.
#if SDL_VERSION_ATLEAST(2, 0, 0)
