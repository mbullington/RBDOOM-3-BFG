/*
================================================================================================
CONFIDENTIAL AND PROPRIETARY INFORMATION/NOT FOR DISCLOSURE WITHOUT WRITTEN
PERMISSION Copyright 2010 id Software LLC, a ZeniMax Media company. All Rights
Reserved.
================================================================================================
*/
...
class idSessionLocalWin : public idSessionLocal {
 ...
 public:
  ...
  virtual void LeaderboardUpload(lobbyUserID_t lobbyUserID,
                                 ...
                                 const idFile_Memory* attachment = NULL);
...
class idLobbyToSessionCBLocal : public idLobbyToSessionCB {
 public:
  ...
  virtual class idLobbyBackend* GetLobbyBackend(
      idLobbyBackend::lobbyBackendType_t type) const {
...
class idVoiceChatMgrWin : public idVoiceChatMgr {
 public:
  virtual bool GetLocalChatDataInternal(int talkerIndex, byte* data,
                                        int& dataSize) {
    return false;
  }
  virtual void SubmitIncomingChatDataInternal(int talkerIndex, const byte* data,
                                              int dataSize) {}
...
========================
*/
int idSessionLocalWin::NumServers() const { return 0; }
...
========================
*/
const serverInfo_t* idSessionLocalWin::ServerInfo(int i) const { return NULL; }
...
void idSessionLocalWin::Connect_f(const idCmdArgs& args) {
  if (args.Argc() < 2) {
    idLib::Printf(
        "Usage: Connect to IP. Use IP:Port to specify port (e.g. "
        "10.0.0.1:1234) \n");
    return;
  }
...
bool idSessionLocalWin::IsSystemUIShowing() const {
  // DG: pausing here when window is out of focus like originally done on
  // windows is hacky it's done with com_pause now.
...
========================
*/
void idSessionLocalWin::SetSystemUIShowing(bool show) { isSysUIShowing = show; }
...
========================
*/
void idSessionLocalWin::HandleServerQueryRequest(lobbyAddress_t& remoteAddr,
                                                 idBitMsg& msg, int msgType) {
...
========================
*/
void idSessionLocalWin::HandleServerQueryAck(lobbyAddress_t& remoteAddr,
                                             idBitMsg& msg) {
...
========================
*/
bool idSessionLocalWin::HasPendingBootableInvite() { return false; }
...
========================
*/
void* idSessionLocalWin::GetDiscSwapMPInviteParms() { return NULL; }
...
========================
*/
void idSessionLocalWin::LeaderboardUpload(
    lobbyUserID_t lobbyUserID, const leaderboardDefinition_t* leaderboard,
    const column_t* stats, const idFile_Memory* attachment) {}
...
========================
*/
void idSessionLocalWin::LeaderboardDownload(
    int sessionUserIndex, const leaderboardDefinition_t* leaderboard,
    int startingRank, int numRows, const idLeaderboardCallback& callback) {}
...
========================
*/
void idSessionLocalWin::LeaderboardDownloadAttachment(
    int sessionUserIndex, const leaderboardDefinition_t* leaderboard,
    int64 attachmentID) {}
...
void idSessionLocalWin::EnsurePort() {
  ...
  if (port.InitPort(net_port.GetInteger(), false)) {
    // TODO: what about canJoinLocalHost when running two instances with
    // different net_port values?
    canJoinLocalHost = false;
  } else {
...
void idSessionLocalWin::PumpLobbies() {
  assert(lobbyBackends[idLobbyBackend::TYPE_PARTY] == NULL ||
         lobbyBackends[idLobbyBackend::TYPE_PARTY]->GetLobbyType() ==
             idLobbyBackend::TYPE_PARTY);
  assert(lobbyBackends[idLobbyBackend::TYPE_GAME] == NULL ||
         lobbyBackends[idLobbyBackend::TYPE_GAME]->GetLobbyType() ==
             idLobbyBackend::TYPE_GAME);
  assert(lobbyBackends[idLobbyBackend::TYPE_GAME_STATE] == NULL ||
...
========================
*/
bool idSessionLocalWin::GetLobbyAddressFromNetAddress(
    const netadr_t& netAddr, lobbyAddress_t& outAddr) const {
  return false;
}
...
========================
*/
bool idSessionLocalWin::GetNetAddressFromLobbyAddress(
    const lobbyAddress_t& lobbyAddress, netadr_t& outNetAddr) const {
  return false;
}
