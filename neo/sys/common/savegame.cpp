...
void Sys_ExecuteSavegameCommandAsyncImpl(idSaveLoadParms* savegameParms) {
  ...
  session->GetSaveGameManager().GetSaveGameThread().data.saveLoadParms =
      savegameParms;

  if (session->GetSaveGameManager().GetSaveGameThread().GetThreadHandle() ==
      0) {
    session->GetSaveGameManager().GetSaveGameThread().StartWorkerThread(
        "Savegame", CORE_ANY);
  }
...
idLocalUserWin* GetLocalUserFromSaveParms(const saveGameThreadArgs_t& data) {
  if ((data.saveLoadParms != NULL) &&
      (data.saveLoadParms->inputDeviceId >= 0)) {
    idLocalUser* user = session->GetSignInManager().GetLocalUserByInputDevice(
        data.saveLoadParms->inputDeviceId);
    if (user != NULL) {
      idLocalUserWin* userWin = static_cast<idLocalUserWin*>(user);
      if (userWin != NULL &&
          data.saveLoadParms->userId == idStr::Hash(userWin->GetGamerTag())) {
        return userWin;
      }
    }
  }
...
int idSaveGameThread::Save() {
  ...
  int64 requiredSizeBytes = 0;
  {
    for (int i = 0; i < callback->files.Num(); i++) {
      ...
      requiredSizeBytes +=
          (file->Length() + sizeof(unsigned int));  // uint for checksum
  ...
  {
          user->StorageSizeAvailable( requiredSizeBytes,
  callback->requiredSpaceInBytes ); if( callback->requiredSpaceInBytes > 0 )
          {
                  // check to make sure savepath actually exists before erroring
                  idStr directory = fs_savepath.GetString();
                  directory += "\\";	// so it doesn't think the last part is
  a file and ignores in the directory creation fileSystem->CreateOSPath(
  directory );  // we can't actually check FileExists in production builds, so
  just try to create it user->StorageSizeAvailable( requiredSizeBytes,
  callback->requiredSpaceInBytes );
  ...
  */
  ...
  // Save the raw files.
  for (int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS &&
                  !callback->cancelled;
       i++) {
    idFile_SaveGame* file = callback->files[i];
    ...
    if (outputFile == NULL) {
               ...
               // with strerror(errno)
      idLib::Warning("[%s]: Couldn't open file for writing, %s. Error = %08x",
                     __FUNCTION__, tempFileName.c_str(), GetLastError());
#else
      idLib::Warning("[%s]: Couldn't open file for writing, %s. Error = %s",
                     __FUNCTION__, tempFileName.c_str(), strerror(errno));
#endif  // DG end
      file->error = true;
    ...
    if ((file->type & SAVEGAMEFILE_PIPELINED) != 0) {
      idFile_SaveGamePipelined* inputFile =
          dynamic_cast<idFile_SaveGamePipelined*>(file);
...
#ifdef _WIN32  // DG: unify windows and posix savegames => replace GetLastError
               // with strerror(errno)
          idLib::Warning("[%s]: Write failed. Error = %08x", __FUNCTION__,
                         GetLastError());
#else
          idLib::Warning("[%s]: Write failed. Error = %s", __FUNCTION__,
                         strerror(errno));
#endif  // DG end
          file->error = true;
          callback->errorCode = SAVEGAME_E_INSUFFICIENT_ROOM;
      ...
      if ((file->type & SAVEGAMEFILE_BINARY) ||
    ...
    if (ret == ERROR_SUCCESS) {
      ...
      if (!fileSystem->RenameFile(tempFileName, fileName, "fs_savePath")) {
        idLib::Warning("Could not start to rename temporary file %s to %s.",
                       tempFileName.c_str(), fileName.c_str());
      }
    } else {
      fileSystem->RemoveFile(tempFileName);
      idLib::Warning("Invalid write to temporary file %s.",
                     tempFileName.c_str());
    }
  }
...
int idSaveGameThread::Load() {
  ...
  for (int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS &&
    ...
    if ((file->type & SAVEGAMEFILE_PIPELINED) != 0) {
      idFile_SaveGamePipelined* outputFile =
          dynamic_cast<idFile_SaveGamePipelined*>(file);
      assert(outputFile != NULL);

      size_t lastReadBytes = 0;
      blockForIO_t block;
      while (outputFile->NextReadBlock(&block, lastReadBytes) &&
             !callback->cancelled) {
        lastReadBytes = inputFile->Read(block.data, block.bytes);
        if (lastReadBytes != block.bytes) {
          // Notify end-of-file to the save game file which will cause all reads
          // on the other end of the pipeline to return zero bytes after the
          // pipeline is drained.
      ...
      if ((file->type & SAVEGAMEFILE_BINARY) != 0 ||
...
int idSaveGameThread::Enumerate() {
  ...
  if (fileSystem->IsFolder(saveFolder, "fs_savePath") == FOLDER_YES) {
    idFileList* files =
        fileSystem->ListFilesTree(saveFolder, SAVEGAME_DETAILS_FILENAME);
    ...
    for (int i = 0; i < fileList.Num() && !callback->cancelled; i++) {
      ...
      // We have more folders on disk than we have room in our save detail list,
      // stop trying to read them in and continue with what we have
      if (details == NULL) {
        break;
      }
      ...
      if (file != NULL) {
        ...
        if (attrRet == TRUE) {
          ...
          if (success) {
            ...
            // Hard coded number of 100-nanosecond units from 1/1/1601 to
            // 1/1/1970
...
#else
        // DG: just use the idFile object's timestamp - the windows code gets
        // file attributes and
        //  other complicated stuff like that.. I'm wonderin what that was good
        //  for.. this seems to work.
...
int idSaveGameThread::EnumerateFiles() {
  ...
  if (fileSystem->IsFolder(folder, "fs_savePath") == FOLDER_YES) {
    ...
    for (int i = 0;
         i < files->GetNumFiles() && ret == 0 && !callback->cancelled; i++) {
      ...
      if (!callback->postPattern.IsEmpty() &&
          filename.Right(callback->postPattern.Length())
                  .IcmpPrefix(callback->postPattern) != 0) {
        continue;
      }
      ...
      if (callback->mode & SAVEGAME_MBF_READ_DETAILS) {
        ...
        assert(fileSystem->IsFolder(details.slotName, "fs_savePath") ==
               FOLDER_YES);
      }

      idFile_SaveGame* file = new (TAG_SAVEGAMES)
          idFile_SaveGame(filename, SAVEGAMEFILE_AUTO_DELETE);
...
int idSaveGameThread::DeleteFiles() {
  ...
  if (fileSystem->IsFolder(folder, "fs_savePath") == FOLDER_YES) {
    ...
    for (int i = 0; i < files->GetNumFiles() && !callback->cancelled; i++) {
      ...
      // If there are post/pre patterns to match, make sure we adhere to the
      // patterns
      if (callback->pattern.IsEmpty() ||
          (filename.IcmpPrefix(callback->pattern) != 0)) {
        continue;
      }
      if (callback->postPattern.IsEmpty() ||
          (filename.Right(callback->postPattern.Length())
...
int idSaveGameThread::DeleteAll() {
  ...
  if (fileSystem->IsFolder(saveFolder, "fs_savePath") == FOLDER_YES) {
    ...
    for (int i = 0; i < files->GetNumFiles() && !callback->cancelled; i++) {
      ...
      if (fileSystem->IsFolder(files->GetFile(i), "fs_savePath") ==
          FOLDER_YES) {
...
int idSaveGameThread::Run() {
  ...
  try {
    ...
    // if anything horrible happens, leave it up to the savegame processors to
    // handle in PostProcess().
    data.saveLoadParms->errorCode = SAVEGAME_E_UNKNOWN;
  }
  ...
  if (savegame_error.GetInteger() != 0) {
    data.saveLoadParms->errorCode =
        (saveGameError_t)savegame_error.GetInteger();
  }
...
void Sys_SaveGameCheck(bool& exists, bool& autosaveExists) {
  ...
  const idStr autosaveFolderStr = AddSaveFolderPrefix(
      SAVEGAME_AUTOSAVE_FOLDER, idSaveGameManager::PACKAGE_GAME);
