...
/*
================================================================================================
Contains the NetworkSystem implementation for SOCKET APIs, currently POSIX
Systems (Linux, FreeBSD, OSX, ...) and Winsock (Windows...) are supported. Note
that other POSIX systems may need some small changes, e.g. in
Sys_InitNetworking()
================================================================================================
*/
...
#include <iptypes.h>
#include <iphlpapi.h>
// force these libs to be included, so users of idLib don't need to add them to
// every project
...
Some #defines and platform specific things etc to abstract differences between
Winsocks and real POSIX sockets away..
...
static int Net_GetLastError() { return errno; }
...
========================
*/
static bool Net_ExtractPort(const char* src, char* buf, int bufsize,
                            int* port) {
  char* p;
...
static bool Net_StringToSockaddr(const char* s, sockaddr_in* sadr,
   ...
   * 1. domains can start with numbers nowadays so the old heuristic to find out
   * if it's an IP (check if the first char is a digit) isn't reliable
   * 2. gethostbyname() works fine for IPs and doesn't do a lookup if the passed
   * string is an IP
   */
  struct hostent* h;
  char buf[256];
  ...
  // try to remove the port first, otherwise the DNS gets confused into multiple
  // timeouts failed or not failed, buf is expected to contain the appropriate
  // host to resolve
...
int NET_IPSocket(const char* bind_ip, int port, netadr_t* bound_to) {
  ...
  if (fcntl(newsocket, F_SETFL, flags) < 0) {
    idLib::Printf(
        "WARNING: UDP_OpenSocket: fcntl F_SETFL with O_NONBLOCK: %s\n",
        NET_ErrorString());
    closesocket(newsocket);
    return 0;
  }
  ...
  if (bind(newsocket, (const sockaddr*)&address, sizeof(address)) ==
      SOCKET_ERROR) {
  ...
  // got bound to ( this used to be in idUDP::InitForPort )
  if (bound_to) {
    socklen_t len = sizeof(address);
    if (getsockname(newsocket, (struct sockaddr*)&address, &len) ==
        SOCKET_ERROR) {
...
void NET_OpenSocks(int port) {
  ...
  if (connect(socks_socket, (sockaddr*)&address, sizeof(address)) ==
      SOCKET_ERROR) {
...
bool Net_WaitForData(int netSocket, int timeout) {
  ...
  FD_ZERO(&set);
  FD_SET(netSocket,
         &set);  // TODO: winsocks may want an unsigned int for netSocket?
...
========================
*/
bool Net_GetUDPPacket(int netSocket, netadr_t& net_from, char* data, int& size,
                      int maxSize) {
  int ret;
  sockaddr_in from;
  socklen_t fromlen;
...
========================
*/
void Net_SendUDPPacket(int netSocket, int length, const void* data,
                       const netadr_t to) {
  int ret;
  sockaddr_in addr;
  ...
  if (usingSocks && to.type == NA_IP) {
    ...
    *(short*)&socksBuf[8] = addr.sin_port;
    ...
    ret = sendto(netSocket, socksBuf, length + 10, 0,
                 (sockaddr*)&socksRelayAddr, sizeof(socksRelayAddr));
  } else {
    ret = sendto(netSocket, (const char*)data, length, 0, (sockaddr*)&addr,
                 sizeof(addr));
  }
  if (ret == SOCKET_ERROR) {
    ...
    // but that means the packet will be dropped so I don't feel it's a good
    // thing to ignore
...
static void ip_to_addr(const char ip[4], char* addr) {
  idStr::snPrintf(addr, 16, "%d.%d.%d.%d", (unsigned char)ip[0],
...
void Sys_InitNetworking() {
...
#if defined(_WIN32)  // DG: add win32 stuff here for socket networking code
                     // unification..
  ...
  if (!pAdapterInfo) {
    idLib::FatalError("Sys_InitNetworking: Couldn't malloc( %d )",
  ...
  if (GetAdaptersInfo(pAdapterInfo, &ulOutBufLen) == ERROR_BUFFER_OVERFLOW) {
    ...
    if (!pAdapterInfo) {
      idLib::FatalError("Sys_InitNetworking: Couldn't malloc( %ld )",
                        ulOutBufLen);
    }
  }
  ...
  if ((dwRetVal = GetAdaptersInfo(pAdapterInfo, &ulOutBufLen)) != NO_ERROR) {
    ...
    while (pAdapter) {
      idLib::Printf("Found interface: %s %s - ", pAdapter->AdapterName,
      ...
      while (pIPAddrString) {
        ...
        idStr::Copynz(netint[num_interfaces].addr,
                      pIPAddrString->IpAddress.String,
  ...
  for (ifp = ifap; ifp; ifp = ifp->ifa_next) {
    ...
    if (ip == INADDR_LOOPBACK) {
      ...
      common->Printf("%d.%d.%d.%d", (unsigned char)ifp->ifa_addr->sa_data[2],
  ...
  // and because I'm extra-cautious I add this check on real operating systems
  // as well :)
...
const char* Sys_NetAdrToString(const netadr_t a) {
  // DG: FIXME: those static buffers look fishy - I would feel better if they
  // were
...
bool Sys_IsLANAddress(const netadr_t adr) {
  ...
  // that are connected through a router (i.e. no IP from that net is on any
  // interface) However, I don't expect most people to have such setups at home
  // and the code would get a lot more complex and less portable. Furthermore,
  // this function isn't even used currently
  if (num_interfaces) {
    int i;
...
bool Sys_CompareNetAdrBase(const netadr_t a, const netadr_t b) {
  ...
  if (a.type == NA_LOOPBACK) {
    // DG: wtf is this comparison about, the comment above says "without the
    // port"
    if (a.port == b.port) {
      return true;
    }
...
========================
*/
int Sys_GetLocalIPCount() { return num_interfaces; }
...
bool idUDP::GetPacket(netadr_t& from, void* data, int& size, int maxSize) {
  // DG: this fake while(1) loop pissed me off so I replaced it.. no functional
  // change.
...
========================
*/
bool idUDP::GetPacketBlocking(netadr_t& from, void* data, int& size,
