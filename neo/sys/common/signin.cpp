...
#endif
#ifdef _DEBUG
idCVar win_userPersistent("win_userPersistent", "1", CVAR_BOOL,
                          "debugging cvar for profile persistence status");
idCVar win_userOnline("win_userOnline", "1", CVAR_BOOL,
                      "debugging cvar for profile online status");
idCVar win_isInParty("win_isInParty", "0", CVAR_BOOL,
                     "debugging cvar for platform party status");
idCVar win_partyCount("win_partyCount", "0", CVAR_INTEGER,
                      "debugginc var for platform party count");
#endif

// DG: D3BFG got the username from steam, in the GPL release it just uses the
// hostname.
//     this adds a name to set a player name
idCVar ui_name(
    "ui_name", "", CVAR_ARCHIVE,
    "player name - leave empty for default name (system's hostname)");
// DG end
...
void idSignInManagerWin::RegisterLocalUser(int inputDevice) {
  ...
  if (idStr::IsValidUTF8(nameSource, nameLength)) {
    ...
    while (nameIndex < nameLength &&
           numChars++ < idLocalUserWin::MAX_GAMERTAG_CHARS) {
  ...
  localUser.SetLocalUserHandle(
      GetUniqueLocalUserHandle(localUser.GetGamerTag()));
...
bool idSignInManagerWin::CreateNewUser(winUserState_t& state) {
  // idScopedGlobalHeap	everythingHereGoesInTheGlobalHeap;	// users
  // obviously persist across maps
...
CONSOLE_COMMAND(testRemoveAllLocalUsers,
                "Forces removal of local users - mainly for PC testing", NULL) {
