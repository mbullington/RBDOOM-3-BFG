...
========================
*/
void idLocalUserWin::Init(int inputDevice_, const char* gamertag_,
                          int numLocalUsers) {
  if (numLocalUsers ==
      1)  // Check for 1, since this is now incremented before we get in here
  {
    // This is the master user
    gamertag = gamertag_;
  } else {
    // On steam, we need to generate a name based off the master user for
    // split-screen users. We use the number of users on the system to generate
    // the name rather than the device number so that it is always consistently
    // "username (2)" for the second player.
