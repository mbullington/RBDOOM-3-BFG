...
idCVar ui_autoSwitch("ui_autoSwitch", "1", CVAR_ARCHIVE | CVAR_BOOL,
                     "auto switch weapon");
idCVar ui_autoReload("ui_autoReload", "1", CVAR_ARCHIVE | CVAR_BOOL,
                     "auto reload weapon");

idCVar net_maxSearchResults(
    "net_maxSearchResults", "25", CVAR_INTEGER,
    "Max results that are allowed to be returned in a search request");
idCVar net_maxSearchResultsToTry(
    "net_maxSearchResultsToTry", "5", CVAR_INTEGER,
    "Max results to try before giving up.");  // At 15 second timeouts per, 1
                                              // min 15 worth of connecting
                                              // attempt time

idCVar net_LobbyCoalesceTimeInSeconds(
    "net_LobbyCoalesceTimeInSeconds", "30", CVAR_INTEGER,
    "Time in seconds when a lobby will try to coalesce with another lobby when "
    "there is only one user.");
idCVar net_LobbyRandomCoalesceTimeInSeconds(
    "net_LobbyRandomCoalesceTimeInSeconds", "3", CVAR_INTEGER,
...
idCVar net_ignoreTitleStorage("net_ignoreTitleStorage", "0", CVAR_BOOL,
                              "Ignore title storage");
#endif

idCVar net_maxLoadResourcesTimeInSeconds(
    "net_maxLoadResourcesTimeInSeconds", "0", CVAR_INTEGER,
    "How long, in seconds, clients have to load resources. Used for loose "
    "asset builds.");
idCVar net_migrateHost(
    "net_migrateHost", "-1", CVAR_INTEGER,
    "Become host of session (0 = party, 1 = game) for testing purposes");
extern idCVar net_debugBaseStates;

idCVar net_testPartyMemberConnectFail(
    "net_testPartyMemberConnectFail", "-1", CVAR_INTEGER,
    "Force this party member index to fail to connect to games.");

// FIXME: this could use a better name.
idCVar net_offlineTransitionThreshold(
    "net_offlineTransitionThreshold", "1000", CVAR_INTEGER,
    "Time, in milliseconds, to wait before kicking back to the main menu when "
    "a profile losses backend connection during an online game");

idCVar net_port(
    "net_port", "27015", CVAR_INTEGER | CVAR_NOCHEAT,
...
idSessionLocal::idSessionLocal()
    : processorSaveFiles(new (TAG_SAVEGAMES) idSaveGameProcessorSaveFiles),
      ...
      processorEnumerate(new (TAG_SAVEGAMES)
...
*/
idSessionLocal::~idSessionLocal() {
  // foresthale 2014-06-08: check before deleting these, the deletes were likely
  // done in Shutdown already
...
void idSessionLocal::FindOrCreateMatch(const idMatchParameters& p) {
  ...
  if ((p.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER) &&
      !GetPartyLobby().IsLobbyActive()) {
    NET_VERBOSE_PRINT(
        "NET: FindOrCreateMatch MATCH_PARTY_INVITE_PLACEHOLDER\n");
    CreatePartyLobby(p);
...
void idSessionLocal::StartLoading() {
  ...
  if (MatchTypeIsOnline(GetActingGameStateLobby().parms.matchFlags)) {
    if (!GetActingGameStateLobby().IsHost()) {
      idLib::Warning(
          "Ignoring call to StartLoading because we are not the host.  state "
          "is %s",
          stateToString[localState]);
      return;
    }
    ...
    for (int p = 0; p < GetActingGameStateLobby().peers.Num(); p++) {
      if (GetActingGameStateLobby().peers[p].IsConnected()) {
        GetActingGameStateLobby().QueueReliableMessage(
            p, idLobby::RELIABLE_START_LOADING);
        GetActingGameStateLobby().peers[p].startResourceLoadTime =
            Sys_Milliseconds();
      }
    }
  }
...
void idSessionLocal::StartMatch() {
  ...
  if (localState != STATE_GAME_LOBBY_HOST) {
    idLib::Warning(
        "idSessionLocal::StartMatch called when not hosting game lobby");
    return;
  }
  ...
  GetGameLobby()
      .migrationInfo.persistUntilGameEndsData.hasRelaunchedMigratedGame = true;

  // Clear snap ack queue between games
  GetGameLobby().snapDeltaAckQueue.Clear();

  extern idCVar net_bw_challenge_enable;
  if (session->GetTitleStorageBool("net_bw_challenge_enable",
                                   net_bw_challenge_enable.GetBool()) &&
      GetGameLobby().HasActivePeers()) {
...
idSessionLocal::sessionState_t idSessionLocal::GetBackState() {
  ...
  const bool isInGame =
      currentState == INGAME ||
      currentState == LOADING;  // Counting loading as ingame as far as what
                                // back state to go to
  ...
  if (!isInPartyLobby && isInGameLobby && ShouldHavePartyLobby()) {
    return PARTY_LOBBY;  // If in the game lobby, and we should have a party
                         // lobby, and we are the host, go back to party lobby
  }
    ...
    case PARTY_LOBBY:
      if (GetPartyLobby().IsHost()) {
        if (sessionOptions & OPTION_LEAVE_WITH_PARTY) {
          ...
          // so it won't be affected by the fact that we're shutting down the
          // game lobby
...
bool idSessionLocal::ShouldRelaunchMigrationGame() const {
  return GetGameLobby().ShouldRelaunchMigrationGame() &&
         !IsCurrentLobbyMigrating();
}
...
========================
*/
bool idSessionLocal::GetMigrationGameDataUser(lobbyUserID_t lobbyUserID,
                                              idBitMsg& msg, bool reading) {
...
Updates the party parameters when in a party lobby OR a game lobby in order to
keep them always in sync.
========================
*/
void idSessionLocal::UpdatePartyParms(const idMatchParameters& p) {
  if ((GetState() != PARTY_LOBBY && GetState() != GAME_LOBBY) ||
      !GetPartyLobby().IsHost()) {
    return;
  }
...
void idSessionLocal::SetLobbiesAreJoinable(bool joinable) {
  // NOTE - We don't manipulate the joinable state when we are supporting join
  // in progress Lobbies will naturally be non searchable when there are no free
  // slots
  if (GetPartyLobby().lobbyBackend != NULL &&
      !MatchTypeIsJoinInProgress(GetPartyLobby().parms.matchFlags)) {
    NET_VERBOSE_PRINT("Party lobbyBackend SetIsJoinable: %d\n", joinable);
    GetPartyLobby().lobbyBackend->SetIsJoinable(joinable);
  }

  if (GetGameLobby().lobbyBackend != NULL &&
      !MatchTypeIsJoinInProgress(GetGameLobby().parms.matchFlags)) {
...
void idSessionLocal::HandleVoiceRestrictionDialog() {
  // don't bother complaining about voice restrictions when in a splitscreen
  // lobby
  if (MatchTypeIsLocal(GetActivePlatformLobby()->parms.matchFlags)) {
    return;
  }

  // Pop a dialog up the first time we are in a lobby and have voice chat
  // restrictions due to account privileges
  if (voiceChat != NULL && voiceChat->IsRestrictedByPrivleges() &&
      !hasShownVoiceRestrictionDialog) {
    common->Dialog().AddDialog(GDM_VOICE_RESTRICTED, DIALOG_ACCEPT, NULL, NULL,
                               false);
...
bool idSessionLocal::WaitOnLobbyCreate(idLobby& lobby) {
  assert(localState == STATE_CREATE_AND_MOVE_TO_PARTY_LOBBY ||
         localState == STATE_CREATE_AND_MOVE_TO_GAME_LOBBY ||
         localState == STATE_CREATE_AND_MOVE_TO_GAME_STATE_LOBBY);
  ...
  if (lobby.GetState() == idLobby::STATE_FAILED) {
    ...
    common->Dialog().AddDialog(GDM_CONNECTION_LOST, DIALOG_ACCEPT, NULL, NULL,
                               true, "", 0, true);
    return false;
  }
...
bool idSessionLocal::DetectDisconnectFromService(bool cancelAndShowMsg) {
  const int DETECT_SERVICE_DISCONNECT_TIMEOUT_IN_SECONDS =
      session->GetTitleStorageInt(
          "DETECT_SERVICE_DISCONNECT_TIMEOUT_IN_SECONDS", 30);
...
void idSessionLocal::HandleConnectionFailed(idLobby& lobby, bool wasFull) {
  assert(localState == STATE_CONNECT_AND_MOVE_TO_PARTY ||
         localState == STATE_CONNECT_AND_MOVE_TO_GAME ||
         localState == STATE_CONNECT_AND_MOVE_TO_GAME_STATE);
  assert(connectType == CONNECT_FIND_OR_CREATE ||
         connectType == CONNECT_DIRECT);
  bool canPlayOnline = true;

  // Check for online status (this is only a problem on the PS3 at the moment.
  // The 360 LIVE system handles this for us
  ...
  if (connectType == CONNECT_FIND_OR_CREATE) {
    ...
    if (!lobby.ConnectToNextSearchResult()) {
      CreateMatch(
          GetGameLobby()
              .parms);  // Assume any time we are connecting to a game lobby, it
                        // is from a FindOrCreateMatch call, so create a match
    }
  } else if (connectType == CONNECT_DIRECT) {
    if (localState == STATE_CONNECT_AND_MOVE_TO_GAME &&
        GetPartyLobby().IsPeer()) {
      int flags = GetPartyLobby().parms.matchFlags;

      if (MatchTypeIsOnline(flags) && (flags & MATCH_REQUIRE_PARTY_LOBBY) &&
          ((flags & MATCH_PARTY_INVITE_PLACEHOLDER) == 0)) {
        // We get here when our party host told us to connect to a game, but the
        // game didn't exist. Just drop back to the party lobby and wait for
        // further orders.
    ...
    if (wasFull) {
      common->Dialog().AddDialog(GDM_LOBBY_FULL, DIALOG_ACCEPT, NULL, NULL,
                                 false);
    } else if (!canPlayOnline) {
      common->Dialog().AddDialog(GDM_PLAY_ONLINE_NO_PROFILE, DIALOG_ACCEPT,
                                 NULL, NULL, false);
    } else {
      // TEMP HACK: We detect the steam lobby is full in idLobbyBackendWin, and
      // then STATE_FAILED, which brings us here. Need to find a way to notify
...
bool idSessionLocal::HandleConnectAndMoveToLobby(idLobby& lobby) {
  ...
  if (lobby.lobbyType == idLobby::TYPE_GAME) {
    if (GetPartyLobby().IsHost()) {
      ...
      const int timeoutMs = session->GetTitleStorageInt(
      ...
      for (int i = 0; i < GetPartyLobby().peers.Num(); i++) {
        if (GetPartyLobby().peers[i].IsConnected()) {
          GetPartyLobby().QueueReliableMessage(
              i, idLobby::RELIABLE_PARTY_CONNECT_OK);
        }
      }
    } else {
      ...
      if (GetGameLobby().waitForPartyOk) {
        ...
        if (timeoutMs != 0 &&
            Sys_Milliseconds() - lobby.helloStartTime > timeoutMs) {
          GetGameLobby().waitForPartyOk =
              false;  // Just connect to this game lobby if we haven't heard
                      // from the party host for the entire timeout duration
        }
      }

      if (GetGameLobby().waitForPartyOk) {
        return HandlePackets();  // Waiting on party host to tell us everyone
                                 // made it
      }
    }
  }
...
bool idSessionLocal::State_Create_And_Move_To_Party_Lobby() {
  if (WaitOnLobbyCreate(GetPartyLobby())) {
    if (GetPartyLobby().parms.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER) {
      ...
      // finding or creating a game lobby
      if (connectType == CONNECT_FIND_OR_CREATE) {
...
bool idSessionLocal::State_Create_And_Move_To_Game_Lobby() {
  if (WaitOnLobbyCreate(GetGameLobby())) {
    ...
    // NOTE - We pass in false to wait on party members since we are the host,
    // and we know they can connect to us
...
bool idSessionLocal::State_Create_And_Move_To_Game_State_Lobby() {
  if (WaitOnLobbyCreate(GetGameStateLobby())) {
    ...
    // Now that we've created our game state lobby, send our own game users to
    // it NOTE - We pass in false to wait on party members since we are the
    // host, and we know they can connect to us
    GetGameLobby().SendMembersToLobby(GetGameStateLobby(), false);

    // If we are the host of a game lobby, we know we are not using dedicated
    // servers, so we want to start the match immediately as soon as we detect
...
bool idSessionLocal::State_Loading() {
  ...
  if (GetActingGameStateLobby().IsHost()) {
    ...
    for (int p = 0; p < GetActingGameStateLobby().peers.Num(); p++) {
      ...
      if (GetActingGameStateLobby().SendResources(p)) {
        ...
        // if client is taking a LONG time to load up - give them the boot:
        // they're just holding up the lunch line. Useful for loose assets
        // playtesting.
        int time = Sys_Milliseconds();
        int maxLoadTime = net_maxLoadResourcesTimeInSeconds.GetInteger();
        if (maxLoadTime > 0 &&
            peer.startResourceLoadTime + SEC2MS(maxLoadTime) < time) {
          NET_VERBOSERESOURCE_PRINT(
              "NET: dropping client %i - %s because they took too long to load "
              "resources.\n Check 'net_maxLoadResourcesTimeInSeconds' to "
  ...
  SetState(STATE_INGAME);  // NOTE - Only the host is in-game at this point, all
                           // peers will start becoming in-game when they
                           // receive their first full snap
  return true;
}
...
bool idSessionLocal::State_Busy() {
  ...
  if (activeLobby == NULL) {
    idLib::Warning(
        "No active session lobby when idSessionLocal::State_Busy called");
    return false;
  }
...
void idSessionLocal::VerifySnapshotInitialState() {
  ...
  for (int p = 0; p < GetActingGameStateLobby().peers.Num(); p++) {
    ...
    if (!verify(GetActingGameStateLobby().peers[p].needToSubmitPendingSnap ==
                false)) {
      idLib::Error("Invalid needToSubmitPendingSnap state\n");
    }
    if (!verify(GetActingGameStateLobby().peers[p].snapProc->HasPendingSnap() ==
                false)) {
      idLib::Error("Invalid HasPendingSnap state\n");
    }
    if (!verify(
            GetActingGameStateLobby().peers[p].snapProc->GetSnapSequence() ==
            idSnapshotProcessor::INITIAL_SNAP_SEQUENCE)) {
      idLib::Error(
          "Invalid INITIAL_SNAP_SEQUENCE state %d for peer %d \n",
          GetActingGameStateLobby().peers[p].snapProc->GetSnapSequence(), p);
    }
    if (!verify(
            GetActingGameStateLobby().peers[p].snapProc->GetBaseSequence() ==
            -1)) {
...
bool idSessionLocal::State_Game_State_Lobby_Host() {
  ...
  if (waitingOnGameStateMembersToLeaveTime != 0) {
    ...
    const bool forceDisconnectMembers =
        (Sys_Milliseconds() - waitingOnGameStateMembersToLeaveTime) >
        MAX_LEAVE_WAIT_TIME_IN_SECONDS * 1000;

    // Check to see if all peers have finally left
    if (GetGameStateLobby().GetNumConnectedPeers() == 0 ||
        forceDisconnectMembers) {
      //
      ...
      if (GetGameLobby().IsHost()) {
        // If we aren't a dedicated game state host, then drop back to the game
        // lobby as host
        GetGameStateLobby().Shutdown();
        SetState(STATE_GAME_LOBBY_HOST);
      } else {
        // A dedicated game state host will remain in
        // State_Game_State_Lobby_Host mode while waiting for another set of
        // users to join DEDICATED_SERVER_FIXME: Notify master server we can
        // server another game now
        GetGameStateLobby().DisconnectAllPeers();
      }
    }
  } else {
    // When all the players from the game lobby are in the game state lobby,
    // StartLoading
    if (GetGameLobby().IsHost()) {
      if (GetGameStateLobby().GetNumLobbyUsers() ==
          GetGameLobby().GetNumLobbyUsers()) {
        waitingOnGameStateMembersToJoinTime = 0;
        StartLoading();
      }
    } else {
      // The dedicated server host relies on the game host to say when all users
      // are in
...
bool idSessionLocal::State_Game_Lobby_Peer() {
  ...
  if (profile != NULL &&
      (profile->GetState() == idPlayerProfile::SAVING ||
       profile->GetRequestedState() == idPlayerProfile::SAVE_REQUESTED)) {
    saving = true;
  }
...
bool idSessionLocal::State_Game_State_Lobby_Peer() {
  ...
  if (GetGameLobby().IsHost() && waitingOnGameStateMembersToJoinTime != 0) {
    ...
    for (int i = 0; i < GetGameLobby().GetNumLobbyUsers(); i++) {
      if (GetGameStateLobby().GetLobbyUserByID(
              GetGameLobby().GetLobbyUser(i)->lobbyUserID, true) != NULL) {
        foundMembers++;
      }
    }

    // Give all of our game members 10 seconds to join, otherwise start without
    // them
    const int MAX_JOIN_WAIT_TIME_IN_SECONDS = 10;

    const bool forceStart =
        (Sys_Milliseconds() - waitingOnGameStateMembersToJoinTime) >
        MAX_JOIN_WAIT_TIME_IN_SECONDS * 1000;
    ...
    if (foundMembers == GetGameLobby().GetNumLobbyUsers() || forceStart) {
      ...
      GetGameStateLobby().QueueReliableMessage(
          GetGameStateLobby().host,
          idLobby::RELIABLE_START_MATCH_GAME_LOBBY_HOST, msg.GetReadData(),
          msg.GetSize());
...
idCVar net_verbose("net_verbose", "0", CVAR_BOOL | CVAR_NOCHEAT,
                   "Print a bunch of message about the network session");
idCVar net_verboseResource("net_verboseResource", "0", CVAR_BOOL,
                           "Prints a bunch of message about network resources");
idCVar net_verboseReliable(
    "net_verboseReliable", "0", CVAR_BOOL,
...
idCVar net_forceLatency(
    "net_forceLatency", "0", CVAR_INTEGER,
    "Simulate network latency (milliseconds round trip time - applied equally "
    "on the receive and on the send)");
idCVar net_forceDrop("net_forceDrop", "0", CVAR_INTEGER,
                     "Percentage chance of simulated network packet loss");
idCVar net_forceUpstream("net_forceUpstream", "0", CVAR_FLOAT,
                         "Force a maximum upstream in kB/s (256kbps <-> "
                         "32kB/s)");  // I would much rather deal in kbps but
                                      // most of the code is written in bytes ..
idCVar net_forceUpstreamQueue(
    "net_forceUpstreamQueue", "64", CVAR_INTEGER,
    "How much data is queued when enforcing upstream (in kB)");
idCVar net_verboseSimulatedTraffic(
    "net_verboseSimulatedTraffic", "0", CVAR_BOOL,
...
void idSessionLocal::Shutdown() {
  // foresthale 2014-05-28: shut down saveGameManager early because the thread
  // it owns will be terminated before the dtor is reached
  if (signInManager) {
    delete signInManager;
  }
  signInManager = NULL;
  ...
  dedicatedServerSearch = NULL;
  // foresthale 2014-06-08: delete these before we get to the dtor to prevent
  // doexit crashes
  if (processorSaveFiles) {
    delete processorSaveFiles;
    processorSaveFiles = NULL;
...
/*
========================
idSession interface semi-common between platforms (#ifdef's in
sys_session_local.cpp)
========================
*/

idCVar com_deviceZeroOverride(
    "com_deviceZeroOverride", "-1", CVAR_INTEGER,
    "change input routing for device 0 to poll a different device");
idCVar mp_bot_input_override("mp_bot_input_override", "-1", CVAR_INTEGER,
                             "Override local input routing for bot control");

/*
========================
idSessionLocal::GetInputRouting
This function sets up inputRouting to be a mapping from inputDevice index to
session user index.
========================
*/
int idSessionLocal::GetInputRouting(int inputRouting[MAX_INPUT_DEVICES]) {
  ...
  for (int i = 0; i < GetActingGameStateLobby().GetNumLobbyUsers(); i++) {
    if (GetActingGameStateLobby().IsSessionUserIndexLocal(i)) {
      ...
      const idLocalUser* localUser =
          GetActingGameStateLobby().GetLocalUserFromLobbyUserIndex(i);
      ...
      if (localUser != NULL) {
        ...
        assert(inputRouting[localDevice] ==
               -1);  // Make sure to only initialize each entry once
...
========================
idSessionLocal::EndMatch
this is for when the game is over before we go back to lobby. Need this incase
the host leaves during this time
========================
*/
void idSessionLocal::MatchFinished() {
  if (verify(GetActingGameStateLobby().IsHost())) {
    // host is putting up end game stats make sure other peers know and clear
    // migration data
...
QuitMatch is considered a premature ending of a match, and does the right thing
depending on whether the host or peer is quitting
========================
*/
void idSessionLocal::QuitMatch() {
  if (GetActingGameStateLobby().IsHost() &&
      !MatchTypeIsRanked(GetActingGameStateLobby().parms.matchFlags)) {
    EndMatch(true);  // When host quits private match, takes members back to
                     // game lobby
  } else {
    // Quitting a public match (or not being a host) before it ends takes you to
    // an empty party lobby
...
========================
*/
void idSessionLocal::QuitMatchToTitle() { MoveToMainMenu(); }
...
void idSessionLocal::EndMatchInternal(bool premature /*=false*/) {
      ...
      if (GetActingGameStateLobby().peers[p].IsConnected()) {
        if (GetActingGameStateLobby().peers[p].packetProc != NULL) {
          GetActingGameStateLobby()
              .peers[p]
              .packetProc->VerifyEmptyReliableQueue(
                  idLobby::RELIABLE_GAME_DATA, idLobby::RELIABLE_DUMMY_MSG);
        }
        GetActingGameStateLobby().peers[p].sessionID =
            GetActingGameStateLobby().IncrementSessionID(
                GetActingGameStateLobby().peers[p].sessionID);
      }
    }
  ...
  gameLobbyWasCoalesced =
      false;  // Reset this back to false.  We use this so the lobby code
              // doesn't randomly choose a map when we coalesce
  numFullSnapsReceived = 0;

  ClearMigrationState();

  if (GetActingGameStateLobby().IsLobbyActive() &&
      (GetActingGameStateLobby().GetMatchParms().matchFlags &
       MATCH_REQUIRE_PARTY_LOBBY)) {
    ...
    if (GetActingGameStateLobby().IsHost()) {
      ...
      SetState(
          GetGameStateLobby().IsHost()
              ...
              : STATE_GAME_LOBBY_HOST);  // We want the dedicated host to go
                                         // back to STATE_GAME_STATE_LOBBY_HOST
    } else {
      SetState(STATE_GAME_LOBBY_PEER);
    }
  } else {
  ...
  if (GetActingGameStateLobby().IsHost()) {
    ...
    for (int p = 0; p < GetActingGameStateLobby().peers.Num(); p++) {
      GetActingGameStateLobby().QueueReliableMessage(
          p, premature ? idLobby::RELIABLE_ENDMATCH_PREMATURE
                       : idLobby::RELIABLE_ENDMATCH);
    }
  } else if (premature) {
    ...
    const bool stats =
        MatchTypeHasStats(
            GetActingGameStateLobby().GetMatchParms().matchFlags) &&
        (GetFlushedStats() == false);
    common->Dialog().AddDialog(stats ? GDM_HOST_RETURNED_TO_LOBBY_STATS_DROPPED
                                     : GDM_HOST_RETURNED_TO_LOBBY,
                               DIALOG_ACCEPT, NULL, NULL, false, __FUNCTION__,
                               __LINE__, true);
  }

  if (GetGameStateLobby().IsLobbyActive()) {
    if (GetGameStateLobby().IsHost()) {
      // As a game state host, keep the lobby around, so we can make sure we
      // know when everyone leaves (which means they got the reliable msg to
      // EndMatch)
...
void idSessionLocal::MatchFinishedInternal() {
  ...
  if (GetActingGameStateLobby().IsHost()) {
    ...
    for (int p = 0; p < GetActingGameStateLobby().peers.Num(); p++) {
      GetActingGameStateLobby().QueueReliableMessage(
          p, idLobby::RELIABLE_MATCHFINISHED);
    }
  }
}
...
========================
*/
void idSessionLocal::EndMatchForMigration() { ClearVoiceGroups(); }
...
bool idSessionLocal::ShouldHavePartyLobby() {
  ...
  return MatchTypeIsOnline(flags) &&
         ((flags & MATCH_PARTY_INVITE_PLACEHOLDER) == 0);
}
...
void idSessionLocal::ValidateLobby(idLobby& lobby) {
  if (lobby.lobbyBackend == NULL ||
      lobby.lobbyBackend->GetState() == idLobbyBackend::STATE_FAILED ||
      lobby.GetState() == idLobby::STATE_FAILED) {
    NET_VERBOSE_PRINT(
        "NET: ValidateLobby: FAILED (lobbyType = %i, state = %s)\n",
        lobby.lobbyType, stateToString[localState]);
    if (lobby.failedReason == idLobby::FAILED_MIGRATION_CONNECT_FAILED ||
        lobby.failedReason == idLobby::FAILED_CONNECT_FAILED) {
      MoveToMainMenu();
      common->Dialog().AddDialog(GDM_INVALID_INVITE, DIALOG_ACCEPT, NULL, NULL,
                                 false);  // The game session no longer exists
    } else {
      ...
      common->Dialog().ClearDialogs(true);
      common->Dialog().AddDialog(GDM_CONNECTION_LOST, DIALOG_ACCEPT, NULL, NULL,
                                 false);  // Lost connection to XBox LIVE
    }
  }
}
...
void idSessionLocal::Pump() {
  ...
  const int SHOW_MIGRATING_INFO_IN_SECONDS =
      3;  // Show for at least this long once we start showing it

  if (ShouldShowMigratingDialog()) {
    showMigratingInfoStartTime = currentTime;
  } else if (showMigratingInfoStartTime > 0 &&
             ((currentTime - showMigratingInfoStartTime) >
              SHOW_MIGRATING_INFO_IN_SECONDS * 1000)) {
    showMigratingInfoStartTime = 0;
  }
  ...
  if (showMigratingInfoStartTime != 0) {
    if (!isShowingMigrate) {
      common->Dialog().AddDialog(GDM_MIGRATING, DIALOG_WAIT, NULL, NULL, false,
                                 "", 0, false, false, true);
    }
  ...
  if (nextGameCoalesceTime != 0) {
    if (GetGameLobby().IsLobbyActive() && GetGameLobby().IsHost() &&
      ...
      gameLobbyWasCoalesced =
          true;  // Remember that this round was coalesced.  We so this so main
                 // menu doesn't randomize the map, which looks odd
      nextGameCoalesceTime = 0;
    }
  }
}
...
void idSessionLocal::UpdatePendingInvite() {
  ...
  if (masterLocalUser == NULL &&
      signInManager->IsDeviceBeingRegistered(pendingInviteDevice)) {
    idLib::Printf("masterLocalUser == NULL\n");
    return;  // Waiting on master to sign in to continue with invite
  }

  const bool wasFromInvite =
      pendingInviteMode ==
      PENDING_INVITE_WAITING;  // Remember if this was a real invite, or a self
                               // invitation (matters when lobby is invite only)

  // At this point, the invitee should be ready
  pendingInviteMode = PENDING_INVITE_NONE;

  if (masterLocalUser == NULL ||
      masterLocalUser->GetInputDevice() != pendingInviteDevice ||
      !masterLocalUser->IsOnline()) {
    idLib::Printf(
        "ignoring invite - master local user is not setup properly\n");
    return;  // If there is no master, if the invitee is not online, or
             // different than the current master, then ignore invite
  }

  // Clear any current dialogs, as we're going into a state which will be
  // unstable for any current dialogs. Do we want to throw an assert if a dialog
  // is currently up?
  common->Dialog().ClearDialogs(true);

  // Everything looks good, let's join the party
  ConnectAndMoveToLobby(GetPartyLobby(), pendingInviteConnectInfo,
                        wasFromInvite);
}
...
void idSessionLocal::LoadingFinished() {
  ...
  } else if (!GetActingGameStateLobby()
                  .IsHost())  // Tell game host we're done loading
  {
    byte buffer[idPacketProcessor::MAX_PACKET_SIZE];
    idBitMsg msg(buffer, sizeof(buffer));
    GetActingGameStateLobby().QueueReliableMessage(
        GetActingGameStateLobby().host, idLobby::RELIABLE_LOADING_DONE,
...
void idSessionLocal::SendUsercmds(idBitMsg& msg) {
  ...
  if (GetActingGameStateLobby().IsPeer()) {
    idLobby::peer_t& hostPeer =
        GetActingGameStateLobby().peers[GetActingGameStateLobby().host];

    // Don't send user cmds if we have unsent packet fragments
    //  (This can happen if we have packets to send, but SendAnotherFragment got
    //  throttled)
    if (hostPeer.packetProc->HasMoreFragments()) {
      idLib::Warning(
          "NET: Client called SendUsercmds while HasMoreFragments(). Skipping "
          "userCmds for this frame.");
      return;
    }
    ...
    if (hostPeer.receivedBpsIndex != sequence) {
      incomingBPS = idMath::ClampFloat(
          0.0f, static_cast<float>(idLobby::BANDWIDTH_REPORTING_MAX),
          hostPeer.packetProc->GetIncomingRateBytes());
      hostPeer.receivedBpsIndex = sequence;
      hostPeer.receivedBps = incomingBPS;
    }
    uint16 incomingBPS_quantized = idMath::Ftoi(
        incomingBPS * ((BIT(idLobby::BANDWIDTH_REPORTING_BITS) - 1) /
    ...
    GetActingGameStateLobby().ProcessOutgoingMsg(GetActingGameStateLobby().host,
                                                 buffer, lzwCompressor.Length(),
                                                 false, 0);
...
void idSessionLocal::UpdateSignInManager() {
  ...
  // FIXME: We need to ask the menu system for this info.  Just making a best
  // guess for now (assume we are allowed to join the party as a splitscreen
  // user if we are in the party lobby)
  bool allowJoinParty = (localState == STATE_PARTY_LOBBY_HOST ||
                         localState == STATE_PARTY_LOBBY_PEER) &&
                        GetPartyLobby().state == idLobby::STATE_IDLE;
  bool allowJoinGame = (localState == STATE_GAME_LOBBY_HOST ||
                        localState == STATE_GAME_LOBBY_PEER) &&
                       GetGameLobby().state == idLobby::STATE_IDLE;

  bool eitherLobbyRunning =
      GetActivePlatformLobby() != NULL &&
      (GetPartyLobby().IsLobbyActive() || GetGameLobby().IsLobbyActive());
  bool onlineMatch =
      eitherLobbyRunning &&
      MatchTypeIsOnline(GetActivePlatformLobby()->parms.matchFlags);

  //=================================================================================
  // Get the number of desired signed in local users depending on what mode
  // we're in.
  //=================================================================================
  ...
  // Update signin mgr.  This manager tracks signed in local users, which the
  // session then uses to determine who should be in the lobby.
  signInManager->Pump();

  // Get the master local user
  idLocalUser* masterUser = signInManager->GetMasterLocalUser();

  if (onlineMatch && masterUser != NULL && !masterUser->CanPlayOnline() &&
      !masterUser->HasOwnerChanged()) {
    if (localState > STATE_IDLE) {
      ...
      if ((Sys_Milliseconds() - offlineTransitionTimerStart) >
          net_offlineTransitionThreshold.GetInteger()) {
        MoveToMainMenu();
    ...
    return;  // Bail out so signInManager->ValidateLocalUsers below doesn't
             // prematurely remove the master user before we can detect loss of
             // connection
  } else {
    offlineTransitionTimerStart = 0;
  }

  // Remove local users (from the signin manager) who aren't allowed to be
  // online if this is an online match. Remove local user (from the signin
  // manager) who are not properly signed into a profile.
  ...
  // Get the master local user (again, after ValidateOnlineLocalUsers, to make
  // sure he is still valid)
  ...
  // See if the master user either isn't persistent (but needs to be), OR, if
  // the owner changed RequirePersistentMaster is poorly named, this really
  // means RequireSignedInMaster
  if (masterUser->HasOwnerChanged() ||
      (RequirePersistentMaster() && !masterUser->IsProfileReady())) {
    MoveToPressStart(GDM_SP_SIGNIN_CHANGE_POST);
    return;
  }
  ...
  // The initial list of session users are normally determined at connect or
  // create time. These functions allow splitscreen users to join in, or check
  // to see if existing users (including the master) need to be removed.
  //=================================================================================
...
idPlayerProfile* idSessionLocal::GetProfileFromMasterLocalUser() {
  ...
  if (profile == NULL) {
    ...
    // idLib::Warning( "Returning fake profile until the code is fixed to handle
    // NULL profiles." );
  }
...
void idSessionLocal::MoveToPressStart(gameDialogMessages_t msg) {
  if (localState != STATE_PRESS_START) {
    MoveToPressStart();
    common->Dialog().ClearDialogs();
    common->Dialog().AddDialog(msg, DIALOG_ACCEPT, NULL, NULL, false, "", 0,
                               true);
  }
}
...
void idSessionLocal::SetState(state_t newState) {
  ...
  if (localState < STATE_LOADING && newState >= STATE_LOADING) {
    ...
    common->Dialog().ClearDialog(
        GDM_LOBBY_DISBANDED);  // The lobby you were previously in has disbanded
  } else if (localState >= STATE_LOADING && newState < STATE_LOADING) {
    // Tell lobby instances that the match has ended
    if (!WasMigrationGame())  // Don't end the session if we are going right
                              // back into the game
    {
      EndSessions();
    }
...
bool idSessionLocal::HandlePackets() {
  ...
  while (ReadRawPacket(remoteAddress, packetBuffer, recvSize, fromDedicated,
                       sizeof(packetBuffer)) &&
         recvSize > 0) {
...
idLobby* idSessionLocal::GetActivePlatformLobby() {
  ...
  if ((state == GAME_LOBBY) || (state == BUSY) || (state == INGAME) ||
      (state == LOADING)) {
    return &GetGameLobby();
...
========================
*/
idLobbyBase& idSessionLocal::GetLobbyFromLobbyUserID(
  ...
  idLobby* lobby =
      GetLobbyFromType((idLobby::lobbyType_t)lobbyUserID.GetLobbyType());
...
void idSessionLocal::TickSendQueue() {
  ...
  while (packet != NULL) {
    ...
    GetPort(packet->dedicated)
        .SendRawPacket(packet->address, packet->data, packet->size);

    if (net_forceUpstream.GetFloat() != 0.0f &&
        net_forceUpstreamQueue.GetFloat() != 0.0f) {
      // FIXME: no can do both
      assert(net_forceLatency.GetInteger() == 0);
      // compute / update an added traffic due to the queuing
      // we can't piggyback on upstreamDropRate because of the way it's computed
      // and clamped to zero
      int time = Sys_Milliseconds();
      if (time > upstreamQueueRateTime) {
        upstreamQueueRate -=
            upstreamQueueRate * (float)(time - upstreamQueueRateTime) / 1000.0f;
    ...
    sendQueue.RemoveFirst();  // we have it already, just push it off the queue
                              // before freeing
...
========================
*/
void idSessionLocal::QueuePacket(
    idQueue<idQueuePacket, &idQueuePacket::queueNode>& queue, int time,
    const lobbyAddress_t& to, const void* data, int size, bool dedicated) {
  // mem.PushHeap();
...
========================
*/
bool idSessionLocal::ReadRawPacketFromQueue(int time, lobbyAddress_t& from,
                                            void* data, int& size,
                                            bool& outDedicated, int maxSize) {
  ...
  recvQueue.RemoveFirst();  // we have it already, just push it off the queue
                            // before freeing
  packetAllocator.Free(packet);
...
void idSessionLocal::SendRawPacket(const lobbyAddress_t& to, const void* data,
  ...
  if (net_forceUpstream.GetFloat() != 0) {
    // the total bandwidth rate at which the networking systems are trying to
    // push data through
    float totalOutgoingRate =
        (float)GetActingGameStateLobby().GetTotalOutgoingRate();  // B/s

    // update the rate at which we have been taking data out by dropping it
    int time = Sys_Milliseconds();
    if (time > upstreamDropRateTime) {
      upstreamDropRate -=
          upstreamDropRate * (float)(time - upstreamDropRateTime) / 1000.0f;
    ...
    if ((float)(totalOutgoingRate - upstreamDropRate + upstreamQueueRate) >
        net_forceUpstream.GetFloat() *
            1024.0f)  // net_forceUpstream is in kB/s, everything else in B/s
    {
      ...
      if ((float)(queuedBytes + size) >
          net_forceUpstreamQueue.GetFloat() *
              1024.0f)  // net_forceUpstreamQueue is in kB/s
      {
      ...
      queuedBytes += size;
      // with queuedBytes and the current upstream, when should this packet be
      // sent?
      int queuedPacketSendDelay =
          1000.0f * ((float)queuedBytes /
                     (net_forceUpstream.GetFloat() * 1024.0f));  // in ms
      // queue for sending
      if (net_verboseSimulatedTraffic.GetBool()) {
        idLib::Printf("queuing packet: %d bytes delayed %d ms\n", size,
                      queuedPacketSendDelay);
      }

      QueuePacket(sendQueue, now + queuedPacketSendDelay, to, data, size,
                  dedicated);
  ...
  QueuePacket(sendQueue, now + net_forceLatency.GetInteger() / 2, to, data,
              size, dedicated);
...
bool idSessionLocal::ReadRawPacket(lobbyAddress_t& from, void* data, int& size,
  ...
  for (int i = 0; i < 2; i++) {
    // BRIAN_FIXME: Dedicated servers fuck up running 2 instances on the same
    // machine outDedicated = ( i == 0 ) ? currentDedicated : !currentDedicated;
    outDedicated = false;

    if (GetPort(outDedicated).ReadRawPacket(from, data, size, maxSize)) {
      if (net_forceLatency.GetInteger() == 0 && recvQueue.IsEmpty()) {
        // If we aren't forcing latency, and queue is empty, return result
        // immediately
        return true;
      }

      // the cvar is meant to be a round trip latency so we're applying half on
      // the send and half on the recv
      const int time = (net_forceLatency.GetInteger() == 0)
                           ? 0
                           : now + net_forceLatency.GetInteger() / 2;
...
========================
*/
void idSessionLocal::ConnectAndMoveToLobby(
    idLobby& lobby, const lobbyConnectInfo_t& connectInfo, bool fromInvite) {
  // Since we are connecting directly to a lobby, make sure no search results
  // are left over from previous FindOrCreateMatch results If we don't do this,
  // we might think we should attempt to connect to an old search result, and we
...
void idSessionLocal::GoodbyeFromHost(idLobby& lobby, int peerNum,
  ...
  if ((localState == STATE_CONNECT_AND_MOVE_TO_PARTY ||
       localState == STATE_CONNECT_AND_MOVE_TO_GAME) &&
      lobby.peers[peerNum].GetConnectionState() ==
          idLobby::CONNECTION_CONNECTING) {
    ...
    if (leaveGameWithParty && lobby.lobbyType == idLobby::TYPE_GAME &&
        lobby.IsPeer() && GetState() == idSession::GAME_LOBBY &&
        GetPartyLobby().host >= 0 &&
        lobby.peers[peerNum].address.Compare(
            GetPartyLobby().peers[GetPartyLobby().host].address, true)) {
      // If a host is telling us goodbye from a game lobby, and the game host is
      // the same as our party host, and we aren't in a game, and the host wants
      // us to leave with him, then do so now
      GetGameLobby().Shutdown();
      GetGameStateLobby().Shutdown();
      SetState(STATE_PARTY_LOBBY_PEER);
    } else {
...
========================
*/
void idSessionLocal::WriteLeaderboardToMsg(
    idBitMsg& msg, const leaderboardDefinition_t* leaderboard,
...
========================
*/
const leaderboardDefinition_t* idSessionLocal::ReadLeaderboardFromMsg(
    idBitMsg& msg, column_t* stats) {
  int id = msg.ReadLong();
...
========================
*/
void idSessionLocal::SendLeaderboardStatsToPlayer(
    lobbyUserID_t lobbyUserID, const leaderboardDefinition_t* leaderboard,
    const column_t* stats) {
  const int sessionUserIndex =
      GetActingGameStateLobby().GetLobbyUserIndexByID(lobbyUserID);
  ...
  const int peerIndex =
      GetActingGameStateLobby().PeerIndexFromLobbyUser(lobbyUserID);
  ...
  lobbyUser_t* gameUser =
      GetActingGameStateLobby().GetLobbyUser(sessionUserIndex);
  ...
  GetActingGameStateLobby().QueueReliableMessage(
      peerIndex, idLobby::RELIABLE_POST_STATS, msg.GetReadData(),
      msg.GetSize());
}
...
void idSessionLocal::RecvLeaderboardStatsForPlayer(idBitMsg& msg) {
  ...
  const leaderboardDefinition_t* leaderboard =
      ReadLeaderboardFromMsg(msg, stats);
...
void idSessionLocal::ComputeNextGameCoalesceTime() {
  const int coalesceTimeInSeconds =
      session->GetTitleStorageInt("net_LobbyCoalesceTimeInSeconds",
                                  net_LobbyCoalesceTimeInSeconds.GetInteger());
  const int randomCoalesceTimeInSeconds = session->GetTitleStorageInt(
      "net_LobbyRandomCoalesceTimeInSeconds",
      net_LobbyRandomCoalesceTimeInSeconds.GetInteger());

  if (coalesceTimeInSeconds != 0) {
    static idRandom2 random(Sys_Milliseconds());

    nextGameCoalesceTime =
        Sys_Milliseconds() + (coalesceTimeInSeconds +
                              random.RandomInt(randomCoalesceTimeInSeconds)) *
                                 1000;
  } else {
    nextGameCoalesceTime = 0;
  }
...
CONSOLE_COMMAND(Net_ThrottlePeer, "Test network bandwidth", 0) {
  ...
  // Note DebugSetPeerSnaprate will handle peerNum=-1 by printing out list of
  // peers
...
========================
*/
leaderboardDefinition_t* Sys_CreateLeaderboardDef(
    int id_, int numColumns_, const columnDef_t* columnDefs_,
    rankOrder_t rankOrder_, bool supportsAttachments_,
    bool checkAgainstCurrent_) {
  leaderboardDefinition_t* newDef = new (TAG_NETWORKING)
      leaderboardDefinition_t(id_, numColumns_, columnDefs_, rankOrder_,
                              supportsAttachments_, checkAgainstCurrent_);
...
void Sys_DestroyLeaderboardDefs() {
  // delete and clear all the contents of the registeredLeaderboards static
  // list.
...
bool idSessionLocal::StartOrContinueBandwidthChallenge(bool forceStart) {
  ...
  if (activeLobby == NULL) {
    idLib::Warning(
        "No active session lobby when idSessionLocal::StartBandwidthChallenge "
        "called");
    return true;
  }
...
void idSessionLocal::DebugSetPeerSnaprate(int peerIndex, int snapRateMS) {
  ...
  idLib::Printf("Set peer %s new snapRate: %d\n",
                activeLobby->GetPeerName(peerIndex),
                activeLobby->peers[peerIndex].throttledSnapRate);
}
...
float idSessionLocal::GetIncomingByteRate() {
  ...
  if (activeLobby == NULL) {
    idLib::Warning(
        "No active session lobby when idSessionLocal::GetIncomingByteRate "
        "called");
    return 0;
  }
...
void idSessionLocal::SendVoiceAudio() {
  ...
  const int VOICE_THROTTLE_TIME_IN_MS = session->GetTitleStorageInt(
      "VOICE_THROTTLE_TIME_IN_MS",
      33);  // Don't allow faster than 30hz send rate
  ...
  for (int i = 0; i < localTalkers.Num(); i++) {
    // NOTE - For 360, we don't need more than XHV_MAX_VOICECHAT_PACKETS *
    // XHV_VOICECHAT_MODE_PACKET_SIZE bytes
    const int MAX_VDP_DATA_SIZE = 1000;

    byte buffer[MAX_VDP_DATA_SIZE];

    const int titleStorageDataSize =
        session->GetTitleStorageInt("MAX_VDP_DATA_SIZE", 1000);
    const int dataSizeAvailable =
        Min<int>(titleStorageDataSize, sizeof(buffer));
    ...
    for (int j = 0; j < recipients.Num(); j++) {
      activeLobby->SendConnectionLess(*recipients[j], idLobby::OOB_VOICE_AUDIO,
                                      buffer, dataSize);
    }
  }
}
...
========================
*/
void idSessionLocal::HandleOobVoiceAudio(const lobbyAddress_t& from,
                                         const idBitMsg& msg) {
  idLobby* activeLobby = GetActivePlatformLobby();
  ...
  voiceChat->SubmitIncomingChatData(msg.GetReadData() + msg.GetReadCount(),
...
========================
*/
voiceStateDisplay_t idSessionLocal::GetDisplayStateFromVoiceState(
    voiceState_t voiceState) const {
  if ((GetState() == GAME_LOBBY &&
       MatchTypeIsLocal(GetGameLobby().GetMatchParms().matchFlags)) ||
      (GetState() == PARTY_LOBBY &&
       MatchTypeIsLocal(GetPartyLobby().GetMatchParms().matchFlags))) {
...
void idSessionLocal::ToggleLobbyUserVoiceMute(lobbyUserID_t lobbyUserID) {
  ...
  const lobbyUser_t* srcUser =
      activeLobby->GetLobbyUser(activeLobby->GetLobbyUserIndexByLocalUserHandle(
          masterUser->GetLocalUserHandle()));
...
void idSessionLocal::UpdateMasterUserHeadsetState() {
  ...
  lobbyUser_t* user = GetActivePlatformLobby()->GetSessionUserFromLocalUser(
      signInManager->GetMasterLocalUser());
  ...
  int talkerIndex = voiceChat->FindTalkerByUserId(
      user->lobbyUserID, GetActivePlatformLobby()->lobbyType);
  ...
  if (voiceChanged) {
    ...
    idLib::Printf("Sending voicestate %d for user %d %s\n",
                  voiceChat->GetHeadsetState(talkerIndex), talkerIndex,
                  user->gamertag);
    ...
    if (GetActivePlatformLobby()->IsHost()) {
      for (int p = 0; p < GetActivePlatformLobby()->peers.Num(); p++) {
        if (GetActivePlatformLobby()->peers[p].IsConnected()) {
          GetActivePlatformLobby()->QueueReliableMessage(
              p, idLobby::RELIABLE_HEADSET_STATE, msg.GetReadData(),
              msg.GetSize());
        }
      }
      ...
      GetActivePlatformLobby()->QueueReliableMessage(
          GetActivePlatformLobby()->host, idLobby::RELIABLE_HEADSET_STATE,
          msg.GetReadData(), msg.GetSize());
    }
  }
}
...
========================
*/
void idSessionLocal::SetLobbyUserRelativeScore(lobbyUserID_t lobbyUserID,
                                               int relativeScore, int team) {
...
void idSessionLocal::ReadTitleStorage(void* buffer, int bufferLen) {
  ...
  // If the file is not on the server, this will be handled as though a file of
  // 0 bytes were on the server. This means that 0 will be set to contentLength
  // and 0 (for normal termination) will return for the return value. This
  // situation can occur with problems in actual operation, so the application
  // must be designed not to hang up even in such situations
  // bufferLen = 0;

  idLib::Printf("ReadTitleStorage: %i bytes\n", bufferLen);

#if !defined(ID_RETAIL) || defined(ID_RETAIL_INTERNAL)
  if (net_ignoreTitleStorage
          .GetBool())  //&& idLib::GetProduction() < PROD_PRODUCTION ) {
  {
    idLib::Printf(
        "ReadTitleStorage: *********************** IGNORING "
        "********************\n");
    return;
  }
#endif
...
========================
*/
void idSessionLocal::PrePickNewHost(idLobby& lobby, bool forceMe,
                                    bool inviteOldHost) {
  ...
  if (GetActivePlatformLobby() == NULL) {
    NET_VERBOSE_PRINT(
        "idSessionLocal::PrePickNewHost: GetActivePlatformLobby() == NULL "
        "(%s)\n",
        lobby.GetLobbyName());
    return;
  }
  ...
  // This is checking for coop, we should make this a specific option
  // (MATCH_ALLOW_MIGRATION)
  if (GetPartyLobby().parms.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER) {
    NET_VERBOSE_PRINT(
        "idSessionLocal::PrePickNewHost: MATCH_PARTY_INVITE_PLACEHOLDER (%s)\n",
        lobby.GetLobbyName());

    // Can't migrate, shut both lobbies down, and create a new match using the
    // original parms
    GetGameStateLobby().Shutdown();
    GetGameLobby().Shutdown();
    GetPartyLobby().Shutdown();
    ...
    if (localState >= idSessionLocal::STATE_LOADING) {
      NET_VERBOSE_PRINT(
          "idSessionLocal::PrePickNewHost: localState >= "
          "idSessionLocal::STATE_LOADING (%s)\n",
          lobby.GetLobbyName());
      common->Dialog().AddDialog(GDM_BECAME_HOST_GAME_STATS_DROPPED,
                                 DIALOG_ACCEPT, NULL, NULL, false, __FUNCTION__,
                                 __LINE__, true);
    } else {
      NET_VERBOSE_PRINT(
          ...
          "idSessionLocal::STATE_LOADING (%s)\n",
          lobby.GetLobbyName());
      common->Dialog().AddDialog(GDM_LOBBY_BECAME_HOST_GAME, DIALOG_ACCEPT,
                                 NULL, NULL, false, __FUNCTION__, __LINE__,
                                 true);
    }
  ...
  if (GetState() >= idSession::GAME_LOBBY &&
      MatchTypeIsSearchable(GetGameLobby().parms.matchFlags)) {
    NET_VERBOSE_PRINT(
        "idSessionLocal::PrePickNewHost: MatchTypeIsSearchable (%s)\n",
        lobby.GetLobbyName());
    // Searchable games migrate lobbies independently, and don't need to stay in
    // sync
  ...
  //
  // Beyond this point, game lobbies must be sync'd with party lobbies as far as
  // host status So to enforce that, we pull you out of the game lobby if you
  // are in one when migration occurs
  //

  // Check to see if we should go back to a party lobby
  if (GetBackState() >= idSessionLocal::PARTY_LOBBY ||
      GetState() == idSession::PARTY_LOBBY) {
    NET_VERBOSE_PRINT(
        "idSessionLocal::PrePickNewHost: GetBackState() >= "
        "idSessionLocal::PARTY_LOBBY || GetState() == idSession::PARTY_LOBBY "
        "(%s)\n",
        lobby.GetLobbyName());
    // Force the party lobby to start picking a new host if we lost the game
    // lobby host
    ...
    GetGameStateLobby().Shutdown();
    GetGameLobby().Shutdown();
    SetState(GetPartyLobby().IsHost() ? idSessionLocal::STATE_PARTY_LOBBY_HOST
                                      : idSessionLocal::STATE_PARTY_LOBBY_PEER);
  } else {
    NET_VERBOSE_PRINT(
        "idSessionLocal::PrePickNewHost: GetBackState() < "
        ...
        lobby.GetLobbyName());
    if (localState >= idSessionLocal::STATE_LOADING) {
      common->Dialog().AddDialog(
          GDM_HOST_QUIT, DIALOG_ACCEPT, NULL, NULL, false, __FUNCTION__,
          __LINE__,
          true);  // The host has quit the session. Returning to the main menu.
    }
...
bool idSessionLocal::PreMigrateInvite(idLobby& lobby) {
  ...
  // This is checking for coop, we should make this a specific option
  // (MATCH_ALLOW_MIGRATION)
  if (!verify((GetPartyLobby().parms.matchFlags &
               MATCH_PARTY_INVITE_PLACEHOLDER) == 0)) {
    return false;  // Shouldn't get invites for coop (we should make this a
                   // specific option (MATCH_ALLOW_MIGRATION))
  }
  ...
  if (lobby.lobbyType != idLobby::TYPE_PARTY) {
    return false;  // We shouldn't be getting invites from non party lobbies
                   // when in a non searchable game
  }
...
========================
*/
bool lobbyAddress_t::UsingRelay() const { return false; }
...
void idNetSessionPort::SendRawPacket(const lobbyAddress_t& to, const void* data,
  ...
  static idRandom2 random(Sys_Milliseconds());
  if (net_forceDrop.GetInteger() != 0 &&
      net_forceDrop.GetInteger() >= random.RandomInt(100)) {
    return;
  }
...
CONSOLE_COMMAND(voicechat_mute, "TEMP", 0) {
  ...
  session->ToggleLobbyUserVoiceMute(
      session->GetActivePlatformLobbyBase().GetLobbyUserIdByOrdinal(i));
}
...
idCommandLink Net_DebugOutputSignedInUsers(
    "net_debugOutputSignedInUsers", Net_DebugOutputSignedInUsers_f,
    "Outputs all the local users and other debugging information from the sign "
    "in manager");
...
idCommandLink Net_RemoveUserFromLobby("net_removeUserFromLobby",
                                      Net_RemoveUserFromLobby_f,
...
CONSOLE_COMMAND(Net_DropClient, "Drop a client", 0) {
  if (args.Argc() < 3) {
    idLib::Printf(
        "usage: Net_DropClient <clientnum> [<session>] 0/default: drop from "
        "game, 1: drop from party, otherwise drop from both\n");
    return;
  }
  int lobbyType = 0;
  if (args.Argc() > 2) {
...
void idSessionLocal::ListServersCommon() {
  ...
  const unsigned int localChecksum =
      NetGetVersionChecksum();  // DG: use int instead of long for 64bit
                                // compatibility
  ...
  GetPartyLobby().SendConnectionLess(address, idLobby::OOB_MATCH_QUERY,
                                     msg.GetReadData(), msg.GetSize());
}
...
========================
*/
void idSessionLocal::HandleDedicatedServerQueryRequest(
    lobbyAddress_t& remoteAddr, idBitMsg& msg, int msgType) {
  ...
  if (GetPartyLobby().NumFreeSlots() == 0 ||
      GetGameLobby().NumFreeSlots() == 0) {
  ...
  if (canJoin) {
    ...
    for (int i = 0; i < GetActivePlatformLobby()->GetNumLobbyUsers(); i++) {
      retmsg.WriteString(GetActivePlatformLobby()->GetLobbyUserName(
          GetActivePlatformLobby()->GetLobbyUserIdByOrdinal(i)));
    }
  }

  // Send it
  GetPartyLobby().SendConnectionLess(remoteAddr, idLobby::OOB_MATCH_QUERY_ACK,
                                     retmsg.GetReadData(), retmsg.GetSize());
}
...
========================
*/
void idSessionLocal::HandleDedicatedServerQueryAck(lobbyAddress_t& remoteAddr,
                                                   idBitMsg& msg) {
...
========================
*/
const idList<idStr>* idSessionLocal::ServerPlayerList(int i) { return NULL; }
