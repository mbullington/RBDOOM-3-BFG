...
bool idSessionLocalCallbacks::BecomingHost(idLobby& lobby) {
  if (lobby.lobbyType == idLobby::TYPE_GAME) {
    if (sessionLocal->GetActivePlatformLobby() != &lobby) {
      idLib::Printf(
          "BecomingHost: Must be past the party lobby to become host of a game "
          "lobby.\n");
      return false;
    }
    if (sessionLocal->localState == idSessionLocal::STATE_INGAME ||
        sessionLocal->localState == idSessionLocal::STATE_LOADING) {
      // If we are in a game, go back to the lobby before becoming the new host
      // of a game lobby
      sessionLocal->SetState(idSessionLocal::STATE_GAME_LOBBY_PEER);

      // session mgr housekeeping that would usually be done through the
      // standard EndMatch path
...
void idSessionLocalCallbacks::BecameHost(idLobby& lobby) {
  ...
  if (lobby.lobbyType == idLobby::TYPE_PARTY &&
      sessionLocal->localState == idSessionLocal::STATE_PARTY_LOBBY_PEER) {
    sessionLocal->SetState(idSessionLocal::STATE_PARTY_LOBBY_HOST);
  } else if (lobby.lobbyType == idLobby::TYPE_GAME &&
             sessionLocal->localState ==
...
bool idSessionLocalCallbacks::BecomingPeer(idLobby& lobby) {
  if (lobby.lobbyType == idLobby::TYPE_GAME) {
    if (sessionLocal->localState == idSessionLocal::STATE_INGAME ||
      ...
      // This isn't totally necessary but we want to end the current game now
      // and go to some screen. When the connection goes through or fails will
      // send the session mgr to the appropriate state (game lobby or main menu)

      // What happens if we got the game migration before the party migration?
      sessionLocal->SetState(sessionLocal->GetPartyLobby().IsHost()
      ...
      return true;  // return true tells the session that we want him to tell us
                    // when the connects/fails
    }
  }
  return false;
}
...
void idSessionLocalCallbacks::FailedGameMigration(idLobby& lobby) {
  // We failed to complete a game migration this could happen for a couple
  // reasons: -The network invites failed / failed to join migrated session
  // -There was nobody to invite
  lobby.ResetAllMigrationState();
  if (lobby.lobbyType ==
      idLobby::TYPE_GAME)  // this check is a  redundant since we should only
                           // get this CB from the game session
  {
    sessionLocal->SetState(idSessionLocal::STATE_GAME_LOBBY_HOST);
...
void idSessionLocalCallbacks::MigrationEnded(idLobby& lobby) {
  if (lobby.migrationInfo.persistUntilGameEndsData.wasMigratedGame) {
#if 1
    if (lobby.lobbyType == idLobby::TYPE_GAME ||
        (lobby.lobbyType == idLobby::TYPE_PARTY &&
         session->GetState() <= idSession::PARTY_LOBBY)) {
      ...
      if (lobby.GetNumLobbyUsers() <= 1) {
        if (MatchTypeHasStats(lobby.parms.matchFlags)) {
          common->Dialog().AddDialog(GDM_MIGRATING_FAILED_DISBANDED_STATS,
                                     DIALOG_ACCEPT, NULL, NULL, false, "", 0,
                                     true);  // Game has disbanded
        } else {
          common->Dialog().AddDialog(GDM_MIGRATING_FAILED_DISBANDED,
                                     DIALOG_ACCEPT, NULL, NULL, false, "", 0,
                                     true);  // Game has disbanded
        }
      } else {
        // common->Dialog().AddDialog( GDM_MIGRATING_FAILED_CONNECTION,
        // DIALOG_ACCEPT, NULL, NULL, false, "", 0, true );		// Lost
        // connection to game
        if (lobby.lobbyType == idLobby::TYPE_GAME &&
            MatchTypeHasStats(lobby.parms.matchFlags)) {
          // This means we came from a public match, so tell them they didn't
          // lose stats
          common->Dialog().AddDialog(
              GDM_HOST_CONNECTION_LOST_STATS, DIALOG_ACCEPT, NULL, NULL, false,
              "", 0, true);  // The connection to the host has been lost. This
                             // game will not count towards your ranking.
        } else {
          // This means we came from a private match, just say host quit
          common->Dialog().AddDialog(
              GDM_HOST_CONNECTION_LOST, DIALOG_ACCEPT, NULL, NULL, false, "", 0,
              true);  // The connection to the host has been lost.
        }
      }
    ...
    if (lobby.GetNumLobbyUsers() <= 1 &&
        lobby.lobbyType == idLobby::TYPE_GAME) {
      if (!MatchTypeIsJoinInProgress(lobby.parms.matchFlags)) {
        // Handles 'soft' failed game migration where we migrated from a game
        // and are now alone
        gameDialogMessages_t errorDlg = GDM_INVALID;
        lobby.migrationInfo.persistUntilGameEndsData.hasGameData =
            false;  // never restart the game if we are by ourselves
        if (lobby.migrationInfo.invites.Num() > 0) {
          // outstanding invites: migration failed
          errorDlg = (MatchTypeHasStats(lobby.migrateMsgFlags) &&
                      (sessionLocal->GetFlushedStats() == false))
        ...
        if (errorDlg != GDM_INVALID) {
          common->Dialog().AddDialog(errorDlg, DIALOG_ACCEPT, NULL, NULL,
                                     false);
        }
        common->Dialog().ClearDialog(GDM_MIGRATING);
        common->Dialog().ClearDialog(GDM_MIGRATING_WAITING);
      ...
      if (session->GetState() <= idSession::PARTY_LOBBY) {
        ...
        if (lobby.GetNumLobbyUsers() <= 1) {
          ...
          // DIALOG_ACCEPT, NULL, NULL, false, "", 0, true );	// Lost
          // connection to game
          common->Dialog().AddDialog(GDM_HOST_CONNECTION_LOST_STATS,
                                     DIALOG_ACCEPT, NULL, NULL, false, "", 0,
                                     true);
        }
...
#endif
  } else if (lobby.GetNumLobbyUsers() <= 1 &&
             session->GetState() == idSession::PARTY_LOBBY) {
    // If they didn't come from a game, and are by themselves, just show the
    // lobby disband msg
    common->Dialog().AddDialog(
        GDM_LOBBY_DISBANDED, DIALOG_ACCEPT, NULL, NULL, false, "", 0,
        true);  // The lobby you were previously in has disbanded
...
========================
*/
bool idSessionLocalCallbacks::AnyPeerHasAddress(
    const lobbyAddress_t& remoteAddress) const {
  return sessionLocal->GetPartyLobby().FindAnyPeer(remoteAddress) ||
         sessionLocal->GetGameLobby().FindAnyPeer(remoteAddress);
}
...
void idSessionLocalCallbacks::RecvLeaderboardStats(idBitMsg& msg) {
  // Steam and PS3 just write them as they come per player, they don't need to
  // flush
...
void idSessionLocalCallbacks::ReceivedFullSnap() {
  ...
  if (sessionLocal->localState != idSessionLocal::STATE_INGAME) {
    sessionLocal->GetActingGameStateLobby().QueueReliableMessage(
        ...
        idLobby::RELIABLE_IN_GAME);  // Let host know we are in game now
...
========================
*/
void idSessionLocalCallbacks::PrePickNewHost(idLobby& lobby, bool forceMe,
                                             bool inviteOldHost) {
...
========================
*/
void idSessionLocalCallbacks::ConnectAndMoveToLobby(
    idLobby::lobbyType_t destLobbyType, const lobbyConnectInfo_t& connectInfo,
    bool waitForPartyOk) {
  ...
  if (lobby->lobbyBackend != NULL &&
      lobby->lobbyBackend->IsOwnerOfConnectInfo(connectInfo)) {
    idLib::Printf("RELIABLE_CONNECT_AND_MOVE_TO_LOBBY: Already in lobby.\n");
    return;
  }

  // See if we are in a game, or loading into a game.  If so, ignore invites
  // from our party host
  if (destLobbyType == idLobby::TYPE_GAME ||
      destLobbyType == idLobby::TYPE_GAME_STATE) {
    if (GetState() == idSession::INGAME || GetState() == idSession::LOADING) {
      idLib::Printf(
          "RELIABLE_CONNECT_AND_MOVE_TO_LOBBY: In a different game, "
          "ignoring.\n");
      return;
    }
  }
  ...
  // waitForPartyOk will be true if the party host wants us to wait for his ok
  // to stay in the lobby
  lobby->waitForPartyOk = waitForPartyOk;

  // Connect to new game lobby
  sessionLocal->ConnectAndMoveToLobby(
      *lobby, connectInfo,
      true);  // Consider this an invite if party host told us to connect
}
...
========================
*/
void idSessionLocalCallbacks::HandleServerQueryAck(lobbyAddress_t& remoteAddr,
                                                   idBitMsg& msg) {
