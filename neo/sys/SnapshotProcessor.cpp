...
idCVar net_optimalSnapDeltaSize("net_optimalSnapDeltaSize", "1000",
                                CVAR_INTEGER,
                                "Optimal size of snapshot delta msgs.");
idCVar net_debugBaseStates("net_debugBaseStates", "0", CVAR_BOOL,
                           "Log out base state information");
idCVar net_skipClientDeltaAppend("net_skipClientDeltaAppend", "0", CVAR_BOOL,
                                 "Simulate delta receive buffer overflowing");
...
========================
*/
void idSnapshotProcessor::PeekDeltaSequence(const char* deltaMem, int deltaSize,
                                            int& deltaSequence,
                                            int& deltaBaseSequence) {
  idSnapShot::PeekDeltaSequence(deltaMem, deltaSize, deltaSequence,
                                deltaBaseSequence);
}
...
========================
*/
bool idSnapshotProcessor::ApplyDeltaToSnapshot(idSnapShot& snap,
                                               const char* deltaMem,
                                               int deltaSize, int visIndex) {
  return snap.ReadDeltaForJob(deltaMem, deltaSize, visIndex, &templateStates);
}

#ifdef STRESS_LZW_MEM
// When this defined, we'll stress the lzw compressor with the smallest possible
// buffer, and detect when we need to grow it to make sure we are gacefully
...
========================
*/
void idSnapshotProcessor::SubmitPendingSnap(int visIndex, uint8* objMemory,
                                            int objMemorySize,
  ...
  assert(net_optimalSnapDeltaSize.GetInteger() <
         jobMemory_t::MAX_LZW_MEM - 128);  // Leave padding
...
int idSnapshotProcessor::GetPendingSnapDelta(byte* outBuffer, int maxLength) {
  ...
  if (!verify(size != -1)) {
    ...
    // This can happen if there wasn't enough maxlzwMem to process one full obj
    // in a single delta
  ...
  PeekDeltaSequence((const char*)deltaData, size, deltaSequence,
                    deltaBaseSequence);
  // sanity check: does the compressed data we are about to send have the
  ...
  // deltas.GetDataLength() );

  // Copy to delta buffer
  // NOTE - We don't need to save this delta off if peer has already ack'd this
  // basestate. This can happen due to the fact that we defer the processing of
  // snap deltas on jobs. When we start processing a delta, we use the currently
  // ack'd basestate.  If while we were processing the delta, the client acks a
  // new basestate, we can get into this situation.  In this case, we simply
  // don't store the delta, since it will just take up space, and just get
  // removed anyways during ApplySnapshotDelta.
  //	 (and cause lots of spam when it sees the delta's basestate doesn't
  //match the current ack'd one)
...
========================
*/
bool idSnapshotProcessor::ReceiveSnapshotDelta(const byte* deltaData,
                                               int deltaLength, int visIndex,
  ...
  PeekDeltaSequence((const char*)deltaData, deltaLength, deltaSequence,
                    deltaBaseSequence);
  ...
  // Bring the base state up to date with the basestate this delta was compared
  // to
  ...
  // Save the new delta
  if (net_skipClientDeltaAppend.GetBool() ||
      !deltas.Append(deltaSequence, deltaData, deltaLength)) {
    // This can happen if the delta queues get desync'd between the server and
    // client. With recent fixes, this should be extremely rare, or impossible.
    // Just in case this happens, we can recover by assuming we didn't even
    // receive this delta.
  ...
  // Update our snapshot sequence number to the newer one we just got (now that
  // it's safe)
  snapSequence = deltaSequence;

  if (deltas.Num() > 10) {
    NET_VERBOSESNAPSHOT_PRINT(
        "NET: ReceiveSnapshotDelta: deltas.Num() > 10: %d\n   ", deltas.Num());
  ...
  if (baseSequence != deltaBaseSequence) {
    // NOTE - With recent fixes, this should no longer be possible unless the
    // delta is trashed We should probably disconnect from the server when this
    // happens now.
    static bool failed = false;
    if (!failed) {
      idLib::Printf(
          "NET: incorrect base state? not sure how this can happen... "
          "baseSequence: %d  deltaBaseSequence: %d \n",
  ...
  if (baseSequence < 50 && net_debugBaseStates.GetBool()) {
    idLib::Printf(
        "NET: Proper basestate...  baseSequence: %d  deltaBaseSequence: %d \n",
        baseSequence, deltaBaseSequence);
  }
  ...
  fullSnap = ApplyDeltaToSnapshot(outSnap, (const char*)deltaData, deltaLength,
                                  visIndex);
...
bool idSnapshotProcessor::ApplySnapshotDelta(int visIndex, int snapshotNumber) {
  NET_VERBOSESNAPSHOT_PRINT_LEVEL(
      6, va("idSnapshotProcessor::ApplySnapshotDelta snapshotNumber: %d\n",
            snapshotNumber));

  // Sanity check deltas
  SanityCheckDeltas();

  // dump any deltas older than the acknoweledged snapshot, which should only
  // happen if there is packet loss
  deltas.RemoveOlderThan(snapshotNumber);

  if (deltas.Num() == 0 || deltas.ItemSequence(0) != snapshotNumber) {
    // this means the snapshot was either already acknowledged or came out of
    // order On the server, this can happen because the client is
    // continuously/redundantly sending acks Once the server has ack'd a certain
    // base sequence, it will need to ignore all the redundant ones. On the
    // client, this will only happen due to out of order, or dropped packets.
    ...
    if (!common->IsServer()) {
      ...
      if (deltas.Num() == 0) {
        NET_VERBOSESNAPSHOT_PRINT(
            "NET: Got snapshot but ignored... deltas.Num(): %d snapshotNumber: "
            "%d \n",
            deltas.Num(), snapshotNumber);
      } else {
        NET_VERBOSESNAPSHOT_PRINT(
            "NET: Got snapshot but ignored... deltas.ItemSequence( 0 ): %d != "
            "snapshotNumber: %d \n   ",
  ...
  PeekDeltaSequence((const char*)deltas.ItemData(0), deltas.ItemLength(0),
                    deltaSequence, deltaBaseSequence);

  assert(deltaSequence == snapshotNumber);  // Make sure compressed sequence
                                            // number matches that in data queue
  assert(baseSequence ==
         deltaBaseSequence);  // If this delta isn't based off of our currently
                              // ack'd basestate, something is trashed...
  assert(deltaSequence > baseSequence);

  if (baseSequence != deltaBaseSequence) {
    // NOTE - This should no longer happen with recent fixes.
    // We should probably disconnect from the server if this happens. (packets
    // are trashed most likely)
    NET_VERBOSESNAPSHOT_PRINT(
        "NET: Got snapshot %d but baseSequence does not match. baseSequence: "
  ...
  if (ApplyDeltaToSnapshot(baseState, (const char*)deltas.ItemData(0),
                           deltas.ItemLength(0), visIndex)) {
...
Remove deltas for basestate we no longer have. We know we can remove them,
because we will never be able to apply them, since the basestate needed to
generate a full snap from these deltas is gone.

Ways we can get deltas based on basestate we no longer have:
        1. Server sends sequence 50 based on 49.  It then sends sequence 51
based on 49. Client acks 50, server applies it to 49, 50 is new base state.
           Server now has a delta sequence 51 based on 49 that it won't ever be
able to apply (50 is new basestate).

This is annoying, because it makes a lot of our sanity checks incorrectly fire
off for benign issues. Here is a series of events that make the old (
baseSequence != deltaBaseSequence ) assert:
...
The client above will ack 51, even though he can't even apply that delta.  To
get around this, we simply don't allow delta to exist in the list, unless their
basestate is the current basestate we maintain. This allows us to put sanity
checks in place that don't fire off during benign conditions, and allow us to
truly check for trashed conditions.
...
void idSnapshotProcessor::RemoveDeltasForOldBaseSequence() {
  ...
  // (we will never be able to apply these, since we don't have that base
  // anymore)
  for (int i = deltas.Num() - 1; i >= 0; i--) {
    int deltaSequence = 0;
    int deltaBaseSequence = 0;
    baseState.PeekDeltaSequence((const char*)deltas.ItemData(i),
                                deltas.ItemLength(i), deltaSequence,
                                deltaBaseSequence);
    if (deltaBaseSequence < baseSequence) {
...
void idSnapshotProcessor::SanityCheckDeltas() {
  ...
  for (int i = 0; i < deltas.Num(); i++) {
    ...
    assert(
        deltaSequence ==
        deltas.ItemSequence(i));  // Make sure delta stored in compressed form
                                  // matches the one stored in the data queue
    assert(deltaSequence >
           lastDeltaSequence);  // Make sure they are in order (we reject out of
                                // order sequences in ApplysnapshotDelta)
    assert(deltaBaseSequence >=
           lastDeltaBaseSequence);  // Make sure they are in order (they can be
                                    // the same, since base sequences don't
                                    // change until they've been ack'd)
    assert(deltaBaseSequence >=
           baseSequence);  // We should have removed old delta's that can no
                           // longer be applied
    assert(deltaBaseSequence == baseSequence ||
           deltaBaseSequence ==
               lastDeltaSequence);  // Make sure we still have a base (or
                                    // eventually will have) that we can apply
                                    // this delta to
...
void idSnapshotProcessor::AddSnapObjTemplate(int objID, idBitMsg& msg) {
  ...
  idSnapShot::objectState_t* state =
      templateStates.S_AddObject(objID, MAX_UNSIGNED_TYPE(uint32), msg);
  if (verify(state != NULL)) {
    if (net_ssTemplateDebug.GetBool()) {
      idLib::PrintfIf(net_ssTemplateDebug.GetBool(),
                      "InjectingSnapObjBaseState[%d] size: %d\n", objID,
                      state->buffer.Size());
