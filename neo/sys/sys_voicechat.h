...
class idVoiceChatMgr {
 public:
  ...
  virtual void ToggleMuteLocal(const lobbyUser_t* src,
 ...
 protected:
  ...
  struct talker_t {
    ...
    int lobbyType;
    int groupIndex;
    bool registered;  // True if this user is currently registered with the XHV
                      // engine
    bool registeredSuccess;  // True if this user is currently successfully
                             // registered with the XHV engine
    int machineIndex;  // Index into remote machines array (-1 if this is a
                       // local talker)
  ...
  idStaticList<talker_t, MAX_PLAYERS * 2>
      talkers;  // * 2 to account for handling both session types
  idStaticList<remoteMachine_t, MAX_PLAYERS * 2>
      remoteMachines;  // * 2 to account for handling both session types
