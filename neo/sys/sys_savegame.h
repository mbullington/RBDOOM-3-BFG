...
#endif
#define OUTPUT_FUNC() \
  idLib::PrintfIf(saveGame_verbose.GetBool(), "[%s] Enter\n", __FUNCTION__)
#define OUTPUT_FUNC_EXIT() \
  idLib::PrintfIf(saveGame_verbose.GetBool(), "[%s] Exit\n", __FUNCTION__)
...
#define MAX_SAVEGAME_STRING_TABLE_SIZE \
  (400 * 1024 * 2)  // 400 kB max string table size
...
// PS3 restrictions:  The only characters that can be used are 0-9 (numbers),
// A-Z (uppercase alphabet), "_" (underscore), and "-" (hyphen)
...
enum saveGameModeBitfield_t {
  SAVEGAME_MBF_NONE = 0,
  SAVEGAME_MBF_LOAD = BIT(0),  // standard file load (can be individual/multiple
                               // files described in parms)
  SAVEGAME_MBF_SAVE = BIT(1),  // standard file save (can be individual/multiple
                               // files described in parms)
  SAVEGAME_MBF_DELETE_FOLDER = BIT(2),  // standard package delete
  SAVEGAME_MBF_DELETE_ALL_FOLDERS =
      BIT(3),  // deletes all of the savegame folders (should only be used in
               // testing)
  SAVEGAME_MBF_ENUMERATE =
      BIT(4),  // gets listing of all savegame folders, typically used with
               // READ_DETAILS to read the description file
  SAVEGAME_MBF_NO_COMPRESS =
      BIT(5),  // tells the system the files aren't compressed, usually only
               // needed when reading the descriptors file internally
  SAVEGAME_MBF_ENUMERATE_FILES = BIT(
      6),  // enumerates all the files within a particular savegame folder (can
           // be individual/multiple files or pattern described in parms)
  SAVEGAME_MBF_DELETE_FILES = BIT(
      7),  // deletes individual files within a particular savegame folder (can
           // be individual/multiple files or pattern described in parms)
  SAVEGAME_MBF_READ_DETAILS =
      BIT(8),  // reads the description file (if specified,
               // parms.enumeratedEntry.name & parms.enumeratedEntry.type must
               // be specified)
...
class idSaveGameDetails {
 ...
 public:
  idDict descriptors;  // [in] Descriptors available to be shown on the
                       // save/load screen.  Each game can define their own,
                       // e.g. Difficulty, level, map, score, time.
  bool damaged;        // [out]
  time_t date;         // [out] read from the filesystem, not set by client
  idStrStatic<MAX_FOLDER_NAME_LENGTH>
      slotName;  // [out] folder/slot name, e.g. AUTOSAVE
};
...
class idSaveLoadParms {
 public:
  ...
  void Init();
  void SetDefaults(
      int inputDevice = -1);  // doesn't clear out things that should be
                              // persistent across entire processor
 ...
 public:
  idStrStatic<MAX_FOLDER_NAME_LENGTH>
      directory;  // [in] real directory of the savegame package
  idStrStatic<MAX_FILENAME_LENGTH_PATTERN>
      pattern;  // [in] pattern to use while enumerating/deleting files within a
                // savegame folder
  idStrStatic<MAX_FILENAME_LENGTH_PATTERN>
      postPattern;  // [in] pattern at the end of the file to use while
                    // enumerating/deleting files within a savegame folder

  int mode;  // [in] SAVE, LOAD, ENUM, DELETE, etc.
  idSaveGameDetails
      description;  // [in/out] in: description used to serialize into
                    // game.details file, out: if SAVEGAME_MBF_READ_DETAILS used
                    // with certain modes, item 0 contains the read details
  saveFileEntryList_t files;  // [in/out] in: files to be saved, out: objects
                              // loaded, for SAVEGAME_MBF_ENUMERATE_FILES, it
                              // contains a listing of the filenames only
  saveGameDetailsList_t
      detailList;  // [out] listing of the enumerated savegames used only with
                   // SAVEGAME_MBF_ENUMERATE

  int errorCode;               // [out] combination of saveGameError_t bits
  int handledErrorCodes;       // [out] combination of saveGameError_t bits
  int64 requiredSpaceInBytes;  // [out] when fails for insufficient space, this
                               // is populated with additional space required
  ...
  // ----------------------
  idSysSignal
      callbackSignal;  // [internal] used to signal savegame manager that the
                       // Process() call is completed (we still might have more
                       // Process() calls to make though...)
  volatile bool cancelled;  // [internal] while processor is running, this can
                            // be set outside of the normal operation of the
                            // processor.  Each implementation should check this
                            // during operation to allow it to shutdown cleanly.
  savegameUserId_t
      userId;         // [internal] to get the proper user during every step
  int inputDeviceId;  // [internal] consoles will use this to segregate each
                      // player's files
...
// 1. CompletedCallback methods in processors weren't generic enough, we could
// use SaveFiles processors
//		for profiles/games, but there would be a single completed callback
//and we'd have to update 		the callback to detect what type of call it was, store
//the type in the processor, etc.
// 2. Using a functor class would require us to define classes for each
// callback.  The definition of those
//		classes could be scattered and a little difficult to follow
// 3. With callback methods, we assign them when needed and know exactly where
...
struct saveGameThreadArgs_t {
  saveGameThreadArgs_t() : saveLoadParms(NULL) {}
...
class idSaveGameProcessor {
 ...
 public:
  ...
  // This method should returns true if the processor has additional sub-states
  // to manage.  The saveGameManager will retain the current state and Process()
  // will be called again. When this method returns false Process() will not be
  // called again. For example, during save, you might want to load other files
  // and save them somewhere else, return true until you are done with the
  // entire state.
  virtual bool Process() { return false; }

  // Gives each processor to validate an error returned from the previous
  // process call. This is useful when processors have a multi-stage Process()
  // and expect some benign errors like deleting a savegame folder before
  // copying into it.
  virtual bool ValidateLastError() { return false; }

  // Processors need to override this if they will eventually reset the map.
  // If it could possibly reset the map through any of its stages, including
  // kicking off another processor in completed callback, return false. We will
  // force non-simple processors to execute last and won't block the map heap
  // reset due if non-simple processors are still executing.
  virtual bool IsSimpleProcessor() const { return true; }

  // This is a fail-safe to catch a timing issue on the PS3 where the nextmap
  // processor could sometimes hang during a level transition
  virtual bool ShouldTimeout() const { return false; }

  //------------------------
  // Commands
  //------------------------
  // Cancels this processor in whatever state it's currently in and sets an
  // error code for SAVEGAME_E_CANCELLED
  void Cancel() {
  ...
  // This is a way to tell the processor which errors shouldn't be handled by
  // the processor or system.
  ...
  // These can be overridden by game code, like the GUI, when the processor is
  // done executing. Game classes like the GUI can create a processor derived
  // from a game's Save processor impl and simply use this method to know when
  // everything is done.  It eases the burden of constantly checking the working
  // flag. This will be called back within the game thread during
  // SaveGameManager::Pump().
  void AddCompletedCallback(const idCallback& callback);

 private:
  // Returns whether or not the thread is finished operating, should only be
  // called by the savegame manager
...
- Savegames need to be processed asynchronously, saving/loading/deleting files
should happen during the game frame so there is a common way to update the
render device.
- When executing commands, if no "strategy"s are used, the pump() method would
need to have a switch statement, extending the manager for other commands would
mean modifying the manager itself for various commands. By making it a strategy,
we are able to create custom commands and define the behavior within game code
and keep the manager code in the engine static.
================================================
*/
class idSaveGameManager {
 public:
  ...
  // Has the storage device been selected yet?  This is only an issue on the
  // 360, and primarily for development purposes
  ...
  // Assign a processor to the manager.  The processor should belong in
  // game-side code This queues up processors and executes them serially Returns
  // whether or not the processor is immediately executed
  ...
  bool IsSaveGameCompletedFromHandle(const saveGameHandle_t& handle) const {
    return handle <= lastExecutedProcessorHandle ||
           handle ==
               0;  // last case should never be reached since it would be also
                   // be true in first case, this is just to show intent
  }
  void Set360RetrySaveAfterDeviceSelected(const char* folder,
                                          const int64 bytes);
  ...
  void ClearRetryInfo();
  void RetrySave();
  // This will cause the processor to cancel execution, the completion callback
  // will be called
 ...
 private:
  // These are to make sure that all processors start and finish in the same way
  // without a lot of code duplication. We need to make sure that we adhere to
  // PS3 system combination initialization issues.
 ...
 private:
  ...
  bool storageAvailable;  // On 360, this is false by default, after the storage
                          // device is selected
...
// Synchronous check, just checks if any savegame exists for master local user
// and if one is an autosave
void Sys_SaveGameCheck(bool& exists, bool& autosaveExists);

const idStr& GetSaveFolder(idSaveGameManager::packageType_t type);
idStr AddSaveFolderPrefix(const char* folder,
                          idSaveGameManager::packageType_t type);
idStr RemoveSaveFolderPrefix(const char* folder,
                             idSaveGameManager::packageType_t type);
