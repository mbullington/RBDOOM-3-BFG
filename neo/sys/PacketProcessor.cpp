...
idCVar net_maxRate("net_maxRate", "50", CVAR_INTEGER,
                   "max send rate in kilobytes per second");

idCVar net_showReliableCompression("net_showReliableCompression", "0",
                                   CVAR_BOOL,
...
int idPacketProcessor::FinalizeRead(idBitMsg& inMsg, idBitMsg& outMsg,
  ...
  if (!verify(
          header.Type() !=
          PACKET_TYPE_FRAGMENTED))  // We shouldn't be fragmented at this point
  {
  ...
  if (header.Type() == PACKET_TYPE_OOB) {
    ...
    if (numReliableRecv > 0) {
      ...
      lzwCompressor.Start((uint8*)inMsg.GetReadData() + inMsg.GetReadCount(),
                          compressedSize);  // Read from msg
      ...
      for (int r = 0; r < numReliableRecv; r++) {
        ...
        {
          if (!verify(bufferPos + reliableDataLength <=
                      sizeof(reliableBuffer))) {
          ...
          if (net_verboseReliable.GetBool()) {
            idLib::Printf(
                "Ignoring reliable msg %i because %i was already acked\n",
...
================================================
*/
bool idPacketProcessor::QueueReliableMessage(byte type, const byte* data,
                                             int dataLen) {
...
void idPacketProcessor::UpdateOutgoingRate(const int time, const int size) {
  ...
  if (time > outgoingRateTime) {
    outgoingRateBytes -=
        outgoingRateBytes * (float)(time - outgoingRateTime) / 1000.0f;
  ...
  if (time - lastOutgoingRateTime > BANDWIDTH_AVERAGE_PERIOD) {
    currentOutgoingRate = 1000 * (outgoingBytes - lastOutgoingBytes) /
                          (time - lastOutgoingRateTime);
...
void idPacketProcessor::UpdateIncomingRate(const int time, const int size) {
  ...
  if (time > incomingRateTime) {
    incomingRateBytes -=
        incomingRateBytes * (float)(time - incomingRateTime) / 1000.0f;
  ...
  if (time - lastIncomingRateTime > BANDWIDTH_AVERAGE_PERIOD) {
    currentIncomingRate = 1000 * (incomingBytes - lastIncomingBytes) /
                          (time - lastIncomingRateTime);
...
bool idPacketProcessor::ProcessOutgoing(const int time, const idBitMsg& msg,
  ...
  if (isOOB) {
    ...
    if (reliable.Num() > 0) {
      ...
      lzwCompressor.Start(
          unsentMsg.GetWriteData() + unsentMsg.GetSize() + 2,
          unsentMsg.GetRemainingSpace() -
              2);  // Write to compressed mem, not exceeding MAX_MSG_SIZE (+2 to
      ...
      if (net_showReliableCompression.GetBool()) {
        ...
        idLib::Printf(
            "Uncompressed: %i, Compressed: %i, TotalUncompressed: %i, "
            "TotalCompressed: %i, (%2.2f / %2.2f )\n",
            uncompressedSize, lzwCompressor.Length(), totalUncompressed,
            totalCompressed, ratio1, ratio2);
      }
    }
  }
...
bool idPacketProcessor::GetSendFragment(const int time, sessionId_t sessionID,
  ...
  if (!fragmentedSend) {
    ...
    {
      assert(moreFragments);  // If we have a first, we must have more or
...
int idPacketProcessor::ProcessIncoming(int time, sessionId_t expectedSessionID,
  ...
  if (!verify(sessionID != SESSION_ID_CONNECTIONLESS_PARTY &&
  ...
  memcpy(msgBuffer + msgWritePos, msg.GetReadData() + msg.GetReadCount(),
  ...
  if (!verify(header.Value() == FRAGMENT_START ||
...
================================================
*/
idCVar net_verifyReliableQueue(
    "net_verifyReliableQueue", "2", CVAR_INTEGER,
    "0: warn only, 1: error, 2: fixup, 3: fixup and verbose, 4: force test");
#define RELIABLE_VERBOSE \
  if (net_verifyReliableQueue.GetInteger() >= 3) idLib::Printf
void idPacketProcessor::VerifyEmptyReliableQueue(byte keepMsgBelowThis,
                                                 byte replaceWithThisMsg) {
  ...
  for (int i = 0; i < reliable.Num(); i++) {
    ...
    if (peek < keepMsgBelowThis) {
      ...
      clean.Append(reliable.ItemSequence(i), reliable.ItemData(i),
                   reliable.ItemLength(i));
    } else {
      // Replace with fake msg, so we retain itemsequence ordering.
      // If we don't do this, it's possible we remove the last msg, then append
      // a single msg before the next send, and the client may think he already
      // received the msg, since his last reliableSequenceRecv could be greater
      // than our reliableSequenceSend if he already received the group of
      // reliables we are mucking with
