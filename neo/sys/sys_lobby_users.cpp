...
void idLobby::SaveDisconnectedUser(const lobbyUser_t& user) {
  ...
  for (int i = 0; i < disconnectedUsers.Num(); i++) {
    if (user.lobbyUserID.CompareIgnoreLobbyType(
            disconnectedUsers[i].lobbyUserID)) {
      found = true;
      memcpy(disconnectedUsers[i].gamertag, user.gamertag,
...
========================
*/
lobbyUser_t* idLobby::GetLobbyUserByID(lobbyUserID_t lobbyUserID,
                                       bool ignoreLobbyType) {
...
lobbyUser_t idLobby::CreateLobbyUserFromLocalUser(
  ...
  idStr::Copynz(lobbyUser.gamertag, localUser->GetGamerTag(),
                sizeof(lobbyUser.gamertag));
  lobbyUser.peerIndex = -1;
  lobbyUser.lobbyUserID =
      lobbyUserID_t(localUser->GetLocalUserHandle(),
                    lobbyType);  // Generate the lobby using a combination of
                                 // local user id, and lobby type
  lobbyUser.disconnecting = false;

  // If we are in a game lobby (or dedicated game state lobby), and we have a
  // party lobby running, assume we can grab the party token from our equivalent
  // user in the party.
  if ((lobbyType == TYPE_GAME || lobbyType == TYPE_GAME_STATE) &&
      sessionCB->GetPartyLobby().IsLobbyActive()) {
    if ((sessionCB->GetPartyLobby().parms.matchFlags &
         MATCH_REQUIRE_PARTY_LOBBY) &&
        !(sessionCB->GetPartyLobby().parms.matchFlags &
          MATCH_PARTY_INVITE_PLACEHOLDER)) {
      // copy some things from my party user
      const int myPartyUserIndex =
          sessionCB->GetPartyLobby().GetLobbyUserIndexByLocalUserHandle(
              lobbyUser.lobbyUserID.GetLocalUserHandle());

      if (verify(myPartyUserIndex >= 0))  // Just in case
      {
        lobbyUser_t* myPartyUser =
            sessionCB->GetPartyLobby().GetLobbyUser(myPartyUserIndex);
        if (myPartyUser != NULL) {
  ...
  NET_VERBOSE_PRINT(
      "NET: CreateLobbyUserFromLocalUser: party %08x name %s (%s)\n",
...
void idLobby::InitSessionUsersFromLocalUsers(bool onlineMatch) {
  ...
  for (int i = 0; i < sessionCB->GetSignInManager().GetNumLocalUsers(); i++) {
    idLocalUser* localUser =
        sessionCB->GetSignInManager().GetLocalUserByIndex(i);
    ...
    if (verify(createdUser != NULL) &&
        migrationInfo.persistUntilGameEndsData.wasMigratedHost) {
      createdUser->migrationGameData =
          migrationInfo.persistUntilGameEndsData.ourGameData;
      NET_VERBOSE_PRINT(
          "NET: Migration game data set for local user %s at index %d \n",
...
========================
*/
int idLobby::GetLobbyUserIndexByLocalUserHandle(
...
idLocalUser* idLobby::GetLocalUserFromLobbyUserIndex(int lobbyUserIndex) {
  ...
  return sessionCB->GetSignInManager().GetLocalUserByHandle(
      lobbyUser->lobbyUserID.GetLocalUserHandle());
}
...
========================
*/
lobbyUser_t* idLobby::GetSessionUserFromLocalUser(
    const idLocalUser* localUser) {
  if (localUser == NULL) {
    return NULL;
  }
  ...
  int sessionUserIndex =
      GetLobbyUserIndexByLocalUserHandle(localUser->GetLocalUserHandle());
...
void idLobby::RemoveUsersWithDisconnectedPeers() {
  ...
  for (int u = 0; u < GetNumLobbyUsers(); u++) {
    ...
    if (user->peerIndex == -1) {
      ...
      // If this is a user on the hosts machine, IsSessionUserIndexLocal should
      // catch it above.
      assert(false);
      // The user is on the host.
      // The host's peer is disconnected via other mechanisms that I don't have
      // a firm grasp on yet.
      continue;
    }
    ...
    if (user->peerIndex >= peers.Num()) {
      ...
      // only throwing an assertion in debug, with no crashing, so adding a
      // warning verbose
      idLib::Warning(
          "idLobby::RemoveUsersWithDisconnectedPeers: user %d %s is out of "
          "range in the peers list (%d elements)",
          u, user->gamertag, peers.Num());
      continue;
    }
...
It will handle all the housekeeping of removing from various platform lists
(xsession user tracking, etc). Called from both host and client.
========================
*/
void idLobby::RemoveSessionUsersByIDList(
  ...
  for (int i = 0; i < usersToRemoveByID.Num(); i++) {
    for (int u = 0; u < GetNumLobbyUsers(); u++) {
      ...
      if (user->IsDisconnected()) {
        // User already disconnected from session  but not removed from the
        // list. This will happen when users are removed during the game.
        continue;
      }
      ...
      if (user->lobbyUserID == usersToRemoveByID[i]) {
        ...
        // Save the user so we can still get his gamertag, which may be needed
        // for a disconnection HUD message.
        SaveDisconnectedUser(*user);
        FreeUser(user);
  ...
  if (usersToRemoveByID.Num() > 0 && IsHost()) {
    ...
    // If we are the host, send a message to all peers with a list of users who
    // have disconnected
    ...
    for (int p = 0; p < peers.Num(); p++) {
      QueueReliableMessage(p, RELIABLE_USER_DISCONNECTED, msg.GetReadData(),
                           msg.GetSize());
    }
  }
}
...
void idLobby::SendPeersMicStatusToNewUsers(int peerNumber) {
  ...
  for (int i = 0; i < GetNumLobbyUsers(); ++i) {
    ...
    int talkerIndex = sessionCB->GetVoiceChat()->FindTalkerByUserId(
        user->lobbyUserID, lobbyType);
  ...
  QueueReliableMessage(peerNumber, RELIABLE_HEADSET_STATE, outmsg.GetReadData(),
                       outmsg.GetSize());
}
...
The host calls this when he receives new users, to forward the list to the other
peers.
========================
*/
void idLobby::SendNewUsersToPeers(int skipPeer, int userStart, int numUsers) {
  ...
  for (int p = 0; p < peers.Num(); p++) {
    if (p == skipPeer ||
        peers[p].GetConnectionState() != CONNECTION_ESTABLISHED) {
      continue;  // If they are not connected in this session type, don't send
                 // anything to them.
    }
...
lobbyUserID_t idLobby::AllocLobbyUserSlotForBot(const char* botName) {
  ...
  idStr::Copynz(botSessionUser.gamertag, botName,
                sizeof(botSessionUser.gamertag));

  localUserHandle_t localUserHandle(
      session->GetSignInManager().GetUniqueLocalUserHandle(
          botSessionUser.gamertag));
  ...
  SendNewUsersToPeers(peerIndexOnHost, userList.Num() - 1,
                      1);  // bot has been added to the lobby user list - update
                           // the peers so that they can see the bot too.
...
void idLobby::RemoveBotFromLobbyUserList(lobbyUserID_t lobbyUserID) {
  ...
  if (!botUser->isBot) {
    idLib::Warning(
        "RemoveBotFromLobbyUserList: User Index Is Not A Bot!");  // don't
                                                                  // accidentally
                                                                  // disconnect
                                                                  // a human
                                                                  // player.
...
If we are the host, we will forward this to all peers except the peer that we
...
void idLobby::UpdateSessionUserOnPeers(idBitMsg& msg) {
  for (int p = 0; p < peers.Num(); p++) {
    QueueReliableMessage(p, RELIABLE_UPDATE_SESSION_USER,
                         msg.GetReadData() + msg.GetReadCount(),
                         msg.GetSize() - msg.GetReadCount());
  }
...
void idLobby::HandleHeadsetStateChange(int fromPeer, idBitMsg& msg) {
  ...
  for (int i = 0; i < userCount; ++i) {
    ...
    int talkerIndex =
        sessionCB->GetVoiceChat()->FindTalkerByUserId(lobbyUserID, lobbyType);
    sessionCB->GetVoiceChat()->SetHeadsetState(talkerIndex, state);

    idLib::Printf("User %d headset status: %d\n", talkerIndex, state);

    // If we are the host, let the other clients know about the headset state of
    // this peer
    if (IsHost()) {
      // We should not be receiving a message with a user count > 1 if we are
      // the host
      assert(userCount == 1);
      ...
      for (int j = 0; j < peers.Num(); ++j) {
        ...
        QueueReliableMessage(j, RELIABLE_HEADSET_STATE, outMsg.GetReadData(),
                             outMsg.GetSize());
      }
    }
  }
...
void idLobby::UpdateLocalSessionUsers() {
  for (int i = 0; i < GetNumLobbyUsers(); i++) {
    ...
    if (IsHost()) {
      ...
      QueueReliableMessage(host, RELIABLE_SESSION_USER_MODIFIED,
...
========================
*/
void idLobby::HandleUserConnectFailure(int p, idBitMsg& inMsg,
                                       int reliableType) {
  ...
  byte buffer[idPacketProcessor::MAX_PACKET_SIZE];
  idBitMsg msg(buffer, sizeof(buffer));
  user.lobbyUserID.GetLocalUserHandle().WriteToMsg(
      msg);  // Let peer know which user failed to connect
...
We need to do this, since when in a game, we aren't allowed to remove users from
the game session.
========================
*/
...
void idLobby::RequestLocalUserJoin(idLocalUser* localUser) {
  ...
  if (IsHost()) {
    ...
    QueueReliableMessage(host, RELIABLE_USER_CONNECT_REQUEST, msg.GetReadData(),
                         msg.GetSize());
  }
}
...
void idLobby::RequestSessionUserDisconnect(int sessionUserIndex) {
  if (!IsRunningAsHostOrPeer()) {
    ...
    // This is so we accurately reflect the local user list through the session
    // users in the menus, etc.
    // FIXME:
    // This is a total hack, and we should really look at better separation of
    // local users/session users and not rely on session users while in the
    // menus.
  ...
  if (IsHost()) {
    ...
    // As the host, just disconnect immediately (we'll still send the
    // notification to all peers though)
    ProcessUserDisconnectMsg(readMsg);
  } else {
    // Send the message
    QueueReliableMessage(host, RELIABLE_USER_DISCONNECT_REQUEST,
                         msg.GetReadData(), msg.GetSize());
...
This function will simply try and keep session[lobbyType].users sync'd up with
local users. As local users come and go, this function will detect that, and
send msg's to the host that will add/remove the users from the session.
========================
*/
void idLobby::SyncLobbyUsersWithLocalUsers(bool allowLocalJoins,
                                           bool onlineMatch) {
  if (lobbyBackend == NULL) {
    return;
  }
  ...
  if (allowLocalJoins) {
    // If we are allowed to do so, allow local users to join the session user
    // list
    for (int i = 0; i < sessionCB->GetSignInManager().GetNumLocalUsers(); i++) {
      idLocalUser* localUser =
          sessionCB->GetSignInManager().GetLocalUserByIndex(i);
      ...
      localUser->SetJoiningLobby(
          lobbyType,
          true);  // We'll reset this when we get the ack for this request
  ...
  for (int i = GetNumLobbyUsers() - 1; i >= 0; i--) {
    ...
    if (localUser == NULL || (onlineMatch && !localUser->CanPlayOnline())) {
      ...
      localUser->SetJoiningLobby(
          lobbyType,
          false);  // Remove joining lobby flag if we are in the lobby
    }
  }
}
...
bool idLobby::LobbyUserHasFirstFullSnap(lobbyUserID_t lobbyUserID) const {
  ...
  if (peers[user->peerIndex].snapProc->GetFullSnapBaseSequence() <
...
========================
*/
void idLobby::QueueReliablePlayerToPlayerMessage(int fromSessionUserIndex,
  ...
  QueueReliableMessage(sendToPeer, RELIABLE_PLAYER_TO_PLAYER_BEGIN + (int)type,
...
void idLobby::KickLobbyUser(lobbyUserID_t lobbyUserID) {
  ...
  if (user != NULL && !IsSessionUserLocal(user)) {
    ...
    if (user->peerIndex >= 0 && user->peerIndex < peers.Num()) {
      ...
      QueueReliableMessage(user->peerIndex, RELIABLE_KICK_PLAYER,
                           msg.GetReadData(), msg.GetSize());
    }
  }
}
