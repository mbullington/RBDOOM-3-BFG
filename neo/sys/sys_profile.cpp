...
idCVar profile_verbose("profile_verbose", "0", CVAR_BOOL,
...
void idProfileMgr::Pump() {
  // profile can be NULL if we forced the user to register as in the case of
  // map-ing into a level from the press start screen
  if (profile == NULL) {
    return;
  }
  ...
  if ((saving || loading) && session->IsSaveGameCompletedFromHandle(handle)) {
    ...
    if (saving) {
      ...
      if (parms.GetError() == SAVEGAME_E_FOLDER_NOT_FOUND ||
          parms.GetError() == SAVEGAME_E_FILE_NOT_FOUND) {
        profile->SaveSettings(true);
      } else if (parms.GetError() == SAVEGAME_E_CORRUPTED) {
        idLib::Warning("Profile corrupt, creating a new one...");
        common->Dialog().AddDialog(GDM_CORRUPT_PROFILE, DIALOG_CONTINUE, NULL,
                                   NULL, false);
        profile->SetDefaults();
        profile->SaveSettings(true);
...
idPlayerProfile* idProfileMgr::GetProfile() {
  ...
  bool loading =
      (profile->GetState() == idPlayerProfile::LOADING) ||
      (profile->GetRequestedState() == idPlayerProfile::LOAD_REQUESTED);
  if (loading) {
    return NULL;
  }
  ...
  if (GetProfile() != NULL) {
    ...
    if (profileSaveProcessor->InitSaveProfile(profile, "")) {
      profileSaveProcessor->AddCompletedCallback(
          MakeCallback(this, &idProfileMgr::OnSaveSettingsCompleted,
                       &profileSaveProcessor->GetParmsNonConst()));
      handle = session->GetSaveGameManager().ExecuteProcessor(
          profileSaveProcessor.get());
...
void idProfileMgr::LoadSettingsAsync() {
  if (profile != NULL && saveGame_enable.GetBool()) {
    if (profileLoadProcessor->InitLoadProfile(profile, "")) {
      // Skip the not found error because this might be the first time to play
      // the game!
      profileLoadProcessor->SetSkipSystemErrorDialogMask(
          SAVEGAME_E_FOLDER_NOT_FOUND | SAVEGAME_E_FILE_NOT_FOUND);

      profileLoadProcessor->AddCompletedCallback(
          MakeCallback(this, &idProfileMgr::OnLoadSettingsCompleted,
                       &profileLoadProcessor->GetParmsNonConst()));
      handle = session->GetSaveGameManager().ExecuteProcessor(
          profileLoadProcessor.get());
...
void idProfileMgr::OnLoadSettingsCompleted(idSaveLoadParms* parms) {
  ...
  idFile_SaveGame** profileFileContainer =
      FindFromGenericPtr(parms->files, SAVEGAME_PROFILE_FILENAME);
  idFile_SaveGame* profileFile =
      profileFileContainer == NULL ? NULL : *profileFileContainer;
  ...
  if (foundProfile) {
    ...
    unsigned int checksum = XXHash_Checksum(buffer.Ptr(), dataLength);
    if (originalChecksum != checksum) {
      idLib::Warning("Checksum: 0x%08x, originalChecksum: 0x%08x, size = %d",
                     checksum, originalChecksum, dataLength);
...
void idProfileMgr::OnSaveSettingsCompleted(idSaveLoadParms* parms) {
  ...
  if (parms->GetError() != SAVEGAME_E_NONE) {
    common->Dialog().AddDialog(GDM_PROFILE_SAVE_ERROR, DIALOG_CONTINUE, NULL,
                               NULL, false);
  }
  if (game) {
...
========================
*/
bool idSaveGameProcessorSaveProfile::InitSaveProfile(idPlayerProfile* profile_,
                                                     const char* folder) {
  // Serialize the profile and pass a file to the processor
  profileFile = new (TAG_SAVEGAMES)
      idFile_SaveGame(SAVEGAME_PROFILE_FILENAME,
                      SAVEGAMEFILE_BINARY | SAVEGAMEFILE_AUTO_DELETE);
  profileFile->MakeWritable();
  profileFile->SetMaxLength(MAX_PROFILE_SIZE);

  // Create a serialization object and let the game serialize the settings into
  // the buffer
  const int serializeSize =
      MAX_PROFILE_SIZE -
      8;  // -8 for checksum (all platforms) and length (on 360)
  ...
  unsigned int checksum = XXHash_Checksum(msg.GetReadData(), msg.GetSize());
  ...
  if (!idSaveGameProcessorSaveFiles::InitSave(
          folder, files, description, idSaveGameManager::PACKAGE_PROFILE)) {
    return false;
  }
...
========================
*/
bool idSaveGameProcessorLoadProfile::InitLoadProfile(idPlayerProfile* profile_,
                                                     const char* folder_) {
  ...
  parms.directory =
      AddSaveFolderPrefix(folder_, idSaveGameManager::PACKAGE_PROFILE);
...
bool Sys_SaveGameProfileCheck() {
  ...
  if (fileSystem->IsFolder(saveFolder, "fs_savePath") == FOLDER_YES) {
    idFileList* files =
        fileSystem->ListFiles(saveFolder, SAVEGAME_PROFILE_FILENAME);
