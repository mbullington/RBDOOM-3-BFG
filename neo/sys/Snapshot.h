...
class idSnapShot {
 public:
  ...
  static void PeekDeltaSequence(const char* deltaMem, int deltaSize,
                                int& sequence, int& baseSequence);

  // Reads a new object state packet, which is assumed to be delta compressed
  // against this snapshot
  bool ReadDeltaForJob(const char* deltaMem, int deltaSize, int visIndex,
  ...
  struct objectState_t {
    ...
    uint32 visMask;
    bool stale;  // easy way for clients to check if ss obj is stale. Probably
                 // temp till client side of vismask system is more fleshed out
    bool deleted;
  ...
  struct submitDeltaJobsInfo_t {
    ...
    int maxObjParms;   // Max parms (which will dictate how many objects can be
                       // processed)
    uint8* objMemory;  // Memory that objects were written out to
    objHeader_t* headers;  // Memory for headers
    int maxHeaders;
    int maxObjMemory;     // Max memory (which will dictate when syncs need to
                          // occur)
    lzwParm_t* lzwParms;  // Start of lzw parms
    int maxDeltaParms;    // Max lzw parms (which will dictate how many syncs we
                          // can have)

    idSnapShot*
        oldSnap;  // snap we are comparing this snap to (to produce a delta)
    int visIndex;
    int baseSequence;

    idSnapShot*
        templateStates;  // states for new snapObj that arent in old states
  ...
  bool WriteDelta(idSnapShot& old, int visIndex, idFile* file, int maxLength,
                  int optimalLength = 0);

  // Adds an object to the state, overwrites any existing object with the same
  // number
  objectState_t* S_AddObject(int objectNum, uint32 visMask, const idBitMsg& msg,
                             const char* tag = NULL) {
    return S_AddObject(objectNum, visMask, msg.GetReadData(), msg.GetSize(),
                       tag);
  }
  objectState_t* S_AddObject(int objectNum, uint32 visMask, const byte* buffer,
                             int size, const char* tag = NULL) {
    return S_AddObject(objectNum, visMask, (const char*)buffer, size, tag);
  }
  objectState_t* S_AddObject(int objectNum, uint32 visMask, const char* buffer,
                             int size, const char* tag = NULL);
  bool CopyObject(const idSnapShot& oldss, int objectNum,
                  bool forceStale = false);
  int CompareObject(const idSnapShot* oldss, int objectNum, int start = 0,
                    int end = 0, int oldStart = 0);
  ...
  bool GetObjectMsgByID(int objectNum, idBitMsg& msg,
                        bool ignoreIfStale = false) {
    return GetObjectMsgByIndex(FindObjectIndexByID(objectNum), msg,
                               ignoreIfStale) == objectNum;
  }
  ...
  objectState_t* GetTemplateState(int objNum, idSnapShot* templateStates,
                                  objectState_t* newState = NULL);
 ...
 private:
  ...
  objectState_t& FindOrCreateObjectByID(
      int objectNum);  // objIndex is optional parm for returning the index of
                       // the obj

  void SubmitObjectJob(
      const submitDeltaJobsInfo_t&
          submitDeltaJobsInfo,  // Struct containing parameters originally
                                // passed in to SubmitWriteDeltaToJobs
      objectState_t*
          newState,  // New obj state (can be NULL, which means deleted)
  ...
  void SubmitLZWJob(
      const submitDeltaJobsInfo_t&
          writeDeltaInfo,  // Struct containing parameters originally passed in
                           // to SubmitWriteDeltaToJobs
      objParms_t*&
      ...
      bool saveDictionary  // If true, this is the first of several calls which
                           // will be appended
  );

  void WriteObject(idFile* file, int visIndex, objectState_t* newState,
                   objectState_t* oldState, int& lastobjectNum);
