...
#define INVALID_LOBBY_USER_NAME \
  " "  // Used to be "INVALID" but Sony might not like that.
...
class idLobby : public idLobbyBase {
 public:
  ...
  void Initialize(lobbyType_t sessionType_,
                  class idSessionCallbacks* callbacks);
 ...
 public:
  enum reliablePlayerToPlayer_t {
    ...
    // RELIABLE_GAME_DATA is. I'm worried about using up the 0xff values we have
    // for reliable type, so I'm not going to reserve anything here just yet.
  ...
  enum reliableType_t {
    ...
    RELIABLE_PLAYER_TO_PLAYER_END =
        RELIABLE_PLAYER_TO_PLAYER_BEGIN + NUM_RELIABLE_PLAYER_TO_PLAYER,

    // * to * : misc reliable game data above this
    RELIABLE_GAME_DATA = RELIABLE_PLAYER_TO_PLAYER_END
  };

  // JGM: Reliable type in packet is a byte and there are a lot of reliable game
  // messages. Feel free to bump this up since it's arbitrary anyway, but take a
  // look at gameReliable_t. At the moment, both Doom and Rage have around 32
  // gameReliable_t values.
  ...
  static const int PEER_HEARTBEAT_IN_SECONDS =
      5;  // Make sure something was sent every 5 seconds, so we don't time out
  ...
  static const int MIN_CONNECT_FREQUENCY_IN_SECONDS =
      1;  // Min frequency of connection attempts
  static const int MAX_CONNECT_ATTEMPTS = 5;
  static const int BANDWIDTH_REPORTING_MAX =
      10240;  // make bps to report receiving (clamp if higher). For quantizing
  static const int BANDWIDTH_REPORTING_BITS =
      16;  // number of bits to use for bandwidth reporting
  static const int MAX_BPS_HISTORY =
      32;  // size of outgoing bps history to maintain for each client
  ...
  struct peer_t {
    ...
    bool inGame;       // true if this peer received the first snapshot, and is
                       // in-game
    int lastSnapTime;  // Last time a snapshot was sent on the network to this
                       // peer
    float snapHz;
    int lastProcTime;  // Used to determine when a packet was processed for
                       // sending to this peer
    int lastInBandProcTime;    // Last time a in-band packet was processed for
                               // sending
    int lastFragmentSendTime;  // Last time a fragment was sent out (fragments
                               // are processed msg's, waiting to be fully sent)
    // DG: use int instead of long for 64bit compatibility
    unsigned int
        networkChecksum;  // Checksum used to determine if a peer loaded the
                          // network resources the EXACT same as the server did
    // DG end
    int pauseSnapshots;
    ...
    int lastHeartBeat;
    int nextPing;  // next Sys_Milliseconds when I'll send this peer a
                   // RELIABLE_PING
    int lastPingRtt;
    bool needToSubmitPendingSnap;
    int lastSnapJobTime;  // Last time a snapshot was sent to the joblist for
                          // this peer

    int startResourceLoadTime;  // Used to determine how long a peer has been
                                // loading resources
    ...
    float receivedBps;  // peer's reported bps (they tell us their effective
                        // downstream)
    float maxSnapBps;
    float receivedThrottle;    // amount of accumlated time this client has been
                               // lagging behind
    int receivedThrottleTime;  // last time we did received based throttle
                               // calculations
    ...
    int bandwidthChallengeStartSendTime;  // time we sent first packet of bw
                                          // challenge to this peer
    int bandwidthTestLastSendTime;        // last time in MS we sent them a bw
                                          // challenge packet
  ...
  virtual lobbyUserID_t AllocLobbyUserSlotForBot(
      const char* botName);  // find a open user slot for the bot, and return
                             // the userID.
  virtual void RemoveBotFromLobbyUserList(
      lobbyUserID_t lobbyUserID);  // release the session user slot, so that it
                                   // can be claimed by a player, etc.
  virtual bool GetLobbyUserIsBot(lobbyUserID_t lobbyUserID)
      const;  // check to see if the lobby user is a bot or not
  ...
  struct reliablePlayerToPlayerHeader_t {
    ...
    // The network packets contain userIds, and Read/Write will translate from
    // userId to a
    //	sessionUserIndex.  The sessionUserIndex should be the same on all peers,
    //but the 	userId has to be used in case the target player quits while the
    //message is on the 	wire from the originating peer to the server.
    bool Read(idLobby* lobby, idBitMsg& msg);
    bool Write(idLobby* lobby, idBitMsg& msg);
  };

  int GetTotalOutgoingRate();  // returns total instant outgoing bandwidth in
                               // B/s

  // private:
 public:  // Turning this on for now, for the sake of getting this up and
          // running to see where things are
  // State functions
  void State_Idle();
  ...
  void HandleReliablePlayerToPlayerMsg(
  ...
  void HandleReliableMsg(int p, idBitMsg& msg,
                         const lobbyAddress_t* remoteAddress = NULL);
  ...
  lobbyUser_t* GetLobbyUserByID(lobbyUserID_t lobbyUserId,
                                bool ignoreLobbyType = false);
  ...
  // Convert an local userhandle to a session user (-1 if there is no session
  // user with this handle)
  int GetLobbyUserIndexByLocalUserHandle(
      const localUserHandle_t localUserHandle) const;

  // This takes a session user, and converts to a controller user
  idLocalUser* GetLocalUserFromLobbyUserIndex(int lobbyUserIndex);

  // Takes a controller user, and converts to a session user (will return NULL
  // if there is no session user for this controller user)
  ...
  // This function sycs the session users with the current list of of local
  // users on the signin manager. It will remove the session users that are
  // either no longer on the signin manager, or it will remove them if they are
  // no longer allowed to be in the session. If it finds a local users that are
  // not in a particular session, it will add that user if allowed.
  ...
  void QueueReliablePlayerToPlayerMessage(int fromSessionUserIndex,
                                          int toSessionUserIndex,
                                          reliablePlayerToPlayer_t type,
                                          const byte* data, int dataLen);
  ...
  bool IsBetterHost(int ping1, lobbyUserID_t userId1, int ping2,
  ...
  bool GetMigrationGameDataUser(lobbyUserID_t lobbyUserID, idBitMsg& msg,
                                bool reading);
  ...
  int lastPingValuesRecvTime;  // so clients can display something when server
                               // stops pinging
  ...
  bool needToDisplayMigrateMsg;  // If true, we migrated as host, so we need to
                                 // display the msg as soon as the lobby is
                                 // active
  gameDialogMessages_t
      migrationDlg;  // current migration dialog we should be showing

  uint8 migrateMsgFlags;  // cached match flags from the old game we migrated
                          // from, so we know what type of msg to display

  bool joiningMigratedGame;  // we are joining a migrated game and need to tell
                             // the session mgr if we succeed or fail
  ...
  bool
      bandwidthChallengeFinished;  // (HOST) test is finished and we received
                                   // results back from all peers (or timed out)
  int bandwidthChallengeNumGoodSeq;  // (PEER) num of good, in order packets we
                                     // recevieved

  int lastSnapBspHistoryUpdateSequence;

  void SaveDisconnectedUser(
      const lobbyUser_t& user);  // This is needed to get the a user's gamertag
                                 // after disconnection.
  ...
  struct migrationInfo_t {
    ...
    struct persistUntilGameEnds_t {
      ...
      // A generic blob of data that the gamechallenge (or anything else) can
      // read and write to for host migration
  ...
  struct disconnectedUser_t {
    lobbyUserID_t lobbyUserID;  // Locally generated to be unique, and
                                // internally keeps the local user handle
  ...
  idList<disconnectedUser_t>
      disconnectedUsers;  // List of users which were connected, but aren't
                          // anymore, for printing their name on the hud

  idStaticList<peer_t, MAX_PEERS>
      peers;  // Unique machines connected to this lobby
  ...
  bool respondToArbitrate;  // true when the host has requested us to arbitrate
                            // our session (for TYPE_GAME only)
...
class idSessionCallbacks {
 public:
  ...
  virtual bool BecomingHost(
      idLobby& lobby) = 0;  // Called when a lobby is about to become host
  virtual void BecameHost(
      idLobby& lobby) = 0;  // Called when a lobby becomes a host
  virtual bool BecomingPeer(
      idLobby& lobby) = 0;  // Called when a lobby is about to become peer
  virtual void BecamePeer(
      idLobby& lobby) = 0;  // Called when a lobby becomes a peer
  ...
  virtual void PrePickNewHost(idLobby& lobby, bool forceMe,
                              bool inviteOldHost) = 0;
  virtual bool PreMigrateInvite(idLobby& lobby) = 0;

  virtual void HandleOobVoiceAudio(const lobbyAddress_t& from,
                                   const idBitMsg& msg) = 0;

  // ConnectAndMoveToLobby is called when the lobby receives a
  // RELIABLE_CONNECT_AND_MOVE_TO_LOBBY
  virtual void ConnectAndMoveToLobby(idLobby::lobbyType_t destLobbyType,
                                     const lobbyConnectInfo_t& connectInfo,
                                     bool waitForPartyOk) = 0;

  virtual class idVoiceChatMgr* GetVoiceChat() = 0;

  virtual void HandleServerQueryRequest(lobbyAddress_t& remoteAddr,
                                        idBitMsg& msg, int msgType) = 0;
  virtual void HandleServerQueryAck(lobbyAddress_t& remoteAddr,
                                    idBitMsg& msg) = 0;
