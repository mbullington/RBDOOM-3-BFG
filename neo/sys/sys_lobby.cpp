...
idCVar net_checkVersion(
    "net_checkVersion", "0", CVAR_INTEGER,
    "Check for matching version when clients connect. 0: normal rules, 1: "
    "force check, otherwise no check (pass always)");
idCVar net_peerTimeoutInSeconds(
    "net_peerTimeoutInSeconds", "30", CVAR_INTEGER,
    "If the host hasn't received a response from a peer in this amount of time "
    "(in seconds), the peer will be disconnected.");
idCVar net_peerTimeoutInSeconds_Lobby(
    "net_peerTimeoutInSeconds_Lobby", "20", CVAR_INTEGER,
    "If the host hasn't received a response from a peer in this amount of time "
    "(in seconds), the peer will be disconnected.");

// NOTE - The snapshot exchange does the bandwidth challenge
idCVar net_bw_challenge_enable(
    "net_bw_challenge_enable", "0", CVAR_BOOL,
...
idCVar net_bw_test_packetSizeBytes("net_bw_test_packetSizeBytes", "1024",
                                   CVAR_INTEGER,
                                   "Size of each packet to send out");
idCVar net_bw_test_timeout(
    "net_bw_test_timeout", "500", CVAR_INTEGER,
    "MS after receiving a bw test packet that client will time out");
idCVar net_bw_test_host_timeout(
    "net_bw_test_host_timeout", "3000", CVAR_INTEGER,
    "How long host will wait in MS to hear bw results from peers");

idCVar net_bw_test_throttle_rate_pct(
    "net_bw_test_throttle_rate_pct", "0.80", CVAR_FLOAT,
    "Min rate % a peer must match in bandwidth challenge before being "
    "throttled. 1.0=perfect, 0.0=received nothing");
idCVar net_bw_test_throttle_byte_pct(
    "net_bw_test_throttle_byte_pct", "0.80", CVAR_FLOAT,
    "Min byte % a peer must match in bandwidth challenge before being "
    "throttled. 1.0=perfect (received everything) 0.0=Received nothing");
idCVar net_bw_test_throttle_seq_pct(
    "net_bw_test_throttle_seq_pct", "0.80", CVAR_FLOAT,
    "Min sequence % a peer must match in bandwidth test before being "
    "throttled. 1.0=perfect. This score will be more adversely affected by "
    "packet loss than byte %");

idCVar net_ignoreConnects("net_ignoreConnects", "0", CVAR_INTEGER,
                          "Test as if no one can connect to me. 0 = off, 1 = "
                          "ignore with no reply, 2 = send goodbye");
...
void idLobby::StartFinding(const idMatchParameters& parms_) {
  ...
  lobbyBackend = sessionCB->FindLobbyBackend(
      parms, sessionCB->GetPartyLobby().GetNumLobbyUsers(),
...
*/
void idLobby::Pump() {
  // Check the heartbeat of all our peers, make sure we shouldn't disconnect
  // from peers that haven't sent a heartbeat in awhile
  CheckHeartBeats();
...
void idLobby::ProcessSnapAckQueue() {
  ...
  while (snapDeltaAckQueue.Num() > 0 &&
         numProcessed < SNAP_ACKS_TO_PROCESS_PER_FRAME) {
    if (ApplySnapshotDeltaInternal(snapDeltaAckQueue[0].p,
                                   snapDeltaAckQueue[0].snapshotNumber)) {
      numProcessed++;
    }
...
void idLobby::Shutdown(bool retainMigrationInfo, bool skipGoodbye) {
  // Cancel host migration if we were in the process of it and this is the
  // session type that was migrating
  ...
  for (int p = 0; p < peers.Num(); p++) {
    if (peers[p].GetConnectionState() != CONNECTION_FREE) {
      SetPeerConnectionState(p, CONNECTION_FREE,
                             skipGoodbye);  // This will send goodbye's
    }
  }
...
void idLobby::HandlePacket(lobbyAddress_t& remoteAddress, idBitMsg fragMsg,
  ...
  if (peerNum >= 0) {
    // Update their heart beat (only if we've received a valid packet (we've
    // checked type == idPacketProcessor::RETURN_TYPE_NONE))
  ...
  if (type == idPacketProcessor::RETURN_TYPE_OOB) {
    if (userData == OOB_VOICE_AUDIO) {
      ...
      NET_VERBOSE_PRINT(
          "NET: Migration invite for session %s from %s (state = %s)\n",
      ...
      if (lobbyBackend != NULL &&
          lobbyBackend->GetState() != idLobbyBackend::STATE_FAILED &&
          lobbyBackend->IsOwnerOfConnectInfo(
              connectInfo))  // Ignore duplicate invites
      {
      ...
      if (migrationInfo.state == MIGRATE_NONE) {
        if (IsPeer() && host >= 0 && host < peers.Num() &&
            Sys_Milliseconds() - peers[host].lastHeartBeat > 8 * 1000) {
          // Force migration early if we get an invite, and it has been some
          // time since we've heard from the host
          PickNewHost();
        } else {
          idLib::Printf(
              "NET: Ignoring migration invite because we are not migrating "
              "%s\n",
              remoteAddress.ToString());
          SendGoodbye(
              remoteAddress);  // So they can remove us from their invite list
          return;
        }
      }
      ...
      if (!sessionCB->PreMigrateInvite(*this)) {
        NET_VERBOSE_PRINT(
            "NET: sessionCB->PreMigrateInvite( *this ) failed from %s\n",
            remoteAddress.ToString());
        return;
      }
      ...
      if (migrationInfo.state == MIGRATE_BECOMING_HOST) {
        ...
        if (inviteIndex != -1) {
          ...
          if (IsBetterHost(ping1, userId1, ping2, userId2)) {
            idLib::Printf(
                "NET: Ignoring migration invite from %s, since our ping is "
                "better (%i / %i).\n",
      ...
      ConnectTo(connectInfo,
                true);  // Pass in true for the invite flag, so we can connect
                        // to invite only lobby if we need to
      ...
      if (peerNum >= 0 && !peers[peerNum].IsConnected()) {
        NET_VERBOSE_PRINT(
            "NET: Resource list from host with no game connection: %i, %s\n",
      ...
      bool inOrder =
          (seqNum == 0 || peers[peerNum].bandwidthSequenceNum + 1 == seqNum);
      int timeSinceLast = 0;
      ...
      if (bandwidthChallengeStartTime <= 0) {
        ...
        peers[peerNum].bandwidthTestBytes =
            peers[peerNum].packetProc->GetIncomingBytes();
      } else {
        timeSinceLast = time - (bandwidthChallengeEndTime -
                                session->GetTitleStorageInt(
                                    "net_bw_test_timeout",
                                    net_bw_test_timeout.GetInteger()));
      }
      ...
      bandwidthChallengeEndTime =
          time + session->GetTitleStorageInt("net_bw_test_timeout",
                                             net_bw_test_timeout.GetInteger());
      NET_VERBOSE_PRINT(
          "  NET: %sRecevied OOB bandwidth test %d delta time: %d incoming "
          "rate: %.2f  incoming rate 2: %d\n",
          inOrder ? "^2" : "^1", seqNum, timeSinceLast,
          peers[peerNum].packetProc->GetIncomingRateBytes(),
          peers[peerNum].packetProc->GetIncomingRate2());
      peers[peerNum].bandwidthSequenceNum = seqNum;

    } else {
      NET_VERBOSE_PRINT("NET: Unknown oob packet %d from %s (%d)\n", userData,
                        remoteAddress.ToString(), peerNum);
    }
    ...
    if (msg.GetRemainingData() > 0 && loaded) {
      ...
      if (peerNum == host) {
        ...
        if (peers[peerNum].snapProc->ReceiveSnapshotDelta(
                deltaData, deltaLength, 0, sequence, baseseq, localSnap,
                fullSnap)) {
          ...
          if (sessionCB->GetState() != idSession::INGAME && sequence != -1) {
            ...
            ackmsg.WriteQuantizedUFloat<idLobby::BANDWIDTH_REPORTING_MAX,
                                        idLobby::BANDWIDTH_REPORTING_BITS>(
                incomingBPS);
        ...
        // If we are the host, we assume we only receive usercmds on the inband
        // channel
        ...
        lzwCompressor.Start(
            const_cast<byte*>(msg.GetReadData()) + msg.GetReadCount(),
            msg.GetRemainingData());
        lzwCompressor.ReadAgnostic(snapNum);
        lzwCompressor.ReadAgnostic(receivedBps_quantized);
        int usercmdSize =
            lzwCompressor.Read(usercmdBuffer, sizeof(usercmdBuffer), true);
        lzwCompressor.End();

        float receivedBps =
            (receivedBps_quantized /
             (float)(BIT(idLobby::BANDWIDTH_REPORTING_BITS) - 1)) *
...
void idLobby::State_Idle() {
  ...
  if (migrationInfo.persistUntilGameEndsData.hasGameData &&
      sessionCB->GetState() <= idSession::IDLE) {
    // This can happen with 'leaveGame' or 'disconnect' since those paths don't
    // go through endMatch This seems like an ok catch all place but there may
...
void idLobby::State_Create_Lobby_Backend() {
  ...
  if (migrationInfo.state == MIGRATE_BECOMING_HOST) {
    ...
    if (DETECT_SERVICE_DISCONNECT_TIMEOUT_IN_SECONDS > 0) {
      if (Sys_Milliseconds() - migrationInfo.migrationStartTime >
...
void idLobby::State_Searching() {
  ...
  const int maxSearchResultsToTry = session->GetTitleStorageInt(
      "net_maxSearchResultsToTry", net_maxSearchResultsToTry.GetInteger());
...
void idLobby::State_Connect_Hello_Wait() {
  ...
  const int timeoutMs =
      session->GetTitleStorageInt("net_connectTimeoutInSeconds",
  ...
  if (connectionAttempts < MAX_CONNECT_ATTEMPTS) {
    ...
    int resendTime =
        Max(MIN_CONNECT_FREQUENCY_IN_SECONDS,
...
void idLobby::StartCreating() {
  ...
  lobbyBackend = sessionCB->CreateLobbyBackend(
      parms, skillLevel, (idLobbyBackend::lobbyBackendType_t)lobbyType);
  ...
  for (int p = 0; p < peers.Num(); p++) {
    ...
    if (peers[p].address.Compare(remoteAddress)) {
      ...
      if (IsPeer()) {
        ...
        // Since we only roll the code between matches, we should only need to
        // look ahead a couple increments. Worse case, if the stars line up, the
        // client doesn't see the new sessionId, and times out, and gets booted.
        // This should be impossible though, since the timings won't be possible
        // considering how long it takes to end the match, and restart, and then
        // restart again.
        int numTries = 2;
...
========================
idLobby::FindAnyPeer
Find a peer when we don't know the session id, and we don't care since it's a
connectionless msg
========================
*/
...
int idLobby::AddPeer(const lobbyAddress_t& remoteAddress,
  ...
  assert(p == -1);  // When using session ID's, we SHOULDN'T find this
...
void idLobby::SendGoodbye(const lobbyAddress_t& remoteAddress, bool wasFull) {
  ...
  } else if (lobbyType == TYPE_GAME &&
             (sessionCB->GetSessionOptions() &
              idSession::OPTION_LEAVE_WITH_PARTY) &&
             !(parms.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER)) {
...
void idLobby::SetPeerConnectionState(int p, connectionState_t newState,
  ...
  if (peer.GetConnectionState() == newState) {
    idLib::Printf("NET: SetPeerConnectionState: Peer already in state %i\n",
                  newState);
    assert(0);  // This case means something is most likely bad, and it's the
                // programmers fault
    assert((peer.packetProc != NULL) == peer.IsActive());
    assert(((peer.snapProc != NULL) == peer.IsActive()) ==
           (actingGameStateLobbyType == lobbyType));
    return;
  }
...
========================
*/
void idLobby::QueueReliableMessage(int p, byte type, const byte* data,
                                   int dataLen) {
  if (!verify(p >= 0 && p < peers.Num())) {
    return;
  }
  ...
  if (peer.packetProc->NumQueuedReliables() > 2) {
    idLib::PrintfIf(
        false, "NET: peer.packetProc->NumQueuedReliables() > 2: %i (%i / %s)\n",
...
void idLobby::SendMatchParmsToPeers() {
  ...
  for (int p = 0; p < peers.Num(); p++) {
    ...
    QueueReliableMessage(p, RELIABLE_MATCH_PARMS, msg.GetReadData(),
                         msg.GetSize());
  }
}
...
bool idLobby::IsReliablePlayerToPlayerType(byte type) {
  return (type >= RELIABLE_PLAYER_TO_PLAYER_BEGIN) &&
         (type < RELIABLE_PLAYER_TO_PLAYER_END);
}
...
========================
*/
void idLobby::HandleReliablePlayerToPlayerMsg(int peerNum, idBitMsg& msg,
                                              int type) {
  ...
  const bool isForLocalPlayer =
      IsSessionUserIndexLocal(info.toSessionUserIndex);

  if (isForLocalPlayer) {
    HandleReliablePlayerToPlayerMsg(info, msg, type);
  } else if (IsHost()) {
    const int targetPeer =
        PeerIndexForSessionUserIndex(info.toSessionUserIndex);
    msg.RestoreReadState(c, b);
    ...
    idLib::Warning(
        "NET: Can't forward reliable message for remote player: I'm not the "
        "host");
  }
}
...
========================
*/
void idLobby::HandleReliablePlayerToPlayerMsg(
    const reliablePlayerToPlayerHeader_t& info, idBitMsg& msg,
    int reliableType) {
#if 0
...
========================
*/
void idLobby::SendConnectionLess(const lobbyAddress_t& remoteAddress, byte type,
  ...
  // Process the send
  idPacketProcessor::ProcessConnectionlessOutgoing(msg, processedMsg, lobbyType,
                                                   type);
...
void idLobby::SendConnectionRequest() {
  ...
  NET_VERBOSE_PRINT(
      ...
      hostAddress.ToString(), GetLobbyName(), peers[host].sessionID,
      connectionAttempts);
...
bool idLobby::ConnectToNextSearchResult() {
  ...
  // Clear the "Lobby was Full" dialog in case it's up, since we are going to
  // try to connect to a different lobby now
...
bool idLobby::CheckVersion(idBitMsg& msg, lobbyAddress_t peerAddress) {
  const unsigned int remoteChecksum =
      msg.ReadLong();  // DG: use int instead of long for 64bit compatibility
  ...
  if (net_checkVersion.GetInteger() == 1) {
    ...
    NET_VERBOSE_PRINT(
        "NET: Comparing handshake version - localChecksum = %u, remoteChecksum "
        "= %u\n",
...
bool idLobby::VerifyNumConnectingUsers(idBitMsg& msg) {
  ...
  NET_VERBOSE_PRINT(
      "NET: VerifyNumConnectingUsers %i users, %i free slots for %s\n",
  ...
  if (numUsers <= 0 || numUsers > MAX_PLAYERS - 1) {
    ...
    return false;
             ...
             sessionCB->GetState() >= idSession::GAME_LOBBY &&
             sessionCB->GetGameLobby().IsLobbyActive() && !IsMigrating()) {
    const int numFreeGameSlots = sessionCB->GetGameLobby().NumFreeSlots();

    if (numUsers > numFreeGameSlots) {
      NET_VERBOSE_PRINT(
          "NET: %i slots requested, but only %i are available on the active "
          "game session\n",
...
int idLobby::HandleInitialPeerConnection(idBitMsg& msg,
  ...
  if (!IsHost()) {
    NET_VERBOSE_PRINT(
        "NET: Got connectionless hello from peer %s (num %i) on session, and "
        "we are not a host\n",
  ...
  // See if this is a peer migrating to us, if so, remove them from our invite
  // list
  ...
  for (int i = migrationInfo.invites.Num() - 1; i >= 0; i--) {
    if (migrationInfo.invites[i].address.Compare(peerAddress, true)) {
      ...
      migrationInfo.invites.RemoveIndex(
          i);  // Remove this peer from the list, since this peer will now be
               // connected (or rejected, either way we don't want to keep
               // sending invites)
      migrationInvite = true;
      NET_VERBOSE_PRINT(
          "^2NET: Response from migration invite %s. GameData: %d\n",
          peerAddress.ToString(), migrationGameData);
    }
  }

  if (!MatchTypeIsJoinInProgress(parms.matchFlags) && lobbyType == TYPE_GAME &&
      migrationInfo.persistUntilGameEndsData.wasMigratedHost &&
      IsMigratedStatsGame() && !migrationInvite) {
    // No matter what, don't let people join migrated game sessions that are
    // going to continue on to the same game Not on invite list in a migrated
    // game session - bounce him
    NET_VERBOSE_PRINT(
        "NET: Denying game connection from %s since not on migration invite "
        "list\n",
  ...
  if (MatchTypeIsJoinInProgress(parms.matchFlags)) {
    ...
    // If this is for a party connection, make sure we are not in game, unless
    // this was for host migration invite
    if (!migrationInvite && lobbyType == TYPE_PARTY &&
        (sessionCB->GetState() == idSession::INGAME ||
         sessionCB->GetState() == idSession::LOADING)) {
      NET_VERBOSE_PRINT(
          "NET: Denying party connection from %s because we were already in a "
          "game\n",
          peerAddress.ToString());
      SendGoodbye(peerAddress);
      return -1;
  ...
  if (peerNum >= 0) {
    ...
    if (existingPeer.snapProc != NULL) {
      assert(
          lobbyType ==
          TYPE_GAME);  // Only games sessions should be creating snap processors
  ...
  // See if this was from an invite we sent out. If it wasn't, make sure we
  // aren't invite only
  const bool fromInvite = msg.ReadBool();

  if (!fromInvite && MatchTypeInviteOnly(parms.matchFlags)) {
    idLib::Printf(
        "NET: Denying user %s because they were not invited to an invite only "
        "match\n",
        peerAddress.ToString());
    SendGoodbye(peerAddress);
    return -1;
  ...
  if (!VerifyNumConnectingUsers(msg)) {
    NET_VERBOSE_PRINT(
        "NET: Denying connection from %s in session %s due to being out of "
        "user slots\n",
        peerAddress.ToString(), GetLobbyName());
  ...
  if (!verify(VerifyLobbyUserIDs(msg))) {
    NET_VERBOSE_PRINT(
        "NET: Denying connection from %s in session %s due to lobby id "
        "conflict\n",
        peerAddress.ToString(), GetLobbyName());
    SendGoodbye(peerAddress, true);
    return -1;
  ...
  // peerIndexOnHost (put this here so it shows up in search results when
  // finding out where it's used/referenced)
  ...
  QueueReliableMessage(peerNum, RELIABLE_HELLO, outmsg.GetReadData(),
                       outmsg.GetSize());

  if (MatchTypeIsJoinInProgress(parms.matchFlags)) {
    // If have an active game lobby, and someone joins our party, tell them to
    // join our game
    if (lobbyType == TYPE_PARTY &&
        sessionCB->GetState() >= idSession::GAME_LOBBY) {
    ...
    if ((lobbyType == TYPE_GAME || lobbyType == TYPE_GAME_STATE) &&
      ...
      if (sessionCB->GetState() == idSession::INGAME) {
        newPeer.pauseSnapshots =
            true;  // Since this player joined in progress, let game dictate
                   // when to start sending snaps
      }
      QueueReliableMessage(peerNum, idLobby::RELIABLE_START_LOADING);
    }
  } else {
    // If we are in a game lobby, and someone joins our party, tell them to join
    // our game
    if (lobbyType == TYPE_PARTY &&
        sessionCB->GetState() == idSession::GAME_LOBBY) {
      SendPeerMembersToLobby(peerNum, TYPE_GAME, false);
...
void idLobby::InitStateLobbyHost() {
  ...
  if (migrationInfo.state != MIGRATE_NONE) {
    migrationInfo.persistUntilGameEndsData.wasMigratedHost =
        true;  // InitSessionUsersFromLocalUsers needs to know this
  ...
  const int myUserIndex = GetLobbyUserIndexByLocalUserHandle(
      sessionCB->GetSignInManager().GetMasterLocalUserHandle());
  if (myUserIndex != -1) {
  ...
  // Make sure and register all the addresses of the invites we'll send out as
  // the new host
  if (migrationInfo.state != MIGRATE_NONE) {
    // Tell the session that we became the host, so the session mgr can adjust
    // state if needed
...
void idLobby::SendPeerMembersToLobby(int peerIndex, lobbyType_t destLobbyType,
  ...
  QueueReliableMessage(peerIndex, RELIABLE_CONNECT_AND_MOVE_TO_LOBBY,
...
void idLobby::SendPeerMembersToLobby(int peerIndex, lobbyType_t destLobbyType,
  ...
  SendPeerMembersToLobby(peerIndex, destLobbyType, connectInfo,
                         waitForOtherMembers);
}
...
uint32 idLobby::GetPartyTokenAsHost() {
  ...
  if (partyToken == 0) {
    ...
    // Don't do this at app startup (i.e. in the constructor) or it will be a
    // lot less random. DG: use int instead of long for 64bit compatibility
...
const char* idLobby::GetLobbyUserName(lobbyUserID_t lobbyUserID) const {
  ...
  if (user == NULL) {
    for (int i = 0; i < disconnectedUsers.Num(); i++) {
      if (disconnectedUsers[i].lobbyUserID.CompareIgnoreLobbyType(
              lobbyUserID)) {
...
========================
*/
void idLobby::HandleReliableMsg(
    int p, idBitMsg& msg, const lobbyAddress_t* remoteAddress /* = NULL */) {
  peer_t& peer = peers[p];
  ...
  if (reliableType == RELIABLE_HELLO) {
    ...
    // This message is sent from a peer requesting for a new user to join the
    // game lobby This will be sent while we are in a game lobby as a host.
    // otherwise, denied.
    ...
    if (sessionCB->GetState() == expectedState && IsHost() &&
        NumFreeSlots() > 0)  // This assumes only one user in the msg
    {
      // Add user to session, which will also forward the operation to all other
      // peers
      AddUsersFromMsg(msg, p);
    } else {
    ...
    // RELIABLE_PARTY_USER_CONNECT_REQUEST failed
    VERIFY_FROM_HOST(p, lobbyType, RELIABLE_PARTY_USER_CONNECT_DENIED);

    // Remove this user from the sign-in manager, so we don't keep trying to add
    // them
    if (!sessionCB->GetSignInManager().RemoveLocalUserByHandle(
            localUserHandle_t(msg.ReadLong()))) {
      NET_VERBOSE_PRINT(
          "NET: RELIABLE_PARTY_USER_CONNECT_DENIED, local user not found\n");
      return;
    }
    ...
    // This message is sent back from the host when users have connected, and we
    // need to update our lists to reflect that
    ...
    // This message is sent back from the host when users have diconnected, and
    // we need to update our lists to reflect that
    ...
    unsigned int networkChecksum =
        0;  // DG: use int instead of long for 64bit compatibility
    ...
    NET_VERBOSESNAPSHOT_PRINT_LEVEL(
        2, va("NET: Peer %d Ack'd snapshot %d\n", p, snapNum));
    ...
    // This message should be from the host of the game lobby, telling us (as
    // the host of the GameStateLobby) to start loading
    ...
    // Set a flag to do this later, since the lobby may not be in a state where
    // it can fulfil the request at the moment
    ...
    if (everyoneArbitrated) {
      NET_VERBOSE_PRINT(
          "NET: Everyone says they registered for arbitration, verifying\n");
    ...
    assert(
        destLobbyType >
        lobbyType);  // Make sure this is a proper transition (i.e. TYPE_PARTY
                     // moves to TYPE_GAME, TYPE_GAME moves to TYPE_GAME_STATE)
    ...
    if (sessionCB->GetState() != idSession::GAME_LOBBY) {
      idLib::Printf(
          "RELIABLE_PARTY_LEAVE_GAME_LOBBY: Not in a game lobby, ignoring.\n");
      return;
    }

    if (IsHost()) {
      idLib::Printf(
          "RELIABLE_PARTY_LEAVE_GAME_LOBBY: Host of party, ignoring.\n");
      return;
    }
...
void idLobby::DrawDebugNetworkHUD() const {
  ...
  renderSystem->DrawFilled(idVec4(0.0f, 0.0f, 0.0f, 0.7f), X_OFFSET - 10.0f,
                           curY - 10.0f, 1550,
                           (peers.Num() + numLines) * Y_SPACING + 20.0f);

  renderSystem->DrawSmallStringExt(
      idMath::Ftoi(X_OFFSET), idMath::Ftoi(curY),
      "# Peer                   | Sent kB/s | Recv kB/s | Sent MB | Recv MB | "
      "Ping   | L |  %  | R.NM | R.SZ | R.AK | T",
      colorGreen, false);
  curY += Y_SPACING;
  ...
  for (int p = 0; p < peers.Num(); p++) {
    ...
    renderSystem->DrawSmallStringExt(
        X_OFFSET, curY,
        va("%i %22s | %2.02f kB/s | %2.02f kB/s | %2.02f MB | %2.02f MB |%4i "
           "ms | %i | %i%% | %i | %i | %i | %2.2f / %2.2f / %i",
           ...
           peer.packetProc->NeedToSendReliableAck(), peer.snapHz,
           peer.maxSnapBps, peer.failedPingRecoveries),
        color, false);
    curY += Y_SPACING;
  }
  ...
  renderSystem->DrawSmallStringExt(
      X_OFFSET, curY,
      va("# %20s | %2.02f KB/s | %2.02f KB/s | %2.02f MB | %2.02f MB", "",
         totalSentKps, totalRecvKps, totalSentMB, totalRecvMB),
      color, false);
  curY += Y_SPACING;
  ...
  if (net_forceUpstream.GetFloat() != 0.0f) {
    ...
    renderSystem->DrawSmallStringExt(
        X_OFFSET, curY,
        va("Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s -> Effective "
           "%2.02f kB/s",
           queuedBytes, upstreamDropRate / 1024.0f,
           upstreamQueuedRate / 1024.0f,
           totalSentKps - (upstreamDropRate / 1024.0f) +
               (upstreamQueuedRate / 1024.0f)),
        color, false);
  }
}
...
void idLobby::DrawDebugNetworkHUD2() const {
  ...
  renderSystem->DrawFilled(idVec4(1.0f, 1.0f, 1.0f, 0.7f), X_OFFSET - 10.0f,
                           curY - 10.0f, 550,
                           (peers.Num() + 5) * Y_SPACING + 20.0f);

  renderSystem->DrawSmallStringExt(
      idMath::Ftoi(X_OFFSET), idMath::Ftoi(curY),
      va("State: %s. Local time: %d", stateName, Sys_Milliseconds()),
      colorGreen, false);
  curY += Y_SPACING;

  renderSystem->DrawSmallStringExt(
      idMath::Ftoi(X_OFFSET), idMath::Ftoi(curY),
      "Peer           | Sent kB/s | Recv kB/s | L | R | Resources", colorGreen,
      false);
  curY += Y_SPACING;
  ...
  for (int p = 0; p < peers.Num(); p++) {
    ...
    // right now I am mostly concerned about fragmentation and the latency
    // spikes it will cause
    idVec4 color = proc.TickFragmentAccumulator() ? colorRed : colorGreen;

    int rLoaded = peers[p].numResources;
    int rTotal = 0;

    // show the names of the clients connected to the server. Also make sure it
  ...
  renderSystem->DrawSmallStringExt(
      X_OFFSET, curY,
      va("Total | %2.02f KB/s | %2.02f KB/s", totalSentKps, totalRecvKps),
      colorGreen, false);
}
...
========================
*/
idCVar net_debughud3_bps_max(
    "net_debughud3_bps_max", "5120.0f", CVAR_FLOAT,
  ...
  for (int p = 0; p < peers.Num(); p++) {
    ...
    for (int i = 0; i < GRAPH_MAX; i++) {
      ...
      if (peer.debugGraphs[i] == NULL) {
        ...
        peer.debugGraphs[i]->SetPosition(X_OFFSET - 10.0f + width, curY - 10.0f,
                                         width, Y_SPACING * numLines);
      }
    ...
    renderSystem->DrawSmallStringExt(
        ...
        va("Peer %d - %s RTT %d %sPeerSnapRate: %d %s", p, GetPeerName(p),
           peer.lastPingRtt, throttled ? "^1" : "^2",
           peer.throttledSnapRate / 1000, throttled ? "^1Throttled" : ""),
        color, false);
    curY += Y_SPACING;

    renderSystem->DrawSmallStringExt(
        X_OFFSET, curY,
        va("SnapSeq %d  BaseSeq %d  Delta %d  Queue %d", snapSeq, snapBase,
           deltaSeq, snapProc->GetSnapQueueSize()),
        color, false);
    curY += Y_SPACING;
    ...
    if (net_forceUpstream.GetFloat() != 0.0f) {
      ...
      renderSystem->DrawSmallStringExt(
          X_OFFSET, curY,
          va("Queued: %d | Dropping: %2.02f kB/s Queue: %2.02f kB/s ",
             queuedBytes, upstreamDropRate / 1024.0f,
             upstreamQueuedRate / 1024.0f),
          color, false);
    }
    ...
    if (peer.debugGraphs[GRAPH_OUTGOING] != NULL) {
      ...
      lineColor.w = 0.5f;
      float outgoingRate =
          peer.sentBpsHistory[peer.receivedBpsIndex % MAX_BPS_HISTORY];
      // peer.packetProc->GetOutgoingRateBytes()
      peer.debugGraphs[GRAPH_OUTGOING]->SetValue(
          -1,
          idMath::ClampFloat(0.0f, 1.0f,
                             outgoingRate / net_debughud3_bps_max.GetFloat()),
          lineColor);
    }
    ...
    if (peer.debugGraphs[GRAPH_INCOMINGREPORTED] != NULL) {
      ...
      if (peer.packetProc->GetOutgoingRateBytes() >
          net_peer_throttle_bps_host_threshold.GetFloat()) {
        float pct =
            peer.packetProc->GetOutgoingRateBytes() > 0.0f
      ...
      peer.debugGraphs[GRAPH_INCOMINGREPORTED]->SetFillMode(
          idDebugGraph::GRAPH_LINE);
      peer.debugGraphs[GRAPH_INCOMINGREPORTED]->SetValue(
          -1,
          idMath::ClampFloat(
              0.0f, 1.0f, peer.receivedBps / net_debughud3_bps_max.GetFloat()),
          lineColor);
    }
...
void idLobby::CheckHeartBeats() {
  ...
  int timeoutInMs =
      session->GetTitleStorageInt("net_peerTimeoutInSeconds",
                                  net_peerTimeoutInSeconds.GetInteger()) *
      1000;

  if (sessionCB->GetState() < idSession::LOADING &&
      migrationInfo.state == MIGRATE_NONE) {
    // Use shorter timeout in lobby (TCR)
    timeoutInMs = session->GetTitleStorageInt(
  ...
  if (timeoutInMs > 0) {
    for (int p = 0; p < peers.Num(); p++) {
      if (peers[p].IsConnected()) {
        ...
        // (this seems reasonable since the reliable queue is set to 64
        // currently. In practice we should never have more than 3 or 4 queued)
        if (peers[p].packetProc->NumQueuedReliables() >
            idPacketProcessor::MAX_RELIABLE_QUEUE - 1) {
          peerTimeout = true;
        }
        ...
        if (peerTimeout) {
          ...
          if (IsHost()) {
            idLib::Printf(
                "Peer %i timed out for %s session @ %d (lastHeartBeat %d)\n", p,
                GetLobbyName(), time, peers[p].lastHeartBeat);
            DisconnectPeerFromSession(p);
          }
...
bool idLobby::IsLosingConnectionToHost() const {
  ...
  // (this seems reasonable since the reliable queue is set to 64 currently. In
  // practice we should never have more than 3 or 4 queued)
  if (peers[host].packetProc->NumQueuedReliables() >
      idPacketProcessor::MAX_RELIABLE_QUEUE / 2) {
    return true;
  }
...
bool idLobby::IsMigratedStatsGame() const {
  ...
  return migrationInfo.persistUntilGameEndsData.wasMigratedGame &&
         migrationInfo.persistUntilGameEndsData.hasGameData;
}
...
bool idLobby::ShouldRelaunchMigrationGame() const {
  ...
  if (!IsMigratedStatsGame()) {
    return false;  // If we are not migrating stats, we don't want to relaunch a
                   // new game
  }
...
bool idLobby::ShouldShowMigratingDialog() const {
  if (IsMigrating()) {
    return true;  // If we are in the process of truly migrating, then
  ...
  // We're either waiting on the server (which could be us) to relaunch, so show
  // the dialog
...
void idLobby::BeginBandwidthTest() {
  ...
  for (int p = 0; p < peers.Num(); ++p) {
    ...
    peers[p].bandwidthTestBytes =
        peers[p]
            .packetProc->GetOutgoingBytes();  // cache this off so we can see
                                              // the difference when we are done
  }
}
...
void idLobby::ServerUpdateBandwidthTest() {
  ...
  for (int i = 0; i < peers.Num(); ++i) {
    ...
    if (time - peer.bandwidthTestLastSendTime <
        session->GetTitleStorageInt("net_bw_test_interval",
                                    net_bw_test_interval.GetInteger())) {
      continue;
    }
    ...
    byte buffer[idPacketProcessor::
                    MAX_OOB_MSG_SIZE];  // <---- NOTE - When calling
                                        // ProcessOutgoingMsg with true for oob,
                                        // we can't go over this size
    idBitMsg msg(buffer, sizeof(buffer));

    msg.WriteLong(peer.bandwidthSequenceNum++);

    unsigned int randomSize =
        Min((unsigned int)(sizeof(buffer) - 12),
    ...
    unsigned int checksum = id::XXHash_Checksum(&buffer[8], randomSize);
    msg.WriteLong(checksum);

    NET_VERBOSE_PRINT(
        "Net: Sending bw challenge to peer %d time %d packet size %d\n", i,
        time, msg.GetSize());

    ProcessOutgoingMsg(i, buffer, msg.GetSize(), true, OOB_BANDWIDTH_TEST);

    if (session->GetTitleStorageInt("net_bw_test_numPackets",
                                    net_bw_test_numPackets.GetInteger()) > 0 &&
        peer.bandwidthSequenceNum >= net_bw_test_numPackets.GetInteger()) {
      int sentBytes = peers[i].packetProc->GetOutgoingBytes() -
                      peers[i].bandwidthTestBytes;  // FIXME: this won't include
                                                    // the last sent msg
      peers[i].bandwidthTestBytes =
          sentBytes;  // this now means total bytes sent (we don't care about
                      // starting/ending total bytes sent to peer)
      peers[i].bandwidthChallengeSendComplete = true;

      NET_VERBOSE_PRINT(
          "Sent enough packets to peer %d for bandwidth test in %dms. Total "
          "bytes: %d\n",
  ...
  if (sentAll) {
    if (bandwidthChallengeEndTime == 0) {
      ...
      bandwidthChallengeEndTime =
          time +
          session->GetTitleStorageInt("net_bw_test_host_timeout",
                                      net_bw_test_host_timeout.GetInteger());
      NET_VERBOSE_PRINT(
          "Net: finished sending BWC to peers. Waiting until %d to hear back\n",
          bandwidthChallengeEndTime);
    }
  }
  ...
  } else if (bandwidthChallengeEndTime != 0 &&
             bandwidthChallengeEndTime < time) {
    // Timed out waiting for someone - throttle them and move on
    NET_VERBOSE_PRINT(
        "^2Net: timed out waiting for bandwidth challenge results \n");
    for (int i = 0; i < peers.Num(); i++) {
      NET_VERBOSE_PRINT("  Peer[%d] %s. SentAll: %d  RecAll: %d\n", i,
                        GetPeerName(i), peers[i].bandwidthChallengeSendComplete,
                        peers[i].bandwidthChallengeResults);
      if (peers[i].bandwidthChallengeSendComplete &&
          !peers[i].bandwidthChallengeResults) {
...
void idLobby::ClientUpdateBandwidthTest() {
  ...
  // Its been long enough since we last received bw test msg. So lets send the
  // results to the server
  byte buffer[idPacketProcessor::MAX_MSG_SIZE];
  idBitMsg msg(buffer, sizeof(buffer));

  // Send total time it took to receive all the msgs
  // (note, subtract net_bw_test_timeout to get 'last recevied bandwidth test
  // packet')
  // (^^ Note if the last packet is fragmented and we never get it, this is
  // technically wrong!)
  int totalTime =
      (bandwidthChallengeEndTime -
       session->GetTitleStorageInt("net_bw_test_timeout",
                                   net_bw_test_timeout.GetInteger())) -
  ...
  // Note that sending the number of good packets is not enough. If the packets
  // going out are fragmented, and we drop fragments, the number of good
  // sequences will be lower than the bandwidth we actually received.
  int totalIncomingBytes = peers[host].packetProc->GetIncomingBytes() -
                           peers[host].bandwidthTestBytes;
  ...
  QueueReliableMessage(host, RELIABLE_BANDWIDTH_VALUES, msg.GetReadData(),
                       msg.GetSize());
}
...
void idLobby::HandleBandwidhTestValue(int p, idBitMsg& msg) {
  ...
  // This is the % of complete packets we received. If the packets used in the
  // BWC are big enough to fragment, then pctPackets will be lower than bytesPct
  // (we will have received a larger PCT of overall bandwidth than PCT of full
  // packets received). Im not sure if this is a useful distinction or not, but
  // it may be good to compare against for now.
  float pctPackets =
      peers[p].bandwidthSequenceNum > 0
  ...
  float bytesPct =
      peers[p].bandwidthTestBytes > 0
          ? (float)totalReceivedBytes / (float)peers[p].bandwidthTestBytes
          : -1.0f;

  // Calculate overall bandwidth for the test. That is, total amount received
  // over time. We may want to expand this to also factor in an average
  // instantaneous rate. For now we are mostly concerned with culling out poor
  // performing clients
  float peerKBS = -1.0f;
  ...
  int totalSendTime = peers[p].bandwidthTestLastSendTime -
                      peers[p].bandwidthChallengeStartSendTime;
  float outgoingKBS = -1.0f;
  if (verify(totalSendTime > 0)) {
    outgoingKBS =
        ((float)peers[p].bandwidthTestBytes / 1024.0f) / MS2SEC(totalSendTime);
  }

  float pctKBS = peerKBS / outgoingKBS;

  bool failedRate = (pctKBS < session->GetTitleStorageFloat(
                                  "net_bw_test_throttle_rate_pct",
                                  net_bw_test_throttle_rate_pct.GetFloat()));
  bool failedByte = (bytesPct < session->GetTitleStorageFloat(
                                    "net_bw_test_throttle_byte_pct",
                                    net_bw_test_throttle_byte_pct.GetFloat()));
  bool failedSeq = (pctPackets < session->GetTitleStorageFloat(
  ...
  idLib::Printf(
      "  %sEffective downstream: %.2fkbs (host: %.2fkbs) -> %.2f%%\n\n",
      (failedRate ? "^1" : "^2"), peerKBS, outgoingKBS, pctKBS);

  // If shittConnection(totalTime, totalGoodSeq/totalSeq,
...
void idLobby::SendPingValues() {
  ...
  for (int i = 0; i < peers.Num(); i++) {
    if (peers[i].IsConnected()) {
      QueueReliableMessage(i, RELIABLE_PING_VALUES, msg.GetReadData(),
                           msg.GetSize());
    }
  }
}
...
void idLobby::PumpPings() {
  if (IsHost()) {
    ...
    if (lastPingValuesRecvTime + PING_INTERVAL_MS + 1000 < Sys_Milliseconds() &&
        migrationInfo.state == MIGRATE_NONE) {
...
void idLobby::HandleReliablePing(int p, idBitMsg& msg) {
  ...
  if (IsHost()) {
    // we should probably verify here whether or not this ping was solicited or
    // not
    HandlePingReply(p, ping);
  } else {
    // this means the server is requesting a ping, so reply
    msg.RestoreReadState(c, b);
    QueueReliableMessage(p, RELIABLE_PING,
                         msg.GetReadData() + msg.GetReadCount(),
                         msg.GetRemainingData());
  }
}
...
void idLobby::HandlePingValues(idBitMsg& msg) {
  ...
  for (int userIndex = 0; userIndex < GetNumLobbyUsers(); ++userIndex) {
    lobbyUser_t* u = GetLobbyUser(userIndex);
    if (u->peerIndex != -1 &&
        verify(u->peerIndex >= 0 && u->peerIndex < MAX_PEERS)) {
  ...
  if (peerIndexOnHost != -1 &&
      verify(peerIndexOnHost >= 0 && peerIndexOnHost < MAX_PEERS)) {
...
bool idLobby::SendAnotherFragment(int p) {
  ...
  if (!peer.IsConnected())  // Not connected to any mode (party or game), so no
                            // need to send
  {
    return false;
  }
  ...
  if (!CanSendMoreData(p)) {
    return false;  // We need to throttle the sends so we don't saturate the
                   // connection
  }
...
void idLobby::ResendReliables(int p) {
  ...
  if (sessionCB->GetState() == idSession::INGAME) {
    ...
    if (lobbyType == TYPE_PARTY) {
      resendWait = Max(500, resendWait);  // party session does not need fast
                                          // frequency at all once in game
    }
  } else {
  ...
  if (peer.packetProc->NumQueuedReliables() > 0 ||
    ...
    ProcessOutgoingMsg(p, NULL, 0, false,
...
const char* idLobby::GetHostUserName() const {
  ...
  return GetPeerName(-1);  // This will just grab the first user with this
                           // peerIndex (which should be the host)
}
...
========================
*/
void idLobby::SendReliable(
    int type, idBitMsg& msg, bool callReceiveReliable /*= true*/,
  ...
  // I don't know if whole message is a good thing or a bad thing, but if the
  // passed message has been read from already, this is most likely not going to
  // do what the caller expects
  assert(msg.GetReadCount() + msg.GetReadBit() == 0);

  if (callReceiveReliable) {
    // NOTE: this will put the msg's read status to fully read - which is why
    // the assert check is above
  ...
  for (int i = 0; i < GetNumLobbyUsers(); ++i) {
    ...
    if ((sentPeerMask & (1 << user->peerIndex)) == 0) {
      QueueReliableMessage(user->peerIndex, idLobby::RELIABLE_GAME_DATA + type,
...
can only be used on the server. will take care of calling locally if addressed
to player 0
========================
*/
void idLobby::SendReliableToLobbyUser(lobbyUserID_t lobbyUserID, int type,
                                      idBitMsg& msg) {
  assert(lobbyType == GetActingGameStateLobbyType());
  assert(type < 256);  // QueueReliable only accepts a byte for message type
  ...
  if (peerIndex >= 0) {
    // will send the remainder of a message that was started reading through,
    // but not handling a partial byte read
    assert(msg.GetReadBit() == 0);
    QueueReliableMessage(peerIndex, idLobby::RELIABLE_GAME_DATA + type,
                         msg.GetReadData() + msg.GetReadCount(),
                         msg.GetRemainingData());
  } else {
    common->NetReceiveReliable(-1, type, msg);
...
void idLobby::SendReliableToHost(int type, idBitMsg& msg) {
  ...
  if (IsHost()) {
    ...
    // but not handling a partial byte read
    assert(msg.GetReadBit() == 0);
    QueueReliableMessage(host, idLobby::RELIABLE_GAME_DATA + type,
                         msg.GetReadData() + msg.GetReadCount(),
                         msg.GetRemainingData());
  }
}
...
*/
idLobby::reliablePlayerToPlayerHeader_t::reliablePlayerToPlayerHeader_t()
    : fromSessionUserIndex(-1), toSessionUserIndex(-1) {}
...
========================
*/
bool idLobby::reliablePlayerToPlayerHeader_t::Read(idLobby* lobby,
                                                   idBitMsg& msg) {
  assert(lobby != NULL);
...
========================
*/
bool idLobby::reliablePlayerToPlayerHeader_t::Write(idLobby* lobby,
                                                    idBitMsg& msg) {
  if (!verify(lobby->GetLobbyUser(fromSessionUserIndex) != NULL)) {
    return false;
  }
...
bool idLobby::AllPeersInGame() const {
  assert(lobbyType ==
         GetActingGameStateLobbyType());  // This function doesn't make sense on
                                          // a party lobby currently
...
int idLobby::PeerIndexFromLobbyUser(lobbyUserID_t lobbyUserID) const {
  ...
  if (user == NULL) {
    // This needs to be OK for bot support ( or else add bots at the session
    // level )
    return -1;
  }
