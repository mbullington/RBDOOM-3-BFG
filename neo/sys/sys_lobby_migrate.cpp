...
idCVar net_migration_forcePeerAsHost(
    "net_migration_forcePeerAsHost", "-1", CVAR_INTEGER,
    "When set to >-1, it forces that peer number to be the new host during "
    "migration");
...
========================
*/
bool idLobby::IsBetterHost(int ping1, lobbyUserID_t userId1, int ping2,
                           lobbyUserID_t userId2) {
  if (lobbyType == TYPE_PARTY) {
    return userId1 <
           userId2;  // Only use user id for party, since ping doesn't matter
  }
...
void idLobby::UpdateHostMigration() {
  ...
  if (migrationInfo.state == MIGRATE_PICKING_HOST) {
    const int MIGRATION_PICKING_HOST_TIMEOUT_IN_SECONDS =
        20;  // FIXME: set back to 5 // Give other hosts 5 seconds

    if (time - migrationInfo.migrationStartTime >
        session->GetTitleStorageInt("MIGRATION_PICKING_HOST_TIMEOUT_IN_SECONDS",
  ...
  // See if we are a new migrated host that needs to invite the original members
  // back
  ...
  if (lobbyBackend == NULL ||
      lobbyBackend->GetState() != idLobbyBackend::STATE_READY) {
    return;
  }
  ...
  const int MIGRATION_TIMEOUT_IN_SECONDS =
      30;  // FIXME: setting to 30 for dev purposes. 10 seems more reasonable.
           // Need to make unloading game / loading lobby async
  const int MIGRATION_INVITE_TIME_IN_SECONDS = 2;

  if (migrationInfo.invites.Num() == 0 ||
      time - migrationInfo.migrationStartTime >
          session->GetTitleStorageInt("MIGRATION_TIMEOUT_IN_SECONDS",
                                      MIGRATION_TIMEOUT_IN_SECONDS) *
              1000) {
    // Either everyone acked, or we timed out, just keep who we have, and stop
    // sending invites
    EndMigration();
    return;
  }
  ...
  for (int i = 0; i < migrationInfo.invites.Num(); i++) {
    if (time - migrationInfo.invites[i].lastInviteTime <
        session->GetTitleStorageInt("MIGRATION_INVITE_TIME_IN_SECONDS",
                                    MIGRATION_INVITE_TIME_IN_SECONDS) *
            1000) {
    ...
    SendConnectionLess(migrationInfo.invites[i].address, OOB_MIGRATE_INVITE,
...
void idLobby::BuildMigrationInviteList(bool inviteOldHost) {
  ...
  // Build a list of addresses we will send invites to (gather all unique remote
  // addresses from the session user list)
  for (int i = 0; i < GetNumLobbyUsers(); i++) {
    ...
    if (IsSessionUserIndexLocal(i)) {
      ...
      NET_VERBOSE_PRINT(
          "^2NET: Migration game data for local user is index %d \n",
          user->migrationGameData);
...
void idLobby::PickNewHostInternal(bool forceMe, bool inviteOldHost) {
  ...
  for (int i = 0; i < GetNumLobbyUsers(); i++) {
    ...
    if (bestUserIndex == -1 ||
        IsBetterHost(user->pingMs, user->lobbyUserID, bestPingMs, bestUserId)) {
      bestUserIndex = i;
  ...
  migrationInfo.persistUntilGameEndsData.wasMigratedGame =
      sessionCB->GetState() == idSession::INGAME;

  if (bestUserIndex == -1)  // This can happen if we call PickNewHost on an
                            // lobby that was Shutdown
  {
    NET_VERBOSE_PRINT(
        "MIGRATION: PickNewHost was called on an lobby that was Shutdown\n");
    BecomeHost();
    return;
  }
  ...
  bool bestWasLocal = IsSessionUserIndexLocal(
      bestUserIndex);                  // Check before shutting down the lobby
  migrateMsgFlags = parms.matchFlags;  // Save off match parms

  // Build invite list
  BuildMigrationInviteList(inviteOldHost);

  // If the best user is on this machine, then we become the host now,
  // otherwise, wait for a new host to contact us
...
This gets called when we are done migrating, and invites will no longer be sent
out.
========================
*/
...
This will setup the passed in idBitMsg to either read or write from the global
migration game data buffer
========================
*/
bool idLobby::GetMigrationGameData(idBitMsg& msg, bool reading) {
  if (reading) {
    ...
    msg.InitRead(migrationInfo.persistUntilGameEndsData.gameData,
                 sizeof(migrationInfo.persistUntilGameEndsData.gameData));
  } else {
    migrationInfo.persistUntilGameEndsData.hasGameData = true;
    memset(migrationInfo.persistUntilGameEndsData.gameData, 0,
           sizeof(migrationInfo.persistUntilGameEndsData.gameData));
    msg.InitWrite(migrationInfo.persistUntilGameEndsData.gameData,
                  sizeof(migrationInfo.persistUntilGameEndsData.gameData));
  }
...
This will setup the passed in idBitMsg to either read or write from the user's
migration game data buffer
========================
*/
bool idLobby::GetMigrationGameDataUser(lobbyUserID_t lobbyUserID, idBitMsg& msg,
                                       bool reading) {
  ...
  if (u != NULL) {
    if (reading) {
      ...
      if (u->migrationGameData >= 0 && u->migrationGameData < MAX_PLAYERS) {
        msg.InitRead(
            migrationInfo.persistUntilGameEndsData
                .gameDataUser[u->migrationGameData],
            sizeof(migrationInfo.persistUntilGameEndsData.gameDataUser[0]));
      } else {
        // We don't have migration data for this user
        idLib::Warning("No migration data for user %d in a migrated game (%d)",
                       userNum, u->migrationGameData);
        return false;
      }
    } else {
      // Writing
      ...
      memset(migrationInfo.persistUntilGameEndsData.gameDataUser[userNum], 0,
             sizeof(migrationInfo.persistUntilGameEndsData.gameDataUser[0]));
      msg.InitWrite(
          migrationInfo.persistUntilGameEndsData.gameDataUser[userNum],
          sizeof(migrationInfo.persistUntilGameEndsData.gameDataUser[0]));
    }
    return true;
  }
...
void idLobby::HandleMigrationGameData(idBitMsg& msg) {
  // Receives game migration data from the server. Just save off the raw data.
  // If we ever become host we'll let the game code read that chunk in (we can't
  // do anything with it now anyways: we don't have entities or any server code
  // to read it in to)
  migrationInfo.persistUntilGameEndsData.hasGameData = true;

  // Reset each user's migration game data. If we don't receive new data for
  // them in this msg, we don't want to use the old data
  for (int i = 0; i < GetNumLobbyUsers(); i++) {
  ...
  msg.ReadData(migrationInfo.persistUntilGameEndsData.gameData,
               sizeof(migrationInfo.persistUntilGameEndsData.gameData));
  ...
  for (int i = 0; i < numUsers; i++) {
    ...
    if (user != NULL) {
      ...
      msg.ReadData(
          migrationInfo.persistUntilGameEndsData.gameDataUser[dataIndex],
          sizeof(
              migrationInfo.persistUntilGameEndsData.gameDataUser[dataIndex]));
      dataIndex++;
    }
...
void idLobby::SendMigrationGameData() {
  ...
  // Write global data
  msg.WriteData(&migrationInfo.persistUntilGameEndsData.gameData,
                sizeof(migrationInfo.persistUntilGameEndsData.gameData));
  ...
  for (int userIndex = 0; userIndex < GetNumLobbyUsers(); ++userIndex) {
    ...
    msg.WriteData(migrationInfo.persistUntilGameEndsData
                      .gameDataUser[u->migrationGameData],
                  sizeof(migrationInfo.persistUntilGameEndsData
                             .gameDataUser[u->migrationGameData]));
  }
  ...
  for (int i = 0; i < peers.Num(); i++) {
    ...
    if (peers[peerToSend].IsConnected() && peers[peerToSend].loaded) {
      if (peers[peerToSend].packetProc->NumQueuedReliables() >
          idPacketProcessor::MAX_RELIABLE_QUEUE / 2) {
        // This is kind of a hack for development so we don't DC clients by
        // sending them too many reliable migration messages when they aren't
        // responding. Doesn't seem like a horrible thing to have in a shipping
        // product but is not necessary.
        NET_VERBOSE_PRINT(
            "NET: Skipping reliable game migration data msg because client "
            "reliable queue is > half full\n");

      } else {
        if (net_migration_debug.GetBool()) {
          idLib::Printf(
              "NET: Sending migration game data to peer %d. size: %d\n",
              peerToSend, msg.GetSize());
        }
        QueueReliableMessage(peerToSend, RELIABLE_MIGRATION_GAME_DATA,
