...
#if defined(_WIN32)
idCVar r_useOpenGL45("r_useOpenGL45", "1", CVAR_INTEGER,
                     "0 = OpenGL 4.0, 1 = OpenGL 4.5 compatibility profile, 2 "
                     "= OpenGL 4.5 core profile",
                     0, 2);
#else
idCVar r_useOpenGL45("r_useOpenGL45", "2", CVAR_INTEGER,
                     "0 = OpenGL 4.0, 1 = OpenGL 4.5 compatibility profile, 2 "
                     "= OpenGL 4.5 core profile",
                     0, 2);
#endif
...
void GLW_CheckWGLExtensions(HDC hDC) {
  ...
  if (GLEW_OK != glewResult) {
    ...
    common->Printf(
        "^GLW_CheckWGLExtensions() - GLEW could not load OpenGL subsystem: %s",
  ...
  // wglSwapIntervalEXT = ( PFNWGLSWAPINTERVALEXTPROC ) GLimp_ExtensionPointer(
  // "wglSwapIntervalEXT" );
...
static HGLRC CreateOpenGLContextOnDC(const HDC hdc, const bool debugContext) {
  ...
  // RB: for GLintercept 1.2.0 or otherwise we can't diff the framebuffers using
  // the XML log
  ...
  for (int i = 0; i < 2; i++) {
    ...
    const int glProfileMask =
        (useCoreProfile != 0) ? WGL_CONTEXT_PROFILE_MASK_ARB : 0;
    const int glProfile =
        (useCoreProfile == 1)
            ? WGL_CONTEXT_COMPATIBILITY_PROFILE_BIT_ARB
            : ((useCoreProfile == 2) ? WGL_CONTEXT_CORE_PROFILE_BIT_ARB : 0);
...
static int GLW_ChoosePixelFormat(const HDC hdc, const int multisamples,
  ...
  int iAttributes[] = {WGL_SAMPLE_BUFFERS_ARB,
                       ((multisamples > 1) ? 1 : 0),
                       WGL_SAMPLES_ARB,
                       multisamples,
  ...
  int pixelFormat;
  UINT numFormats;
  if (!wglChoosePixelFormatARB(hdc, iAttributes, fAttributes, 1, &pixelFormat,
                               &numFormats)) {
    return -1;
  }
  return pixelFormat;
...
static bool GLW_InitDriver(glimpParms_t parms) {
  ...
  if (WGLEW_ARB_pixel_format) {
    win32.pixelformat =
        GLW_ChoosePixelFormat(win32.hDC, parms.multiSamples, parms.stereo);
  } else {
...
static const char* GetDisplayName(const int deviceNum) {
  ...
  if (!EnumDisplayDevices(0,  // lpDevice
                          deviceNum, &device, 0 /* dwFlags */)) {
    return NULL;
  }
...
========================
*/
static bool GetDisplayCoordinates(const int deviceNum, int& x, int& y,
                                  int& width, int& height, int& displayHz) {
  ...
  if (!EnumDisplayDevices(deviceName.c_str(), 0, &monitor, 0 /* dwFlags */)) {
    return false;
  }
...
void DumpAllDisplayDevices() {
  ...
  for (int deviceNum = 0;; deviceNum++) {
    ...
    for (int monitorNum = 0;; monitorNum++) {
      DISPLAY_DEVICE monitor = {};
      monitor.cb = sizeof(monitor);
      if (!EnumDisplayDevices(device.DeviceName, monitorNum, &monitor,
                              0 /* dwFlags */)) {
        break;
      }
      ...
      if (!EnumDisplaySettings(device.DeviceName, ENUM_CURRENT_SETTINGS,
                               &currentDevmode)) {
        common->Printf(
            "ERROR:  EnumDisplaySettings(ENUM_CURRENT_SETTINGS) failed!\n");
      }
      common->Printf("          -------------------\n");
      ...
      if (!EnumDisplaySettings(device.DeviceName, ENUM_REGISTRY_SETTINGS,
                               &registryDevmode)) {
        common->Printf(
            "ERROR:  EnumDisplaySettings(ENUM_CURRENT_SETTINGS) failed!\n");
      }
...
bool R_GetModeListForDisplay(const int requestedDisplayNum,
  ...
  for (int displayNum = requestedDisplayNum;; displayNum++) {
    ...
    DISPLAY_DEVICE monitor;
    monitor.cb = sizeof(monitor);
    if (!EnumDisplayDevices(device.DeviceName, 0, &monitor, 0 /* dwFlags */)) {
      continue;
    }
...
====================
*/
static bool GLW_GetWindowDimensions(const glimpParms_t parms, int& x, int& y,
                                    int& w, int& h) {
  //
...
#if !defined(USE_VULKAN)
  // Check to see if we can get a stereo pixel format, even if we aren't going
  // to use it, so the menu option can be
...
static bool GLW_ChangeDislaySettingsIfNeeded(glimpParms_t parms) {
  ...
  if (!GetDisplayCoordinates(parms.fullScreen - 1, x, y, width, height,
                             displayHz)) {
    return false;
  }
  if (width == parms.width && height == parms.height &&
      (displayHz == parms.displayHz || parms.displayHz == 0)) {
    return true;
  }
  ...
  int cdsRet;
  if ((cdsRet = ChangeDisplaySettingsEx(deviceName, &dm, NULL, CDS_FULLSCREEN,
...
#if !defined(USE_VULKAN)
  cmdSystem->AddCommand("testSwapBuffers", GLimp_TestSwapBuffers, CMD_FL_SYSTEM,
                        "Times swapbuffer options");

  common->Printf(
      "Initializing OpenGL subsystem with multisamples:%i stereo:%i "
      "fullscreen:%i\n",
