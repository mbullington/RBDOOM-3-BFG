...
void WIN_Sizing(WORD side, RECT* rect) {
  if (!renderSystem->IsInitialized() || renderSystem->GetWidth() <= 0 ||
      renderSystem->GetHeight() <= 0) {
    return;
  }
  ...
  switch (side) {
    case WMSZ_LEFT:
      ...
      rect->bottom = rect->top +
                     idMath::ClampInt(minHeight, maxHeight, height) +
                     decoHeight;
      break;
    case WMSZ_RIGHT:
    ...
    case WMSZ_BOTTOMRIGHT:
      ...
      rect->right =
          rect->left + idMath::ClampInt(minWidth, maxWidth, width) + decoWidth;
      break;
    case WMSZ_TOP:
    case WMSZ_TOPRIGHT:
    ...
    case WMSZ_BOTTOMLEFT:
      rect->bottom = rect->top + height + decoHeight;
      rect->left =
          rect->right - idMath::ClampInt(minWidth, maxWidth, width) - decoWidth;
      break;
    case WMSZ_TOPLEFT:
      rect->top = rect->bottom - height - decoHeight;
      rect->left =
  ...
  switch (uMsg) {
      ...
      if (renderSystem->IsInitialized()) {
        ...
        if (::GetClientRect(win32.hWnd, &rect)) {
          if (rect.right > rect.left && rect.bottom > rect.top) {
            ...
            ImGuiHook::NotifyDisplaySizeChanged(glConfig.nativeScreenWidth,
    ...
    case WM_KEYUP:
      ...
      if (key == K_PRINTSCREEN) {
        ...
        break;
      } else if (key == K_LCTRL || key == K_LALT || key == K_RCTRL ||
                 key == K_RALT) {
        // let direct-input handle this because windows sends Alt-Gr
        // as two events (ctrl then alt)
        break;
    ...
    case WM_CHAR:
      // DG: make sure it's an utf-16 non-surrogate character (and thus a valid
      // utf-32 character as well)
      // TODO: will there ever be two messages with surrogate characters that
      // should be combined?
    ...
    case WM_MOUSEMOVE: {
      ...
      const bool isShellActive =
          (game && (game->Shell_IsActive() || game->IsPDAOpen()));
