...
idCVar Win32Vars_t::sys_cpustring("sys_cpustring", "detect",
                                  CVAR_SYSTEM | CVAR_INIT, "");
idCVar Win32Vars_t::in_mouse("in_mouse", "1", CVAR_SYSTEM | CVAR_BOOL,
                             "enable mouse input");
idCVar Win32Vars_t::win_allowAltTab("win_allowAltTab", "0",
                                    CVAR_SYSTEM | CVAR_BOOL,
                                    "allow Alt-Tab when fullscreen");
idCVar Win32Vars_t::win_notaskkeys("win_notaskkeys", "0",
                                   CVAR_SYSTEM | CVAR_INTEGER,
                                   "disable windows task keys");
idCVar Win32Vars_t::win_username("win_username", "", CVAR_SYSTEM | CVAR_INIT,
                                 "windows user name");
idCVar Win32Vars_t::win_outputEditString("win_outputEditString", "1",
                                         CVAR_SYSTEM | CVAR_BOOL, "");
idCVar Win32Vars_t::win_viewlog("win_viewlog", "0", CVAR_SYSTEM | CVAR_INTEGER,
                                "");
idCVar Win32Vars_t::win_timerUpdate(
    "win_timerUpdate", "0", CVAR_SYSTEM | CVAR_BOOL,
    "allows the game to be updated while dragging the window");
idCVar Win32Vars_t::win_allowMultipleInstances(
    "win_allowMultipleInstances", "0", CVAR_SYSTEM | CVAR_BOOL,
    "allow multiple instances running concurrently");

// RB
idCVar Win32Vars_t::sys_useSteamPath(
    "sys_useSteamPath", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_ARCHIVE,
    "Look for Steam Doom 3 BFG path instead of local installation");
idCVar Win32Vars_t::sys_useGOGPath(
    "sys_useGOGPath", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_ARCHIVE,
    "Look for GOG Doom 3 BFG path instead of local installation");
...
void Sys_SetDPIAwareness() {
  ...
  (D3_PROCESS_DPI_AWARENESS dpiAwareness) = NULL;

  HINSTANCE userDLL = LoadLibrary("USER32.DLL");
  if (userDLL) {
    SetProcessDPIAware =
        (BOOL(WINAPI*)(void))GetProcAddress(userDLL, "SetProcessDPIAware");
  }

  HINSTANCE shcoreDLL = LoadLibrary("SHCORE.DLL");
  if (shcoreDLL) {
    SetProcessDpiAwareness =
        (HRESULT(WINAPI*)(YQ2_PROCESS_DPI_AWARENESS))GetProcAddress(
...
========================
*/
void Sys_Launch(const char* path, idCmdArgs& args, void* data,
...
========================
*/
const char* Sys_GetCmdLine() { return sys_cmdline; }
...
void Sys_ReLaunch() {
  ...
  // DG: we don't have function arguments in Sys_ReLaunch() anymore, everyone
  // only passed
  //     the command-line +" +set com_skipIntroVideos 1" anyway and it was
  //     painful on POSIX systems so let's just add it here.
...
==============
*/
void Sys_Sleep(int msec) { Sleep(msec); }
...
==============
*/
bool Sys_IsWindowVisible() { return (::IsWindowVisible(win32.hWnd) != 0); }
...
========================
*/
bool Sys_Rmdir(const char* path) { return _rmdir(path) == 0; }
...
static int WidePath2ASCI(char* dst, size_t size, const WCHAR* src) {
  ...
  if (default_char) {
    // The following lines implement a horrible hack to connect the UTF-16
    // WinAPI to the ASCII doom3 strings. While this should work in most cases,
    // it'll fail if the "Windows to DOS filename translation" is switched off.
...
// NOTE: FOLIDERID_SavedGames is already exported from in shell32.dll in
// Windows 7.  We can only detect the compiler version, but that doesn't doesn't
// tell us which version of the OS we're linking against. This GUID value should
// never change, so we name it something other than FOLDERID_SavedGames to get
// around this problem.
const GUID FOLDERID_SavedGames_IdTech5 = {
    0x4c5c32ff,
    0xbb9d,
    0x43b0,
...
const char* Sys_DefaultSavePath() {
  ...
  if (hShell) {
    SHGetKnownFolderPath_t SHGetKnownFolderPath =
        (SHGetKnownFolderPath_t)GetProcAddress(hShell, "SHGetKnownFolderPath");
    if (SHGetKnownFolderPath) {
      wchar_t* path;

      // RB FIXME?
#if defined(__MINGW32__)
      if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_SavedGames_IdTech5,
                                         CSIDL_FLAG_CREATE, 0, &path)))
#else
      if (SUCCEEDED(SHGetKnownFolderPath(
              FOLDERID_SavedGames_IdTech5,
              CSIDL_FLAG_CREATE | CSIDL_FLAG_PER_USER_INIT, 0, &path)))
#endif
      // RB end
      {
...
#if defined(__MINGW32__)
    SHGetFolderPath(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL, 1,
                    savePath);
#else
    SHGetFolderPath(NULL, CSIDL_PERSONAL | CSIDL_FLAG_CREATE, NULL,
                    SHGFP_TYPE_CURRENT, savePath);
#endif
    // RB end
...
========================
*/
static void ExecOutputFn(const char* text) { idLib::Printf(text); }
...
bool Sys_Exec(const char* appPath, const char* workingPath, const char* args,
  ...
  // we duplicate args here so we can concatenate the exe name and args into a
  // single command line
  const char* imageName = appPath;
  char* cmdLine = NULL;
  {
    // if we have any args, we need to copy them to a new buffer because
    // CreateProcess modifies the command line buffer.
    if (args != NULL) {
      if (appPath != NULL) {
        int len = idStr::Length(args) + idStr::Length(appPath) +
                  1 /* for space */ + 1 /* for NULL terminator */ +
                  2 /* app quotes */;
        cmdLine = (char*)Mem_Alloc(len, TAG_TEMP);
        // note that we're putting quotes around the appPath here because when
        // AAS2.exe gets an app path with spaces in the path
        // "w:/zion/build/win32/Debug with Inlines/AAS2.exe" it gets more than
        // one arg for the app name, which it most certainly should not, so I am
        // assuming this is a side effect of using CreateProcess.
      ...
      // the image name should always be NULL if we have command line arguments
      // because it is already prefixed to the command line.
      imageName = NULL;
    }
  }

  BOOL result = CreateProcess(imageName, (LPSTR)cmdLine, NULL, NULL, TRUE, 0,
                              NULL, workingPath, &si, &pi);
  ...
  } else if (waitMS >= 0)  // if waitMS == -1, don't wait for process to exit
  {
    ...
    for (;;) {
      ...
      DWORD bytesLeft = 0;
      BOOL ok =
          PeekNamedPipe(hStdOutRead, NULL, 0, NULL, &bytesAvail, &bytesLeft);
      if (ok && bytesAvail != 0) {
        ok =
            ReadFile(hStdOutRead, buffer, sizeof(buffer) - 3, &bytesRead, NULL);
        if (ok && bytesRead > 0) {
          buffer[bytesRead] = '\0';
          if (outputFn != NULL) {
            int length = 0;
  ...
  // this assumes that windows duplicates the command line string into the
  // created process's environment space.
  if (cmdLine != NULL) {
    Mem_Free(cmdLine);
  }
...
================
*/
void Sys_ClearEvents() { eventHead = eventTail = 0; }
  ...
  if (win32.osversion.dwPlatformId == VER_PLATFORM_WIN32_NT) {
    if (win32.osversion.dwMajorVersion <= 4) {
      ...
      win32.sys_arch.SetString("Win7");
               ...
               win32.osversion.dwMinorVersion == 2) {
      win32.sys_arch.SetString("Win8");
    } else if (win32.osversion.dwMajorVersion == 6 &&
               win32.osversion.dwMinorVersion == 3) {
...
================
*/
cpuid_t Sys_GetProcessorId() { return win32.cpuid; }
...
================
*/
const char* Sys_GetProcessorString() { return win32.sys_cpustring.GetString(); }
...
void EmailCrashReport(LPSTR messageText) {
  ...
  if (mapi) {
    LPMAPISENDMAIL MAPISendMail =
        (LPMAPISENDMAIL)GetProcAddress(mapi, "MAPISendMail");
    if (MAPISendMail) {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  // DG: tell Windows 8+ we're high dpi aware, otherwise display scaling screws
  // up the game
  Sys_SetDPIAwareness();
...
void Sys_SetLanguageFromSystem() { sys_lang.SetString(Sys_DefaultLanguage()); }
