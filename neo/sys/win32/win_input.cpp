...
int Sys_ReturnKeyboardInputEvent(const int n, int& ch, bool& state) {
  ...
  if (ch == K_PRINTSCREEN || ch == K_LCTRL || ch == K_LALT || ch == K_RCTRL ||
      ch == K_RALT) {
...
void JoystickSamplingThread(void* data) {
  ...
  const uint64 waitTime =
      5000000;  // poll every 5 seconds to see if a controller was connected
  while (1) {
...

    {
      ...
      for (int i = 0; i < MAX_JOYSTICKS; i++) {
        if (now >= nextCheck[i]) {
          ...
          // Checking input state infrequently for newly connected devices
          // prevents severe slowdowns on PC, especially on WinXP64.
          if (validData[i]) {
            nextCheck[i] = 0;
          } else {
      ...
      for (int i = 0; i < MAX_JOYSTICKS; i++) {
        ...
        // Switch from using cs->current to current to reduce chance of
        // Load-Hit-Store on consoles
...
bool idJoystickWin32::Init() {
  ...
  Sys_CreateThread((xthread_t)JoystickSamplingThread, NULL, THREAD_HIGHEST,
                   "Joystick", CORE_1A);
...
========================
*/
void idJoystickWin32::SetRumble(int inputDeviceNum, int rumbleLow,
                                int rumbleHigh) {
...
========================
*/
void idJoystickWin32::PostInputEvent(int inputDeviceNum, int event, int value,
                                     int range) {
...
int idJoystickWin32::PollInputEvents(int inputDeviceNum) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if ((xis.Gamepad.wButtons & mask) != (old.Gamepad.wButtons & mask)) {
      PostInputEvent(inputDeviceNum, joyRemap[i],
                     (xis.Gamepad.wButtons & mask) > 0);
    }
  }

  // Check the triggers
  if (xis.Gamepad.bLeftTrigger != old.Gamepad.bLeftTrigger) {
    PostInputEvent(inputDeviceNum, J_AXIS_LEFT_TRIG,
                   xis.Gamepad.bLeftTrigger * 128);
  }
  if (xis.Gamepad.bRightTrigger != old.Gamepad.bRightTrigger) {
    PostInputEvent(inputDeviceNum, J_AXIS_RIGHT_TRIG,
                   xis.Gamepad.bRightTrigger * 128);
  }
