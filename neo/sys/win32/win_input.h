...
struct controllerState_t {
  ...
  // complete missing of a button press that went down and up between two
  // usercmd_t creations, although it can add sn extra frame of latency to
  // sensing a release.
  int buttonBits;
...
class idJoystickWin32 : idJoystick {
 ...
 protected:
  ...
  idSysMutex
      mutexXis;  // lock this before using currentXis or stickIntegrations
  ...
  bool buttonStates[MAX_INPUT_DEVICES]
                   [K_LAST_KEY];  // For keeping track of button up/down events
  int joyAxis[MAX_INPUT_DEVICES]
             [MAX_JOYSTICK_AXIS];  // For keeping track of joystick axises
};
