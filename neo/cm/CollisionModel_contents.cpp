  ...
  for (i = 0; i < p->numEdges; i++) {
    ...
    for (j = 0; j < tw->numPolys; j++) {
      ...
      for (k = 0; k < tw->polys[j].numEdges; k++) {
        ...
        if (INT32_SIGNBITSET(trmEdgeNum) ^ ((edge->side >> bitNum) & 1) ^
            flip) {
          break;
        }
#else
...
int idCollisionModelManagerLocal::TransformedPointContents(
    const idVec3& p, cmHandle_t model, const idVec3& origin,
...
int idCollisionModelManagerLocal::ContentsTrm(
    trace_t* results, const idVec3& start, const idTraceModel* trm,
