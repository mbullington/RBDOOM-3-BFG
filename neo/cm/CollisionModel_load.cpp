...
idCVar preLoad_Collision("preLoad_Collision", "1", CVAR_SYSTEM | CVAR_BOOL,
                         "preload collision beginlevelload");
...
void idCollisionModelManagerLocal::ParseProcNodes(idLexer* src) {
  ...
  procNodes = (cm_procNode_t*)Mem_ClearedAlloc(
      numProcNodes * sizeof(cm_procNode_t), TAG_COLLISION);
...
cm_node_t* idCollisionModelManagerLocal::AllocNode(cm_model_t* model,
  ...
  if (!model->nodeBlocks || !model->nodeBlocks->nextNode) {
    nodeBlock = (cm_nodeBlock_t*)Mem_ClearedAlloc(
        sizeof(cm_nodeBlock_t) + blockSize * sizeof(cm_node_t), TAG_COLLISION);
    nodeBlock->nextNode =
...
cm_polygonRef_t* idCollisionModelManagerLocal::AllocPolygonReference(
  ...
  if (!model->polygonRefBlocks || !model->polygonRefBlocks->nextRef) {
    prefBlock = (cm_polygonRefBlock_t*)Mem_ClearedAlloc(
...
cm_brushRef_t* idCollisionModelManagerLocal::AllocBrushReference(
  ...
  if (!model->brushRefBlocks || !model->brushRefBlocks->nextRef) {
    brefBlock = (cm_brushRefBlock_t*)Mem_ClearedAlloc(
...
cmHandle_t idCollisionModelManagerLocal::SetupTrmModel(
    const idTraceModel& trm, const idMaterial* material) {
  int i, j;
  cm_vertex_t* vertex;
  cm_edge_t* edge;
  cm_polygon_t* poly;
...
void idCollisionModelManagerLocal::ChopWindingListWithBrush(
    cm_windingList_t* list, cm_brush_t* b) {
...
void idCollisionModelManagerLocal::R_ChopWindingListWithTreeBrushes(
    cm_windingList_t* list, cm_node_t* node) {
  int i;
  cm_brushRef_t* bref;
  cm_brush_t* b;
...
idFixedWinding* idCollisionModelManagerLocal::WindingOutsideBrushes(
    idFixedWinding* w, const idPlane& plane, int contents, int primitiveNum,
    cm_node_t* headNode) {
  int i, windingLeft;
    ...
    GetEdge(model,
            model
                ->vertices[model->edges[abs(edgeNum1)]
                               .vertexNum[INT32_SIGNBITSET(edgeNum1)]]
                .p,
            model
                ->vertices[model->edges[abs(edgeNum2)]
                               .vertexNum[INT32_SIGNBITNOTSET(edgeNum2)]]
                .p,
            &newEdgeNum1, -1);
    if (newEdgeNum1 == 0) {
      keep1 = true;
...
bool idCollisionModelManagerLocal::MergePolygonWithTreePolygons(
    cm_model_t* model, cm_node_t* node, cm_polygon_t* polygon) {
  int i;
  cm_polygonRef_t* pref;
  cm_polygon_t *p, *newp;
...
void idCollisionModelManagerLocal::FindInternalEdgesOnPolygon(
    cm_model_t* model, cm_polygon_t* p1, cm_polygon_t* p2) {
  int i, j, k, edgeNum;
  cm_edge_t* edge;
...
================
*/
static int CM_FindSplitter(const cm_node_t* node, const idBounds& bounds,
...
void idCollisionModelManagerLocal::R_FilterPolygonIntoTree(
    cm_model_t* model, cm_node_t* node, cm_polygonRef_t* pref,
    cm_polygon_t* p) {
  assert(node != NULL);
...
cm_node_t* idCollisionModelManagerLocal::R_CreateAxialBSPTree(
    cm_model_t* model, cm_node_t* node, const idBounds& bounds) {
  int planeType;
  float planeDist;
...
================
*/
int idCollisionModelManagerLocal::GetVertex(cm_model_t* model, const idVec3& v,
                                            int* vertexNum) {
  int i, hashKey, vn;
  idVec3 vert, *p;
...
================
*/
int idCollisionModelManagerLocal::GetEdge(cm_model_t* model, const idVec3& v1,
                                          const idVec3& v2, int* edgeNum,
                                          int v1num) {
  int v2num, hashKey, e;
  int found, *vertexNum;
...
void idCollisionModelManagerLocal::PolygonFromWinding(
    cm_model_t* model, idFixedWinding* w, const idPlane& plane,
...
void idCollisionModelManagerLocal::CreatePatchPolygons(
    cm_model_t* model, idSurface_Patch& mesh, const idMaterial* material,
    int primitiveNum) {
  int i, j;
  float dot;
  int v1, v2, v3, v4;
...
=================
*/
static void CM_EstimateVertsAndEdges(const idMapEntity* mapEnt, int* numVerts,
                                     int* numEdges) {
  int j, width, height;
...
void idCollisionModelManagerLocal::ConvertBrushSides(
    cm_model_t* model, const idMapBrush* mapBrush, int primitiveNum,
    const idVec3& originOffset) {
  int i, j;
...
// RB begin
void idCollisionModelManagerLocal::ConvertMesh(cm_model_t* model,
                                               const MapPolygonMesh* mesh,
                                               int primitiveNum) {
...
void idCollisionModelManagerLocal::OptimizeArrays(cm_model_t* model) {
  ...
  remap = (int*)Mem_ClearedAlloc(
      Max(model->numVertices, model->numEdges) * sizeof(int), TAG_COLLISION);
  ...
  model->vertices = (cm_vertex_t*)Mem_ClearedAlloc(
      model->numVertices * sizeof(cm_vertex_t), TAG_COLLISION);
  if (oldVertices) {
    memcpy(model->vertices, oldVertices,
           model->numVertices * sizeof(cm_vertex_t));
    Mem_Free(oldVertices);
  ...
  model->edges = (cm_edge_t*)Mem_ClearedAlloc(
      model->numEdges * sizeof(cm_edge_t), TAG_COLLISION);
  if (oldEdges) {
    memcpy(model->edges, oldEdges, model->numEdges * sizeof(cm_edge_t));
    Mem_Free(oldEdges);
  }
...
static const unsigned int BCM_MAGIC =
    ('B' << 24) | ('C' << 16) | ('M' << 16) | BCM_VERSION;
...
================
*/
cm_model_t* idCollisionModelManagerLocal::LoadBinaryModelFromFile(
    idFile* file, ID_TIME_T sourceTimeStamp) {
  ...
  model->polygonBlock = (cm_polygonBlock_t*)Mem_ClearedAlloc(
      sizeof(cm_polygonBlock_t) + model->polygonMemory, TAG_COLLISION);
  ...
  model->brushBlock = (cm_brushBlock_t*)Mem_ClearedAlloc(
      sizeof(cm_brushBlock_t) + model->brushMemory, TAG_COLLISION);
  ...
  struct local {
    static void ReadNodeTree(idFile* file, cm_model_t* model, cm_node_t* node,
                             idList<cm_polygon_t*>& polys,
                             idList<cm_brush_t*>& brushes) {
      ...
      while (file->ReadBig(i) == sizeof(i) && (i >= 0)) {
        cm_polygonRef_t* pref =
            collisionModelManagerLocal.AllocPolygonReference(
                model, model->numPolygonRefs);
        pref->p = polys[i];
        pref->next = node->polygons;
        node->polygons = pref;
      }
      while (file->ReadBig(i) == sizeof(i) && (i >= 0)) {
        cm_brushRef_t* bref = collisionModelManagerLocal.AllocBrushReference(
            model, model->numBrushRefs);
        bref->b = brushes[i];
        bref->next = node->brushes;
        node->brushes = bref;
      }
  ...
  // We should have only allocated a single block, and used every entry in the
  // block assert( model->nodeBlocks != NULL && model->nodeBlocks->next == NULL
  // && model->nodeBlocks->nextNode == NULL );
  assert(model->brushRefBlocks == NULL ||
         (model->brushRefBlocks->next == NULL &&
          model->brushRefBlocks->nextRef == NULL));
  assert(model->polygonRefBlocks == NULL ||
         (model->polygonRefBlocks->next == NULL &&
          model->polygonRefBlocks->nextRef == NULL));
...
================
*/
cm_model_t* idCollisionModelManagerLocal::LoadBinaryModel(
    const char* fileName, ID_TIME_T sourceTimeStamp) {
...
================
*/
void idCollisionModelManagerLocal::WriteBinaryModelToFile(
    cm_model_t* model, idFile* file, ID_TIME_T sourceTimeStamp) {
  ...
  struct local {
    static void BuildUniqueLists(cm_node_t* node, idList<cm_polygon_t*>& polys,
                                 idList<cm_brush_t*>& brushes) {
    ...
    static void WriteNodeTree(idFile* file, cm_node_t* node,
                              idList<cm_polygon_t*>& polys,
                              idList<cm_brush_t*>& brushes) {
...
================
*/
void idCollisionModelManagerLocal::WriteBinaryModel(cm_model_t* model,
                                                    const char* fileName,
...
cm_model_t* idCollisionModelManagerLocal::LoadRenderModel(
  ...
  if ((extension.Icmp("ase") != 0) && (extension.Icmp("lwo") != 0) &&
      (extension.Icmp("ma") != 0) && (extension.Icmp("dae") != 0) &&
...
void idCollisionModelManagerLocal::Preload(const char* mapName) {
  ...
  if (manifest.NumResources() >= 0) {
    ...
    common->Printf(
        "%05d collision models preloaded ( or were already loaded ) in %5.1f "
        "seconds\n",
...
===================
*/
bool idCollisionModelManagerLocal::GetModelEdge(cmHandle_t model, int edgeNum,
                                                idVec3& start,
                                                idVec3& end) const {
  if (model < 0 || model > MAX_SUBMODELS || model >= numModels ||
      !models[model]) {
...
bool idCollisionModelManagerLocal::GetModelPolygon(
    cmHandle_t model, int polygonNum, idFixedWinding& winding) const {
  int i, edgeNum;
  cm_polygon_t* poly;
  ...
  for (i = 0; i < poly->numEdges; i++) {
    ...
    winding += models[model]
                   ->vertices[models[model]
                                  ->edges[abs(edgeNum)]
                                  .vertexNum[INT32_SIGNBITSET(edgeNum)]]
                   .p;
  }
...
cmHandle_t idCollisionModelManagerLocal::LoadModel(const char* modelName) {
  ...
  if (models == NULL) {
    ...
    maxModels = MAX_SUBMODELS;
    numModels = 0;
    models = (cm_model_t**)Mem_ClearedAlloc(
        (maxModels + 1) * sizeof(cm_model_t*), TAG_COLLISION);
  }
  ...
  if (models[numModels] != NULL) {
    ...
    if (cvarSystem->GetCVarBool("fs_buildresources")) {
      // for resource gathering write this model to the preload file for this
      // map
