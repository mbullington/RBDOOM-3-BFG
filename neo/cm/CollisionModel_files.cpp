...
void idCollisionModelManagerLocal::WriteCollisionModelsToFile(
    const char* filename, int firstModel, int lastModel,
...
void idCollisionModelManagerLocal::ParsePolygons(idLexer* src,
  ...
  if (src->CheckTokenType(TT_NUMBER, 0, &token)) {
    model->polygonBlock = (cm_polygonBlock_t*)Mem_ClearedAlloc(
        sizeof(cm_polygonBlock_t) + token.GetIntValue(), TAG_COLLISION);
...
void idCollisionModelManagerLocal::ParseBrushes(idLexer* src,
  ...
  if (src->CheckTokenType(TT_NUMBER, 0, &token)) {
    model->brushBlock = (cm_brushBlock_t*)Mem_ClearedAlloc(
        sizeof(cm_brushBlock_t) + token.GetIntValue(), TAG_COLLISION);
...
bool idCollisionModelManagerLocal::LoadCollisionModelFile(
    const char* name, unsigned int mapFileCRC) {
  idToken token;
  idLexer* src;
  unsigned int crc;
  ...
  if (file != NULL) {
    ...
    if (fileID == CM_FILEID && fileVersion == CM_FILEVERSION &&
        crc == mapFileCRC && numEntries > 0) {
      ...
      for (int i = 0; i < numEntries; i++) {
        ...
        //     (otherwise we'll get a segfault when someone wants to use
        //     models[numModels])
        if (model == NULL) {
          loaded = false;
          break;
