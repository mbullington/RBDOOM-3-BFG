...
class idCollisionModelManager {
 public:
  ...
  virtual bool GetModelVertex(cmHandle_t model, int vertexNum,
                              idVec3& vertex) const = 0;
  // Gets an edge of a model.
  virtual bool GetModelEdge(cmHandle_t model, int edgeNum, idVec3& start,
                            idVec3& end) const = 0;
  // Gets a polygon of a model.
  virtual bool GetModelPolygon(cmHandle_t model, int polygonNum,
                               idFixedWinding& winding) const = 0;

  // Translates a trace model and reports the first collision if any.
  virtual void Translation(trace_t* results, const idVec3& start,
                           const idVec3& end, const idTraceModel* trm,
  ...
  virtual void Rotation(trace_t* results, const idVec3& start,
  ...
  // in free space.
  virtual int Contents(const idVec3& start, const idTraceModel* trm,
                       const idMat3& trmAxis, int contentMask, cmHandle_t model,
                       const idVec3& modelOrigin, const idMat3& modelAxis) = 0;
  ...
  // Draws a model.
  virtual void DrawModel(cmHandle_t model, const idVec3& modelOrigin,
  ...
  virtual bool WriteCollisionModelForMapEntity(
      const idMapEntity* mapEnt, const char* filename,
