...
int idAASFileLocal::BoundsReachableAreaNum_r(
    int nodeNum, const idBounds& bounds, const int areaFlags,
    const int excludeTravelFlags) const {
  int res;
  const aasNode_t* node;
...
void idAASFileLocal::PushPointIntoAreaNum(int areaNum, idVec3& point) const {
  ...
  for (i = 0; i < area->numFaces; i++) {
    ...
    const idPlane& plane =
        planeList[face->planeNum ^ INT32_SIGNBITSET(faceNum)];
...
bool idAASFileLocal::Trace(aasTrace_t& trace, const idVec3& start,
...
============
*/
void idAASFileLocal::MaxTreeDepth_r(int nodeNum, int& depth,
                                    int& maxDepth) const {
  const aasNode_t* node;
