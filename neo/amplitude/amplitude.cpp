...
enum errorCodes_t { E_OK = 0, E_ARGS, E_OPEN_IN, E_OPEN_OUT, E_PROCESSING };
...
int WAVE_ReadChunks(FILE* f, unsigned int fileSize, chunk_t* chunks,
                    int maxChunks) {
...
bool Process(FILE* in, FILE* out) {
  ...
  unsigned short expectedSampleSize =
...
int main(int argc, char* argv[]) {
  ...
  if (strcpy_s(dot, sizeof(outputFileName) - (dot - outputFileName), ".amp") !=
      0) {
    printf("Filename too long\n");
    return E_ARGS;
  }
